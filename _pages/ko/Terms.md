...
<div class="term-index">
  ...
  <section class="term-index__section" id="idx-{{ pair[0] }}">
    ...
    <ul class="term-index__list">
      {%- for t in pair[1] %}
      <li class="term-index__entry{% unless t.defs or t.refs %} term-index__entry--bare{% endunless %}" id="{{ t.id }}" data-search="{{ t.en | remove: '$' | downcase | escape }} {{ t.ko | remove: '$' | escape }}{% if t.alias %} {{ t.alias | join: ' ' | remove: '$' | escape }}{% endif %}{% if t.alias_en %} {{ t.alias_en | join: ' ' | remove: '$' | downcase | escape }}{% endif %}">
