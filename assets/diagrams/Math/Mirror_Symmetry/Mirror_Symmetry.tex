...
%% 예시11: Gr(2,5) σ_3⋆σ_(2)=σ_(3,2)+q; λ=(2) 회색 + horizontal strip 빨강, 초과분이 q (Quantum_Pieri_Rule-4.png)
\ytableausetup{boxsize=1.6em, centertableaux}
\begin{tikzpicture}
\node (eq) at (0,0) {$\sigma_3 \star \sigma_{(2)} \;=$};
\node (a) at (2.7,0) {\begin{ytableau} *(gray!45) & *(gray!45) & *(red!75) \\ *(red!75) & *(red!75) \end{ytableau}};
\node at (4.3,0) {$+\; q$};
\node at (2.7,-1.4) {\scriptsize $\sigma_{(3,2)}$};
\end{tikzpicture}
...
%% rim hook: Gr(2,4)에서 직사각형(2x2) 밖 (3,1)의 4-rim hook 제거 → ∅, +q (Quantum_Pieri_Rule-5.png)
\begin{tikzpicture}[scale=0.5]
  \begin{scope}[shift={(0,0)}]
