...
으로 정의하자. 그럼 $f:U\rightarrow\Gamma(f)$와, 앞의 $m$개의 좌표로의 projection $\pr:\Gamma(f)\rightarrow U$가 모두 연속이고, 서로의 역함수가 되므로 $\Gamma(f)$와 $U$는 homeomorphic이다. 따라서 $\Gamma(f)$는 topological manifold이며, 특히 projection $\pr:\Gamma(f)\rightarrow U$이 하나의 chart로 이루어진 atlas $\\{(\Gamma(f),\pr)\\}$를 이루므로 자연스러운 (differentiable) manifold 구조 또한 갖는다. 
