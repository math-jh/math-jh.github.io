...
우선 교환법칙을 만족하진 않지만 결합법칙을 만족하는 마그마의 예시가 존재한다. 임의의 집합 $E$에 대하여, $E$에서 $E$로의 함수들의 모임 $\Fun(E,E)$를 생각하자. 임의의 두 함수 $f,g\in\Fun(E,E)$에 대하여, 여기에서의 연산을 함수의 합성 $(f,g)\mapsto f\circ g$로 준다면 이는 결합법칙을 만족하지만 교환법칙은 만족하지 않는다.
...
대수학에서는 $f$의 image를 $f(A)$ 대신 $\imf$와 같이 적는 것이 보통이다. $\imf$는 자명하게 $A'$의 부분집합이다. 그런데, $A'$는 단순히 부분집합일 뿐만 아니라, $\imf$ 그 자체로도 마그마가 된다. 즉, $\imf$는 $\star'$에 대해 닫혀있다.  

임의의 $w,z\in\imf$를 택하자. 그럼 어떤 $x,y\in A$가 존재하여 $w=f(x)$이고, $z=f(y)$이다. 이제

$$w\star'z=f(x)\star'f(y)=f(x\star y)$$

이고, $x\star y\in A$이므로, $w\star'z\in\imf$가 성립한다.
...
당연하게 왼쪽 역원의 존재성과 오른쪽 역원의 존재성은 서로 관련이 없다. 무한집합 $E$에 대하여, $E$에서 $E$로의 함수들의 모임 $\Fun(E,E)$를 생각하면 이 집합은 함수의 합성을 연산으로, $\id\_E$를 항등원으로 갖는 모노이드가 되지만 이 집합에는 왼쪽 가역이지만 오른쪽 가역은 아닌 원소와, 오른쪽 가역이지만 왼쪽 가역은 아닌 원소가 모두 존재한다. ([Set Theory, §함수, 정의 10](/ko/math/set_theory/functions#df10))
