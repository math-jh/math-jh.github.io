...
임의의 정수 $k$에 대하여, $k=pn+q$를 만족하는 정수 $p$와 정수 $0\leq q&lt;n$이 존재한다. $r^n=\operatorname{id}$이므로, $r^k=r^q$이고 따라서 $r$의 거듭제곱들은 모두 위의 나열된 $n$개의 변환 중 하나와 같다. 또, 만일 $0\leq k_1,k_2&lt;n$에 대하여 $r^{k_1}=r^{k_2}$라면, $r^{k_1-k_2}=\operatorname{id}$이고 $0\leq\left\lvert k_1-k_2\right\rvert&lt;n$이므로 $k_1-k_2=0$이어야 한다. 즉, 위에 열거된 $n$개의 변환들은 모두 서로 다르다.
...
우선 $s^2=\operatorname{id}$이다. 따라서 만일 어떤 $0\leq m,n&lt;n$에 대해 $sr^m=sr^n$이라면, $s^2r^m=s^2r^n$이고 따라서 $r^m=r^n$이다. 그럼 앞선 명제에 의하여 $m=n$이어야 하므로, $s$, $sr$, $\ldots$, $sr^{n-1}$은 서로 다른 원소들이다. 한편, 임의의 $sr^m$에 대하여 어떤 $r^k$가 존재하여 $sr^m=r^k$라 하자. 그럼 점 $\left(\cos\frac{-2\pi m}{n},\sin\frac{-2\pi m}{n}\right)$가 두 변환에 의해 옮겨지는 점 또한 같아야 하므로,
...
$sr$은 reflection이므로 $(sr)^2=srsr=\operatorname{id}$이 성립한다. 또, $s$도 reflection이므로 $s^2=\operatorname{id}$이고 $s=s^{-1}$이다. 이제
...
으로 쓸 수 있다. 만일 $\beta=1$이라면, $s^{\beta+1}=s^2=\operatorname{id}$이므로 더 이상 증명할 것이 없다. 만일 $\beta=0$이라면, 우리는 $rs=sr^{-1}$로부터 

$$r^{\alpha}sr^{a_{k+2}}=r^{\alpha}r^{-a_{k+2}}s=r^{\alpha-a_{k+2}}s.$$

임을 보일 수 있다. (이는 $a_{k+2}$에 대한 귀납법을 이용하면 된다.) 이제 $r^n=\operatorname{id}$이므로, 우변의 왼쪽에다 $r^{kn}$을 적절히 곱하면 원하는 결과를 얻는다.
...
$$\langle r,s\mathrel{|}r^n=1,s^2=\operatorname{id},rs=sr^{-1}\rangle$$
...
으로 바꾸어 쓸 수 있다. 또, $\sigma$는 bijection이므로, 이렇게 모든 term을 바꾸어 써 주면 $\sigma(\Delta)$는 부호만 제외하면 $\Delta$와 정확하게 같은 polynomial이 됨을 알 수 있다. 따라서 $\operatorname{sgn}(\sigma)=\sigma(\Delta)/\Delta$로 정의하면, $\operatorname{sgn}(\sigma)$는 정확하게 [정의 11](#df11)의 sense에서 정의된 $\sigma$의 parity가 된다.   

정의로부터, $\sigma$가 transposition이라면 $\operatorname{sgn}(\sigma)=-1$인 것은 자명하다. 따라서 만일 $\operatorname{sgn}$이 multiplicative하다는 것만 보인다면, 

$$\text{$\sigma$ odd}\iff\text{$\operatorname{sgn}(\sigma)=-1$}\iff\text{$\sigma$ is a product of odd number of permutation}$$ 

이 되므로 원하는 결과를 얻을 것이다. 두 permutation $\sigma$, $\tau$가 주어졌다 하자. $\operatorname{sgn}(\sigma\tau)$의 값을 계산해야 한다. $\tau$가 $k$개의 inversion을 갖는다 가정하자. 즉, $\operatorname{sgn}(\tau)=(-1)^k$이고, 이 $k$개의 $-1$들은 모두 식 (1)에서 나오는 factor들이다. 이제 $\operatorname{sgn}(\sigma\tau)$의 값을 계산하기 위해서는 우선

$$\prod_{1\leq i<j\leq n}(x_{\tau(i)}-x_{\tau(j)})=(-1)^k\Delta$$

의 양 변에 $(-1)^k$를 곱하여, 좌변의 각 term들을 원래대로 되돌려놓고 (즉, 이 상황에서 좌변은 곱셈 순서만 바뀐 $\Delta$이다), 이 원래대로 되돌아간 polynomial에 $\sigma$를 적용시켜주면 된다. 따라서, 만일 $\sigma$의 inversion의 갯수가 $l$개였다면, $\Delta$ 앞에 붙는 factor는 $(-1)^{k+l}$이었을 것이고, 이것이 곧 $\operatorname{sgn}$의 multiplicativity를 보장한다.
