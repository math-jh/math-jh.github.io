...
from terms_common import (  # noqa: E402
    ...
    join_file, letter_of, permalink_map, semantic_checks, split_file, url_slug,
    yaml_quote,
)
...
def run(path: Path, fix: bool, notify: bool) -> int:
    ...
    issues = semantic_checks(data, pmap, ko_by_slug)
    ...
    if fix and any(i.fixable for i in issues):
        ...
        if new != text:
            ...
            tmp.replace(path)
            log(f"자동 수정: 라벨 {n_lbl}건, 그룹 이동 {n_grp}건 (백업: {BACKUP_PATH.name})")
            issues = semantic_checks(nd, pmap, ko_by_slug)
