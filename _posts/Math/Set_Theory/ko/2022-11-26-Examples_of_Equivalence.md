...
동치관계 $(R,A,A)$와 이로부터 유도된 $p:A\rightarrow A/R$에 대하여, 동치관계 $R$은 [정의 2](#def2){: data-lid="6xbs8" data-relation="required" }를 $p$에 적용하여 얻은 동치관계와 정확하게 같다는 것을 확인할 수 있다.
...
이번에는 canonical projection $p:A\rightarrow A/R$와 $X\subseteq A$를 생각하자. [§이항관계들 사이의 연산, ⁋명제 7](/ko/math/set_theory/operation_of_binary_relations#prop7){: data-lid="vuhuz" data-relation="required" }에 의하여 
...
즉, $f$가 $R$과 compatible하려면 $f$는 각각의 equivalence class로 제한하였을 때 상수함수가 되어야 한다. 이제 [§Retraction과 section, ⁋명제 4](/ko/math/set_theory/retraction_and_section#prop4){: data-lid="sb26q" data-relation="required" }를 적용하면 다음을 얻는다.
...
특별히 $R$이 $f$에 의해 정의된 동치관계라 하자. ([정의 2](#def2){: data-lid="mo9f0" data-relation="required" }) 그럼 다음의 diagram을 생각할 수 있다.
...
만일 $q\circ f$가 $R$과 compatible하다면, $f$가 *$(R,S)$-compatible*하다고 한다. [명제 7](#prop7){: data-lid="t2li2" data-relation="required" }에 의해 이는 다시 $h:A/R\rightarrow B/S$가 존재하여 $h\circ p=q\circ f$인 것과 동치이다. 
...
그럼 자연스레 함수 $p\circ f:A\rightarrow B/S$가 정의되며, 이 함수가 [정의 2](#def2){: data-lid="nzq4s" data-relation="required" }를 통해 만드는 동치관계를 $f$에 의한 $S$의 *preimage*라 부른다.

## 동치관계의 quotient

다음 정의는 이미 [§동치관계, ⁋예시 5](/ko/math/set_theory/equivalence_relations#ex5){: data-lid="hgmn7" data-relation="weak" }에서 언급했던 것이다.
