...
중 하나가 성립한다. 이 경우엔 $x\leq y$의 부정이 $x > y$가 된다. 하지만 totally ordered set이라는 조건이 빠진 상태에서 이는 일반적으로 성립하지 않는다. ([§순서관계의 정의, ⁋참고 11](/ko/math/set_theory/order_relations#rmk11){: data-relation="weak" })
...
위의 명제 또한 일반적인 ordered set에서는 성립하지 않았었다. ([§단조함수, ⁋참고 6](/ko/math/set_theory/monotone_functions#rmk6){: data-relation="weak" })
