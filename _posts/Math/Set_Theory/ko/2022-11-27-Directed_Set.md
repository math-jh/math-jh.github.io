...
::: 명제 2
 Ordered set $A$가 right directed라면 $A$의 maximal element는 greatest element이기도 하다.
:::
...
::: 명제 3
$(A_i)$가 right directed set들의 family라면 $\prod A_i$ 또한 right directed이다.
:::
::: 증명
...
중 하나가 성립한다. 이 경우엔 $x\leq y$의 부정이 $x > y$가 된다. 하지만 totally ordered set이라는 조건이 빠진 상태에서 이는 일반적으로 성립하지 않는다. ([§순서관계의 정의, ⁋참고 11](/ko/math/set_theory/order_relations#rmk11){: data-lid="mlqdv" data-relation="weak" })

::: 명제 6
Totally ordered set $A$에서 ordered set $B$로의 모든 순단조함수 $f$는 단사함수다. 만약 $f$가 순증가라면, $f$는 $A$에서 $f(A)$로의 isomorphism이다.
:::
::: 증명
...
위의 명제 또한 일반적인 ordered set에서는 성립하지 않았었다. ([§단조함수, ⁋참고 6](/ko/math/set_theory/monotone_functions#rmk6){: data-lid="5iy57" data-relation="weak" })

::: 명제 7
$A$가 totally ordered set이고 $X$가 그 부분집합이라 하자. 그럼 $b\in A$가 $X$의 supremum인 것은 $b$가 $X$의 upper bound이고, $c < b$를 만족하는 임의의 $c\in A$에 대하여 $x\in X$이 존재하여 $c < x\leq b$인 것과 동치이다.
:::
...
::: 명제 8
Lattice에서 두 interval의 교집합도 interval이다.
:::
