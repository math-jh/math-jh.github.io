---
title: "순서쌍"
description: "집합의 포함관계를 이항관계로 살펴보고, 순서쌍을 집합으로 정의한 뒤 그 존재성과 유일성을 증명한다."
...
위 두 명제로부터 $\subseteq$가 집합들 사이의 order relation이 된다는 것을 안다. ([§순서관계의 정의, ⁋정의 2](/ko/math/set_theory/order_relations#def2){: data-lid="kqyue" data-relation="forward" })
...
::: 보조정리 5
두 집합 $x$, $y$에 대하여, 집합 $(x,y)$는 항상 존재하고 유일하다.  
:::
::: 증명
...
::: 정의 7
위의 과정으로 얻어진 두 집합 $\{x\}$, $\{y\}$에 대하여, $\{x\}$의 유일한 원소 $x$를 $z=(x,y)$의 *첫 번째 성분*, $\{y\}$의 유일한 원소 $y$를 $z=(x,y)$의 *두 번째 성분*이라 부르며, 이를 각각 
...
또, [정의 7](#def7){: data-lid="1888f" data-relation="weak" }과 유사하게 집합 $A$와 $B$를 $A\times B$의 첫 번째와 두 번째 성분이라 부른다.
:::
...
[^1]: 물론 이 "집합"은 집합이 아니다. ([§ZFC 공리계, ⁋예시 4](/ko/math/set_theory/zfc_axioms#ex4){: data-lid="p57o8" data-relation="weak" })
