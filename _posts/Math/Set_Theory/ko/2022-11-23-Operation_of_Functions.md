...
이제 역으로 $f^{-1}$이 함수라 하자. 그럼 정의에 의해 $\pr_1 f^{-1}=B$이다. 그런데 [§이항관계들 사이의 연산, ⁋명제 8](/ko/math/set_theory/operation_of_binary_relations#prop8){: data-lid="c7ul5" }의 첫 번째 식에 $R_2=\id_A$, $R_1=f^{-1}$을 넣으면 $\pr_1f^{-1}=f(A)$이므로, $B=f(A)$이고 따라서 $f$는 전사함수다. 
