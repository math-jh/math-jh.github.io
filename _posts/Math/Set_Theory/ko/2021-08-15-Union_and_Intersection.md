...
[§ZFC 공리계](/ko/math/set_theory/zfc_axioms){: data-lid="lumof" data-relation="weak" }에서 우리는 합집합이 존재한다는 것을 공리로 받아들였다. 이 공리를 도입했을 때 사용한 표기법보다는 다음의 표기법이 좀 더 자주 쓰인다.
...
를 만족하는 $x$들의 모임이다. 만일 $I=\emptyset$이라면 $i\in I$가 거짓이 되어 전체 문장이 $x$에 관계없이 참이 되고 $\bigcap_{i\in\emptyset} A_i$는 전체집합이 되어야 하므로 모순이다. ([§ZFC 공리계, ⁋예시 4](/ko/math/set_theory/zfc_axioms#ex4){: data-lid="qcd9n" data-relation="required" }) $(A_i)_{i\in I}$의 target $\mathcal{S}$를 잘 정해준다면 이러한 모순을 피해 교집합을 정의할 수 있다.
...
::: 명제 7
 $f:A\rightarrow B$가 함수라 하고 $(B_i)_{i\in I}$가 $B$의 부분집합들의 family라 하자. 그럼 
