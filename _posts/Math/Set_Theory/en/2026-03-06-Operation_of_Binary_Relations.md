...
::: Definition 1
Let $R$ be a binary relation. Then the binary relation consisting of all $(y,x)$ such that $(x,y)\in R$ is called the *inverse* of $R$, and is denoted by $R^{-1}$. Also, the set $R^{-1}(X)$ is called the *preimage* of $X$. If $R^{-1}=R$, then $R$ is said to be *symmetric*.
:::
...
::: Proposition 2
The inverse of $R^{-1}$ is $R$. Moreover, $\pr_1R^{-1}=\pr_2R$ and $\pr_2R^{-1}=\pr_1R$.
:::
::: Proof
...
::: Definition 3
Let $R_1$ and $R_2$ be binary relations. The *composition* $R_2\circ R_1$ of these two binary relations is the set of ordered pairs $(x,z)$ for which there exists a $y$ such that $(x,y)\in R_1$ and $(y,z)\in R_2$.
:::
...
::: Proposition 5
Composition of binary relations is associative. That is, for any three binary relations $R_1,R_2,R_3$, 
...
::: Proposition 6
Let $R_1$, $R_2$ be binary relations and let $A$ be a set. Then
...
::: Proposition 7
Let $(R,A,B)$ be a binary relation, and let $X\subseteq A$, $Y\subseteq B$. Then 
...
:::
::: Proof
Before beginning the proof proper, we observe that since the two formulas above must hold for <em>all</em> $R$, they must also hold when $R^{-1}$ is substituted for $R$. Therefore, once we prove 1, then 2 follows immediately from [Proposition 2](#prop2){: data-lid="0ektm" data-relation="required" }.  
...
::: Definition 9
For a set $A$, $\Delta_A$ denotes the binary relation
