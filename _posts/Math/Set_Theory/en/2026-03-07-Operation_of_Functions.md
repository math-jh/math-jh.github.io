...
Conversely, suppose $f^{-1}$ is a function. Then by definition, $\pr_1 f^{-1}=B$. Substituting $R_2=\id_A$ and $R_1=f^{-1}$ into the first equation of [§Operations on Binary Relations, ⁋Proposition 8](/en/math/set_theory/operation_of_binary_relations#prop8){: data-lid="c7ul5" data-relation="required" } yields $\pr_1f^{-1}=f(A)$, so $B=f(A)$, and therefore $f$ is surjective.
