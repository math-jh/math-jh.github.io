...
Define the set $\mathcal{S}$ to be the collection of all $x$ satisfying $x\not\in x$. Then $\mathcal{S}$ is either an element of itself or not.
...
::: misc The Axiom of Existence. {#axiom-existence}
...
::: Proposition 2
There exists at most one set with no elements.
:::
::: Proof
...
The following axiom is worth remembering in particular because it prevents [Example 1](#ex1){: data-lid="0o7xn" data-relation="required" }.
...
Now, according to the comprehension schema introduced above, unlike in [Example 1](#ex1){: data-lid="n6tdz" data-relation="weak" }, we cannot directly define $\mathcal{S}=\{x\mid x\not\in x\}$; we can only define
...
::: Example 5
For any sets $A$ and $B$, let the property $P$ concerning $x$ be given by $x\in B$. Then the set
...
::: Example 6
Now, given two sets $A$ and $B$, let the property $Q$ concerning $x$ be given by $x\not\in B$. Then the set
