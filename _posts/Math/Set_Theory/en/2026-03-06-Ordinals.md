...
The set of natural numbers $\mathbb{N}$ together with a *successor function* $S$ defined on it satisfies the following axioms:
...
Let a set $A$ be given. For an element $a\in A$ and an arbitrary function $g:A\times \mathbb{N}\rightarrow A$, there exists a unique infinite sequence $f:\mathbb{N}\rightarrow A$ satisfying the following conditions:
...
There exists a set $I$ that contains $0$ and is closed under the successor function.
:::
...
::: Example 3
$\mathbb{N}$ is a well-ordered set, but $\mathbb{R}$ is not.
...
::: Definition 4
Let $(A,\leq)$ be an ordered set. A subset $S$ of $A$ is called an *initial segment* of $A$ if it satisfies the following condition:
...
::: Proposition 5
Let $(A,\leq)$ be a well-ordered set. Every segment of $A$ other than $A$ itself is of the form $(-\infty, a)$ for some $a\in A$.
:::
::: Proof
...
::: remark Remark {#rmk}
The above proposition does not say that the set $(-\infty, a]$ cannot be a segment of $A$. Rather, it means that if $(-\infty, a]$ is a segment of $A$, then this set can also be written in the form $(-\infty, a')$ for some $a'\in A$.
