...
Von Neumann presented a specific model of the natural numbers $$\mathbb{N}$$ satisfying the Peo axioms above, which provides good motivation for defining ordinals, so we introduce it first. Von Neumann's idea is to define a natural number $$n$$ as <em-ko>a set with $$n$$ elements</em-ko>. Then $$0$$ is uniquely determined:
...
Now we only need to show that $$A\setminus S$$ is <em-ko>exactly</em-ko> $$[a,\infty)$$, and then $$S=(-\infty, a)$$, completing the proof. Assume to the contrary that some $$x\in [a,\infty)$$ is not an element of $$A\setminus S$$. That is, $$x$$ satisfies both $$a\leq x$$ and $$x\in S$$. But since $$S$$ is a segment, $$a\leq x$$ implies $$a\in S$$, which contradicts the assumption that $$a$$ is the least element of $$A\setminus S$$, so $$x\not\in S$$. Therefore, $$A\setminus S$$ is exactly $$[a,\infty)$$, and the proof is complete.
