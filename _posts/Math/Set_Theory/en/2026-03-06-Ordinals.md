...
weight: 19
translated_at: 2026-05-23T21:00:03+00:00
...
The mathematical induction we commonly use is in fact a proposition derived from the above axioms. In standard set theory textbooks, such as **[HJJ]**, which we used in defining the ZFC axioms and ordered pairs, operations on natural numbers are defined at this point using the following recursion theorem.

<div class="proposition" markdown="1">

<ins id="thm1">**Theorem 1 (The recursion theorem)**</ins> Let a set $$A$$ be given. For an element $$a\in A$$ and an arbitrary function $$g:A\times \mathbb{N}\rightarrow A$$, there exists a unique infinite sequence $$f:\mathbb{N}\rightarrow A$$ satisfying the following conditions:
...
Von Neumann presented a specific model of the natural numbers $$\mathbb{N}$$ satisfying the Peano axioms above, which serves as good motivation for defining ordinals, so we introduce it first. Von Neumann's idea is to define a natural number $$n$$ as *a set with $n$ elements*. Then $$0$$ is uniquely determined:
...
and define $$3=\{0,1,2\}$$ in the same manner. In this model, we can easily verify that $$S$$ is given by the function
...
Repeating this again, we obtain $$\omega\cdot 2, \omega\cdot 2+1, \ldots, \omega\cdot 3, \ldots,$$ and repeating once more, we even obtain $$\omega\cdot\omega=\omega^2$$, and repeating this yet again gives $$\omega^3, \ldots, \omega^\omega$$, then $$\omega^{\omega^{\omega}}, \ldots$$. Of course, when this process finishes and we assign a new symbol to this set, we can repeat the same process again.
...
<ins id="def2">**Definition 2**</ins> A total order $$R$$ on a set $$A$$ is a *well-ordering<sub>정렬순서</sub>* if every non-empty subset $$X$$ of $$A$$ has a least element.
...
However, a segment of the form $$(-\infty, a)$$ never contains $$a$$, so no segment of $$A$$ of this form can be $$A$$ itself.
