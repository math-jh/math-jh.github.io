...
We now construct natural numbers using the cardinals we have already defined, rather than following the approach of [§Ordinals and Well-Ordered Sets](/en/math/set_theory/ordinals){: data-lid="wvue3" }. We then explore the structure of natural numbers using the operations and order relations previously defined on cardinal numbers.
...
Since natural numbers form a subset of the set of cardinals, they are well-ordered. ([§Cardinals, ⁋Theorem 5](/en/math/set_theory/cardinals#thm5){: data-lid="yfzlc" }) Thus, we can use induction on natural numbers. ([§Properties of Well-Ordered Sets, ⁋Lemma 7 (Transfinite Induction)](/en/math/set_theory/well_ordering#lem7){: data-lid="01vqd" })
...
::: Proof
By [§Operations on Cardinals, ⁋Proposition 6](/en/math/set_theory/operation_of_cardinals#prop6){: data-lid="1bf64" }, $\mathfrak{a}=\mathfrak{b}$ is equivalent to $\mathfrak{a}+\mathbf{1}=\mathfrak{b}+\mathbf{1}$. Now let $\mathfrak{b}=\mathfrak{a}+\mathbf{1}$. By assumption, $\mathfrak{a}\neq\mathfrak{b}$, and therefore
...
:::
::: Proof
This is a direct consequence of [Lemma 3](#lem3){: data-lid="rldhh" }. Since $a\leq b$, such a $c\geq 0$ exists, and if $c=0$ then $a=b$, so we must have $c\neq 0$.
:::
