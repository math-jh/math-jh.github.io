...
If $i$ is a bijection from $\mathfrak{a}$ to a subset of $\mathfrak{b}$, then $i(\mathfrak{a})\subseteq\mathfrak{b}$ and $\mathfrak{a}$ is equipotent to $i(\mathfrak{a})$. Hence it suffices to show that a bijection exists between $i(\mathfrak{a})$ and $\mathfrak{b}$.  
...
By the well-ordering principle, there exists a well-ordering on this set; denote it by $\leq$. Moreover, every subset of $A$ is equipotent to a segment of $A$ ([Proposition](#prop-0){: data-lid="md6h3" data-relation="required" } in Review). Thus for every cardinal $\mathfrak{a}$, the set of segments of $A$ equipotent to it is non-empty, and by the well-orderedness of $A^\ast$, there exists a least element. Denote this element by $\varphi(\mathfrak{a})$.  
