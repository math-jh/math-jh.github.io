...
By the well-ordering principle, there exists a well-ordering on this set; denote it by $\leq$. Moreover, every subset of $A$ is equipotent to a segment of $A$ ([Proposition](#prop-0){: data-lid="md6h3" } in Review). Thus for every cardinal $\mathfrak{a}$, the set of segments of $A$ equipotent to it is non-empty, and by the well-orderedness of $A^\ast$, there exists a least element. Denote this element by $\varphi(\mathfrak{a})$.  
