...
[명제 17](#prop17){: data-lid="blg2p" data-relation="required" }이 말하는 것은 $\varphi_\ast$가 abelian sheaf 수준의 연산에 그치지 않고 $\mathcal{O}_Y$-module 구조를 존중한다는 것이다. 이는 [\[대수적 위상수학\] §벡터다발의 특성류](/ko/math/algebraic_topology/characteristic_classes){: data-lid="va1j4" data-relation="weak" }에서 Gysin homomorphism $\pi_!$가 주는 등식
...
즉 affine morphism은 $S$ 위의 quasi-coherent algebra와 정확히 같은 데이터이며, [정리 20](#thm20){: data-lid="mcnlf" data-relation="weak" }의 셋째 주장은 이 대응이 morphism까지 보존한다는 것을 보여준다. 즉 $S$ 위의 affine scheme들의 category $\AffSch_{/S}$와 quasi-coherent $\mathcal{O}_S$-algebra들의 category 사이의 contravariant equivalenc가 존재한다. 이제 이 대응의 한쪽 방향에 이름을 붙인다.
