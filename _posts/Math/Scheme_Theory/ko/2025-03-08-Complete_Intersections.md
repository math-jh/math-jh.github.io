...
2. $\mathbb{P}^3_\mathbb{K}$의 twisted cubic $C$를 보자. 이는 $\mathbb{P}^1=\Proj \mathbb{K}[\y_0,\y_1]$ 위의 invertible sheaf $\mathcal{O}_{\mathbb{P}^1}(3)$과 그 globally generating section $\y_0^3, \y_0^2\y_1, \y_0\y_1^2, \y_1^3$이 정의하는 morphism $\varphi:\mathbb{P}^1 \rightarrow \mathbb{P}^3_\mathbb{K}$으로 주어진 것이다. Standard affine chart $D_+(\x_j)$ 위에서 $\varphi$에 대응하는 ring homomorphism은 $\x_i/\x_j\mapsto \y_0^{3-i}\y_1^i/\y_0^{3-j}\y_1^j$이므로, 이것이 surjective인 것과 그 kernel이 두 원소의 regular sequence로 생성되는 것을 함께 확인하면 $\varphi$가 closed embedding이라는 것과 $C$가 codimension $2$의 local complete intersection이라는 것을 한꺼번에 얻는다. 표기의 편의를 위해 $v=\y_1/\y_0$으로 적자.

	$\varphi^{-1}(D_+(\x_0))=D_+(\y_0)=\Spec\mathbb{K}[v]$이고 $D_+(\x_0)$의 좌표 $a=\x_1/\x_0$, $b=\x_2/\x_0$, $c=\x_3/\x_0$은 각각 $v$, $v^2$, $v^3$으로 가므로, 대응하는 ring homomorphism은 surjective이고 $\mathbb{K}[a,b,c]/(b-a^2, c-a^3)\cong\mathbb{K}[a]$가 $a\mapsto v$로 $\mathbb{K}[v]$와 isomorphic하다는 데서 그 kernel은 정확히 $(b-a^2, c-a^3)$이다. 마찬가지로 $\varphi^{-1}(D_+(\x_1))=D_+(\y_0\y_1)=\Spec\mathbb{K}[v,v^{-1}]$이고 $D_+(\x_1)$의 좌표 $p=\x_0/\x_1$, $q=\x_2/\x_1$, $r=\x_3/\x_1$은 각각 $v^{-1}$, $v$, $v^2$으로 가 surjective이며, $\mathbb{K}[p,q,r]/(r-q^2, pq-1)\cong\mathbb{K}[q,q^{-1}]$이 $q\mapsto v$로 $\mathbb{K}[v,v^{-1}]$와 isomorphic하므로 kernel은 $(r-q^2, pq-1)$이다. 두 경우 모두 ambient가 integral domain이고 첫 generator로 나눈 quotient $\mathbb{K}[a,c]$와 $\mathbb{K}[p,q]$ 또한 integral domain이므로, 각 kernel의 두 generator는 regular sequence를 이룬다. 남은 두 chart에 대해서는 $\x_i\mapsto \x_{3-i}$가 정의하는 $\mathbb{P}^3_\mathbb{K}$의 automorphism이 $\y_0\leftrightarrow \y_1$과 어울려 $C$를 보존하면서 $D_+(\x_0)$과 $D_+(\x_3)$을, $D_+(\x_1)$과 $D_+(\x_2)$를 맞바꾸므로 위의 두 계산이 그대로 옮겨진다.
