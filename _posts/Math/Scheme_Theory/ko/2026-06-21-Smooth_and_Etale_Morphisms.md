...
Jacobian 조건이 실제로 주는 것은 fiber에 대한 정보이다. $s=f(x)$라 하고, $x$를 fiber $\mathbb{A}^n_{\kappa(s)}$의 점으로 볼 때의 local ring을 $R$이라 하자. Polynomial ring $\kappa(s)[\x_1,\ldots, \x_n]$은 regular ring이므로 ([\[가환대수학\] §정칙성의 호몰로지 판정, ⁋따름정리 6](/ko/math/commutative_algebra/homological_criterion_for_regularity#cor6)) $R$은 regular local ring이며, 그 maximal ideal을 $\mathfrak{m}$이라 하면 residue field는 $\kappa(x)$이다. $x\in X$이므로 $f_i$를 fiber로 내린 $\bar f_i$는 $\mathfrak{m}$에 속하고, Leibniz 규칙에 의하여 $h\mapsto \sum_j(\partial h/\partial \x_j)(x)d\x_j$는 $\mathfrak{m}^2$을 죽이므로 $\kappa(x)$-linear map $\mathfrak{m}/\mathfrak{m}^2 \rightarrow \kappa(x)^{\oplus n}$을 유도한다. 이 map은 $\bar f_i$를 $J$의 $i$번째 행으로 보내고 가정에 의하여 그 행들은 일차독립이므로, $\bar f_1,\ldots, \bar f_r$의 $\mathfrak{m}/\mathfrak{m}^2$에서의 image 또한 일차독립이다. $R$이 regular이므로 $\dim_{\kappa(x)}\mathfrak{m}/\mathfrak{m}^2=\dim R$이고, 이 image들을 $\mathfrak{m}/\mathfrak{m}^2$의 기저로 확장한 뒤 Nakayama 보조정리를 적용하면 ([\[가환대수학\] §정수적 확장, ⁋보조정리 8](/ko/math/commutative_algebra/integral_extension#lem8)) $\mathfrak{m}$을 생성하는 $\dim R$개의 원소를 얻는다. 곧 $\bar f_1,\ldots, \bar f_r$은 $R$의 regular system of parameters의 앞부분이고, [\[가환대수학\] §정칙국소환, ⁋따름정리 3](/ko/math/commutative_algebra/regular_local_rings#cor3)에 의하여 그 전체가 $R$-sequence이므로 앞부분인 $\bar f_1,\ldots, \bar f_r$ 또한 $R$-regular sequence이다. 각각의 $i$에 대하여 $\bar f_{i+1}$의 곱셈이 $\kappa(s)[\x_1,\ldots, \x_n]/(\bar f_1,\ldots, \bar f_i)$에 만드는 kernel은 finitely generated이고 $x$에서의 stalk이 $0$이므로 $x$를 담는 어떤 $D(h)$ 위에서 소멸하며, 그러한 $h$들을 모두 곱하여 얻은 $D(h)\subseteq \mathbb{A}^n_{\kappa(s)}$에 대하여 $X_s\cap D(h)\hookrightarrow D(h)$는 [§완전교차, ⁋정의 1](/ko/math/scheme_theory/complete_intersections#def1)의 의미에서 codimension $r$의 complete intersection이 된다.

Fiber에서의 이 조건은 base 방향으로 들어올려진다. $P$가 $A$ 위에서 free이므로 $\mathbb{A}^n_S \rightarrow S$는 flat이고 ([§평탄사상, ⁋보조정리 2](/ko/math/scheme_theory/flat_morphisms#lem2)), $A$가 Noetherian이므로 $\mathcal{O}_{S,s} \rightarrow \mathcal{O}_{\mathbb{A}^n_S,x}$는 Noetherian local ring 사이의 flat local homomorphism이며 이를 $\mathcal{O}_{S,s}$의 maximal ideal로 나눈 것이 정확히 $R$이다. 그럼 $\bar f_1,\ldots, \bar f_r$이 $R$-regular sequence라는 것으로부터 $f_1,\ldots, f_r$이 $\mathcal{O}_{\mathbb{A}^n_S,x}$의 regular sequence이고 그 quotient $\mathcal{O}_{X,x}$가 $\mathcal{O}_{S,s}$ 위에서 flat임이 따라나온다. ([[Stacks, Lemma 10.99.3]](https://stacks.math.columbia.edu/tag/00MG)) [§평탄사상, ⁋정의 1](/ko/math/scheme_theory/flat_morphisms#def1)의 의미에서 이는 $f$가 $x$에서 flat이라는 것이므로, [§평탄사상, ⁋정리 20](/ko/math/scheme_theory/flat_morphisms#thm20)에 의하여 $f$는 $x$의 어떤 열린근방 위에서 flat하다. 그 근방과 $D(g)$의 교집합 위에서 $f$는 flat하고 $\Omega$는 rank $n-r$의 국소자유이므로, [정리 4](#thm4)에 의하여 $f$는 상대차원 $n-r$의 smooth morphism이다.
:::
...
역으로 모든 square-zero 확대에 대하여 lifting이 존재한다 하자. 이 lifting property를 $T_0=X$ 위의 conormal 확대에 적용하면, conormal exact sequence $\mathfrak{a}/\mathfrak{a}^2 \rightarrow \Omega_{P/A}\otimes B \rightarrow \Omega_{B/A} \rightarrow 0$의 좌측 morphism이 split됨을 얻고, 따라서 $\Omega_{B/A}$가 국소자유이다. 또한 formal smoothness(모든 square-zero 확대에 대한 lifting의 존재)와 locally of finite presentation으로부터 local criterion of flatness를 거쳐 $f$가 flat임이 따른다 (표준 결과, EGA IV 17.5.1). 그러므로 [정리 4](#thm4)에 의하여 $f$는 smooth하다.
:::
