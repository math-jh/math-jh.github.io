...
이제 $s=\varphi(x)$라 하자. 위의 논증을 따라 $A$가 Noetherian이라 가정하면, 이 따름정리를 *Noetherian* local ring $(\mathcal{O}_{S,s},\mathfrak{m}_s)$와 그 위의 local *Noetherian* algebra $(\mathcal{O}_{\mathbb{A}^n_S,x},\mathfrak{m}_x)$, 그리고 그 자신을 module로 본 $M=\mathcal{O}_{\mathbb{A}^n_S,x}$와 $\mathfrak{m}_x$의 원소 $f_1,\ldots, f_r$에 적용할 수 있다. $B$가 $A$ 위에서 free이므로 $M$은 $\mathcal{O}_{S,s}$ 위에서 flat이고, 따라서 우리가 보여야 할 것은 $R=M/\mathfrak{m}_sM$, 즉 fiber $\mathbb{A}^n_{\kappa(s)}$의 $x$에서의 local ring에서 $f_1,\ldots, f_r$의 image가 regular sequence를 이룬다는 것뿐이다. $R$의 maximal ideal을 $\mathfrak{m}$이라 적자.
