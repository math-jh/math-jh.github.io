...
이제 $s=\varphi(x)$라 하자. 위의 논증을 따라 $A$가 Noetherian이라 가정하면, 위의 따름정리를 Noetherian local ring $(\mathcal{O}_{S,s},\mathfrak{m}_s)$와 그 위의 local Noetherian algebra $(\mathcal{O}_{\mathbb{A}^n_S,x},\mathfrak{m}_x)$, 그리고 그 자신을 module로 본 $M=\mathcal{O}_{\mathbb{A}^n_S,x}$와 $\mathfrak{m}_x$의 원소 $f_1,\ldots, f_r$에 적용할 수 있다.

앞서 살펴본 논증에 의해 $A$가 Noetherian이라 가정할 수 있으므로, 이 가정 하에서 두 local ring은 모두 Noetherian이고 $\mathcal{O}_{S,s} \rightarrow \mathcal{O}_{\mathbb{A}^n_S,x}$는 local homomorphism이며, $B$가 $A$ 위에서 free이므로 $\mathbb{A}^n_S \rightarrow S$가 flat이 되어 $\mathcal{O}_{\mathbb{A}^n_S,x}$는 $\mathcal{O}_{S,s}$ 위에서 flat이고, $x\in X$이므로 $f_i$는 그 maximal ideal에 속한다. 이를 $\mathcal{O}_{S,s}$의 maximal ideal로 나눈 것은 fiber $\mathbb{A}^n_{\kappa(s)}$의 $x$에서의 local ring이므로, 이를 $R$, 그 maximal ideal을 $\mathfrak{m}$이라 하면 남는 가정은 $f_1,\ldots, f_r$의 $R$에서의 image가 regular sequence를 이룬다는 것 하나이다.
