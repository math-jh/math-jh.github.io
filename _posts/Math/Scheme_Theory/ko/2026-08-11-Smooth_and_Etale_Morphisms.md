...
::: 예시 14
위에서 살펴봤듯, finite separable field extension $\mathbb{K}\subseteq\mathbb{L}$에 대하여 $\Spec\mathbb{L}\rightarrow\Spec\mathbb{K}$는 étale이다. Primitive element theorem에 의하여 $\mathbb{L}=\mathbb{K}[\t]/(f)$로 쓸 수 있고, $f$가 separable이므로 $f'$는 $\mathbb{L}$에서 가역이기 때문이다. ([\[체론\] §분리가능확대체, ⁋정의 8](/ko/math/field_theory/separable_extensions#def8){: data-lid="g2l7t" data-relation="weak" }) 반면 $\mathbb{K}=\mathbb{F}_p(\t)$와 $\mathbb{L}=\mathbb{F}_p(\t^{1/p})$의 inseparable extension은 étale morphism이 아니다. 여기서 추가되는 원소 $u=\t^{1/p}$의 minimal polynomial은 $\x^p-\t$인데, 이를 미분하면 $0$이 되어 가역이 될 수 없기 때문이다.
