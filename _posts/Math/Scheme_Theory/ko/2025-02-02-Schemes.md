...
를 얻는다. 한편 [\[가환대수학\] §국소화의 성질들, ⁋명제 2](/ko/math/commutative_algebra/properties_of_localization#prop2)의 flatness로부터 localization은 exact functor이므로, 이를 exact sequence $0\rightarrow \mathfrak{p}_x \rightarrow A \rightarrow A/\mathfrak{p}_x \rightarrow 0$에 적용하면 $A_{\mathfrak{p}_x}/\mathfrak{p}_xA_{\mathfrak{p}_x}\cong (A/\mathfrak{p}_x)_{\mathfrak{p}_x}$를 얻는다. 그런데 $A\setminus \mathfrak{p}_x$의 $A/\mathfrak{p}_x$에서의 image는 정확히 $A/\mathfrak{p}_x$의 nonzero element들이므로 우리는
...
$$\mathbb{A}_{\mathbb{K},\mathrm{classical}}^n=\{(x_1,\ldots, x_n)\mid x_i\in \mathbb{K}\}$$
...
만일 $\mathbb{K}$가 algebraically closed라면 [\[가환대수학\] §영점정리, ⁋보조정리 5](/ko/math/commutative_algebra/nullstellensatz#lem5)에 의하여 $\mathbb{K}[\x_1,\ldots, \x_n]$의 maximal ideal들은 모두 $(\x_1-a_1,\ldots, \x_n-a_n)$의 꼴이므로, $(a_1,\ldots, a_n)\mapsto (\x_1-a_1,\ldots, \x_n-a_n)$은 일대일 대응

$$\mathbb{A}_{\mathbb{K},\mathrm{classical}}^n\cong \MaxSpec \mathbb{K}[\x_1,\ldots, \x_n]$$

를 준다. 이는 algebraically closed가 아닌 $\mathbb{K}$에 대해서는 성립하지 않는데, 가령 $\mathbb{K}=\mathbb{R}$이고 $n=1$이면 maximal ideal $(\x^2+1)$에 대응되는 $\mathbb{R}$의 점이 없다. 
