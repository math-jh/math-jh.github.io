...
:::
::: 증명
우리는 [§스펙트럼, ⁋명제 9](/ko/math/scheme_theory/spectrums#prop9)의 셋째 결과로부터 위상공간 사이의 injective continuous map $\Spec\epsilon: \Spec A_f \rightarrow \Spec A$가 homeomorphism $D(f)\cong \Spec A_f$를 주는 알고 있지만, 위의 isomorphism을 정당화하기 위해서는 이를 scheme morphism으로서 정당화해야 한다. 
