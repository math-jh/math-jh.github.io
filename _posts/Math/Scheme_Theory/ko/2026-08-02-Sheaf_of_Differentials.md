...
이 정리의 증명은 $\mathbb{P}^n$ 위에서 trace map과 cup product로 만든 pairing이 perfect pairing임을 보인 뒤 이를 finite surjective morphism을 따라 일반의 $X$로 옮기는 논증을 사용하는데, 구체적인 증명은 이 글의 범위를 넘어서므로 [\[대수다양체\] §세르 쌍대성, ⁋명제 2](/ko/math/algebraic_varieties/serre_duality#prop2)에 위임하기로 한다. 
