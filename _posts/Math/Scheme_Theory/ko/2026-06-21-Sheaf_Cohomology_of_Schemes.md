...
를 얻는다. 그런데 위의 계산에서, 우리는 이미 $\mathcal{I}_x$가 $\{x\}$ 바깥에서는 $\mathcal{O}_X$ 전체가 되는 것을 살펴보았고, 따라서 마지막 항 $\mathcal{F}/\mathcal{I}_x\mathcal{F}$의 stalk은 $x$ 바깥에서 소멸하며, $x$에서는 $\mathcal{I}_x$가 reduced structure의 ideal sheaf이므로 정확히 $\mathcal{G}_x=\mathcal{F}_x/\mathfrak{m}_x\mathcal{F}_x=\mathcal{F}_x\otimes_{\mathcal{O}_{X,x}}$


 따라서 $\mathcal{G}=\mathcal{F}/\mathcal{I}_x\mathcal{F}$의 stalk이 $x$ 밖에서 소멸하며, $x$를 포함하는 chart 위에서는 reduced structure가 $\mathcal{I}_x$로 $x$에 대응하는 maximal ideal을 주므로 ([§닫힌 부분스킴, ⁋정의 14](/ko/math/scheme_theory/closed_subschemes#def14)) $\mathcal{G}_x=\mathcal{F}_x/\mathfrak{m}_x\mathcal{F}_x$는 $\mathcal{F}$의 $x$에서의 fiber $\mathcal{F}_x\otimes_{\mathcal{O}_{X,x}}\kappa(x)$이다. 그럼 $x$를 포함하는 임의의 열린집합 $U$에 대하여, section이 germ들로 결정되고 $x$ 밖에서는 stalk이 $0$이므로 restriction $\mathcal{G}(U) \rightarrow \mathcal{G}_x$가 단사이고, $\{x\}$가 닫힌집합이라 $U\setminus\{x\}$ 또한 열려 있어 $x$의 근방 위의 section을 $U\setminus\{x\}$ 위의 zero section과 붙일 수 있으므로 전사이다. 곧 $\mathcal{G}$는 그 값이 위의 fiber인 $x$ 위의 skyscraper sheaf이며, 특히 그 global section module 또한 이 fiber이다.
