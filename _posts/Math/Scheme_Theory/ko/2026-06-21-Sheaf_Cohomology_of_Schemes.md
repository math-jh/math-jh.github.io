...
이 성립하도록 하는 공통의 $N$과 global section $t_{ij}$들이 존재하여, $s_1^N,\ldots, s_k^N$들과 $t_{ij}$들이 $\mathcal{L}^{\otimes mN}$을 globally generate하도록 할 수 있다는 것이다. 만일 이것이 성립한다면, 이들이 $s_i^N$의 chart 위에서 주는 ratio 중 $t_{ij}/s_i^N=b_{ij}$가 있으므로 우리가 원하는 전사성, 즉 affine scheme으로 제한했을 때 $\varphi$가 closed immersion이 된다는 사실이 증명될 것이다. 
...
따라서 이 정리의 핵심은 둘째 조건을 가정하고 첫째 조건을 보이는 것이다. 즉, 임의의 coherent sheaf $\mathcal{F}$를 고정하고, 충분히 큰 모든 $n$에 대해 $\mathcal{F}\otimes\mathcal{L}^{\otimes n}$이 globally generated라는 것을 보여야 한다. 이는 각 점의 stalk에 대한 조건이므로, 우선 closed point 하나의 근방에서 이를 확인하고, 그 다음 그 근방을 $n$에 무관하게 만든 뒤, 마지막으로 유한 개의 근방으로 $X$를 덮는 순서로 논증한다. 

우선 closed point $x\in X$를 하나 고정하고, $\{x\}$에 reduced closed subscheme 구조를 주어 얻은 ideal sheaf를 $\mathcal{I}_x$라 하자. 또 $\mathcal{I}_x\mathcal{F}$를 multiplication map $\mathcal{I}_x\otimes\mathcal{F} \rightarrow \mathcal{F}$의 image라 하자. 그럼 각각의 affine chart $\Spec A$ 위에서 $\mathcal{I}_x$와 $\mathcal{F}$는 ideal $I\subseteq A$와 finitely generated $A$-module $M$에 대응하고 associated sheaf functor가 exact이므로 ([§준연접층, ⁋명제 6](/ko/math/scheme_theory/quasicoherent_sheaves#prop6)), $\mathcal{I}_x\mathcal{F}$는 chart마다 submodule $IM\subseteq M$의 associated sheaf이다. 이때 $X$가 Noetherian이라 $IM$ 또한 finitely generated이므로 ([\[가환대수학\] §기본 개념들, ⁋정리 3](/ko/math/commutative_algebra/basic_notions#thm3)), $\mathcal{I}_x\mathcal{F}$는 coherent sheaf이다. 그럼 short exact sequence

$$0 \rightarrow \mathcal{I}_x\mathcal{F} \rightarrow \mathcal{F} \rightarrow \mathcal{F}/\mathcal{I}_x\mathcal{F} \rightarrow 0$$

를 얻는다. 

여기서 마지막 항 $\mathcal{G}=\mathcal{F}/\mathcal{I}_x\mathcal{F}$는 $x$ 위의 skyscraper sheaf가 된다. $x$가 closed point이므로 $X\setminus\{x\}$ 위에서는 $\mathcal{I}_x=\mathcal{O}_X$이고 따라서 $\mathcal{G}$의 stalk이 $x$ 밖에서 소멸하며, $x$를 포함하는 chart 위에서는 $\mathcal{I}_x$가 $x$에 대응하는 maximal ideal이므로 $\mathcal{G}_x=\mathcal{F}_x/\mathfrak{m}_x\mathcal{F}_x$는 $\mathcal{F}$의 $x$에서의 fiber $\mathcal{F}_x\otimes_{\mathcal{O}_{X,x}}\kappa(x)$이다. 그럼 $x$를 포함하는 임의의 열린집합 $U$에 대하여, section이 germ들로 결정되고 $x$ 밖에서는 stalk이 $0$이므로 restriction $\mathcal{G}(U) \rightarrow \mathcal{G}_x$가 단사이고, $\{x\}$가 닫힌집합이라 $U\setminus\{x\}$ 또한 열려 있어 $x$의 근방 위의 section을 $U\setminus\{x\}$ 위의 zero section과 붙일 수 있으므로 전사이다. 곧 $\mathcal{G}$는 그 값이 위의 fiber인 $x$ 위의 skyscraper sheaf이며, 특히 그 global section module 또한 이 fiber이다.

여기에 $\mathcal{L}^{\otimes n}$을 텐서하면, 이것이 invertible이라 국소적으로 $\mathcal{O}_X$와 동형이므로 exactness가 보존되어
...
또한 exact이며, 마지막 항은 값이 $(\mathcal{F}\otimes\mathcal{L}^{\otimes n})_x\otimes_{\mathcal{O}_{X,x}}\kappa(x)$인 $x$ 위의 skyscraper sheaf이다. 이제 가정을 coherent sheaf $\mathcal{I}_x\mathcal{F}$에 적용하면 적당한 $n_1$이 존재하여 $n\geq n_1$마다 $H^1(X, \mathcal{I}_x\mathcal{F}\otimes\mathcal{L}^{\otimes n})=0$이므로, [명제 2](#prop2)의 long exact sequence의 일부
...
이 exact이다. 가운데 항은 위에서 본 대로 skyscraper sheaf의 global section module이 그 값 자체라는 것을 써서 적은 것이며, 따라서 이 exactness는 $\mathcal{F}\otimes\mathcal{L}^{\otimes n}$의 global section들이 $x$에서의 fiber를 남김없이 준다는 뜻이다.

이로부터 $x$의 근방에서의 global generation이 따라나온다. Stalk $M=(\mathcal{F}\otimes\mathcal{L}^{\otimes n})_x$는 Noetherian local ring $\mathcal{O}_{X,x}$ 위의 finitely generated module이고 위의 fiber가 $M/\mathfrak{m}_xM$이므로, 유한 개의 global section $s_1,\ldots, s_c$를 그 germ들이 $M/\mathfrak{m}_xM$을 생성하도록 택할 수 있고, [\[가환대수학\] §정수적 확장, ⁋보조정리 8](/ko/math/commutative_algebra/integral_extension#lem8)의 2번에 의하여 이들은 $M$ 자체를 생성한다. 그럼 $s_1,\ldots, s_c$가 정의하는 $\mathcal{O}_X^{\oplus c} \rightarrow \mathcal{F}\otimes\mathcal{L}^{\otimes n}$의 cokernel $\mathcal{Q}$는 coherent sheaf이면서 $\mathcal{Q}_x=0$을 만족한다. 여기서 $x$를 포함하는 affine 열린집합 $\Spec A$를 잡아 그 위에서 $\mathcal{Q}$에 대응하는 finitely generated $A$-module을 $N$, $x$에 대응하는 prime ideal을 $\mathfrak{p}$라 하면 $N_\mathfrak{p}=0$이므로 $N$의 각 generator를 소멸시키는 $\mathfrak{p}$ 밖의 원소가 존재하고, 유한 개인 이들의 곱을 $f$라 두면 $N_f=0$이다. 곧 $\mathcal{Q}$는 $x$의 열린근방 $D(f)$ 위에서 소멸하며, 이는 $\mathcal{F}\otimes\mathcal{L}^{\otimes n}$이 $D(f)$ 위에서 $s_1,\ldots, s_c$로 생성된다는 뜻이다.

이 논증은 $n\geq n_1$인 각각의 $n$마다 열린근방을 주지만 그 근방이 $n$에 따라 달라지므로, 근방을 고정한 채로 지수를 키울 수 있어야 한다. 이를 위해 지금까지의 논증을 $\mathcal{F}=\mathcal{O}_X$에 적용하면 적당한 $e\geq1$에 대하여 $\mathcal{L}^{\otimes e}$과 $\mathcal{L}^{\otimes(e+1)}$이 각각 $x$의 어떤 열린근방 위에서 globally generated이고, 두 근방의 교집합 $W$ 위에서는 둘 다 그러하다. Globally generated인 두 sheaf의 tensor product는 stalk의 generator들의 tensor를 보면 다시 globally generated이므로, $a,b\geq0$에 대하여 $\mathcal{L}^{\otimes(ae+b(e+1))}$은 $W$ 위에서 globally generated이다. 그런데 $k\geq e^2$인 정수 $k$를 $e$로 나눈 몫을 $s$, 나머지를 $b$라 하면 $s\geq e>b\geq0$이므로 $a=s-b$가 음이 아니고 $ae+b(e+1)=se+b=k$이니, $W$ 위에서는 $k\geq e^2$인 모든 $k$에 대하여 $\mathcal{L}^{\otimes k}$이 globally generated이다. 이제 $\mathcal{F}$ 자신으로 돌아와 $n_2\geq n_1$을 하나 택하면 $\mathcal{F}\otimes\mathcal{L}^{\otimes n_2}$이 $x$의 어떤 열린근방 $V$ 위에서 globally generated이므로, $n\geq n_2+e^2$인 모든 $n$에 대하여
...
먼저 $X=\mathbb{P}^n_\mathbb{K}$인 경우로 환원한다. Closed immersion $\iota:X\hookrightarrow\mathbb{P}^n_\mathbb{K}$에 대하여, [정리 8](#thm8) 직전의 isomorphism $(\ast)$을 [정리 9](#thm9)의 증명에서 본 등식 $\iota_\ast(\mathcal{F}(d))\cong(\iota_\ast\mathcal{F})(d)$과 결합하면 $H^i(X, \mathcal{F}(d))\cong H^i(\mathbb{P}^n, (\iota_\ast\mathcal{F})(d))$이고 $\operatorname{Supp}\iota_\ast\mathcal{F}=\iota(\operatorname{Supp}\mathcal{F})$이므로, [정리 8](#thm8)의 증명에서 보았듯 coherent인 $\iota_\ast\mathcal{F}$로 $\mathcal{F}$를 바꾸어도 무방하다.

다음으로 $\mathbb{K}$가 무한체라 가정하여도 됨을 본다. 정리의 결론은 cohomology의 차원들과 $\operatorname{Supp}\mathcal{F}$의 차원만으로 진술되므로, 무한체로의 확대 $\mathbb{L}\supseteq\mathbb{K}$를 잡고 $\mathbb{P}^n_\mathbb{L} \rightarrow \mathbb{P}^n_\mathbb{K}$를 따라 $\mathcal{F}$를 끌어올린 것을 $\mathcal{F}_\mathbb{L}$이라 할 때 이 두 차원이 계수 확대에서 보존된다는 것을 보이면 충분하다. Cohomology의 차원부터 보자. Standard affine cover에 대한 Čech complex는 계수를 확대한 $\check C^\bullet(\mathcal{U}, \mathcal{F})\otimes_\mathbb{K}\mathbb{L}$이고 $-\otimes_\mathbb{K}\mathbb{L}$은 exact이므로, [따름정리 4](#cor4)에 의하여 $\dim_\mathbb{L}H^i(\mathbb{P}^n_\mathbb{L}, \mathcal{F}_\mathbb{L}(d))=\dim_\mathbb{K}H^i(\mathbb{P}^n_\mathbb{K}, \mathcal{F}(d))$이다.

다음은 support의 차원이다. 우선 support 자체가 계수 확대와 호환됨을 본다. 각각의 affine chart $\Spec A$ 위에서 $\mathcal{F}$에 대응하는 finitely generated module $M$의 generator $m_1,\ldots, m_r$를 잡으면 $\ann M$은 $a\mapsto(am_1,\ldots, am_r)$로 주어지는 $A \rightarrow M^{\oplus r}$의 kernel인데, $m_k\otimes1$들이 $M\otimes_\mathbb{K}\mathbb{L}$을 생성하고 $-\otimes_\mathbb{K}\mathbb{L}$이 kernel을 보존하므로 $\ann(M\otimes_\mathbb{K}\mathbb{L})=(\ann M)\otimes_\mathbb{K}\mathbb{L}$이다. 곧 $\operatorname{Supp}\mathcal{F}$가 chart마다 $\Spec(A/\ann M)$인 것에 대응하여 $\operatorname{Supp}\mathcal{F}_\mathbb{L}$은 chart마다 $\Spec\bigl((A/\ann M)\otimes_\mathbb{K}\mathbb{L}\bigr)$이다. 그럼 이제 남은 것은 finitely generated $\mathbb{K}$-대수의 차원이 계수를 확대하여도 변하지 않는다는 것을 보이는 것이다.

이를 위한 우리의 계획은 차원을 보존하는 ring homomorphism들을 확대 이전에 잡아 두고, 이들이 base change 후에도 차원을 보존함을 확인하는 것이다. 임의의 finitely generated $\mathbb{K}$-대수 $A$가 주어졌다 하자. 우리가 사용할 [§차원, ⁋정리 9](/ko/math/scheme_theory/dimension#thm9)는 integral domain에 대한 결과이므로, 먼저 $A$를 domain들로 쪼개는 것부터 시작한다. $A$의 minimal prime들을 $\mathfrak{p}_1,\ldots, \mathfrak{p}_s$라 하면 $A \rightarrow \prod_kA/\mathfrak{p}_k$의 kernel은 nilradical이고 그 image는 $\prod_kA/\mathfrak{p}_k$ 안에서 integral인데, nilpotent는 spectrum을 바꾸지 않고 integral extension은 차원을 보존하므로 ([§차원, ⁋명제 5](/ko/math/scheme_theory/dimension#prop5)) $\dim A=\max_k\dim A/\mathfrak{p}_k$이다. $\mathbb{L}$이 $\mathbb{K}$ 위에서 flat이므로 이 ring homomorphism은 base change 후에도 nilpotent kernel과 integral image를 유지하여, 같은 등식이 $A\otimes_\mathbb{K}\mathbb{L}$에 대해서도 성립한다. 이제 각각의 $A/\mathfrak{p}_k$는 finitely generated $\mathbb{K}$-대수인 integral domain이므로 [§차원, ⁋정리 9](/ko/math/scheme_theory/dimension#thm9)를 적용하여 injective한 finite ring homomorphism $\mathbb{K}[x_1,\ldots, x_{n_k}]\hookrightarrow A/\mathfrak{p}_k$를 얻을 수 있고, 이를 $\mathbb{L}$로 base change한 $\mathbb{L}[x_1,\ldots, x_{n_k}]\hookrightarrow(A/\mathfrak{p}_k)\otimes_\mathbb{K}\mathbb{L}$ 또한 injective이고 finite이므로 다시 [§차원, ⁋명제 5](/ko/math/scheme_theory/dimension#prop5)에 의하여 양변의 차원이 $n_k=\dim A/\mathfrak{p}_k$로 같다. 곧 $\dim(A\otimes_\mathbb{K}\mathbb{L})=\max_kn_k=\dim A$이며, 따라서 처음부터 $\mathbb{K}$가 무한체라 두어도 된다.
...
곱하기 $\ell$은 morphism $\mathcal{F}(-1) \rightarrow \mathcal{F}$을 주며, chart $D_+(\x_j)$ 위에서 이는 $M_j$ 위의 $\ell/\x_j$ 배이다. $M_j$의 각 associated prime은 $D_+(\x_j)$에 놓인 어떤 $\mathfrak{p}_k$가 [§사영공간과 Proj 구성, ⁋보조정리 8](/ko/math/scheme_theory/projective_schemes#lem8)의 대응으로 주는 prime $\mathfrak{p}_kS_{\x_j}\cap S_{(\x_j)}$인데, 만일 $\ell/\x_j$가 이러한 prime에 속한다면 $\ell=\x_j\cdot(\ell/\x_j)$이 $\mathfrak{p}_kS_{\x_j}\cap S=\mathfrak{p}_k$에 속하게 되어 $\ell$의 선택에 모순이다. 따라서 $\ell/\x_j$는 $M_j$의 어떠한 associated prime에도 속하지 않아 $M_j$의 zerodivisor가 아니고 ([\[가환대수학\] §동반소아이디얼, ⁋정리 7](/ko/math/commutative_algebra/associated_primes#thm7)의 2번), 이 morphism은 injective이다. 그 cokernel을 $\mathcal{F}''$이라 하면 각각의 $d$에 대해 short exact sequence
...
가장 단순한 예시는 projective space 자신이다. [따름정리 15](#cor15)에서 $P_{\mathcal{O}_{\mathbb{P}^n}}(t)=\binom{n+t}{n}$이므로 최고차항 계수는 $1/n!$이고 따라서 $\deg\mathbb{P}^n_\mathbb{K}=1$이며, $P_{\mathcal{O}_{\mathbb{P}^n}}(0)=1$에서 $p_a(\mathbb{P}^n_\mathbb{K})=0$이다. 다음으로 $n\geq1$이라 하고, positive degree $e$의 nonzero homogeneous polynomial $f$가 정의하는 hypersurface $X=V_+(f)\subseteq\mathbb{P}^n_\mathbb{K}$를 보자. ([§사영공간의 닫힌 부분스킴, ⁋예시 2](/ko/math/scheme_theory/closed_subschemes_of_projective_spaces#ex2)) $f$는 많아야 하나의 $j$에 대해서만 $\x_j^e$의 상수배일 수 있으므로, $n\geq1$에 의하여 $f$가 $\x_j^e$의 상수배가 아닌 chart $D_+(\x_j)$가 존재한다. 이 chart 위에서 $X$는 dehomogenization $f/\x_j^e$의 zero set인데, 이 원소가 $0$도 unit도 아니므로 그 zero set은 공집합이 아니고, 곧 $X$는 공집합이 아닌 closed subscheme이어서 [정의 17](#def17)을 적용할 수 있다. $\mathbb{K}[\x_0,\ldots, \x_n]$가 integral domain이므로 곱하기 $f$는 각 chart 위에서 injective이고, 따라서 short exact sequence
