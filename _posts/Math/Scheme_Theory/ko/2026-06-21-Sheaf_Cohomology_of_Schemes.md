...
거꾸로 $\mathcal{L}$이 ample이라 하자. 우리는 $\mathcal{L}$의 적당한 거듭제곱의 global section들이 정의하는 locally closed embedding을 만들어야 한다. 정의에 의해 $\mathcal{L}$의 적당한 거듭제곱이 globally generated이며 이러한 section들은 언제나 morphism $X \rightarrow \mathbb{P}^M_\mathbb{K}$을 주므로 우리가 실제로 보여야 할 것은 이 morphism이 embedding을 준다는 것이다. 

$\mathcal{L}^{\otimes k}$을 globally generate하는 section들 $s_0,\ldots, s_M\in\Gamma(X, \mathcal{L}^{\otimes k})$이 주는 morphism $\varphi:X \rightarrow \mathbb{P}^M_\mathbb{K}$을 생각하자. 


이제 morphism이 closed immersion이 되는 것은 target에서 local한 성질이고, affine scheme 사이의 morphism이 closed immersion인 것은 그에 대응되는 ring homomorphism이 전사인 것과 동치이므로, 이 두 사실을 우리가 만들 morphism에 적용해 보면 필요한 데이터가 무엇인지 드러난다.

$\mathcal{L}^{\otimes k}$을 globally generate하는 section들 $s_0,\ldots, s_M\in\Gamma(X, \mathcal{L}^{\otimes k})$이 주는 morphism $\varphi:X \rightarrow \mathbb{P}^M_\mathbb{K}$는 $\mathbb{P}^M_\mathbb{K}$의 좌표 $\x_i$를 $s_i$로 pullback하는 것이므로, standard chart $D_+(\x_i)$의 preimage는 정확히 $s_i$가 소멸하지 않는 열린집합 $X_{s_i}$이다. 이 chart들이 $\varphi$의 image를 덮으니 위의 국소성에 의하여 각각의 $\varphi\vert_{X_{s_i}}:X_{s_i} \rightarrow D_+(\x_i)$가 closed immersion임만 보이면 되고, 이를 전사성으로 판정하려면 우선 $X_{s_i}$ 자신이 affine이어야 한다. 한편 $D_+(\x_i)$의 coordinate ring은 비 $\x_j/\x_i$들이 생성하는 다항식환이고 $\varphi\vert_{X_{s_i}}$에 대응하는 ring homomorphism은 이를 $s_j/s_i\in\Gamma(X_{s_i}, \mathcal{O}_X)$로 보내므로, 전사성은 이 비들이 $\Gamma(X_{s_i}, \mathcal{O}_X)$를 $\mathbb{K}$-대수로서 생성한다는 것과 같다.

곧 우리가 확보해야 할 데이터는 두 겹이다. 첫째로 우리가 택하는 section들의 non-vanishing locus $X_{s_i}$가 각각 affine이면서 $X$를 유한 개로 덮어야 하고, 둘째로 각각의 $X_{s_i}$의 coordinate ring이 비 $s_j/s_i$들로 생성되어야 한다. 앞의 것은 ampleness가 곧바로 내어주며, 뒤의 것은 $X_{s_i}$ 위에서만 정의된 coordinate ring의 generator에 $s_i$의 거듭제곱을 곱해 $X$ 전체의 section으로 확장함으로써 얻는다.


 실질적인 관문은 그 morphism이 embedding이 되는 것인데, closed immersion인 것은 target에 대해 국소적인 조건이고 affine scheme 사이의 morphism이 closed immersion인 것은 대응하는 ring homomorphism이 전사인 것과 동치이다. 그럼 확보해야 할 데이터는 두 겹이 된다. 우선 $X$를 덮는 유한 개의 affine 열린집합이 각각 우리가 택한 section이 소멸하지 않는 열린집합으로 실현되어야 하고, 다음으로 그 조각의 coordinate ring의 generator들이 모두 그 section들의 비로 적혀야 한다. 앞의 것은 ampleness가 곧바로 내어주며, 뒤의 것은 조각 위에서만 정의된 함수를 global section으로 확장하는 작업이다.

첫 겹을 위해서는 각각의 closed point $x\in X$마다 $x$를 품는 affine 열린집합을 section 하나의 non-vanishing locus로 실현해야 한다. 이때 그것이 affine임을 확인하는 가장 쉬운 길은 애초에 affine 열린집합 안에 갇히도록 만드는 것이므로, $\mathcal{L}$이 trivialize되는 affine open neighborhood $U\ni x$를 먼저 고정하고 $x$에서 소멸하지 않으면서 $Y=X\setminus U$ 위에서는 소멸하는 section을 찾는다. 뒤의 조건은 그 section이 $Y$의 ideal sheaf를 통과한다는 뜻이므로, 이로써 ampleness를 적용할 대상이 정해진다.

$Y$에 reduced closed subscheme 구조를 주어 그 ideal sheaf $\mathcal{I}_Y$를 생각하면 이는 coherent sheaf이므로, ampleness에 의하여 적당한 $n>0$에 대해 $\mathcal{I}_Y\otimes\mathcal{L}^{\otimes n}$은 globally generated이다. $x\notin Y$이므로 $(\mathcal{I}_Y)_x=\mathcal{O}_{X,x}$이고, 따라서 $x$에서 소멸하지 않는 section $s\in\Gamma(X, \mathcal{I}_Y\otimes\mathcal{L}^{\otimes n})\subseteq\Gamma(X, \mathcal{L}^{\otimes n})$이 존재한다. 이 $s$는 $Y$ 위에서 소멸하므로 $s$가 stalk를 생성하는 점들의 열린집합 $X_s$는 ([§인자와 선형계, §§Ample invertible sheaf](/ko/math/scheme_theory/divisors_and_linear_systems#ample-invertible-sheaf)) $U$에 포함되고, $U$ 위에서 $\mathcal{L}^{\otimes n}$을 trivialize하면 $s$는 함수 $f\in\Gamma(U, \mathcal{O}_X)$에 대응하여 $X_s=D(f)$는 affine이다.

이제까지의 구성은 closed point 하나마다 이루어졌으므로, 이렇게 얻은 열린집합들이 $X$를 덮는지는 closed point가 충분히 많은지에 달려 있다. $X$가 $\mathbb{K}$ 위에서 finite type이므로 각 affine chart의 coordinate ring은 Jacobson ring이고 ([\[가환대수학\] §영점정리, ⁋정리 4](/ko/math/commutative_algebra/nullstellensatz#thm4)), 따라서 $X$의 공집합이 아닌 닫힌집합은 언제나 $X$의 closed point를 포함한다. 곧 closed point들에서 얻은 위의 열린집합들의 합집합은 여집합이 closed point를 갖지 않는 닫힌집합이라 $X$ 전체이며, $X$가 quasi-compact이므로 그 가운데 유한 개 $X_{s_1},\ldots, X_{s_k}$만으로도 $X$를 덮는다.

다만 이들은 각각 $s_i\in\Gamma(X, \mathcal{L}^{\otimes n_i})$로 지수가 서로 다른 sheaf에서 얻어졌으므로, 함께 하나의 morphism을 정의하려면 같은 sheaf의 section이 되도록 지수를 맞추어야 한다. 임의의 $e\geq1$에 대해 $X_{s^e}=X_s$이라 지수를 올려도 cover가 유지되므로, $m$을 $n_i$들의 최소공배수라 하고 $s_i$를 $s_i^{m/n_i}$로 바꾸면 모든 $i$에 대해 $s_i\in\Gamma(X, \mathcal{L}^{\otimes m})$이라 가정할 수 있다.

이로써 첫 겹이 갖추어졌으므로 두 번째 겹으로 넘어간다. $X_{s_i}$가 affine이고 $X$가 $\mathbb{K}$ 위에서 finite type이므로 각각의 $B_i=\Gamma(X_{s_i}, \mathcal{O}_X)$는 finitely generated $\mathbb{K}$-대수이고, 그 generator $b_{i1},\ldots, b_{ir_i}$를 택하자. 이들이 우리가 좌표로 쓰려는 함수이지만 $X_{s_i}$ 위에서만 정의되어 있어 그대로는 $X$의 section이 되지 못한다. 그런데 $s_i$는 $X_{s_i}$의 여집합에서 소멸하므로 $s_i$의 거듭제곱을 곱하는 것이 $b_{ij}$의 극점을 죽이는 수단이 되고, 그렇게 얻은 section을 다시 그 거듭제곱으로 나눈 비가 $b_{ij}$이다. 따라서 보여야 할 것은 충분히 큰 공통의 $N$에 대하여 $s_i^Nb_{ij}$가 $X$ 전체의 section $t_{ij}\in\Gamma(X, \mathcal{L}^{\otimes mN})$으로 extend된다는 것이다.

이는 chart마다 따로 extension을 만든 뒤 붙이는 방식으로 얻어지며, 두 단계 모두 $s_i$의 거듭제곱을 곱하는 것이 localization에서 무엇을 하는지를 읽는 것이 전부이다. $\mathcal{L}^{\otimes m}$이 trivialize되는 유한 개의 affine open subset $V_1,\ldots, V_p$로 $X$를 덮고 각각의 $V_l=\Spec A_l$ 위에서 trivialization을 하나 고정하면, $s_i$는 함수 $g_{il}\in A_l$에 대응하고 $X_{s_i}\cap V_l=D(g_{il})$이다. 그럼 $b_{ij}$의 $D(g_{il})$로의 restriction은 $(A_l)_{g_{il}}$의 원소이므로 $g_{il}$의 충분히 큰 거듭제곱을 곱하면 $A_l$의 원소가 되고, 유한 개의 $i$와 $j$, $l$에 대해 지수의 최댓값 $N_0$을 취하면 각각의 $V_l$ 위에서 $s_i^{N_0}b_{ij}$가 $\mathcal{L}^{\otimes mN_0}$의 section으로 extend된다. 서로 다른 두 chart 위의 extension은 $X_{s_i}\cap V_l\cap V_{l'}$ 위에서 일치하므로, 남은 것은 그 차를 overlap 전체에서 죽이는 일이다. $X$가 Noetherian이라 $V_l\cap V_{l'}$은 유한 개의 affine open subset $\Spec C$로 덮이는데, 그 각각의 위에서 $V_l$에서 고정한 $\mathcal{L}^{\otimes m}$의 trivialization을 제한하고 그 거듭제곱으로 $\mathcal{L}^{\otimes mN_0}$을 함께 trivialize하면, $s_i$는 함수 $g\in C$에 대응하고 두 extension의 차는 $C$의 원소 $h$가 된다. $h$의 $D(g)$로의 restriction이 $0$이므로 $C_g$에서 $h=0$, 곧 적당한 지수 $c$에 대해 $g^ch=0$이며, 이는 두 extension에 $s_i^c$를 곱하면 그 차가 사라진다는 뜻이다. 유한 개의 $i$와 $j$, 그리고 overlap들을 덮는 유한 개의 chart에서 얻은 이러한 지수 $c$들의 최댓값을 $c_0$이라 하고 $N=N_0+c_0$으로 두자. 그럼 $s_i^{N_0}b_{ij}$의 chart별 extension에 $s_i^{c_0}$을 곱한 것이 $s_i^Nb_{ij}$의 extension이고 이들이 모든 overlap 위에서 일치하므로, 이들이 붙어 원하는 global section $t_{ij}\in\Gamma(X, \mathcal{L}^{\otimes mN})$을 준다.

이제 두 겹이 모두 갖추어졌으므로 section들 $s_1^N,\ldots, s_k^N$과 $t_{ij}$들을 함께 생각하자. $X_{s_i^N}=X_{s_i}$들이 $X$를 덮으므로 이들은 $\mathcal{L}^{\otimes mN}$을 globally generate하고, 따라서 morphism $\varphi:X \rightarrow \mathbb{P}^M_\mathbb{K}$를 정의한다. ([§인자와 선형계, §§Ample invertible sheaf](/ko/math/scheme_theory/divisors_and_linear_systems#ample-invertible-sheaf)) $s_i^N$에 대응하는 좌표가 소멸하지 않는 standard chart를 $V_i\subseteq\mathbb{P}^M_\mathbb{K}$라 하면 $\varphi^{-1}(V_i)=X_{s_i}$이고, $\varphi$의 구성에 의하여 $\varphi\vert_{X_{s_i}}:X_{s_i} \rightarrow V_i$는 좌표를 $t_{ij}/s_i^N=b_{ij}$로 보내는 ring homomorphism에 대응한다. $b_{ij}$들이 $B_i$를 생성하므로 이 homomorphism은 surjective이고, 따라서 $\varphi\vert_{X_{s_i}}$는 closed immersion이다. ([§아핀스킴, ⁋정리 13](/ko/math/scheme_theory/affine_schemes#thm13), [§닫힌 부분스킴, ⁋명제 3](/ko/math/scheme_theory/closed_subschemes#prop3)) Closed immersion인 것은 target에 대해 국소적인 조건이므로 $\varphi$는 열린집합 $\bigcup_iV_i$ 위로의 closed immersion, 곧 locally closed embedding이며 $\mathcal{L}^{\otimes mN}\cong\varphi^\ast\mathcal{O}(1)$은 very ample이다.
:::
