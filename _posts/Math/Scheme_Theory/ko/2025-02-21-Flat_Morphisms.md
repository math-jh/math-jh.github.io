...
무엇이 잘못되었는지는 fiber를 꺼내는 연산이 대수적으로 무엇인지를 보면 드러난다. Fiber를 취하는 것은 base change이고, affine에서 base change는 tensor product이다. 즉 $X=\Spec B$와 $S=\Spec A$에 대하여 $s$에서의 fiber는 $\Spec (B\otimes_A\kappa(s))$이며, 더 일반적으로 family를 base를 따라 옮기는 연산은 함자 $-\otimes_AB$이다. 그런데 tensor product는 완전하지 않다. 위의 예시에서 $A=\mathbb{K}[t]$의 단사사상 $A\xrightarrow{\ \times t\ }A$에 $-\otimes_AB$를 적용하면 $B\xrightarrow{\ \times t\ }B$가 되는데, $\x\neq 0$이면서 $t\x=0$이므로 이는 더 이상 단사가 아니다. 방정식이 사라진 것과 관계식이 무너진 것은 같은 사건이다.
...
우선 $B$가 $A$-평탄이라 가정하자. Localization $B \rightarrow B_\mathfrak{q}$는 평탄이므로 ([\[가환대수학\] §국소화의 성질들, ⁋명제 2](/ko/math/commutative_algebra/properties_of_localization#prop2)), 함자 $-\otimes_AB_\mathfrak{q}$는 $-\otimes_AB$와 $-\otimes_BB_\mathfrak{q}$의 합성이 되어 완전하다. 즉 $B_\mathfrak{q}$는 $A$-평탄이다. 한편 $B_\mathfrak{q}$는 $A_\mathfrak{p}$-대수이므로 $A_\mathfrak{p}\otimes_AB_\mathfrak{q}\cong B_\mathfrak{q}$이고, 따라서 임의의 $A_\mathfrak{p}$-가군 $M$에 대하여
...
라 두자. Localization이 완전함자이므로 $B$의 임의의 maximal ideal $\mathfrak{q}$에 대하여 $K_\mathfrak{q}=\ker(M'\otimes_AB_\mathfrak{q} \rightarrow M\otimes_AB_\mathfrak{q})=0$이다. 그런데 $0\neq \xi\in K$가 존재한다면 $\ann(\xi)$는 $B$의 진 ideal이므로 어떤 maximal ideal $\mathfrak{q}$에 포함되고, $K_\mathfrak{q}=0$으로부터 $s\xi=0$인 $s\in B\setminus \mathfrak{q}$가 존재하여 $\ann(\xi)\not\subseteq \mathfrak{q}$가 되어 모순이다. 따라서 $K=0$이고 $B$는 $A$-평탄이다.
:::

평탄성이 base change와 합성에 대해 안정적이라는 것은 이제 순수하게 대수적인 계산이다.
...
(2) $A \rightarrow B$와 $B \rightarrow C$가 모두 평탄이라 하자. 임의의 $A$-가군 $N$에 대하여 $N \otimes_A C \cong (N \otimes_A B) \otimes_B C$이므로 함자 $-\otimes_A C$는 $-\otimes_A B$와 $-\otimes_B C$의 합성이다. 두 함자가 모두 완전하므로 그 합성 또한 완전하고, 따라서 $C$는 $A$-평탄이다.
:::
...
1. 열린 부분스킴의 포함사상 $U \hookrightarrow X$는 평탄이다. 이는 국소적으로 localization이며, localization은 언제나 평탄하기 때문이다. ([\[가환대수학\] §국소화의 성질들, ⁋명제 2](/ko/math/commutative_algebra/properties_of_localization#prop2))
...
:::
::: 증명
[보조정리 2](#lem2)에 의하여 이는 $B$가 $A$-평탄인 것과 동치이다. $A$가 PID이므로 특히 정역이고, 따라서 $A$의 $0$이 아닌 원소는 모두 $A$에서 zerodivisor가 아니다. 이제 [\[가환대수학\] §평탄성, ⁋따름정리 3](/ko/math/commutative_algebra/flatness#cor3)을 $M=B$에 적용하면 된다.
:::

즉 $\mathbb{K}[t]$ 위의 family가 평탄인 것은 그 좌표환이 $\mathbb{K}[t]$-가군으로서 torsion을 갖지 않는 것과 같다. Torsion 원소란 base의 함수 하나에 의해 죽는 원소이며, 기하학적으로는 fiber 하나에 갇힌 성분이 그러한 원소를 낳는다. 이를 정확히 적으면 다음과 같다.
...
를 생각하자. 관계식이 $t = \y^2 - \x^3$을 주므로 좌표환은 $\mathbb{K}[\x,\y]$와 동형이며, 이 동형 아래에서 $\mathbb{K}[t]$의 작용은 $t \mapsto \y^2-\x^3$으로 주어진다. 좌표환이 정역이므로 $\mathbb{K}[t]$의 $0$이 아닌 원소는 zerodivisor일 수 없고, [명제 6](#prop6)에 의하여 이 사상은 평탄이다. 그럼에도 $t=0$ 위의 fiber는 cusp $\y^2=\x^3$이 되어 특이점을 갖는다.

node의 family

$$\Spec \mathbb{K}[t, \x, \y]/(\x\y - t) \longrightarrow \mathbb{A}^1_\mathbb{K}$$

도 마찬가지이다. $t=\x\y$로 두면 좌표환은 정역 $\mathbb{K}[\x,\y]$와 동형이므로 평탄이며, $t\neq 0$ 위의 fiber는 매끄러운 쌍곡선 $\x\y=t$이지만 $t=0$ 위의 fiber는 두 직선이 만나는 node $\x\y=0$이다.
...
이 사상이 평탄인 것은 세 개의 affine chart에서 확인된다. $D_+(\y)$ 위에서 $u=\x/\y$, $v=\z/\y$로 두면 관계식을 $\y^2$으로 나누어 $uv=t$를 얻으므로 좌표환은 $\mathbb{K}[t][u,v]/(uv-t)$이며, 이는 $t=uv$를 통해 정역 $\mathbb{K}[u,v]$와 동형이다. 따라서 [명제 6](#prop6)에 의하여 평탄이다. $D_+(\x)$ 위에서 $w=\y/\x$, $s=\z/\x$로 두면 관계식을 $\x^2$으로 나누어 $s=tw^2$을 얻으므로 좌표환은 $\mathbb{K}[t][w]$이고, 이는 자유 $\mathbb{K}[t]$-가군이므로 평탄이다. $D_+(\z)$의 경우도 대칭적이다. 이 셋이 $X$를 덮으므로 $X \rightarrow \mathbb{A}^1_\mathbb{K}$는 평탄이다.

Fiber를 보면, $t=a\neq 0$ 위에서는 $\mathbb{P}^2$의 원뿔곡선 $\x\z=a\y^2$이 되어 매끄럽고 $\mathbb{P}^1$과 동형이다. 반면 $t=0$ 위에서는 $\x\z=0$, 즉 두 직선 $\{\x=0\}$과 $\{\z=0\}$이 점 $[0:1:0]$에서 만나는 곡선이 된다. 즉 이것은 매끄러운 원뿔곡선이 두 직선으로 퇴화하는 상수 아닌 곡선들의 family이며, 그럼에도 모든 fiber가 $1$차원으로 남는다.
:::
...
Noetherian 정역 $A$와 finite type $A$-대수 $B$, 그리고 유한생성 $B$-가군 $M$이 주어졌다 하자. 그럼 $0$이 아닌 $a\in A$가 존재하여 $M_a$가 자유 $A_a$-가군이 된다.
:::
::: 증명
$B$는 Noetherian 정역 $A$ 위의 finite type 대수이므로 Hilbert 기저정리에 의하여 Noetherian ring이다.
...
를 얻는다. 각 $B/\mathfrak{q}_i$에 대하여 결론이 성립한다 하고 그 원소들의 곱을 $a$라 하면, 완전열

$$0 \longrightarrow (M_{i-1})_a \longrightarrow (M_i)_a \longrightarrow (B/\mathfrak{q}_i)_a \longrightarrow 0$$

의 오른쪽 항이 자유가군이므로 이 완전열은 split하고, $i$에 대한 귀납법으로 $(M_i)_a$가 모두 자유 $A_a$-가군이 된다. 따라서 $M=B/\mathfrak{q}$인 경우, 즉 $B$를 $B/\mathfrak{q}$로 바꾸어 $B$가 정역이고 $M=B$인 경우만 보이면 충분하다.
...
**귀납.** $K=\Frac(A)$라 하자. $B\otimes_AK$는 $A\setminus\{0\}$에서의 $B$의 국소화이므로 $0$이 아닌 정역이며, $K$ 위의 finite type 대수이다. $d=\dim (B\otimes_AK)$에 대한 귀납법을 쓴다.
...
$B$를 $A$-대수로서 생성하는 원소를 $b_1,\ldots, b_m$이라 하면 각 $b_j$는 $K[y_1,\ldots,y_d]$ 위에서 정수적이다. 이 정수 방정식들에 등장하는 계수는 유한개이므로, 그 분모를 모두 없애는 $0\neq a_0\in A$를 택할 수 있다. 그럼 각 $b_j$는

$$C=A_{a_0}[y_1,\ldots, y_d]$$

위에서 정수적이고, 따라서 $B_{a_0}$는 유한 $C$-가군이다. 여기에서 $y_i$들이 $K$ 위에서 대수적으로 독립이므로 $C$는 $A_{a_0}$ 위의 다항식환이며, 특히 자유 $A_{a_0}$-가군이다.
...
인데, $0\neq g\in \mathfrak{p}K[y_1,\ldots,y_d]$를 택하면 이 몫환에서 $y_1,\ldots,y_d$의 image들은 $g=0$이라는 대수적 관계를 만족하므로 대수적으로 독립일 수 없다. 그런데 이 몫환은 이들 image로 생성되는 $K$-대수이므로 그 분수체의 초월차수는 $d$보다 작고, [§차원, ⁋명제 9](/ko/math/scheme_theory/dimension#prop9)에 의하여

$$\dim\big((C/\mathfrak{p})\otimes_{A_{a_0}}K\big)<d$$

이다. $A_{a_0}$ 또한 $K$를 분수체로 갖는 Noetherian 정역이고 $C/\mathfrak{p}$는 그 위의 finite type 정역이므로, 귀납가정을 적용하면 $0\neq a_\mathfrak{p}\in A_{a_0}$가 존재하여 $(C/\mathfrak{p})_{a_\mathfrak{p}}$가 자유 $(A_{a_0})_{a_\mathfrak{p}}$-가군이다. 필요하다면 $a_0$의 거듭제곱을 곱하여 $a_\mathfrak{p}\in A$로 볼 수 있다.
...
:::
::: 증명
$Y$가 irreducible이므로 $Y$의 공집합이 아닌 열린집합은 모두 조밀하다. 따라서 $Y$의 affine open $V=\Spec A$를 하나 고정하고 그 안에서 $U$를 찾으면 충분하다. $Y$가 Noetherian integral이므로 $A$는 Noetherian 정역이다.

$f$가 finite type이므로 $f^{-1}(V)$는 유한개의 affine open $\Spec B_1,\ldots, \Spec B_k$로 덮이고, 각 $B_i$는 finite type $A$-대수이다. [정리 10](#thm10)을 $M=B_i$에 적용하면 $0\neq a_i\in A$가 존재하여 $(B_i)_{a_i}$가 자유 $A_{a_i}$-가군이다. $a=a_1\cdots a_k$로 두면 각 $(B_i)_a$는 자유가군 $(B_i)_{a_i}$의 국소화이므로 여전히 자유 $A_a$-가군이고, 자유가군은 평탄하므로 [보조정리 2](#lem2)에 의하여

$$\Spec (B_i)_a \longrightarrow \Spec A_a=D(a)$$

는 평탄이다. 평탄성은 $X$ 위에서 국소적인 조건이고 $\Spec (B_i)_a$들이 $f^{-1}(D(a))$를 덮으므로 $f^{-1}(D(a)) \rightarrow D(a)$는 평탄이다. $A$가 정역이고 $a\neq 0$이므로 $D(a)$는 공집합이 아니며, 따라서 $U=D(a)$가 원하는 열린집합이다.
:::
...
:::
::: 증명
$Y$의 닫힌 부분집합 $Z$에 대하여, "$Z$ 위의 임의의 finite type morphism의 상은 $Z$의 constructible subset이다"라는 명제를 $P(Z)$라 하자. $Y$가 Noetherian이므로 닫힌 부분집합들의 모임은 내림사슬조건을 만족하고, 따라서 Noetherian 귀납법을 쓸 수 있다. 즉 $Y$의 모든 진부분 닫힌집합 $Z\subsetneq Y$에 대하여 $P(Z)$가 성립한다고 가정하고 $P(Y)$를 보이면 된다. 또 $Z$의 constructible subset은 $Y$의 constructible subset이므로 ($Z$가 닫힌집합이기 때문이다) 결론을 $Y$ 안에서 읽어도 좋다.
...
**정역으로의 환원.** $A$의 minimal prime을 $\mathfrak{p}_1,\ldots, \mathfrak{p}_k$라 하면 $A$가 reduced이므로 $Y=\bigcup_j V(\mathfrak{p}_j)$이다. 만일 $k\geq 2$라면 각 $V(\mathfrak{p}_j)$는 $Y$의 진부분 닫힌집합이고

$$f(X)=\bigcup_{j=1}^k f\big(X\times_Y V(\mathfrak{p}_j)\big)$$

인데, 각 base change $X\times_YV(\mathfrak{p}_j) \rightarrow V(\mathfrak{p}_j)$는 여전히 finite type이므로 귀납가정 $P(V(\mathfrak{p}_j))$에 의하여 각 항이 constructible이고 따라서 $f(X)$도 constructible이다. 그러므로 $k=1$, 즉 $A$가 정역인 경우만 남는다.

**정역인 경우.** $B=0$이면 $f(X)=\emptyset$이므로 자명하다. $B\neq 0$이라 하고 [정리 10](#thm10)을 $M=B$에 적용하면 $0\neq a\in A$가 존재하여 $B_a$가 자유 $A_a$-가군이다. $a$가 unit이면 $D(a)=Y$이므로 $a$를 unit이 아닌 것으로 택했다고 가정해도 좋고 (unit이라면 아래 논증에서 $V(a)=\emptyset$이 되어 결론이 더 간단해진다), 이 경우 $A$가 정역이므로 $V(a)$는 $Y$의 진부분 닫힌집합이다.
...
::: 보조정리 15
국소환 사이의 국소사상 $\varphi: (A,\mathfrak{m}) \rightarrow (B,\mathfrak{n})$이 $B$를 평탄 $A$-가군으로 만든다 하자. 그럼 $0$이 아닌 임의의 $A$-가군 $M$에 대하여 $M\otimes_AB\neq 0$이며, 특히 $\Spec B \rightarrow \Spec A$는 전사이다.
:::
::: 증명
$0\neq \xi\in M$을 택하자. $\ann(\xi)$는 $A$의 진 ideal이므로 $\ann(\xi)\subseteq \mathfrak{m}$이고, $A/\ann(\xi)\cong A\xi$는 $M$의 부분가군이다. 여기에 평탄한 함자 $-\otimes_AB$를 적용하면 단사사상
...
:::
::: 증명
먼저 fiber의 국소환이 무엇인지를 확인한다. $X=\Spec B$, $Y=\Spec A$인 affine 상황으로 국소화하고 $x=\mathfrak{q}$, $y=\mathfrak{p}=\varphi^{-1}(\mathfrak{q})$라 두자. 정의에 의하여 $X_y=\Spec (B\otimes_A\kappa(\mathfrak{p}))$이고, $x$에 대응하는 점에서의 국소환은
...
이다. 즉 fiber의 국소환은 $\mathcal{O}_{X,x}$를 $\mathcal{O}_{Y,y}$의 maximal ideal로 나눈 것이다.
...
이 등식이 [예시 5](#ex5)의 비평탄성을 다시 한 번 설명해준다. 그 예시에서 $X=\Spec \mathbb{K}[t,\x]/(t\x)$의 원점 $x$를 생각하면 $X$의 두 성분이 모두 원점을 지나고 각각 $1$차원이므로 $\dim \mathcal{O}_{X,x}=1$이다. 또 $y=f(x)$는 $\mathbb{A}^1_\mathbb{K}$의 원점이므로 $\dim \mathcal{O}_{Y,y}=1$이고, fiber $X_y=\mathbb{A}^1_\mathbb{K}$의 원점에서의 국소환도 $1$차원이다. 등식이 요구하는 것은 $1=1+1$이므로 이 사상은 평탄일 수 없다. 반대로 [예시 9](#ex9)의 $X$는 $2$차원이고 base는 $1$차원이므로, 평탄성으로부터 모든 fiber가 $1$차원이어야 한다는 것이 자동으로 따라 나온다.

$X$와 $Y$가 field $\mathbb{K}$ 위의 finite type integral scheme인 경우에는 닫힌점에서 $\dim \mathcal{O}_{X,x}=\dim X$가 성립하므로 ([\[Har\] I.1.8A](https://link.springer.com/book/10.1007/978-1-4757-3849-0)의 등차원성), [명제 17](#prop17)은 익숙한 형태
...
:::
::: 증명
$Y$가 Noetherian이고 $f$가 finite type이므로 $X$ 또한 Noetherian scheme이다. 따라서 열린집합 $U\subseteq X$는 quasi-compact이며, 열린 부분스킴의 포함사상은 평탄이므로 ([예시 4](#ex4)) [명제 3](#prop3)에 의하여 합성 $f\vert_U: U \rightarrow Y$ 또한 평탄하고 finite type이다. 그러므로 처음부터 $U=X$인 경우, 즉 $f(X)$가 열린집합임을 보이면 충분하다.
...
은 구조층 위에서 $p$제곱사상 $a \mapsto a^p$을 유도한다. Kunz의 정리에 의하면 $X$가 정칙인 것과 $F$가 평탄인 것은 서로 동치이다. 따라서 $X$가 특이점을 가지면 Frobenius 사상은 평탄이 아니며, 예를 들어 $X = \Spec \mathbb{K}[\x, \y]/(\x\y)$의 node에서 $F$는 평탄하지 않다. 이는 지금까지 본 판정법들과는 성격이 다른데, 여기에서는 평탄성이 family의 성질이 아니라 $X$ 자체의 정칙성을 재고 있다.
:::
