...
Classically, an algebraic group was defined as a variety equipped with a group structure whose multiplication and inversion are morphisms. ([\[Algebraic Varieties\] §Algebraic Groups, ⁋Definition 1](/en/math/algebraic_varieties/algebraic_groups#def1){: data-relation="weak" }) To transport this into the world of schemes, we use the categorical product and the terminal object of $\Sch_{/S}$: we regard $\Sch_{/S}$ as a cartesian monoidal category and consider group objects inside it.
...
satisfying all the conditions of [\[Category Theory\] §Monoid Objects, ⁋Definition 3](/en/math/category_theory/monoid_objects#def3){: data-relation="required" }. A *homomorphism* between two group schemes $G, H$ is an $S$-morphism preserving this structure.
:::

[Definition 1](#def1){: data-relation="required" } specifies a group scheme as a structure internal to $\Sch_{/S}$. On the other hand, using [§Functor of Points](/en/math/scheme_theory/functor_of_points){: data-relation="required" }, we can turn it into an actual group, since for any test scheme $T$ the set $G(T)$ of $T$-points is a group.
...
:::
::: Proof
By [\[Category Theory\] §Representable Functors, ⁋Theorem 4](/en/math/category_theory/representable_functors#thm4){: data-relation="required" }, the Yoneda embedding $h_{(-)}:\Sch_{/S}\rightarrow\Fun((\Sch_{/S})^\op,\Set)$ is fully faithful, and by [§Functor of Points, ⁋Proposition 7](/en/math/scheme_theory/functor_of_points#prop7){: data-relation="required" } it preserves finite products. Meanwhile, a group object structure in a functor category is given pointwise, so a group object structure on $h_G$ is the same data as a functor $\widetilde{h}_G:(\Sch_{/S})^\op\rightarrow\Grp$ with $U\circ\widetilde{h}_G=h_G$. The fully faithfulness of the Yoneda embedding therefore puts group object structures on $G$ in one-to-one correspondence with such functors $\widetilde{h}_G$, and applying the same argument to morphisms between group objects yields the final claim.
:::
...
1. The *additive group* $\mathbb{G}_a=\Spec \mathbb{Z}[\x]=\mathbb{A}^1$. By [§Functor of Points, ⁋Proposition 1](/en/math/scheme_theory/functor_of_points#prop1){: data-relation="required" }, we have $\mathbb{G}_a(T)\cong \Gamma(T, \mathcal{O}_T)$, and equipping this with the addition of the ring $\Gamma(T, \mathcal{O}_T)$ makes it a group. The map induced by any morphism is a ring homomorphism, hence preserves addition, so functoriality holds.

2. The *multiplicative group* $\mathbb{G}_m=\Spec \mathbb{Z}[\x, \x^{-1}]$. By [§Functor of Points, ⁋Proposition 3](/en/math/scheme_theory/functor_of_points#prop3){: data-relation="required" }, we have $\mathbb{G}_m(T)\cong \Gamma(T, \mathcal{O}_T)^\times$, which we equip with the multiplication of units.

3. The *$n$-th roots of unity* $\mu_n=\Spec \mathbb{Z}[\x]/(\x^n-1)$. By the adjunction of [§Affine Scheme, ⁋Theorem 13](/en/math/scheme_theory/affine_schemes#thm13){: data-relation="required" },
...
4. The *general linear group* $\GL_n=\Spec \mathbb{Z}[\x_{11},\ldots, \x_{nn}, \det{}^{-1}]$. Here $\det$ is the determinant of the matrix $(\x_{ij})$, and we have taken the localization making it invertible. Since a matrix is invertible if and only if its determinant is,
   ...
   holds. ([\[Ring Theory\] §Invertible Elements and Zero Divisors, ⁋Example 9](/en/math/ring_theory/units_and_zero_divisors#ex9){: data-relation="required" }) In particular, when $T=\Spec A$ we have $\GL_n(T)=\GL(n; A)$.
...
In each of the examples above, the group scheme structure was obtained via [Proposition 2](#prop2){: data-relation="required" }, which demonstrates the usefulness of that proposition. Moreover, although these examples were defined over $\Spec \mathbb{Z}$, they are essentially defined over every base $S$. As we saw after [§Morphisms of Schemes, ⁋Example 4](/en/math/scheme_theory/morphism_of_schemes#ex4){: data-relation="weak" }, $\Spec \mathbb{Z}$ is the terminal object of $\Sch$, so every scheme $S$ carries a unique structure morphism $p: S\rightarrow \Spec \mathbb{Z}$, and we can consider the base change morphism it induces
...
All of the schemes in the examples above are affine group schemes, and the way they were defined is explicit. It is also easy to see that the examples other than $\mu_n$ are smooth over affine space. First, $\mathbb{G}_a$ is the affine line itself, so no further argument is needed, and $\GL_n$ is the open subscheme $D(\det)$ of $n^2$-dimensional affine space cut out by $\det$, of which the special case $n=1$ is $\mathbb{G}_m$. Finally, for $\SL_n$, the Laplace expansion of [\[Linear Algebra\] §Existence and Uniqueness of the Determinant, ⁋Theorem 12](/en/math/linear_algebra/existence_and_uniqueness_of_determinant#thm12){: data-relation="required" } shows that the partial derivative of $f=\det-1$ with respect to $\x_{ij}$ is the cofactor $C_{ij}$ in the $(i,j)$ direction. Therefore, by [§Smooth and Étale Morphisms, ⁋Theorem 4](/en/math/scheme_theory/smooth_and_etale_morphisms#thm4){: data-relation="required" }, its Jacobian is given by the $1\times n^2$ matrix
...
Here $\epsilon_H:S\rightarrow H$ is the identity morphism of [Definition 1](#def1){: data-relation="required" }.
:::

Computing with [§Functor of Points, ⁋Proposition 7](/en/math/scheme_theory/functor_of_points#prop7){: data-relation="required" }, for each $S$-scheme $T$ we have

$$(\ker \varphi)(T)=G(T)\times_{H(T)}S(T)=\{g\in G(T)\mid \varphi_T(g)=\epsilon_{H,T}\}$$

so this definition transports the usual definition of the kernel unchanged. Since the right-hand side is a subgroup of $G(T)$ and the correspondence is natural in $T$, [Proposition 2](#prop2){: data-relation="required" } implies that $\ker \varphi$ is a group scheme. What remains is whether $\ker \varphi \rightarrow G$ is a closed embedding; this depends on whether $\epsilon_H$ is a closed embedding, and that condition is precisely separatedness. ([§Valuation Rings, ⁋Definition 3](/en/math/scheme_theory/valuative_criteria#def3){: data-relation="required" })
...
:::
::: Proof
First, closed embeddings are preserved under base change. ([§Closed Subschemes, ⁋Proposition 3](/en/math/scheme_theory/closed_subschemes#prop3){: data-relation="required" }) Now let $\vartheta_H:H\rightarrow S$ be the structure morphism, and consider the following diagram.
...
holds. ([§Fiber Products, ⁋Lemma 2](/en/math/scheme_theory/fiber_products#lem2){: data-relation="required" }) Here, since $\mathbb{Z}=\mathbb{Z}[\y,\y^{-1}]/(\y-1)$, the tensor product becomes the quotient by the ideal generated by $\x^n-1$, and since $\x$ is already invertible here, the localization can be stripped off. Thus $\mu_n$ is a closed subgroup scheme of $\mathbb{G}_m$.
...
The associativity, identity, and inverse conditions of a group object are likewise flipped into the coassociativity, counit, and antipode conditions, respectively, and as it happens, we have already defined such an object. ([\[Category Theory\] §Monoid Objects, ⁋Definition 7](/en/math/category_theory/monoid_objects#def7){: data-relation="required" }) There we called a Hopf monoid object of the symmetric monoidal category $(\rMod{A},\otimes_A,A)$ a *Hopf algebra*, and in the cases we deal with, it suffices to assume that $B$ is a commutative ring. It is then natural to expect the following.
...
:::
::: Proof
By [§Affine Scheme, ⁋Theorem 13](/en/math/scheme_theory/affine_schemes#thm13){: data-relation="required" }, for any $A$-algebra $E$ we have $G(E)=\Hom_{\cAlg{A}}(B, E)$, and as noted immediately after [Theorem 6](#thm6){: data-relation="required" }, its group structure is given by $g\ast h=\mu_E\circ(g\otimes h)\circ\Delta$, identity $\eta_E\circ\epsilon$, and inverse $g\circ\iota$.
...
::: Example 10
Using [Theorem 9](#thm9){: data-relation="required" }, we classify the linear representations of the torus $\mathbb{G}_m=\Spec A[\x,\x^{-1}]$. Our claim is that representations of $\mathbb{G}_m$ on $V$ are in one-to-one correspondence with $\mathbb{Z}$-gradings
...
Now let us find a necessary and sufficient condition for $\rho$ to satisfy the comodule conditions of [Definition 8](#def8){: data-relation="required" }. For the counit condition, we must have $(\id_V\otimes\epsilon)\circ\rho=\id_V$ under the identification $V\otimes_AA\cong V$, so
...
Then again, in the spirit of [Proposition 2](#prop2){: data-relation="required" }, the condition above is equivalent to the following two conditions that a group action must satisfy:
...
1. $P \rightarrow S$ is faithfully flat and locally of finite presentation. ([§Flat Morphisms, ⁋Definition 1](/en/math/scheme_theory/flat_morphisms#def1){: data-relation="required" }, [§Properties of Scheme Morphisms, ⁋Definition 18](/en/math/scheme_theory/properties_of_scheme_morphisms#def18){: data-relation="required" })
...
Then for every $S$-scheme $T$ we have $\varphi_T(g)=g\cdot s_T$. By the second condition of [Definition 12](#def12){: data-relation="required" }, $g\mapsto g\cdot s_T$ is a bijection from $G(T)$ to $P(T)$, so by [\[Category Theory\] §Representable Functors, ⁋Theorem 4](/en/math/category_theory/representable_functors#thm4){: data-relation="required" }, $\varphi$ is an isomorphism. Moreover, $\varphi_T(g'g)=(g'g)\cdot s_T=g'\cdot\varphi_T(g)$, so $\varphi$ is $G$-equivariant, and therefore $P$ is trivial.
:::
...
2. Consider the multiplicative group scheme $G=\mathbb{G}_{m, S}=\Spec_S \mathcal{O}_S[\x, \x^{-1}]$ over a base scheme $S$, and an invertible sheaf $\mathcal{L}$. Just as we assembled the bases (frames) of each fiber of a vector bundle to form a principal bundle in [\[Algebraic Topology\] §Classifying Spaces, ⁋Proposition 4](/en/math/algebraic_topology/classifying_spaces#prop4){: data-relation="weak" }, in algebraic geometry we can likewise construct from $\mathcal{L}$ a $\mathbb{G}_m$-torsor $P_\mathcal{L}$, a kind of frame bundle. First, since $\mathcal{L}$ is by definition Zariski-locally free of rank $1$, there exists an affine Zariski open cover $\{U_i\}$ with local trivializations $\varphi_i: \mathcal{L}\vert_{U_i} \xrightarrow{\sim} \mathcal{O}_{U_i}$. Let the transition functions be
    ...
    then by [§Schemes, ⁋Lemma 9](/en/math/scheme_theory/schemes#lem9){: data-relation="required" }, an $S$-scheme $p:P_\mathcal{L}\rightarrow S$ is well defined. The group scheme action $\varrho: \mathbb{G}_m\times_S P_\mathcal{L} \rightarrow P_\mathcal{L}$ on it is now defined over each $U_i$ by the multiplication of $\mathbb{G}_m$
