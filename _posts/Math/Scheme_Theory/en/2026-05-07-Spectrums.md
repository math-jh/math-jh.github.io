...
Earlier, in [Proposition 2](#prop2){: data-lid="qbslt" }, we saw that $\Spec$ can be regarded as a functor $\Spec: \cRing^\op \rightarrow \Set$. Moreover, $\Spec$ is also a functor from $\cRing^\op$ to $\Top$.
...
Now for any $f\in A$, let $S_f=\{1,f,f^2,\ldots\}$ and define $A_f=S_f^{-1}A$. Then by [Proposition 9](#prop9){: data-lid="3e8n9" }, the image of $\Spec A_f$ under $\Spec \epsilon$ in $\Spec A$ is an open set. That this open set is exactly the same as $D(f)$ as a set follows from

$$\mathfrak{p}\not\in Z(f)\iff (f)\not\subseteq \mathfrak{p} \iff f\not\in \mathfrak{p}\iff f^k\not\in \mathfrak{p}\text{ for all $k\geq 0$}\iff S_f\cap \mathfrak{p}=\emptyset$$

and [\[Commutative Algebra\] §Localization, ⁋Proposition 8](/en/math/commutative_algebra/localization#prop8){: data-lid="szh1x" }. Moreover, that $D(f)$ has the same topological structure as $\Spec A_f$ also follows from the first result of [Proposition 9](#prop9){: data-lid="fng2w" }.
...
:::
::: Proof
For any open set $\Spec A \setminus Z(S)$, using the second result of [Lemma 6](#lem6){: data-lid="ql68f" } we have the following computation:
...
Then by a computation similar to this lemma we can verify that $D(fg)=D(f)\cap D(g)$. However, since this computation uses the first result of [Lemma 6](#lem6){: data-lid="hkc7r" }, it cannot in general be extended to infinite indices.
...
:::
::: Proof
By [Lemma 11](#lem11){: data-lid="g64p2" }, it suffices to show that whenever a collection of principal open sets $\{D(f_i)\}_{i\in I}$ covering $\Spec A$ is given, there exists a finite subset $J$ of $I$ such that still $\Spec A=\bigcup_{j\in J} D(f_j)$.
...
On the other hand, by [Lemma 6](#lem6){: data-lid="d0fqa" }
...
Now combining [Definition 3](#def3){: data-lid="61st4" } and [Definition 13](#def13){: data-lid="cy8iz" }, we have defined two functions between the two ordered sets $\mathcal{P}(A)$, $\mathcal{P}(\Spec A)$:
...
First, since $I(\{\mathfrak{p}\})=\mathfrak{p}$, the second result of [Proposition 14](#prop14){: data-lid="svp2g" } gives the formula
...
:::
::: Proof
By [Proposition 14](#prop14){: data-lid="iuic4" }, for any prime ideal $\mathfrak{p}$ we have $IZ(\mathfrak{p})=\sqrt{\mathfrak{p}}=\mathfrak{p}$, and for any closed subset $Y$ of $\Spec A$ we have $ZI(Y)=\cl(Y)=Y$, so $Z$ and $I$ are mutually inverse correspondences. Therefore, by [Proposition 16](#prop16){: data-lid="kz0ry" }, the map $\mathfrak{p}\mapsto Z(\mathfrak{p})$ is a bijection between the prime ideals of $A$ and the irreducible closed subsets of $\Spec A$, and its inverse is $Y\mapsto I(Y)$. Moreover, applying the third result of [Lemma 6](#lem6){: data-lid="orme0" } to two prime ideals $\mathfrak{p},\mathfrak{q}$ gives
