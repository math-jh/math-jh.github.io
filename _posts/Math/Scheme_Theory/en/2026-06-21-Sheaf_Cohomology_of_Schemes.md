...
where $\mathcal{K}$ is the kernel of this surjection and is a coherent sheaf for the same reason as in the proof of [Theorem 9](#thm9){: data-lid="v7phl" data-relation="weak" }. Twisting this by $\mathcal{O}(d-d_0)$ yields
...
On the other hand, the crucial fact used to prove the results in the preceding section was that $\mathcal{O}_X(1)$ comes from an embedding of $X$ into projective space, that is, it is very ample. We already know from [\[Algebraic Varieties\] §Linear Systems, ⁋Definition 10](/en/math/algebraic_varieties/linear_systems#def10){: data-lid="6gv63" data-relation="weak" } that very ampleness and ampleness are closely related, and hence the (scheme version of) ampleness defined in [§Divisors and Linear Systems](/en/math/scheme_theory/divisors_and_linear_systems){: data-lid="43faj" data-relation="weak" } should also be related. First, the following holds.
...
We first reduce to the case $X=\mathbb{P}^n_\mathbb{K}$. For the closed embedding $\iota:X\hookrightarrow\mathbb{P}^n_\mathbb{K}$, combining the isomorphism $(\ast)$ just before [Theorem 9](#thm9){: data-lid="qoq9c" data-relation="required" } with the equality $\iota_\ast(\mathcal{F}(d))\cong(\iota_\ast\mathcal{F})(d)$ seen in the proof of [Theorem 10](#thm10){: data-lid="qj82d" data-relation="forward" }, we obtain $H^i(X, \mathcal{F}(d))\cong H^i(\mathbb{P}^n, (\iota_\ast\mathcal{F})(d))$; and since $\supp\iota_\ast\mathcal{F}=\iota(\supp\mathcal{F})$, as seen in the proof of [Theorem 9](#thm9){: data-lid="o8jy6" data-relation="required" } we may replace $\mathcal{F}$ by the coherent sheaf $\iota_\ast\mathcal{F}$ without loss of generality.
...
The overall structure of the proof is now the same dévissage used in the proofs of [Theorem 9](#thm9){: data-lid="w99ne" data-relation="weak" } and [Theorem 10](#thm10){: data-lid="w2ayd" data-relation="required" }: we construct a short exact sequence
