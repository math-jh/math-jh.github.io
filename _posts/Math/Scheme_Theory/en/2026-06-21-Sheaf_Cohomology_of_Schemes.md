...
Passing [Theorem 3](#thm3){: data-relation="required" } to its relative form gives the following.
...
:::
::: Proof
By the isomorphism $(\ast)$ above, we only need to check that the quasi-coherent sheaf $\iota_\ast\mathcal{F}$ is ([§Quasi-coherent Sheaves, ⁋Theorem 16](/en/math/scheme_theory/quasicoherent_sheaves#thm16){: data-relation="required" }) a coherent sheaf on $\mathbb{P}^n$ to reduce to the case $X=\mathbb{P}^n_{\mathbb{K}}$. For this, it suffices to check the finite type condition affine-locally: since $\iota$ is a closed embedding, on each chart $\iota_\ast\mathcal{F}$ is the finitely generated module over the $A$-algebra $A/I$ viewed as an $A$-module, and because $A \rightarrow A/I$ is surjective, lifting the generators over $A/I$ to $A$ shows that the same elements generate over $A$ as well.
...
The left term is finite-dimensional by [Theorem 7](#thm7){: data-relation="required" }, and the right term is finite-dimensional by the induction hypothesis, so the middle term $H^i(\mathbb{P}^n, \mathcal{F})$ is also finite-dimensional.
:::

The key argument in the proof above is to twist a coherent sheaf sufficiently to make it globally generated, and then cover it by a free sheaf. From this, the finiteness of cohomology on $\mathbb{P}^n$ propagated along the long exact sequence to give the theorem's claim. But [Theorem 7](#thm7){: data-relation="required" } shows not only that cohomology is finite-dimensional in every degree, but also that higher-degree cohomology vanishes entirely, so developing the argument in this direction yields the following result.
...
:::
::: Proof
As in [Theorem 9](#thm9){: data-relation="required" }, by $(\ast)$ we may reduce to the case $X=\mathbb{P}^n_{\mathbb{K}}$ and $\mathcal{O}_X(1)=\mathcal{O}(1)$. Since the proposition involves twisting, the additional equality we need for this is
...
:::
::: Proof
Suppose $\mathcal{L}^{\otimes m}$ is very ample. By [Corollary 11](#cor11){: data-relation="required" }, $\mathcal{L}^{\otimes m}$ is ample, and by the second result of [§Divisors and Linear Systems, ⁋Proposition 19](/en/math/scheme_theory/divisors_and_linear_systems#prop19){: data-relation="required" }, $\mathcal{L}$ is also ample.
...
By part 2 of [§Divisors and Linear Systems, ⁋Proposition 19](/en/math/scheme_theory/divisors_and_linear_systems#prop19){: data-relation="required" }, $\mathcal{L}$ being ample is equivalent to $\mathcal{L}^{\otimes m}$ being ample for every $m\geq1$, so [Theorem 12](#thm12){: data-relation="required" } tells us that the only difference between the two notions lies in taking a power. Adding [Theorem 10](#thm10){: data-relation="required" } to this, we can characterize ampleness solely by cohomology vanishing.
...
:::
::: Proof
The direction that the first condition implies the second is easy, so let us look at it first. If $\mathcal{L}$ is ample, then by [Theorem 12](#thm12){: data-relation="required" } there exists $m>0$ such that $\mathcal{L}^{\otimes m}$ is very ample. Then, as in the proof of [Corollary 11](#cor11){: data-relation="required" }, the embedding defined by $\mathcal{L}^{\otimes m}$ is a closed embedding, so we may apply [Theorem 10](#thm10){: data-relation="required" } with $\mathcal{L}^{\otimes m}$ as the twisting sheaf. In particular, applying this to each of the finitely many coherent sheaves
...
It remains to cover $X$ by these neighborhoods; as seen in the proof of [Theorem 12](#thm12){: data-relation="weak" }, since $X$ is of finite type over $\mathbb{K}$, the coordinate ring of each affine chart is a Jacobson ring, so every non-empty closed subset of $X$ contains a closed point. Hence the union of the $U_x$ is the complement of a closed set with no closed points, which is all of $X$, and since $X$ is quasi-compact, finitely many $U_{x_1},\ldots, U_{x_r}$ suffice to cover $X$. Then setting $n_0=\max_jn_{x_j}$, for every $n\geq n_0$ the sheaf $\mathcal{F}\otimes\mathcal{L}^{\otimes n}$ is generated by global sections at every point of $X$, and since $\mathcal{F}$ was an arbitrary coherent sheaf, $\mathcal{L}$ is ample.
:::
...
By [Theorem 9](#thm9){: data-relation="required" }, a coherent sheaf on a projective scheme has only finitely many finite-dimensional cohomologies, so we may take their alternating sum.
...
consists of finite-dimensional vector spaces by [Theorem 9](#thm9){: data-relation="required" } and terminates in sufficiently high degree, so it is finite. Applying the observation above, the alternating sum of the cohomology dimensions of the three sheaves is $0$, and rearranging signs gives the desired formula.
...
In particular, if a coherent sheaf $\mathcal{F}$ has a finite resolution $0 \rightarrow \mathcal{E}_k \rightarrow \cdots \rightarrow \mathcal{E}_0 \rightarrow \mathcal{F} \rightarrow 0$, then from (2) of [Proposition 15](#prop15){: data-relation="required" } we obtain $\rchi(\mathcal{F})=\sum_{j=0}^k(-1)^j\rchi(\mathcal{E}_j)$. This is the standard path for actually computing the Euler characteristic, and its starting point is line bundles on projective space.
...
:::
::: Proof
The last claim follows immediately from [Theorem 10](#thm10){: data-relation="required" }: for sufficiently large $d$ we have $H^i(X, \mathcal{F}(d))=0$ ($i>0$), so only $H^0$ remains in the alternating sum. Uniqueness follows from the fact that two distinct polynomials cannot agree at infinitely many integers.

We first reduce to the case $X=\mathbb{P}^n_\mathbb{K}$. For the closed embedding $\iota:X\hookrightarrow\mathbb{P}^n_\mathbb{K}$, combining the isomorphism $(\ast)$ just before [Theorem 9](#thm9){: data-relation="required" } with the equality $\iota_\ast(\mathcal{F}(d))\cong(\iota_\ast\mathcal{F})(d)$ seen in the proof of [Theorem 10](#thm10){: data-relation="forward" }, we obtain $H^i(X, \mathcal{F}(d))\cong H^i(\mathbb{P}^n, (\iota_\ast\mathcal{F})(d))$; and since $\supp\iota_\ast\mathcal{F}=\iota(\supp\mathcal{F})$, as seen in the proof of [Theorem 9](#thm9){: data-relation="required" } we may replace $\mathcal{F}$ by the coherent sheaf $\iota_\ast\mathcal{F}$ without loss of generality.
...
The overall structure of the proof is now the same dévissage used in the proofs of [Theorem 9](#thm9){: data-relation="weak" } and [Theorem 10](#thm10){: data-relation="required" }: we construct a short exact sequence

$$0 \rightarrow \mathcal{F}(-1) \rightarrow \mathcal{F} \rightarrow \mathcal{F}'' \rightarrow 0\tag{$\ast\ast$}$$

and transfer the desired property along it. Here $\mathcal{F}(-1) \rightarrow \mathcal{F}$ is the morphism given by multiplication by an element $\ell\in S_1$ of degree $1$ in the homogeneous coordinate ring $S_\bullet=\mathbb{K}[\x_0,\ldots, \x_n]$, and $\mathcal{F}''$ is its cokernel; intuitively this is the restriction of $\mathcal{F}$ to the hyperplane $V_+(\ell)$, i.e. a hyperplane section of $\mathcal{F}$. The difference from the previous proofs is that, whereas those ran induction by covering a coherent sheaf with a free sheaf and computing with the long exact sequence, we use the additivity from [Proposition 15](#prop15){: data-relation="required" } to run induction on the dimension $r=\dim\supp \mathcal{F}$ of the support.
...
This polynomial $P_\mathcal{F}$ is called the *Hilbert polynomial* of $\mathcal{F}$. [Corollary 16](#cor16){: data-relation="weak" } is the case $\mathcal{F}=\mathcal{O}_{\mathbb{P}^n}$, for which $P_{\mathcal{O}_{\mathbb{P}^n}}(t)=\binom{n+t}{n}$ and its degree is $\dim\mathbb{P}^n=n$. The last claim of [Theorem 17](#thm17){: data-relation="required" } says that this polynomial measures, for sufficiently large degree, the dimension of the space of global sections of $\mathcal{F}(d)$; this is the point of contact with the classical viewpoint of introducing the Hilbert polynomial via the Hilbert function of the homogeneous coordinate ring. ([\[Commutative Algebra\] §Hilbert–Samuel Function, ⁋Definition 4](/en/math/commutative_algebra/hilbert-samuel_function#def4)) In particular, when $\mathcal{F}=\mathcal{O}_X$ this polynomial becomes an invariant of $X$ itself.
...
Twisting this by $\mathcal{O}(d)$ and applying part 1 of [Proposition 15](#prop15){: data-relation="required" } and [Corollary 16](#cor16){: data-relation="required" } yields
