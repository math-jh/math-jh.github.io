...
:::
::: Proof
Since $\mathcal{L}$ is very ample, finitely many sections globally generating it define a locally closed embedding $\iota:X \rightarrow \mathbb{P}^N_\mathbb{K}$ and satisfy $\mathcal{L}\cong\iota^\ast\mathcal{O}(1)$. ([§Divisors and Linear Systems, §§Ample Invertible Sheaves](/en/math/scheme_theory/divisors_and_linear_systems#ample-invertible-sheaves)) Since $X$ is projective over $\mathbb{K}$, the image $\iota(X)$ is a closed set ([§Valuation Rings, ⁋Corollary 16](/en/math/scheme_theory/valuative_criteria#cor16)), and hence $\iota$ is a closed immersion. Then $\mathcal{L}$ can play exactly the same role as $\mathcal{O}_X(1)$ in [Theorem 9](#thm9), so for any coherent sheaf $\mathcal{F}$ and all sufficiently large $d$, the sheaf $\mathcal{F}\otimes\mathcal{L}^{\otimes d}$ is globally generated. This is the definition of ampleness.
:::
