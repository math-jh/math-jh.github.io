...
::: Definition 1
A module $M$ over a commutative ring $A$ is called **flat** if the tensor product functor $-\otimes_A M$ is an exact functor. That is, for any short exact sequence of $A$-modules
...
Let us now move to the context of schemes. That a morphism $f: X \rightarrow Y$ is flat means, roughly speaking, that the structure sheaf of $X$ carries the structure of a flat module over the structure sheaf of $Y$.

::: Definition 2
A morphism $f: X \rightarrow Y$ is called **flat** if for every $x \in X$, the local ring $\mathcal{O}_{X,x}$ is flat as an $\mathcal{O}_{Y,f(x)}$-module. Additionally, if the corresponding map of topological spaces is surjective, we call it **faithfully flat**.
:::
...
::: Proposition 3
If a flat morphism $f: X \rightarrow Y$ between locally Noetherian schemes is locally of finite type, then the function
...
::: Proposition 4
A flat morphism $f: X \rightarrow Y$ that is locally of finite type and dominant is an open map. That is, for any open set $U \subseteq X$, the image $f(U)$ is an open set of $Y$.
:::
::: Proof
...
::: Proposition 6
(Local criterion for flatness). Let $(A, \mathfrak{m})$ be a Noetherian local ring and $M$ a finitely generated $A$-module. If for every ideal $I$ of $A$, $M/IM$ is flat over $A/I$, and moreover $\operatorname{Tor}_1^A(M, A/I) = 0$, then $M$ is flat over $A$. In particular, for $I = \mathfrak{m}$, if $M/\mathfrak{m}M$ is flat over $A/\mathfrak{m}$ (that is, free) and $\operatorname{Tor}_1^A(M, A/\mathfrak{m}) = 0$, then $M$ is flat.
:::
::: Proof
...
::: Proposition 7
($\operatorname{Tor}$ criterion). An $A$-module $M$ is flat if and only if $\operatorname{Tor}_1^A(M, A/I) = 0$ for every ideal $I \subseteq A$.
:::
::: Proof
...
(2) The projection morphism between affine spaces $\mathbb{A}_k^{n+m} \rightarrow \mathbb{A}_k^n$, that is, $k[x_1, \dots, x_n] \rightarrow k[x_1, \dots, x_n, y_1, \dots, y_m]$, is flat. This follows immediately from the free module structure of a multivariate polynomial ring.
:::
...
::: Example 11
For a scheme $X$ over a field $k$ of positive characteristic $p > 0$, the **Frobenius morphism**
...
::: Proposition 12
(1) Base change of a flat morphism ([§Fiber Products](/en/math/scheme_theory/fiber_products)) is flat. That is, if $f: X \rightarrow Y$ is flat and $Z \rightarrow Y$ is any morphism, then the projection $X \times_Y Z \rightarrow Z$ is flat.

(2) The composition of flat morphisms is flat. That is, if $f: X \rightarrow Y$ and $g: Y \rightarrow Z$ are both flat, then $g \circ f: X \rightarrow Z$ is also flat.
:::
::: Proof
