...
::: Definition 2
A morphism $f: X \to Y$ is called **flat** if for every $x \in X$, the local ring $\mathcal{O}_{X,x}$ is flat as an $\mathcal{O}_{Y,f(x)}$-module. Additionally, if the corresponding map of topological spaces is surjective, we call it **faithfully flat**.
:::
...
::: Proposition 3
If a flat morphism $f: X \to Y$ between locally Noetherian schemes is locally of finite type, then the function
...
::: Proposition 4
A flat morphism $f: X \to Y$ that is locally of finite type and dominant is an open map. That is, for any open set $U \subseteq X$, the image $f(U)$ is an open set of $Y$.
:::
::: Proof
...
::: Proposition 8
(Generic flatness). Let $f: X \to Y$ be a dominant morphism of finite type from an integral scheme $X$ to a Noetherian scheme $Y$. Then there exists a dense open subset $U$ of $Y$ such that the restriction $f\rvert_{f^{-1}(U)}: f^{-1}(U) \to U$ is flat.
:::
::: Proof
...
::: Proposition 13
For a morphism $f: X \to Y$ locally of finite presentation, the set of points of $X$ at which $f$ is not flat is a closed set. Equivalently, the flat locus is an open subset of $X$.
:::
::: Proof
