...
Since the above $B$-module arises from the $A$-algebra structure of $B$, or in other words from the ring homomorphism $A\rightarrow B$, we may regard it as a scheme morphism $\Spec B\rightarrow \Spec A$. Then the $B$-module $\Omega_{B/A}$ defined above translates, in the language of schemes, into the quasi-coherent sheaf $\widetilde{\Omega_{B/A}}$ on $\Spec B$, and for a general scheme morphism $\varphi:X\rightarrow S$ we glue these to form $\Omega_{X/S}$. ([§Morphisms of Schemes, ⁋Proposition 1](/en/math/scheme_theory/morphism_of_schemes#prop1){: data-relation="required" }) Thus, following the above intuition, we may think of this as fixing the base $S$ direction and treating only the fiber direction of $\varphi:X\rightarrow S$ as functions when taking differentials.
...
Now we must glue the local models $\widetilde{\Omega_{B/A}}$ defined for each pair of affine open subsets $U=\Spec B\subseteq X$ and $V=\Spec A\subseteq S$ with $\varphi(U)\subseteq V$. This form of gluing was already encountered in [§Fiber Products, ⁋Theorem 8](/en/math/scheme_theory/fiber_products#thm8){: data-relation="weak" } when constructing fiber products, where we likewise reduced on the factor side and the base side before gluing the pieces.
...
for each pair of affine open subsets $U=\Spec B\subseteq X$ and $V=\Spec A\subseteq S$ with $\varphi(U)\subseteq V$. ([§Quasi-coherent Sheaves, ⁋Definition 4](/en/math/scheme_theory/quasicoherent_sheaves#def4){: data-relation="required" })
:::
...
Since both sides are determined by their values on affine opens and the identification comes from the canonical one arising from the universal derivation, this isomorphism glues to the scheme level. That is, for a morphism $S' \rightarrow S$, letting $X'=X\times_SS'$ with projection $\pi:X' \rightarrow X$, we have $\Omega_{X'/S'}\cong \pi^\ast\Omega_{X/S}$. In particular, for a point $s\in S$ setting $S'=\Spec\kappa(s)$, since $X'$ is the fiber $X_s$ over $s$ ([§Fiber Products, ⁋Definition 12](/en/math/scheme_theory/fiber_products#def12){: data-relation="required" }), letting $\iota:X_s \rightarrow X$ be the canonical morphism we obtain
...
As seen in the proof above, $\mathcal{I}/\mathcal{I}^2$ is a sheaf on $\Delta(X)$, and since $\Delta$ identifies $X$ with its image, it is pulled back to a sheaf on $X$ via $\Delta^\ast$; when we actually compute, we will calculate $\widetilde{\Omega_{B/A}}$ on affine opens according to [Definition 4](#def4){: data-relation="required" }, but the above proposition gives a coordinate-independent description of this sheaf.
...
([§Quasi-coherent Sheaves, ⁋Definition 2](/en/math/scheme_theory/quasicoherent_sheaves#def2){: data-relation="required" })
:::

To see what this definition does, let us again look at the affine case. For $X=\Spec B$ and $S=\Spec A$, [Definition 4](#def4){: data-relation="required" } gives $\Omega_{X/S}=\widetilde{\Omega_{B/A}}$. The global sections of the $\sHom$ constructed from this are $\mathcal{O}_X$-module homomorphisms, so a global section of $\mathcal{T}_{X/S}$ is $\Hom_{\mathcal{O}_X}(\widetilde{\Omega_{B/A}}, \widetilde B)\cong\Hom_B(\Omega_{B/A}, B)$ ([§Quasi-coherent Sheaves, ⁋Theorem 7](/en/math/scheme_theory/quasicoherent_sheaves#thm7){: data-relation="required" }), and therefore from the definition we obtain

$$\mathcal{T}_{X/S}(X)\cong \Der_A(B, B).$$

([[Commutative Algebra] §Differentials, ⁋Lemma 2](/en/math/commutative_algebra/differentials#lem2)) Repeating the same computation on a principal open $D(g)$ gives $\Der_A(B_g, B_g)$ by the first assertion of [Proposition 3](#prop3){: data-relation="required" }, so $\mathcal{T}_{X/S}$ is the sheaf obtained by locally collecting the $A$-derivations of $B$, and gluing these gives the general case as well.
...
For this reason, care is needed even when computing the tangent space at a point. For a point $x$ of a $\mathbb{K}$-scheme $X$, letting $\kappa(x)$ be the residue field ([§Schemes, ⁋Definition 5](/en/math/scheme_theory/schemes#def5){: data-relation="required" }), we can consider the fiber $\Omega_{X/\mathbb{K}}\otimes_{\mathcal{O}_X}\kappa(x)$ of the cotangent sheaf. If we consider the fiber
...
and then over an affine open subset $\Spec A$ of $S$ this gives $\mathbb{A}^n_A$. ([§Fiber Products, ⁋Example 9](/en/math/scheme_theory/fiber_products#ex9){: data-relation="required" }) Now, by [Definition 4](#def4){: data-relation="required" }, the cotangent sheaf is determined by the local model on each affine open of the base, so applying the above proposition on each chart yields $\Omega_{\mathbb{A}^n_S/S}\cong \mathcal{O}_{\mathbb{A}^n_S}^{\oplus n}$.
...
For the projective space $\mathbb{P}^n_A=\Proj A[\x_0,\ldots, \x_n]$ over a ring $A$ ([§Projective Schemes, ⁋Definition 1](/en/math/scheme_theory/projective_schemes#def1){: data-relation="required" }), there exists a short exact sequence of $\mathcal{O}_{\mathbb{P}^n_A}$-modules
...
This agrees with the computation on varieties using the transition functions of $n$-forms. ([\[Algebraic Varieties\] §Canonical Line Bundle, ⁋Example 8](/en/math/algebraic_varieties/canonical_bundle#ex8){: data-relation="weak" })
:::
