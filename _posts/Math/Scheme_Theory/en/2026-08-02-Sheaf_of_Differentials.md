...
For the second claim, apply [Proposition 1](#prop1){: data-relation="required" } to the $A$-algebra $A_h$ and the $A_h$-algebra $B$ to obtain the exact sequence of $B$-modules
...
The actual gluing argument is as follows. First, shrinking $U$ to the principal open $D(g)$ defined by $g\in B$, the first claim of [Proposition 3](#prop3){: data-relation="required" } gives
...
Now let us see how these appear in $\Omega_{X/S}$. Since $B$ is $A[\x, \y]$ modulo the ideal $\mathfrak{a}=(\x\y-t)$, applying [Proposition 2](#prop2){: data-relation="required" } to the $A$-algebra $A[\x, \y]$ and its ideal $\mathfrak{a}$ gives the exact sequence of $B$-modules
...
:::
::: Proof
Set $B=A[\x_1,\ldots, \x_n]$. By [Definition 4](#def4){: data-relation="required" }, we have $\Omega_{\mathbb{A}^n_A/A}\cong \widetilde{\Omega_{B/A}}$, so it suffices to show that $\Omega_{B/A}$ is a free $B$-module with basis $\dd{\x_1},\ldots, \dd{\x_n}$.
...
and then over an affine open subset $\Spec A$ of $S$ this gives $\mathbb{A}^n_A$. ([§Fiber Products, ⁋Example 9](/en/math/scheme_theory/fiber_products#ex9){: data-lid="sfrnt" data-relation="required" }) Now, by [Definition 4](#def4){: data-relation="required" }, the cotangent sheaf is determined by the local model on each affine open of the base, so applying the above proposition on each chart yields $\Omega_{\mathbb{A}^n_S/S}\cong \mathcal{O}_{\mathbb{A}^n_S}^{\oplus n}$.
