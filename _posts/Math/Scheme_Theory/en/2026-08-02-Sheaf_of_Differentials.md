...
date: 2026-06-21
weight: 19
translated_at: 2026-08-07T09:45:04+00:00
translation_source: kimi-cli
---
Now we define the notion of a differential. Since analytic limits do not exist on an algebraic variety or a scheme, we must take a purely algebraic approach to define it, and the starting point is the Kähler differential module constructed in commutative algebra. In this post, we attach this as the cotangent sheaf of a scheme morphism, pass through the tangent sheaf and Zariski tangent space, and computations in affine and projective space, and finally organize the definition of the canonical sheaf and the statement of Serre duality.
...
for any $B$-module $M$. ([[Commutative Algebra] §Differentials, ⁋Lemma 2](/en/math/commutative_algebra/differentials#lem2)) That is, $\Omega_{B/A}$ is the $B$-module generated by elements $\dd{b}$ with the Leibniz rule $\dd{(xy)}=x\dd{y}+y\dd{x}$ and $A$-linearity as relations, and intuitively one may think of $\Omega_{B/A}$ as viewing elements of $A$ as constants and elements of $B$ as functions, then requiring the Leibniz rule accordingly.

Since the above $B$-module comes from the $A$-algebra structure of $B$, or in other words from the ring homomorphism $A\rightarrow B$, we may think of it as a scheme morphism $\Spec B\rightarrow \Spec A$. Then the $B$-module $\Omega_{B/A}$ defined above will be translated, in the language of schemes, as the quasi-coherent sheaf $\widetilde{\Omega_{B/A}}$ defined on $\Spec B$, and for a general scheme morphism $\varphi:X\rightarrow S$ we glue these to form $\Omega_{X/S}$. ([[Morphisms of Schemes] §Morphisms of Schemes, ⁋Proposition 1](/en/math/scheme_theory/morphism_of_schemes#prop1)) Then, according to the above intuition, we may think of this as fixing the base $S$ direction and considering only the fiber direction of $\varphi:X\rightarrow S$ as functions to take differentials.

Before beginning the main story, we first bring the algebraic tools into the form we will use. The Kähler differential module has the following two key exact sequences.
...
As mentioned above, we must glue these to define Kähler differentials for a scheme morphism. Since this essentially requires reducing both the base and the fiber sides to affines, we need gluing conditions for each of these.

The $\Omega$ defined above is functorial with respect to commutative squares of ring homomorphisms ([[Commutative Algebra] §Differentials, ⁋Proposition 6](/en/math/commutative_algebra/differentials#prop6)), so an $A$-algebra homomorphism $B \rightarrow B'$ induces a canonical $B'$-linear map $\Omega_{B/A}\otimes_BB' \rightarrow \Omega_{B'/A}$, and a change of base $A \rightarrow A'$ induces a canonical $B$-module homomorphism $\Omega_{B/A} \rightarrow \Omega_{B/A'}$. What is needed for gluing are the following cases where these canonical maps become isomorphisms.

::: Proposition 3
For an $A$-algebra $B$, the following hold.

1. For any $g\in B$, the canonical map gives a $B_g$-module isomorphism $(\Omega_{B/A})_g\cong\Omega_{B_g/A}$.
2. If the image of $h\in A$ is invertible in $B$ so that $B$ can be viewed as an $A_h$-algebra, then the canonical map gives an isomorphism $\Omega_{B/A}\cong\Omega_{B/A_h}$.
:::
::: Proof
...
However, the universal derivation $d:A \rightarrow \Omega_{A/A}$ satisfies

$$\dd{(1)}=\dd{(1\cdot 1)}=2\dd{(1)},$$

that is $\dd{(1)}=0$, and since it is $A$-linear, we have $\dd{a}=a\cdot \dd{(1)}=0$ for any $a\in A$, and therefore $\Omega_{A/A}=0$. Applying the first claim to $B=A$ and $g=h$ gives $\Omega_{A_h/A}\cong(\Omega_{A/A})_h=0$, so the first term of the above sequence vanishes and the remaining $\Omega_{B/A} \rightarrow \Omega_{B/A_h}$ is an isomorphism.
:::

Now we must glue the local models $\widetilde{\Omega_{B/A}}$ defined for each pair of affine open subsets $U=\Spec B\subseteq X$ and $V=\Spec A\subseteq S$ with $\varphi(U)\subseteq V$. This form of gluing was already encountered in [[Fiber Products] §Fiber Products, ⁋Theorem 8](/en/math/scheme_theory/fiber_products#thm8) when constructing fiber products, where we also reduced on the factor side and the base side respectively before gluing the pieces.

The actual gluing argument is as follows. First, if we shrink $U$ to the principal open $D(g)$ defined by $g\in B$, then the first claim of [Proposition 3](#prop3) gives

$$\Omega_{B_g/A}\cong (\Omega_{B/A})_g,$$

so by [[Quasi-Coherent Sheaves] §Quasi-Coherent Sheaves, ⁋Proposition 5](/en/math/scheme_theory/quasicoherent_sheaves#prop5) we have $\widetilde{\Omega_{B/A}}\vert_{D(g)}\cong \widetilde{\Omega_{B_g/A}}$. Reducing on the base side is similar: if we replace $V$ by a principal open $D(h)=\Spec A_h$ ($h\in A$) with $\varphi(U)\subseteq D(h)$, then the image of $h$ is invertible since it does not belong to any prime ideal of $B$, so by the second claim of [Proposition 3](#prop3) we have $\Omega_{B/A}\cong\Omega_{B/A_h}$ and the local model does not change. Since these are all canonical isomorphisms coming from functoriality, we can cover the overlap of two charts by open sets principal on both sides to canonically identify the local models, and these identifications satisfy the cocycle condition on triple intersections. That is, the following definition uniquely determines a sheaf on $X$. ([[Topology] §Sheaves, ⁋Proposition 8](/en/math/topology/sheaves#prop8))
...
for each pair of affine open subsets $U=\Spec B\subseteq X$ and $V=\Spec A\subseteq S$ with $\varphi(U)\subseteq V$. ([[Quasi-Coherent Sheaves] §Quasi-Coherent Sheaves, ⁋Definition 4](/en/math/scheme_theory/quasicoherent_sheaves#def4))
:::

What this $\Omega_{X/S}$ measures becomes apparent when we view $\varphi$ as a family parametrized by $S$ and perform a base change. The tool for this is that $\Omega_{X/S}$ behaves well under base change.
...
:::
::: Proof
For any $B'$-module $M$, restricting an $A'$-derivation $D:B' \rightarrow M$ to $B$ gives an $A$-derivation $B \rightarrow M$. Conversely, any $A$-derivation $D_0:B \rightarrow M$ extends uniquely to $b\otimes a'\mapsto a'D_0(b)$ by the Leibniz rule, so this restriction gives $\Der_{A'}(B', M)\cong\Der_A(B, M)$. But by [[Commutative Algebra] §Differentials, ⁋Lemma 2](/en/math/commutative_algebra/differentials#lem2) the left-hand side of this isomorphism is $\Hom_{B'}(\Omega_{B'/A'}, M)$, and the right-hand side is

$$\Hom_B(\Omega_{B/A}, M)\cong\Hom_{B'}(\Omega_{B/A}\otimes_BB', M)$$

([[Algebraic Structures] §Change of Base Ring, ⁋Proposition 6](/en/math/algebraic_structures/change_of_base_ring#prop6)), so the two $B'$-modules are canonically isomorphic since they represent the same functor. ([[Category Theory] §Representable Functors, ⁋Proposition 8](/en/math/category_theory/representable_functors#prop8))
:::

Since both sides are determined by their values on affine opens and the identification comes from the canonical one arising from the universal derivation, this isomorphism glues to the scheme level. That is, for a morphism $S' \rightarrow S$, letting $X'=X\times_SS'$ with projection $\pi:X' \rightarrow X$, we have $\Omega_{X'/S'}\cong \pi^\ast\Omega_{X/S}$. In particular, for a point $s\in S$ setting $S'=\Spec\kappa(s)$, since $X'$ is the fiber $X_s$ over $s$ ([[Fiber Products] §Fiber Products, ⁋Definition 12](/en/math/scheme_theory/fiber_products#def12)), letting $\iota:X_s \rightarrow X$ be the canonical morphism into $X$ we obtain
...
When the family is nontrivial, $\Omega_{X/S}$ records even the geometry of the fibers. For $A=\mathbb{K}[t]$ and $B=\mathbb{K}[t, \x, \y]/(\x\y-t)$, consider $\varphi:X=\Spec B \rightarrow S=\Spec A$. Then the relation gives $t=\x\y$ so $B\cong \mathbb{K}[\x, \y]$ and $X$ itself is the affine plane. However, to see this properly it is better to see how $\varphi$ makes $X$ into a family. Specifically, when $a\in \mathbb{K}$ is nonzero the fiber $X_a$ over $t=a$ is the hyperbola $\x\y=a$, but the fiber $X_0$ over $t=0$ degenerates into the two lines $\x\y=0$.
...
The middle term's $\Omega_{A[\x, \y]/A}$ is the free module with basis $\dd{\x}$ and $\dd{\y}$ ([[Commutative Algebra] §Differentials, ⁋Proposition 5](/en/math/commutative_algebra/differentials#prop5)) so this term is $B\dd{\x}\oplus B\dd{\y}$, and since $\mathfrak{a}$ is generated by $\x\y-t$ alone its quotient $\mathfrak{a}/\mathfrak{a}^2$ is likewise. Now using that $\dd{t}=0$ in $t\in A$, we have $\bar{d}(\x\y-t)=\x \dd{\y}+\y \dd{\x}$, so there is only this one relation and

$$\Omega_{B/A}\cong\bigl(B\dd{\x}\oplus B\dd{\y}\bigr)/(\x \dd{\y}+\y \dd{\x}).$$

Then we can see how this relation resolves on each fiber. First, over $t=a$ the element $\x$ is invertible so the relation becomes $\dd{\y}=-(\y/\x)\dd{\x}$, and $\Omega_{X_a/\mathbb{K}}$ is the free module of rank $1$ with basis $\dd{\x}$. On the other hand, on the fiber $X_0$ at the origin the situation changes slightly: for a point $q$ on the $x$-axis (not the origin) the maximal ideal $\mathfrak{m}_q$ does not contain $\x$, and conversely for a point $q$ on the $y$-axis the maximal ideal $\mathfrak{m}_q$ does not contain $\y$, so in both cases the relation is nontrivial over $\kappa(q)$. That is, $\Omega_{X/S}\otimes\kappa(q)$ becomes $1$-dimensional. At the origin $p$ where the two lines meet, both $\x$ and $\y$ belong to $\mathfrak{m}_p$ so the above relation vanishes and $\Omega_{X/S}\otimes\kappa(p)$ becomes the $2$-dimensional vector space generated by $\dd{\x}$ and $\dd{\y}$. Thus the dimension of the fiber $\Omega_{X/S}\otimes\kappa(q)$ jumps from $1$ to $2$ only at the intersection point of the two lines, which is information not visible from the fact that $\Omega_{X/\mathbb{K}}$, taking only differentials over $\mathbb{K}$ on $X$, is a free module of rank $2$.

The above definition can be used directly for computation, but its definition requires a choice of chart. A coordinate-independent description was already obtained at the algebraic level, because [[Commutative Algebra] §Differentials, ⁋Proposition 4](/en/math/commutative_algebra/differentials#prop4) gives a canonical isomorphism $\mathfrak{I}/\mathfrak{I}^2\cong\Omega_{B/A}$ for the kernel $\mathfrak{I}$ of the multiplication $m:B\otimes_AB \rightarrow B$. Translating this geometrically, $\Spec(B\otimes_AB)$ is the fiber product of $\Spec B$ over $\Spec A$ ([[Fiber Products] §Fiber Products, ⁋Lemma 2](/en/math/scheme_theory/fiber_products#lem2)) and what corresponds to $m$ is the diagonal morphism $\Delta$, so $\mathfrak{I}$ is the ideal of $\Delta$ and $\mathfrak{I}/\mathfrak{I}^2$ is its conormal module. That is, we have read off differentials in the first-order neighborhood of $\Delta$, and transferring this to schemes gives a description of $\Omega_{X/S}$ without choosing a chart.
...
For the projective space $\mathbb{P}^n_A=\Proj A[\x_0,\ldots, \x_n]$ over a ring $A$ ([§Projective Spaces and the Proj Construction, ⁋Definition 1](/en/math/scheme_theory/projective_schemes#def1)), there exists a short exact sequence of $\mathcal{O}_{\mathbb{P}^n_A}$-modules
...
This agrees with the computation on varieties using the transition functions of $n$-forms. ([\[Algebraic Varieties\] §Canonical Bundle, ⁋Example 8](/en/math/algebraic_varieties/canonical_bundle#ex8))
:::
...
Let $\Omega_{X/\mathbb{K}}$ be a locally free sheaf of rank $n$ for an $n$-dimensional integral projective scheme $X$ over an algebraically closed field $\mathbb{K}$ ([§Closed Subschemes of Projective Spaces, ⁋Definition 7](/en/math/scheme_theory/closed_subschemes_of_projective_spaces#def7), [§Algebra of Schemes, §§Reduced and Integral Schemes](/en/math/scheme_theory/algebra_of_schemes#reduced-and-integral-schemes)). Then for any locally free sheaf $\mathcal{E}$ on $X$ and any $0\leq i\leq n$, there exists an isomorphism
