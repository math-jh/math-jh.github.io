...
Conversely, assume that for any square there exists at most one lifting. By [Proposition 4](#prop4){: data-lid="e3n7c" data-relation="required" } it suffices to show that $\Delta(X)$ is a closed subset of $X\times_YX$.
...
Meanwhile from [Theorem 6](#thm6){: data-lid="wqkn1" data-relation="required" } we obtain the following.
...
The rest is obtained from the criterion of [Theorem 6](#thm6){: data-lid="w0ats" data-relation="required" }. That is, we only need to check that uniqueness of liftings is inherited.
...
Since a proper morphism requires both the separated condition and the universally closed condition, the criterion also splits into two pieces. [Theorem 6](#thm6){: data-lid="n623r" data-relation="required" } tested separatedness by uniqueness of liftings, so what remains is that existence of liftings tests universal closedness.
...
Let us examine the residue fields. The morphism $\kappa((0))=K \rightarrow K$ induced by $\pi\circ \tilde{\alpha}=\iota$ at $(0)$ is the identity, and this is the composition of the morphism $K\rightarrow\kappa(\xi)$ induced by $\pi$ and the morphism $\kappa(\xi) \rightarrow K$ induced by $\tilde{\alpha}$, so the two morphisms are inverse isomorphisms of each other. That is, $\kappa(\xi)\cong K$. Then since $Z$ is an integral scheme with generic point $\xi$, as in the proof of [Theorem 6](#thm6){: data-lid="0skwb" data-relation="required" } we choose an affine open subset containing $z$ to obtain
...
Conversely, assume the existence part holds. Following the convention stated after [Definition 9](#def9){: data-lid="sw2of" data-relation="required" }, we only consider base changes $Y' \rightarrow Y$ for Noetherian schemes.
...
Finally, we repeat the topological observation from the proof of [Theorem 6](#thm6){: data-lid="o4z6c" data-relation="required" } verbatim. If the generic points of the irreducible components $X_1,\ldots,X_r$ of $X$ are $\eta_1,\ldots,\eta_r$, then $X=\bigcup_{i=1}^r\cl(\{\eta_i\})$, so
...
:::
::: Proof
Since $\varphi$ is assumed finite type, by [Definition 9](#def9){: data-lid="y3pol" data-relation="required" } $\varphi$ is proper if and only if $\varphi$ is separated and universally closed. By [Theorem 6](#thm6){: data-lid="pfj5u" data-relation="required" }, $\varphi$ is separated if and only if any outer square has at most one lifting, and by [Proposition 10](#prop10){: data-lid="evq3p" data-relation="required" }, $\varphi$ is universally closed if and only if any outer square has at least one lifting. Both conditions holding together is exactly that every square has exactly one lifting.
:::
...
For item 3, separated is item 3 of [Corollary 8](#cor8){: data-lid="nkost" data-relation="required" }, finite type is stable under base change, and universally closed follows immediately from the definition since a base change of a base change is again a base change. Then item 4 follows by applying the same decomposition as in item 4 of [Corollary 8](#cor8){: data-lid="t17og" data-relation="required" } to items 2 and 3.
...
:::
::: Proof
A finite morphism $\varphi:X \rightarrow Y$ is an affine morphism, so for any affine open subset $V\subseteq Y$, $\varphi^{-1}(V)$ is affine, and hence by [Lemma 5](#lem5){: data-lid="pvdpg" data-relation="required" } and the affine-local criterion for closed embeddings ([§Closed Subschemes, ⁋Proposition 3](/en/math/scheme_theory/closed_subschemes#prop3){: data-lid="wadgf" data-relation="required" }), $\varphi$ is separated. Also a finite morphism is finite type. ([§Properties of Scheme Morphisms, ⁋Proposition 15](/en/math/scheme_theory/properties_of_scheme_morphisms#prop15){: data-lid="jgfwg" data-relation="required" }) Therefore by [Theorem 11](#thm11){: data-lid="8wfql" data-relation="required" } we only need to check existence of liftings.
...
The same criterion gives much larger results. A projective morphism is by definition a composition of a closed embedding and a projection $\mathbb{P}^n_Y \rightarrow Y$ ([Definition 1](#def1){: data-lid="2nr2q" data-relation="required" }), and the closed embedding side is already handled by item 1 of [Corollary 13](#cor13){: data-lid="f125a" data-relation="required" }, so what remains to check is that projective space is proper over the base. And this reduces along base changes to $\mathbb{P}^n_\mathbb{Z} \rightarrow \Spec\mathbb{Z}$ alone.
...
:::
::: Proof
The heart of the proof is that $\pi:\mathbb{P}^n_\mathbb{Z} \rightarrow \Spec\mathbb{Z}$ is proper, which is obtained by directly verifying the criterion of [Theorem 11](#thm11){: data-lid="cmkgi" data-relation="required" }. First, $\mathbb{P}^n_\mathbb{Z}$ is obtained by gluing $n+1$ affine charts

$$U_i=\Spec \mathbb{Z}[\x_0/\x_i,\ldots,\x_n/\x_i]\qquad (i=0,\ldots,n)$$

along $U_i\cap U_j=D(\x_j/\x_i)\subseteq U_i$. Here since $\x_i/\x_i=1$, each $U_i$ is the spectrum of an $n$-variable polynomial ring over $\mathbb{Z}$. In particular each $U_i$ is the spectrum of a Noetherian ring and the number of charts is finite, so $\mathbb{P}^n_\mathbb{Z}$ is a Noetherian scheme, and $\pi$ is finite type. ([§Properties of Scheme Morphisms, ⁋Definition 14](/en/math/scheme_theory/properties_of_scheme_morphisms#def14){: data-lid="ucjii" data-relation="required" })
...
this morphism is given by $\x_j/\x_k\mapsto a_j/a_k=b_j$, so restricting the $\Spec A \rightarrow U_k$ constructed above to $\Spec K$ gives exactly the same thing. That is, existence in the criterion holds, and by [Theorem 11](#thm11){: data-lid="wmpbt" data-relation="required" }, $\pi$ is proper.

For any Noetherian scheme $Y$, we have $\mathbb{P}^n_Y=\mathbb{P}^n_\mathbb{Z}\times_{\Spec\mathbb{Z}}Y$. This is because when $Y=\Spec B$, the chart $\Spec B[\x_0/\x_i,\ldots,\x_n/\x_i]$ of $\mathbb{P}^n_B$ coincides with $U_i\times_{\Spec\mathbb{Z}}\Spec B$, and for general $Y$ we glue these. Therefore by [Corollary 13](#cor13){: data-lid="71esr" data-relation="required" }, $\mathbb{P}^n_Y \rightarrow Y$ is proper, and in particular finite type, so $\mathbb{P}^n_Y$ is a Noetherian scheme.

Now if $\varphi:X \rightarrow Y$ is projective, then $\varphi$ is a composition of a closed embedding $X\hookrightarrow \mathbb{P}^n_Y$ and the projection $\mathbb{P}^n_Y \rightarrow Y$. ([Definition 1](#def1){: data-lid="4u0j5" data-relation="required" }) A closed embedding is proper and the composition of two proper morphisms is proper ([Corollary 13](#cor13){: data-lid="6gk09" data-relation="required" }), so $\varphi$ is proper.

Finally, let $\varphi:X \rightarrow Y$ be quasi-projective, and decompose it as $\varphi=\psi\circ\lambda$ where $\lambda: X \rightarrow X'$ is an open embedding and $\psi:X' \rightarrow Y$ is a projective morphism. ([Definition 1](#def1){: data-lid="on8fx" data-relation="required" }) By what we just showed, $\psi$ is proper, hence separated and finite type. Meanwhile an open embedding is separated, and the composition of two separated morphisms is separated ([Corollary 8](#cor8){: data-lid="pi4hp" data-relation="required" }), so $\varphi$ is separated. Also an open embedding is locally of finite type, and since $X$ is Noetherian, the preimage by $\lambda$ of any affine open subset of $X'$ is quasi-compact as an open subset of a Noetherian space. That is, $\lambda$ is finite type ([§Properties of Scheme Morphisms, ⁋Definition 14](/en/math/scheme_theory/properties_of_scheme_morphisms#def14){: data-lid="i1ubv" data-relation="required" }), and since the composition of two finite type morphisms is finite type, $\varphi$ is also finite type.
:::
