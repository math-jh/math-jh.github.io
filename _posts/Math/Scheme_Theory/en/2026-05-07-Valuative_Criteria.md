...
Finally, suppose $f:X \rightarrow Y$ is quasi-projective, and factor it as a composition $f=g\circ\iota$ of an open immersion $\iota: X \rightarrow X'$ and a projective morphism $g:X' \rightarrow Y$. ([Definition 1](#def1)) By what we just showed, $g$ is proper, hence separated and of finite type. On the other hand, open immersions are separated and the composition of two separated morphisms is separated ([Corollary 8](#cor8)), so $f$ is separated. Also an open immersion is locally of finite type, and since $X$ is Noetherian, the preimage of any affine open subset of $X'$ under $\iota$ is quasi-compact as an open subset of a Noetherian space. That is, $\iota$ is of finite type ([§Properties of Scheme Morphisms, ⁋Definition 13](/en/math/scheme_theory/properties_of_scheme_morphisms#def13)), and the composition of two finite type morphisms is of finite type, so $f$ is also of finite type.
:::
