...
we have $(\x^2,\x\y)=(\x)\cap(\x^2,\y)$, so $\Ass(A)=\{(\x),(\x,\y)\}$; this is the scheme with the origin embedded as an embedded point on the $\y$-axis. Here $\y$ does not belong to the unique minimal prime ideal $(\x)$, so $Z(\y)$ does not swallow any component and has codimension $1$, but $\x\y=0$ and $\x\neq 0$, so $\y$ is a zerodivisor with $\ann(\y)=(\x)$. Therefore $Z(\y)\hookrightarrow \Spec A$ has codimension $1$ but is not an effective Cartier divisor, which also means the converse of [Proposition 3](#prop3){: data-relation="required" } does not hold in general.

In both examples, the problem was that the local ring at the relevant point was not even a domain. If we exclude this situation, the converse of [Proposition 3](#prop3){: data-relation="required" } holds.
...
There is nothing to check at points not meeting $Z$. Since $Z$ is a closed set, such a point has an affine open neighborhood disjoint from $Z$, and on it $Z$ is $Z(1)=\emptyset$, so the condition of [Definition 1](#def1){: data-relation="required" } is satisfied with $s=1$.

Now choose $z\in Z$ and an affine open subset $U=\Spec A$ containing $z$. Since $Z$ is integral, $\mathfrak{p}=\mathcal{I}_{Z/X}(U)$ is a prime ideal, and by [§Dimension, ⁋Proposition 8](/en/math/scheme_theory/dimension#prop8){: data-relation="required" }, $Z$ having codimension $1$ means $\mathfrak{p}$ has codimension $1$. Let $\mathfrak{q}\supseteq \mathfrak{p}$ be the prime ideal corresponding to $z$; then by the factoriality assumption, $A_\mathfrak{q}=\mathcal{O}_{X,z}$ is a Noetherian local UFD, and $\mathfrak{p}A_\mathfrak{q}$ is a prime ideal of codimension $1$. Hence by part 1 of [\[Commutative Algebra\] §Homological Criterion for Regularity, ⁋Lemma 8](/en/math/commutative_algebra/homological_criterion_for_regularity#lem8), $\mathfrak{p}A_\mathfrak{q}$ is principal.
...
Then $D(g)$ is an affine open neighborhood of $z$ on which $Z$ is $Z(f)$; since $A_g$ is a domain and $f\neq 0$, $f$ is a non-zerodivisor. The open sets obtained in this way cover $X$, so by [Definition 1](#def1){: data-relation="required" }, $\iota$ is an effective Cartier divisor.
:::
...
:::
::: Proof
Given an effective $D=\{(U_i,f_i)\}$. That $f_i\in \Gamma(U_i,\mathcal{O}_X)$ is an element of $K(U_i)^\times$ means $f_i$ is a non-zerodivisor. Considering the ideal $(f_i)$ on each $U_i$, since $f_i/f_j$ is a unit on the overlap, these define the same ideal sheaf on overlaps, and hence glue to a single closed subscheme $Z$ of $X$ by [§Closed Subschemes, ⁋Proposition 6](/en/math/scheme_theory/closed_subschemes#prop6){: data-lid="l08fk" data-relation="required" }. This $Z$ satisfies the condition of [Definition 1](#def1){: data-relation="required" } by construction.

Conversely, given an effective Cartier divisor $\iota:Z\hookrightarrow X$ with data $\{(U_i,s_i)\}$ from [Definition 1](#def1){: data-relation="required" }. Since $s_i$ is a non-zerodivisor, $s_i\in K(U_i)^\times$. On overlaps, $\mathcal{I}_{Z/X}$ is generated by both $s_i$ and $s_j$, so at each point there is a local unit $u$ with $s_i=us_j$, and such $u$ is uniquely determined from $s_i$ and $s_j$ being non-zerodivisors and glues to a section over the entire overlap. Hence $\{(U_i,s_i)\}$ is an effective Cartier divisor. That the two constructions are mutual inverses is immediate since either way we are passing back and forth between elements that locally generate $\mathcal{I}_{Z/X}$.
:::

Henceforth we do not distinguish between an effective Cartier divisor and an effective element of $\CaDiv(X)$. A general Cartier divisor can be written locally on each $U_i$ as the ratio of two non-zerodivisors, so it is locally the difference of two effective Cartier divisors; in this sense $\CaDiv(X)$ is generated locally by the closed subschemes of [Definition 1](#def1){: data-relation="required" }.
...
Now we show $\mathcal{O}_X(D)\cong \mathcal{L}$. By [Proposition 9](#prop9){: data-relation="required" }, $\mathcal{O}_X(D)\vert_{U_i}$ is generated by $f_i^{-1}$, so for $V\subseteq U_i$ with $gf_i\in \Gamma(V,\mathcal{O}_X)$, the map
...
Before examining linear systems, we briefly review Weil divisors. As mentioned above, Weil divisors were defined only on better spaces than Cartier divisors, and our first topic is how to carry this over into the language of schemes. For Weil divisors to be defined, we must be able to define the order of zeros and poles of a rational function at each codimension $1$ part, which requires the local ring at that point to be a discrete valuation ring. However, what this condition actually entails is somewhat unclear, so we assume normality. ([§Algebraic Structure of Schemes, ⁋Definition 6](/en/math/scheme_theory/algebra_of_schemes#def6){: data-lid="m16bz" data-relation="required" }) Then in [Lemma 11](#lem11){: data-relation="forward" } we will verify that this assumption makes the local ring a DVR at codimension $1$ parts.
...
For $Y$ and its generic point $\eta$ satisfying part 1 of [Lemma 11](#lem11){: data-relation="required" }, choosing a uniformizer $\pi$ of the DVR $\mathcal{O}_{X, \eta}$ allows us to write any $f\in K(X)^\times$ uniquely as
...
The second result of [Lemma 11](#lem11){: data-relation="required" } guarantees the finiteness of this sum. Also, since each $\ord_Y$ is a group homomorphism, $\divisor:K(X)^\times \rightarrow \Div(X)$ is also a group homomorphism, so principal divisors form a subgroup of $\Div(X)$ and $\Cl(X)$ is well-defined.
