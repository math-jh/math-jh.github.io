...
In [§Morphisms of Schemes, ⁋Definition 3](/en/math/scheme_theory/morphism_of_schemes#def3){: data-relation="required" } we agreed to call a scheme morphism $X \rightarrow S$ an *$S$-scheme*. In this post we define the product in the category $\Sch_{/S}$.

::: Definition 1
The fiber product of two scheme morphisms $\varphi_X:X \rightarrow S$, $\varphi_Y:Y \rightarrow S$ is denoted $X\times_SY$. ([\[Category Theory\] §Limits, ⁋Example 8](/en/math/category_theory/limits#ex8){: data-relation="weak" })
:::
...
After [§Morphisms of Schemes, ⁋Example 4](/en/math/scheme_theory/morphism_of_schemes#ex4){: data-relation="weak" } we saw that any scheme $X$ can always be regarded as a $\mathbb{Z}$-scheme in a unique way. Thus, assuming that a fiber product $X\times_SY$ satisfying [Definition 1](#def1){: data-relation="required" } always exists, we know that for any two schemes $X, Y$, the object $X\times_{\Spec \mathbb{Z}}Y$ gives the product of $X$ and $Y$.

Since [Definition 1](#def1){: data-relation="required" } guarantees nothing about the existence of the fiber product $X\times_SY$, for this to be a genuine definition we must separately prove existence. ([Theorem 8](#thm8){: data-relation="forward" }) However, the existence of fiber products in $\AffSch$ is almost obvious, and this will be the starting point of our proof.
...
we can verify from [Lemma 3](#lem3){: data-relation="required" } that the open subscheme $\rho_Y^{-1}(Y')$ of $X\times_ZY$ is the fiber product. In general, if the two small squares in the following diagram
...
Earlier we agreed to think of a scheme morphism $X \rightarrow S$ as a family parametrized by $S$ ([§Morphisms of Schemes, ⁋Example 10](/en/math/scheme_theory/morphism_of_schemes#ex10){: data-relation="weak" }); from this viewpoint $S$ can be thought of as the base of the family $X$. Now, given an arbitrary $S$-family $X \rightarrow S$ and a scheme morphism $S' \rightarrow S$, through the fiber product we obtain a new $S'$-family $X\times_SS' \rightarrow S'$. From this perspective we often call the fiber product a *base change*.
...
Now, given in addition a scheme morphism $\Spec A \rightarrow \Spec C$, let us see what the above base change yields; by [Lemma 2](#lem2){: data-relation="required" } we know that what is obtained in this way is

$$\Spec A\times_{\Spec C}\Spec B=\Spec(A\otimes_CB) \rightarrow \Spec A,$$

that is, the ring homomorphism $A \rightarrow A\otimes_CB$. In other words, base change is (in the case of affine schemes) nothing other than [\[Algebraic Structures\] §Change of Base Ring, ⁋Definition 3](/en/math/algebraic_structures/change_of_base_ring#def3){: data-relation="weak" }.
:::
...
Now $X\times_ZY$ is, by [Lemma 2](#lem2){: data-relation="required" },
