...
---
In [§Schemes, ⁋Lemma 2](/en/math/scheme_theory/schemes#lem2){: data-relation="required" } we saw that for an affine scheme $\Spec A$, any element $f$ defines an open affine subscheme $D(f)\cong \Spec A_f$; in particular, to compare the two structure sheaves we applied [\[Topology\] §Sheaves, ⁋Lemma 11](/en/math/topology/sheaves#lem11){: data-relation="required" } to
...
On the other hand, by the second result of [§The Spectrum, ⁋Proposition 9](/en/math/scheme_theory/spectrums#prop9){: data-relation="required" }, given an affine scheme $\Spec A$ and an ideal $\mathfrak{a}$ of $A$, the $\Spec$ functor gives
...
But localization is an exact functor ([\[Commutative Algebra\] §Properties of Localization, ⁋Proposition 2](/en/math/commutative_algebra/properties_of_localization#prop2){: data-relation="required" }), so this morphism is surjective, and therefore $\varphi^\sharp$ is surjective. That is, $\varphi$ is a closed embedding.
...
is an isomorphism. This is a condition of exactly the same form as the localization condition required for ideals in [Proposition 6](#prop6){: data-relation="weak" }, but since we do not yet know whether $\varphi$ is an affine morphism, we cannot obtain this with the tools we have. Therefore we will only assert this fact without proof, and complete the remaining argument with the tools we already have. This fact is proved by [§Quasi-coherent Sheaves, ⁋Proposition 18](/en/math/scheme_theory/quasicoherent_sheaves#prop18){: data-relation="required" } as the quasi-coherence of $\varphi_\ast \mathcal{O}_X$.
...
First, from [§The Topological Structure of Schemes, ⁋Lemma 11 (Nike)](/en/math/scheme_theory/topology_of_schemes#lem11){: data-relation="required" } we can cover the intersection of $\Spec A_i$ and $\Spec A_j$ by principal open subsets
...
Then any locally closed embedding is always locally of finite type. To check this, fix an affine open subset $V=\Spec B$ of $Y$. First $Z\cap V$ is covered by principal open subsets $D(f)\cong \Spec B_f$ of $\Spec B$ ([§The Spectrum, ⁋Lemma 11](/en/math/scheme_theory/spectrums#lem11){: data-relation="required" }), and $B \rightarrow B_f$ is of finite type since it adjoins only $1/f$. Also since $\varphi\vert^Z$ is a closed embedding, by [Proposition 3](#prop3){: data-relation="required" } the preimage of each $D(f)$ is affine and its coordinate ring is a quotient of $B_f$, so the composition $B \rightarrow B_f \rightarrow B_f/\mathfrak{b}$ is also of finite type. That is, $\varphi^{-1}(V)$ is covered by affine open subsets on which $B \rightarrow \mathcal{O}_X(-)$ is of finite type, and therefore by [§Properties of Scheme Morphisms, ⁋Lemma 13](/en/math/scheme_theory/properties_of_scheme_morphisms#lem13){: data-relation="required" } we obtain the same conclusion for any affine open subset of $\varphi^{-1}(V)$.
...
and the restriction of $\iota_k$ to $\iota_k^{-1}(U)$ corresponds to the canonical projection $A \rightarrow A/\mathfrak{a}_k$. By assumption $\mathfrak{a}_2\subseteq \mathfrak{a}_1$, so $A \rightarrow A/\mathfrak{a}_1$ factors uniquely through $A \rightarrow A/\mathfrak{a}_2$, and the resulting $\pi_U: A/\mathfrak{a}_2 \rightarrow A/\mathfrak{a}_1$ is surjective. Therefore by the discussion right before [Definition 2](#def2){: data-relation="required" }, $\varphi_U=\Spec\pi_U: \iota_1^{-1}(U) \rightarrow \iota_2^{-1}(U)$ is a closed embedding, and by construction the restriction of $\iota_1$ to $\iota_1^{-1}(U)$ is the composition of the restriction of $\iota_2$ and $\varphi_U$.

Now it remains to show that $\varphi_U$ and $\varphi_{U'}$ agree on the intersection for two affine open subsets $U=\Spec A$, $U'$ of $X$. By [§The Topological Structure of Schemes, ⁋Lemma 11 (Nike)](/en/math/scheme_theory/topology_of_schemes#lem11){: data-relation="required" } we can cover $U\cap U'$ by open sets that are principal open in both $U$ and $U'$, and since localization is an exact functor ([\[Commutative Algebra\] §Properties of Localization, ⁋Proposition 2](/en/math/commutative_algebra/properties_of_localization#prop2){: data-relation="required" }), on such $D(f)\cong \Spec A_f$ we have $\mathcal{I}_{Z_k/X}(D(f))=\mathfrak{a}_kA_f$. Therefore both $\varphi_U$ and $\varphi_{U'}$ correspond on $D(f)$ to the canonical projection $A_f/\mathfrak{a}_2A_f \rightarrow A_f/\mathfrak{a}_1A_f$ induced by $\mathfrak{a}_2A_f\subseteq \mathfrak{a}_1A_f$, so they agree. Then by [§Morphisms of Schemes, ⁋Proposition 1](/en/math/scheme_theory/morphism_of_schemes#prop1){: data-relation="required" } they glue to a scheme morphism $\varphi: Z_1 \rightarrow Z_2$, and by construction $\iota_1=\iota_2\circ\varphi$.
...
::: Corollary 12
Let a scheme morphism $\varphi: X \rightarrow Y$ be given. If $X$ is reduced, or $\varphi$ is quasi-compact, then the ideal sheaf $\mathcal{I}$ defined above satisfies the condition of [Proposition 6](#prop6){: data-relation="required" } and therefore $\mathcal{I}$ defines a closed subscheme of $Y$, and this becomes the scheme-theoretic image of $\varphi$.
:::
::: Proof
...
Without the assumption of [Corollary 12](#cor12){: data-relation="weak" }, this does not happen.
...
and let $Y=\Spec \mathbb{K}[\x]$. Now on each component of $X$ we can define a scheme morphism $X \rightarrow Y$ via $\x\mapsto \epsilon$. Then from [Example 11](#ex11){: data-relation="required" } we know that the image of $X \rightarrow Y$ (as a continuous map) is the single point $0\in \mathbb{A}^1$.
