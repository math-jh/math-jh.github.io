...
and again by [\[Topology\] §Sheaves, ⁋Lemma 11](/en/math/topology/sheaves#lem11){: data-relation="required" },
...
:::
::: Proof
Let $x$ be an arbitrary point of the open subset $U$. Since $X$ is a scheme, there exist a suitable open neighborhood $V$ of $x$ (in $X$) and a ring $A$ such that $(V, \mathcal{O}_X\vert_V)\cong(\Spec A, \mathcal{O}_{\Spec A})$. Also, since $U$ is open, $U\cap V$ is an open subset of $V$. Now by [§The Spectrum, ⁋Lemma 11](/en/math/scheme_theory/spectrums#lem11){: data-relation="required" }, there exist $f_i\in A$ satisfying
...
In [§Affine Scheme, §§Algebraic functions on $\Spec A$](/en/math/scheme_theory/affine_schemes#algebraic-functions-on-spec-a){: data-relation="weak" }, we decided to think of the value of a function $f\in A$ at an arbitrary point $\mathfrak{p}$ as the image of $f$ in $A/\mathfrak{p}$. Applying this directly to a general scheme $(X, \mathcal{O}_X)$ is somewhat tricky, not only because this process is unclear in $(X, \mathcal{O}_X)$, but also because even if we repeat the above discussion using an affine open neighborhood of $x\in X$, it is not obvious that this is independent of the choice of affine open neighborhood.
...
::: Example 7
In [\[Algebraic Varieties\] §Affine Varieties, ⁋Definition 1](/en/math/algebraic_varieties/affine_varieties#def1){: data-relation="weak" } we defined the (classical) affine $n$-space as
...
Now consider the set $X=\coprod_{i\in I}X_i\big/{\sim}$ and the canonical map $\sigma_i: X_i \rightarrow X$, and give $X$ the final topology with respect to the $\sigma_i$. ([\[Topology\] §Initial and Final Topology, ⁋Definition 4](/en/math/topology/initial_and_final_topology#def4){: data-relation="required" }) Then by [\[Topology\] §Initial and Final Topology, ⁋Proposition 5](/en/math/topology/initial_and_final_topology#prop5){: data-relation="required" }, $U\subseteq X$ is open if and only if $\sigma_i^{-1}(U)$ is open in $X_i$ for all $i$, which is exactly the quotient space topology on $\coprod_{i\in I} X_i$. ([\[Topology\] §Quotient Spaces, ⁋Definition 3](/en/math/topology/quotient_spaces#def3){: data-relation="weak" })
...
The last calculation in [Example 10](#ex10){: data-relation="required" } above shows that $\mathbb{P}^1$ is not an affine scheme. If $\mathbb{P}^1$ were an affine scheme, it would necessarily have to be $\Spec \mathbb{K}$, because $\Spec \mathbb{K}$ is a scheme consisting of a single point.
