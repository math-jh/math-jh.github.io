...
::: Definition 1
A locally ringed space $$(X, \mathcal{O}_X)$$ is called a *scheme* if for every $$x\in X$$, there exists an open neighborhood $$U$$ of $$x$$ such that $$(U, \mathcal{O}_X\vert_U)$$ is an affine scheme. Morphisms and isomorphisms between schemes are defined as those of locally ringed spaces.
:::
...
::: Lemma 2
For an affine scheme $$\Spec A$$ and $$f\in A$$, the set $$D(f)$$ is always an affine scheme via the isomorphism ($$\ast\ast$$) above.
:::
::: Proof
...
::: Definition 4
For an arbitrary open subset $$U$$ of a scheme $$(X, \mathcal{O}_X)$$, we call the scheme $$(U, \mathcal{O}_X\vert_U)$$ an *open subscheme* of $$X$$.
:::
...
::: Definition 5
Consider a scheme $$(X, \mathcal{O}_X)$$, a point $$x\in X$$, and the local ring $$(\mathcal{O}_{X, x}, \mathfrak{m}_x)$$. The *residue field* of $$X$$ at $$x$$ is defined as $$\mathcal{O}_{X,x}/\mathfrak{m}_x$$, and we denote it by $$\kappa(x)$$. For a function $$f\in \mathcal{O}_X(U)$$ defined on an open subset $$U$$ containing $$x$$, the *value* of $$f$$ at $$x$$ is defined as the image of $$f$$ in $$\kappa(x)$$.
:::
...
::: Definition 6
For a scheme $$(X, \mathcal{O}_X)$$ and $$f\in \mathcal{O}_X(X)$$, the *support* of $$f$$ is given by
...
::: Example 7
In [§Spectra, §§Classical Algebraic Geometry](/en/math/scheme_theory/spectrums#classical-algebraic-geometry), we defined the (classical) affine $$n$$-space as
