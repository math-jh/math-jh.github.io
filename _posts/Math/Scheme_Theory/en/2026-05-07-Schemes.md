...
<ins id="def1">**Definition 1**</ins> A locally ringed space $$(X, \mathcal{O}_X)$$ is called a *scheme* if for every $$x\in X$$, there exists an open neighborhood $$U$$ of $$x$$ such that $$(U, \mathcal{O}_X\vert_U)$$ is an affine scheme. Morphisms and isomorphisms between schemes are defined as those of locally ringed spaces.
...
In general, the restriction $$(U, \mathcal{O}_{\Spec A}\vert_U)$$ of an affine scheme $$(\Spec A, \mathcal{O}_{\Spec A})$$ to an *arbitrary* open subset is not always an affine scheme. ([Example 8](#ex8)) However, a principal open subset $$D(f)$$ of an affine scheme $$\Spec A$$ is always an affine scheme via the isomorphism
...
<ins id="lem3">**Lemma 3**</ins> Let $$U$$ be an arbitrary open subset of a scheme $$(X, \mathcal{O}_X)$$. Then $$(U, \mathcal{O}_X\vert_U)$$ is also a scheme.

</div>
<details class="proof" markdown="1">
<summary>Proof</summary>

Let $$x$$ be an arbitrary point of the open subset $$U$$. Since $$X$$ is a scheme, there exist a suitable open neighborhood $$V$$ of $$x$$ (in $$X$$) and a ring $$A$$ such that $$(V, \mathcal{O}_X\vert_V)\cong(\Spec A, \mathcal{O}_{\Spec A})$$. Also, since $$U$$ is open, $$U\cap V$$ is an open subset of $$V$$. Now by [§Spectra, ⁋Lemma 11](/en/math/scheme_theory/spectrums#lem11), there exist $$f_i\in A$$ satisfying
...
<ins id="def4">**Definition 4**</ins> For an arbitrary open subset $$U$$ of a scheme $$(X, \mathcal{O}_X)$$, we call the scheme $$(U, \mathcal{O}_X\vert_U)$$ an *open subscheme* of $$X$$.
...
<ins id="def5">**Definition 5**</ins> Consider a scheme $$(X, \mathcal{O}_X)$$, a point $$x\in X$$, and the local ring $$(\mathcal{O}_{X, x}, \mathfrak{m}_x)$$. The *residue field* of $$X$$ at $$x$$ is defined as $$\mathcal{O}_{X,x}/\mathfrak{m}_x$$, and we denote it by $$\kappa(x)$$. For a function $$f\in \mathcal{O}_X(U)$$ defined on an open subset $$U$$ containing $$x$$, the *value* of $$f$$ at $$x$$ is defined as the image of $$f$$ in $$\kappa(x)$$.
...
and thus we see that [Definition 5](#def5) generalizes the notion of function values on affine schemes correctly. From this expression, for any $$f\in \mathcal{O}_X(X)$$, let us define
...
<ins id="def6">**Definition 6**</ins> For a scheme $$(X, \mathcal{O}_X)$$ and $$f\in \mathcal{O}_X(X)$$, the *support* of $$f$$ is given by
