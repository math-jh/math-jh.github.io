...
::: Definition 7
The sheaf on $\Spec A$ defined by [Lemma 6](#lem6){: data-lid="064f0" } is denoted $\mathcal{O}_{\Spec A}$ and called the *structure sheaf*.
:::
...
::: Definition 10
The essential image of the functor $\Spec:\cRing^\op \rightarrow \LRS$ from [Proposition 9](#prop9){: data-lid="82zvo" } is called an *affine scheme*.
:::
...
Setting $f=1$ in [Lemma 8](#lem8){: data-lid="e4iwn" }, we obtain the following diagram
...
But in the construction of [Proposition 9](#prop9){: data-lid="o2pn8" }, $(\Spec\phi)^\sharp(D(f))$ was exactly the unique map satisfying this equation, so by the uniqueness in [\[Commutative Algebra\] §Localization, ⁋Proposition 6](/en/math/commutative_algebra/localization#prop6){: data-lid="xz6we" }, $\varphi^\sharp(D(f))=(\Spec\phi)^\sharp(D(f))$. Since two sheaf morphisms agreeing on the base $\{D(f)\}_{f\in A}$ are equal, by the identity axiom for $\varphi_\ast\mathcal{O}_{\Spec B}$ we have $\varphi^\sharp=(\Spec\phi)^\sharp$.
:::

Thus, viewing $\Spec$ as a contravariant functor from $\cRing$ to $\AffSch$, it is a categorical equivalence between $\cRing^\op$ and $\AffSch$. Moreover, by [Proposition 11](#prop11){: data-lid="55ndo" }, $\AffSch$ is a full subcategory of $\LRS$.
...
Then, just as in the proof of [Proposition 9](#prop9){: data-lid="3y1cc" }, the maps given on the base extend uniquely to a sheaf morphism $\varphi^\sharp$ by the gluability and identity axioms of $\varphi_\ast\mathcal{O}_X$.

Finally, we show $(\varphi,\varphi^\sharp)$ is a morphism of locally ringed spaces. For any $x\in X$ and $\mathfrak{p}=\varphi(x)$, by [Lemma 8](#lem8){: data-lid="mmdq8" } we have $\mathcal{O}_{\Spec A,\mathfrak{p}}\cong A_\mathfrak{p}$, and under this identification the stalk morphism $\varphi_x^\sharp: A_\mathfrak{p} \rightarrow \mathcal{O}_{X,x}$ induced by $\varphi^\sharp$ satisfies, taking germs at $x$ of both sides of the equation $\varphi^\sharp(D(f))\circ\epsilon_f=\theta_f$,
...
That is, $\varphi_x^\sharp$ is a local homomorphism, and by [Definition 2](#def2){: data-lid="silke" }, $\Psi(\phi)=(\varphi,\varphi^\sharp)$ is a morphism of locally ringed spaces.
...
Also, given a ring homomorphism $\theta: A \rightarrow A'$, from the construction in [Proposition 9](#prop9){: data-lid="cc24r" } with $f=1$ we have $(\Spec\theta)^\sharp(\Spec A)=\theta$, so for any $\varphi: X \rightarrow \Spec A'$,
