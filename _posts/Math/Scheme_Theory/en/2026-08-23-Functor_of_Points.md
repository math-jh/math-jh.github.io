...
date: 2026-08-23
weight: 22
translated_at: 2026-08-27T11:45:05+00:00
...
In the language introduced in the previous section, $\mathbb{A}^1$ represents the global section functor $T\mapsto\Gamma(T,\mathcal{O}_T)$. Here the universal element is the one corresponding to the identity morphism $\id_{\mathbb{A}^1}$ in $h_{\mathbb{A}^1}(\mathbb{A}^1)=\Hom_\Sch(\mathbb{A}^1,\mathbb{A}^1)$, and chasing through the correspondence of [Proposition 1](#prop1) above, we know that this corresponds to $\x$ in $\Gamma(\mathbb{A}^1,\mathcal{O}_{\mathbb{A}^1})=\mathbb{Z}[\x]$.
...
:::
::: Proof
As in the proof of [Proposition 1](#prop1), a ring homomorphism out of the free ring $\mathbb{Z}[\x_1,\ldots, \x_n]$ is the same as freely choosing the images $a_i\in \Gamma(T, \mathcal{O}_T)$ of the generators $\x_i$, so we obtain
...
Now, to see how [Theorem 5](#thm5) works, let us look at its isomorphism classes: since the automorphisms of $\mathcal{O}_T$ are now only multiplication by elements of $A^\times$, two tuples give the same $A$-point if and only if they are $A^\times$-multiples of each other. Therefore
...
::: Example 6
We now concretely examine the functor defined by the projective space considered above. First, the $\mathcal{L}$ and globally generating sections $s_0,\ldots, s_n$ representing $\mathbb{P}^n(T)$ can be rewritten as the surjection
