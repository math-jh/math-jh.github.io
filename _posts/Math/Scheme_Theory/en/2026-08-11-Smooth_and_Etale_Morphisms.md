...
To be a regular local ring, it must be a Noetherian local ring whose maximal ideal is generated by $\dim$ many elements ([\[Commutative Algebra\] §Krull Dimension, ⁋Definition 12](/en/math/commutative_algebra/Krull_dimension#def12){: data-relation="weak" }), and by [\[Commutative Algebra\] §Integral Extensions, ⁋Lemma 8](/en/math/commutative_algebra/integral_extension#lem8){: data-relation="required" } this is equivalent to the equality above, so $\mathcal{O}_{X_{\overline{s}},z}$ is a regular local ring.
...
:::
::: Proof
By [Proposition 3](#prop3){: data-relation="required" }, what we must show is that on some neighborhood of $x$, $\varphi$ is flat, $\Omega_{X/S}$ is a locally free sheaf of rank $n-r$, and at each point of that neighborhood the fiber has local dimension $n-r$. First we factor $\varphi$ as in the following diagram:
...
:::
::: Proof
With notation as in the proof of [Theorem 4](#thm4){: data-relation="required" } above, write $\mathfrak{a}=(g_1,\ldots, g_m)$ and regard $X_{\overline{\mathbb{K}}}$ as the closed subscheme of $\mathbb{A}^n_{\overline{\mathbb{K}}}$ defined by $\mathfrak{a}\overline{\mathbb{K}}[\x_1,\ldots, \x_n]$. From the proof of [Lemma 2](#lem2){: data-relation="required" } we know that $X_{\overline{\mathbb{K}}} \rightarrow X$ is surjective, so for any given $x$ we may choose a point $\overline{x}\in X_{\overline{\mathbb{K}}}$ lying over it.
...
In the previous section we claimed that an unramified morphism has $0$-dimensional fiber directions, and the computation in the above proof supports this. On the other hand, like [Theorem 6](#thm6){: data-relation="weak" }, étale morphisms also admit the following standard form.
...
However, unlike the proof of [Theorem 6](#thm6){: data-relation="weak" }, the proof of this theorem is somewhat technical, so we omit it. Another important point is that the derivative condition here is the algebraic expression of the separability condition that $f=0$ has no multiple roots; from this one can see that the inseparable example above is not an étale morphism.
...
For $f\in\mathfrak{a}$, the value of this derivation is the class of $f$ in $\mathfrak{a}/\mathfrak{a}^2$, so $r\circ\bar{d}=\id_{\mathfrak{a}/\mathfrak{a}^2}$. That is, the leftmost morphism of the conormal sequence is a split injection, and by [Proposition 8](#prop8){: data-relation="required" }, $\varphi$ is smooth.
:::
