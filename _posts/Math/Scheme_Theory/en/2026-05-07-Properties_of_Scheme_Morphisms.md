...
As before, let $V\cong \Spec B$ and $U\cong\Spec A\subseteq \varphi^{-1}(V)$. Then the scheme morphism $\varphi\vert_U: U \rightarrow V$ can be viewed as $\Spec A \rightarrow \Spec B$, and what we require is that the corresponding ring homomorphism $B \rightarrow A$ is of finite type. Then finite type morphisms are defined as follows.
...
Then the geometric intuition for finite morphisms in [Example 15](#ex15) is always true. That is, any finite morphism is always quasi-finite. It is possible to prove this right now, but we postpone it until after we define fiber products.
