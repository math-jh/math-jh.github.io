...
date: 2025-02-21
weight: 9
translated_at: 2026-07-13T19:00:02+00:00
translation_source: kimi-cli
---
In the previous post, we examined several perspectives on scheme morphisms. In this post, we begin in earnest to define the properties that scheme morphisms possess. First, we define the following common property shared by them.

::: Definition 1
A property $P$ of a scheme morphism is said to be *local on target* if the following two conditions hold.
...
2. If for a scheme morphism $\varphi:X \rightarrow Y$, there exists an open covering $\{V_j\}$ of $Y$ such that all $\varphi\vert_{\varphi^{-1}(V_j)}: \varphi^{-1}(V_j) \rightarrow V_j$ satisfy $P$, then so does $\varphi$.
:::

Schemes are built from affine schemes. If a property $P$ of a scheme morphism is local on target, we may assume the target $Y$ of a scheme morphism $\varphi:X \rightarrow Y$ to be $\Spec B$, and then via the adjunction
...
:::
::: Proof
Suppose any affine open subset $V\subseteq Y$ is given, and we must show that $\varphi^{-1}(V)$ is quasi-compact. However, from the first result of [\[Topology\] §Dimension, ⁋Proposition 12](/en/math/topology/dimension#prop12) and [\[Topology\] §Dimension, ⁋Proposition 13](/en/math/topology/dimension#prop13), any subspace of a Noetherian topological space is quasi-compact.
:::
...
Then the composition of affine morphisms being affine is obvious. Moreover, this property also satisfies the property of [Definition 1](#def1), but the proof for this is somewhat lengthy, so we omit it.
