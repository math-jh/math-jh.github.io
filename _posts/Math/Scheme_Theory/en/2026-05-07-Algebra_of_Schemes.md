...
::: Proposition 4
$$X$$ is integral if and only if it is reduced and irreducible.
:::
::: Proof
...
::: Proposition 5
A Noetherian scheme $$X$$ is integral if and only if it is nonempty and connected and each stalk $$\mathcal{O}_{X,x}$$ is an integral domain.
:::
::: Proof
...
::: Definition 6
A scheme $$X$$ is *normal* if for every $$x\in X$$, the stalk $$\mathcal{O}_{X,x}$$ is a normal domain. ([\[Commutative Algebra\] §Integral Extensions, ⁋Definition 3](/en/math/commutative_algebra/integral_extension#def3))
:::
...
::: Definition 7
A scheme $$X$$ is *factorial* if for every $$x\in X$$, the stalk $$\mathcal{O}_{X,x}$$ is a unique factorization domain.
:::
...
::: Example 13
Consider the affine scheme $$X=\Spec \mathbb{K}[\x_1,\x_2]/(\x_2-\x_1^2)$$. Then $$X$$ has a unique associated prime $$(0)$$, and any open subset of $$X$$ contains this point, so a rational function on $$X$$ consists of any nonempty open subset $$U$$ together with a function $$f\in\Gamma(U, \mathcal{O}_X)$$ on it.
