...
Similarly, the spectrum of an integral domain is an integral scheme. This is not difficult to show directly, but in [Proposition 4](#prop4){: data-relation="forward" } we prove that a scheme $X$ is integral if and only if $X$ is an irreducible, reduced scheme. Then the spectrum $\Spec A$ of an integral domain $A$ is
...
For the proof of [Proposition 4](#prop4){: data-relation="forward" }, it is useful to rewrite irreducibility in algebraic language as follows.
...
For the opposite direction, that the scheme $X$ is reduced is trivial because every integral domain is reduced and reducedness is a stalk-local property. Therefore, using the given conditions to show that $X$ is irreducible, the rest follows trivially from [Proposition 4](#prop4){: data-relation="required" }.
...
2. Let $x$ be a point where two irreducible components meet; then any open neighborhood of $x$ will contain the generic point of each irreducible component ([§The Spectrum, ⁋Proposition 16](/en/math/scheme_theory/spectrums#prop16){: data-relation="required" }),
...
Then examining where this condition comes from is essential for understanding [Definition 12](#def12){: data-relation="required" }. First, by [Proposition 10](#prop10){: data-relation="required" }, for any associated point $\mathfrak{p}$ of $X=\Spec A$,

$$\mathfrak{p}=\ann(f),\qquad \supp(f)=Z(\mathfrak{p})$$

there exists a nonzero function $f\in \Gamma(X, \mathcal{O}_X)$ satisfying this, and this satisfies the condition of [Definition 12](#def12){: data-relation="required" }, so the pair $(X, f)$ defines some nonzero rational function along ($\ast$).
...
For example, consider $Z(\x\y)=\Spec \mathbb{K}[\x,\y]/(\x\y)$ from [§The Topological Structure of Schemes, ⁋Example 6](/en/math/scheme_theory/topology_of_schemes#ex6){: data-lid="udwon" data-relation="weak" }. This scheme has two irreducible components, the $y$-axis $Z(\x)$ and the $x$-axis $Z(\y)$, whose generic points are $(\x)$ and $(\y)$ respectively. Also, since $\ann(\y)=(\x)$, the function $f$ obtained from the result of [Proposition 10](#prop10){: data-relation="required" } is precisely $\y$, and indeed $\supp(\y)=Z(\x)$ shows that this gives the entire $y$-axis. Now if we consider an open subset $U=D(\x)$ that excludes this generic point $(\x)$, then from $\x\y=0$,
...
From the above, the total quotient ring $K(X)$ formed by all rational functions on $X$, using the same notation for elements as in [Example 11](#ex11){: data-relation="required" }, becomes
...
What makes this different from the classical function field is that the above $K(X)$ contains nonzero nilpotents, and this is precisely due to the embedded point. Specifically, $\x_2$ is a nilpotent element satisfying $\x_2^2=0$ but becomes a nonzero function in $K(X)$. If [Definition 12](#def12){: data-relation="required" } had not required this embedded point to be contained in the domain of definition, then $D(\x_1)$ excluding the origin would also have been allowed as a domain of definition, and as seen in [Example 11](#ex11){: data-relation="required" }, $\x_2$ is already $0$ on it, so $\x_2$ would have disappeared in $K(X)$, and thus the nilpotent-directional thickening of $X$ would not have been detected in $K(X)$.
:::
