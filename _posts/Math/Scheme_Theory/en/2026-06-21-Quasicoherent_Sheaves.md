...
The finite type condition spreads generation at a single point out to a neighborhood of that point. Suppose $\mathcal{F}$ is of finite type and $\mathcal{F}\vert_U\cong\widetilde M$ on an affine open neighborhood $U=\Spec A$ provided by [Definition 11](#def11). Writing $\mathfrak{p}$ for the prime corresponding to a point $x\in U$, if the images of $m_1,\ldots, m_r\in M$ generate the stalk $M_\mathfrak{p}$, then $N=M/\sum_iAm_i$ is finitely generated with $N_\mathfrak{p}=0$, so each generator of $N$ is annihilated by some element outside $\mathfrak{p}$, and since $\mathfrak{p}$ is prime their product $f$ also lies outside $\mathfrak{p}$, whence $fN=0$ gives $N_f=0$. That is, sections generating a stalk always generate all of $\mathcal{F}$ on the neighborhood $D(f)$ of that point.
...
What corresponds to the fiber of a vector bundle over each point also carries over verbatim to the language of sheaves. For an $\mathcal{O}_X$-module $\mathcal{F}$ and a point $x\in X$, using the residue field $\kappa(x)=\mathcal{O}_{X,x}/\mathfrak{m}_x$ defined in [§Schemes](/en/math/scheme_theory/schemes), we define the *fiber* of $\mathcal{F}$ at $x$ to be

$$\mathcal{F}\otimes\kappa(x)=\mathcal{F}_x\otimes_{\mathcal{O}_{X,x}}\kappa(x)=\mathcal{F}_x/\mathfrak{m}_x\mathcal{F}_x$$

This is to be distinguished from the stalk, since the stalk $\mathcal{F}_x$ is an $\mathcal{O}_{X,x}$-module remembering all germs around $x$, whereas the fiber pushes it down by the maximal ideal and retains only the value at that point as a $\kappa(x)$-vector space. For instance, the stalk of $\mathcal{O}_X$ is the entire local ring $\mathcal{O}_{X,x}$ while its fiber is $\kappa(x)$, and if $\mathcal{E}$ is a locally free sheaf of rank $r$, then on a neighborhood trivializing it the stalk is $\mathcal{O}_{X,x}^{\oplus r}$ and the fiber is $\kappa(x)^{\oplus r}$, so that the fiber recovers exactly the fiber of the corresponding vector bundle over $x$. Of course, for a general quasi-coherent sheaf the dimension of the fiber may vary as the point moves, and for a finitely generated module on an affine, the upper semicontinuity of this dimension is [§Flat Morphisms, ⁋Proposition 22](/en/math/scheme_theory/flat_morphisms#prop22).
