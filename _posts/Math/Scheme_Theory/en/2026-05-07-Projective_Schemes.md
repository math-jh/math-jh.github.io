...
description: "We construct projective space by gluing affine lines in a suitable way, and generalize this to define projective schemes. From a topological perspective, we understand projective space and examine how to glue affine lines via stereographic projection and cocycle conditions."
...
Thus, for the point on the right-hand side to equal
...
Similarly, matching a point of $P_j$ to a point of $P_i$ would give formulas like $x_{k/j}/x_{i/j}=x_{k/i}$, but this is not a new formula because it follows from $x_{i/j}=1/x_{j/i}$.
...
so $f$ is not well-defined. Instead, if we take $f$ to be a *homogeneous polynomial*, then although $f$ is not well-defined as a function, its zero locus $Z(f)$ is well-defined. This is because the formula
...
::: remark Remark {#rmk}
Unless otherwise stated, a graded ring is always assumed to be $\mathbb{N}_{\geq0}$-graded. That is, the ring of interest to us is always of the form
...
is trivially a homogeneous ideal of $A_\bullet$. However, considering the case $A_\bullet=\mathbb{K}[\x_0,\ldots, \x_n]$, the point where all elements of $A_+$ vanish, that is, the point that is identically zero for all polynomials, is only the origin. Since the origin is the point removed when constructing $\mathbb{P}^n$, it is appropriate to exclude from our discussion ideals containing the ideal $A_+$. From this perspective, we call $A_+$ the *irrelevant ideal*.
...
By definition, $\Proj A_\bullet$ is a subset of $\Spec A$. That is, all points of $\Proj A_\bullet$ are also points of $\Spec A$. This would be a somewhat awkward result if we had used $\MaxSpec A$ instead of $\Spec A$, but $\Spec A$ contains, in addition to traditional points, points corresponding to prime ideals. For example, considering the ideal $(\x_1-\x_2)$ of $A=\mathbb{K}[\x_1,\x_2]$, since $\mathbb{K}[\x_1,\x_2]/(\x_1-\x_2)\cong \mathbb{K}[\x_1]$, this ideal is a prime ideal. Moreover, this ideal is a homogeneous prime ideal not containing $A_+$ when $\mathbb{K}[\x_1,\x_2]$ is viewed as a graded ring $A_\bullet$, so it is also a point of $\Proj A_\bullet$.
...
Of course, it is trivial that $\mathfrak{a}\mathfrak{b}$, $\sqrt{\mathfrak{a}}$, and $\sum \mathfrak{a}_i$ appearing in the above lemma are homogeneous. Then the first through third results are already observed results for spectra, and only the fourth result is new.
...
::: Definition 4
Let a graded ring $A_\bullet$ be given. For any homogeneous ideal $\mathfrak{a}$, we call the unique topology on $\Proj A_\bullet$ having sets of the form $Z_+(\mathfrak{a})$ as closed sets the *Zariski topology*.
:::

Also, by the fourth result of this lemma, we know that in defining $\Proj A_\bullet$ we only need to consider homogeneous ideals contained in $A_+$. This is intuitively obvious as well: if we let $A=\mathbb{K}[\x_0,\ldots, \x_n]$, elements in $A_0$ are constant functions anyway.
...
:::
On the other hand, we have seen that in the spectrum $\Spec A$ of a ring $A$, for any element $f\in A$, the set $D(f)$ is isomorphic (as a scheme) to $\Spec A_f$. A similar result holds for $D_+(f)$.
...
and from the fact that $\mathfrak{q}$ is a prime ideal we know that $\mathfrak{p}$ is a prime ideal. Also $f^{\deg f}/f^{\deg f}=1\not\in \mathfrak{q}$ so $f\not\in \mathfrak{p}$, and hence $\mathfrak{p}$ does not contain $A_+$, so $\mathfrak{p}\in D_+(f)$. Now one can easily check that this correspondence $\mathfrak{p}\mapsto \mathfrak{p}A_f\cap A_{(f)}$ and $\mathfrak{q}\mapsto \mathfrak{p}$ are inverse to each other, and for any homogeneous ideal $\mathfrak{a}$ of $A_\bullet$, the closed set $Z_+(\mathfrak{a})\cap D_+(f)$ of $D_+(f)$ is carried by this function to the closed set $Z(\mathfrak{a}A_f\cap A_{(f)})$ of $\Spec A_{(f)}$, so we see that this is a homeomorphism.
:::
...
Finally, let us verify that this isomorphism is compatible with the homeomorphism of [Lemma 8](#lem8). Letting the image of $\mathfrak{p}\in D_+(f)$ be $\mathfrak{q}=\mathfrak{p}A_f\cap A_{(f)}$, since $f\not\in \mathfrak{p}$,
...
defined above satisfy all the conditions of [§Schemes, ⁋Lemma 9](/en/math/scheme_theory/schemes#lem9), and hence give a unique scheme structure on $\Proj A_\bullet$.
:::
::: Proof
As the index set we take all nonzero homogeneous elements of $A_+$. For two elements $f,g$, as in the proof of [Lemma 9](#lem9), write

$$\theta_{f,g}=\frac{g^{\deg f}}{f^{\deg g}}\in A_{(f)}$$

Then what the proof of that lemma gave us is that the canonical ring homomorphism $\rho_{f,fg}:A_{(f)} \rightarrow A_{(fg)}$ sends $\theta_{f,g}$ to a unit, and the induced map

$$\Phi_{f,g}:(A_{(f)})_{\theta_{f,g}} \rightarrow A_{(fg)}$$

is an isomorphism extending $\rho_{f,fg}$. Now letting $X_f=\Spec A_{(f)}$ and its open subscheme $X_{fg}=D(\theta_{f,g})$, and noting that $A_{(fg)}=A_{(gf)}$, we define the isomorphism $\varphi_{fg}:X_{fg} \rightarrow X_{gf}$ as the spectrum of the ring isomorphism

$$\Phi_{f,g}^{-1}\circ \Phi_{g,f}:(A_{(g)})_{\theta_{g,f}} \rightarrow (A_{(f)})_{\theta_{f,g}}$$

Here, if $fg=0$, then $\theta_{f,g}=0$ and $\theta_{g,f}=0$, so $X_{fg}$ and $X_{gf}$ are both empty, and in this case we let $\varphi_{fg}$ be the identity of the empty scheme. First, if $f=g$, then $\theta_{f,f}=1$, so $X_{ff}=X_f$, and since $A_{f\cdot f}=A_f$, the map $\rho_{f,ff}$ is the identity, and hence $\varphi_{ff}=\id$.
...
First, let us verify that $\varphi_{fg}$ carries the triple intersection to the triple intersection. $\Phi_{f,g}$ and $\Phi_{g,f}$ send $\theta_{f,h}$ and $\theta_{g,h}$ respectively to the elements
...
and $\rho_{f,fg}(\theta_{f,g})$ is a unit. Therefore $u^e$ and $v^d$ differ only by a unit multiple, so $D(u)=D(v)$ in $\Spec A_{(fg)}$, and localizing $A_{(fg)}$ at $u$ and at $v$ give the same ring. That is, $\varphi_{fg}$ carries $X_{fg}\cap X_{fh}$ onto $X_{gf}\cap X_{gh}$, and this restriction is the spectrum of the ring homomorphism
...
since $\Phi_{f,g}$ and $\Psi_f$ respectively extend $\rho_{f,fg}$ and $\rho_{f,fgh}$, we have $\tau\circ\rho_{f,fg}=\rho_{f,fgh}$. However, since $A_{(fg)}$ is the localization of $A_{(f)}$ at $\theta_{f,g}$ via $\Phi_{f,g}$, and $\rho_{fg,fgh}$ also satisfies the same equation, by the uniqueness in [[Commutative Algebra] §Localization, ⁋Proposition 6](/en/math/commutative_algebra/localization#prop6) we have $\tau=\rho_{fg,fgh}$. From this

$$\Psi_f\circ\alpha\vert_{A_{(g)}}=\tau\circ \rho_{g,fg}=\rho_{fg,fgh}\circ\rho_{g,fg}=\rho_{g,fgh}=\Psi_g\vert_{A_{(g)}}$$

and applying the uniqueness of [[Commutative Algebra] §Localization, ⁋Proposition 6](/en/math/commutative_algebra/localization#prop6) to the localization $(A_{(g)})_{\theta_{g,fh}}$ of $A_{(g)}$, we get $\Psi_f\circ\alpha=\Psi_g$, that is, $\alpha=\Psi_f^{-1}\circ\Psi_g$. Thus
...
:::
::: Proof
Since $\mathfrak{p}\in \Proj A_\bullet$, we have $A_+\not\subseteq \mathfrak{p}$, and hence there exists a homogeneous element $f$ of $A_+$ not belonging to $\mathfrak{p}$. Let $d=\deg f\geq 1$. Then $\mathfrak{p}\in D_+(f)$, and by [Theorem 10](#thm10), $D_+(f)$ is an open subscheme of $\Proj A_\bullet$ isomorphic to $\Spec A_{(f)}$. Since the stalk of an open subscheme is the same as the stalk of the original scheme, for the point $\mathfrak{q}=\mathfrak{p}A_f\cap A_{(f)}$ to which [Lemma 8](#lem8) carries $\mathfrak{p}$, we obtain from [§Affine Schemes, ⁋Lemma 8](/en/math/scheme_theory/affine_schemes#lem8)
...
Let $S$ be the multiplicative set consisting of homogeneous elements not belonging to $\mathfrak{p}$; then $A_{(\mathfrak{p})}=(S^{-1}A)_0$. ([[Commutative Algebra] §Localization of Graded Rings, ⁋Definition 5](/en/math/commutative_algebra/localization_of_graded_rings#def5)) Now since $f\in S$, there exists a localization $A_f \rightarrow S^{-1}A$, and since this preserves the grading, restricting to the degree $0$ part we obtain the canonical ring homomorphism
...
Therefore, if $x\not\in \mathfrak{q}$, then $a\in S$, and $f^n/a$ is an element of $S^{-1}A$ of degree $0$ that becomes the inverse of $\sigma(x)$. Now by [[Commutative Algebra] §Localization, ⁋Proposition 6](/en/math/commutative_algebra/localization#prop6), there exists a unique ring homomorphism
...
we have $\deg (as^{d-1})=l+l(d-1)=ld=\deg (f^l)$ and $\deg (s^d)=ld$, so $u,v\in A_{(f)}$. Also $s\not\in \mathfrak{p}$ and since $\mathfrak{p}$ is a prime ideal, $s^d\not\in \mathfrak{p}$, and hence $v\not\in \mathfrak{q}$. Then in $S^{-1}A$

$$\Theta\left(\frac{u}{v}\right)=\frac{as^{d-1}}{f^l}\cdot\frac{f^l}{s^d}=\frac{a}{s}$$

so $\Theta$ is surjective.

Now let us show that $\Theta$ is injective. Suppose an element $x/v$ of $(A_{(f)})_\mathfrak{q}$ is carried to $0$ by $\Theta$. Since $\Theta(v)$ is a unit, we have $\sigma(x)=0$. Writing $x=a/f^n$, this means $a/f^n=0$ in $S^{-1}A$, so there exists a homogeneous element $t\in S$ such that $ta=0$. Now since $\deg (t^d)=d\deg t=\deg (f^{\deg t})$, we have

$$w=\frac{t^d}{f^{\deg t}}$$

is an element of $A_{(f)}$, and since $t\not\in \mathfrak{p}$, we have $w\not\in \mathfrak{q}$. On the other hand, since $d\geq 1$, we have $t^da=0$ and hence in $A_{(f)}$
...
Something to be careful about is that, unlike $\Spec$, $\Proj$ does not define a functor from $\bgr_{\mathbb{N}_{\geq 0}}\cRing^\op$ to $\LRS$. This is because, for a graded ring homomorphism $\phi_\bullet:A_\bullet \rightarrow B_\bullet$, even if an arbitrary homogeneous ideal $\mathfrak{q}$ of $B$ does not contain $B_+$, its inverse image $\phi^{-1}(\mathfrak{q})$ may contain $A_+$.

Finally, we translate the projective space we first examined for motivation into the language of algebraic geometry (almost) completely.
...
Then the $n+1$ open cover of projective space in this language can be thought of as
...
Now any $\mathfrak{p}\in \mathbb{P}^n_\mathbb{K}$ is contained in some $D_+(\x_i)$. Suppose the point $\mathfrak{p}$ of $D_+(\x_i)$ is carried to the point $\mathfrak{q}$ of $U_i=\Spec \mathbb{K}[\x_{0/i}, \ldots, \x_{n/i}]/(\x_{i/i}-1)$ via the above isomorphism. Then in this case it would be natural to expect the isomorphism
