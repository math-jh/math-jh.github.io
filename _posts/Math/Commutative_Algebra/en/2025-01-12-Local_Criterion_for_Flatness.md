...
and in this situation, it suffices to show that $x$ maps to $0$ in $(\mathfrak{a}/\mathfrak{m}^t\cap \mathfrak{a})\otimes_AM$. Meanwhile, consider the commutative diagram
...
The left-hand map $\mathfrak{a}\otimes_AM \rightarrow M$ is the multiplication map $m$, so $x\in\ker m$ is carried to $0$ under the composition in the $\llcorner$ direction. Therefore, it suffices to show that the right-hand map $(\mathfrak{a}/(\mathfrak{m}^t\cap \mathfrak{a}))\otimes_AM \rightarrow (A/\mathfrak{m}^t)\otimes_AM$ is injective. Through the isomorphism

$$\frac{\mathfrak{a}}{\mathfrak{m}^t\cap \mathfrak{a}}\cong \frac{\mathfrak{a}+\mathfrak{m}^t}{\mathfrak{m}^t},$$

the map $\mathfrak{a}/(\mathfrak{m}^t\cap \mathfrak{a}) \rightarrow A/\mathfrak{m}^t$ giving rise to it is exactly the left-hand map of the short exact sequence
...
By the inductive hypothesis, $\Tor_1^A(N', M)=\Tor_1^A(N/N',M)=0$, so we obtain the desired result.
:::
...
:::
::: Proof
Suppose $M/aM$ is a flat $A/(a)$-module. Since the residue field of $A/(a)$ is $A/\mathfrak{m}$, the assumption gives

$$\Tor_1^{A/(a)}(A/\mathfrak{m}, M/aM)=0,$$

and applying [Lemma 2](#lem2){: data-lid="84lc7" data-relation="required" }, we obtain $\Tor_1^A(A/\mathfrak{m}, M)=0$. Therefore, by [Theorem 1](#thm1){: data-lid="88cwf" data-relation="required" }, $M$ is a flat $A$-module.
:::

Whereas [Corollary 3](#cor3){: data-lid="m4ivn" data-relation="weak" } above essentially cuts by an element on the base side, the following variant, which cuts by an element on the fiber side, is used to lift flatness from the fiber to the base.
...
is exact. Since we saw above that the right-hand morphism is injective, $\Tor_1^A(\kappa, C)=0$, and since $C$ is a finitely generated $E$-module, [Theorem 1](#thm1){: data-lid="qkffy" data-relation="required" } implies that $C=M/fM$ is a flat $A$-module.
...
Likewise, in the same setting, we define the *extended Rees algebra* by

$$A[\mathfrak{a}t, t^{-1}]=\bigoplus_{n=-\infty}^\infty \mathfrak{a}^nt^n\subseteq A[t, t^{-1}]$$

with the convention that $\mathfrak{a}^n=A$ for $n\leq 0$.
:::
