...
so $M/aM$ is a flat $A/(a)$-module without any additional hypotheses. In [Corollary 3](#cor3){: data-lid="ouck5" }, we prove the converse of this claim under the hypotheses of [Theorem 1](#thm1){: data-lid="a42b0" }. For this, we first need the following lemma.
...
and applying [Lemma 2](#lem2){: data-lid="84lc7" }, we obtain $\Tor_1^A(A/\mathfrak{m}, M)=0$. Therefore, by [Theorem 1](#thm1){: data-lid="88cwf" }, $M$ is a flat $A$-module.
:::

Whereas [Corollary 3](#cor3){: data-lid="m4ivn" } above essentially cuts by an element on the base side, the following variant, which cuts by an element on the fiber side, is used to lift flatness from the fiber to the base.
...
Since $M$ is flat, $\Tor_1^A(\kappa, M)=0$ ([§Flatness, ⁋Proposition 1](/en/math/commutative_algebra/flatness#prop1){: data-lid="qhjtw" }), and hence

$$0 \rightarrow \Tor_1^A(\kappa, C) \rightarrow fM\otimes_A\kappa \rightarrow M\otimes_A\kappa$$

is exact. Since we saw above that the right-hand morphism is injective, $\Tor_1^A(\kappa, C)=0$, and since $C$ is a finitely generated $E$-module, [Theorem 1](#thm1){: data-lid="qkffy" } implies that $C=M/fM$ is a flat $A$-module.
