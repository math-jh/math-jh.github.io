...
As another example, again taking $M=A$ and letting $S=A\setminus \mathfrak{p}$ for a prime ideal $\mathfrak{p}$ of $A$, we can consider $A_\mathfrak{p}=S^{-1}A$. Using [Definition 4](#def4){: data-lid="xoitz" } we can apply this to any $A$-module $M$, and the resulting $A$-module is denoted $M_\mathfrak{p}$.

Both of the above examples carry a multiplication structure in addition to the addition and scalar multiplication by $A$ defined in [Definition 4](#def4){: data-lid="h58hy" }. Explicitly, this structure is given by
