...
of $M$. Then, by [Lemma 7](#lem7){: data-relation="required" }, the filtration
...
. Since $A$ is Noetherian and $M$ is finitely generated, $\bigcap \mathfrak{a}^iM$ is also finitely generated, so applying [§Integral Extensions, ⁋Lemma 7](/en/math/commutative_algebra/integral_extension#lem7){: data-relation="required" } gives the first result.
...
:::
::: Proof
Assume $ab=0$ in $A$; it suffices to show that $a=0$ or $b=0$. Now, in $\gr_\mathfrak{a}A$ we must have $\initial(a)\initial(b)=0$, so $\initial(a)$ or $\initial(b)$ must be $0$. Since $\bigcap \mathfrak{a}^n=0$ by [Corollary 8](#cor8){: data-relation="required" }, we must have $a=0$ or $b=0$.
:::
