...
:::
::: Proof
Let $\mathfrak{m}$ be the maximal ideal of $A$, set $d=\dim A$, and let $a_1,\ldots, a_d$ be a regular system of parameters generating $\mathfrak{m}$. Then $(a_1,\ldots, a_d)=\mathfrak{m}$ is proper. Now, for each $i$, let $A_i=A/(a_1,\ldots, a_i)$; then $A_i$ is a Noetherian local ring, and its maximal ideal $\mathfrak{m}A_i$ is generated by the images of $a_{i+1},\ldots, a_d$, that is, by $d-i$ elements, so $\dim A_i\leq d-i$ by [§Krull Dimension, ⁋Theorem 7](/en/math/commutative_algebra/Krull_dimension#thm7){: data-relation="required" }. Conversely, applying [§System of Parameters, ⁋Corollary 7](/en/math/commutative_algebra/system_of_parameters#cor7){: data-relation="required" } $i$ times gives $\dim A_i\geq d-i$, so $\dim A_i=d-i$ and therefore $A_i$ is again a regular local ring.

Then by [Corollary 1](#cor1){: data-relation="required" }, each $A_i$ is an integral domain, so it suffices to show that the image of $a_{i+1}$ in $A_i$ is nonzero. If $a_{i+1}\in (a_1,\ldots, a_i)$, then $\mathfrak{m}$ would be generated by the $d-1$ elements obtained by removing $a_{i+1}$ from $a_1,\ldots, a_d$, contradicting the fact that the maximal ideal of a Noetherian local ring of dimension $d$ requires at least $d$ generators. ([§Krull Dimension, ⁋Theorem 7](/en/math/commutative_algebra/Krull_dimension#thm7){: data-relation="required" })
:::
...
and for this to hold we must have $\codim\ker\phi=0$. Since $\kappa[[\x_1,\ldots, \x_d]]$ is an integral domain by [Corollary 1](#cor1){: data-relation="required" }, this precisely means that $\ker\phi=0$.
:::
...
where each $A_\mathfrak{p}$ is viewed as a subset of the quotient field $K$ of $A$. This was already proved in greater generality in [Proposition 7](#prop7){: data-relation="required" }.
