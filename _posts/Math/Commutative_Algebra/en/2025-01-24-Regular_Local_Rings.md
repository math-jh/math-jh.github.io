...
Now set $A'=A/(a)$, and consider the maximal ideal $\mathfrak{m}'=\mathfrak{m}A'$ of $A'$. By the choice of $a$, no prime ideal of $A'$ corresponds to any $\mathfrak{p}_i$, so necessarily $\dim A'<\dim A$; combining this with [§System of Parameters, ⁋Corollary 7](/en/math/commutative_algebra/system_of_parameters#cor7){: data-lid="icpvs" }, we conclude that $\dim A'=d$. Therefore, from the identity

$$\mathfrak{m}'/(\mathfrak{m}')^2=\mathfrak{m}/(\mathfrak{m}^2+(a))$$

together with [§Integral Extensions, ⁋Lemma 8](/en/math/commutative_algebra/integral_extension#lem8){: data-lid="2jssj" }, we see that $\mathfrak{m}'$ is generated by $d$ elements, and hence by the induction hypothesis $A'$ is an integral domain. That is, $(a)$ is a prime ideal, and therefore $\mathfrak{p}_i\subsetneq (a)$ holds for some $i$.

Now, for any $x\in \mathfrak{p}_i$, choose $\alpha\in A$ such that $x=\alpha a$. Since $a\not\in \mathfrak{p}_i$, we have $\alpha\in \mathfrak{p}_i$, and hence $\mathfrak{p}_i=a \mathfrak{p}_i$, from which $\mathfrak{p}_i=\mathfrak{m}\mathfrak{p}_i$. Applying [§Integral Extensions, ⁋Lemma 8](/en/math/commutative_algebra/integral_extension#lem8){: data-lid="yy4vx" } once more gives $\mathfrak{p}_i=0$, so $A$ is an integral domain.
:::
...
:::
::: Proof
Let $\mathfrak{m}$ be the maximal ideal of $A$, set $d=\dim A$, and let $a_1,\ldots, a_d$ be a regular system of parameters generating $\mathfrak{m}$. Then $(a_1,\ldots, a_d)=\mathfrak{m}$ is proper. Now, for each $i$, let $A_i=A/(a_1,\ldots, a_i)$; then $A_i$ is a Noetherian local ring, and its maximal ideal $\mathfrak{m}A_i$ is generated by the images of $a_{i+1},\ldots, a_d$, that is, by $d-i$ elements, so $\dim A_i\leq d-i$ by [§Krull Dimension, ⁋Theorem 7](/en/math/commutative_algebra/Krull_dimension#thm7){: data-lid="qyn6j" }. Conversely, applying [§System of Parameters, ⁋Corollary 7](/en/math/commutative_algebra/system_of_parameters#cor7){: data-lid="3jlzq" } $i$ times gives $\dim A_i\geq d-i$, so $\dim A_i=d-i$ and therefore $A_i$ is again a regular local ring.

Then by [Corollary 1](#cor1){: data-lid="4smxa" }, each $A_i$ is an integral domain, so it suffices to show that the image of $a_{i+1}$ in $A_i$ is nonzero. If $a_{i+1}\in (a_1,\ldots, a_i)$, then $\mathfrak{m}$ would be generated by the $d-1$ elements obtained by removing $a_{i+1}$ from $a_1,\ldots, a_d$, contradicting the fact that the maximal ideal of a Noetherian local ring of dimension $d$ requires at least $d$ generators. ([§Krull Dimension, ⁋Theorem 7](/en/math/commutative_algebra/Krull_dimension#thm7){: data-lid="ck6u7" })
:::
...
By [Proposition 4](#prop4){: data-lid="im1uv" }, then, if two complete discrete valuation rings each contain a field and have isomorphic residue fields, they are isomorphic to each other. In general, however, no classification of this kind exists for discrete valuation rings that are not complete.
