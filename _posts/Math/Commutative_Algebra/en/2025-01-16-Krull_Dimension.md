...
One somewhat careful point: according to the above definition, $$\mathfrak{a}$$ has two definitions of dimension—the dimension as an ideal defined first, and the dimension when viewed as an $$A$$-module—and the values given by these two definitions may differ. Therefore, when we use the notation $$\dim \mathfrak{a}$$, we agree that it means only the dimension as an ideal of $$A$$, as defined first in [Definition 2](#def2).
...
holds. Despite its name, the reverse inequality does not generally hold.

On the other hand, for a local ring $$(A, \mathfrak{m})$$, we can always insert $$\mathfrak{m}$$ at the beginning of any chain of prime ideals giving $$\dim A$$, so necessarily $$\dim A=\codim \mathfrak{m}$$ holds.
...
By definition, $$\mathfrak{p}^{(n)}$$ is the inverse image of the ideal $$(\mathfrak{p}A_\mathfrak{p})^n$$ in $$A$$ under the localization $$A \rightarrow A_\mathfrak{p}$$. Then elements outside $$\mathfrak{p}$$ become non-zerodivisors modulo $$\mathfrak{p}^({n})$$, and it is obvious that $$\mathfrak{p}^{(n)}A_\mathfrak{p}=(\mathfrak{p}A_\mathfrak{p})^n$$. Also, there exists a descending chain of symbolic powers
...
It suffices to show that $$\codim \mathfrak{q}=0$$ for any prime ideal $$\mathfrak{q}\subsetneq \mathfrak{p}$$, and again by [§Localization, ⁋Proposition 8](/en/math/commutative_algebra/localization#prop8), it is enough to show that $$\dim A_\mathfrak{q}=0$$.

Now in $$A_\mathfrak{p}$$, $$\mathfrak{p}A_\mathfrak{p}$$ is the unique maximal ideal, so $$\mathfrak{p}$$ contains the ideals $$\mathfrak{q}A_\mathfrak{p}$$, $$(\mathfrak{q}A_\mathfrak{p})^{(n)}$$, $$\mathfrak{a}A_\mathfrak{p}$$. In particular, we obtain the following two chains
...
must stop. Therefore, suppose that $$(\mathfrak{q}A_\mathfrak{p})^{(n)}+\mathfrak{a}A_\mathfrak{p}= (\mathfrak{q}A_\mathfrak{p})^{(n+1)}+\mathfrak{a}A_\mathfrak{p}$$. Then
...
That is, in a noetherian ring, any prime ideal satisfies the descending chain condition, and in this case the length of a chain starting from $$\mathfrak{p}$$ is at most the number of generators of the ideal it minimally contains. Nevertheless, there exist noetherian rings of infinite dimension. (**[Nag, Appendix, Example 1]**)

On the other hand, [Theorem 7](#thm7) also has the following converse.
...
In particular, we saw that for a noetherian local ring $$(A, \mathfrak{m})$$, $$\dim A=\codim \mathfrak{m}$$ holds. Therefore, since $$\codim \mathfrak{m}=d$$, by [Corollary 8](#cor8), $$\mathfrak{m}$$ must be generated by at least $$d$$ elements.

## Dimension in Graded Rings

Let us examine some useful properties for computing dimension in a graded ring $$R = \bigoplus_{d \ge 0} R_d$$. First, let us recall the following. ([§Localization of Graded Rings](/en/math/commutative_algebra/localization_of_graded_rings))
...
That this becomes a graded prime ideal can be seen immediately from the definition. The key point is that $$\mathfrak{p} = \mathfrak{p}^*$$. First, $$\mathfrak{m} \subseteq \mathfrak{p}^* \subseteq \mathfrak{p}$$ is obvious, so let us assume $$x \in \mathfrak{p}$$ and show that $$x\in \mathfrak{p}^\ast$$.
...
From Proposition 10 we learned that prime ideals containing the irrelevant ideal $$\mathfrak{m}$$ are homogeneous. Conversely, homogeneous prime ideals not containing $$\mathfrak{m}$$ correspond to the points of $$\operatorname{Proj} R$$. Now we will show that any prime ideal chain can be refined to a homogeneous prime ideal chain.
...
Now let us show that $$\mathfrak{p}_i^* \supseteq \mathfrak{p}_{i+1}^*$$ is a strict inclusion. Since $$\mathfrak{p}_i \supsetneq \mathfrak{p}_{i+1}$$, there exists an element $$f$$ belonging to $$\mathfrak{p}_i \setminus \mathfrak{p}_{i+1}$$. Writing $$f$$ as the sum of homogeneous components $$f = f_{d_1} + \cdots + f_{d_k}$$, at least one of the $$f_{d_j}$$ does not belong to $$\mathfrak{p}_{i+1}$$ (otherwise $$f \in \mathfrak{p}_{i+1}$$). Therefore this $$f_{d_j} \in \mathfrak{p}_i^* \setminus \mathfrak{p}_{i+1}^*$$.
...
Then by [§Integral Extensions, ⁋Lemma 8](/en/math/commutative_algebra/integral_extension#lem8), for $$a_1,\ldots, a_d\in \mathfrak{m}$$, their images in $$\mathfrak{m}/\mathfrak{m}^2$$ generate $$\mathfrak{m}/\mathfrak{m}^2$$ as an $$A/\mathfrak{m}$$-vector space if and only if $$a_1,\ldots, a_d$$ generate $$\mathfrak{m}$$ as an $$A$$-module. We will study more properties of these at the end of the next post.
