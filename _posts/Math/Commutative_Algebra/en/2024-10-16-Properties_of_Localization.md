...
The proof follows by applying [Lemma 3](#lem3){: data-lid="lsenz" } to the kernel and cokernel.
...
is a splitting exact sequence. Moreover, examining the proof of [\[Multilinear Algebra\] §Hom and the Tensor Product, ⁋Proposition 1](/en/math/multilinear_algebra/hom_and_tensor#prop1){: data-lid="2lt05" }, we see that in fact if the above sequence is exact when $K=N$, that is, if
...
:::
::: Proof
One direction is obvious. For the converse, if $a\not\in \sqrt{\mathfrak{a}}$ then set $S=\{a^k\mid k\geq 1\}$ and apply [§Localization, ⁋Proposition 8](/en/math/commutative_algebra/localization#prop8){: data-lid="4pvsb" }.
:::
