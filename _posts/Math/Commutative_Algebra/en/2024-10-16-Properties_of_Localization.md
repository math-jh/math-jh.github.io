...
::: Proposition 2
$S^{-1}A$ is a flat $A$-module. ([\[Multilinear Algebra\] §Projective, Injective, and Flat Modules, ⁋Definition 7](/en/math/multilinear_algebra/various_modules#def7){: data-lid="4i7jv" data-relation="required" })
:::
::: Proof
Suppose an injective $A$-linear map $u:M \rightarrow M'$ is given; we must show that $S^{-1}A\otimes_A u$ is injective. By [Lemma 1](#lem1){: data-lid="pirbk" data-relation="required" }, it suffices to show that the induced linear map $S^{-1}M \rightarrow S^{-1}M'$ is injective. Take any $x/s\in S^{-1}M$ and suppose its image $u(x)/s$ in $S^{-1}M'$ is zero. Then from $u(x)/s=0/1$ there exists some $t\in S$ such that
...
The proof follows by applying [Lemma 3](#lem3){: data-lid="lsenz" data-relation="required" } to the kernel and cokernel.
...
is a splitting exact sequence. Moreover, examining the proof of [\[Multilinear Algebra\] §Hom and the Tensor Product, ⁋Proposition 1](/en/math/multilinear_algebra/hom_and_tensor#prop1){: data-lid="2lt05" data-relation="required" }, we see that in fact if the above sequence is exact when $K=N$, that is, if
...
:::
::: Proof
One direction is obvious. For the converse, if $a\not\in \sqrt{\mathfrak{a}}$ then set $S=\{a^k\mid k\geq 1\}$ and apply [§Localization, ⁋Proposition 8](/en/math/commutative_algebra/localization#prop8){: data-lid="4pvsb" data-relation="required" }.
:::
