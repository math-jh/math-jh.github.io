...
:::
::: Proof
1. First, suppose that $M/\mathfrak{a}M$ has finite length. Then, by the argument examined immediately after [Proposition–Definition 3](#prop-def3){: data-relation="required" }, we know that a sufficiently large power of $\mathfrak{m}$ always annihilates $M/\mathfrak{a}M$, and combining this with [Lemma 4](#lem4){: data-relation="required" }, we have
...
so by [§Dimension, ⁋Theorem 7](/en/math/commutative_algebra/Krull_dimension#thm7){: data-relation="required" }, $\dim B\leq d+e$ holds. 
...
we see that what we need to show is $\codim \mathfrak{q}\geq\dim A$. But by definition $\phi^{-1}(\mathfrak{q})=\mathfrak{m}$, so by [Lemma 8](#lem8){: data-relation="required" } whenever a chain starting at $\mathfrak{m}$ of prime ideals of $A$
