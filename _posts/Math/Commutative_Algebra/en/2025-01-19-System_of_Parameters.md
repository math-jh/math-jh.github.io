...
:::
::: Proof
1. First, suppose that $M/\mathfrak{a}M$ has finite length. Then, by the argument examined immediately after [Proposition–Definition 3](#prop-def3){: data-lid="i2ceh" data-relation="required" }, we know that a sufficiently large power of $\mathfrak{m}$ always annihilates $M/\mathfrak{a}M$, and combining this with [Lemma 4](#lem4){: data-lid="pq3zi" data-relation="required" }, we have
...
2. Suppose $M/\mathfrak{a}M$ has finite length. Then by the first result, for sufficiently large $n$, we have $\mathfrak{m}^n\subseteq \mathfrak{a}+\ann(M)$, and since $\ann(M)\subseteq \ann(M')\cap \ann(M'')$ by [Lemma 5](#lem5){: data-lid="pt0g8" data-relation="required" }, the same inclusion holds for $M'$ and $M''$. Thus, again by the first result, $M'/\mathfrak{a}M'$ and $M''/\mathfrak{a}M''$ have finite length. Conversely, from the following exact sequence obtained by applying $A/\mathfrak{a}\otimes-$,
...
and from the assumption that $\phi$ is flat, we know that this is also flat. Hence it suffices to assume that $\mathfrak{p}_2=0$ and that $A$ is an integral domain. Then by [§Flatness, ⁋Corollary 3](/en/math/commutative_algebra/flatness#cor3){: data-lid="q4at1" data-relation="required" }, $\phi$ must map non-zerodivisors of $A$ to non-zerodivisors of $B$. 

Meanwhile, by [\[Set Theory\] §Axiom of Choice, ⁋Theorem 4](/en/math/set_theory/axiom_of_choice#thm4){: data-lid="p8rhs" data-relation="required" }, we know that within $\mathfrak{q}_1$ there exists a minimal prime ideal $\mathfrak{q}_2$. However, viewing $B$ as a module over itself, we have $\ann B=0$, so by the first result of [§Associated Primes, ⁋Theorem 7](/en/math/commutative_algebra/associated_primes#thm7){: data-lid="v5n57" data-relation="required" }, we have $\mathfrak{q}_2\in \Ass B$, and by the second result of that theorem, $\mathfrak{q}_2$ must consist entirely of zero-divisors. Therefore, by the property of $\phi$ observed above, we see that $\phi^{-1}(\mathfrak{q}_2)=0$. 
:::
...
:::
::: Proof
For convenience, let $\dim A=d$ and $e=\dim B/\mathfrak{m}B$. First, by [Corollary 1](#cor1){: data-lid="qskd6" data-relation="required" }, there exist $a_1,\ldots, a_d$ such that for sufficiently large $s$, we always have $\mathfrak{m}^s\subseteq (a_1,\ldots, a_d)$, and similarly there exist $b_1,\ldots, b_e\in B$ such that for sufficiently large $t$, we always have $\mathfrak{n}^t\subseteq \phi(\mathfrak{m})B+(b_1,\ldots, b_e)$. Then
...
so by [§Dimension, ⁋Theorem 7](/en/math/commutative_algebra/Krull_dimension#thm7){: data-lid="csrva" data-relation="required" }, $\dim B\leq d+e$ holds. 
...
we see that what we need to show is $\codim \mathfrak{q}\geq\dim A$. But by definition $\phi^{-1}(\mathfrak{q})=\mathfrak{m}$, so by [Lemma 8](#lem8){: data-lid="uo96i" data-relation="required" } whenever a chain starting at $\mathfrak{m}$ of prime ideals of $A$
