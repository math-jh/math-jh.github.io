...
[§호몰로지 차원](/ko/math/commutative_algebra/homological_dimension){: data-lid="r8ioc" }에서 우리는 Noetherian local ring 위의 finitely generated module에 대하여 minimal free resolution을 세우고 projective dimension을 residue field와의 Tor가 소멸하지 않는 가장 높은 차수로 읽어내었으며, [§Depth](/ko/math/commutative_algebra/depth){: data-lid="zo3g0" }에서는 maximal regular sequence의 길이로 depth를 정의하였다. 이 글은 이 두 불변량이 서로 독립적이지 않다는 Auslander--Buchsbaum 공식
...
이다. $r\geq 1$이므로 $\Ext_A^i(\kappa,A^{\oplus r})=0$인 것은 $\Ext_A^i(\kappa,A)=0$인 것과 동치이다. 그럼 [§Depth, ⁋정리 2](/ko/math/commutative_algebra/depth#thm2){: data-lid="7cfkg" }가 주는 표현 $\operatorname{depth}X=\min\{i\mid \Ext_A^i(\kappa,X)\neq 0\}$을 $X=F$와 $X=A$에 각각 적용하여
...
다음으로 $\operatorname{depth}M=0$인 경우를 다룬다. 만일 $M$이 free라면 [명제 1](#prop1){: data-lid="vdxug" }에 의하여 $\operatorname{depth}M=\operatorname{depth}A=\delta\geq 1$이 되어 $\operatorname{depth}M=0$에 모순이므로 $M$은 free가 아니고, 따라서 $\pd_A M\geq 1$이다. $M$의 minimal free resolution의 첫 단계에서 얻어지는 short exact sequence
...
이제 [§Depth, ⁋명제 10](/ko/math/commutative_algebra/depth#prop10){: data-lid="or6pr" }의 부등식들을 이 short exact sequence에 적용한다. $F_0$은 free이므로 [명제 1](#prop1){: data-lid="d94te" }에 의하여 $\operatorname{depth}F_0=\operatorname{depth}A=\delta$이다. 둘째 부등식에서
...
공식은 $\pd_A M\geq 0$과 결합하여 $\operatorname{depth}M\leq\operatorname{depth}A$를 즉시 주며, [§Depth, ⁋따름정리 8](/ko/math/commutative_algebra/depth#cor8){: data-lid="5y5jx" }의 $\operatorname{depth}A\leq\dim A$와 종합하면 finite projective dimension을 갖는 module은 언제나 $\operatorname{depth}M\leq\operatorname{depth}A\leq\dim A$를 만족한다. 한편 $\pd_A M<\infty$라는 가정은 공식이 성립하기 위한 필수 조건이며, 이를 떼어내면 [예시 7](#ex7){: data-lid="1dnhn" }에서 보듯 등식이 깨진다. 공식의 첫 응용으로 projective dimension의 크기와, depth가 $0$일 때의 경직성을 얻는다.
...
:::
::: 증명
첫째 결과는 [정리 3](#thm3){: data-lid="cqzwf" }과 $\operatorname{depth}M\geq 0$으로부터 $\pd_A M=\operatorname{depth}A-\operatorname{depth}M\leq\operatorname{depth}A$이다. 둘째 결과의 경우, $\operatorname{depth}A=0$이면 첫째 결과에 의하여 $\pd_A M\leq 0$, 곧 $\pd_A M=0$이므로 [§호몰로지 차원, ⁋명제 11](/ko/math/commutative_algebra/homological_dimension#prop11){: data-lid="9gzj7" }에 의하여 minimal free resolution이 $F_0$에서 끝나 $M\cong F_0$은 free이다. Artinian local ring $(A,\mathfrak{m})$은 [§조르단-횔더 정리, ⁋정리 4](/ko/math/commutative_algebra/Jordan-Holder_theorem#thm4){: data-lid="awhcz" }에 의하여 모든 prime ideal이 maximal이므로 $\dim A=0$이고, [§Depth, ⁋따름정리 8](/ko/math/commutative_algebra/depth#cor8){: data-lid="6dq9p" }에 의하여 $\operatorname{depth}A\leq\dim A=0$이 되어 앞의 경우에 해당한다.
:::
...
:::
::: 증명
[§호몰로지 차원, ⁋명제 13](/ko/math/commutative_algebra/homological_dimension#prop13){: data-lid="wk23h" }에 의하여 $\operatorname{gldim}A=d$이므로 임의의 $M$에 대하여 $\pd_A M\leq d<\infty$이다. 또 $A$는 [§Cohen-Macaulay 환, ⁋따름정리 5](/ko/math/commutative_algebra/cohen_macaulay_rings#cor5){: data-lid="zhd29" }에 의하여 Cohen--Macaulay이므로 $\operatorname{depth}A=\dim A=d$이고, [정리 3](#thm3){: data-lid="y07dz" }에 의하여 $\pd_A M=\operatorname{depth}A-\operatorname{depth}M=d-\operatorname{depth}M$이다. $M$이 Cohen--Macaulay module이면 [§Cohen-Macaulay 환, ⁋정의 1](/ko/math/commutative_algebra/cohen_macaulay_rings#def1){: data-lid="sgpd8" }에 의하여 $\operatorname{depth}M=\dim M$이므로 $\pd_A M=d-\dim M$을 얻는다.
:::
...
을 얻는다. 두 differential의 성분이 모두 $\mathfrak{m}$에 속하므로 이는 $\mathfrak{m}$의 minimal free resolution이고, $F_2=0$이므로 $\pd_A\mathfrak{m}=1$이다. $A$가 regular local ring이라 $\operatorname{depth}A=\dim A=2$이므로 [정리 3](#thm3){: data-lid="ps53n" }에 의하여

$$\operatorname{depth}\mathfrak{m}=\operatorname{depth}A-\pd_A\mathfrak{m}=2-1=1$$

이다.

이를 직접 확인해 보자. $A$가 domain이고 $\mathfrak{m}\subseteq A$이므로 곱하기 $\x$는 $\mathfrak{m}$ 위에서 injective이고, [§Depth, ⁋따름정리 4](/ko/math/commutative_algebra/depth#cor4){: data-lid="205l0" }의 둘째 결과에 의하여 $\operatorname{depth}\mathfrak{m}=\operatorname{depth}(\mathfrak{m}/\x\mathfrak{m})+1$이다. $\mathfrak{m}/\x\mathfrak{m}$에서 $\x$의 class를 $\overline{\x}$라 하면, $\x\in\x\mathfrak{m}=(\x^2,\x\y)$일 경우 $\x=\x(a\x+b\y)$로부터 domain에서 $1=a\x+b\y\in\mathfrak{m}$이 되어 모순이므로 $\overline{\x}\neq 0$이다. 한편 $\x\cdot\overline{\x}=\x^2\in\x\mathfrak{m}$과 $\y\cdot\overline{\x}=\x\y\in\x\mathfrak{m}$으로부터 $\mathfrak{m}\overline{\x}=0$이므로 $\ann(\overline{\x})=\mathfrak{m}$이 되어 $\mathfrak{m}\in\Ass(\mathfrak{m}/\x\mathfrak{m})$이다. 따라서 [§Depth, ⁋따름정리 4](/ko/math/commutative_algebra/depth#cor4){: data-lid="mkn4g" }의 첫째 결과에 의하여 $\operatorname{depth}(\mathfrak{m}/\x\mathfrak{m})=0$이고, $\operatorname{depth}\mathfrak{m}=0+1=1$로 공식과 부합한다.
:::

다음 예시는 $\pd_A M<\infty$라는 가정을 떼면 공식이 실패한다는 것을, projective dimension이 infinite module로 보여준다.

::: 예시 7
Field $\mathbb{K}$에 대하여 $A=\mathbb{K}[[\x,\y]]/(\x\y)$를 생각하자. 이는 [§Depth, ⁋예시 11](/ko/math/commutative_algebra/depth#ex11){: data-lid="grj1k" }에서 살펴본 것처럼 $\operatorname{depth}A=1=\dim A$인 Cohen--Macaulay local ring이다. $\x,\y$의 image를 각각 $\overline{\x},\overline{\y}$로 적고 $M=A/(\overline{\x})$를 생각하자.
