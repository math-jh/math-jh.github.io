...
이다. 목표는 이 complex가 언제 exact인지, 곧 언제 각각의 $1\leq i\leq n$에서 $H_i(F_\bullet)=0$인지를 ([\[호몰로지 대수학\] §호몰로지, ⁋정의 2](/ko/math/homological_algebra/homology#def2){: data-lid="dtqjf" data-relation="requires-review" }) $\varphi_i$들의 행렬 자료만으로 판정하는 것이다. 여기서 행렬 자료란 [§Fitting 아이디얼, ⁋정의 1](/ko/math/commutative_algebra/fitting_ideals#def1){: data-lid="bkx46" data-relation="requires-review" }에서 도입한 minor ideal $I_r(\varphi)$를 뜻한다. $\varphi:A^m \rightarrow A^n$을 표준 basis에 대한 $n\times m$ 행렬로 보면 ([\[다중선형대수학\] §행렬과 선형사상, ⁋정의 1](/ko/math/multilinear_algebra/matrices_and_linear_maps#def1){: data-lid="1i52d" data-relation="requires-review" }), $1\leq r\leq \min(m,n)$에서 $I_r(\varphi)$는 $r\times r$ minor들이 생성하는 ideal이고, $r\leq 0$이면 $I_r(\varphi)=A$, $r>\min(m,n)$이면 $I_r(\varphi)=0$이며, 언제나 $I_{r+1}(\varphi)\subseteq I_r(\varphi)$이다. ([§Fitting 아이디얼, ⁋정의 1](/ko/math/commutative_algebra/fitting_ideals#def1){: data-lid="5tls7" data-relation="requires-review" } 직후의 포함관계)
...
:::
::: 증명
우선 basis 변경이 모든 $I_r$를 보존함을 확인한다. Basis 변경은 행렬 $X$를 invertible matrix $P,Q$에 대한 $PXQ$로 바꾼다. ([\[다중선형대수학\] §기저변환, ⁋명제 4](/ko/math/multilinear_algebra/change_of_basis#prop4){: data-lid="cyyzl" data-relation="requires-review" }, [\[다중선형대수학\] §기저변환, ⁋명제 5](/ko/math/multilinear_algebra/change_of_basis#prop5){: data-lid="onvaj" data-relation="requires-review" }) $PX$의 각 행은 $X$의 행들의 $A$-linear combination이므로, $PX$의 $r\times r$ minor를 행에 대한 multilinearity로 전개하면 $X$의 행 $r$개를 고른 행렬식들의 combination이 되고, 행이 겹치는 항은 alternating 성질로 소멸하므로 남는 항들은 $X$의 $r\times r$ minor들이다. 이는 [§Fitting 아이디얼, ⁋정리 3](/ko/math/commutative_algebra/fitting_ideals#thm3){: data-lid="r8ozx" data-relation="weak" reviewed="" }의 증명에서 열에 대해 수행한 논증과 같다. 따라서 $I_r(PX)\subseteq I_r(X)$이고, $P$가 invertible이므로 반대 포함도 성립하며, 열에 대해서도 마찬가지이므로 $I_r(PXQ)=I_r(X)$이다.
...
:::
::: 증명
$K_i=\bigwedge\nolimits^iA^n$은 rank $\binom{n}{i}$의 free module이고, differential $d_i$의 행렬 성분은 [§코쥴 복합체](/ko/math/commutative_algebra/koszul_complex){: data-lid="lttim" data-relation="requires-review" }에서 살펴본 명시적인 식에 의하여 basis $e_J$ ($J$는 $i$개의 원소를 갖는 index 집합)와 $e_K$ ($K$는 $i-1$개) 사이에서 $J=K\cup\{l\}$일 때 $\pm x_l$, 아닐 때 $0$이다. $c_i=\binom{n-1}{i-1}$로 두면 Pascal의 규칙으로 $c_i+c_{i+1}=\binom{n}{i}$이다.
...
::: 예시 13
Field $\mathbb{K}$에 대하여 $A=\mathbb{K}[[\x,\y,\z]]$와 ideal $I=(\y\z,\x\z,\x\y)$를 생각하자. $V(I)$는 세 좌표축의 합집합이다. [§Depth, ⁋예시 11](/ko/math/commutative_algebra/depth#ex11){: data-lid="xd5hf" data-relation="requires-review" }에서 본 것처럼 $A$는 차원 $3$의 Noetherian local ring이고, maximal ideal $(\x,\y,\z)$가 정확히 $3$개의 원소로 생성되므로 regular local ring이다. ([§차원, ⁋정의 12](/ko/math/commutative_algebra/Krull_dimension#def12){: data-lid="6w0bl" data-relation="requires-review" })
