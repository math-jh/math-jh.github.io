...
우선 보조정리 하나를 증명하며 시작한다. 함수 $S^{-1}A\times M \rightarrow  S^{-1}M$을 $(r/u, x)\mapsto rx/u$으로 정의하면 이는 $A$-bilinear map이고, 따라서 $A$-linear map $S^{-1}A\otimes_A M \rightarrow S^{-1}M$을 유도한다. ([\[대수적 구조\] §가군의 직접곱과 직합, 텐서곱, ⁋정리 5](/ko/math/algebraic_structures/operations_of_modules#thm5){: data-lid="t8y44" data-relation="required" reviewed="" }) 
...
위의 [명제 2](#prop2){: data-lid="432mt" data-relation="required" reviewed="" }와 tensor product의 right exactness에 의하여, $u:M \rightarrow M'$이 injective (resp. surjective, bijective)라면 이로부터 유도되는 $S^{-1}M \rightarrow S^{-1}M'$ 또한 그러하다는 것을 안다. [명제 4](#prop4){: data-lid="urbx0" data-relation="forward" reviewed="" }는 이에 대한 일종의 (강력한) 역이라고 생각할 수 있다. 이를 위해 우선 다음 보조정리를 보인다.
...
이에 대한 증명은 [보조정리 3](#lem3){: data-lid="lsenz" data-relation="required" reviewed="" }를 kernel과 cokernel에 대해 적용하면 충분하다. 
...
이 splitting exact sequence인 것과 동치이며, [\[다중선형대수학\] §Hom과 텐서곱, ⁋명제 1](/ko/math/multilinear_algebra/hom_and_tensor#prop1){: data-lid="2lt05" data-relation="required" reviewed="" }의 증명을 보면 실은 $K=N$일 때 위의 sequence가 exact이기만 하면, 즉
...
:::
::: 증명
한쪽 방향은 자명하며, 반대로 만일 $a\not\in \sqrt{\mathfrak{a}}$라면 $S=\{a^k\mid k\geq 0\}$로 두고 [§국소화, ⁋명제 8](/ko/math/commutative_algebra/localization#prop8){: data-lid="4pvsb" data-relation="required" reviewed="" }를 적용하면 된다.
:::
