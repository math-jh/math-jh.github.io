...
우선 보조정리 하나를 증명하며 시작한다. 함수 $S^{-1}A\times M \rightarrow  S^{-1}M$을 $(r/u, x)\mapsto rx/u$으로 정의하면 이는 $A$-bilinear map이고, 따라서 $A$-linear map $S^{-1}A\otimes_A M \rightarrow S^{-1}M$을 유도한다. ([\[대수적 구조\] §가군의 직접곱과 직합, 텐서곱, ⁋정리 5](/ko/math/algebraic_structures/operations_of_modules#thm5){: data-lid="t8y44" data-relation="required" }) 
...
::: 명제 2
$S^{-1}A$는 flat $A$-module이다. ([\[다중선형대수학\] §사영가군, 단사가군, 평탄가군, ⁋정의 7](/ko/math/multilinear_algebra/various_modules#def7){: data-lid="4i7jv" data-relation="weak" })
:::
::: 증명
임의의 injective $A$-linear map $u:M \rightarrow M'$이 주어졌다 하고, $S^{-1}A\otimes_A u$이 injective인 것을 보여야 한다. 그런데 [보조정리 1](#lem1){: data-lid="pirbk" data-relation="required" }에 의해, 이는 linear map $S^{-1}M \rightarrow S^{-1}M'$이 injective인 것을 보이면 충분하다. 어떠한 $x/s\in S^{-1}M$에 대하여, 이를 $S^{-1}M'$으로 보낸 원소인 $u(x)/s$가 $S^{-1}M'$에서 $0$이라 하자. 그럼 $u(x)/s=0/1$로부터 적당한 $t\in S$가 존재하여 
...
위의 [명제 2](#prop2){: data-lid="432mt" data-relation="required" }와 tensor product의 right exactness에 의하여, $u:M \rightarrow M'$이 injective (resp. surjective, bijective)라면 이로부터 유도되는 $S^{-1}M \rightarrow S^{-1}M'$ 또한 그러하다는 것을 안다. [명제 4](#prop4){: data-lid="urbx0" data-relation="forward" }는 이에 대한 일종의 (강력한) 역이라고 생각할 수 있다. 이를 위해 우선 다음 보조정리를 보인다.
...
이에 대한 증명은 [보조정리 3](#lem3){: data-lid="lsenz" data-relation="required" }를 kernel과 cokernel에 대해 적용하면 충분하다. 
...
이 splitting exact sequence인 것과 동치이며, [\[다중선형대수학\] §Hom과 텐서곱, ⁋명제 1](/ko/math/multilinear_algebra/hom_and_tensor#prop1){: data-lid="2lt05" data-relation="required" }의 증명을 보면 실은 $K=N$일 때 위의 sequence가 exact이기만 하면, 즉
...
:::
::: 증명
한쪽 방향은 자명하며, 반대로 만일 $a\not\in \sqrt{\mathfrak{a}}$라면 $S=\{a^k\mid k\geq 0\}$로 두고 [§국소화, ⁋명제 8](/ko/math/commutative_algebra/localization#prop8){: data-lid="4pvsb" data-relation="required" }를 적용하면 된다.
:::
