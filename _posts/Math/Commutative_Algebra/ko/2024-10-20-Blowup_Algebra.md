...
:::
::: 증명
$\mathcal{J}$가 $\mathfrak{a}$-stable이므로 $\Bl_\mathcal{J}M$은 $\Bl_\mathfrak{a}A$-module로서 finitely generated이다. 한편 $\Bl_\mathfrak{a}A$는 finitely generated $A$-algebra이고 $A$가 Noetherian이므로 [§기본 개념들, ⁋따름정리 13](/ko/math/commutative_algebra/basic_notions#cor13){: data-lid="6z38n" data-relation="required" reviewed="" }에 의하여 $\Bl_\mathfrak{a}A$도 Noetherian이다. 따라서, $\Bl_\mathcal{J}M$의 submodule $\Bl_{\mathcal{J}'}M'$ 또한 finitely generated이고, 다시 [명제 6](#prop6){: data-lid="eqdco" data-relation="required" reviewed="" }을 적용하면 원하는 결과를 얻는다.
:::
...
을 생각하자. 그럼 [보조정리 7](#lem7){: data-lid="yb442" data-relation="required" reviewed="" }에 의하여, 다음의 filtration
...
을 얻는다. 이제 $A$가 Noetherian이고 $M$이 finitely generated이므로 $\bigcap \mathfrak{a}^iM$ 또한 finitely generated이고, 따라서 [§정수적 확장, ⁋보조정리 7](/ko/math/commutative_algebra/integral_extension#lem7){: data-lid="yt9m7" data-relation="required" reviewed="" }을 적용하면 첫째 결과를 얻는다. 
...
:::
::: 증명
$A$에서 $ab=0$임을 가정하고, $a=0$이거나 $b=0$임을 보이면 충분하다. 이제 $\gr_\mathfrak{a}A$에서 $\initial(a)\initial(b)=0$이 성립해야 하고, 따라서 $\initial(a)$ 혹은 $\initial(b)$가 $0$이어야 한다. 이제 [따름정리 8](#cor8){: data-lid="n7v5o" data-relation="required" reviewed="" }로부터 $\bigcap \mathfrak{a}^n=0$이므로, $a=0$이거나 $b=0$이어야 한다. 
:::
