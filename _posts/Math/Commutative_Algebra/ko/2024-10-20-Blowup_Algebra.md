...
::: 보조정리 2
위에서 정의한 $\gr_\mathfrak{a}A$의 곱셈은 잘 정의된다.
:::
::: 증명
...
::: 정의 3
Ring $A$와 $A$의 임의의 ideal $\mathfrak{a}$, $A$-module $M$에 대하여 다음의 filtration
...
위의 정의에서 $\gr_\mathcal{J}M$은 $\gr_\mathfrak{a}A$-module 구조를 가지며, 이는 임의의 $a\in \mathfrak{a}^k/\mathfrak{a}^{k+1}$과 $x\in M_l/M_{l+1}$에 대하여 이들의 representative $\tilde{a}\in \mathfrak{a}^k$, $\tilde{x}\in M_l$을 택한 후 $\tilde{a}\tilde{x}$를 $M_{k+l}/M_{k+l+1}$로 제한시킨 것이며, [보조정리 2](#lem2){: data-lid="xnj0s" data-relation="required" }와 유사한 계산을 통해 이것이 잘 정의된다는 것을 확인할 수 있다. 특별히 $M=A$이고 $M_i$들이 모든 $i,j$에 대하여 $M_iM_j\subseteq M_{i+j}$를 만족하는 $A$의 ideal들인 경우, [정의 1](#def1){: data-lid="va2wz" data-relation="required" }과 마찬가지로 $\gr_\mathcal{J}A$ 또한 ring의 구조를 가지며, 이 또한 filtration $\mathcal{J}$에 대한 associated graded ring이라 부른다. 

이제 다음이 성립한다. 

::: 명제 4
Finitely generated module $M$의 $\mathfrak{a}$-stable filtration $\mathcal{J}$가 주어졌다 하고, $\mathcal{J}$의 모든 항 $M_k$가 $M$의 finitely generated submodule이라 하자. 그럼 $\gr_\mathcal{J}M$는 finitely generated $\gr_\mathfrak{a}A$-module이다.
:::
::: 증명
...
::: 정의 5
Ring $A$와 ideal $\mathfrak{a}$에 대하여, $\mathfrak{a}$의 $A$에서의 *blowup algebra<sub>부풀림 대수</sub>*는 다음의 graded $A$-algebra
...
::: 보조정리 7 (Artin-Rees)
Noetherian ring $A$와 ideal $\mathfrak{a}\subseteq A$를 고정하고, finitely generated $A$-module $M$과 그 submodule $M'$을 고정하자. 만일 
...
:::
::: 증명
$\mathcal{J}$가 $\mathfrak{a}$-stable이므로 $\Bl_\mathcal{J}M$은 $\Bl_\mathfrak{a}A$-module로서 finitely generated이다. 한편 $\Bl_\mathfrak{a}A$는 finitely generated $A$-algebra이고 $A$가 Noetherian이므로 [§기본 개념들, ⁋따름정리 13](/ko/math/commutative_algebra/basic_notions#cor13){: data-lid="6z38n" data-relation="required" }에 의하여 $\Bl_\mathfrak{a}A$도 Noetherian이다. 따라서, $\Bl_\mathcal{J}M$의 submodule $\Bl_{\mathcal{J}'}M'$ 또한 finitely generated이고, 다시 [명제 6](#prop6){: data-lid="eqdco" data-relation="required" }을 적용하면 원하는 결과를 얻는다.
:::
...
을 생각하자. 그럼 [보조정리 7](#lem7){: data-lid="yb442" data-relation="required" }에 의하여, 다음의 filtration
...
을 얻는다. 이제 $A$가 Noetherian이고 $M$이 finitely generated이므로 $\bigcap \mathfrak{a}^iM$ 또한 finitely generated이고, 따라서 [§정수적 확장, ⁋보조정리 7](/ko/math/commutative_algebra/integral_extension#lem7){: data-lid="yt9m7" data-relation="required" }을 적용하면 첫째 결과를 얻는다. 
...
:::
::: 증명
$A$에서 $ab=0$임을 가정하고, $a=0$이거나 $b=0$임을 보이면 충분하다. 이제 $\gr_\mathfrak{a}A$에서 $\initial(a)\initial(b)=0$이 성립해야 하고, 따라서 $\initial(a)$ 혹은 $\initial(b)$가 $0$이어야 한다. 이제 [따름정리 8](#cor8){: data-lid="n7v5o" data-relation="required" }로부터 $\bigcap \mathfrak{a}^n=0$이므로, $a=0$이거나 $b=0$이어야 한다. 
:::
