...
을 생각하자. 그럼 [보조정리 7](#lem7){: data-lid="yb442" }에 의하여, 다음의 filtration
...
을 얻는다. 이제 $A$가 Noetherian이고 $M$이 finitely generated이므로 $\bigcap \mathfrak{a}^iM$ 또한 finitely generated이고, 따라서 [§정수적 확장, ⁋보조정리 7](/ko/math/commutative_algebra/integral_extension#lem7){: data-lid="yt9m7" }을 적용하면 첫째 결과를 얻는다. 
...
:::
::: 증명
$A$에서 $ab=0$임을 가정하고, $a=0$이거나 $b=0$임을 보이면 충분하다. 이제 $\gr_\mathfrak{a}A$에서 $\initial(a)\initial(b)=0$이 성립해야 하고, 따라서 $\initial(a)$ 혹은 $\initial(b)$가 $0$이어야 한다. 이제 [따름정리 8](#cor8){: data-lid="n7v5o" }로부터 $\bigcap \mathfrak{a}^n=0$이므로, $a=0$이거나 $b=0$이어야 한다. 
:::
