...
::: 정리 4
Jacobson ring $A$와, finitely generated $A$-algebra $E$가 주어졌다 하자. 그럼 $E$도 Jacobson ring이다. 또, 만일 $\mathfrak{n}$이 $E$의 maximal ideal이라면 $\mathfrak{m}=\mathfrak{n}\cap A$는 $A$의 maximal ideal이며, $E/\mathfrak{n}$은 $A/\mathfrak{m}$의 finite field extension이다.
:::
::: 증명
세 단계로 나누어 증명한다.
...
2. 다음 단계로, 임의의 Jacobson ring $A$와, 하나의 원소로 생성되는 $A$-algebra $E$를 생각하고 $E$가 Jacobson임을 보이기 위해 [보조정리 3](#lem3){: data-lid="21aht" data-relation="required" reviewed="" }의 둘째 조건이 성립함을 보이자. 즉 이번 단계에서 우리의 목표는 다음 명제를 증명하는 것이다.
    ...
    으로부터 $E'[x^{-1}]$이 integral $A'[(p_nq_0)^{-1}]$-algebra임을 안다. 이제 [§정수적 확장과 아이디얼, ⁋따름정리 3](/ko/math/commutative_algebra/lying_over_and_going_up#cor3){: data-lid="5mzqr" data-relation="required" reviewed="" }으로부터 $A'[(p_nq_0)^{-1}]$는 field이고, 가정에 의해 $A'$는 Jacobson이므로 [보조정리 3](#lem3){: data-lid="ngsed" data-relation="required" reviewed="" }에 의해 $A'$는 field이다. 따라서 $E'$는 integral $A'$-algebra이고 다시 [§정수적 확장과 아이디얼, ⁋따름정리 3](/ko/math/commutative_algebra/lying_over_and_going_up#cor3){: data-lid="s8x58" data-relation="required" reviewed="" }으로부터 $E'$가 field임을 안다. 
...
따라서 [§기본 개념들, ⁋명제 11](/ko/math/commutative_algebra/basic_notions#prop11){: data-lid="47akg" data-relation="required" reviewed="" }로부터 다음을 얻는다.

::: 보조정리 5
Field $\mathbb{K}$가 주어졌다 하자. 그럼 $\mathfrak{m}_a=(\x_1-a_1,\ldots, \x_n-a_n)$은 $\mathbb{K}[\x_1,\ldots, \x_n]$의 maximal ideal이다. 또, 만일 $\mathbb{K}$가 algebraically closed라면 $\mathbb{K}[\x_1,\ldots,\x_n]/(f_1,\ldots, f_r)$의 maximal ideal들과, 다음 식
