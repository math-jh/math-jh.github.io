...
2. 다음 단계로, 임의의 Jacobson ring $A$와, 하나의 원소로 생성되는 $A$-algebra $E$를 생각하고 $E$가 Jacobson임을 보이기 위해 [보조정리 3](#lem3){: data-lid="21aht" data-relation="required" }의 둘째 조건이 성립함을 보이자. 즉 이번 단계에서 우리의 목표는 다음 명제를 증명하는 것이다.
    ...
    으로부터 $E'[x^{-1}]$이 integral $A'[(p_nq_0)^{-1}]$-algebra임을 안다. 이제 [§정수적 확장과 아이디얼, ⁋따름정리 3](/ko/math/commutative_algebra/lying_over_and_going_up#cor3){: data-lid="5mzqr" data-relation="required" }으로부터 $A'[(p_nq_0)^{-1}]$는 field이고, 가정에 의해 $A'$는 Jacobson이므로 [보조정리 3](#lem3){: data-lid="ngsed" data-relation="required" }에 의해 $A'$는 field이다. 따라서 $E'$는 integral $A'$-algebra이고 다시 [§정수적 확장과 아이디얼, ⁋따름정리 3](/ko/math/commutative_algebra/lying_over_and_going_up#cor3){: data-lid="s8x58" data-relation="required" }으로부터 $E'$가 field임을 안다. 
...
따라서 [§기본 개념들, ⁋명제 11](/ko/math/commutative_algebra/basic_notions#prop11){: data-lid="47akg" data-relation="required" }로부터 다음을 얻는다.
