...
이제 원래의 방향을 보이기 위해 먼저 $N$이 free module이고 $\{y_j\}_{j\in J}$가 $N$의 basis인 경우를 살펴보자. 그럼 다음의 isomorphism

$$M\otimes_AN\cong\bigoplus_{j\in J} (M\otimes_A Ay_j)\cong \bigoplus_{j\in J} M$$

을 생각하면 ([\[대수적 구조\] §가군의 직접곱과 직합, 텐서곱, ⁋정리 6](/ko/math/algebraic_structures/operations_of_modules#thm6)), 원소 $\sum_j x_j\otimes y_j$는 $(x_j)_{j\in J}$에 해당되므로 이 원소가 $0$인 것과 모든 $x_j$가 $0$인 것이 동치이다. 
...
::: 따름정리 7
Field $\mathbb{K}$와 $A=\mathbb{K}[\x]$를 고정하고, $E$가 flat $A$-algebra라 하자. 만일 $E/\x E$가 domain이고, $S$가 $f\in E$에 대해 $1-\x f$ 꼴의 원소들의 모임이라면 $S^{-1}E$는 domain이다. 
:::
::: 증명
