...
1. Field $\mathbb{K}$와 polynomial ring $R=\mathbb{K}[\x_1,\ldots,\x_s]$를 각 변수가 degree $1$을 갖는 graded ring으로 보자. 그럼 $R_n$은 degree $n$의 monomial들을 basis로 가지며, 그 개수는 중복조합의 수이므로 모든 $n\geq 0$에 대하여
    ...
    이다. 즉 $P_R$는 degree가 정확히 $s-1$이고, [정리 5](#thm5){: data-lid="kuiaq" data-relation="requires-review" }의 degree 상한이 실제로 달성된다.  
...
이로써 Noetherian local ring 위의 finitely generated module의 차원은 prime ideal chain의 길이 ([§차원, ⁋정의 2](/ko/math/commutative_algebra/Krull_dimension#def2){: data-lid="66dg9" data-relation="requires-review" }), system of parameters의 크기 ([§매개계, ⁋명제 6](/ko/math/commutative_algebra/system_of_parameters#prop6){: data-lid="o7z3j" data-relation="requires-review" }), 그리고 Hilbert-Samuel polynomial의 degree라는 세 가지 서로 다른 방식으로 계산할 수 있게 되었다. 가령 [예시 10](#ex10){: data-lid="6cvno" data-relation="required" reviewed="" }의 cusp의 local ring $A$는 $d(A)=1$이므로 $\dim A=1$이다.
