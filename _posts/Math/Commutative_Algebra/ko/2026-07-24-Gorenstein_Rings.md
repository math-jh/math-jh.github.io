...
:::
::: 증명
$(A/\mathfrak{p})_\mathfrak{p}$가 [§국소화, ⁋정의 10](/ko/math/commutative_algebra/localization#def10){: data-lid="npq5g" data-relation="required" reviewed="" }의 residue field $\kappa(\mathfrak{p})$이므로, [보조정리 3](#lem3){: data-lid="b07k4" data-relation="required" reviewed="" }에 의하여
...
를 얻는다. 첫째 함의는 $c=0$인 경우이고 마지막은 [정리 6](#thm6){: data-lid="myj18" data-relation="required" reviewed="" }이다. 이 위계에서 적어도 두 포함이 진성일 수 있다는 것은 [예시 11](#ex11){: data-lid="ty6vx" data-relation="forward" reviewed="" }이 보여주는데, 거기서 $\mathbb{K}[[\x,\y]]/(\x^2,\y^2)$은 regular가 아닌 complete intersection이고 $\mathbb{K}[[t^3,t^4,t^5]]$은 Gorenstein이 아닌 Cohen--Macaulay이다.
