...
<ins id="prop1">**명제 1**</ins> $$\Sh(X, \Ab)$$와 $$\operatorname{Mod}(\mathcal{O}_X)$$는 abelian category이다. 이때 sheaf morphism $$\phi: \mathcal{F} \to \mathcal{G}$$의 kernel, cokernel, image는 stalk 차원에서
...
<ins id="thm2">**정리 2 (Grothendieck)**</ins> $$\Sh(X, \Ab)$$와 $$\operatorname{Mod}(\mathcal{O}_X)$$는 충분한 injective를 가진다.
...
<ins id="cor3">**따름정리 3**</ins> 위상공간 $$X$$에 대해 $$D^+(\Sh(X))$$ 위의 right derived functor
...
<ins id="prop4">**명제 4**</ins> Continuous map $$f: X \to Y$$와 sheaf $$\mathcal{F} \in \Sh(X)$$가 주어졌다 하자. $$\mathcal{F} \to \mathcal{A}^\bullet$$이 $$f_\ast$$-acyclic sheaf들로 이루어진 resolution, 즉 모든 항 $$\mathcal{A}^k$$가 $$R^q f_\ast \mathcal{A}^k = 0$$ ($$q > 0$$)을 만족하는 resolution이라면, derived category에서
...
<ins id="prop5">**명제 5**</ins> Continuous map $$f: X \to Y$$에 대해 $$f^{-1}: \Sh(Y) \to \Sh(X)$$는 exact functor이며, 따라서 그것이 유도하는 $$D(\Sh(Y)) \to D(\Sh(X))$$는 derivation 없이 곧바로 정의되고 $$Lf^{-1} = f^{-1} = Rf^{-1}$$이다.
...
<ins id="def6">**정의 6**</ins> Ringed space 사이의 morphism $$f: (X, \mathcal{O}_X) \to (Y, \mathcal{O}_Y)$$가 주어졌다 하자. $$\mathcal{O}_Y$$-module $$\mathcal{G}$$의 *module pullback<sub>가군 당김</sub>* $$f^\ast \mathcal{G}$$를
...
<ins id="def7">**정의 7**</ins> Ringed space $$(X, \mathcal{O}_X)$$ 위에서 다음을 정의한다.
...
<ins id="prop8">**명제 8**</ins> Ringed space $$(X, \mathcal{O}_X)$$ 위의 complex $$\mathcal{F}^\bullet, \mathcal{G}^\bullet, \mathcal{H}^\bullet \in D(\operatorname{Mod}(\mathcal{O}_X))$$에 대해 derived tensor-hom adjunction
...
<ins id="thm9">**정리 9**</ins> Continuous map $$f: X \to Y$$에 대해 다음의 derived adjunction이 성립한다.
...
<ins id="thm10">**정리 10 (Grothendieck 합성 함자 정리)**</ins> Continuous map $$f: X \to Y$$, $$g: Y \to Z$$에 대해 $$D^+(\Sh(X))$$ 위에서 자연스러운 동형
...
<ins id="ex11">**예시 11**</ins> 원 $$S^1$$ 위의 constant sheaf $$\mathbb{Z}_{S^1}$$의 cohomology를 구하고, 이로부터 torus $$T^2 = S^1 \times S^1$$의 cohomology를 Leray spectral sequence로 복원한다.
