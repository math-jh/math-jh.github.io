...
::: 예시 8
한 점 $$X = \{\ast\}$$ 위에서는 $$D^b_c(\{\ast\}) = D^b(\operatorname{Vec}^{fd}_k)$$ (유한차원 $$k$$-vector space의 유계 유도 범주)이고, $$\dim\{\ast\} = 0$$이므로 perverse t-structure는 standard t-structure와 일치한다. 따라서
...
::: 정의 10
열린 매장 $$j: U \hookrightarrow X$$와 perverse 층 $$\mathcal{F} \in \operatorname{Perv}(U)$$에 대해, 표준 사상 $$j_! \mathcal{F} \to Rj_\ast \mathcal{F}$$에 perverse cohomology $${}^{p}\mathcal{H}^0$$을 적용하여 얻는 $$\operatorname{Perv}(X)$$ 안의 사상의 상
...
::: 정리 12
$$\operatorname{Perv}(X)$$의 단순 대상은 정확히, 기약 닫힌 부분다양체 $$Z \subseteq X$$와 그 매끄러운 조밀 열린 부분 위의 기약 local system $$L$$의 쌍 $$(Z, L)$$에 대한 IC 층 $$\operatorname{IC}_Z(L)$$들이다. 서로 다른 쌍 $$(Z, L)$$ (단, $$L$$은 동형을 제외)은 서로 동형이 아닌 단순 대상을 주며, 이로써 단순 대상이 $$(Z, L)$$로 완전히 분류된다.
:::
::: 증명
...
::: 정리 13 (분해정리)
$$f: X \to Y$$를 복소대수다양체 사이의 proper 사상이라 하자. 그럼 $$D^b_c(Y)$$ 안에서 자연스러운 직합 분해
...
::: 예시 16
$$\Delta^\ast$$ 위의 rank $$r$$ local system $$L$$의 monodromy를 $$T: V \to V$$ ($$V = k^r$$)이라 하자. $$\Delta^\ast$$가 매끄러운 복소 $$1$$차원이므로 $$L[1]$$이 그 위의 perverse 층이다. [정리 15](#thm15)의 동치 아래 세 가지 표준 연장이 다음 자료에 대응한다.
