...
::: 정리 12
$\operatorname{Perv}(X)$의 simple 대상은 정확히, irreducible closed 부분다양체 $Z \subseteq X$와 그 smooth한 dense 열린 부분 위의 irreducible local system $L$의 쌍 $(Z, L)$에 대한 IC sheaf $\operatorname{IC}_Z(L)$들이다. 서로 다른 쌍 $(Z, L)$ (단, $L$은 동형을 제외)은 서로 isomorphic하지 않은 simple 대상을 주며, 이로써 simple 대상이 $(Z, L)$로 완전히 분류된다.
:::
::: 증명
