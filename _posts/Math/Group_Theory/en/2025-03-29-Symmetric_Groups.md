...
::: Definition 1
For the set $[n]=\{1,\ldots, n\}$, we call the automorphism group $\Aut([n])$ of $[n]$ in $\Set$ the *symmetric group* and write it as $S_n$.
:::
...
::: Example 2
For example, consider the element $\sigma=(1\;2\;3)$ of $S_3$. This function sends $1$ to $2$, $2$ to $3$, and $3$ to $1$. On the other hand, consider the following composition
...
::: Definition 3
Permutations $\sigma_1,\ldots,\sigma_r\in S_n$ are called *disjoint* if whenever some $i$ satisfies $\sigma_i(k)\neq k$, then for all other $j\neq i$ we have $\sigma_j(k)=k$.
:::
...
::: Proposition 4
Any non-identity permutation of $S_n$ can be written as a product of disjoint cycles.
:::
::: Proof
...
:::
::: Proof
By [Proposition 4](#prop4){: data-lid="rifbr" } it suffices to prove that an arbitrary cycle can be written as a product of transpositions.
...
::: Proposition 6
Every element of $S_n$ can be written as a product of $(1\;2)$ and $(1\;2\;\cdots\;n)$.
:::

To prove this we first examine the following.

::: Lemma 7
Let $\sigma\in S_n$. Then $\sigma(1\;2)\sigma^{-1}=(\sigma(1)\;\sigma(2))$ holds.
:::
...
Substituting $\sigma=(1\;2\;\cdots\;n)$, $(1\;2\;\cdots\;n)^2$, $\ldots$ into [Lemma 7](#lem7){: data-lid="iq4mh" }, we obtain $(2\;3)$, $(3\;4)$, $\ldots$. On the other hand, for any $(a\;b)$, assuming without loss of generality that $a<b$,
...
holds, so $(1\;2)$ and $(1\;2\;\cdots\;n)$ can generate all cycles, and hence any element of $S_n$, so [Proposition 6](#prop6){: data-lid="w83dx" } holds.
...
::: Definition 9
For each element $\sigma$ of $S_n$, pairs $(i,j)$ with $i<j$ but $\sigma(i)>\sigma(j)$ are called *inversions*. If the number of inversions is odd, we call $\sigma$ an *odd permutation*, and if the number of inversions is even we call $\sigma$ an *even permutation*.
:::
...
Moreover, since $\sigma$ is a bijection, if we rewrite all terms in this way we see that $\sigma(\Delta)$ is exactly the same polynomial as $\Delta$ up to sign. Therefore, defining $\sgn(\sigma)=\sigma(\Delta)/\Delta$, the quantity $\sgn(\sigma)$ becomes exactly the parity of $\sigma$ defined in the sense of [Definition 9](#def9){: data-lid="x677c" }.
...
::: Definition 11
We call the kernel of the group homomorphism $\sgn$ defined above the *alternating group* of degree $n$, and denote it by $A_n$. In other words, $A_n$ is the set of all even permutations.
:::

We also make the following definition.

::: Definition 12
A group $G$ is called *simple* if it has no normal subgroups other than $\{e\}$ and itself.
:::

One notable fact is that $A_5$ is simple.

::: Example 13
$A_5$ is simple. This can be shown in a more elegant way using Sylow theorems, but $A_5$ is not such a large group, so with a little effort we can show this with only our current knowledge.
...
In general, for a subgroup $N$ of a group $G$ to be a normal subgroup, by definition $N$ must be expressible as a union of conjugacy classes under inner automorphisms. Now using [Lemma 7](#lem7){: data-lid="au756" }, we can see that the 3-cycles and the products of two disjoint 2-cycles each form single conjugacy classes, while the $5$-cycles split into two conjugacy classes each containing 12 elements. The size of a normal subgroup of $A_5$ must divide the size of $A_5$, which is $60$, but no union of non-trivial conjugacy classes containing the identity can produce a divisor of $60$, so $A_5$ has no non-trivial normal subgroup.
:::
