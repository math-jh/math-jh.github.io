...
:::
::: Proof
That is, we must show that the size of $E\setminus E^G$ is a multiple of $p$. But $E\setminus E^G$ is the union of (disjoint) $G$-orbits each of size greater than $1$, and the size of each such orbit is a power of $p$ by [\[Algebraic Structures\] §Group Actions, ⁋Theorem 14](/en/math/algebraic_structures/group_actions#thm14){: data-lid="8cb30" }, so this holds.
:::

In particular, considering the case where $E=G$ and $G$ acts by inner automorphisms, $E^G$ is exactly the center of $G$, so by [Lemma 2](#lem2){: data-lid="q5psv" } we know that the center $Z(G)$ of a $p$-group $G$ is not the trivial group.
...
Therefore, by the equivalence of the first and second conditions of [§Series of a Group, ⁋Proposition 7](/en/math/group_theory/series_of_groups#prop7){: data-lid="cbyes" }, we know that every $p$-group is nilpotent.

On the other hand, by [§Series of a Group, ⁋Proposition 8](/en/math/group_theory/series_of_groups#prop8){: data-lid="owo05" } we obtain the following.
...
so $\lvert E^G\rvert=m$, and now by [Lemma 2](#lem2){: data-lid="w0sse" }
...
:::
::: Proof
Let $E$ be the set of subsets of $G$ with $p^r$ elements. Then by [Lemma 6](#lem6){: data-lid="wmnec" }
...
and view this action as an action on $E$ in the same way as in the proof of [Lemma 6](#lem6){: data-lid="jhsc5" }. Then from the assumption $\lvert E\rvert\not\equiv 0\pmod{p}$, there exists an orbit $O$ whose size is not a multiple of $p$. Now let $X$ be an element of $O$, and let the stabilizer of $X$ be $\Stab(\{X\})=\Stab(X)$. Then $\Stab(X)$ is a subgroup of $G$, and by ([\[Algebraic Structures\] §Group Actions, ⁋Corollary 8](/en/math/algebraic_structures/group_actions#cor8){: data-lid="5p0t3" }) this is the subgroup we want.

To show this, we first obtain from [\[Algebraic Structures\] §Group Actions, ⁋Theorem 14](/en/math/algebraic_structures/group_actions#thm14){: data-lid="2r8ij" }
...
holds. Therefore by [Lemma 2](#lem2){: data-lid="2kbrn" } we know that $\lvert \Syl_p(G) \rvert \equiv \lvert \Syl_p(G)^P \rvert = 1 \pmod{p}$.
:::
...
::: Proof
Apply the second result of [Theorem 8](#thm8){: data-lid="5bmy8" } to the subgroup $f(P_1)$ of $G_2$.
:::
...
Considering the left translation action of $G$ on $G'/P'$, this is a transitive action, so the orbit of $G$ is $G'/P'$ itself. Now by [\[Algebraic Structures\] §Group Actions, ⁋Theorem 14](/en/math/algebraic_structures/group_actions#thm14){: data-lid="aj9q9" }
...
As mentioned earlier, the Sylow theorems are useful for the classification of finite groups. To this end, let us examine [Theorem 8](#thm8){: data-lid="z4myc" } more closely. Let $n_p$ be the size of $\Syl_p(G)$. By the second part of the first result of [Theorem 8](#thm8){: data-lid="4wf13" }, we have $n_p\equiv 1\pmod{p}$. On the other hand, the first part of the first result of [Theorem 8](#thm8){: data-lid="vroku" } shows that $G$ acts transitively on $\Syl_p(G)$, so by [\[Algebraic Structures\] §Group Actions, ⁋Theorem 14](/en/math/algebraic_structures/group_actions#thm14){: data-lid="pkkly" }
...
First consider the Sylow 3-subgroups. Then by [Theorem 8](#thm8){: data-lid="3sckx" } the number $n_3$ of Sylow 3-subgroups satisfies the following two conditions:

- $n_3\equiv 1\pmod{3}$,
- $n_3$ divides $5$.

The only $n_3$ satisfying these two conditions is $1$, and by the result of [Theorem 8](#thm8){: data-lid="grova" } this means that the (unique) Sylow $3$-subgroup $P_3$ of $G$ is a normal subgroup.
...
Now $P_3\cap P_5$ is a subgroup of both $P_3$ and $P_5$, so its order must be $1$, and therefore $P_3\cap P_5=\{e\}$. Now considering the subgroup $P_3P_5$ of $G$, by [\[Algebraic Structures\] §Isomorphism Theorems, ⁋Theorem 5](/en/math/algebraic_structures/isomorphism_theorems#thm5){: data-lid="s0z6m" }
...
Also, as we saw in [§Symmetric Groups, ⁋Example 13](/en/math/group_theory/symmetric_groups#ex13){: data-lid="e936n" }, the elements of $A_5$ consist of $1$ identity element, $20$ elements of order $3$ (3-cycles), $15$ elements of order $2$ (double transpositions), and $24$ elements of order $5$ (5-cycles).
