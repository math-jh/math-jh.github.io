...
:::
::: Proof
We prove this by induction on the order of $G$. The case $G=\{e\}$ is trivial. Now assume that the given statement holds for all $p$-groups of order less than $\lvert G\rvert=p^r$, and let us prove the case $\lvert G\rvert=p^r$. From the previous argument, $C(G)\neq\{e\}$, so there exists some $x\in C(G)$ whose order is $p^s$ ($1\leq s\leq r$).
...
Therefore, by the equivalence of the first and second conditions of [[Group Theory] §Series of Groups, ⁋Proposition 7](/en/math/group_theory/series_of_groups#prop7), we know that every $p$-group is nilpotent.
