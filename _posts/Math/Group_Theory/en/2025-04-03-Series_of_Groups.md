...
so $H' \subseteq N_G(H)$, and the converse is likewise immediate. Finally, if $H$ and $H'$ are both *normal* subgroups of $G$, then for any generator $[h, h'] = h^{-1}h'^{-1}hh'$ of $[H, H']$ and any element $x$ of $G$,
...
We summarize results of this kind for computations in [Lemma 1](#lem1){: data-relation="required" }. Before that, to simplify notation, for the inner automorphism $\rho_g: x \mapsto gxg^{-1}$ we write

$$\rho_{g^{-1}}(x) = x^g$$

Then by [\[Algebraic Structures\] §Group Actions, ⁋Proposition 9](/en/math/algebraic_structures/group_actions#prop9){: data-relation="required" }, for any $g_1, g_2 \in G$ and $x \in G$,
...
::: Lemma 1
For any $x, y, z \in G$ the following hold.
...
::: Proposition 2
For any three subgroups $H, H', H''$ of a group $G$ the following hold.
...
:::
::: Proof
1. For any generator $[h_1, h'] \in [H, H']$ and any $h_2 \in H$ it suffices to show $[h_1, h']^{h_2} \in [H, H']$. This follows from the fourth result of [Lemma 1](#lem1){: data-relation="required" }:
...
2. For convenience let $K$ denote the subgroup of $G$ generated by elements of the form $[h, [h', h'']]$. Then $K \subseteq [H, [H', H'']]$ is obvious, so what we must show is the reverse inclusion $[H, [H', H'']] \subseteq K$. Now $[H, [H', H'']]$ is generated by elements of the form $[h, [h_1', h_1''] \cdots [h_k', h_k'']]$, so we must show that elements of this form lie in $K$.
    In general, fix $h \in H$, $h' \in H'$, $h'' \in H''$ and $x \in G$, and consider the element $[h, [h', h'']x]$. Then by the third result of [Lemma 1](#lem1){: data-relation="required" },

    $$[h, [h', h'']x] = [h, x][h, [h', h'']]^x = [h, x][x, [[h', h''], h]][h, [h', h'']]$$

    From the assumption that $[H', H'']$ normalizes $H$ it follows that $[[h', h''], h]$ is an element of $H$; thus if $x$ were an element of $[H', H'']$ then each term in the above expression would be an element of $[H, [H', H'']]$. Therefore by induction we can show that any generator of $[H, [H', H'']]$ lies in $K$.
...
::: Proposition 4
For a group homomorphism $f: G \rightarrow G'$, $f(C_n(G)) \subseteq C_n(G')$ always holds. Moreover, if $f$ is surjective then $f(C_n(G)) = C_n(G')$ holds.
:::
::: Proof
...
:::
::: Proof
From the third result of [Proposition 2](#prop2){: data-relation="required" } we know
...
::: Proposition 7
For a group $G$ and a natural number $n$, the following are all equivalent.
...
For the remaining equivalence, assuming the first condition, the third condition holds by taking $A = C_n(G)$. To show the first condition holds assuming the third, we send $C_n(G)$ via the canonical morphism $G \rightarrow G/A$; then by [Proposition 4](#prop4){: data-relation="required" } its image equals $C_n(G/A)$, which is $\{e\}$ by assumption, so $C_n(G) \subseteq A$ and therefore $C_{n+1}(G) = \{e\}$.
:::
...
::: Proposition 8
Fix a nilpotent group $G$ of nilpotency class $\leq n$ and a subgroup $H$ of $G$. Then there exists a sequence of subgroups
...
:::
::: Proof
Take a sequence of subgroups of $G$ satisfying the second equivalent condition of [Proposition 7](#prop7){: data-relation="required" }, and set $H_k = HG_k$.
:::
...
The proof of this is the same as for [Proposition 4](#prop4){: data-relation="weak" }, using induction. Just as nilpotent groups were defined by the stability condition of the lower central series, solvable groups are defined by the stability condition of the derived series.
...
By definition $D_0(G) = C_1(G) = G$ and $D_1(G) = [G, G] = C_2(G)$ hold. From this fact and [Proposition 5](#prop5){: data-relation="required" }, by induction we know the inclusion

$$D_n(G) \subseteq C_{2^n}(G)$$

holds. That is, every nilpotent group is always solvable. The following proposition is the characterization of solvable groups corresponding to [Proposition 7](#prop7){: data-relation="weak" }.
...
First, $H' \cap K = H' \cap (H \cap K)$ and $K' \cap H = K' \cap (K \cap H)$ are each normal subgroups of $H \cap K$ by the result of [\[Algebraic Structures\] §Isomorphism Theorems, ⁋Lemma 4](/en/math/algebraic_structures/isomorphism_theorems#lem4){: data-relation="required" }. Therefore their product $(H' \cap K)(K' \cap H)$ is also a normal subgroup of $H \cap K$. Now looking at the left-hand side of the claimed isomorphism, from [\[Algebraic Structures\] §Isomorphism Theorems, ⁋Theorem 5](/en/math/algebraic_structures/isomorphism_theorems#thm5){: data-relation="required" } we know that
...
:::
::: Proof
Insert $G_i \cap H_j$ between $G_i$ and $G_{i+1}$ varying $j$, and insert $G_i \cap H_j$ between $H_j$ and $H_{j+1}$ varying $i$; then the refinements so constructed can be shown to be equivalent to each other via [Lemma 14](#lem14){: data-relation="required" }.
:::
...
is a consequence of [\[Algebraic Structures\] §Isomorphism Theorems, ⁋Theorem 7](/en/math/algebraic_structures/isomorphism_theorems#thm7){: data-relation="required" } and [Theorem 16](#thm16){: data-relation="required" }.

## Solvability of Symmetric Groups

The most classical application of solvability is determining from what point symmetric groups cease to be solvable, and the branching point lies in the simplicity of $A_5$ ([§Symmetric Groups, ⁋Example 13](/en/math/group_theory/symmetric_groups#ex13){: data-relation="required" }).

::: Proposition 18
$A_5$ is not solvable.
:::
::: Proof
First we observe that $A_5$ is non-abelian. If $A_5$ were abelian then for any $g \neq e$ the cyclic subgroup $\langle g \rangle$ would be a normal subgroup (as all subgroups of an abelian group are) and not $\{e\}$, so by the simplicity of $A_5$ ([§Symmetric Groups, ⁋Example 13](/en/math/group_theory/symmetric_groups#ex13){: data-relation="required" }) we would have $\langle g \rangle = A_5$, i.e. $A_5$ would be a cyclic group of order $60$. But then $\langle g^{30} \rangle$ would form a nontrivial proper normal subgroup of order $2$, contradicting simplicity. Therefore $A_5$ is non-abelian.

Now consider the derived series ([Definition 9](#def9){: data-relation="required" }). The commutator subgroup $D_1(A_5) = [A_5, A_5]$ is always a normal subgroup, and since $A_5$ is non-abelian we have $D_1(A_5) \neq \{e\}$. Since $A_5$ is simple we must have $D_1(A_5) = A_5$, and thus by induction $D_n(A_5) = A_5 \neq \{e\}$ for all $n$. Since $D_{n+1}(A_5) = \{e\}$ cannot hold for any $n$, by [Definition 11](#def11){: data-relation="required" } $A_5$ is not solvable.
:::
...
:::
::: Proof
First we observe that a subgroup of a solvable group is again solvable. Applying [Proposition 10](#prop10){: data-relation="required" } to the inclusion $\iota: H \hookrightarrow G$ of a subgroup $H \subseteq G$ gives $D_n(H) \subseteq D_n(G)$, so if $G$ is solvable and $D_n(G) = \{e\}$ for some $n$ then $D_n(H) = \{e\}$ and $H$ is also solvable.

Now if $n \geq 5$, the permutations fixing the elements $\{6, \ldots, n\}$ form a copy of $S_5$, so $A_5 \subseteq S_5 \subseteq S_n$. If $S_n$ were solvable then its subgroup $A_5$ would also have to be solvable, which contradicts [Proposition 18](#prop18){: data-relation="required" }. Therefore $S_n$ is not solvable.
:::
