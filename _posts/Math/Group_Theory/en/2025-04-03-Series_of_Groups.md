...
Then by [\[Algebraic Structures\] §Group Actions, ⁋Proposition 9](/en/math/algebraic_structures/group_actions#prop9){: data-lid="unn2a" data-relation="required" }, for any $g_1, g_2 \in G$ and $x \in G$,
...
:::
::: Proof
From the third result of [Proposition 2](#prop2){: data-lid="xv5lb" data-relation="required" } we know
...
:::
::: Proof
Take a sequence of subgroups of $G$ satisfying the second equivalent condition of [Proposition 7](#prop7){: data-lid="oe5ym" data-relation="required" }, and set $H_k = HG_k$.
:::
...
By definition $D_0(G) = C_1(G) = G$ and $D_1(G) = [G, G] = C_2(G)$ hold. From this fact and [Proposition 5](#prop5){: data-lid="buirc" data-relation="required" }, by induction we know the inclusion

$$D_n(G) \subseteq C_{2^n}(G)$$

holds. That is, every nilpotent group is always solvable. The following proposition is the characterization of solvable groups corresponding to [Proposition 7](#prop7){: data-lid="f7ut4" data-relation="weak" }.
...
First, $H' \cap K = H' \cap (H \cap K)$ and $K' \cap H = K' \cap (K \cap H)$ are each normal subgroups of $H \cap K$ by the result of [\[Algebraic Structures\] §Isomorphism Theorems, ⁋Lemma 4](/en/math/algebraic_structures/isomorphism_theorems#lem4){: data-lid="3a9cx" data-relation="required" }. Therefore their product $(H' \cap K)(K' \cap H)$ is also a normal subgroup of $H \cap K$. Now looking at the left-hand side of the claimed isomorphism, from [\[Algebraic Structures\] §Isomorphism Theorems, ⁋Theorem 5](/en/math/algebraic_structures/isomorphism_theorems#thm5){: data-lid="i8rsw" data-relation="required" } we know that
...
:::
::: Proof
Insert $G_i \cap H_j$ between $G_i$ and $G_{i+1}$ varying $j$, and insert $G_i \cap H_j$ between $H_j$ and $H_{j+1}$ varying $i$; then the refinements so constructed can be shown to be equivalent to each other via [Lemma 14](#lem14){: data-lid="roxna" data-relation="required" }.
:::
...
is a consequence of [\[Algebraic Structures\] §Isomorphism Theorems, ⁋Theorem 7](/en/math/algebraic_structures/isomorphism_theorems#thm7){: data-lid="rp4oa" data-relation="required" } and [Theorem 16](#thm16){: data-relation="required" }.
...
Now consider the derived series ([Definition 9](#def9){: data-lid="v9kqs" data-relation="required" }). The commutator subgroup $D_1(A_5) = [A_5, A_5]$ is always a normal subgroup, and since $A_5$ is non-abelian we have $D_1(A_5) \neq \{e\}$. Since $A_5$ is simple we must have $D_1(A_5) = A_5$, and thus by induction $D_n(A_5) = A_5 \neq \{e\}$ for all $n$. Since $D_{n+1}(A_5) = \{e\}$ cannot hold for any $n$, by [Definition 11](#def11){: data-lid="vi823" data-relation="required" } $A_5$ is not solvable.
:::
...
:::
::: Proof
First we observe that a subgroup of a solvable group is again solvable. Applying [Proposition 10](#prop10){: data-lid="7pr8r" data-relation="required" } to the inclusion $\iota: H \hookrightarrow G$ of a subgroup $H \subseteq G$ gives $D_n(H) \subseteq D_n(G)$, so if $G$ is solvable and $D_n(G) = \{e\}$ for some $n$ then $D_n(H) = \{e\}$ and $H$ is also solvable.

Now if $n \geq 5$, the permutations fixing the elements $\{6, \ldots, n\}$ form a copy of $S_5$, so $A_5 \subseteq S_5 \subseteq S_n$. If $S_n$ were solvable then its subgroup $A_5$ would also have to be solvable, which contradicts [Proposition 18](#prop18){: data-lid="nqgvi" data-relation="required" }. Therefore $S_n$ is not solvable.
:::
