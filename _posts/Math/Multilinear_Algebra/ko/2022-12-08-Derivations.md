...
우리가 가장 관심있는 예시는 $\Delta=\mathbb{Z}$인 경우이다. 이 경우, [정의 1](#def1){: data-lid="3tkp7" }에 의하여 $\varepsilon$은 $\varepsilon(1,1)$에서의 값에 의해 완전하게 결정되며, 따라서 $\Delta=\mathbb{Z}$에 정의되는 commutation factor는 오직
...
두 번째 경우를 motivation 삼아 우리는 표기의 편의상 $d, d', d''$를 모두 같은 문자 $d$로 통일하여 쓰기도 하며, 그럼 [정의 2](#def2){: data-lid="xzt8b" }의 식은
...
우선 degree $0$ derivation은 항상 commutation factor를 무시할 수 있으므로, $E$를 non-graded $\mathbb{K}$-algebra로 본 후 $E$에서 $E$로의 derivation을 생각하면 $\varepsilon$은 등장하지 않는다. 이제 각각의 $i$에 대하여, $\partial_i:E \rightarrow E$를 편미분 $\partial/\partial \x_i$로 정의하면 Leibniz rule에 의해 [정의 2](#def2){: data-lid="sl6o3" }의 등식이 만족된다. 
...
의 꼴로 생성되는 free $E$-module이다. ([§텐서대수, ⁋명제 13](/ko/math/multilinear_algebra/tensor_algebras#prop13){: data-lid="5d9yv" }) 이제 각각의 원소
...
한편, 위의 두 경우 중 첫째 조건이 성립한다 가정하자. 그럼 $d=(d,d',d'')$은 $(E,E',E'')$에서 자기자신으로의 함수로 생각할 수 있으므로, $\varepsilon$-derivation들의 합성 또한 생각할 수 있다. 그러나 일반적으로 [정의 2](#def2){: data-lid="7dx3t" }의 식을 보면, 임의로 주어진 두 degree $\delta_1$, $\delta_2$의 $\varepsilon$-derivation $d_1,d_2$와 임의의 $x\in E$, $x'\in E'$에 대하여
...
이제 우리는 [정의 2](#def2){: data-lid="vto0t" } 이후 다뤘던 두 가지 특별한 경우 중 두 번째 경우를 살펴본다. 즉 $\Delta$-graded $A$-algebra $E$와 graded $A$-module $F$, 그리고 두 개의 곱셈 $E\otimes_AF \rightarrow F$와 $F\otimes_AE \rightarrow F$가 주어졌다 하자. 
...
:::
::: 증명
[명제 6](#prop6){: data-lid="duuoc" }에 의해 $\dd{(1)} = 0$이므로,
...
::: 증명
임의의 derivation $d:E \rightarrow F$가 주어졌다 하고, $d$를 $K$ 위로 확장한 $\bar{d}$가 존재한다면, [명제 7](#prop7){: data-lid="m8qfk" }을 적용하여 다음의 식
...
따라서, $E$가 associative graded $A$-algebra라면 [정의 10](#def10){: data-lid="6kzg0" }을 통해 $E$의 임의의 homogeneous element가 $E$에서 자기 자신으로의 $\varepsilon$-derivation을 정의하며, 우리는 이를 *inner $\varepsilon$-derivation*이라 부른다. 
