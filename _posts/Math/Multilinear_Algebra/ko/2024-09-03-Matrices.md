---
title: "행렬"
description: "일반적인 환 위에서 정의된 행렬의 기본 정의부터 부분행렬, 전치행렬까지 정리하고, 행렬 연산을 통해 아벨 군과 모듈 구조를 살펴본다."
...
기초 선형대수학에서의 정의와 마찬가지로, 두 개의 행렬 $X\in\Mat_{I\times J}(A)$, $Y\in\Mat_{K\times L}(A)$에 대하여, 행렬의 곱 $XY$는 오직 $J=K$일 때만 정의되며, 이 경우 $XY$는 $I\times L$ 행렬이 될 것이다. [\[선형대수학\] §행렬, ⁋정의 2](/ko/math/linear_algebra/matrices#def2) 이후에 살펴본 행렬의 곱의 각 성분의 표현을 따라, 일반적인 상황에서도 $XY$의 $(i,l)$ 성분은 다음의 식
...
다만 [\[선형대수학\] §행렬, ⁋명제 8](/ko/math/linear_algebra/matrices#prop8)과는 다르게 행렬의 곱과 전치행렬의 관계를 생각할 때는 주의해야 할 점이 있는데, 식
