...
는 일반적으로 성립하지 않는다는 것이다. 이는 양 변을 성분별로 써 보면 바로 확인할 수 있고, 무엇이 문제인지도 명확한데, 바로 $A$가 commutative가 아니기 때문이다. 위의 식은 우변의 두 행렬 $X^t, Y^t$가 모두 $A$의 곱의 순서를 뒤집은 opposite ring 위에서 정의된 행렬이라 생각하면 말이 되지만 ([\[환론\] §아틴-웨더번 정리, ⁋보조정리 9](/ko/math/ring_theory/artin_wedderburn#lem9){: data-relation="forward" }), 대부분의 경우 우리는 $A$가 commutative인 경우를 생각할 것이므로 이는 큰 의미가 없다. 
