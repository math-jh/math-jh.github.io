...
---
Given a group $H$ and a ring $A$, we have defined the group ring $AH$. ([\[Algebraic Structures\] §Algebras, ⁋Definition 5](/en/math/algebraic_structures/algebras#def5){: data-lid="n282q" }) Now fix an $AH$-module $M$, and define $M^H$ as the set
...
1. The product $x^k$ of $x$ defined in [Proposition 4](#prop4){: data-lid="8yg92" data-relation="required" } equals $k!\gamma_k(x)$.
...
:::
::: Proof
1. Use the second formula of [Corollary 5](#cor5){: data-lid="9lo98" data-relation="required" }.
2. For each $k$, set $H=S_k$, $N=\T^kM$ and apply [Lemma 6](#lem6){: data-lid="aos4n" data-relation="required" }.
:::
...
is well-defined. We write $\T(u)\vert_{\Sym(M)}$ simply as $\Sym(u)$. Then we know that $\Sym$ becomes a functor, and moreover from the corresponding properties in [§Tensor Algebra](/en/math/multilinear_algebra/tensor_algebras){: data-lid="qskm9" data-relation="required" } we know that the natural isomorphism
...
For the symmetrization map $s:\T(M) \rightarrow \Sym(M)$ obtained in this way, by [§Tensor Algebra, ⁋Proposition 6](/en/math/multilinear_algebra/tensor_algebras#prop6){: data-lid="c32gr" data-relation="required" } we obtain $\bar{s}: \S(M) \rightarrow \Sym(M)$ satisfying
...
For a reason similar to what we observed in [Remark 8](#rmk8){: data-lid="dxro4" data-relation="required" } above, if $A$ is a $\mathbb{Q}$-algebra (as well as a $\mathbb{Z}$-algebra), then $x\mapsto n!.x$ is a bijection, and thus the $\bar{s}: \S(M) \rightarrow \Sym(M)$ defined above becomes an isomorphism.

## Polynomial Mappings

On the other hand, the symmetric algebra $\S(M)$ could be thought of as a representation of symmetric $n$-linear maps, and thinking of $\Sym(M)$ together via [Proposition 9](#prop9){: data-lid="7smfc" data-relation="required" } above, we obtain the following proposition.
...
::: Proposition 12
Assume all the conditions of [Proposition 10](#prop10){: data-lid="7rykx" data-relation="required" }, and additionally assume that $y\mapsto n!.y$ is an automorphism of $N$. Then for any $u\in\Poly^n(M,N)$, there exists a *unique* symmetric $n$-linear map $v:M^n \rightarrow N$ satisfying the identity
...
This is a consequence that follows from [Corollary 5](#cor5){: data-lid="8g6xk" data-relation="required" }. Now combining this with the observation in [Remark 8](#rmk8){: data-lid="lzu75" data-relation="required" }, we obtain the following result.

::: Proposition 13
Assume the situation of [Proposition 10](#prop10){: data-lid="3rpqm" data-relation="required" }, and consider the canonical homomorphism $\Hom_A(\Sym^n(M), N) \rightarrow \Poly^n(M,N)$. Then the following hold.
...
::: Remark 17
Schur polynomials are described in several equivalent ways. First, by the *Jacobi–Trudi identity* using the $h_k$ from [Definition 15](#def15){: data-lid="vmzkl" data-relation="required" },
