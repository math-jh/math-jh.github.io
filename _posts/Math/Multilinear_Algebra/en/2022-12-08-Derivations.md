...
The example of greatest interest to us is the case $\Delta=\mathbb{Z}$. In this case, by [Definition 1](#def1){: data-relation="required" }, $\varepsilon$ is completely determined by its value at $\varepsilon(1,1)$; therefore, the commutation factors defined on $\Delta=\mathbb{Z}$ are only
...
Motivated by the second case, we often abuse notation by writing all of $d, d', d''$ with the same letter $d$; then the formula of [Definition 2](#def2){: data-relation="required" } can be written as
...
([§Tensor Algebra, ⁋Proposition 13](/en/math/multilinear_algebra/tensor_algebras#prop13){: data-relation="required" }) Now for each basis element
...
Now assume that the first of the two cases above holds. Then $d=(d,d',d'')$ can be regarded as a function from $(E,E',E'')$ to itself, so we may also consider the composition of $\varepsilon$-derivations. However, in general, looking at the formula of [Definition 2](#def2){: data-relation="required" }, for arbitrary $\varepsilon$-derivations $d_1,d_2$ of degrees $\delta_1$, $\delta_2$ and arbitrary $x\in E$, $x'\in E'$, we have
...
We now examine the second of the two special cases discussed after [Definition 2](#def2){: data-relation="required" }. That is, let a $\Delta$-graded $A$-algebra $E$ and a graded $A$-module $F$ be given, together with two multiplications $E\otimes_AF \rightarrow F$ and $F\otimes_AE \rightarrow F$.
...
:::
::: Proof
By [Proposition 6](#prop6){: data-relation="required" } we have $\dd{(1)} = 0$, so
...
::: Proof
Let an arbitrary derivation $d:E \rightarrow F$ be given, and suppose an extension $\bar{d}$ of $d$ to $K$ exists. Applying [Proposition 7](#prop7){: data-relation="required" }, we know that the formula
...
Therefore, if $E$ is an associative graded $A$-algebra, then by [Definition 10](#def10){: data-relation="required" } any homogeneous element of $E$ defines an $\varepsilon$-derivation from $E$ to itself, and we call this an *inner $\varepsilon$-derivation*.
