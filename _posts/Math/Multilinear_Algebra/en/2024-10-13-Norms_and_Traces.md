...
description: "We define the trace, norm, and characteristic polynomial of an element in an algebra over a commutative ring, and examine their relationship with matrix representations. We also prove a multiplicative formula for submodule reduction chains."
...
last_polished_at: 2026-06-01T19:00:06+00:00
---
...
::: Definition 1
Let an $E$-module $M$ be given, and suppose that $M$ has a finite basis as an $A$-module. For any $\alpha\in E$, consider the $E$-module endomorphism
...
In particular, let a commutative $A$-algebra $A'$ be given. Then via $M'=A'\otimes_AM$ and $E'=A'\otimes_AE$ we can regard $M'$ as an $E'$-module, and in this case the basis is given by tensoring $1\in A'$ with an $E$-basis of $M$, so again from the matrix representation we obtain
...
In particular, when $A' = A[\x]$,

$$\chi_{M/A,\,\alpha}(\x) = N_{M[\x]/A[\x]}(\x - \alpha)$$

holds.

A notable fact is that the above construction depends only on the isomorphism class of $M$. This is because if there is an isomorphism from $M$ to $M'$, then the basis of $M$ is carried to a basis of $M'$ along this isomorphism, and the matrix representation of $\alpha_{M'}$ with respect to this basis is the same as the matrix representation of $\alpha_M$ with respect to the original basis. The following proposition can also be proved in a similar manner.

::: Proposition 2
Let a decreasing sequence of submodules of an $E$-module $M$
...
has a block upper-triangular form with diagonal blocks $X_{ii}$. Since the trace is given by the sum of the diagonal entries, the determinant by the product of the diagonal entries, and the characteristic polynomial by the product of the diagonal terms, the proposition follows.
:::
...
:::
::: Proof
Suppose that $\alpha$ has an inverse $\alpha' \in E$. Then
