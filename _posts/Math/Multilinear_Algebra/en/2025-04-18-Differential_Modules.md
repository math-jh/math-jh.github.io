...
---
Now we define differential modules. For this we need a simple lemma; first consider the second case assumed in [§Differentiation, ⁋Definition 2](/en/math/multilinear_algebra/derivations#def2){: data-relation="required" }. That is, we consider a commutative ring $A$, a $\Delta$-graded $A$-algebra $E$, a graded $A$-module $F$, and an $\varepsilon$-derivation $d:E \rightarrow F$.
...
::: Proposition 2
Assume the situation of [Proposition 1](#prop1){: data-relation="required" }, and let an $\varepsilon$-derivation $d': F \rightarrow N$ be given. Then $d'$ is left (resp. right) $E$-linear if and only if $d'$ vanishes identically on the subalgebra $\rho(E)$ of $F$.
:::

Now define $\Der_A(F, N)$ to be the set of $A$-derivations from $F$ to $N$. Then the subset of derivations satisfying the condition of [Proposition 2](#prop2){: data-relation="required" } — that is, those that become $E$-linear — coincides with the set of derivations vanishing identically on $\rho(E)$; hence this subset forms an $A$-submodule of $\Der_A(F, N)$. We denote it by $\Der_E(F,N)$.
...
::: Proposition 7
The map $\delta_E$ obtained in [Lemma 6](#lem6){: data-relation="required" } satisfies the following universal property.
...
:::
::: Proof
First, by [Proposition 1](#prop1){: data-relation="required" } we know that for every $(E, E)$-bimodule homomorphism $f: \mathfrak{I} \rightarrow M$, the composition $f \circ \delta_E$ is an $A$-derivation from $E$ to $M$.

On the other hand, for uniqueness, from the definition of $\delta_E$ we know that we must have

$$f(x \otimes 1 - 1 \otimes x) = \dd{x},$$

and since by [Lemma 6](#lem6){: data-relation="required" } $\mathfrak{I}$ is generated by the image of $\delta_E$, any $f$ satisfying the given condition must be unique if it exists. Moreover, using the calculation from the preceding lemma, we know that for any $\sum x_i \otimes y_i\in \mathfrak{I}$ the formula
...
On the other hand, since $\Omega_{A'}(E')$ is an $A'$-module, by [\[Algebraic Structures\] §Change of Base Ring, ⁋Proposition 6](/en/math/algebraic_structures/change_of_base_ring#prop6){: data-relation="required" } we obtain from [Proposition 11](#prop11){: data-relation="required" } the following $A'$-linear map:
...
is exact for any $E'$-module $N$. However, using the commutative diagrams employed before and after [Proposition 12](#prop12){: data-relation="required" }, we can convert this into the sequence of modules of derivations

$$0 \rightarrow \Der_E(E', N) \rightarrow \Der_A(E', N) \rightarrow \Der_A(E, N),$$

and we showed immediately after [Proposition 2](#prop2){: data-relation="required" } that this is exact.
:::
