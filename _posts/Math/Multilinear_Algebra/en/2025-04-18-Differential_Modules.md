...
---
Now we define differential modules. For this we need a simple lemma; first consider the second case assumed in [§Differentiation, ⁋Definition 2](/en/math/multilinear_algebra/derivations#def2){: data-lid="tonzh" }. That is, we consider a commutative ring $A$, a $\Delta$-graded $A$-algebra $E$, a graded $A$-module $F$, and an $\varepsilon$-derivation $d:E \rightarrow F$.
...
::: Proposition 2
Assume the situation of [Proposition 1](#prop1){: data-lid="u3lh3" }, and let an $\varepsilon$-derivation $d': F \rightarrow N$ be given. Then $d'$ is left (resp. right) $E$-linear if and only if $d'$ vanishes identically on the subalgebra $\rho(E)$ of $F$.
:::

Now define $\Der_A(F, N)$ to be the set of $A$-derivations from $F$ to $N$. Then the subset of derivations satisfying the condition of [Proposition 2](#prop2){: data-lid="l73b2" } — that is, those that become $E$-linear — coincides with the set of derivations vanishing identically on $\rho(E)$; hence this subset forms an $A$-submodule of $\Der_A(F, N)$. We denote it by $\Der_E(F,N)$.
...
::: Proposition 3
A graded $K$-linear map $f: A \rightarrow E$ of degree $\delta$ is an $\varepsilon$-derivation if and only if the map $x \mapsto (x, f(x))$ is a graded $K$-algebra homomorphism from $A$ to $A \oplus E[\delta]$.
:::
...
::: Proposition 7
The map $\delta_E$ obtained in [Lemma 6](#lem6){: data-lid="ogpuq" } satisfies the following universal property.

> For every $(E,E)$-bimodule $M$ and every $A$-derivation $d: E \rightarrow M$, there exists a unique $(E,E)$-bimodule homomorphism $f: \mathfrak{I} \rightarrow M$ such that $d=f\circ\delta_E$.
:::
::: Proof
First, by [Proposition 1](#prop1){: data-lid="ntesk" } we know that for every $(E, E)$-bimodule homomorphism $f: \mathfrak{I} \rightarrow M$, the composition $f \circ \delta_E$ is an $A$-derivation from $E$ to $M$.

On the other hand, for uniqueness, from the definition of $\delta_E$ we know that we must have

$$f(x \otimes 1 - 1 \otimes x) = \dd{x},$$

and since by [Lemma 6](#lem6){: data-lid="ip4fi" } $\mathfrak{I}$ is generated by the image of $\delta_E$, any $f$ satisfying the given condition must be unique if it exists. Moreover, using the calculation from the preceding lemma, we know that for any $\sum x_i \otimes y_i\in \mathfrak{I}$ the formula

$$f\left( \sum_i x_i \otimes y_i \right) = \sum_i x_i  f(1 \otimes y_i - y_i \otimes 1) = - \sum_i x_i  \dd{y_i}$$

must necessarily hold. Therefore, to show existence we must verify that this defines an $(E,E)$-bimodule homomorphism. For this, first note that the map $(x, y) \mapsto -x \cdot \dd{y}$ is an $A$-bilinear map from $E$ to $M$, so from this we know that the $A$-bilinear map $g: E \otimes E \rightarrow M$ defined by $g(x \otimes y) = -x \cdot \dd{y}$ is well-defined. Then its restriction to $\mathfrak{I}$ equals $f$, and it now suffices to show that this restriction preserves the $E$-bimodule structure, which is a simple calculation.
:::
...
::: Definition 9
The $E$-module $\mathfrak{I}/\mathfrak{I}^2$ is called the ($E$-)module of *$A$-differentials*, and is denoted by $\Omega_{A}(E)$ or $\Omega_{E/A}$. Also, we write $\delta_{E/A}(x)$ as $d_{E/A}(x)$, and when there is no danger of confusion we simply write this as $\dd{x}$. For each $x \in E$, we call $d_{E/A}(x)$ the *differential* of $x$.
:::
...
holds, and moreover, if we write this isomorphism as $\omega:M\otimes_A\S(M)\rightarrow\Omega_{\S(M)/A}$, then for any $x\in M$ we know that $\omega(x\otimes1)=d_{\S(M)/A}(x)=\dd{x}$.

In particular, if $M$ is a free $A$-module of finite rank $n$, then $\S(M)$ can be identified with the polynomial algebra $A[\x_1,\ldots, \x_n]$, and under this identification the $\dd{\x_i}$ are *really* the images of the $\x_i$ under $d=d_{\S(M)/A}$; moreover, if the image of a polynomial $p\in A[\x_1,\ldots, \x_n]$ under $d$ is expressed as a linear combination of the basis elements $\dd{\x_i}$ of $\Omega_{\S(M)/A}$, then the coefficients attached to them are precisely the $i$-th partial derivatives $\partial p/\partial \x_i$ of $p$.
:::
...
:::
::: Proof
The proof is nothing but an appropriate use of the other universal properties employed in proving [Proposition 8](#prop8){: data-lid="5h9gb" }.
:::
...
On the other hand, since $\Omega_{A'}(E')$ is an $A'$-module, by [\[Algebraic Structures\] §Change of Base Ring, ⁋Proposition 6](/en/math/algebraic_structures/change_of_base_ring#prop6){: data-lid="fznhe" } we obtain from [Proposition 11](#prop11){: data-lid="0d3dl" } the following $A'$-linear map:
...
given by the universal property of [Proposition 8](#prop8){: data-lid="w5u4e" }, we obtain the following commutative diagram:

{% diagram Math/Multilinear_Algebra/Differential_Modules-3.svg width="29.41em" alt="change_of_base_ring-3" %}

Here the right-hand vertical map is the composite of the above isomorphism and the isomorphism of [\[Algebraic Structures\] §Change of Base Ring, ⁋Proposition 6](/en/math/algebraic_structures/change_of_base_ring#prop6){: data-lid="mrxt7" },
...
::: Proposition 12
Suppose $E' = E \otimes_AA'$, and let $\eta : A \rightarrow E'$ and $u : E \rightarrow E'$ be the canonical morphisms. Then the $A'$-linear map
...
is exact for any $N$. Now since $\Hom$ is a left exact functor ([§Projective, Injective, and Flat Modules, ⁋Proposition 2](/en/math/multilinear_algebra/various_modules#prop2){: data-lid="mvd8r" }), the exactness of the above sequence for all $N$ is equivalent to the exactness of the sequence
...
:::
::: Proof
Again from the fact that $\Hom$ is a left exact functor ([§Projective, Injective, and Flat Modules, ⁋Proposition 2](/en/math/multilinear_algebra/various_modules#prop2){: data-lid="54shr" }), it suffices to show that the sequence

$$0 \rightarrow \Hom_{E'}(\Omega_{E'/E},N) \rightarrow \Hom_{E'}(\Omega_{E'/A}, N) \rightarrow \Hom_{E'}(\Omega_{E/A}\otimes_EE',N)$$

is exact for any $E'$-module $N$. However, using the commutative diagrams employed before and after [Proposition 12](#prop12){: data-lid="gergw" }, we can convert this into the sequence of modules of derivations

$$0 \rightarrow \Der_E(E', N) \rightarrow \Der_A(E', N) \rightarrow \Der_A(E, N),$$

and we showed immediately after [Proposition 2](#prop2){: data-lid="psha7" } that this is exact.
:::
...
:::
::: Proof
Using the notation of the preceding argument, we can identify $\Omega_{E/A}\otimes_EE'$ with $\Omega_{E/A}/\mathfrak{I}\Omega_{E/A}$. Then under this identification, the image of $\overline{d}$ becomes the image of $\dd{(\mathfrak{I})}\subseteq\Omega_{E/A}$ in the quotient module $\Omega_{E/A}/\mathfrak{I}\Omega_{E/A}$. Therefore, taking the $A$-submodule $I$ of $\Omega_{E/A}$ to be that generated by $\mathfrak{I}\Omega_{E/A}$ and $\dd{(\mathfrak{I})}$, we obtain the isomorphism
