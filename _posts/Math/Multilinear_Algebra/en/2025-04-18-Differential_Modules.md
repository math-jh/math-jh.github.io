...
must necessarily hold. Therefore, to show existence we must verify that this defines an $(E,E)$-bimodule homomorphism. For this, first note that the map $(x, y) \mapsto -x \cdot dy$ is an $A$-bilinear map from $E$ to $M$, so from this we know that the $A$-bilinear map $g: E \otimes E \rightarrow M$ defined by $g(x \otimes y) = -x \cdot dy$ is well-defined. Then its restriction to $\mathfrak{I}$ equals $f$, and it now suffices to show that this restriction preserves the $E$-bimodule structure, which is a simple calculation.
:::
...
:::
::: Proof
Using the notation of the preceding argument, we can identify $\Omega_{E/A}\otimes_EE'$ with $\Omega_{E/A}/\mathfrak{I}\Omega_{E/A}$. Then under this identification, the image of $\overline{d}$ becomes the image of $d(\mathfrak{I})\subseteq\Omega_{E/A}$ in the quotient module $\Omega_{E/A}/\mathfrak{I}\Omega_{E/A}$. Therefore, taking the $A$-submodule $I$ of $\Omega_{E/A}$ to be that generated by $\mathfrak{I}\Omega_{E/A}$ and $d(\mathfrak{I})$, we obtain the isomorphism
