...
---
We now define the determinant; to this end, we first define the tensor algebra, the symmetric algebra, and the exterior algebra. Throughout, $A$ will always be taken to be a commutative ring. In particular, $A$ then has the IBN property. ([§Bases, ⁋Proposition 6](/en/math/multilinear_algebra/basis_of_free_modules#prop6){: data-lid="bqaw2" data-relation="required" })
...
([\[Algebraic Structures\] §Algebras, ⁋Proposition 4](/en/math/algebraic_structures/algebras#prop4){: data-lid="ud1ni" data-relation="required" }). This is not merely an algebra, but naturally carries the structure of an $\mathbb{N}_{\geq 0}$-graded associative unital algebra. We name it as follows.
...
This is because each $\T^n(M)$ has as its basis the $e_s$ defined using finite sequences $s$ of length $n$, and their direct sum is $\T(M)$. On the other hand, we know from [§Bases, ⁋Definition 9](/en/math/multilinear_algebra/basis_of_free_modules#def9){: data-lid="9rkz1" data-relation="weak" } that the structure constants can be used to describe the multiplication in $\T(M)$; according to the above explanation, this is nothing other than concatenation of sequences. That is, for two sequences
...
let us obtain the $A$-linear map $i: M \rightarrow \phi^\ast\phi_!M$ corresponding to $\id_{\phi_!M}$. ([\[Algebraic Structures\] §Change of Base Ring, ⁋Proposition 6](/en/math/algebraic_structures/change_of_base_ring#prop6){: data-lid="lns8g" data-relation="required" }) Then, viewing the $A$-module $\phi^\ast\phi_!M$ as the $B$-module $\phi_!M$ and considering

$$\iota_{\phi_!M}: \phi_!M \rightarrow \T_B(\phi_!M)$$

this is an $A$-linear map from the $A$-module $M$ to the $A$-module $\phi^\ast \T_B(\phi_!M)$ (more precisely, $U\phi^\ast \T_B(\phi^\ast\phi_!M)$). Therefore, by [Proposition 2](#prop2){: data-relation="required" }, there exists a unique $A$-algebra homomorphism $T_A(M)\rightarrow \phi^\ast T_{B}(\phi_!M)$ making the following diagram
...
Now recall an $A$-module $M$, its dual module $M^\ast$, and the Kronecker pairing $\langle x,\xi\rangle$ between them. ([§Dual Spaces, ⁋Definition 1](/en/math/multilinear_algebra/dual_spaces#def1){: data-lid="clvkt" data-relation="weak" }) Many objects treated in linear algebra can be found inside tensor products of several copies of $M$ and $M^\ast$; for example, if $M$ is finitely generated projective, then by [§Hom and the Tensor Product, ⁋Corollary 4](/en/math/multilinear_algebra/hom_and_tensor#cor4){: data-lid="30pl4" data-relation="weak" } we have $M^\ast\otimes_AM\cong \End_\rMod{A}(M)$. To treat such objects all at once, consider the tensor algebra $\T(M\oplus M^\ast)$; applying the direct sum decomposition examined in the previous section to $M_1=M$, $M_2=M^\ast$, we obtain the isomorphism
...
In the simplest case $p=q=1$, the map $c^1_1: M\otimes_AM^\ast\rightarrow A$ sends $x\otimes\xi$ to $\langle x,\xi\rangle$, and viewing this through the canonical isomorphism $M\otimes_AM^\ast\cong M^\ast\otimes_AM$ that swaps the two components, it coincides with the $A$-linear map $\tau: M^\ast\otimes_AM \rightarrow A$ defined in [§Hom and the Tensor Product, §§Trace](/en/math/multilinear_algebra/hom_and_tensor#trace){: data-lid="oea59" data-relation="weak" }. In particular, if $M$ is finitely generated projective, then under the isomorphism $M^\ast\otimes_AM\cong\End_\rMod{A}(M)$ mentioned above, $c^1_1$ becomes exactly the trace map of [§Hom and the Tensor Product, ⁋Definition 6](/en/math/multilinear_algebra/hom_and_tensor#def6){: data-lid="sdtjh" data-relation="weak" }. In this sense, the general contraction $c^i_j$ can be thought of as the operation that takes the trace on the type $(1,1)$ part formed by the $i$-th contravariant component and the $j$-th covariant component, while leaving the remaining components unchanged.

Let us also examine this in coordinates. Suppose $M$ is a finitely generated free $A$-module, and fix a basis $(e_k)_{1\leq k\leq r}$ of $M$ and its dual basis $(e_k^\ast)_{1\leq k\leq r}$. ([§Dual Spaces, ⁋Definition 6](/en/math/multilinear_algebra/dual_spaces#def6){: data-lid="90fr3" data-relation="required" }) Then by the same argument as in [Proposition 3](#prop3){: data-lid="1qkdk" data-relation="required" }, $\T^p_q(M)$ is a free $A$-module with basis the elements of the form
...
On the other hand, the following universal property is also obtained immediately from the universal property of the quotient algebra and [Proposition 2](#prop2){: data-lid="ggt02" data-relation="required" }.
...
The result corresponding to [Proposition 4](#prop4){: data-lid="1pw5c" data-relation="required" } is the following proposition, and its proof is identical as well.
...
It is customary to write the product of elements in $\bigwedge(M)$ as $\wedge$. On the other hand, by the same discussion as after [Definition 5](#def5){: data-lid="dq044" data-relation="weak" }, it is obvious that $\mathfrak{J}$ is a homogeneous ideal and that the canonical inclusion $\iota:M \hookrightarrow\bigwedge(M)$ exists. Also, for the same reason as in [Proposition 6](#prop6){: data-lid="p2nhc" data-relation="required" }, the following universal property holds.
...
A property similar to [Proposition 7](#prop7){: data-lid="hasyo" data-relation="weak" } also holds for the exterior algebra. In what follows we assume that the characteristic of $A$ is not $2$. For arbitrary $A$-modules $M,N$ and an integer $n\geq 1$, an *alternating linear map* $f$ from $M^n$ to $N$ is one satisfying the condition
...
[^1]: Recall that the coproduct in the category $\Ring$ was defined in a manner similar to the free product. ([§Product, Coproduct, and Tensor Product of Rings, ⁋Proposition 4](/en/math/algebraic_structures/operations_of_rings#prop4){: data-lid="73zwj" data-relation="weak" }) On the other hand, in the same post we also verified that the coproduct in the category $\cRing$ is given by the tensor product $\otimes$.
