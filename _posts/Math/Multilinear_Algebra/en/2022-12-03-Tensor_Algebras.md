...
date: 2022-12-03
weight: 10
translated_at: 2026-07-21T05:49:10+00:00
translation_source: kimi-cli
---
We now define the determinant; to this end, we first define the tensor algebra, the symmetric algebra, and the exterior algebra. Throughout this process, $A$ will always be assumed to be a commutative ring. In particular, $A$ then has the IBN property. ([§Bases, ⁋Proposition 6](/en/math/multilinear_algebra/basis_of_free_modules#prop6))
...
We shall write each component $M^{\otimes n}$ as $\T^n(M)$. Then since $\T^1(M)=M$, there is a canonical injection $\iota: M \rightarrow \T(M)$ from $M$ into $\T(M)$.
...
::: Proposition 2
Let an $A$-algebra $E$ and an $A$-linear map $u:M \rightarrow E$ be given. Then there exists a unique $A$-algebra homomorphism $g: \T(M) \rightarrow E$ such that $f=g \circ\iota$.
...
This may look complicated as a formula, but it is essentially nothing more than unpacking the coproduct of graded algebras on the right-hand side, since $T$ is a left adjoint:
...
By definition, since $\mathfrak{I}$ is a homogeneous ideal, it is obvious that $\T(M)/\mathfrak{I}$ becomes a $\mathbb{Z}_{\geq 0}$-graded ideal. Moreover, since each generator $x\otimes y-y\otimes x$ is an element of degree $2$, taking the quotient by $\mathfrak{I}$ has no effect on $\T^0(M)$ and $\T^1(M)$. That is, $\S^0(M)\cong A$ and $\S^1(M)\cong M$.
...
A property similar to [Proposition 7](#prop7) also holds for the exterior algebra. For arbitrary $A$-modules $M,N$ and an integer $n\geq 1$, an *alternating linear map* $f$ from $M^n$ to $N$ is one satisfying the condition
