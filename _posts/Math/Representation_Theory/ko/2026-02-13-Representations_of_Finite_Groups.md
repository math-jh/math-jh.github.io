...
이와 마찬가지 관점에서 우리는 임의의 representation $V,W$에 대하여, 이들의 벡터공간에서의 연산을 이용하여 $V\oplus W$, $V\otimes W$ 등을 정의할 수 있다. 다음 정의에서 다소 주의할 것은, 위의 [정의 2](#def2){: data-lid="clc16" }와는 다르게 $V\otimes W$ 등에서는 <em-ko>자연스러운</em-ko> $G$-action이 존재하지 않을 수도 있다는 것으로, 우리는 이 때문에 각 벡터공간 위에 $G$-action을 명시적으로 정의해준다. 
...
위의 [정의 3](#def3){: data-lid="qebu2" }에서 tensor product와 $\Hom$의 경우 다소 그 정의가 인위적으로 보일 수 있는데, 이를 탐구하기 위해서는  group algebra의 언어가 유용하다. ([\[대수적 구조\] §대수, ⁋정의 5](/ko/math/algebraic_structures/algebras#def5){: data-lid="8vat6" }) 이를 간단히 리뷰하자면, 집합으로서 $\mathbb{C}[G]$는 $G$에서 $\mathbb{C}$로의 함수들의 모임이었다. 각각의 $x\in G$에 대하여 $\delta_x:G\rightarrow \mathbb{C}$를
...
실제로 위에서 논의한 대부분의 것들이 이 categorical equivalence로 설명될 수 있다. 가령 임의의 $G$-representation $V$에 대하여, $V$의 subrepresentation은 $V$의 $G$-submodule (정확히는 $\mathbb{C}[G]$-submodule)이다. 또, [정의 3](#def3){: data-lid="3w66x" }의 tensor product도 납득할만한데, 일반적으로 coproduct $\Delta:A\rightarrow A\otimes A$가 주어진 $\mathbb{K}$-algebra $A$와 두 $A$-module $M,N$에 대하여 이들의 tensor product를 정의하기 위해서는 $\Delta$를 활용하여 
...
이기 때문이다. 그럼 마찬가지로 [정의 3](#def3){: data-lid="lkxfx" }에서 정의한 $\Hom$이 이 $\otimes$와 adjunction 관계에 있고, 이러한 이유로 다소 인위적으로 보이는 [정의 3](#def3){: data-lid="2oqok" }의 $G$-action들이 등장하는 것이다. 
...
을 살펴보았다. 그럼 [따름정리 7](#cor7){: data-lid="jp86p" }이 주장하는 것은 임의의 유한차원 $G$-representation $V$는 항상 *semisimple* $\mathbb{C}[G]$-module이라는 것이다 ([\[환론\] §반단순가군, ⁋정의 2](/ko/math/ring_theory/semisimple_modules#def2){: data-lid="5meml" }). 따라서 $\mathbb{C}[G]$는 그 자체를 regular representation으로 보면 [따름정리 7](#cor7){: data-lid="vrt42" }에 의해 semisimple module이므로 Artinian semisimple ring이 되며, [\[환론\] §아틴-웨더번 정리, ⁋정리 11](/ko/math/ring_theory/artin_wedderburn#thm11){: data-lid="fblol" }에 의하여 simple algebra들의 곱으로의 decomposition
...
:::
::: 증명
1. $G$-map은 $\mathbb{C}[G]$-module homomorphism이고 irreducible $G$-module은 simple $\mathbb{C}[G]$-module이므로, 이는 [\[환론\] §나눗셈환, ⁋보조정리 10](/ko/math/ring_theory/division_rings#lem10){: data-lid="w3u04" }의 특수한 경우이다. 
...
이 정의는 일반론과 일치한다. $d_W$의 image는 $W$와 isomorphic한 $V$의 simple submodule들이 생성하는 부분공간이므로 정확히 [\[환론\] §반단순가군, ⁋정의 8](/ko/math/ring_theory/semisimple_modules#def8){: data-lid="fgneo" }의 $W$-isotypic component이고, [같은 글, ⁋명제 9](/ko/math/ring_theory/semisimple_modules#prop9){: data-lid="cbnum" }에 의해 이 분해는 irreducible decomposition의 선택과 무관하게 canonical하다.

Representation의 유일성 또한 이제 [\[환론\] §반단순가군, ⁋명제 10](/ko/math/ring_theory/semisimple_modules#prop10){: data-lid="9hfdt" }로부터 얻어진다. 즉, 우리는 임의의 representation $V$가 주어졌을 때 이를 다음의 decomposition
