...
date: 2026-02-13
weight: 1
translated_at: 2026-07-13T14:30:02+00:00
translation_source: kimi-cli
---
One common strategy when dealing with mathematical objects whose structure is complicated is to observe how they act on simpler objects. In this category, we study representation theory, and among them, the representation theory of finite groups.
...
In general, the ground field $\mathbb{K}$ can be replaced by an arbitrary ring $A$ without any problem, but for our discussion it suffices to fix $\mathbb{K}=\mathbb{C}$. Also, let us keep in mind that we are mainly considering the case where $V$ is a *finite-dimensional* vector space as the representation space. This too can be generalized to infinite-dimensional vector spaces, but for this one needs standard methods such as endowing $V$ with a topological vector space structure.

The above definition can be thought of briefly as being given $G\rightarrow\Aut(V)$. Through a slight abuse of notation, we sometimes write $\rho(g,-): V\rightarrow V$ simply as $\rho(g)\in \Aut(V)$, and further abbreviate the notation by writing $g\cdot v$ instead of $\rho(g)v$. As this notation suggests, we think of $V$ as a $G$-module, and from this point of view (with scalar multiplication implicitly given) we also simply call $V$ a representation of $G$.
...
which, expressed as an equation, can simply be written as
...
3. If $V$ is not the zero representation and the only subrepresentations of $V$ are the trivial subrepresentations, namely itself and $G\times\{0\}\rightarrow\{0\}$, then $V$ is called an *irreducible representation*.
:::

From the same perspective, for arbitrary representations $V,W$ we can define $V\oplus W$, $V\otimes W$, etc., using the operations on their underlying vector spaces. One thing to be somewhat careful about in the next definition is that, unlike in [Definition 2](#def2) above, for $V\otimes W$ etc. a *natural* $G$-action may not exist, so we explicitly define a $G$-action on each vector space.
...
In [Definition 3] above, the definitions of tensor product and $\Hom$ may look somewhat artificial, but to explore this the language of group algebras is useful. ([\[Algebraic Structures\] §Algebras, ⁋Definition 5](/en/math/algebraic_structures/algebras#def5)) To briefly review, as a set $\mathbb{C}[G]$ was the collection of functions from $G$ to $\mathbb{C}$. For each $x\in G$, defining $\delta_x:G\rightarrow \mathbb{C}$ by
...
so the $\delta_x$ form a basis of $\mathbb{C}[G]$. The part where the $\mathbb{C}[G]$ we deal with differs from simply being the ring of functions from $G$ to $\mathbb{C}$ is the multiplication defined on it. Instead of defining the product of two functions $\phi,\psi$ by

$$(\phi\psi)(x)=\phi(x)\psi(x), \qquad \text{for all $x\in G$}$$

multiplication on this space is defined by the convolution product

$$(\phi\ast \psi)(x)=\sum_{y\in G}\phi(y)\psi(y^{-1}x)$$

If we identify the delta function $\delta_x$ with $x\in G$ as above, then the following identity
...
holds, so choosing this multiplication is natural. For example, the product of $\delta_x$ and $\delta_y$ is $0$, but their convolution is $\delta_{xy}$, so in order for expressions such as the following, which are included in the group action,
...
That is, what we called a $G$-module can also be thought of, strictly speaking, as having only looked at the action when $G\hookrightarrow \mathbb{C}[G]$ is given from a $\mathbb{C}[G]$-module structure.
...
Similarly, the $\Hom$ defined in [Definition 3](#def3) is in an adjunction relationship with this $\otimes$, and it is for this reason that the $G$-actions in [Definition 3](#def3), which look somewhat artificial, appear.

In particular, thinking that subrepresentations of $G$ and $\mathbb{C}[G]$-submodules are the same thing, $V$ being an irreducible representation is the same as $V$ being a *simple* $\mathbb{C}[G]$-module.

## Maschke's Theorem

We have now roughly surveyed the basic notions needed for representations of finite groups. Before starting the main story, for any representation $V$ let us consider the following subspace

$$V^G=\{v\in V\mid g\cdot v=v\text{ for all $g\in G$}\}$$

This space is the space of vectors fixed by the $G$-action, and while it is also a $G$-invariant space, what we want to observe more than that is that the obvious projection map

$$p: V\rightarrow V^G;\qquad v\mapsto \frac{1}{\lvert G\rvert}\sum_{g\in G}g\cdot v$$

exists. In particular, the idea contained in this projection map, namely the idea of averaging over all the actions of $G$ to obtain a $G$-invariant object, is used importantly.
...
If such a $G$-invariant inner product is given, then for any $g\in G$, $\rho(g)\in \Aut(V)$ is a unitary operator. To see this, suppose a $G$-invariant inner product $\langle -,-\rangle$ is given, and for any $g\in G$,
...
::: Proposition 6
Any representation $V$ admits a $G$-invariant inner product. That is, any representation is a unitary representation.
:::
::: Proof
For any Hermitian inner product $\langle -,- \rangle$ on $V$, we define a new inner product $\langle\kern-1.5pt\langle-,-\rangle\kern-1.5pt\rangle$ by
...
::: Corollary 7 (Maschke)
For any finite-dimensional $G$-representation $V$ and any $G$-invariant subspace $W$, there exists a suitable $G$-invariant subspace $W'$ such that $V = W \oplus W'$. Therefore, inductively, any finite-dimensional $G$-representation decomposes into a direct sum of irreducible representations.
:::
::: Proof
...
::: Proposition 9
In the situation above, the function
...
Although written in a complicated way, the above $d$ counts how many of each irreducible $G$-module $W$ (or its isomorphism class) appear inside $V$, so the following definition is natural.
...
Uniqueness of the representation also now follows from [\[Ring Theory\] §Semisimple module, ⁋Proposition 9](/en/math/ring_theory/semisimple_modules#prop9). That is, we know that when any representation $V$ is given, it can be written in the form of the decomposition

$$V=V_1^{\oplus r_1}\oplus\cdots\oplus V_k^{\oplus r_k}$$

where the isomorphism classes of the factors and their multiplicities are unique.
