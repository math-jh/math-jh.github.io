...
In [Definition 3](#def3){: data-lid="qebu2" } above, the definitions of the tensor product and $\Hom$ may look somewhat artificial; to understand them, the language of group algebras is useful. ([\[Algebraic Structures\] §Algebras, ⁋Definition 5](/en/math/algebraic_structures/algebras#def5){: data-lid="8vat6" }) To briefly review, as a set $\mathbb{C}[G]$ was the collection of functions from $G$ to $\mathbb{C}$. For each $x\in G$, defining $\delta_x:G\rightarrow \mathbb{C}$ by
...
Indeed, most of what we discussed above can be explained by this categorical equivalence. For example, for any $G$-representation $V$, a subrepresentation of $V$ is a $G$-submodule of $V$ (more precisely, a $\mathbb{C}[G]$-submodule). Also, the tensor product of [Definition 3](#def3){: data-lid="3w66x" } is reasonable: in general, for a $\mathbb{K}$-algebra $A$ equipped with a coproduct $\Delta:A\rightarrow A\otimes A$ and two $A$-modules $M,N$, to define their tensor product one must use $\Delta$ via
...
Similarly, the $\Hom$ defined in [Definition 3](#def3){: data-lid="lkxfx" } is adjoint to this $\otimes$, and it is for this reason that the $G$-actions in [Definition 3](#def3){: data-lid="2oqok" }, which look somewhat artificial, arise.
...
:::
::: Proof
1. A $G$-map is a $\mathbb{C}[G]$-module homomorphism, and an irreducible $G$-module is a simple $\mathbb{C}[G]$-module, so this is a special case of [\[Ring Theory\] §Division ring, ⁋Lemma 10](/en/math/ring_theory/division_rings#lem10){: data-lid="w3u04" }.
