...
<ins id="rmk7">**참고 7**</ins> sectional curvature의 언어로 Gauss 방정식을 다시 읽으면 더 기하학적인 형태를 얻는다. $$M$$의 한 점에서 정규직교 vector $$X, Y \in T_p M$$이 펼치는 평면의 sectional curvature $$K(X, Y)$$와 같은 평면을 $$\bar M$$ 안에서 본 $$\bar K(X, Y)$$ 사이에
