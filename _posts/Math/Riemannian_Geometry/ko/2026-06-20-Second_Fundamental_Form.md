...
[§리만 계량, §§Normal bundle](/ko/math/riemannian_geometry/Riemannian_metric#normal-bundle){: data-lid="25fqe" data-relation="required" }에서 우리는 Riemannian manifold $(\bar M, \bar g)$의 submanifold $M$ 위에 restriction metric $g = \iota^\ast \bar g$를 주고, 각 점에서 tangent space $T_p M$의 orthogonal complement로 normal bundle $NM$을 정의했다. 이 글에서는 이러한 embedding이 $M$에 부여하는 *외재적<sub>extrinsic</sub>* 기하를 다룬다. 핵심 도구는 주변 공간 $\bar M$의 Levi-Civita connection $\bar\nabla$를 접성분과 법성분으로 쪼개는 일이며, 그 법성분이 곧 *제2기본형식*이다. 이를 통해 우리는 $M$의 내재적 곡률과 $\bar M$의 곡률을 잇는 Gauss 방정식을 얻고, [§측지선, ⁋예시 9](/ko/math/riemannian_geometry/geodesics#ex9){: data-lid="geyue" data-relation="weak" }에서 구면의 대원이 측지선임을 보일 때 사용한 "주변 가속도를 tangent space로 orthogonal projection"하는 논증의 일반적 배경을 마련한다.
...
::: 명제 3
[정의 2](#def2){: data-lid="kvrl7" data-relation="required" }의 제2기본형식 $\mathrm{II}$는 다음 성질을 갖는다.
...
[명제 3](#prop3){: data-lid="ttw3q" data-relation="required" }에서 $\mathrm{II}$가 텐서임이 중요하다. 정의에는 $X, Y$를 $\bar M$ 위로 확장하는 절차가 들어 있지만, 결과는 extension과 무관하게 $T_p M$의 두 벡터 $X_p, Y_p$에만 의존한다. 이로써 우리는 $\mathrm{II}_p : T_p M \times T_p M \rightarrow N_p M$을 각 점에서의 대칭 쌍선형사상으로 다룰 수 있다.
...
이 성립한다. 특히 좌변이 extension $N$에 무관하므로 [정의 4](#def4){: data-lid="dxung" data-relation="required" }의 $S_\nu$는 well-defined이며, $S_\nu$는 $T_p M$ 위의 self-adjoint 선형사상이다.
:::
...
제2기본형식의 가장 중요한 귀결은 $M$의 내재적 곡률과 $\bar M$의 곡률 사이의 관계이다. [§리만 곡률, ⁋정의 2](/ko/math/riemannian_geometry/curvature#def2){: data-lid="a8hnz" data-relation="required" }에서 곡률 텐서를 $R(X, Y) Z = \nabla_X \nabla_Y Z - \nabla_Y \nabla_X Z - \nabla_{[X, Y]} Z$로 정의했음을 상기하자. $\bar M$의 곡률 텐서 $\bar R$도 같은 식으로 $\bar\nabla$으로부터 정의된다. 이 둘의 차이가 정확히 제2기본형식의 이차식으로 통제된다.
...
::: 증명
계산의 편의를 위해 점 $p$의 한 근방에서 $[X, Y] = 0$이 되도록 $X, Y$를 잡아도 일반성을 잃지 않는다. 실제로 곡률 텐서가 [§리만 곡률, ⁋명제 3](/ko/math/riemannian_geometry/curvature#prop3){: data-lid="0m1sb" data-relation="required" }에 의해 텐서이고 $\mathrm{II}$도 텐서이므로, 등식의 양변은 $X_p, Y_p, Z_p, W_p$에만 의존하며, 주어진 벡터들을 좌표 벡터장으로 확장해 $[X, Y] = 0$이 되도록 할 수 있기 때문이다. 이때 $R(X, Y)Z = \nabla_X \nabla_Y Z - \nabla_Y \nabla_X Z$이고 $\bar R(X, Y)Z = \bar\nabla_X \bar\nabla_Y Z - \bar\nabla_Y \bar\nabla_X Z$이다.

Gauss 공식 $\bar\nabla_Y Z = \nabla_Y Z + \mathrm{II}(Y, Z)$을 한 번 적용하고, 여기에 다시 $\bar\nabla_X$을 적용하자. $\nabla_Y Z$은 $M$에 접하는 벡터장이므로 Gauss 공식을 다시 적용할 수 있고, $\mathrm{II}(Y, Z)$은 법벡터장이므로 그 미분에는 Weingarten 사상이 등장한다. $\mathrm{II}(Y, Z)$을 법선 방향으로 보면 [정의 4](#def4){: data-lid="htdoy" data-relation="required" }에 의해 $\bar\nabla_X \mathrm{II}(Y, Z) = -S_{\mathrm{II}(Y, Z)}(X) + (\bar\nabla_X \mathrm{II}(Y, Z))^\perp$이다. 이를 모아 쓰면
...
만 남는다. Weingarten 방정식 [명제 5](#prop5){: data-lid="7mati" data-relation="required" }로 마지막 항을 $\langle S_{\mathrm{II}(Y, Z)}(X), W\rangle = \langle \mathrm{II}(X, W), \mathrm{II}(Y, Z)\rangle$로 바꾸면
...
::: 참고 7
[§비교 정리, ⁋정의 1](/ko/math/riemannian_geometry/comparison_theorems#def1){: data-lid="663e5" data-relation="weak" }에서 정의할 sectional curvature의 언어로 Gauss 방정식을 다시 읽으면 더 기하학적인 형태를 얻는다. $M$의 한 점에서 정규직교 벡터 $X, Y \in T_p M$이 펼치는 평면의 sectional curvature $K(X, Y)$와 같은 평면을 $\bar M$ 안에서 본 $\bar K(X, Y)$ 사이에

$$K(X, Y) = \bar K(X, Y) + \langle \mathrm{II}(X, X), \mathrm{II}(Y, Y)\rangle - \lvert \mathrm{II}(X, Y)\rvert^2$$

이 성립한다. 이는 [정리 6](#thm6){: data-lid="xp449" data-relation="required" }에 $Z = Y, W = X$를 대입하고 $\langle R(X, Y) Y, X\rangle = K(X, Y)$ ($X, Y$가 정규직교일 때)를 쓴 것이다.
:::
...
제2기본형식은 [§측지선, ⁋예시 9](/ko/math/riemannian_geometry/geodesics#ex9){: data-lid="5qw8j" data-relation="weak" }에서 구면의 대원을 다룰 때 암묵적으로 사용한 "주변 가속도의 접성분"이라는 개념을 정확히 메운다. $M$ 위의 곡선 $\gamma$의 주변 가속도 $\bar D_t \dot\gamma$와 내재적 가속도 $D_t \dot\gamma$ 사이의 간격이 곧 제2기본형식이다.
...
::: 증명
[명제 1](#prop1){: data-lid="pqt4w" data-relation="required" }과 [정의 2](#def2){: data-lid="4cr2l" data-relation="required" }는 벡터장에 대한 Gauss 공식 $\bar\nabla_X Y = \nabla_X Y + \mathrm{II}(X, Y)$를 준다. 이 공식은 곡선 $\gamma$를 따른 covariant derivative로 그대로 옮겨진다. 즉 곡선 $\gamma$ 위의 접 벡터장 $V$에 대해 $\bar D_t V = D_t V + \mathrm{II}(\dot\gamma, V)$가 성립하는데, 이는 양변이 $\bar M$의 connection과 $M$의 connection을 곡선을 따라 표현한 것이며 좌표 계산으로 $\mathrm{II}$가 $\dot\gamma$와 $V$의 점별 값에만 의존한다는 [명제 3](#prop3){: data-lid="s59xt" data-relation="required" }을 적용해 얻어진다. 특히 $V = \dot\gamma$로 두면

$$\bar D_t \dot\gamma = D_t \dot\gamma + \mathrm{II}(\dot\gamma, \dot\gamma)$$

이며, 우변에서 $D_t \dot\gamma$는 $M$에 접하고 $\mathrm{II}(\dot\gamma, \dot\gamma)$는 법선이므로 이것이 정확히 $\bar D_t \dot\gamma$의 접-법 분해이다.

$\gamma$가 $M$의 측지선인 것은 [§측지선, ⁋정의 4](/ko/math/riemannian_geometry/geodesics#def4){: data-lid="8czwo" data-relation="required" }에 의해 $D_t \dot\gamma = 0$인 것, 즉 위 분해의 접성분이 소멸하는 것과 동치이다. 접성분이 소멸하는 것은 $\bar D_t \dot\gamma = \mathrm{II}(\dot\gamma, \dot\gamma)$가 법선인 것과 동치이므로, $\gamma$가 측지선인 것과 주변 가속도가 매 점에서 법선인 것이 동치이다.
:::

[명제 9](#prop9){: data-lid="hw73e" data-relation="required" }는 측지선의 외재적 특징을 분명히 한다. $M$의 측지선이란 "$M$ 안에서는 가속도가 없으나 $\bar M$ 안에서는 $M$을 벗어나지 않으려 법선 방향으로만 휘는" 곡선이다. $\bar M = \mathbb{R}^{n+1}$인 경우 주변 가속도 $\bar D_t \dot\gamma$는 보통의 이계 도함수 $\ddot\gamma$이므로, $\gamma$가 측지선인 것은 $\ddot\gamma$가 매 점에서 $M$에 법선인 것과 같다. 이것이 [§측지선, ⁋예시 9](/ko/math/riemannian_geometry/geodesics#ex9){: data-lid="mhy15" data-relation="weak" }에서 사용한 판정법이다.
...
임을 보인다. 점 $p$를 지나는 접 벡터 $X, Y \in T_p S^n$을 잡고, $\nu(q) = q$를 $\mathbb{R}^{n+1}$ 전체에서의 법벡터장으로 확장하자 (단위 구면 위에서 $\lvert q\rvert = 1$이므로 이 extension은 $S^n$ 위에서 단위 법벡터장이다). 평탄한 $\mathbb{R}^{n+1}$의 Levi-Civita connection은 [§레비-치비타 접속, ⁋예시 7](/ko/math/riemannian_geometry/Levi-Civita_connection#ex7){: data-lid="oq3yg" data-relation="required" }에 의해 성분별 미분이므로, $\nu(q) = q = (q^1, \ldots, q^{n+1})$의 $X$ 방향 미분은
...
이다. 이제 Weingarten 방정식 [명제 5](#prop5){: data-lid="utt0c" data-relation="required" }에 의해 임의의 $Y \in T_p S^n$에 대해
...
이로부터 곡선의 주변 가속도가 [명제 9](#prop9){: data-lid="l1zol" data-relation="required" }를 통해 곧장 회복된다. 단위 속력 곡선 $\gamma$에 대해 $\mathrm{II}(\dot\gamma, \dot\gamma) = -\langle \dot\gamma, \dot\gamma\rangle \nu = -\nu = -\gamma$이므로, $\gamma$가 측지선이면 $\bar D_t \dot\gamma = \ddot\gamma = \mathrm{II}(\dot\gamma, \dot\gamma) = -\gamma$, 즉 [§측지선, ⁋예시 9](/ko/math/riemannian_geometry/geodesics#ex9){: data-lid="czyre" data-relation="weak" }에서 대원에 대해 직접 계산한 $\ddot\gamma = -\gamma$를 다시 얻는다. 또한 평균곡률은 $T_p S^n$의 orthonormal basis $e_1, \ldots, e_n$에 대해 $H = \sum_{i=1}^n \mathrm{II}(e_i, e_i) = -\sum_{i=1}^n \langle e_i, e_i\rangle \nu = -n \nu$이므로 $S^n$은 minimal submanifold가 아니다.
:::

마지막으로 Gauss 방정식 [정리 6](#thm6){: data-lid="yv29l" data-relation="required" }을 구면에 적용하면 그 곡률이 회복된다. $\bar M = \mathbb{R}^{n+1}$이 평탄하여 $\bar R = 0$이므로, 정규직교 $X, Y \in T_p S^n$에 대해 [참고 7](#rmk7){: data-lid="426oy" data-relation="required" }의 식은
