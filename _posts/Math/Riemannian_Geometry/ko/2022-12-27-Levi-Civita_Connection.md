...
위 호환성 조건은 $g$를 $(0, 2)$-tensor field로 보고 [§접속, §§Tensor field 위에서의 공변미분](/ko/math/riemannian_geometry/connection#tensor-field-위에서의-공변미분){: data-lid="anwec" }을 따라 그 covariant derivative $\nabla g$를 계산했을 때 얻는 깔끔한 형태로 다시 표현된다.

::: 명제 2
(Pseudo-)Riemannian manifold $(M, g)$ 위의 connection $\nabla$에 대해, 다음의 세 조건이 동치이다.

1. $\nabla$는 $g$와 compatible이다. 즉 [정의 1](#def1){: data-lid="bfu93" }의 식이 성립한다.
...
Koszul 공식은 local coordinate에서 connection coefficient를 metric으로부터 직접 계산할 수 있게 해 준다. Local coordinate $(x^i)$에서 $E_i = \partial/\partial x^i$를 잡으면 $[E_i, E_j] = 0$이므로 [명제 5](#prop5){: data-lid="s47aq" }에 $X = E_i, Y = E_j, Z = E_k$를 대입하면
...
Christoffel 기호는 index $i, j$에 대해 대칭이며 ($\Gamma_{ij}^k = \Gamma_{ji}^k$), 이는 [정의 3](#def3){: data-lid="nyx72" } 직후의 관찰과 일치하는 결과이다.
...
::: 명제 9
[정의 8](#def8){: data-lid="xrz0x" }의 parallel transport는 well-defined이다. 즉 임의의 초기값 $v_0 \in T_{\gamma(t_0)} M$에 대해 [정의 8](#def8){: data-lid="rxqrl" }의 식과 초기값 조건을 동시에 만족하는 parallel 벡터장 $V$가 유일하게 존재한다.
:::
...
::: 증명
$V, W$를 $\gamma$ 위의 parallel 벡터장으로 잡고 $f(t) := \langle V(t), W(t)\rangle$을 보면 [명제 2](#prop2){: data-lid="697d8" }의 (3)에서 $f$는 상수이다.
:::

Parallel transport는 임의의 곡선 $\gamma$에 대해 정의되지만, 일반적으로 두 끝점 $p = \gamma(t_0), q = \gamma(t_1)$ 사이에 여러 곡선이 존재하는 경우 곡선마다 다른 isomorphism $P_\gamma^{t_0, t_1}$이 얻어진다. 이 *path-dependence*가 어떻게 측정되는지를 다루는 자료가 [§리만 곡률](/ko/math/riemannian_geometry/curvature){: data-lid="u2zl0" }에서 정의되는 *리만 곡률 텐서*이다.
