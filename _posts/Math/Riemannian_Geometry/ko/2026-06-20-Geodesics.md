...
::: 참고 10
[정리 7](#thm7)은 각 $(p, v) \in TM$마다 그 점에서 출발하는 유일한 측지선 $\gamma_v$를 대응시킨다. $v$를 적절히 줄여 측지선이 $t = 1$까지 정의되도록 하면, 대응 $v \mapsto \gamma_v(1)$은 $T_p M$의 한 근방에서 $M$으로의 매끄러운 함수를 정의하는데, 이를 점 $p$에서의 *exponential map*이라 부른다. Exponential map은 측지선의 자료를 한 점의 접공간 위로 모아 manifold의 국소 구조를 선형화하는 도구이다.
:::
