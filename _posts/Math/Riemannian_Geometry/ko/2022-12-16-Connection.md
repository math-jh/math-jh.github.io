...
어쨌든 $TM$ 위에서 정의된 connection을 생각하면, [명제 2](#prop2){: data-lid="hcd2y" }에 의하여 점 $p$에서의 $\nabla_XY$의 값은 $p$ 근방에서의 local frame $(E_i)$들에 의해 완전히 결정된다. 이는
...
우리는 tangent bundle $TM$ 위에 정의된 connection $\nabla$가 임의의 $(r,s)$-tensor field $\mathcal{T}^{r,s}(M)$로 잘 확장된다는 것을 보인다. ([명제 5](#prop5){: data-lid="i5g02" }) 이를 위해서는 우선 $\nabla$가 cotangent bundle $T^\ast M$ 위에서는 어떻게 확장되는지를 정해줘야 한다.
...
으로 정의된다. 한편, $M=\mathbb{R}^m$으로 두고 [§리만 계량, §§Musical isomorphism](/ko/math/riemannian_geometry/Riemannian_metric#musical-isomorphism){: data-lid="khi28" }에서와 같이 함수 $f$의 그라디언트 벡터 $\operatorname{grad} f$를 다음의 식
