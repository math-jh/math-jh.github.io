...
---
In [§Overview of Mirror Symmetry](/en/math/mirror_symmetry/overview){: data-lid="d668d" } we saw that the mirror symmetry of a toric Fano variety $X_\Sigma$ is summarized by an isomorphism
...
Meanwhile, in the examples of [§Overview of Mirror Symmetry](/en/math/mirror_symmetry/overview){: data-lid="pfswh" } we introduced the Landau–Ginzburg model, which consists of a holomorphic function $W$ on a given manifold $\check{X}$; the Jacobi ring containing the critical points of $W$ carried the information of the B-model. Locally this is written as follows.
...
In other words, intuitively this can be viewed as localizing the global integral $\int_X$ of [Example 2](#ex2){: data-lid="z4ug9" } to integrals over the (finitely many) points of the critical scheme, and one can show that $(\Jac(f), \eta)$ is indeed a Frobenius algebra in a manner similar to that example.
...
As a special example, consider the Hori–Vafa superpotential of $\mathbb{P}^1$ seen in [§Overview of Mirror Symmetry, ⁋Example 5](/en/math/mirror_symmetry/overview#ex5){: data-lid="qz850" }:
...
In the same way, consider the Hori–Vafa superpotential of $\mathbb{P}^2$ seen in [§Overview of Mirror Symmetry, ⁋Example 6](/en/math/mirror_symmetry/overview#ex6){: data-lid="3jl2x" }:
...
Similarly, in our intuition $\eta$ is the Poincaré pairing ([Example 2](#ex2){: data-lid="341lo" }), which survives only in the top degree, so this degree condition translates to
...
The last condition in [Definition 5](#def5){: data-lid="zvo4a" } is called potentiality because it allows the trilinear form $c$ to be expressed as the third partial derivatives of a suitable scalar function $F:M \rightarrow \mathbb{C}$.
...
::: Proof
For flat coordinates $t^\alpha$ the covariant derivative $\nabla_{\partial_{t^\delta}}$ coincides with the ordinary partial derivative $\partial_{t^\delta}$, so the potentiality condition in [Definition 5](#def5){: data-lid="k6ngz" } means that
...
The potential $F$ of [Proposition 6](#prop6){: data-lid="5flln" } satisfies the following equation for all $\alpha, \beta, \gamma, \delta$:
...
Substituting the result $c_{\alpha\beta\gamma} = \partial_{t^\alpha} \partial_{t^\beta} \partial_{t^\gamma} F$ of [Proposition 6](#prop6){: data-lid="fku6d" } gives
...
is a Frobenius manifold ([Definition 5](#def5){: data-lid="j7emg" }).
:::

::: Proof
We verify the six conditions of [Definition 5](#def5){: data-lid="j1m5q" } in order.
...
5. Similarly one checks that $\nabla c$ is symmetric in the four indices; under this potentiality, associativity is equivalent to the WDVV equation of [Proposition 7](#prop7){: data-lid="6j8s9" }, and on the A-model side it is guaranteed by the splitting axiom for GW invariants.
...
([\[Symplectic Geometry\] §Quantum Cohomology, ⁋Definition 12](/en/math/symplectic_geometry/quantum_cohomology#def12)) Here the first term is the contribution from the classical cup product, and the second term can be thought of as the contribution $\langle H, H, H\rangle_{0,3,1} = 1$ from the degree-$1$ rational curve of [§Overview of Mirror Symmetry, ⁋Example 5](/en/math/mirror_symmetry/overview#ex5){: data-lid="xphkx" }, accumulated exponentially along the $H^2$ direction coordinate $t^1$ (via the Euler vector field). The metric was already computed in [Example 4](#ex4){: data-lid="p30hw" }, and computing the third partial derivatives of $F$ from the above formula gives
...
Now setting the Novikov variable to $q = e^{t^1}$, the second equation becomes $\partial_{t^1} \circ \partial_{t^1} = qe$, which translates back into cohomology language as $H \star H = q \cdot 1$, recovering the small quantum ring of [§Overview of Mirror Symmetry, ⁋Example 5](/en/math/mirror_symmetry/overview#ex5){: data-lid="6xwdd" }. Moreover, this isomorphism is now parametrized by the variation of $q=e^{t^1}$, upgrading the previous mirror symmetry at the level of ring isomorphisms.
:::
