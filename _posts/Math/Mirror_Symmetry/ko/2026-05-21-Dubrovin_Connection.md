...
특히 $$M = H^\ast(X, \mathbb{C})$$인 경우를 자세히 들여다보자. 이 때 base는 $$M \times \mathbb{C}^\ast$$이고, 이 manifold의 점을 $$(t,z)\in M\times \mathbb{C}^\ast$$으로 쓰자. 그럼 한 점 $$(t,z)$$ 위의 fiber는 $$(\pr_1^\ast TM)_{(t,z)}\cong T_tM$$이고, $$M$$은 원래부터 벡터공간이었으므로 이 fiber는 $$H^\ast(X, \mathbb{C})$$와 canonical하게 isomorphic하다. 

이 bundle 위에서 Dubrovin connection은 [정의 1](#def1)대로 $$\nabla^z_{\partial_\alpha} = \partial_\alpha + z^{-1}\mathcal{C}_\alpha$$이며, $$\mathcal{C}_\alpha = \partial_\alpha \star_q -$$는 quantum product로 곱하는 endomorphism이다. small quantum cohomology에서 의미를 갖는 $$H^2(X)$$ 방향만 보면, $$H^2(X, \mathbb{C})$$의 basis $$\{T_a\}$$와 대응하는 Novikov 좌표 $$q_a = e^{t^a}$$ ($$a = 1, \ldots, r = \dim H^2(X)$$)에 대해 그 connection 계수는 $$\mathcal{C}_a = T_a \star_q$$, 곧 divisor $$T_a$$를 quantum 곱하는 연산이 된다. 여기서 $$T_a$$가 *미분 방향* $$q_a\partial_{q_a}$$과 *곱하는 class* $$T_a \star_q$$ 양쪽에 동시에 나타나는 것은 우연이 아니라, $$H^2$$ 방향의 connection 계수가 divisor $$T_a$$의 quantum 곱으로 흡수된다는 divisor equation의 귀결이다. ([\[사교기하학\] §Gromov-Witten 불변량, ⁋명제 4](/ko/math/symplectic_geometry/gromov_witten#prop4))
