...
이 섹션에서 우리는 symmetric group을 정의한다. 여기에서 소개하는 결과들은 별다른 증명없이 진행하지만, 원한다면 그 증명은 [\[군론\] §대칭군](/ko/math/group_theory/symmetric_groups){: data-lid="6sup7" }에서 찾아볼 수 있다. 
...
::: 보조정리 2
[§행렬식, ⁋정의 4](/ko/math/linear_algebra/determinant#def4){: data-lid="z5mif" }을 만족하는 함수 $D$는 유일하게 존재한다.
:::
::: 증명
...
이 성립한다. [§행렬식, ⁋명제 3](/ko/math/linear_algebra/determinant#prop3){: data-lid="zj3f0" }에 의하여, $i_1,\ldots, i_n$들 중 같은 것이 존재한다면 $f(e_{i_1},\ldots,e_{i_n})$의 값은 항상 0이 되므로, 우변의 식은 
...
[§행렬식, ⁋명제 3](/ko/math/linear_algebra/determinant#prop3){: data-lid="1kgu4" }에 의하여 $i_1,\ldots, i_n$들 중 같은 것이 존재한다면 $\det(A_{i_1},\ldots, A_{i_n})$의 값은 항상 0이 되므로, 위 식의 우변에서는 $i_1,\ldots, i_n$이 서로 다른 항들만 남는다. 이제 $\upsilon\in S_n$을 다음의 식
...
우리는 이전 글에서 행렬 $A$가 가역인 것과 $\det A\neq 0$이 동치라는 것을 기하학적으로 설명했다. 방금 전의 [보조정리 4](#lem4){: data-lid="mh6o9" }를 이용하면 이를 엄밀하게 증명할 수 있다.

::: 명제 5
임의의 행렬 $A\in\Mat_n(\mathbb{K})$에 대하여, $\det A\neq 0$인 것과 $A$가 가역인 것이 동치이다.
:::
::: 증명
[§선형대수학의 기본정리](/ko/math/linear_algebra/ftla){: data-lid="ga49b" }로부터, $A$가 가역인 것은 $A$에 의해 정의되는 linear map $L_A:\mathbb{K}^n\rightarrow \mathbb{K}^n$이 가역인 것과 동치라는 것을 안다. $\mathbb{K}^n$은 유한차원이므로 이는 다시 $L_A$가 surjective인 것과 동치이고, 이는 $\col(A)$의 basis인 열벡터들 $A_1,\ldots, A_n$들이 일차독립인 것과 동치이다. 만일 $A_1,\ldots, A_n$들이 일차독립이 아니라면 [§행렬식, ⁋명제 3](/ko/math/linear_algebra/determinant#prop3){: data-lid="zcri0" }에 의하여 $\det A=0$이 된다. 즉, $\det A\neq 0$이라면 $A$는 가역이다.
...
특별히 모든 $n\times n$ 행사다리꼴행렬은 모두 상삼각행렬이다. ([§가우스 소거법, ⁋정의 4](/ko/math/linear_algebra/Gaussian_elimination#def4){: data-lid="bogif" })
...
[§가우스 소거법](/ko/math/linear_algebra/Gaussian_elimination){: data-lid="kek14" }에 의해 임의의 행렬은 기본행연산을 반복하여 행사다리꼴행렬로 바꿀 수 있다. 임의의 행사다리꼴행렬은 모두 상삼각행렬이므로, 위의 명제를 통하면 이렇게 얻어진 행사다리꼴행렬의 행렬식을 매우 쉽게 구할 수 있다. 한편 기본행연산을 적용하는 것은 기본행렬들을 곱하는 것과 같다. 따라서, 만일 행렬 $A$로부터 기본행연산 $E_1,\ldots, E_k$를 반복하여 행사다리꼴행렬 $A'$를 얻었다면, 
...
:::
::: 증명
[명제 8](#prop8){: data-lid="dbzx9" }의 증명과 거의 동일하다. 식 (2)을 통해 주어진 행렬의 행렬식을 계산하면
...
과 [명제 9](#prop9){: data-lid="l13qf" }, 그리고 [보조정리 4](#lem4){: data-lid="ausxq" }에 의해 자명하다. 가운데 행렬의 경우 대각성분이 모두 1인 하삼각행렬이므로 [명제 8](#prop8){: data-lid="283r5" }에 의해 그 행렬식은 1이다. 마지막 행렬의 경우, $l$번의 행 바꿈 후 $l$번의 열 바꿈을 하여 $2l$번의 부호 변경이 생겨 주어진 행렬의 행렬식이 $\det B$와 같게 된다.
:::
...
이다. 이제 [따름정리 10](#cor10){: data-lid="zo1sz" }에 의하여 이 행렬의 행렬식은 $A_{ij}\det A^{(i,j)}$와 같고, 따라서

$$\det B_j=(-1)^{i+j-2}\det B_j'=(-1)^{i+j-2}A_{ij}\det A^{(i,j)}=(-1)^{i+j}A_{ij}\det A^{(i,j)}$$

이다. 한편, [따름정리 3](#cor3){: data-lid="k303y" }에 의해 행렬식은 행에 대해서도 multilinear하고, $i$번째 행에 대한 multilinearity를 사용하면 $B_j$들의 행렬식의 합은 $A$의 행렬식과 같으므로 원하는 식
