...
이 섹션에서 우리는 symmetric group을 정의한다. 여기에서 소개하는 결과들은 별다른 증명없이 진행하지만, 원한다면 그 증명은 [\[군론\] §대칭군](/ko/math/group_theory/symmetric_groups){: data-lid="6sup7" data-relation="weak" }에서 찾아볼 수 있다. 
...
으로 정의할 수 있다. 이 함수는 group homomorphism이 된다. 즉, 임의의 $\tau,\tau'\in S_n$에 대하여 
...
::: 보조정리 2
[§행렬식, ⁋정의 4](/ko/math/linear_algebra/determinant#def4){: data-lid="z5mif" data-relation="required" }을 만족하는 함수 $D$는 유일하게 존재한다.
:::
::: 증명
...
이 성립한다. [§행렬식, ⁋명제 3](/ko/math/linear_algebra/determinant#prop3){: data-lid="zj3f0" data-relation="required" }에 의하여, $i_1,\ldots, i_n$들 중 같은 것이 존재한다면 $f(e_{i_1},\ldots,e_{i_n})$의 값은 항상 0이 되므로, 우변의 식은 
...
이 된다. 가령 $n=2$인 경우, $S_2$의 두 원소는 $\id$, 그리고 $1$과 $2$의 위치를 바꾸는 함수 $\tau$이므로 행렬식은
...
::: 따름정리 3
임의의 행렬 $A\in\Mat_n(\mathbb{K})$에 대하여, $\det(A^t)=\det A$가 성립한다.
:::
::: 증명
우선, transposition들 $\upsilon_i$에 대하여 $\tau=\upsilon_1\circ\ldots\upsilon_k$라면 $\tau^{-1}=\upsilon_k^{-1}\circ\cdots\circ\upsilon_1^{-1}$이므로 $\sgn(\tau)=\sgn(\tau^{-1})$가 항상 성립한다. 이제 $A^t$의 정의로부터 $A_{ij}=(A^t)_{ji}$이고,
...
::: 보조정리 4
임의의 행렬 $A,B\in\Mat_n(\mathbb{K})$에 대하여, $\det(AB)=\det(A)\det(B)$가 성립한다.
:::
::: 증명
...
[§행렬식, ⁋명제 3](/ko/math/linear_algebra/determinant#prop3){: data-lid="1kgu4" data-relation="required" }에 의하여 $i_1,\ldots, i_n$들 중 같은 것이 존재한다면 $\det(A_{i_1},\ldots, A_{i_n})$의 값은 항상 0이 되므로, 위 식의 우변에서는 $i_1,\ldots, i_n$이 서로 다른 항들만 남는다. 이제 $\upsilon\in S_n$을 다음의 식
...
우리는 이전 글에서 행렬 $A$가 가역인 것과 $\det A\neq 0$이 동치라는 것을 기하학적으로 설명했다. 방금 전의 [보조정리 4](#lem4){: data-lid="mh6o9" data-relation="required" }를 이용하면 이를 엄밀하게 증명할 수 있다.

::: 명제 5
임의의 행렬 $A\in\Mat_n(\mathbb{K})$에 대하여, $\det A\neq 0$인 것과 $A$가 가역인 것이 동치이다.
:::
::: 증명
[§선형대수학의 기본정리](/ko/math/linear_algebra/ftla){: data-lid="ga49b" data-relation="required" }로부터, $A$가 가역인 것은 $A$에 의해 정의되는 linear map $L_A:\mathbb{K}^n\rightarrow \mathbb{K}^n$이 가역인 것과 동치라는 것을 안다. $\mathbb{K}^n$은 유한차원이므로 이는 다시 $L_A$가 surjective인 것과 동치이고, 이는 $\col(A)$의 basis인 열벡터들 $A_1,\ldots, A_n$들이 일차독립인 것과 동치이다. 만일 $A_1,\ldots, A_n$들이 일차독립이 아니라면 [§행렬식, ⁋명제 3](/ko/math/linear_algebra/determinant#prop3){: data-lid="zcri0" data-relation="required" }에 의하여 $\det A=0$이 된다. 즉, $\det A\neq 0$이라면 $A$는 가역이다.
...
::: 따름정리 6
가역행렬 $A\in\Mat_n(\mathbb{K})$에 대하여 $\det(A^{-1})=(\det A)^{-1}$이 성립한다.
:::
...
::: 정의 7
행렬 $A\in\Mat_n(\mathbb{K})$이 *상삼각행렬<sub>upper triangular matrix</sub>*이라는 것은 $i>j$일 때마다 $A_{ij}=0$인 것이다. 비슷하게, $i < j$일 때마다 $A_{ij}=0$이라면 $A$를 *하삼각행렬<sub>lower triangular matrix</sub>*이라 부르고, 상삼각행렬과 하삼각행렬을 합쳐 간단히 *삼각행렬<sub>triangular matrix</sub>*이라 부른다.

한편, 행렬 $A$의 성분들 $A_{ii}$를 $A$의 *대각성분*이라 하고, 만일 $i\neq j$일 때마다 $A_{ij}=0$이라면 $A$를 *대각행렬<sub>diagonal matrix</sub>*이라 부른다. 
:::

특별히 모든 $n\times n$ 행사다리꼴행렬은 모두 상삼각행렬이다. ([§가우스 소거법, ⁋정의 4](/ko/math/linear_algebra/Gaussian_elimination#def4){: data-lid="bogif" data-relation="required" })


::: 명제 8
임의의 삼각행렬 $A$에 대하여, $\det(A)$는 대각성분들의 곱과 같다.
:::
::: 증명
위에서 살펴본 행렬식의 공식
...
[§가우스 소거법](/ko/math/linear_algebra/Gaussian_elimination){: data-lid="kek14" data-relation="required" }에 의해 임의의 행렬은 기본행연산을 반복하여 행사다리꼴행렬로 바꿀 수 있다. 임의의 행사다리꼴행렬은 모두 상삼각행렬이므로, 위의 명제를 통하면 이렇게 얻어진 행사다리꼴행렬의 행렬식을 매우 쉽게 구할 수 있다. 한편 기본행연산을 적용하는 것은 기본행렬들을 곱하는 것과 같다. 따라서, 만일 행렬 $A$로부터 기본행연산 $E_1,\ldots, E_k$를 반복하여 행사다리꼴행렬 $A'$를 얻었다면, 
...
::: 명제 9
$A\in\Mat_k(\mathbb{K})$이고 $I$가 $l\times l$ 항등행렬이라 하자. 그럼 다음의 블록행렬
...
:::
::: 증명
[명제 8](#prop8){: data-lid="dbzx9" data-relation="required" }의 증명과 거의 동일하다. 식 (2)을 통해 주어진 행렬의 행렬식을 계산하면
...
이 아닌 이상 우변에서 더해지는 값은 항상 0이 되고, 따라서 뒤의 $l$개가 고정되는 $\tau$에 대해서만 합을 계산하면 된다. 즉 주어진 행렬의 행렬식은 정확히 식 (2)와 동일하게 되어 주어진 명제가 성립한다. 
:::

::: 따름정리 10
$A\in\Mat_k(\mathbb{K}),B\in\Mat_l(\mathbb{K}), C\in\Mat_{l\times k}(\mathbb{K})$에 대하여, 다음의 블록행렬
...
과 [명제 9](#prop9){: data-lid="l13qf" data-relation="required" }, 그리고 [보조정리 4](#lem4){: data-lid="ausxq" data-relation="required" }에 의해 자명하다. 가운데 행렬의 경우 대각성분이 모두 1인 하삼각행렬이므로 [명제 8](#prop8){: data-lid="283r5" data-relation="required" }에 의해 그 행렬식은 1이다. 마지막 행렬의 경우, $l$번의 행 바꿈 후 $l$번의 열 바꿈을 하여 $2l$번의 부호 변경이 생겨 주어진 행렬의 행렬식이 $\det B$와 같게 된다.
:::
...
::: 정의 11
행렬 $A\in\Mat_n(\mathbb{K})$이 주어졌다 하자. $1\leq i,j\leq n$에 대하여, $A^{(i,j)}$는 행렬 $A$의 $i$행, $j$열을 없애서 얻어지는 $(n-1)$차 정사각행렬이다.  
:::

라플라스 전개는 $A$의 행렬식을 $\det A^{(i,j)}$들에 대한 식으로 나타내어준다. 

::: 정리 12
임의의 행렬 $A\in\Mat_n(\mathbb{K})$과, 임의의 $1\leq i\leq n$에 대하여 다음의 식
...
이다. 이제 [따름정리 10](#cor10){: data-lid="zo1sz" data-relation="required" }에 의하여 이 행렬의 행렬식은 $A_{ij}\det A^{(i,j)}$와 같고, 따라서

$$\det B_j=(-1)^{i+j-2}\det B_j'=(-1)^{i+j-2}A_{ij}\det A^{(i,j)}=(-1)^{i+j}A_{ij}\det A^{(i,j)}$$

이다. 한편, [따름정리 3](#cor3){: data-lid="k303y" data-relation="required" }에 의해 행렬식은 행에 대해서도 multilinear하고, $i$번째 행에 대한 multilinearity를 사용하면 $B_j$들의 행렬식의 합은 $A$의 행렬식과 같으므로 원하는 식
