...
Jordan canonical form은 각 block에 고유값 $\lambda$를 직접 담으므로 $\mathbb{K}$가 algebraically closed라는 가정에 의존한다. 같은 정보를 고유값을 명시하지 않고 다항식의 계수만으로 담는 표준형이 있는데, 그 기본 단위가 companion matrix이다.
