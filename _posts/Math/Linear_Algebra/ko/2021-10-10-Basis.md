...
이제, 임의의 $S$에 대하여, <phrase>$S$를 포함하는 $V$의 부분공간들의 family</phrase>를 $(W_i)_{i\in I}$라 하자. 이 family는 $V$를 포함하므로 공집합이 아니다. 따라서 교집합 $\bigcap_{i\in I} W_i$가 잘 정의되며, 이렇게 정의된 공간이 [정의 2](#def2)의 조건을 만족하는 부분공간임을 알 수 있다. 
...
라 할 수 있다. 유일한 것은 이 family이지 $v$를 적는 일차결합의 꼴이 아니다. 항의 순서를 바꾸거나 계수가 $0$인 항을 덧붙이면 서로 다른 $x_1,\ldots,x_n$과 $\alpha_1,\ldots,\alpha_n$이 나오지만, 이들이 정하는 family $(\alpha_x)_{x\in\mathcal{B}}$는 모두 같기 때문이다.

이 family는 오직 $v$에 의해서만 결정되므로 이를 $(\alpha_x^v)_{x\in\mathcal{B}}$와 같이 적기로 하자. [\[집합론\] §집합의 곱](/ko/math/set_theory/product_of_sets)에서 우리는 이러한 family가 $\mathcal{B}$에서 $\mathbb{K}$로의 함수, 즉 $x\in\mathcal{B}$를 받아 $\alpha_x^v$를 내놓는 함수임을 알고 있다. 따라서 이 함수의 $x$에서의 함수값을 $\alpha_x^v$ 대신 간략하게 $v_x$로 표현한다. 
...
::: 예시 8
Trivial한 $\mathbb{K}$-벡터공간 $\{0\}$의 basis는 $\emptyset$이다.
:::
...
이에 대한 증명은 학부 수준으로 충분히 가능하지만, [\[집합론\] §선택공리, ⁋정리 4](/ko/math/set_theory/axiom_of_choice#thm4)를 필요로 하므로 별도의 글로 남겨둔다. 그 대신 약간의 예시를 더 살펴보자. 
