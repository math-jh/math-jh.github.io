...
곧 직합 $V=W\oplus W^\perp$에서 $W$를 접어 없애면 $W^\perp$이 정확히 그 몫 $V/W$을 실현하게 된다. 일반적으로 몫공간 $V/W$은 $\langle-,-\rangle$ 없이도 정의되는 표준적인 대상인 반면, 이를 $V$ 안에서 실현한 대상인 $W^\perp$은 form에 의존하게 되며 [명제 8](#prop8){: data-relation="required" }의 non-degeneracy 없이는 이러한 표현이 불가능하다. 가령 $W\neq\{0\}$이 isotropic이면 $W\cap W^\perp\neq\{0\}$이라 $p\vert_{W^\perp}$이 단사가 되지 못한다.
