...
이므로 $A^tA$은 positive semidefinite이다. ([§스펙트럼 정리, ⁋정의 8](/ko/math/linear_algebra/spectral_theorem#def8){: data-lid="ultrl" }) 따라서 [§스펙트럼 정리, ⁋명제 9](/ko/math/linear_algebra/spectral_theorem#prop9){: data-lid="zkvp6" }에 의하여 $A^tA$의 모든 고윳값은 $0$ 이상이다.
:::
...
:::
::: 증명
[명제 1](#prop1){: data-lid="aqnpm" }에 의하여 $A^tA$은 self-adjoint이므로, [§스펙트럼 정리, ⁋정리 5](/ko/math/linear_algebra/spectral_theorem#thm5){: data-lid="faehg" }에 의하여 $A^tA$의 고유벡터들로 이루어진 $\mathbb{R}^n$의 orthonormal basis $\{v_1,\ldots, v_n\}$이 존재한다. 고윳값을 $\sigma_1^2\geq\cdots\geq\sigma_n^2\geq 0$의 순서로 두고, $\sigma_1,\ldots,\sigma_r$이 양수이고 $\sigma_{r+1}=\cdots=\sigma_n=0$이라 하자. 즉 $A^tAv_i=\sigma_i^2v_i$이다.
...
이므로 $\{u_1,\ldots, u_r\}$은 $\mathbb{R}^m$의 orthonormal set이다. 이를 확장하여 $\mathbb{R}^m$의 orthonormal basis $\{u_1,\ldots, u_m\}$을 얻을 수 있다. ([§벡터공간의 차원, ⁋명제 5](/ko/math/linear_algebra/dimension#prop5){: data-lid="8ff95" }, [§내적공간, §§정규직교기저](/ko/math/linear_algebra/inner_product_spaces#정규직교기저){: data-lid="4wm8i" }) 
...
[§최소제곱법, ⁋정의 7](/ko/math/linear_algebra/least_squares_method#def7){: data-lid="fimxb" }에서 우리는 $A$가 full column rank이거나 full row rank일 때 유사역행렬을 정의하였고, 일반적인 경우의 정의는 특이값 분해를 이용한다고 예고하였다. 이제 그 정의를 제시한다. 
...
::: 명제 6
[정의 5](#def5){: data-lid="n9ovz" }의 행렬 $A^+$은 다음의 네 조건
...
그럼 [§최소제곱법, ⁋정의 7](/ko/math/linear_algebra/least_squares_method#def7){: data-lid="j55ph" }에서 위의 네 조건이 $A^+$를 유일하게 특정지어준다는 것을 살펴보았으므로, [정의 5](#def5){: data-lid="a6ok6" }의 $A^+$은 특이값 분해의 선택과 무관하게 잘 정의될 뿐 아니라 full rank의 경우 두 정의가 일치하는 것 또한 안다. 예를 들어 $A$가 full column rank라면 $A^tA$이 가역이고, 행렬 $(A^tA)^{-1}A^t$이 
