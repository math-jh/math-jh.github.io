...
정의에서 coset $v+W$은 $v$와의 차이가 $W$에 속하는 벡터들, 즉 도입부에서 $V/W$ 안에서 $v$와 같게 취급하기로 한 벡터들을 모두 모은 집합이다. 이는 집합론에서 다루는 동치류의 한 예시이지만 ([\[집합론\] §동치관계, ⁋정의 4](/ko/math/set_theory/equivalence_relations#def4){: data-lid="vzbna" }), 우리에게 필요한 것은 위의 도입에서 주장한, 두 coset이 같은지를 대표원의 차이로 판정하는 다음 사실 뿐이다.
...
이다. [보조정리 2](#lem2){: data-lid="sig9k" }에 의해 이는 정확히 우리가 원하던 식이다. 따라서 $V/W$ 위의 두 연산은 잘 정의된다.

::: 명제 4
[정의 3](#def3){: data-lid="5wsju" }의 연산을 부여한 $V/W$는 $\mathbb{K}$-벡터공간이다. 이 때 덧셈에 대한 항등원은 $0+W=W$이고, $v+W$의 덧셈에 대한 역원은 $(-v)+W$이다.
:::
::: 증명
...
:::
::: 증명
$\dim W=k$, $\dim V=n$이라 하고, $W$의 basis $\{x_1,\ldots, x_k\}$를 택하자. 이는 $V$의 일차독립인 부분집합이므로, [§벡터공간의 차원, ⁋명제 5](/ko/math/linear_algebra/dimension#prop5){: data-lid="68z3f" }에 의하여 이를 확장하여 $V$의 basis $\{x_1,\ldots, x_k, x_{k+1},\ldots, x_n\}$을 얻을 수 있다. 우리는 다음의 coset들
...
라 하자. 그럼 앞에서 살펴봤듯 $\sum_{i=k+1}^n\alpha_ix_i+W=W=0+W$이므로, [보조정리 2](#lem2){: data-lid="karqj" }에 의해 $\sum_{i=k+1}^n\alpha_ix_i\in W$이다. 따라서 이 벡터를 $W$의 basis로 나타내어 적당한 스칼라들 $\beta_1,\ldots,\beta_k$에 대해
...
이 글이 별도의 글로 존재하는 이유 중 하나는 [§동형사상, ⁋정리 7](/ko/math/linear_algebra/isomorphic_vector_spaces#thm7){: data-lid="xyiwg" }에 더 본질적인 의미를 부여하기 위한 것이다. 이 마지막 섹션에서 우리는 이를 해결한다.  

임의의 $\mathbb{K}$-벡터공간 $V$와 부분공간 $W\leq V$에 대하여, 다음의 식

$$p(v)=v+W$$

으로 정의된 함수 $p:V\rightarrow V/W$를 생각하자. 그럼 [정의 3](#def3){: data-lid="iuq5d" }의 연산은 정확히 $p$가 다음의 두 식 
...
이 성립하며, 이는 정확히 [§동형사상, ⁋정리 7](/ko/math/linear_algebra/isomorphic_vector_spaces#thm7){: data-lid="nhl7t" }의 식이다. 즉 rank-nullity 정리는 $L$이 $\ker L$을 "접어버린" 후에는 단사가 된다는 사실을 차원의 언어로 옮긴 것에 지나지 않는다. 
