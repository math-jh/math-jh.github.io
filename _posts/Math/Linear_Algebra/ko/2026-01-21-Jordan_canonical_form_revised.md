...
이 행렬의 유일한 고유값은 1이지만, 해당 고유공간은 벡터 $(1,0)$으로 생성되는 1차원 공간이다. 즉, 우리가 기대하는 2차원 공간 대신 "부족한" 1차원의 고유공간만을 얻게 된다.

이러한 상황에서 우리가 할 수 있는 자연스러운 일은 "고유공간을 어떻게 확장할 수 있을까?"를 생각하는 것이다. 바꾸어 말하자면, 고유벡터가 부족할 때 우리는 어떤 "거의 고유벡터인" 벡터들을 찾아야 한다. 이를 위해 우리는 *generalized eigenspace*라는 개념을 도입할 것이다.

## 일반화된 고유공간

앞서 우리는 diagonalizable operator $A$가 주어질 때마다 주어진 공간을 eigenspace들로 분해하여 이 위에서는 $A$가 스칼라곱처럼 행동하도록 할 수 있음을 보았다. 그러나 [§고유공간분해, ⁋정리 6](/ko/math/linear_algebra/eigenspace_decomposition#thm6)에서 살펴봤듯, 설령 $\mathbb{K}$가 algebraically closed field라 가정하여도 (그리고, 해당 명제 이후에 살펴 봤듯 우리는 항상 이를 가정할 것이다.) 모든 linear operator가 항상 diagonalizable인 것은 아니다. 

[§고유공간분해, ⁋정리 6](/ko/math/linear_algebra/eigenspace_decomposition#thm6)의 둘째 조건은 $A$의 어떤 고유값 $\lambda$에 대하여, $\lambda$의 기하적 중복도가 $\lambda$의 대수적 중복도보다 작을 때 발생한다는 것을 알려준다. ([§고유공간분해, ⁋명제 5](/ko/math/linear_algebra/eigenspace_decomposition#prop5)) 즉, 직관적으로 다음의 벡터공간
...
즉, 위의 보조정리는 고유공간 $\ker (L-\lambda I)$ 자체는 그 차원이 부족하지만, 이 공간을 늘려가다 볼면 "맞는 차원"을 얻게 된다는 실마리를 제공한다. 이를 엄밀하게 서술하기 위해서는 다음과 같은 정의가 필요하다. 

<div class="definition" markdown="1">

<ins id="def2">**정의 2**</ins> 유한차원 벡터공간 $V$ 위에 정의된 linear operator $L$와 $L$의 한 eigenvalue $\lambda$에 대하여, $L$의 $\lambda$에 대한 *generalized eigenspace<sub>일반화된 고유공간</sub>*를 다음의 식
...
직관적으로 생각하면, generalized eigenspace는 "진짜" 고유벡터뿐만 아니라 "$(L-\lambda I)$를 여러 번 적용하면 결국 0이 되는" 모든 벡터들을 포함하는 공간이다. 즉, 우리가 고유공간이 "너무 작다"는 문제에 직면했을 때, 이 공간을 자연스럽게 확장하는 방법을 제공한다.

<div class="proposition" markdown="1">

<ins id="thm3">**정리 3**</ins> 유한차원 벡터공간 $V$ 위에 정의된 linear operator $L:V\rightarrow V$과 그 eigenvalue $\lambda$에 대하여, 적당한 양의 정수 $k$가 존재하여 $G_\lambda(L)=\ker(L-\lambda I)^k$이다.
...
임을 안다. 즉 $v\in\ker (L-\lambda I)^{k'}$이다. 그런데 $k$의 정의에 의하여 $\ker(L-\lambda I)^k=\ker(L-\lambda I)^{k'}$이고 이로부터 $v\in \ker (L-\lambda I)^k$이다. ($k'$는 $v$에 의존하지 않지만, $k$는 그렇지 않다.) 포함관계 $\ker (L-\lambda I)^k\subset G_\lambda(L)$은 자명하므로 원하는 결과를 얻는다. 
...
<ins id="thm4">**정리 4 (일차분해정리)**</ins> 유한차원 벡터공간 $V$ 위에 정의된 linear operator $L:V\rightarrow V$에 대하여, $L$의 모든 eigenvalue들을 $\lambda_1,\ldots,\lambda_m$이라 하자. 그럼 다음의 직합분해
...
먼저 $i\neq j$일 때 $G_{\lambda_i}(L)\cap G_{\lambda_j}(L)=\{0\}$임을 보이자. $v\in G_{\lambda_i}(L)\cap G_{\lambda_j}(L)$이고 $v\neq 0$이라 가정하자. [정리 3](#thm3)로부터 $(L-\lambda_i I)^{k_i}v=0$과 $(L-\lambda_j I)^{k_j}v=0$을 만족하는 정수 $k_i, k_j$가 존재한다. 
...
마지막으로 차원을 계산하자. [정리 3](#thm3)로부터 각 $i$에 대해 적당한 $k_i$가 존재하여 $G_{\lambda_i}(L)=\ker(L-\lambda_i I)^{k_i}$이다. $G_{\lambda_i}(L)$의 basis를 부분공간들의 filtration
...
<ins id="def5">**정의 5**</ins> Linear operator $N:V\rightarrow V$이 *nilpotent<sub>멱영</sub>*이라는 것은 적당한 양의 정수 $k$에 대하여 $N^k=0$인 것이다.

</div>

<div class="proposition" markdown="1">

<ins id="prop6">**명제 6**</ins> 일차분해정리의 상황에서, 각 $G_{\lambda_i}(L)$ 위에서 $L-\lambda_i I$는 nilpotent operator이다.

</div>
<details class="proof" markdown="1">
<summary>증명</summary>

[정리 3](#thm3)로부터 적당한 $k$에 대해 $G_{\lambda_i}(L)}=\ker(L-\lambda_i I)^k$이므로, 임의의 $v\in G_{\lambda_i}(L)}$에 대해 $(L-\lambda_i I)^k v=0$이다.
...
즉, 대각선 위 원소들은 모두 $\lambda$이고, 대각선 바로 위(superdiagonal)의 원소들은 모두 1이며, 나머지는 모두 0이다.
...
이다. 직접 계산하면 $N^2$는 superdiagonal에서 한 칸 더 위의 대각선에만 1이 있고, 일반적으로 $N^i$는 $i$번째 위 대각선에만 1이 있다. 따라서 $N^k=0$이다.
...
[정리 4](#thm4)로부터 $V=\bigoplus_{i=1}^m G_{\lambda_i}(L)}$이고, 각 $G_{\lambda_i}(L)}$ 위에서 $L-\lambda_i I$는 nilpotent이다. 따라서 각 generalized eigenspace에서 적당한 basis를 택하면, 그 basis에 대한 $L$의 행렬 표현이 Jordan block들의 직합으로 나타난다는 것을 보이면 충분하다.
...
각 generalized eigenspace $G_{\lambda_i}(L)}$에 대해 이 과정을 적용하고, 얻은 basis들을 합치면 $V$ 전체의 basis를 얻고, 이 basis에 대한 $L$의 행렬 표현은 정리 10의 형태가 된다.

</details>

<div class="example" markdown="1">

<ins id="ex11">**예시 11**</ins> [예시 1](#ex1)의 행렬

$$A=\begin{pmatrix}1&1&1\\0&1&1\\0&0&1\end{pmatrix}$$

의 Jordan canonical form을 구해 보자. $A$의 유일한 eigenvalue는 $\lambda=1$이고,
...
**해석**: 이 Jordan form은 $A$가 3차원 공간에서 "3단계 chain" 구조를 가진다는 것을 보여준다. 즉, $A$는 벡터를 "3단계에 걸쳐" 변환시키며, 각 단계에서 이전 단계의 정보를 "한 단계 위로" 전달한다.
