...
weight: 120
translated_at: 2026-06-26T10:30:02+00:00
...
In this post, we show that the answer to this question is the self-adjoint operator. As we saw in [§Inner Product Spaces](/en/math/linear_algebra/inner_product_spaces), the inner product essentially depends heavily on the base field, so we divide this theorem into the cases where the base field is $$\mathbb{R}$$ and $$\mathbb{C}$$, and treat the latter case separately in [§Complex Spectral Theorem](/en/math/linear_algebra/complex_spectral_theorem).
...
Now consider the eigenvector $$z\in\mathbb{C}^n$$ corresponding to this, and let $$\bar z$$ be the vector obtained by taking the complex conjugate of each component of $$z$$. Consider the following complex number
...
On the other hand, since $$s$$ is a $$1\times 1$$ matrix, it equals its own transpose, and using that $$A$$ is a real symmetric matrix so $$A=A^t=\bar A$$, its complex conjugate is

$$\bar s=\overline{\bar z^tAz}=z^t\bar A\bar z=z^tA\bar z=(z^tA\bar z)^t=\bar z^tA^tz=\bar z^tAz=s$$

Thus, $$s$$ is real. However, since $$z\neq 0$$, $$\sum_i\lvert z_i\rvert^2$$ is a positive real number, and therefore for $$s=\lambda\sum_i\lvert z_i\rvert^2$$ to be real, $$\lambda$$ must also be real.

</details>

It is inevitable that we must move the polynomial to $$\mathbb{C}$$ for the proof of this proposition, and indeed in the next post we will prove the complex version of the spectral theorem. In particular, from this we know that a self-adjoint operator always has an eigenvalue (and hence an eigenvector).
...
We now prove that if a self-adjoint operator $$L$$ preserves a subspace $$U$$ of $$V$$, that is, if $$L(U)\subseteq U$$ holds, then its orthogonal complement is also invariant under this operator. This is, of course, essential work for constructing the *orthogonal* basis sequentially as we proposed in the introduction.
...
and since $$L(U)\subseteq U$$, we have $$Lu\in U$$, and since $$w\in U^\perp$$, $$\langle w,Lu\rangle=0$$. That is, $$\langle Lw,u\rangle=0$$ holds for all $$u\in U$$, so $$Lw\in U^\perp$$.
...
We prove by induction on $$\dim V$$. The case $$\dim V=0$$ is trivial. Suppose $$\dim V\geq 1$$. By [Corollary 3](#cor3), $$L$$ has an eigenvector, and dividing by its norm we obtain an eigenvector $$v_1$$ with $$\lVert v_1\rVert=1$$. Its eigenvalue $$\lambda_1$$ is real by [Lemma 2](#lem2).

Let $$U=\span v_1$$. Then $$L(U)\subseteq U$$, so by [Lemma 4](#lem4), $$L(U^\perp)\subseteq U^\perp$$. On the other hand, as we saw in the discussion after [§Inner Product Spaces, ⁋Theorem 9](/en/math/linear_algebra/inner_product_spaces#thm9), we have $$V=U\oplus U^\perp$$ and $$\dim U^\perp=\dim V-1$$. Also, the restriction $$L\vert_{U^\perp}:U^\perp\rightarrow U^\perp$$ still satisfies $$\langle Lw,w'\rangle=\langle w,Lw'\rangle$$ for all $$w,w'\in U^\perp$$, so it is again self-adjoint on $$U^\perp$$. Therefore, by the inductive hypothesis, there exists an orthonormal basis $$\{v_2,\ldots, v_n\}$$ of $$U^\perp$$ consisting of eigenvectors of $$L\vert_{U^\perp}$$.

These vectors are also eigenvectors of $$L$$, and since $$v_1\in U$$ and $$v_2,\ldots, v_n\in U^\perp$$, $$v_1$$ is orthogonal to the rest. Therefore $$\{v_1,v_2,\ldots, v_n\}$$ is an orthonormal basis of $$V$$ consisting of eigenvectors of $$L$$.
...
The matrix of a positive definite operator also decomposes concisely via a triangular matrix. In fact, this decomposition is a simplified form of the LU decomposition ([§Gaussian Elimination, ⁋Definition 8](/en/math/linear_algebra/Gaussian_elimination#def8)) for general square matrices applied to symmetric positive definite matrices, where only the $$L$$ part needs to be computed since the $$U$$ part comes out automatically, thus halving the computational cost.
...
and all diagonal entries of $$L$$ are positive. Uniqueness follows from the first column being determined by $$\sqrt\alpha$$ and $$\alpha^{-1/2}b$$, and $$L'$$ being unique inductively.
