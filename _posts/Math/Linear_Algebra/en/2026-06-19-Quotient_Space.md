...
By [Lemma 2](#lem2){: data-lid="sig9k" } this is exactly the equality we wanted. Hence the two operations on $V/W$ are well defined.

::: Proposition 4
The set $V/W$ equipped with the operations of [Definition 3](#def3){: data-lid="5wsju" } is a $\mathbb{K}$-vector space. The additive identity is $0+W=W$, and the additive inverse of $v+W$ is $(-v)+W$.
:::
::: Proof
...
:::
::: Proof
Let $\dim W=k$, $\dim V=n$, and choose a basis $\{x_1,\ldots, x_k\}$ of $W$. Since this is a linearly independent subset of $V$, by [§Dimension of Vector Spaces, ⁋Proposition 5](/en/math/linear_algebra/dimension#prop5){: data-lid="68z3f" } we can extend it to a basis $\{x_1,\ldots, x_k, x_{k+1},\ldots, x_n\}$ of $V$. We show that the cosets
...
Then, as seen above, $\sum_{i=k+1}^n\alpha_ix_i+W=W=0+W$, so by [Lemma 2](#lem2){: data-lid="karqj" } we have $\sum_{i=k+1}^n\alpha_ix_i\in W$. Hence we can express this vector in the basis of $W$ to obtain scalars $\beta_1,\ldots,\beta_k$ such that
...
One reason this post exists as a separate article is to give a more essential meaning to [§Isomorphisms, ⁋Theorem 7](/en/math/linear_algebra/isomorphic_vector_spaces#thm7){: data-lid="xyiwg" }. In this final section we resolve this.

For any $\mathbb{K}$-vector space $V$ and subspace $W\leq V$, consider the function $p:V\rightarrow V/W$ defined by

$$p(v)=v+W.$$

Then the operations of [Definition 3](#def3){: data-lid="iuq5d" } are defined precisely so that $p$ satisfies the two identities
...
holds, which is exactly the formula of [§Isomorphisms, ⁋Theorem 7](/en/math/linear_algebra/isomorphic_vector_spaces#thm7){: data-lid="nhl7t" }. In other words, the rank-nullity theorem is nothing more than the statement that after "folding away" $\ker L$, $L$ becomes injective, expressed in the language of dimensions.
