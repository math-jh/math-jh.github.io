...
The Gram matrix with respect to the basis of [Proposition 2](#prop2){: data-lid="4b5sy" data-relation="required" } is a diagonal matrix with diagonal entries $1$, $-1$, or $0$. By reordering the basis appropriately, we can make this diagonal matrix into the form with $p$ copies of $1$, $q$ copies of $-1$, and $r$ copies of $0$:
...
so if there existed $0\neq v\in U\cap U_-$, then $Q(v)>0$ and $Q(v)\leq 0$ would hold simultaneously, a contradiction. Therefore $U\cap U_-=\{0\}$, and by [§Dimension of Vector Spaces, ⁋Example 8](/en/math/linear_algebra/dimension#ex8){: data-lid="i1j0s" data-relation="required" },
...
::: Definition 4
For a symmetric bilinear form $\langle-,-\rangle$ defined on an $\mathbb{R}$-vector space $V$, the three numbers $(p,q,r)$ determined by [Theorem 3](#thm3){: data-lid="qzj7n" data-relation="required" } are called the *signature* of $\langle-,-\rangle$.
:::
...
The case where the signature is $(n,0,0)$, i.e., where $\langle v,v\rangle>0$ for all $0\neq v\in V$, is particularly important. This is the same as the Gram matrix being positive definite in the sense of [§Spectral Theorem, ⁋Definition 8](/en/math/linear_algebra/spectral_theorem#def8){: data-lid="e4w9e" data-relation="weak" }, and in this case we call $\langle-,-\rangle$ itself positive definite.
