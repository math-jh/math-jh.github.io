...
::: Definition 1
For a symmetric bilinear form $\langle -,-\rangle$ defined on an $\mathbb{R}$-vector space $V$, the function $Q:V\rightarrow\mathbb{R}$ defined by the formula
...
::: Proposition 2
For any symmetric bilinear form $\langle-,-\rangle$ defined on an $\mathbb{R}$-vector space $V$, there exists an appropriate basis $\{e_1,\ldots, e_n\}$ of $V$ such that $\langle e_i,e_j\rangle=0$ when $i\neq j$, and each $\langle e_i,e_i\rangle$ is one of $1$, $-1$, or $0$.
:::
::: Proof
...
The Gram matrix with respect to the basis of [Proposition 2](#prop2){: data-lid="4b5sy" } is a diagonal matrix with diagonal entries $1$, $-1$, or $0$. By reordering the basis appropriately, we can make this diagonal matrix into the form with $p$ copies of $1$, $q$ copies of $-1$, and $r$ copies of $0$:
...
so if there existed $0\neq v\in U\cap U_-$, then $Q(v)>0$ and $Q(v)\leq 0$ would hold simultaneously, a contradiction. Therefore $U\cap U_-=\{0\}$, and by [§Dimension of Vector Spaces, ⁋Example 8](/en/math/linear_algebra/dimension#ex8){: data-lid="i1j0s" },
...
::: Definition 4
For a symmetric bilinear form $\langle-,-\rangle$ defined on an $\mathbb{R}$-vector space $V$, the three numbers $(p,q,r)$ determined by [Theorem 3](#thm3){: data-lid="qzj7n" } are called the *signature* of $\langle-,-\rangle$.
:::
...
:::
::: Proof
If two matrices $G,G'$ are congruent, then they are the same bilinear form written with respect to different bases, so their signatures are the same. Conversely, if the signature is $(p,q,r)$ for both, then by [Proposition 2](#prop2){: data-lid="cmtw8" } both $G$ and $G'$ are congruent to the same diagonal matrix with $p$ copies of $1$, $q$ copies of $-1$, and $r$ copies of $0$, and since congruence is an equivalence relation, $G$ and $G'$ are congruent to each other.
:::

## Positive Definiteness

The case where the signature is $(n,0,0)$, i.e., where $\langle v,v\rangle>0$ for all $0\neq v\in V$, is particularly important. This is the same as the Gram matrix being positive definite in the sense of [§Spectral Theorem, ⁋Definition 8](/en/math/linear_algebra/spectral_theorem#def8){: data-lid="e4w9e" }, and in this case we call $\langle-,-\rangle$ itself positive definite.
...
:::
::: Proof
For the basis $\{e_1,\ldots, e_n\}$ of [Proposition 2](#prop2){: data-lid="qc1qa" }, if $v=\sum a_ie_i$ then $Q(v)=\sum_i\langle e_i,e_i\rangle a_i^2$. If some $\langle e_i,e_i\rangle$ is $-1$ or $0$, then for $v=e_i$ we have $Q(e_i)\leq 0$, so it is not positive definite. Conversely, if all $\langle e_i,e_i\rangle$ are $1$, i.e., the signature is $(n,0,0)$, then for any $0\neq v$ we have $Q(v)=\sum a_i^2>0$. Thus 1 and 2 are equivalent.

On the other hand, the definition of an inner product is a symmetric bilinear form such that $\langle v,v\rangle\geq 0$ for all $v$ and equality holds only when $v=0$ ([§Inner Product Spaces, ⁋Definition 1](/en/math/linear_algebra/inner_product_spaces#def1){: data-lid="607wh" }), which is exactly the condition of being positive definite. Thus 1 and 3 are equivalent.
:::
