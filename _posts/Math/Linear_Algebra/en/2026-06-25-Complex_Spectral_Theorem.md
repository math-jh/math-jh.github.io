...
::: Definition 1
A linear operator $L:V\rightarrow V$ on a complex inner product space $V$ is called *self-adjoint*, or *Hermitian*, if $L=L^\ast$; equivalently, $\langle Lv,w\rangle=\langle v,Lw\rangle$ for all $v,w\in V$.
:::
...
::: Definition 2
A linear operator $L:V\rightarrow V$ on a complex inner product space $V$ is called a *normal operator* if
...
Then in particular a self-adjoint operator satisfies $LL^\ast=L^2=L^\ast L$, so it is normal, and a unitary operator is as well. ([§Complex Inner Product Spaces, ⁋Definition 6](/en/math/linear_algebra/complex_inner_product_spaces#def6){: data-lid="5btee" data-relation="required" }) Moreover, a *skew-Hermitian* operator satisfying $L^\ast=-L$ also satisfies $LL^\ast=-L^2=L^\ast L$, so it is normal. Thus normal operators form a broad class containing many special cases, and the goal of this post is to show that they are exactly the operators diagonalized by an orthonormal basis.
...
::: Corollary 4
A normal matrix $A\in\Mat_n(\mathbb{C})$ is diagonalized by a unitary matrix.
:::
::: Proof
By [Theorem 3](#thm3){: data-lid="scc8a" data-relation="required" } we may write $A=UTU^\ast$ with $T$ upper triangular, and we need to show that if $A$ is normal then $T$ is diagonal. Since $U$ is unitary, $T=U^\ast AU$ is also normal, and we show by induction on the rows of $T$ that an upper triangular normal matrix is diagonal.
...
The Schur decomposition forms a nice contrast with [§Jordan Canonical Form](/en/math/linear_algebra/Jordan_canonical_form){: data-lid="blwr3" data-relation="weak" }. The Jordan canonical form uses a generally non-unitary change of basis to put a matrix into a canonical block form, whereas the Schur decomposition restricts the change of basis to be unitary at the cost of giving up a canonical form and settling for an upper triangular one. The latter preserves orthonormality and is numerically stable, an advantage that makes it frequently used in applications.

## Complex Spectral Theorem

The unitary diagonalization of a normal matrix is a direct consequence of [Theorem 3](#thm3){: data-lid="kot0j" data-relation="required" }. More precisely, the complex version of the spectral theorem shows that a normal operator is exactly an operator diagonalized by an orthonormal basis.

::: Theorem 5 (Complex Spectral Theorem)
...
:::
::: Proof
If $L$ is normal, then it is diagonalized by an orthonormal basis by [Corollary 4](#cor4){: data-lid="sibgn" data-relation="required" }, so the key is the converse. Suppose there exists an orthonormal basis $\{v_1,\ldots,v_n\}$ of $V$ consisting of eigenvectors of $L$ with $Lv_i=\lambda_i v_i$. Then by [§Complex Inner Product Spaces, ⁋Proposition 5](/en/math/linear_algebra/complex_inner_product_spaces#prop5){: data-lid="8mdl2" data-relation="required" }, the matrix representation of $L^\ast$ with respect to this basis is $\diag(\bar\lambda_1,\ldots,\bar\lambda_n)$. Now two diagonal matrices commute, so $LL^\ast=L^\ast L$, and hence $L$ is normal.
:::
...
::: Corollary 6
A matrix $A\in\Mat_n(\mathbb{C})$ is a normal matrix if and only if there exist a unitary matrix $U$ and a diagonal matrix $D$ such that
...
As explained above, self-adjoint operators and unitary operators are normal, so by [Theorem 5](#thm5){: data-lid="k5fdw" data-relation="required" } they are both diagonalized by an orthonormal basis. What makes them special is that the diagonal matrix obtained from the diagonalization has a particular form.

::: Proposition 7
All eigenvalues of a self-adjoint operator on a complex inner product space are real, and all eigenvalues of a unitary operator have absolute value $1$.
:::
::: Proof
...
The complex spectral theorem includes the real version ([§Spectral Theorem, ⁋Theorem 5](/en/math/linear_algebra/spectral_theorem#thm5){: data-lid="x8ljq" data-relation="weak" }) as a special case. That is, a real symmetric matrix $A$ has real entries, so regarded as a complex matrix we have $A^\ast=\bar A^t=A^t=A$, making it self-adjoint over the complex numbers as well, and hence by [Proposition 7](#prop7){: data-lid="71nsi" data-relation="required" } all its eigenvalues are real.
