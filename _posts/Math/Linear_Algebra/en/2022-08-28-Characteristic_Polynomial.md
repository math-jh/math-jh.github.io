...
:::
::: Proof
From the preceding argument we know that $A$ and $B$ have the same characteristic polynomial, and by [Proposition 2](#prop2){: data-lid="zij1p" } the trace and determinant of a matrix are determined by its characteristic polynomial.
:::
...
The characteristic equation of the matrix $J$ in [Example 7](#ex7){: data-lid="fvymc" } above has no roots in $\mathbb{R}$, but has two roots in $\mathbb{C}$. Henceforth, it will be important to distinguish over which field the roots of a polynomial are defined.
