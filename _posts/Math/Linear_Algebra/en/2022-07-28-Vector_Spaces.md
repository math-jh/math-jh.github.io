...
A vector space is an abelian group $V$ equipped with the additional structure of $\mathbb{K}$-scalar multiplication. Therefore $V$ enjoys all the properties that abelian groups have. ([§Abelian Groups and Fields, ⁋Proposition 2](/en/math/linear_algebra/fields#prop2){: data-lid="ope03" } and [§Abelian Groups and Fields, ⁋Corollary 3](/en/math/linear_algebra/fields#cor3){: data-lid="gjx11" })
...
:::
::: Proof
The first two claims proceed similarly to [§Abelian Groups and Fields, ⁋Proposition 6](/en/math/linear_algebra/fields#prop6){: data-lid="yyqns" }. For example, since
...
A slightly less trivial example is a field itself. For any field $\mathbb{K}$, $\mathbb{K}$ is a $\mathbb{K}$-vector space. Since $\mathbb{K}$ is a field, it is trivially an abelian group under addition. It suffices to give it a scalar multiplication structure, which we can do simply by taking multiplication in $\mathbb{K}$, $\mathbb{K}\times \mathbb{K}\rightarrow \mathbb{K}$. With this definition, one can check that scalar multiplication satisfies all the conditions of [Definition 1](#def1){: data-lid="58x64" }, and therefore $\mathbb{K}$ is a $\mathbb{K}$-vector space in its own right.
...
If we think of $\Fun(I,\mathbb{R})$ as the product set $\mathbb{R}^I$, [Example 6](#ex6){: data-lid="jthiz" } can also be regarded as a natural generalization of [Example 5](#ex5){: data-lid="7wfwt" }. ([\[Set Theory\] §Product of Sets, ⁋Definition 1](/en/math/set_theory/product_of_sets#def1){: data-lid="aznt1" })
