...
weight: 2
translated_at: 2026-08-27T19:17:52+00:00
translation_source: kimi-cli
---
As mentioned at the beginning of the previous post, the *vector space*, the space studied in linear algebra, is a concept that generalizes the coordinate space we learn in high school. To this end, we defined abelian groups and fields in the previous post.
...
::: Definition 1
Let $\mathbb{K}$ be a field and $V$ an abelian group. We say that $V$ is a *vector space over $\mathbb{K}$*, or simply a *$\mathbb{K}$-vector space*, if there is an additional operation (*scalar multiplication*) $\cdot:\mathbb{K}\times V\rightarrow V$ such that
...
A slightly less trivial example is a field itself. For any field $\mathbb{K}$, $\mathbb{K}$ is a $\mathbb{K}$-vector space. Since $\mathbb{K}$ is a field, it is trivially an abelian group under addition. It suffices to give it a scalar multiplication structure, which we can do simply by using multiplication in $\mathbb{K}$, $\mathbb{K}\times \mathbb{K}\rightarrow \mathbb{K}$. With this definition, one can check that the scalar multiplication satisfies all the conditions of [Definition 1](#def1), and therefore $\mathbb{K}$ is a $\mathbb{K}$-vector space in its own right.
...
The two vector spaces we examined above are fairly concrete examples. As the next example shows, in general a vector space need not be visually represented like the coordinate plane or coordinate space.
