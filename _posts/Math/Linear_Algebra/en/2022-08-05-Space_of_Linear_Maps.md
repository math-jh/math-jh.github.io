...
Using the theorem above, we can prove the following proposition corresponding to [\[Set Theory\] §Retraction and Section, ⁋Proposition 1](/en/math/set_theory/retraction_and_section#prop1){: data-lid="7jn0o" }.
...
1. First suppose $L$ is injective, and choose a basis $x_1,\ldots,x_n$ of $V$. Then $L(x_1),\ldots, L(x_n)$ are linearly independent, and therefore we can find a basis $\mathcal{B}$ of $W$ containing them. Now define a function $r:\mathcal{B}\rightarrow V$ by
    ...
    and apply [Theorem 1](#thm1){: data-lid="a3qxb" } to obtain a linear map, which we call $R$. Then for any element $x_i$ of the basis $\{x_1,\ldots,x_n\}$ of $V$, we have $(R\circ L)(x_i)=x_i$, and hence by the uniqueness part of Theorem 1, $R\circ L=\id_V$.
...
2. Suppose $L$ is surjective, and choose a basis $x_1,\ldots,x_n$ of $V$. Then $L(x_1),\ldots, L(x_n)$ span $W$, so we can select some of these vectors to form a basis $\mathcal{B}$ of $W$. Without loss of generality, let $\mathcal{B}=\{L(x_1),\ldots, L(x_m)\}$ ($m\leq n$). Define a function $s:\mathcal{B}\rightarrow V$ by
    ...
    and apply [Theorem 1](#thm1){: data-lid="jlj1s" } to obtain a linear map, which we call $S$. Now for any element $L(x_k)$ of the basis $\mathcal{B}$ of $W$, we have $(L\circ S)(L(x_k))=L(x_k)$, so again by the uniqueness part of Theorem 1, $L\circ S=\id_W$.
:::
...
::: Definition 4
For two $\mathbb{K}$-vector spaces $V,W$, the $\mathbb{K}$-vector space obtained by equipping the set of linear maps from $V$ to $W$ with the operations from [Lemma 3](#lem3){: data-lid="sg0kl" } is denoted $\Hom_\mathbb{K}(V,W)$, or simply $\Hom(V,W)$ when the field $\mathbb{K}$ is clear from context.
...
The zero vector in the vector space $\Hom(V,W)$ is the function $0$ sending every element to 0. ([§Linear Maps, ⁋Example 10](/en/math/linear_algebra/linear_map#ex10){: data-lid="a7fkx" }) When referring to this function, we shall call it the zero function for convenience.
...
That is, $f_i^j$ is the function sending only $x_i$ to $y_j$ and everything else to 0. Then by [Theorem 1](#thm1){: data-lid="bbbep" }, there exists a unique linear map $B_i^j$ such that $f_i^j=B_i^j\circ\iota$.
...
Hence by the uniqueness part of [Theorem 1](#thm1){: data-lid="nfvf9" }, we have $L'=L$.
:::
