...
weight: 5
translated_at: 2026-06-26T19:30:02+00:00
translation_source: kimi-cli
last_polished_at: 2026-06-26T19:30:02+00:00
---
## Dimension of a Vector Space

From [§Basis of a Vector Space, ⁋Example 9](/en/math/linear_algebra/basis#ex9){: data-lid="jomic" data-relation="weak" } and [§Basis of a Vector Space, ⁋Example 11](/en/math/linear_algebra/basis#ex11){: data-lid="uvnhd" data-relation="weak" }, we see that a basis of a vector space $V$ need not be unique. However, in these examples the number of elements in each basis remains the same. This is not a coincidence.
...
Of course, there is no reason we could not prove this theorem now, but just as with [§Basis of a Vector Space, ⁋Theorem 10](/en/math/linear_algebra/basis#thm10){: data-lid="8kb3u" data-relation="weak" }, doing so requires a bit of set-theoretic background, so we defer it to a separate post. The last step, however, can be proved without any additional prerequisites.
...
Since by [Theorem 1](#thm1){: data-lid="7p5f3" data-relation="required" } all bases of $V$ have the same cardinality, the following definition is well-posed.
...
Choose a natural number $m$ such that $S\setminus\langle S\rangle_m=\emptyset$. That is, $S\subseteq\langle S\rangle_m$. From [§Basis of a Vector Space, ⁋Lemma 4](/en/math/linear_algebra/basis#lem4){: data-lid="egvx8" data-relation="required" }, we know that taking $\span$ preserves inclusion relations between sets, so

$$\langle S\rangle\subseteq\span\bigl(\langle S\rangle_m\bigr)$$

and since $\langle S\rangle_m$ on the right-hand side is already a subspace of $V$, by [§Basis of a Vector Space, ⁋Definition 2](/en/math/linear_algebra/basis#def2){: data-lid="gxq0q" data-relation="required" } we have $\span\bigl(\langle S\rangle\bigr)=\langle S\rangle_m$. Therefore, from
...
It is not difficult to verify that if $\mathcal{B}_1$ and $\mathcal{B}_2$ are bases of $V$ and $W$ respectively, then the subset
...
then the left-hand side is an element of $W_1$ and the right-hand side is an element of $W_2$, so this common vector lies in $W_1\cap W_2$. Since $\mathcal{B}_0$ is a basis of $W_1\cap W_2$, we can choose appropriate scalars $\alpha_i'$ so that

$$\beta_1y_1+\cdots+\beta_my_m=\alpha_1'x_1+\cdots+\alpha_k'x_k=-\gamma_1z_1-\cdots-\gamma_nz_n$$

In the case of the first equality, moving the $\alpha_i'x_i$ terms back to the left-hand side and combining them with the $\beta_iy_i$ terms gives
