...
::: Example 1
Consider the Euclidean $n$-space $\mathbb{K}^n$ and a matrix $A\in\Mat_{m\times n}(\mathbb{K})$. For any $x\in\mathbb{K}^n$, define $L_A(x)$ by the formula
...
::: Theorem 3
Let three Euclidean spaces $\mathbb{K}^n,\mathbb{K}^m,\mathbb{K}^k$ be given. Then for any $L_1:\mathbb{K}^n\rightarrow \mathbb{K}^m$, $L_2:\mathbb{K}^m\rightarrow \mathbb{K}^k$, we always have
...
{% diagram Math/Linear_Algebra/Fundamental_Theorem_of_Linear_Algebra-1.svg width="13.53em" alt="FTLA" %}
...
{% diagram Math/Linear_Algebra/Change_of_Basis-1.svg width="5.93em" alt="change_of_basis" %}
...
::: Definition 8
Let arbitrary $n\times n$ matrices $A,B$ be given. Then $A$ and $B$ are called *similar matrices* if there exists an invertible matrix $P$ such that $A=PBP^{-1}$.
:::
