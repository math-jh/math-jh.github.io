---
title: "대수적 stack과 quotient stack"
description: "대각선의 표현가능성과 매끄러운 atlas로 대수적 stack(Artin·Deligne–Mumford)을 정의하고, 군작용의 quotient stack [X/G]와 BG=[pt/G]를 구성하여 그 기하적 의미를 다룬다."
...
<ins id="def1">**정의 1**</ins> Site $$\mathcal{C}$$ 위의 CFG들의 사상 $$f:\mathcal{X} \rightarrow \mathcal{Z}$$과 $$g:\mathcal{Y} \rightarrow \mathcal{Z}$$이 주어졌다 하자. 이들의 *2-fiber product<sub>2-올곱</sub>* $$\mathcal{X}\times_\mathcal{Z}\mathcal{Y}$$은 다음 CFG이다. $$T\in \mathcal{C}$$ 위의 대상은 삼중쌍 $$(x, y, \alpha)$$로서, $$x\in \mathcal{X}(T)$$, $$y\in \mathcal{Y}(T)$$이고
...
<ins id="def4">**정의 4**</ins> Site $$(\Sch, \mathrm{\acute{e}t})$$ ([§Grothendieck 위상과 site, ⁋예시 8](/ko/math/stacks/grothendieck_topology#ex8)) 위의 sheaf $$F:\Sch^\op \rightarrow \Set$$이 *algebraic space<sub>대수적 공간</sub>*라는 것은 다음 두 조건을 만족하는 것이다.

1. (표현가능 분리 대각선) 대각선 사상 $$F \rightarrow F\times F$$이 scheme에 의해 표현가능하다. 즉 임의의 scheme $$T$$과 $$T \rightarrow F\times F$$에 대하여 $$F\times_{F\times F}T$$이 scheme이다.
2. (étale atlas) scheme $$U$$과 étale 전사 사상 $$U \rightarrow F$$ (sheaf의 epimorphism이며 표현가능 étale)이 존재한다.

</div>

조건 1은 두 점의 일치 궤적 $$U\times_F U\subseteq U\times U$$이 scheme임을 보장하며, 이로써 algebraic space는 정확히 scheme $$U$$을 étale 동치관계 $$R=U\times_F U\rightrightarrows U$$으로 나눈 몫 $$U/R$$으로 실현된다. 모든 scheme은 algebraic space이고($$U=F$$, 대각선이 immersion), 두 부류는 정규(normal) 또는 quasi-projective 등의 가정 아래 흔히 일치한다. 우리는 algebraic space를 대각선의 표현가능성에서 "scheme보다 한 단계 약한 target"으로 사용할 뿐이며, 그 이론을 본격적으로 전개하지 않는다. 자세한 내용은 [Ols]나 [Stacks]를 참조하라. 이하에서 "표현가능"은 별도 언급이 없는 한 algebraic space에 의한 표현가능성을 뜻한다.

## 대각선의 표현가능성

대수적 stack의 정의에서 대각선이 차지하는 위치를 이해하려면, 대각선의 base change가 무엇인지 먼저 계산해야 한다. Scheme에서 대각선 $$\Delta:X \rightarrow X\times_S X$$을 두 사상 $$a, b: T \rightarrow X$$을 따라 base change하면 둘이 일치하는 궤적 $$\operatorname{Eq}(a, b)$$이 나온다. Stack에서는 일치 대신 isomorphism을 기억하므로, 그 자리에 $$\operatorname{\underline{Isom}}$$이 등장한다.

<div class="proposition" markdown="1">

<ins id="prop5">**명제 5**</ins> $$\mathcal{X}$$이 base scheme $$S$$ 위의 stack이라 하자. 다음이 성립한다.
...
이 있다. 따라서 $$\Delta$$이 표현가능한 것은 모든 $$\operatorname{\underline{Isom}}_T(x, y)$$이 algebraic space에 의해 표현가능한 것과 동치이다.

2. $$\Delta$$이 표현가능하면, scheme $$T$$로부터의 임의의 사상 $$T \rightarrow \mathcal{X}$$이 표현가능하다.

</div>
<details class="proof" markdown="1">
<summary>증명</summary>

(1) [정의 1](#def1)을 적용한다. $$\mathcal{X}\times_{\mathcal{X}\times_S \mathcal{X}}T$$의 $$T'$$-점은 $$\xi\in \mathcal{X}(T')$$, $$T'$$-점 $$t: T' \rightarrow T$$, 그리고 $$\mathcal{X}\times_S \mathcal{X}$$에서의 isomorphism $$\Delta(\xi)=(\xi, \xi)\xrightarrow{\sim}(x, y)\vert_{T'}=(x\vert_{T'}, y\vert_{T'})$$의 자료이다. 마지막 isomorphism은 두 성분의 isomorphism쌍 $$\xi\xrightarrow{\sim}x\vert_{T'}$$과 $$\xi\xrightarrow{\sim}y\vert_{T'}$$이며, 둘을 합성하면 $$x\vert_{T'}\xrightarrow{\sim}y\vert_{T'}$$, 곧 $$\operatorname{\underline{Isom}}_T(x, y)(T'\xrightarrow{t}T)$$의 원소를 얻는다. 역으로 그러한 isomorphism $$\beta$$이 주어지면 $$\xi=x\vert_{T'}$$과 쌍 $$(\id, \beta)$$을 취해 위 자료를 복원한다. 이 대응이 함자적이고 가역이므로 두 CFG가 동치이다. 따라서 $$\Delta$$의 모든 base change가 $$\operatorname{\underline{Isom}}$$이며, $$\Delta$$의 표현가능성은 모든 $$\operatorname{\underline{Isom}}_T(x, y)$$의 표현가능성과 같다.

(2) 사상 $$T \rightarrow \mathcal{X}$$과 임의의 scheme $$T'$$로부터의 사상 $$T' \rightarrow \mathcal{X}$$이 주어졌다 하자. $$T\times_\mathcal{X}T'$$이 algebraic space임을 보이면 된다. 곱 $$T\times_S T' \rightarrow \mathcal{X}\times_S \mathcal{X}$$을 두 점 $$(x, y)$$ (단, $$x$$은 $$T \rightarrow \mathcal{X}$$의 $$T\times_S T'$$로의 제한, $$y$$은 $$T'$$ 쪽의 제한)으로 보면, 표준적인 graph 동형

$$T\times_\mathcal{X}T'\cong(T\times_S T')\times_{\mathcal{X}\times_S \mathcal{X}, \Delta}\mathcal{X}$$

이 성립한다. 우변은 $$\Delta$$을 사상 $$T\times_S T' \rightarrow \mathcal{X}\times_S \mathcal{X}$$을 따라 base change한 것이고, $$T\times_S T'$$이 scheme이므로 (1)에 의해 표현가능, 곧 algebraic space이다. 따라서 $$T\times_\mathcal{X}T'$$이 algebraic space이고 $$T \rightarrow \mathcal{X}$$이 표현가능하다.

</details>

[명제 5](#prop5)이 대각선 조건의 기하학적 의미를 밝힌다. 대각선이 표현가능하다는 것은 두 대상 $$x, y$$ 사이의 isomorphism들이 algebraic space를 이룬다는 것, 곧 두 점의 비교가 통상적인 기하의 범위에서 일어난다는 것이다. 특히 한 대상의 automorphism들 $$\operatorname{\underline{Aut}}_T(x)=\operatorname{\underline{Isom}}_T(x, x)$$이 $$T$$ 위의 group algebraic space가 되며, 이것이 stack의 점에 붙은 *stabilizer*(또는 *inertia*)이다. 둘째 항은 대각선이 표현가능하기만 하면 scheme(나아가 algebraic space)으로부터의 모든 사상이 자동으로 표현가능함을 말한다. 따라서 다음 절에서 atlas $$U \rightarrow \mathcal{X}$$의 매끄러움·전사성을 논할 때 그 사상이 표현가능함을 따로 요구할 필요가 없다.
...
<ins id="def6">**정의 6**</ins> Site $$(\Sch, \mathrm{fppf})$$ 위의 stack $$\mathcal{X}$$ (base scheme $$S$$ 위)이 *대수적 stack* 또는 *Artin stack<sub>아틴 stack</sub>*이라는 것은 다음 두 조건을 만족하는 것이다.
...
<ins id="rmk7">**참고 7**</ins> 대수적 stack $$\mathcal{X}$$에 대하여 다음 조건들이 동치임이 알려져 있다 (증명은 [\[스킴\] §매끄러운 사상과 étale 사상, ⁋정리 11](/ko/math/scheme_theory/smooth_and_etale_morphisms#thm11)의 무한소 lifting 판정과 group scheme의 매끄러움 이론을 쓰며, 자세한 논증은 [Ols] 또는 [LMB]를 참조하라).
...
<ins id="def8">**정의 8**</ins> 위의 자료에 대하여 *quotient stack<sub>몫 stack</sub>* $$[X/G]$$은 다음 CFG이다. $$T\in \Sch/S$$ 위의 대상은 쌍 $$(P, \varphi)$$로서
...
<ins id="prop9">**명제 9**</ins> 사상 $$\pi: X \rightarrow [X/G]$$을, $$T$$-점 $$a\in X(T)$$에 자명한 torsor와 그것이 결정하는 equivariant 사상을 대응시키는 것으로 정의하면, 곧

$$\pi(a)=\bigl(G\times_S T,\ \varphi_a\bigr),\qquad \varphi_a(g, t)=g\cdot a(t),$$

이는 stack의 사상이며 sheaf의 epimorphism, 곧 전사이다.

</div>
<details class="proof" markdown="1">
<summary>증명</summary>

$$\pi$$이 함자적임은 $$a$$의 base change가 자명 torsor의 base change와 호환됨에서 따른다. $$\pi$$이 전사임을 본다. 임의의 $$(P, \varphi)\in [X/G](T)$$에 대하여, $$P$$은 $$G$$-torsor이므로 fppf 덮개 $$\{T_i \rightarrow T\}$$ 위에서 자명해진다. ([§Fibered category와 stack, ⁋정의 18](/ko/math/stacks/fibered_categories_and_stacks#def18)의 국소 비공 조건) 곧 각 $$T_i$$ 위에서 절단 $$s_i\in P(T_i)$$이 존재하여 $$g\mapsto g\cdot s_i$$이 $$G\times_S T_i\xrightarrow{\sim}P\vert_{T_i}$$을 준다. 이 자명화 아래 $$(P, \varphi)\vert_{T_i}$$은 $$a_i:=\varphi(s_i)\in X(T_i)$$이 결정하는 $$\pi(a_i)$$과 동형이다. 따라서 $$(P, \varphi)$$은 덮개 $$\{T_i \rightarrow T\}$$ 위에서 $$\pi$$의 상에 국소적으로 들어가며, 이는 sheaf의 epimorphism의 정의 그대로이다. 그러므로 $$\pi$$은 전사이다.
...
<ins id="prop10">**명제 10**</ins> [명제 9](#prop9)의 atlas $$\pi: X \rightarrow [X/G]$$에 대하여 표준적인 동형
...
을 요구하고, $$h=e$$을 대입하면 $$g\cdot b(t)=a(t)$$, 곧 $$a=g\cdot b$$(작용 $$\sigma$$의 의미)과 동치이다 (역으로 이 등식이면 모든 $$h$$에 대해 성립한다). 그러므로 $$(a, b, \psi)$$은 쌍 $$(g, b)\in(G\times_S X)(T)$$($$a=g\cdot b$$으로 결정됨)과 일대일로 대응하며, 이 대응은 $$T$$에 대해 함자적이고 사상과 호환된다. 따라서 $$X\times_{[X/G]}X\cong G\times_S X$$이다.

이 동형 아래 $$\operatorname{pr}_2(a, b, \psi)=b$$은 $$(g, b)\mapsto b$$, 곧 사영이고, $$\operatorname{pr}_1(a, b, \psi)=a=g\cdot b$$은 $$(g, b)\mapsto g\cdot b$$, 곧 작용 $$\sigma$$이다. 두 사상 $$\sigma, \operatorname{pr}_2: G\times_S X\rightrightarrows X$$이 $$[X/G]$$의 groupoid presentation을 이룬다.
...
<ins id="thm11">**정리 11**</ins> $$G$$이 base scheme $$S$$ 위의 평탄·분리·smooth group scheme이고 $$X$$이 $$S$$-scheme에 $$G$$의 작용을 받는다 하자. 그럼 quotient stack $$[X/G]$$은 대수적 stack이며, [명제 9](#prop9)의 $$\pi: X \rightarrow [X/G]$$이 atlas이다. 나아가 다음이 성립한다.
...
가장 기본적인 예는 $$X=S$$인 분류 stack이다. 이는 [§Fibered category와 stack, ⁋정의 18](/ko/math/stacks/fibered_categories_and_stacks#def18)에서 도입한 $$\mathbf{B}G$$이 사실 대수적 stack임을 [정리 11](#thm11)로 확인하는 것이다.

<div class="example" markdown="1">

<ins id="ex12">**예시 12** ($$\mathbf{B}\mathbb{G}_m$$과 $$\mathbf{B}(\mathbb{Z}/n)$$)</ins> Base를 field $$k$$로 둔다.

1. $$\mathbf{B}\mathbb{G}_m=[\operatorname{Spec}k/\mathbb{G}_m]$$은 대수적 stack이다. $$\mathbb{G}_m$$이 affine·smooth하므로 ([\[스킴\] §점함자, ⁋예시 15](/ko/math/scheme_theory/functor_of_points#ex15)) [정리 11](#thm11)이 적용되고, atlas는 $$\operatorname{Spec}k \rightarrow \mathbf{B}\mathbb{G}_m$$이며 그 base change는 $$\mathbb{G}_m \rightrightarrows \operatorname{Spec}k$$이다. $$\mathbf{B}\mathbb{G}_m(T)$$은 $$T$$ 위의 line bundle들의 groupoid이고 ([§Fibered category와 stack, ⁋정리 19](/ko/math/stacks/fibered_categories_and_stacks#thm19)), 한 점의 stabilizer는 $$\mathbb{G}_m$$이다. Stabilizer가 $$1$$차원이라 무한소 변형을 가지므로 $$\mathbf{B}\mathbb{G}_m$$은 DM이 아닌 Artin stack이며, 그 차원은 $$\dim \operatorname{Spec}k-\dim \mathbb{G}_m=0-1=-1$$이다. 음의 차원은 stabilizer가 점보다 "더 큰" 대수적 stack의 특징이다.

2. $$\mathbb{Z}/n$$을 상수 group scheme으로 볼 때 $$\mathbf{B}(\mathbb{Z}/n)=[\operatorname{Spec}k/(\mathbb{Z}/n)]$$은 DM stack이다. $$\mathbb{Z}/n$$은 유한 étale하므로 ([\[스킴\] §매끄러운 사상과 étale 사상, ⁋예시 9](/ko/math/scheme_theory/smooth_and_etale_morphisms#ex9)에서 분리 확대가 étale함과 같은 이유로 상수군은 étale하다) atlas $$\operatorname{Spec}k \rightarrow \mathbf{B}(\mathbb{Z}/n)$$이 étale 전사이고 ([정리 11](#thm11)의 DM 판정), 그 base change는 $$n$$개의 점의 disjoint union $$\mathbb{Z}/n\times \operatorname{Spec}k\rightrightarrows \operatorname{Spec}k$$이다. $$\mathbf{B}(\mathbb{Z}/n)(T)$$은 $$T$$ 위의 $$\mathbb{Z}/n$$-torsor, 곧 차수 $$n$$의 cyclic étale 덮개의 groupoid이며, 한 점의 stabilizer는 유한군 $$\mathbb{Z}/n$$이다. $$\operatorname{char}k\nmid n$$이면 $$\mu_n$$ 또한 유한 étale하여 $$\mathbf{B}(\mathbb{Z}/n)$$과 $$\mathbf{B}\mu_n$$이 모두 DM이지만, $$\operatorname{char}k\mid n$$이면 $$\mu_n$$이 비환원이 되어 $$\mathbf{B}\mu_n$$은 (DM이 아닌) Artin stack이 되는 반면 상수군 $$\mathbf{B}(\mathbb{Z}/n)$$은 여전히 DM이다. 이 분리가 characteristic $$p$$에서 étale·infinitesimal stabilizer의 차이를 드러낸다.
...
<ins id="ex13">**예시 13** ($$[\mathbb{A}^1/\mathbb{G}_m]$$)</ins> Field $$k$$ 위에서 $$\mathbb{G}_m$$이 affine line $$\mathbb{A}^1$$에 스칼라 배 $$t\cdot x=tx$$으로 작용한다 하자. 작용의 궤도는 둘뿐이다. 원점 $$\{0\}$$은 고정점이라 한 궤도를 이루고, 여집합 $$\mathbb{A}^1\setminus\{0\}=\mathbb{G}_m$$은 $$\mathbb{G}_m$$이 단순추이적으로 작용하는 한 궤도이다. 따라서 순진한 몫 $$\mathbb{A}^1/\mathbb{G}_m$$은 위상적으로 두 점(열린 궤도와 원점)으로 이루어지나, 환의 불변량으로 계산하면 $$\Gamma(\mathbb{A}^1, \mathcal{O})^{\mathbb{G}_m}=k[\x]^{\mathbb{G}_m}=k$$이라 GIT 몫은 한 점 $$\operatorname{Spec}k$$으로 뭉개진다.

Quotient stack $$[\mathbb{A}^1/\mathbb{G}_m]$$은 이 두 궤도의 서로 다른 stabilizer를 모두 기억한다. [정리 11](#thm11)에 의해 이는 $$1$$차원 affine scheme $$\mathbb{A}^1$$을 $$1$$차원 group $$\mathbb{G}_m$$으로 나눈 대수적 stack으로, $$\dim[\mathbb{A}^1/\mathbb{G}_m]=1-1=0$$이다. 그 점은 정확히 두 개이다.

1. 열린 점: 열린 궤도 $$\mathbb{G}_m\hookrightarrow \mathbb{A}^1$$ 위에서 작용이 자유로우므로 $$[\mathbb{G}_m/\mathbb{G}_m]\cong \operatorname{Spec}k$$이고, 이는 stabilizer가 자명한 열린 점 $$\operatorname{Spec}k\hookrightarrow [\mathbb{A}^1/\mathbb{G}_m]$$을 이룬다.

2. 닫힌 점: 원점 $$\{0\}=\operatorname{Spec}k$$은 $$\mathbb{G}_m$$이 자명하게 작용하는 고정점이므로 $$[\{0\}/\mathbb{G}_m]=\mathbf{B}\mathbb{G}_m$$이고, 이는 stabilizer $$\mathbb{G}_m$$을 가진 닫힌 점이다. ([예시 12](#ex12))

즉 $$[\mathbb{A}^1/\mathbb{G}_m]$$은 한 개의 열린 점과, 그 closure에 놓인 $$\mathbf{B}\mathbb{G}_m$$ 한 점으로 이루어진다. Coarse 몫 $$\operatorname{Spec}k$$이 두 점을 하나로 뭉갠 데 반해, stack은 원점의 stabilizer $$\mathbb{G}_m$$을 닫힌 점에 붙은 $$\mathbf{B}\mathbb{G}_m$$으로 정확히 기록한다. Stabilizer가 양의 차원을 가지므로 이 stack은 DM이 아닌 Artin stack이다.
...
<ins id="ex14">**예시 14** (타원곡선의 moduli $$\mathcal{M}_{1, 1}$$)</ins> Base에 따라, $$T$$ 위의 *타원곡선*(절단을 가진 종수 $$1$$의 매끄러운 사영곡선)들의 $$T$$-족과 그 isomorphism이 이루는 CFG를 $$\mathcal{M}_{1, 1}$$로 적는다. 이는 stack이며, characteristic $$0$$(또는 $$2, 3$$을 뒤집은 base) 위에서 Weierstrass 방정식 $$y^2=x^3+ax+b$$의 계수 $$(a, b)$$ ($$\Delta=-16(4a^3+27b^2)\neq 0$$)에 좌표변환군이 작용하는 quotient
