...
을 주어 pullback과 호환되게 하는 자료이다. 두 stack morphism 사이의 2-morphism은 이 functor들을 잇고 pullback coherence와 호환되는 natural transformation이다. CFG의 언어에서는 각각 base 위의 functor와 natural transformation으로 표현된다. ([§스택, ⁋정의 7](/ko/math/stacks/fibered_categories_and_stacks#def7){: data-lid="55se2" })
...
이를 위해 어떠한 데이터가 필요한지 살펴보자. 두 stack morphism $f: \mathcal{X}\rightarrow \mathcal{Z}$와 $g:\mathcal{Y}\rightarrow \mathcal{Z}$에 대하여, [\[스킴\] §점함자, ⁋명제 7](/ko/math/scheme_theory/functor_of_points#prop7){: data-lid="xo82q" }을 생각하면 stack morphism의 fiber product를 정의하기 위해서는 각 $T$마다 fiber product를

$$\mathcal{X}(T)\times_{\mathcal{Z}(T)}\mathcal{Y}(T)$$

로 정의해주면 된다. Scheme에서의 fiber product를 정의할 때는 $X(T)\times_{Z(T)}Y(T)$를, <em-ko>집합</em-ko> $Z(T)$ 안에서 $f_T(x)=g_T(y)$를 만족하는 원소들로 잡았으나, 이제 $\mathcal{Z}(T)$가 groupoid인 이상 이 조건을 isomorphism으로 내려야 한다. 이는 [§스택, ⁋명제 6](/ko/math/stacks/fibered_categories_and_stacks#prop6){: data-lid="w7g1z" } 직후에 살펴본 상황과 정확히 동일한 상황으로, 점 $x\in \mathcal{X}(T)$를 groupoid 사이의 functor $x:\ast\rightarrow \mathcal{X}(T)$로 본다면 $f_T(x)$와 $g_T(y)$는 두 groupoid 사이의 functor
...
::: 정의 3
Site $(\Sch, \et)$ ([§그로텐디크 위상, ⁋예시 8](/ko/math/stacks/grothendieck_topology#ex8){: data-lid="55zg5" }) 위의 sheaf $F:\Sch^\op \rightarrow \Set$이 *algebraic space<sub>대수적 공간</sub>*라는 것은 다음 두 조건을 만족하는 것이다.
...
그럼 étale atlas $p: U\rightarrow F$는 다음과 같은 방식으로 동치관계를 준다. 우선 두 개의 $p: U\rightarrow F$를 곱하여 $R=U\times_F U$로 둔다. [정의 3](#def3){: data-lid="dekbe" }의 첫째 조건에 의해 $R$은 scheme으로서 존재한다. 그럼 명시적으로
...
:::
::: 증명
우선 [정의 1](#def1){: data-lid="kuwvc" }을 적용하면 $\mathcal{X}\times_{\mathcal{X}\times_S\mathcal{X}}T$의 $T'$-object는 $S$-morphism들
...
더 일반적으로, 조건 2에서 étale morphism 대신 smooth morphism을 허용하여 얻어지는 stack을 *algebraic stack* 또는 *Artin stack<sub>아틴 스택</sub>*이라 부른다. ([\[스킴\] §매끄러운 사상과 에탈 사상, ⁋정의 11](/ko/math/scheme_theory/smooth_and_etale_morphisms#def11){: data-lid="7k2uu" })
:::
...
2. Diagonal morphism $\Delta:\mathcal{X} \rightarrow \mathcal{X}\times_S \mathcal{X}$이 unramified하다. ([\[스킴\] §매끄러운 사상과 에탈 사상, ⁋정의 9](/ko/math/scheme_theory/smooth_and_etale_morphisms#def9){: data-lid="uwrq2" })
...
우리는 앞선 섹션의 [정의 6](#def6){: data-lid="mdfvw" }에서 DM stack을 정의하고, 이것이 algebraic space와 갖는 공통점을 강조했으나, 이것을 실제로 stack으로 만드는 중요한 차이, 즉 $\Grpd$-valued functor라는 사실은 아직 충분히 짚고 넘어가지 않았다. 이 차이가 가장 명확하게 드러나는 것은 étale atlas이다. 정의에 의해 algebraic space 혹은 DM stack의 atlas는 scheme $U$에서 해당 대상으로 가는 surjective morphism이다. 그럼 이 morphism이 식별하는 $U$의 point들을 같은 것으로 취급하여 $U$의 quotient를 생각하면 해당 algebraic space 혹은 DM stack을 복원해야 할 것이다.

차이는 이 <em-ko>같은 것으로 취급</em-ko>하는 부분에 있다. Algebraic space는 $\Set$-valued functor로서 두 point가 같은 것이 정말로 같은 것이지만, DM stack은 $\Grpd$-valued functor로서 두 point를 잇는 isomorphism까지 기억한다. 이를 더 명시적으로 쓰자면, 우리는 이미 [정의 3](#def3){: data-lid="2o0oq" } 직후에서 algebraic space에서는
...
1. $P\rightarrow T$는 $G$-torsor이고 ([§스택, ⁋정의 17](/ko/math/stacks/fibered_categories_and_stacks#def17){: data-lid="w2hv2" })
...
그럼 위의 조건을 만족하는 $\psi$는 자동으로 isomorphism으로, 이는 $G$-torsor의 $G$-equivariant morphism은 본질적으로 평행이동 뿐이기 때문이다. 특히 각 fiber는 groupoid이다. 또 base change로 torsor와 equivariant morphism을 pullback하면 cartesian morphism이 주어지므로, 이 fiber groupoid들은 $\Sch_{/S}$ 위의 CFG를 이룬다. 특별한 경우로 $G$가 base space $U=S$에 trivial action으로 작용하는 경우 $[S/G]$는 이미 [§스택, ⁋정의 17](/ko/math/stacks/fibered_categories_and_stacks#def17){: data-lid="u47sy" }에서 살펴본 classifying stack $\bB G$이 된다. 

직관적으로, $[U/G]$의 $T$-point $(P,\varphi)$는 $T$ 위에서 $G$-torsor $P$로 뒤틀린, 각 fiber가 $U$인 공간 $P\times^G U\rightarrow T$의 section을 주는 것과 같다. ([\[대수적 위상수학\] §분류공간, ⁋정의 3](/ko/math/algebraic_topology/classifying_spaces#def3){: data-lid="b97p8" }) Torsor가 trivial하여 $P=G\times_ST$이면 $\varphi(g,t)=g\cdot\varphi(e,t)$이므로, $\varphi$는 identity section $e:T\rightarrow G\times_ST$에서의 값 $a=\varphi\circ e:T\rightarrow U$로 유일하게 결정되므로, trivial torsor 위에서 이를 보는 것은 $U(T)$의 point와 같은 정보를 복원한다.

한편 [§스택, ⁋정리 18](/ko/math/stacks/fibered_categories_and_stacks#thm18){: data-lid="qr2cu" }에서 $\bB\mathbb{G}_m$에 적용한 descent argument와 마찬가지로, torsor와 equivariant morphism은 fppf covering에 대한 effective descent를 만족하며, [명제 2](#prop2){: data-lid="ejx4j" }의 성분별 descent argument가 그대로 적용되므로 $[U/G]$는 stack이다. 이제 이 stack이 algebraic stack임을 보이기 위해 atlas를 구성한다.
...
:::
::: 증명
$\pi$의 functoriality는 $u$의 base change가 trivial torsor의 base change와 호환되므로 얻어진다. 이제 이것이 epimorphism이라는 것을 보이기 위해서는 임의의 $(P,\varphi)\in [U/G](T)$가 국소적으로 이러한 방식으로 얻어진다는 것을 보여야 하는데, 임의의 $G$-torsor는 fppf-locally trivial이므로 ([§스택, ⁋정의 17](/ko/math/stacks/fibered_categories_and_stacks#def17){: data-lid="fby0l" }) trivializing fppf covering $\{T_i\rightarrow T\}$와 각 $T_i$ 위의 section $s_i\in P(T_i)$를 잡을 수 있다. 그럼 이 위로 $(P,\varphi)$를 제한한 $(P,\varphi)\vert_{T_i}$는 $u_i=\varphi(s_i)\in U(T_i)$가 결정하는 $\pi(u_i)$와 isomorphic하고, 따라서 $\pi$가 epimorphism이다.
:::

[명제 8](#prop8){: data-lid="qws4n" }의 $\pi$는 $U$의 point에 trivial torsor를 붙여 $G$-action의 quotient로 보내는 morphism이다. 따라서 위에서 생각한 quotient map 자체이며, epimorphism이므로 atlas의 자연스러운 후보이다. 남은 일은 $\pi$가 representable하고 smooth함을 보이는 것이다. 먼저 $\pi$ 자신을 따른 base change를 계산하면 quotient에서 식별 방법을 기록하는 action groupoid가 다시 드러난다.

::: 명제 9
[명제 8](#prop8){: data-lid="aa4lj" }의 morphism $\pi:U\rightarrow[U/G]$에 대하여, 다음 diagram
...
로 주어진다. 한편, [명제 8](#prop8){: data-lid="tyd1d" }의 $\pi$의 정의에서
...
이므로 $\psi=\psi_\gamma$이다. 즉 trivial torsor의 $G$-equivariant automorphism은 정확히 right translation들이다. ([§스택, ⁋정의 17](/ko/math/stacks/fibered_categories_and_stacks#def17){: data-lid="0ezbz" }) 이제 남은 조건 $\varphi_{u_2}\circ\psi_\gamma=\varphi_{u_1}$은 모든 $(h,t)$에 대하여
...
[명제 9](#prop9){: data-lid="kkrgc" }는 $[U/G]$가 *action groupoid* $G\times_SU\rightrightarrows U$의 stack quotient라는 것으로 요약할 수 있다. 더 일반적으로, source와 target이 smooth한 groupoid object $R\rightrightarrows U$로부터 algebraic stack $[U/R]$을 얻을 수 있으며, quotient stack은 $R=G\times_SU$인 특수한 경우이다.

이제 우리는 이렇게 정의한 quotient stack이 적절한 조건을 만족하면 도입부에서의 직관대로 DM stack이 된다는 것을 보인다. 

::: 정리 10
$S$-scheme $U$와 그 위에 작용하는 smooth group $S$-scheme $G$가 주어졌다 하자. 그럼 quotient stack $[U/G]$는 algebraic stack이며, [명제 8](#prop8){: data-lid="bi7io" }의 $\pi:U\rightarrow[U/G]$가 smooth atlas이다. 나아가 diagonal $\Delta:[U/G]\rightarrow[U/G]\times_S[U/G]$를 $\pi\times\pi$를 따라 base change하면 [명제 9](#prop9){: data-lid="36ck6" }의 두 projection이 정의하는 action morphism
...
:::
::: 증명
우선 공통된 주장부터 보이자. $[U/G]$가 algebraic stack임을 보이기 위해서는 representability와 smooth atlas를 보여야 한다. ([정의 6](#def6){: data-lid="ppm4x" }) 

먼저 diagonal이 representable함을 보이자. [명제 5](#prop5){: data-lid="u672r" }에 의해 임의의 $S$-scheme $T$와 $(P,\varphi),(P',\varphi')\in[U/G](T)$에 대하여 $\rIsom_T((P,\varphi),(P',\varphi'))$이 algebraic space임을 보이면 충분하다. 이들을 모두 trivialize하는 fppf covering $\{T_i\rightarrow T\}$을 잡으면, [명제 8](#prop8){: data-lid="jyd9h" }의 증명에서와 같이 각 $T_i$ 위에서 두 object는 $\pi(u_i),\pi(u_i')$의 꼴이고, [명제 9](#prop9){: data-lid="vohd7" }의 계산에 의해 임의의 $V\rightarrow T_i$에 대하여
...
이제 $\pi$가 smooth atlas임을 보인다. $T\rightarrow[U/G]$가 object $(P,\varphi)$에 의해 주어지면 canonical isomorphism $U\times_{[U/G]}T\cong P$에 의해 $\pi$의 임의의 base change는 $G$-torsor $P\rightarrow T$임을 안다. Torsor는 fppf-locally $G\times_ST$이므로 이는 algebraic space이고, $G$가 smooth이므로 $P\rightarrow T$도 smooth하다. [명제 8](#prop8){: data-lid="239as" }에서 $\pi$가 epimorphism임을 보였으므로 $\pi$는 smooth atlas이고, 따라서 $[U/G]$는 algebraic stack이다.

이제 공통 주장에는 diagonal의 base change 계산이 남았다. [명제 5](#prop5){: data-lid="8iz8z" }를 $T=U\times_SU$와 두 object $\pi\circ\pr_1$, $\pi\circ\pr_2$에 적용하면 $\Delta$를 $\pi\times\pi$를 따라 base change한 것이 $U\times_{[U/G]}U$이므로, [명제 9](#prop9){: data-lid="d1gro" }에 의해 이는 $G\times_SU$이고 $U\times_SU$로 가는 morphism은 두 projection이 주는 $(\rho,\pr_2)=a$이다. 한편 $\pi$가 smooth epimorphism이므로 $\pi\times\pi$ 또한 그러하고, separated·proper·unramified는 모두 base change에 대하여 닫혀 있으면서 fppf-local on target이므로 ([정의 4](#def4){: data-lid="8nms3" }), $\Delta$가 이들 성질을 갖는 것은 $a$가 그러한 것과 동치이다.
...
2. [정의 6](#def6){: data-lid="pq45j" } 직후에 살펴본 동치조건에 의해 $[U/G]$가 DM stack인 것은 $\Delta$가 unramified인 것과 동치이며, 위의 판정에 의해 이는 $a$가 unramified인 것과 동치이다. 그런데 geometric point $(u_1,u_2)\in(U\times_SU)(\mathbb{K})$ 위의 $a$의 fiber는
    
    $$\{g\in G_\mathbb{K}\mid g\cdot u_2=u_1\}$$
    
    이므로 두 점이 서로 다른 orbit에 있으면 empty이고, 그렇지 않으면 stabilizer $\rAut_\mathbb{K}(u_2)$의 torsor이다. 한편 $\pr_2\circ a=\pr_2$가 smooth이므로 locally of finite type이고, 합성이 locally of finite type이면 첫 morphism도 그러하므로 $a$ 또한 locally of finite type이다. 이제 unramifiedness는 locally of finite type인 morphism의 cotangent sheaf가 소멸하는 것이고, [\[스킴\] §미분과 여접층, ⁋명제 5](/ko/math/scheme_theory/sheaf_of_differentials#prop5){: data-lid="kr6oy" }에 의해 cotangent sheaf는 base change와 commute하므로 $\Omega_a$의 fiber로의 restriction이 그 fiber의 cotangent sheaf이다. 이때 $\Omega_a$가 finite type이므로 [\[가환대수학\] §정수적 확장, ⁋보조정리 8](/ko/math/commutative_algebra/integral_extension#lem8){: data-lid="7n9rp" }에 의해 각 점에서의 소멸이 stalk의 소멸을 주고, 따라서 모든 stabilizer가 unramified이면 $a$가 unramified이다. 마지막으로 $G$가 étale이면 위에서 본 base change $P\rightarrow T$가 étale이므로 $\pi$ 자체가 [정의 6](#def6){: data-lid="su3hc" }의 étale atlas이다.
:::
...
그럼 [정의 7](#def7){: data-lid="55vpi" } 직후에 살펴본 $U=S$인 경우, 곧 classifying stack에 [정리 10](#thm10){: data-lid="s5aj6" }을 적용하여 atlas와 stabilizer를 직접 읽어보자.

::: 예시 11 ($\bB\mathbb{G}_m$과 $\bB(\mathbb{Z}/n)$)
Base scheme을 field $\mathbb{K}$로 고정하자.

1. $\bB\mathbb{G}_m=[\Spec\mathbb{K}/\mathbb{G}_m]$은 algebraic stack이다. $\mathbb{G}_m$이 affine·smooth이므로 ([\[스킴\] §군 스킴, §§군 스킴](/ko/math/scheme_theory/group_schemes#군-스킴){: data-lid="1x7hf" }) [정리 10](#thm10){: data-lid="r6won" }이 적용되고, atlas는 $\Spec\mathbb{K}\rightarrow\bB\mathbb{G}_m$이며 그 base change는 $\mathbb{G}_m\rightrightarrows\Spec\mathbb{K}$이다. $\bB\mathbb{G}_m(T)$는 $T$ 위의 line bundle들의 groupoid이고 ([§스택, ⁋정리 18](/ko/math/stacks/fibered_categories_and_stacks#thm18){: data-lid="lv37p" }), point의 stabilizer는 $\mathbb{G}_m$이다. Stabilizer가 $1$-dimensional이므로 $\bB\mathbb{G}_m$은 DM stack이 아닌 Artin stack이며, 그 dimension은 $\dim\Spec\mathbb{K}-\dim\mathbb{G}_m=0-1=-1$이다.
...
2. Origin $\{0\}=\Spec\mathbb{K}$은 $\mathbb{G}_m$이 trivially action하는 fixed point이므로 $[\{0\}/\mathbb{G}_m]=\bB\mathbb{G}_m$이고, 이는 stabilizer $\mathbb{G}_m$을 갖는 closed point이다. ([예시 11](#ex11){: data-lid="emx80" })
