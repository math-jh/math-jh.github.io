...
::: 명제 5
[정의 4](#def4)의 가정 아래 축약공간 $$M /\!\!/_{\!\xi}\,G=\mu^{-1}(\xi)/G_\xi$$은 자연스러운 사교형식을 갖는 매끄러운 다양체이며, 그 차원은
...
::: 예시 6
$$M=\mathbb{C}^{n+1}$$에 좌표 $$z_j=x_j+iy_j$$ ($$j=0,1,\ldots,n$$)와 사교형식 $$\omega=\sum_{j=0}^n dx_j\wedge dy_j$$을 주자 ([§운동량 사상, ⁋예시 8](/ko/math/symplectic_geometry/moment_map#ex8)). $$G=S^1$$이 모든 좌표에 동시에 작용하는 *대각* 회전
