...
[명제 3](#prop3){: data-lid="3rbox" data-relation="required" }은 J-holomorphic curve 이론에서 핵심적인 출발점이다. $J$를 자유롭게 *섭동*할 수 있다는 사실 덕분에, J-holomorphic curve의 moduli space를 generic $J$에 대해 잘 통제된 manifold (또는 orbifold) 로 만들 수 있다.
...
[명제 6](#prop6){: data-lid="xsmp2" data-relation="required" }의 직접적 결과는, 고정된 homology class $\beta$를 represent하는 J-holomorphic curve들은 모두 *uniformly bounded energy*를 가진다는 것이다. 이 사실이 moduli space의 compactness 분석의 출발점이 된다.
...
[명제 7](#prop7){: data-lid="dlj48" data-relation="required" }은 J-holomorphic curve의 *bubble* 형성 시 중요한 역할을 한다. Sequence of J-holomorphic curves가 한 점으로 energy를 집중시키면, *rescaling*을 통해 $\mathbb{C} \rightarrow M$의 J-holomorphic map을 얻고, [명제 7](#prop7){: data-lid="h2mxk" data-relation="required" }에 의해 이는 $\mathbb{P}^1 = \mathbb{C} \cup \{\infty\} \rightarrow M$의 J-holomorphic *sphere*로 확장된다. 이 sphere를 *bubble*이라 부른다.
...
J-holomorphic curve의 가장 영향력 있는 응용은 [§Gromov-Witten 불변량](/ko/math/symplectic_geometry/gromov_witten){: data-lid="yci29" data-relation="forward" }이다. 거기서 우리는 stable map의 moduli space 위에 정의된 *virtual fundamental class*에 cohomology class를 evaluate하여 enumerative invariant를 얻는다. 이로부터 *quantum cohomology* 구조가 유도된다.
