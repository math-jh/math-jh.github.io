...
증명은 mean value inequality와 isoperimetric inequality를 사용하는 비교적 표준적인 기술적 결과이다 ([MS Chapter 4]).
...
## 응용: Gromov-Witten 이론과 mirror symmetry

J-holomorphic curve의 가장 영향력 있는 응용은 [§Gromov-Witten 불변량](/ko/math/symplectic_geometry/gromov_witten)이다. 거기서 우리는 stable map의 moduli space 위에 정의된 *virtual fundamental class*에 cohomology class를 evaluate하여 enumerative invariant를 얻는다. 이 invariant들이 모여 만드는 *quantum cohomology* 구조가 mirror symmetry의 A-model 측 주인공이며, [\[거울 대칭\] §Mirror Symmetry 개요](/ko/math/mirror_symmetry/overview) 이하의 글들이 그 framework을 전개한다.

또 다른 응용은 *Floer homology*이다. 특정 Lagrangian submanifold 사이의 J-holomorphic strip을 세어 만든 chain complex의 homology가 Lagrangian Floer homology이며, 이는 categorical mirror symmetry (homological mirror symmetry of Kontsevich)의 주요 구성요소이다.
