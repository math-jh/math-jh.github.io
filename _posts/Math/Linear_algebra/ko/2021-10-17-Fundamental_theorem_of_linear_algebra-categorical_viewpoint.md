...
$$L\circ L'=\operatorname{id}_W,\qquad L'\circ L=\operatorname{id}_V$$
...
- $\mathcal{C}$의 *대상<sub>object</sub>*들 $X,Y,Z,\ldots$로 이루어진 모임 $\operatorname{obj}(\mathcal{C})$
- 각각의 $X,Y\in \operatorname{obj}(\mathcal{C})$마다 대응되는 *morphism<sub>사상</sub>*들의 모임 $\operatorname{Mor}(X,Y)$
...
- 각각의 $m,n$에 대하여, morphism $\operatorname{Mor}(m,n)$은 $m\times n$ 행렬들의 모임 $\operatorname{Mat}\_{m\times n}(F)$로 주어진다.
- 마지막으로, morphism들의 합성은 행렬의 곱으로 주어진다. 즉 만일 $A\in\operatorname{Mor}(m,n)=\operatorname{Mat}\_{m\times n}(F)$이고, $B\in\operatorname{Mor}(n,k)=\operatorname{Mat}\_{n\times k}(F)$라면 이들의 합성 $(A,B)\mapsto B\circ A$는 단순히 행렬의 곱 $BA\in\operatorname{Mat}\_{m\times k}(F)=\operatorname{Mor}(m,k)$으로 정의된다.
...
1. $F$는 $\mathcal{C}$의 대상을 $\mathcal{D}$의 대상으로 옮긴다. 즉, 임의의 $A\in\operatorname{obj}(\mathcal{C})$에 대하여 $F(A)\in\operatorname{obj}(\mathcal{D})$가 성립한다.
2. $F$는 $\mathcal{C}$의 morphism을 $\mathcal{D}$의 morphism으로 옮긴다. 즉, 임의의 $f:A\rightarrow B$는 $F$에 의해 $F(f): F(A)\rightarrow F(B)$로 옮겨진다.
3. $F$는 합성과 identity morphism을 잘 보존한다. 즉, $F(\operatorname{id}\_A)=\operatorname{id}\_{F(A)}$가 성립하고, 또 임의의 $g\circ f$에 대하여 $F(g\circ f)=F(g)\circ F(f)$이다.
...
1. 임의의 $A,B\in\operatorname{obj}(\mathcal{C})$에 대해 $F_{A,B}$가 injective라면 $F$가 *faithful*이라 한다.
2. 임의의 $A,B\in\operatorname{obj}(\mathcal{C})$에 대해 $F_{A,B}$가 surjective라면 $F$가 *full*이라 한다.
3. 임의의 $A,B\in\operatorname{obj}(\mathcal{C})$에 대해 $F_{A,B}$가 bijective라면 $F$가 *fully faithful*이라 한다.

또, 만일 임의의 $C\in\operatorname{obj}(\mathcal{D})$에 대하여 적당한 $A\in\operatorname{obj}(\mathcal{C})$가 존재하여 $C$와 $F(A)$가 isomorphic한 대상이라면, $F$가 *essentially surjective*라 한다.
...
1. 우선 $\mathbf{FVect}_F$의 대상인 유한차원 $F$-벡터공간 $V$는 $D$에 의하여 $\dim V\in\operatorname{obj}(\mathbf{Mat}_F)$으로 옮겨진다.
2. $\mathbf{FVect}\_F$의 임의의 morphism $L\in\operatorname{Mor}(V,W)$는 $D$에 의하여 $[L]^\mathcal{B}\_\mathcal{C}$로 옮겨진다. 
...
2. $\mathbf{Mat}_F$의 임의의 morphism $A\in\operatorname{Mor}(m,n)$은 $I$를 통해 $L_A:F^m\rightarrow F^n$으로 옮겨진다. ([§선형대수학의 기본정리, 예시 1](/ko/math/linear_algebra/ftla#ex1))
