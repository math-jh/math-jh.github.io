...
뿐만 아니라 이 정의는 우리가 순서쌍들의 모임을 그래프라고 부르는 이유도 보여준다. 고등학교 때까지 배워왔던 함수의 그래프라는 것은 별다른 것이 아니라, 위의 정의에서의 집합
...
이와 반대로 어떠한 함수를 더 작은 정의역으로 제한시킬 수도 있다. $f$가 함수이고 $E\subseteq\operatorname{pr}\_1 f$라 하자. 관계 $R$을 

> $(x\mathrel{R} y)$ if and only if $((x\in E)\wedge(y=f(x)))$

로 정의한다면, 이를 만족하는 $(x,y)$들을 모아둔 $R$의 그래프는 functional이며, 이 그래프의 정의역은 $E$가 된다. 따라서 다음과 같이 새로운 함수를 정의할 수 있다.
