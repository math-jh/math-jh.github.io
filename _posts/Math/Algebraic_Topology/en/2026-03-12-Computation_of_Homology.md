...
Then by [\[Homological Algebra\] §Long Exact Sequence, ⁋Theorem 1](/en/math/homological_algebra/long_exact_sequence#thm1){: data-relation="required" reviewed="" }, we have the following long exact sequence
...
Now from the assumption that $(X,A)$ is a good pair, the inclusions $A\subseteq U\subseteq X$ satisfy the condition $\cl A\subseteq \interior U$ of [Theorem 2](#thm2){: data-relation="required" reviewed="" }, so the inclusion

$$(X\setminus A, U\setminus A)\hookrightarrow (X,U)$$

induces an isomorphism $H_k(X\setminus A, U\setminus A)\rightarrow H_k(X,U)$, and similarly applying [Theorem 2](#thm2){: data-relation="required" reviewed="" } to the inclusions $\{[A]\}\subseteq U/A\subseteq X/A$ shows that the map
...
Then by [\[Homological Algebra\] §Long Exact Sequence, ⁋Proposition 2](/en/math/homological_algebra/long_exact_sequence#prop2){: data-relation="required" reviewed="" } these define the following commutative diagram
...
and apply [\[Homological Algebra\] §Diagram Chasing, ⁋Corollary 2](/en/math/homological_algebra/diagram_chasing#cor2){: data-relation="required" reviewed="" } to the following diagram
...
is an isomorphism. Once this is assumed, first by [§Homology, ⁋Proposition 11](/en/math/algebraic_topology/homology#prop11){: data-relation="required" reviewed="" } we have $H_n^\Delta(X^0)\cong H_n(X^0)$ for all $n$ when $k=1$, so by [\[Homological Algebra\] §Diagram Chasing, ⁋Corollary 2](/en/math/homological_algebra/diagram_chasing#cor2){: data-relation="required" reviewed="" } we can show $H_n^\Delta(X^1)\cong H_n(X^1)$, and then inductively construct the desired isomorphism for larger $k$.
...
Although we have omitted the proof for reasons of length, the excision theorem of [Theorem 2](#thm2){: data-relation="requires-review" } is a powerful tool in homology theory. For example, in the proof of [Theorem 5](#thm5){: data-relation="weak" reviewed="" } we used the excision theorem to ignore simplices of dimension less than $k-1$, which allowed us to use induction with the base step being the homology of a one-point space. This process can be seen as essentially having all the properties that homology should satisfy, and axiomatizing this gives the following.
...
On the other hand, in these homology theories the coefficient group is fixed to be $\mathbb{Z}$, but if we replace it with an arbitrary abelian group $A$, all the axioms of [Definition 6](#def6){: data-relation="requires-review" } still hold unchanged. Indeed, when defining singular or simplicial homology, if we had taken the chain groups
...
we would have obtained this type of homology. ([\[Algebraic Structures\] §Abelian Groups, ⁋Definition 13](/en/math/algebraic_structures/abelian_groups#def13){: data-relation="requires-review" }) In particular, when the coefficient group $A$ is a commutative ring $R$, this corresponds to taking the extension of scalars via the ring homomorphism $\mathbb{Z}\rightarrow R$ ([\[Algebraic Structures\] §Change of Scalars, ⁋Proposition 6](/en/math/algebraic_structures/change_of_base_ring#prop6){: data-relation="requires-review" }), and the resulting chain complex and homology naturally inherit the structure of $R$-modules.
