...
weight: 6
translated_at: 2026-05-25T09:00:02+00:00
translation_source: kimi-cli
---
As its name suggests, cohomology can be thought of as the dual concept to homology. However, if the $k$th cohomology $H^k(X)$ of a space $X$ were simply the dual of the $k$th homology $H_k(X)$, there would be no need to consider it separately.

In fact, cohomology provides a more refined invariant than homology: for instance, there is a natural product structure on cohomology, and even spaces with the same homology may be distinguished as non-homotopic if their product structures differ. In this post, we define cohomology and examine its basic properties.

## The Universal Coefficient Theorem for Homology

Before beginning the main discussion, we first review homology with coefficients, which we covered after [\[Algebraic Topology\] §Computation of Homology, ⁋Definition 6](/en/math/algebraic_topology/computation_of_homology#def6). When defining simplicial or singular homology, we saw that by taking the tensor product with an abelian group $A$ instead of the chain groups $C_\bullet(X)$ or $C_\bullet^\Delta(X)$, we obtain the chain complexes
...
is also a splitting short exact sequence. ([##ref](splitting_tensor_splits)) Writing these out, we obtain a commutative diagram of the form
...
On the other hand, since (1) is a splitting short exact sequence, we can choose a retraction $r_k:C_k \rightarrow Z_k$. ([\[Multilinear Algebra\] §Exact Sequences, ⁋Proposition 10](/en/math/multilinear_algebra/exact_sequences#prop10)) Under this choice, $(p_k\circ r_k)\otimes \id_A$ induces the map $H_k(C;A)\rightarrow H_k(C)\otimes_\mathbb{Z} A$ in homology, and one can verify that this is a retraction of the map $H_k(C)\otimes_\mathbb{Z}A\rightarrow H_k(C;A)$ above. Thus we obtain:
...
To show the existence of a cohomology theory satisfying these conditions, just as in [\[Algebraic Topology\] §Homology](/en/math/algebraic_topology/homology), we consider the chain complex of singular simplices of a topological space $X$
...
By [\[Algebraic Structures\] §Direct Products, Direct Sums, and Tensor Products of Modules, ⁋Theorem 6](/en/math/algebraic_structures/operations_of_modules#thm6), this can be thought of as

$$\qquad \cdots\leftarrow\Hom_A(C_{k+1}(X;A),A)\leftarrow \Hom_A(C_k(X;A),A)\leftarrow \Hom_A(C_{k-1}(X;A),A)\leftarrow\cdots$$

so it can be viewed as the dual of the chain complex $C_\bullet(X;A)$. We then define the $k$th homology of this chain complex $(C^\vee)^\bullet(X;A)$ as

$$H^k(X;A):=H_k(C^\vee)$$

and call it the *$k$th cohomology* of $X$. The reason we use superscripts on $H$ and $C^\vee$ to indicate the index is that, in contrast to homology, the long exact sequence is formed in the direction of increasing index; henceforth, when there is no risk of confusion, we will write $(C^\vee)^\bullet(X)$ as $C^\bullet(X;A)$.

We now examine the relationship between $H^k(X;A)$ and $H_k(X)$ defined in this way. As stated at the beginning of this post, it is not true in general that $H^k(X;A)\cong H_k(X)^\ast$. However, by an argument similar to the proof of [Proposition 1](#prop1) above, we obtain the following proposition.
...
Associating a chain complex $C_\bullet(X)$ to a topological space $X$ can be viewed as algebraically encoding information about the subspaces of $X$. When defining cohomology, we apply $\Hom_\mathbb{Z}(-,A)$ to $C_\bullet(X)$ and then take the homology of the resulting cochain complex; here

$$C^k(X;A)=\Hom_\mathbb{Z}(C_k(X), A)$$

and an arbitrary element of this can be thought of as a function assigning an element of $A$ to each element of $C_k(X)$ (i.e., to each $k$-chain). Thus cohomology is essentially about studying functions defined on a space.
...
and if we denote the boundary map of $C_\bullet(X)$ by $\partial$ and the induced coboundary map of $C^\bullet(X;A)$ by $\delta$, then for any $c\in C_{k+1}(X)$ and $\varphi\in C^k(X;A)$ the identity

$$\langle \partial c, \varphi\rangle=\langle c, \delta\varphi\rangle$$

holds, from which we know that these induce a pairing at the homology and cohomology level
...
A $2$-dimensional subset of $\mathbb{R}^3$ is given by a map from a (unit) rectangle in $\mathbb{R}^2$ into $\mathbb{R}^3$, and through this we know what it means to apply $\omega$ to a $2$-dimensional subset.
...
The de Rham cohomology discussed above is an example of a cohomology theory whose coefficient group is not $\mathbb{Z}$. Unlike singular or simplicial cohomology theory, de Rham cohomology naturally has $\mathbb{R}$ as its coefficient group by definition.
  
Such a cohomology theory has nice properties: for example, since $\mathbb{R}$ is a torsion-free abelian group, $\Tor_1^\mathbb{Z}(A,\mathbb{R})=0$ holds for any abelian group $A$, and therefore by [Proposition 1](#prop1) we have the isomorphism

$$H_k(X;\mathbb{R})\cong H_k(X)\otimes_\mathbb{Z}\mathbb{R}$$

Moreover, since $\mathbb{R}$ is an injective $\mathbb{Z}$-module, $\Ext_\mathbb{Z}^1(A,\mathbb{R})=0$ holds for any abelian group $A$, and thus this time [Proposition 3](#prop3) yields the isomorphism
...
are, if $A$ were a ring, chain complexes of $A$-modules, and the $C^\bullet(X;A)$ defined earlier is likewise. Therefore taking homology or cohomology of these yields $A$-modules as well.

On the other hand, we know that if $A$ is a principal ideal domain, then any submodule of a free $A$-module is again a free $A$-module. Reviewing the proof of [Proposition 1](#prop1), we see that it exploited the fact that since $\mathbb{Z}$ is a principal ideal domain, any submodule of a free $\mathbb{Z}$-module (i.e., a free abelian group) is again a free $\mathbb{Z}$-module. Based on this, we can generalize the preceding two propositions as follows.
...
and the differential on homogeneous elements is defined by
...
Now considering the long exact sequence in homology from this short exact sequence, we obtain
...
The map $(B(C)\otimes H(D))_\bullet \rightarrow (Z(C)\otimes H(D))_\bullet$ is obtained from the inclusion $B_\bullet(C)\rightarrow Z_\bullet(C)$, and under the above identification it is the same map as $\delta_\bullet$. Therefore we obtain
...
Although this looks complicated as a formula, it is merely expressing the way to decompose the prism $\Delta^p\times \Delta^q$ into simplices, i.e., the functions $h_n$ that appeared in the proof of [\[Algebraic Topology\] §Homotopy, ⁋Proposition 6](/en/math/algebraic_topology/homotopy#prop6). Then the result of [Theorem 9](#thm9) follows from the two identities
