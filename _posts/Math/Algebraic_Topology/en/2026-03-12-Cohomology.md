...
and therefore, by [\[Homological Algebra\] §Long Exact Sequence, ⁋Theorem 1](/en/math/homological_algebra/long_exact_sequence#thm1){: data-relation="required" }, the long exact sequence
...
- (Excision) For $(X,A,Z)$ satisfying the conditions of [§Computing Homology, ⁋Theorem 2](/en/math/algebraic_topology/computation_of_homology#thm2){: data-relation="required" }, the inclusion $(X\setminus Z, A\setminus Z)\hookrightarrow(X,A)$ induces an isomorphism.
...
To establish the existence of a cohomology theory satisfying these conditions, just as in [§Homology](/en/math/algebraic_topology/homology){: data-relation="required" }, we consider the chain complex of singular simplices of a topological space $X$,
...
We must now examine the relationship between $H^k(X;A)$ and $H_k(X)$ thus defined. As noted at the outset, it is not the case that $H^k(X;A)\cong H_k(X)^\ast$ in general. However, by an argument similar to the proof of [Proposition 1](#prop1){: data-relation="required" } above, we obtain the following proposition.
...
Roughly speaking, this may be thought of as translating [Proposition 1](#prop1){: data-relation="weak" } via [\[Algebraic Structures\] §Abelian Groups, ⁋Theorem 15](/en/math/algebraic_structures/abelian_groups#thm15){: data-relation="weak" }.
...
Such a cohomology theory enjoys nice properties: for instance, since $\mathbb{R}$ is a torsion-free abelian group, $\Tor_1^\mathbb{Z}(A,\mathbb{R})=0$ holds for any abelian group $A$, and hence by [Proposition 1](#prop1){: data-relation="required" } we have the isomorphism

$$H_k(X;\mathbb{R})\cong H_k(X)\otimes_\mathbb{Z}\mathbb{R}$$

Moreover, since $\mathbb{R}$ is an injective $\mathbb{Z}$-module, $\Ext_\mathbb{Z}^1(A,\mathbb{R})=0$ for any abelian group $A$, and thus this time [Proposition 3](#prop3){: data-relation="required" } gives the isomorphism
...
On the other hand, we know that if $A$ is a principal ideal domain, then any submodule of a free $A$-module is again a free $A$-module. Re-examining the proof of [Proposition 1](#prop1){: data-relation="required" }, we see that it exploited the fact that $\mathbb{Z}$ is a principal ideal domain, so that submodules of free $\mathbb{Z}$-modules (i.e., free abelian groups) are again free $\mathbb{Z}$-modules. Based on this, we may generalize the preceding two propositions as follows.
...
That is, $(C\otimes D)_\bullet$ is the total complex of the double complex whose $(p,q)$-entry is $C_p\otimes D_q$, with horizontal differential $\partial^C\otimes\id_D$ and vertical differential $\id_C\otimes\partial^D$. ([§Homology, ⁋Definition 5](/en/math/homological_algebra/homology#def5){: data-relation="weak" })
...
Since the chain homotopy equivalence of [Theorem 9](#thm9){: data-relation="required" } is between free complexes, it persists after applying $\otimes_\mathbb{Z}A$, and from this we obtain $C_\bullet(X\times Y)\otimes_\mathbb{Z}A\simeq (C(X)\otimes C(Y))_\bullet\otimes_\mathbb{Z}A\cong C_\bullet(X;A)\otimes_AC_\bullet(Y;A)$. Therefore, combining [Lemma 8](#lem8){: data-relation="required" } with [Theorem 9](#thm9){: data-relation="required" }, we obtain the following result.
...
[^1]: Of course, one would hope that this pairing gives a homomorphism from $H^k(X;A)$ to $\Hom(H_k(X),A)$, but as we know from [Proposition 3](#prop3){: data-relation="forward" }, the situation is not so simple and an $\Ext$ term containing hidden torsion must appear.
