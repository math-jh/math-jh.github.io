...
We must now examine the relationship between $H^k(X;A)$ and $H_k(X)$ thus defined. As noted at the outset, it is not the case that $H^k(X;A)\cong H_k(X)^\ast$ in general. However, by an argument similar to the proof of [Proposition 1](#prop1){: data-relation="required" reviewed="" } above, we obtain the following proposition.
...
Such a cohomology theory enjoys nice properties: for instance, since $\mathbb{R}$ is a torsion-free abelian group, $\Tor_1^\mathbb{Z}(A,\mathbb{R})=0$ holds for any abelian group $A$, and hence by [Proposition 1](#prop1){: data-relation="required" reviewed="" } we have the isomorphism
...
On the other hand, we know that if $A$ is a principal ideal domain, then any submodule of a free $A$-module is again a free $A$-module. Re-examining the proof of [Proposition 1](#prop1){: data-relation="required" reviewed="" }, we see that it exploited the fact that $\mathbb{Z}$ is a principal ideal domain, so that submodules of free $\mathbb{Z}$-modules (i.e., free abelian groups) are again free $\mathbb{Z}$-modules. Based on this, we may generalize the preceding two propositions as follows.
