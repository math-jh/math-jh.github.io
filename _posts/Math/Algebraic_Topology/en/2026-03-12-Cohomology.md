...
weight: 6
translated_at: 2026-05-25T16:30:04+00:00
translation_source: kimi-cli
last_polished_at: 2026-05-25T16:30:04+00:00
---
As its name suggests, cohomology can be thought of as the dual concept to homology. Yet if the $k$th cohomology $H^k(X)$ of a space $X$ were simply the dual of the $k$th homology $H_k(X)$, there would be little reason to study it separately.

In fact, cohomology furnishes a more refined invariant than homology: for instance, it carries a natural product structure, and two spaces with the same homology may fail to be homotopic if their product structures differ. In this post we define cohomology and examine its basic properties.

## The Universal Coefficient Theorem for Homology

Before turning to the main discussion, we first review homology with coefficients, which we treated after [\[Algebraic Topology\] §Computation of Homology, ⁋Definition 6](/en/math/algebraic_topology/computation_of_homology#def6). When defining simplicial or singular homology, we observed that by taking the tensor product with an abelian group $A$ in place of the chain groups $C_\bullet(X)$ or $C_\bullet^\Delta(X)$, we obtain the chain complexes
...
as their homology groups. To carry out actual computations, one might ask whether $H_k(X;A)$ or $H_k^\Delta(X;A)$ can be expressed simply as $H_k(X)\otimes_\mathbb{Z}A$ or $H_k^\Delta(X)\otimes_\mathbb{Z}A$; however, since the tensor product is right-exact but not left-exact in general, this would be too much to expect. Instead, we must additionally measure the information lost in passing to the tensor product.
...
Here $Z_k=\ker(\partial:C_k \rightarrow C_{k-1})$ and $B_{k-1}=\im(\partial:C_k \rightarrow C_{k-1})$, the first map is the inclusion, and the second map is the boundary map $\partial$.

Since $Z_k$ and $B_{k-1}$ are subgroups of the free abelian groups $C_k$ and $C_{k-1}$, they are themselves free; hence by [##ref##](third_term_projective_splits) this short exact sequence is split exact, and thus for any abelian group $A$ the sequence
...
is also a split short exact sequence. ([##ref](splitting_tensor_splits)) Expanding these out, we obtain a commutative diagram of the form

![snake_lemma](/assets/images/Math/Algebraic_Topology/Cohomology-1.png){:style="width:32em" class="invert" .align-center}

and therefore by [\[Homological Algebra\] §Long Exact Sequences, ⁋Theorem 1](/en/math/homological_algebra/long_exact_sequence#thm1) we obtain the long exact sequence
...
To extract information from this exact sequence we must examine the connecting maps $\delta$. Tracing through the definitions, $\delta_k:B_k\otimes_\mathbb{Z}A\rightarrow Z_k\otimes_\mathbb{Z}A$ is exactly $-\otimes \id_A$ applied to the inclusion homomorphism $i_k:B_k \rightarrow Z_k$; thus, considering the short exact sequence

$$0 \rightarrow B_k\overset{i_k}{\longrightarrow} Z_k \overset{p_k}{\longrightarrow}H_k(C)\rightarrow 0$$

since $B_k$, $Z_k$, and $0$ are all free abelian groups, this can be viewed as a free resolution of $H_k(C_\bullet)$, and by definition we can fit $\delta_k$ into the exact sequence
...
On the other hand, since (1) is a split short exact sequence, we can choose a retraction $r_k:C_k \rightarrow Z_k$. ([\[Multilinear Algebra\] §Exact Sequences, ⁋Proposition 10](/en/math/multilinear_algebra/exact_sequences#prop10)) With this choice, $(p_k\circ r_k)\otimes \id_A$ induces the map $H_k(C;A)\rightarrow H_k(C)\otimes_\mathbb{Z} A$ on homology, and one verifies that this is a retraction for the map $H_k(C)\otimes_\mathbb{Z}A\rightarrow H_k(C;A)$ above. Thus we obtain:
...
Just as in [\[Algebraic Topology\] §Computation of Homology, ⁋Definition 6](/en/math/algebraic_topology/computation_of_homology#def6), we can define the Eilenberg-Steenrod axioms for cohomology, and call a contravariant functor together with connecting morphisms satisfying these axioms a cohomology theory. Writing this out explicitly, we have:
...
- (Excision) For $(X,A,Z)$ satisfying the conditions of [\[Algebraic Topology\] §Computation of Homology, ⁋Theorem 2](/en/math/algebraic_topology/computation_of_homology#thm2), the inclusion $(X\setminus Z, A\setminus Z)\hookrightarrow (X,A)$ induces an isomorphism.
...
To establish the existence of a cohomology theory satisfying these conditions, we proceed as in [\[Algebraic Topology\] §Homology](/en/math/algebraic_topology/homology) and consider the chain complex of singular simplices of a topological space $X$

$$C_\bullet(X):\qquad\cdots \rightarrow C_{k+1}(X)\rightarrow C_k(X) \rightarrow C_{k-1}(X)\rightarrow \cdots$$

Fixing an abelian group $A$ as the coefficient group, we can consider the cochain complex dual to this:
...
By [\[Algebraic Structures\] §Direct Products, Direct Sums, and Tensor Products of Modules, ⁋Theorem 6](/en/math/algebraic_structures/operations_of_modules#thm6), this can be identified with

$$\qquad \cdots\leftarrow\Hom_A(C_{k+1}(X;A),A)\leftarrow \Hom_A(C_k(X;A),A)\leftarrow \Hom_A(C_{k-1}(X;A),A)\leftarrow\cdots$$

so it can be viewed as the dual of the chain complex $C_\bullet(X;A)$. We then define the $k$th homology of this cochain complex $(C^\vee)^\bullet(X;A)$ as

$$H^k(X;A):=H_k(C^\vee)$$

and call it the *$k$th cohomology* of $X$. The reason we use superscripts on $H$ and $C^\vee$ to index the degree is that, in contrast to homology, the long exact sequence runs in the direction of increasing index; henceforth, when there is no risk of confusion, we will write $(C^\vee)^\bullet(X)$ as $C^\bullet(X;A)$.

We now examine the relationship between $H^k(X;A)$ and $H_k(X)$ defined in this way. As noted at the outset, it is not true in general that $H^k(X;A)\cong H_k(X)^\ast$. However, by an argument parallel to the proof of [Proposition 1](#prop1), we obtain the following.
...
Roughly speaking, this can be viewed as translating [Proposition 1](#prop1) through [\[Algebraic Structures\] §Abelian Groups, ⁋Theorem 15](/en/math/algebraic_structures/abelian_groups#thm15).

## de Rham Cohomology

Associating a chain complex $C_\bullet(X)$ to a topological space $X$ can be regarded as encoding information about the subspaces of $X$ in algebraic terms. When defining cohomology, we apply $\Hom_\mathbb{Z}(-,A)$ to $C_\bullet(X)$ and then take the homology of the resulting cochain complex; here

$$C^k(X;A)=\Hom_\mathbb{Z}(C_k(X), A)$$

and an arbitrary element can be thought of as a function assigning an element of $A$ to each element of $C_k(X)$ (i.e., to each $k$-chain). Thus cohomology is essentially about studying functions defined on a space.
...
and if we write $\partial$ for the boundary map of $C_\bullet(X)$ and $\delta$ for the induced coboundary map of $C^\bullet(X;A)$, then for any $c\in C_{k+1}(X)$ and $\varphi\in C^k(X;A)$ the identity

$$\langle \partial c, \varphi\rangle=\langle c, \delta\varphi\rangle$$

holds, from which we see that these induce a pairing at the homology and cohomology level
...
Here the coboundary map $\Omega^k(\mathbb{R}^n)\rightarrow \Omega^{k+1}(\mathbb{R}^n)$ is given by the exterior derivative, and a differential $k$-form yields a number via integration when presented with a $k$-dimensional subset. Moreover, closed $k$-forms are the kernel of this coboundary, and exact $k$-forms are its image.

For instance, consider the differential $2$-form on $\mathbb{R}^3$

$$\omega=dx\wedge dy$$

A $2$-dimensional subset of $\mathbb{R}^3$ is given by a map from a (unit) rectangle in $\mathbb{R}^2$ into $\mathbb{R}^3$, and through this we know what it means to evaluate $\omega$ on a $2$-dimensional subset.
...
and using $dx \wedge dy = \sin \phi \cos \phi\, d\phi \wedge d\theta$, we compute
...
Thus the differential $2$-form $\omega$ can be thought of as a function that takes a $2$-dimensional subset such as $S$ or $\Sigma$ and returns a number.
...
The de Rham cohomology discussed above is an example of a cohomology theory whose coefficient group is not $\mathbb{Z}$. Unlike singular or simplicial cohomology, de Rham cohomology naturally has $\mathbb{R}$ as its coefficient group by definition.
  
Such a cohomology theory enjoys favorable properties: for example, since $\mathbb{R}$ is a torsion-free abelian group, $\Tor_1^\mathbb{Z}(A,\mathbb{R})=0$ for any abelian group $A$, and therefore by [Proposition 1](#prop1) we have the isomorphism

$$H_k(X;\mathbb{R})\cong H_k(X)\otimes_\mathbb{Z}\mathbb{R}$$

Moreover, since $\mathbb{R}$ is an injective $\mathbb{Z}$-module, $\Ext_\mathbb{Z}^1(A,\mathbb{R})=0$ for any abelian group $A$, and thus this time [Proposition 3](#prop3) yields the isomorphism

$$H^k(X;\mathbb{R})\cong \Hom_\mathbb{Z}(H_k(X),\mathbb{R})$$

It then becomes another matter of interest to study homologies and cohomologies of this kind. Recalling the chain complexes used to define $H_k(X;A)$ and $H^k(X;A)$, we know that the two chain complexes

$$C_\bullet(X;A):=C_\bullet(X)\otimes_\mathbb{Z}A,\qquad C_\bullet^\Delta(X;A):=C_\bullet^\Delta(X)\otimes_\mathbb{Z}A$$

are, if $A$ is a ring, chain complexes of $A$-modules, and the $C^\bullet(X;A)$ defined earlier is likewise. Therefore taking homology or cohomology of these yields $A$-modules as well.

On the other hand, we know that if $A$ is a principal ideal domain, then any submodule of a free $A$-module is again a free $A$-module. Reviewing the proof of [Proposition 1](#prop1), we see that it exploited the fact that $\mathbb{Z}$ is a principal ideal domain, so any submodule of a free $\mathbb{Z}$-module (i.e., a free abelian group) is again a free $\mathbb{Z}$-module. Based on this, we can generalize the preceding two propositions as follows.
...
Using the Mayer–Vietoris sequence, we can compute the homology or cohomology of a large space from those of its small subspaces. On the other hand, we can also multiply two spaces $X,Y$ to form a larger space $X\times Y$, and the Künneth formula helps compute the homology and cohomology of such product spaces. To state it, we must first define the tensor product of two chain complexes $C_\bullet$ and $D_\bullet$.

<div class="definition" markdown="1">

<ins id="def7">**Definition 7**</ins> Let $A$ be a ring and let $C_\bullet, D_\bullet$ be chain complexes of $A$-modules. Their *tensor product* $(C\otimes D)_\bullet$ is defined for each $k$ by

$$(C\otimes D)_k=\bigoplus_{p+q=k}C_p\otimes_A D_q$$

and the differential on homogeneous elements is given by
...
That is, $(C\otimes D)_\bullet$ can be viewed as the total complex of a double complex whose $(p,q)$-component is $C_p\otimes D_q$, with horizontal differential $\partial^C\otimes\id_D$ and vertical differential $\id_C\otimes \partial^D$. ([\[Homological Algebra\] §Homology, ⁋Definition 5](/en/math/homological_algebra/homology#def5))

The algebraic content of the Künneth formula is contained in the following lemma.

<div class="proposition" markdown="1">

<ins id="lem8">**Lemma 8**</ins> Let $A$ be a principal ideal domain and let $C_\bullet, D_\bullet$ be chain complexes of $A$-modules, with $C_\bullet$ a chain complex of free $A$-modules. Then for any $k$ there exists a short exact sequence
...
Here the first term $0$ comes from the fact that $Z_\bullet(C)$ consists of free modules. On the other hand, since free modules are flat, taking the tensor product with a free module commutes with taking homology, and therefore in the above sequence we have

$$(B(C)\otimes H(D))_\bullet\cong H_\bullet(B(C)\otimes D)\qquad (Z(C)\otimes H(D))_\bullet \cong H_\bullet(Z(C)\otimes D)$$

The map $(B(C)\otimes H(D))_\bullet \rightarrow (Z(C)\otimes H(D))_\bullet$ arises from the inclusion $B_\bullet(C)\rightarrow Z_\bullet(C)$, and under the above identification it coincides with $\delta_\bullet$. Therefore we obtain
...
This is generally proved using the [acyclic models theorem](https://en.wikipedia.org/wiki/Acyclic_model), but since the acyclic models theorem is in fact closer to a generalization of the Eilenberg–Zilber theorem, using it for this proof feels somewhat excessive. However, since proving the Eilenberg–Zilber theorem directly is quite tedious, we will only examine the two maps that appear in its proof:

$$\AW:C_\bullet(X\times Y) \rightarrow (C(X)\otimes C(Y))_\bullet,\qquad \EZ:(C(X)\otimes C(Y))_\bullet \rightarrow C_\bullet(X\times Y)$$

The proof of the acyclic models theorem is written in a separate post so as not to interrupt the flow.
...
Although this looks complicated as a formula, it is merely expressing the decomposition of the prism $\Delta^p\times \Delta^q$ into simplices—that is, the functions $h_n$ that appeared in the proof of [\[Algebraic Topology\] §Homotopy, ⁋Proposition 6](/en/math/algebraic_topology/homotopy#prop6). Then the result of [Theorem 9](#thm9) follows from the two identities
