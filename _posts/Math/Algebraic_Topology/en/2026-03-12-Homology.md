...
::: Definition 1
For any natural number $k$, suppose $k+1$ points $v_0,\ldots, v_k\in\mathbb{R}^d$ in general position are given. Then the *$k$-simplex* formed by these points is the smallest convex set containing $\{v_0,\ldots, v_k\}$.
:::
...
::: Example 2
A common example is the (2-dimensional) torus $T^2$. Its elementary definition is the product manifold $S^1\times S^1$, but to see intuitively that this product is a torus, consider the following figure.
...
::: Definition 3
For a topological space $X$, a *$\Delta$-complex structure* on it is a collection of maps $\sigma_\alpha:\Delta^{n(\alpha)}\rightarrow X$ satisfying the following.
...
::: Example 4
For instance, the 2-dimensional torus $T^2$ can be represented as in the following figure,
...
::: Definition 7
A *singular $k$-simplex* on $X$ is a continuous map $\sigma:\Delta^k\rightarrow X$.
:::
...
::: Example 8
Computing singular homology directly from the definition is not advisable, but let us carry out a bit of (non-rigorous) computation for intuition.
...
::: Proposition 9
When a topological space $X$ is expressed as a disjoint union of path-components $X=\coprod X_i$, the following isomorphism holds:
...
::: Proposition 10
For a non-empty path-connected space $X$, we have $H_0(X)\cong \mathbb{Z}$.
:::
::: Proof
...
::: Proposition 11
For a one-point space $X$, we have $H_0(X)\cong \mathbb{Z}$ and $H_k(X)\cong 0$ for all $k>0$.
:::
...
::: Definition 13
If $H_n(X)\cong H_n(Y)$ holds for all $n$, we say that two spaces $X,Y$ are *homologous*.
:::
