...
Consider a topological space $X$ with a given $\Delta$-complex structure, and consider the free abelian group $C^\Delta_k(X)$ generated by the $k$-simplices. ([\[Algebraic Structures\] §Abelian Groups, ⁋Proposition 8](/en/math/algebraic_structures/abelian_groups#prop8)) That is,
...
[^1]: In general, when defining homology the coefficient ring can be not only $\mathbb{Z}$ but an arbitrary ring. ([\[Algebraic Structures\] §Definition of a Ring, ⁋Definition 1](/en/math/algebraic_structures/rings#def1)) This is done simply by replacing $\mathbb{Z}$ in the above definition with the desired ring $R$.
