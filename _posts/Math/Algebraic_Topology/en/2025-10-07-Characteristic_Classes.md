...
description: "Starting from the Euler class of an oriented vector bundle, we define Chern and Pontryagin characteristic classes via the Thom isomorphism and the Gysin exact sequence."
...
last_polished_at: 2026-09-22T03:15:05+00:00
translation_polish_source: antigravity-gemini-3.8-flash-high
---
## Euler Class

So far we have effectively avoided the issue of orientability by using $\mathbb{Z}/2$-coefficients. We now take orientation into account as well. With $\mathbb{Z}/2$-coefficients we cannot distinguish signs, so every fiber automatically had an "orientation"; but upon passing to $\mathbb{Z}$-coefficients, $1$ and $-1$ are clearly distinct elements, and the question becomes whether one can consistently assign an orientation to each fiber. When this is possible, a more refined invariant appears, lifting the top Stiefel-Whitney class $w_n$ to the integers. This is the Euler class.

::: Definition 1
An *orientation* of a rank $n$ vector bundle $p:E\rightarrow B$ is the continuous assignment, in each local trivialization, of an orientation to each fiber $p^{-1}(x)$, that is, a choice in $H^n(p^{-1}(x), p^{-1}(x)\setminus 0;\mathbb{Z})\cong\mathbb{Z}$ of a generator $u_x$. A bundle admitting such an orientation is called an *oriented vector bundle*.
:::

One can think of giving an orientation in three broad ways. First, when given a vector bundle $E\rightarrow B$, through the zero section $0:B\rightarrow E$, we generally think of $B$ as sitting inside $E$. Then in the definition above, the relative cohomology $H^n(p^{-1}(x), p^{-1}(x)\setminus 0;\mathbb{Z})$ in itself is attaching to the origin of the fiber, that is, to the corresponding point of the base $B$, the data of $+$ or $-$.

In areas such as differential geometry, this is interpreted as follows. Since the fiber $p^{-1}(x)\cong\mathbb{R}^n$ with the origin removed deformation retracts to the sphere $S^{n-1}$, we know from the long exact sequence of the pair that the following isomorphism
...
::: Proof
To write down only the core idea, if $E$ is the trivial bundle $B\times\mathbb{R}^n$, then the pair is $(E, E_0)=(B\times\mathbb{R}^n, B\times(\mathbb{R}^n\setminus 0))$, and by the relative version of the Künneth formula,
...
For (3), suppose there exists a nowhere-$0$ section $s':B\rightarrow E_0$. The straight-line homotopy $t\mapsto t\cdot s'$ is a homotopy in $E$ connecting the zero section $0$ and $s'$, so $0^\ast=s'^\ast:H^n(E)\rightarrow H^n(B)$. Meanwhile, $s'$ factors through $i:E_0\hookrightarrow E$, and in the long exact sequence of the pair the composite $H^n(E, E_0)\xrightarrow{j^\ast}H^n(E)\xrightarrow{i^\ast}H^n(E_0)$ is $0$, so $i^\ast(j^\ast u)=0$, that is, $j^\ast u$ vanishes on $E_0$. Since $s'$ factors through $E_0$, we have $s'^\ast(j^\ast u)=0$, and therefore $e(E)=0^\ast(j^\ast u)=s'^\ast(j^\ast u)=0$.
...
For the fourth claim, over $\mathbb{Z}$ we count the zero locus of a generic section with signs, but upon reducing to $\mathbb{Z}/2$ we forget those signs. Then the unsigned count of zeros is precisely the top Stiefel-Whitney class $w_n$, so $e(E)$ is the lift of $w_n$ to the integers so as to remember the signs, and the reverse process is taking mod $2$.
...
That is, the Euler class is an obstruction measuring whether this bundle admits a nonvanishing section, and if not, how much it is obstructed; in the case of the tangent bundle, the answer manifested itself as the topological invariant $\rchi(M)$.

Let us verify this concretely on $S^2$. For example, from [§Mapping Degree and Brouwer–Lefschetz Fixed-Point Theorem, ⁋Theorem 8](/en/math/algebraic_topology/degree_and_fixed_point_theorems#thm8) we know that any section of $TS^2$ always has a point where it becomes $0$. For instance, suppose $S^2$ sits in $\mathbb{R}^3$ via the equation

$$S^2=\{(x,y,z)\mid x^2+y^2+z^2=1\}$$

and consider the height function $z$ defined on it. Then the gradient vector $\nabla z$ of this $z$ is given as a section of the tangent bundle by

$$(\nabla z)_{(x,y,z)}=(-xz,-yz,x^2+y^2)$$

and indeed we see that at points where $(x,y,z)=(0,0,\pm 1)$, this section becomes $0$. Since one can verify that this section meets the zero section transversely and that both intersection points meet in the positive direction, the Euler class must be $2\in H^2(S^2; \mathbb{Z})$. In fact, using the familiar computation of the Euler characteristic, we find that $\rchi(S^2)=2$, confirming that this indeed supports our intuition.

## Chern Classes

The Stiefel-Whitney classes and Euler classes discussed so far were all invariants of real vector bundles. We now turn our attention to complex vector bundles. Of course, any complex vector space can be viewed as a real vector space by separating its real and imaginary parts, but because the natural morphisms between complex vector spaces are not $\GL(2n;\mathbb{R})$ but $\GL(n;\mathbb{C})$, this difference changes many things.

For example, $\GL(2n;\mathbb{R})$ is not connected, whereas $\GL(n;\mathbb{C})$ is connected, so every complex vector bundle is always orientable. Intuitively it suffices to look at what happens on a single fiber $V\cong\mathbb{C}^n$; taking a (complex) basis
...
Our claim is that this orientation is preserved even when a different complex basis is chosen, because when the matrix $A\in\GL(n;\mathbb{C})$ connecting the two bases is viewed as a real linear map, its real determinant is

$$\det\nolimits_{\mathbb{R}}(A)=\lvert\det\nolimits_{\mathbb{C}}(A)\rvert^2>0$$

For instance, in the simplest case $n=1$, multiplication by $z=a+bi$ is, viewed as a real matrix,

$$\begin{pmatrix}a&-b\\ b&a\end{pmatrix}$$

and its determinant is $a^2+b^2>0$, and for general $A$ the determinant always becomes positive in this way. That is, a change of basis in a complex vector space always preserves orientation, so $V$ carries a canonical orientation (as a real vector space), and the calculation above is in fact precisely the statement that $\GL(n;\mathbb{C})\subseteq \GL^+(2n; \mathbb{R})$.
...
becomes a fiber bundle with fiber $S^{n-1}$. This is called the *sphere bundle* of $E$, and in the same way,
...
and then applies a radial retraction to obtain
...
Over a paracompact base space $B$, for an oriented rank $n$ vector bundle $E\rightarrow B$ with sphere bundle $\pi:S(E)\rightarrow B$, the following long exact sequence
...
where the maps in the second sequence are obtained by transporting the maps in the upper exact sequence along the vertical isomorphisms. Concretely, let us follow the first morphism $H^{k-n}(B)\rightarrow H^k(B)$. Using the Thom isomorphism $\Phi:\alpha\mapsto p^\ast\alpha\smile u$ to lift $\alpha\in H^{k-n}(B)$ to $H^k(E, E_0)$ and then composing with the first morphism $j^\ast$ in the upper row yields

$$j^\ast\Phi(\alpha)=j^\ast(p^\ast\alpha\smile u)=p^\ast\alpha\smile j^\ast u$$

Here the second equality uses that $j^\ast:H^\ast(E, E_0)\rightarrow H^\ast(E)$ is an $H^\ast(E)$-module homomorphism, which is intuitively clear since $p^\ast\alpha$ already lives on $H^\ast(E)$. Now we descend this again via the vertical identification $H^k(E)\cong H^k(B)$, which is accomplished through the zero section $0:B\hookrightarrow E$ ($p\circ 0=\id$), so we get
...
To actually use this, consider the tangent bundle of $S^2$, $TS^2$, examined above. Then in the Gysin sequence

$${\smile}e:H^0(S^2)\rightarrow H^2(S^2)$$

is given by the $\times 2$ map, and its cokernel $\mathbb{Z}/2$ appears in the sphere bundle $S(TS^2)$ as torsion in $H^2$. On the other hand, if we had started with the trivial bundle over $S^2$, $E$, this part would have been $\mathbb{Z}$, so the trace of the Euler class pushing the sphere bundle away from the product is contained precisely in this torsion.

Now let us define the Chern classes from this. The key fact is that if $k<n-1$, then
...
holds, and as seen above, if $k\leq 2n-2$, both of the outer terms $H^{k-2n}(B)$ and $H^{k-2n+1}(B)$ have negative degree and thus are $0$, so $\pi_0^\ast:H^k(B)\rightarrow H^k(E_0)$ is an isomorphism.

::: Definition 6
For a complex rank $n$ vector bundle $E\rightarrow B$, we define the *Chern class* $c_i(E)\in H^{2i}(B;\mathbb{Z})$ inductively on the rank $n$ of the vector bundle as follows.

First, $c_0(E)=1$, and if $i>n$ then $c_i(E)=0$, and we set

$$c_n(E)=e(E_{\mathbb{R}})\in H^{2n}(B;\mathbb{Z})$$

For $0<i<n$, since $L^\perp$ examined above is a rank $(n-1)$ vector bundle whose Chern classes are already defined by the inductive hypothesis, we transport them via the isomorphism $\pi_0^\ast:H^{2i}(B)\rightarrow H^{2i}(E_0)$, and requiring

$$\pi_0^\ast c_i(E)=c_i(L^\perp)$$

we define the (unique) $c_i(E)\in H^{2i}(B)$ to be $E$'s $i$-th Chern class. The sum $c(E)=1+c_1(E)+\cdots+c_n(E)\in H^\bullet(B;\mathbb{Z})$ of all of them is called the *total Chern class*.
:::
...
2. $c_0(E)=1$, and if $i>\rank_{\mathbb{C}}E$, then $c_i(E)=0$.
...
is an isomorphism for $k\geq 2$. Now, starting from $H^0(\CP^\infty)=\mathbb{Z}$, we obtain that $c_1(\gamma)$ is a generator of $H^2(\CP^\infty;\mathbb{Z})\cong\mathbb{Z}$, and
...
::: Proof
By [§Projective Bundles and the Leray–Hirsch Theorem, ⁋Theorem 5](/en/math/algebraic_topology/projective_bundles#thm5), with the pullback $\rho^\ast:H^\bullet(B)\hookrightarrow H^\bullet(F(E))$ being injective and splitting $\rho^\ast E$ into a Whitney sum $L_1\oplus\cdots\oplus L_n$ of complex line bundles, there exists a continuous map $\rho:F(E)\rightarrow B$. By naturality and the injectivity of $\rho^\ast$, it suffices to prove the formula assuming every bundle is a sum of line bundles.

Then it suffices to show that for a sum of line bundles,

$$c(L_1\oplus\cdots\oplus L_n)=\prod_{i=1}^n\bigl(1+c_1(L_i)\bigr)$$

holds. The key is the two identities for two line bundles $L,L'$:
...
That is, through the inclusions $j_1:z\mapsto(z,q)$, $j_2:z\mapsto(q,z)$, if we define the restriction maps $j_1^\ast$, $j_2^\ast$, they can read off the respective components. On the other hand, we showed above that the desired formula holds for trivial line bundles, and $j_1^\ast L_2$ and $j_2^\ast L_1$ are trivial, so

$$j_1^\ast c_1(L_1\oplus L_2)=c_1(\gamma\oplus\varepsilon^1)=c_1(\gamma)=j_1^\ast\bigl(c_1(L_1)+c_1(L_2)\bigr)$$

and the analogous formula holds for $j_2^\ast$ as well. That is,

$$c_1(L_1\oplus L_2)=c_1(L_1)+c_1(L_2)$$

and as seen above, adding naturality shows that this holds for arbitrary $L,L'$ as well.

Now if we show the sum formula for an arbitrary vector bundle and a line bundle, we can use induction. First, for $X=(\CP^\infty)^n$ and each projection $\pi_i:X\rightarrow\CP^\infty$, let $L_i=\pi_i^\ast\gamma$ and $x_i=c_1(L_i)$. In this case one can compute that $H^\bullet(X;\mathbb{Z})=\mathbb{Z}[x_1,\ldots,x_n]$, and by the same argument as above it suffices for us to show the formula only over this space. Now, considering the inclusion obtained by fixing the $j$-th coordinate at an arbitrary point, $\iota_j:(\CP^\infty)^{n-1}\rightarrow X$, since $\iota_j^\ast L_j$ is trivial we have $\iota_j^\ast x_j=0$, and therefore applying the identity $c(E\oplus\varepsilon^1)=c(E)$ shown above and using the inductive hypothesis for $n-1$, we obtain

$$\iota_j^\ast\left(c(L_1\oplus\cdots\oplus L_n)-\prod_{i=1}^n(1+x_i)\right)=0$$

However, $\iota_j^\ast$ leaves the remaining $x_i$ unchanged, so its kernel is the ideal $(x_j)$, and the difference inside $\iota_j^\ast$ must be a multiple of $x_j$. But since the same argument holds for every $j$, this difference is a multiple of $x_1\cdots x_n$, and considering the rank of $L_1\oplus\cdots L_n$, this difference must be exactly an integer multiple of $x_1\cdots x_n$. But in the top degree, $c(L_1\oplus\cdots \oplus L_n)$ must equal the Euler class, so it must be $x_1\cdots x_n$, and therefore expanding the latter product and computing shows that this integer is $0$. That is, $c(L_1\oplus\cdots\oplus L_n)=\prod_{i=1}^n(1+x_i)$.
:::

Earlier we stated that Chern classes can distinguish a complex vector bundle $E$ from its conjugate $\bar{E}$, that is, the bundle with the same underlying real bundle but with scalar multiplication twisted by $z\cdot v=\bar{z}v$. We can now make this claim precise.

::: Proposition 10
For a complex vector bundle $E\rightarrow B$ and its conjugate $\bar{E}$, for all $i$,
...
Now consider the tangent bundle. A point $\ell\in\CP^n$ of this space is a line $\ell\subseteq\mathbb{C}^{n+1}$, and fixing a Hermitian inner product, lines near $\ell$ are uniquely represented as graphs of linear maps $\ell\rightarrow\ell^\perp$. That is, writing the bundle of fiberwise $\mathbb{C}$-linear maps as $\Hom$,
...
and expanding this, since $H^\bullet(\CP^n)=\mathbb{Z}[\x]/(\x^{n+1})$, we see that
...
::: Definition 12
For a real vector bundle $E\rightarrow B$, its *Pontryagin class* $p_i(E)\in H^{4i}(B;\mathbb{Z})$ is defined by the Chern class of the complexification $E\otimes_{\mathbb{R}}\mathbb{C}$ as
...
---
[^1]: View $S^\infty$ as the unit sphere in $\mathbb{C}^\infty=\bigcup_n\mathbb{C}^n$. For the shift morphism $T(x_1,x_2,\ldots)=(0,x_1,x_2,\ldots)$, the two straight-line homotopies with vectors normalized by $v\mapsto v/\lvert v\rvert$, namely $x\mapsto\bigl((1-t)x+tT(x)\bigr)/\lvert(1-t)x+tT(x)\rvert$ and $x\mapsto\bigl((1-t)T(x)+te_1\bigr)/\lvert(1-t)T(x)+te_1\rvert$, respectively connect the identity map to $T$, and $T$ to the constant map $x\mapsto e_1=(1,0,\ldots)$. Neither denominator becomes $0$: for the former, the coordinates of $x$ and $T(x)$ are shifted by one position so that $(1-t)x+tT(x)=0$ forces $x=0$, and for the latter, the first coordinate of the sum is $t$, so for it to be $0$ we must have $t=0$, which then gives $T(x)=0$, that is, $x=0$. Joining these two, $S^\infty$ contracts to a point.
