...
4. (Mod 2 reduction) The $\mathbb{Z}/2$-reduction of $e(E)$ is the top Stiefel-Whitney class $w_n(E)$ of [§Stiefel-Whitney Characteristic Classes, ⁋Definition 5](/en/math/algebraic_topology/stiefel_whitney_classes#def5){: data-lid="rurfa" data-relation="required" }.
...
([Definition 3](#def3){: data-lid="tpvtf" data-relation="required" }) Similarly, the second map $H^k(D(E))=H^k(B)\rightarrow H^k(S(E))$ is the restriction, that is $\pi^\ast$, and the Gysin map $\pi_!$ is the connecting homomorphism $\delta$ transported by the Thom isomorphism $H^{k+1}(D(E), S(E))\cong H^{k-n+1}(B)$.
:::
...
This is, just as we saw for real bundles in [§Stiefel-Whitney Characteristic Classes, §§Grassmannians](/en/math/algebraic_topology/stiefel_whitney_classes#grassmannians){: data-lid="eev15" data-relation="weak" }, the *universal family* for complex line bundles. That is, since any complex line bundle is obtained uniquely as a pullback of $\gamma$, the first Chern class gives a one-to-one correspondence
...
Since all examples so far have been line bundles, let us look at one example showing how [Theorem 9](#thm9){: data-lid="jqjbq" data-relation="required" } is actually used in calculations for bundles of higher rank.
