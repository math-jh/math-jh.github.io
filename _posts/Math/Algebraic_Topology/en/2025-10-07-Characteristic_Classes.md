...
for the zero section $0:B\rightarrow E$ and the Thom class $u$ of [Theorem 2](#thm2){: data-relation="required" reviewed="" }. Here $j^\ast:H^n(E, E_0)\rightarrow H^n(E)$ is the restriction from the pair to all of $E$.
:::
...
4. (Mod 2 reduction) The $\mathbb{Z}/2$-reduction of $e(E)$ is the top Stiefel-Whitney class $w_n(E)$ of [§Stiefel-Whitney Characteristic Classes, ⁋Definition 5](/en/math/algebraic_topology/stiefel_whitney_classes#def5){: data-relation="required" reviewed="" }.
...
All five properties of [Proposition 4](#prop4){: data-relation="required" reviewed="" } are read off from the picture seen earlier, namely that $e(E)$ is the Poincaré dual recording the zero locus of a generic section with signs. Apart from the somewhat formal first two conditions, the remaining three are stories of signs and obstructions.
...
([Definition 3](#def3){: data-relation="required" reviewed="" }) Similarly, the second map $H^k(D(E))=H^k(B)\rightarrow H^k(S(E))$ is the restriction, that is $\pi^\ast$, and the Gysin map $\pi_!$ is the connecting homomorphism $\delta$ transported by the Thom isomorphism $H^{k+1}(D(E), S(E))\cong H^{k-n+1}(B)$.
:::
...
This is, just as we saw for real bundles in [§Stiefel-Whitney Characteristic Classes, §§Grassmannians](/en/math/algebraic_topology/stiefel_whitney_classes#grassmannians){: data-relation="weak" reviewed="" }, the *universal family* for complex line bundles. That is, since any complex line bundle is obtained uniquely as a pullback of $\gamma$, the first Chern class gives a one-to-one correspondence
...
For this, let us first observe by [§Cohomology, ⁋Corollary 10](/en/math/algebraic_topology/cohomology#cor10){: data-relation="required" reviewed="" } and [§Cohomology, ⁋Theorem 5](/en/math/algebraic_topology/cohomology#thm5){: data-relation="required" reviewed="" } that
...
Since all examples so far have been line bundles, let us look at one example showing how [Theorem 9](#thm9){: data-relation="required" reviewed="" } is actually used in calculations for bundles of higher rank.
...
For this, let us first consider the tautological line bundle $\gamma\subseteq\CP^n\times\mathbb{C}^{n+1}$ defined over it. This is the restriction of the universal line bundle $\gamma$ of [Example 8](#ex8){: data-relation="required" reviewed="" } via $\CP^n\hookrightarrow\CP^\infty$, and since the cell structure has cells only in even dimensions, the restriction $H^k(\CP^\infty;\mathbb{Z})\rightarrow H^k(\CP^n;\mathbb{Z})$ is an isomorphism for $k\leq 2n$. Therefore, setting $\x=c_1(\bar{\gamma})=-c_1(\gamma)$ by [Proposition 10](#prop10){: data-relation="required" reviewed="" },
...
holds. Here $\varepsilon^{n+1}$ is the rank $n+1$ trivial bundle. Therefore, identifying $\Hom(\gamma, \varepsilon^1)$ on the right-hand side with $\overline{\gamma}$, we obtain from [Theorem 9](#thm9){: data-relation="required" reviewed="" } the identity
