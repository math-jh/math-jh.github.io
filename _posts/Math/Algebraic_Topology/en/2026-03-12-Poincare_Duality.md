...
and thus the $\pi_1(M,x)$-action can be identified precisely with a group homomorphism $\pi_1(M,x)\rightarrow \mathbb{Z}^\times$. Now, $A$-module isomorphisms from $A$ to $A$ correspond exactly to elements of the unit group $A^\times$, so this ultimately amounts to examining a group homomorphism $\pi_1(M,x)\rightarrow A^\times$. That is, [Proposition 6](#prop6){: data-relation="weak" reviewed="" } generalizes as follows.
...
At any rate, by [Lemma 8](#lem8){: data-relation="required" reviewed="" } above, if $M$ is a compact topological manifold of dimension $m$, then taking $C=M$ yields the following theorem.
...
The proof again proceeds by induction using the Mayer–Vietoris sequence, just as in the proof of [Lemma 8](#lem8){: data-relation="weak" reviewed="" }. The difference, however, is that in [Lemma 8](#lem8){: data-relation="weak" reviewed="" } the claim concerned a compact subset $C$, so compactness could be used actively, whereas now the claim is about $M$ itself. Thus, for instance, if a chart $U$ of $M$ is given, it is not compact, so a simple inductive approach does not work. For this we make the following definition.
...
Then by [Lemma 8](#lem8){: data-relation="required" reviewed="" }, for each point $x$ we can find a homology class
...
holds by [§Cup Product, ⁋Proposition 6](/en/math/algebraic_topology/cup_products#prop6){: data-lid="isiu7" data-relation="required" }, and by the uniqueness in [Lemma 8](#lem8){: data-relation="required" reviewed="" } we have $i_\ast s_{K'}=s_K$, so we see that this defines the homomorphism $H_c^p(M;A)\rightarrow H_{m-p}(M;A)$ well.
