...
and thus the $\pi_1(M,x)$-action can be identified precisely with a group homomorphism $\pi_1(M,x)\rightarrow \mathbb{Z}^\times$. Now, $A$-module isomorphisms from $A$ to $A$ correspond exactly to elements of the unit group $A^\times$, so this ultimately amounts to examining a group homomorphism $\pi_1(M,x)\rightarrow A^\times$. That is, [Proposition 6](#prop6){: data-relation="requires-review" } generalizes as follows.
...
The proof again proceeds by induction using the Mayer–Vietoris sequence, just as in the proof of [Lemma 8](#lem8){: data-relation="requires-review" }. The difference, however, is that in [Lemma 8](#lem8){: data-relation="requires-review" } the claim concerned a compact subset $C$, so compactness could be used actively, whereas now the claim is about $M$ itself. Thus, for instance, if a chart $U$ of $M$ is given, it is not compact, so a simple inductive approach does not work. For this we make the following definition.
...
Our claim is that this homomorphism $D_M:H_c^p(M;A)\rightarrow H_{m-p}(M;A)$ is an isomorphism, and to show this we use induction via the Mayer–Vietoris sequence, just as in the proof of [Lemma 8](#lem8){: data-relation="requires-review" }.
...
When $M$ is not $A$-orientable, the main reason [Theorem 11](#thm11){: data-relation="requires-review" } fails is that, fundamentally, $\omega_M^A$ fails to be a constant sheaf and is only locally constant. In the language of covering spaces, this can be thought of as the monodromy action acting nontrivially on the stalk $A$, so that after going "once around" the stalk $A$ becomes twisted upon returning. Since this twist is an automorphism of $A$, it was enough for us to consider elements of the unit group $A^\times$ of $A$.
