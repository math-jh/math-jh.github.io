...
and we know that it satisfies the condition of [Definition 3](#def3){: data-relation="required" reviewed="" }. In general, one can easily prove that covering maps behave well with respect to subspaces and products, as follows.
...
Now, just as when we derived [Corollary 12](#cor12){: data-relation="weak" reviewed="" } above, we apply this theorem to a single object, replace $\Grpd$ by $\Grp$, and use the fact that pushouts in $\Grp$ are amalgamated free products, to obtain the following.
