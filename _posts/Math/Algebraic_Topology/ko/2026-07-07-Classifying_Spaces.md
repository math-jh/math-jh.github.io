...
앞선 글들에서 우리는 rank $$k$$ real vector bundle을 특성류라는 불변량으로 연구하였고, 더 근본적으로는 그 분류 문제 자체를 다루었다. 분류는 두 가지 모습으로 나타났다. 임의의 위상공간 $$B$$ 위의 rank $$k$$ vector bundle은 infinite Grassmannian $$\Gr_k(\mathbb{R}^\infty)$$ 위의 universal bundle을 pullback하여 얻어지며, 따라서 그 isomorphism class는 homotopy class $$[B, \Gr_k(\mathbb{R}^\infty)]$$와 일대일 대응하였다. 같은 bundle은 또 다르게도 open cover $$\{U_i\}$$ 위에서 transition function
...
이 두 관점을 잇는 것이 이번 글의 목표이다. 공통으로 드러나는 것은 이 분류가 사실 bundle의 구조군 $$\mathrm{GL}(k,\mathbb{R})$$에만 의존한다는 점이다. 즉 transition function이 어떤 위상군 $$G$$에 값을 가지면 그 bundle은 $$G$$를 구조군으로 갖는 것이고, 이러한 bundle은 $$G$$에만 의존하는 하나의 공간으로 분류된다. Vector bundle의 경우 $$G=\mathrm{GL}(k,\mathbb{R})$$이었고 그 공간이 $$\Gr_k(\mathbb{R}^\infty)$$이었던 것이다. 이를 임의의 topological group $$G$$에 대하여 통일적으로 수행하기 위해, 우리는 구조군 $$G$$만을 데이터로 갖는 *principal $$G$$-bundle*을 정의하고, 모든 principal $$G$$-bundle을 분류하는 단 하나의 공간 *classifying space* $$BG$$를 구성한다. 앞으로 $$G$$는 항상 topological group을 의미하며, 별다른 언급이 없는 한 base space는 paracompact인 것으로 가정한다.
...
이 명제는 principal bundle과 vector bundle의 결정적인 차이를 드러낸다. Section의 존재가 곧 자명성이므로, nontrivial principal bundle은 어떠한 continuous section도 갖지 않는다. 가령 quotient map $$S^n\rightarrow \RP^n$$은 $$\mathbb{Z}/2$$의 free action에 대한 principal $$\mathbb{Z}/2$$-bundle인데, $$S^n$$이 connected이므로 ($$n\geq 1$$) 두 장의 sheet를 연속적으로 고르는 section은 존재할 수 없고, 따라서 이 bundle은 nontrivial이다.
...
::: 명제 4
위상공간 $$X$$ 위에서, principal $$\mathrm{GL}(n,\mathbb{R})$$-bundle들의 isomorphism class와 rank $$n$$ real vector bundle들의 isomorphism class 사이에는 자연스러운 일대일 대응이 존재한다. 이 대응은 principal bundle $$P$$에 associated bundle $$P\times_{\mathrm{GL}(n,\mathbb{R})}\mathbb{R}^n$$을, vector bundle $$E$$에 frame bundle $$\mathrm{Fr}(E)$$를 대응시킨다.
:::
::: 증명
...
이 동치 덕분에 vector bundle에 대한 모든 분류 문제는 principal $$\mathrm{GL}(n,\mathbb{R})$$-bundle에 대한 문제로 번역된다. 같은 방식으로 complex vector bundle은 principal $$\mathrm{GL}(n,\mathbb{C})$$-bundle에, oriented real vector bundle은 principal $$\mathrm{GL}^+(n,\mathbb{R})$$-bundle에 대응한다. 따라서 임의의 구조군 $$G$$에 대하여 principal $$G$$-bundle을 분류할 수 있다면, 이 모든 경우가 한꺼번에 해결된다.
...
분류정리는 principal $$G$$-bundle의 기하학적 분류를 순수하게 homotopy 이론적인 데이터 $$[X,BG]$$로 옮긴다. [명제 4](#prop4)와 결합하면 rank $$n$$ real vector bundle의 분류가 $$[X,B\mathrm{GL}(n,\mathbb{R})]$$로, complex의 경우 $$[X,B\mathrm{GL}(n,\mathbb{C})]$$로 환원되며, 이것이 앞 글의 Grassmannian을 통한 분류와 같은 내용임을 곧 보게 된다.
...
를 유도한다. 이는 Gram–Schmidt 직교화가 $$\mathrm{GL}(n,\mathbb{C})$$을 $$\mathrm{U}(n)$$ 위로 deformation retract시키는 데서 나온다. 구체적으로 $$\mathrm{GL}(n,\mathbb{C})$$의 임의의 행렬은 unitary 행렬과 양의 정부호 upper-triangular 행렬의 곱으로 유일하게 분해되고($$QR$$ 분해), upper-triangular 인자를 항등원 쪽으로 연속적으로 수축시키면 $$\mathrm{GL}(n,\mathbb{C})$$이 $$\mathrm{U}(n)$$으로 deformation retract됨을 얻는다. Group 수준의 이 homotopy equivalence가 $$B$$를 거쳐 분류공간 수준의 homotopy equivalence를 준다. 따라서 rank $$n$$ complex vector bundle은 그 구조군을 $$\mathrm{GL}(n,\mathbb{C})$$에서 $$\mathrm{U}(n)$$으로 줄여도 분류에 손실이 없으며, 이것이 모든 complex bundle에 Hermitian metric을 줄 수 있다는 사실의 분류공간 판본이다.
:::
