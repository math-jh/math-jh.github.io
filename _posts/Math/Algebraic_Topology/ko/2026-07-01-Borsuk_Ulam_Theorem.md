...
::: 따름정리 6
$n\geq 1$에 대하여, antipodal map $g:S^n\rightarrow S^{n-1}$은 존재하지 않는다. 따라서 [정리 2](#thm2){: data-lid="luvf7" }의 세 진술이 모두 성립한다.
:::
::: 증명
...
전사가 아닌 자기사상은 어떤 점 $q$를 image에서 빠뜨리므로 그 image가 $S^n\setminus\{q\}$에 담긴다. $S^n\setminus\{q\}$은 stereographic projection에 의해 $\mathbb{R}^n$과 homeomorphic한 contractible 공간이므로, $\iota\circ g$는 이 contractible 공간을 거쳐 인수분해되어 상수사상과 homotopic하다. 따라서 그 degree는 [§사상의 차수와 Brouwer·Lefschetz 고정점 정리, ⁋명제 3](/ko/math/algebraic_topology/degree_and_fixed_point_theorems#prop3){: data-lid="cuhxf" }의 homotopy invariance와 상수사상의 degree가 $0$이라는 사실에 의하여 $\deg(\iota\circ g)=0$이다. 그러나 $\iota\circ g$는 antipodal이므로 [정리 5](#thm5){: data-lid="zx4hm" }에 의하여 그 degree가 홀수여야 하고, 이는 $0$과 모순이다. 그러므로 그러한 $g$는 존재하지 않는다.

이로써 [정리 2](#thm2){: data-lid="2iat1" }의 (2)가 성립하고, [명제 3](#prop3){: data-lid="u1smk" }에 의하여 (1)도 성립하며, (3)은 [정리 5](#thm5){: data-lid="8sah0" } 그 자체이다.
:::

::: 참고 7
[정리 2](#thm2){: data-lid="03tj5" }의 세 진술은 모두 참이지만, 우리가 실제로 세운 함의의 방향은 (3) $\Rightarrow$ (2) $\Leftrightarrow$ (1)이다. 곧 degree의 홀짝성을 다루는 (3)이 가장 강한 형태이며, 나머지 둘을 함의한다. (1)과 (2)의 동치는 [명제 3](#prop3){: data-lid="171vn" }에서 보듯 초등적이지만, 이들로부터 degree가 정확히 홀수라는 (3)을 되돌려 얻는 것은 그만큼 직접적이지 않다. 문헌에 따라 "Borsuk–Ulam 정리"는 (1)이나 (2)를 가리키기도 하고, Borsuk의 원래 정리인 (3)을 가리키기도 한다.
:::
...
을 정의하자. 각 $\operatorname{dist}(\cdot,A_i)$은 연속이므로 $f$도 연속이며, [정리 2](#thm2){: data-lid="uui4k" }의 (1)에 의하여 $f(x)=f(-x)$인 점 $x\in S^n$이 존재한다. 곧 모든 $1\leq i\leq n$에서 $\operatorname{dist}(x,A_i)=\operatorname{dist}(-x,A_i)$이다.
...
:::
::: 증명
그러한 embedding $e:S^n\rightarrow\mathbb{R}^n$이 존재한다고 하자. 특히 $e$은 연속함수이므로 [정리 2](#thm2){: data-lid="9twyb" }의 (1)에 의하여 $e(x)=e(-x)$인 점 $x\in S^n$이 존재한다. $n\geq 1$에서 $x\neq-x$이므로 이는 $e$의 단사성에 모순이다. 따라서 그러한 embedding은 존재하지 않는다.
:::
