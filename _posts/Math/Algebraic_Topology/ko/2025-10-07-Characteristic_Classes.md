...
4. (Mod 2 환원) $e(E)$의 $\mathbb{Z}/2$-환원은 [§슈티펠-휘트니 특성류, ⁋정의 5](/ko/math/algebraic_topology/stiefel_whitney_classes#def5){: data-relation="required" }의 top Stiefel-Whitney class $w_n(E)$이다.
...
[명제 4](#prop4){: data-relation="required" }의 다섯 성질은 모두 앞서 본 그림, 곧 $e(E)$가 generic section의 zero locus를 부호까지 담아 적은 Poincaré dual이라는 데서 읽힌다. 다소 형식적인 앞의 두 조건을 제외하면, 나머지 셋은 부호와 obstruction의 이야기이다.
...
Euler class라는 이름은 그것이 재는 양에서 온다. $M$이 closed oriented $n$-manifold이고 $E=TM$이 그 tangent bundle이면, $e(TM)$을 [§푸앵카레 쌍대성, ⁋정의 10](/ko/math/algebraic_topology/Poincare_duality#def10){: data-relation="required" }의 fundamental class $[M]$ 위에서 evaluate한 값이 정확히 Euler characteristic 
...
이를 $S^2$에서 구체적으로 확인해 보자. 가령 [§사상의 차수와 Brouwer·Lefschetz 고정점 정리, ⁋정리 8](/ko/math/algebraic_topology/degree_and_fixed_point_theorems#thm8){: data-relation="weak" }으로부터 우리는 $TS^2$의 임의의 section은 항상 $0$이 되는 곳이 있다는 것을 안다. 예를 들어, $S^2$이 다음의 식
...
를 보자. 그럼 우선 첫째 항은 [정리 2](#thm2){: data-relation="required" }를 사용하여 
...
가 된다. ([정의 3](#def3){: data-relation="required" }) 비슷하게, 둘째 map $H^k(D(E))=H^k(B)\rightarrow H^k(S(E))$는 restriction, 곧 $\pi^\ast$인 것을 알 수 있으며, Gysin map $\pi_!$는 connecting homomorphism $\delta$를 Thom isomorphism $H^{k+1}(D(E), S(E))\cong H^{k-n+1}(B)$로 옮긴 것이다.
:::
...
::: 증명
둘째 조건과 셋째 조건은 $c_n=e(E_{\mathbb{R}})$의 vanishing을 [명제 4](#prop4){: data-relation="required" }의 셋째 조건을 사용하기만 하면 정의로부터 바로 오는 것이다. 

첫째 조건은 $n$에 대한 귀납으로 보인다. $c_n$의 naturality는 Euler class의 naturality로부터 온다. ([명제 4](#prop4){: data-relation="required" }의 첫째 조건) $0<i<n$에서는 $f$가 deleted space와 complement bundle, 그리고 Gysin sequence 전체와 호환되는 bundle map $E_0'\rightarrow E_0$을 유도하고, 그 위에서 $f^\ast(L^\perp)\cong(f^\ast L)^\perp$이므로 귀납가정과 $\pi_0^\ast$의 naturality로부터 $c_i$의 naturality가 따라온다.
:::

즉 Chern class는 Stiefel-Whitney class와 비슷한 종류의 공리적인 성질을 만족한다. ([§슈티펠-휘트니 특성류, ⁋정의 5](/ko/math/algebraic_topology/stiefel_whitney_classes#def5){: data-relation="weak" }) 우리는 Stiefel-Whitney class의 존재성을 보이기 위해 real infinite Grassmannian $\Gr(k,\mathbb{R}^\infty)$를 생각한 후, 여기서의 cohomology class를 원래의 공간으로 pullback해와서 이들이 Stiefel-Whitney class의 공리적 조건을 만족함을 보였었는데, Chern class에 대해서도 비슷한 종류의 construction이 가능하다. 

::: 예시 8
[§슈티펠-휘트니 특성류, ⁋예시 3](/ko/math/algebraic_topology/stiefel_whitney_classes#ex3){: data-relation="weak" }의 real tautological line bundle의 complex analogue로, $\CP^\infty=\Gr(1,\mathbb{C}^\infty)$ 위의 tautological complex line bundle $\gamma$를 생각하자. 그럼 $\gamma$의 sphere bundle은 $\mathbb{C}^\infty$의 단위구 $S^\infty$이며 이는 contractible이므로[^1] 모든 $k>0$에 대해 $H^k(S^\infty)=0$이다. 따라서, [정리 5](#thm5){: data-relation="required" }에 의해 $H^1(\CP^\infty)=0$이고 
...
이는 [§슈티펠-휘트니 특성류, §§그라스만 다양체](/ko/math/algebraic_topology/stiefel_whitney_classes#그라스만-다양체){: data-relation="weak" }에서 real bundle에 대해 본 것과 같이, 이 $\gamma$는 complex line bundle의 *universal family*이다. 곧 임의의 complex line bundle이 $\gamma$의 pullback으로 유일하게 얻어지므로, first Chern class는 일대일 대응
...
이다. 둘째 등식은 임의의 base 위에서 바로 얻어진다. [정의 6](#def6){: data-relation="required" }에 의하여 rank $2$ bundle의 최고차항은 

$$c_2(L\oplus L')=e\bigl((L\oplus L')_{\mathbb{R}}\bigr)$$

인데, [명제 4](#prop4){: data-relation="required" }의 둘째 결과에 의하여 이는 $e(L_{\mathbb{R}})\smile e(L'_{\mathbb{R}})=c_1(L)\smile c_1(L')$과 같기 때문이다.

첫째 등식을 위해 우선 임의의 rank $n$ complex vector bundle $E$와 trivial line bundle $\varepsilon^1$에 대하여 다음 식

$$c(E\oplus\varepsilon^1)=c(E)$$

가 성립함을 보이자. $E'=E\oplus\varepsilon^1$이라 하면, trivial 성분에서 상수 $1$을 취하는 section $s(x)=(0,1)$은 어디서도 $0$이 아니므로 section $s:B\rightarrow E'_0$을 주며 $\pi_0\circ s=\id$이다. 이제 각 점에서 $(0,1)$의 orthogonal complement는 정확히 $E$의 fiber이므로 $s^\ast(E'^\perp)\cong E$이고, 따라서 $0<i\leq n$에 대하여 [정의 6](#def6){: data-relation="required" }의 식 $\pi_0^\ast c_i(E')=c_i(E'^\perp)$에 $s^\ast$를 적용하면 [명제 7](#prop7){: data-relation="required" }의 naturality로부터

$$c_i(E')=s^\ast\pi_0^\ast c_i(E')=s^\ast c_i(E'^\perp)=c_i(s^\ast E'^\perp)=c_i(E)$$

를 얻는다. 최고차 $c_{n+1}(E')=e(E'_{\mathbb{R}})$는 어디서도 $0$이 아닌 section이 존재하므로 [명제 4](#prop4){: data-relation="required" }의 (3)에 의하여 $0$이며, 이는 $c_{n+1}(E)=0$과 일치한다.

한편, [예시 8](#ex8){: data-relation="required" }에서 보았듯 $\gamma$가 complex line bundle의 universal family이므로, 임의의 base $B$ 위의 두 line bundle $L,L'$은 각각 base의 morphism $f_1,f_2:B\rightarrow\CP^\infty$를 따른 pullback $f_1^\ast\gamma$, $f_2^\ast\gamma$이다. 이제
...
를 보이는 것은, [명제 7](#prop7){: data-relation="required" }의 첫째 결과에 의해,
...
이를 위해 우선 [§코호몰로지, ⁋따름정리 10](/ko/math/algebraic_topology/cohomology#cor10){: data-relation="required" }과 [§코호몰로지, ⁋정리 5](/ko/math/algebraic_topology/cohomology#thm5){: data-relation="required" }에 의해 
...
이를 위해 우선 이 위에 정의된 tautological line bundle $\gamma\subseteq\CP^n\times\mathbb{C}^{n+1}$을 생각하자. 이는 [예시 8](#ex8){: data-relation="required" }의 universal line bundle $\gamma$를 $\CP^n\hookrightarrow\CP^\infty$으로 제한한 것이며, cell 구조가 짝수 차원에만 있으므로 restriction $H^k(\CP^\infty;\mathbb{Z})\rightarrow H^k(\CP^n;\mathbb{Z})$은 $k\leq 2n$에서 isomorphism이다. 따라서 [명제 10](#prop10){: data-relation="required" }에 따라 $\x=c_1(\bar{\gamma})=-c_1(\gamma)$로 두면
...
이다. 여기서 $\varepsilon^{n+1}$은 rank $n+1$ trivial bundle이다. 따라서, 우변의 $\Hom(\gamma, \varepsilon^1)$을 $\overline{\gamma}$와 identify하면 [정리 9](#thm9){: data-relation="required" }로부터 다음의 식
