...
이므로 [정리 10](#thm10){: data-relation="required" }에 의하여 $f$는 fixed point를 갖는다. 특히 $D^{n+1}$은 convex하여 contractible하고 삼각화가능한 콤팩트 공간이므로 ($n+1$-simplex와 homeomorphic이다), 임의의 $f:D^{n+1}\rightarrow D^{n+1}$이 fixed point를 가진다는 [정리 7](#thm7)의 Brouwer 정리를 얻는다.
:::
