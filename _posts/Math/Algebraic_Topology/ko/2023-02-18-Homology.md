...
이렇게 얻어지는 chain complex $(C^\Delta_k(X),\partial_k)$의 $n$th homology를 *$n$-th simplicial homology*라 부르고, $H_n^\Delta(X)$로 적는다. ([\[호몰로지 대수학\] §호몰로지, ⁋정의 2](/ko/math/homological_algebra/homology#def2)) 

::: 예시 6
2차원 토러스 $T^2$를 위의 [예시 4](#ex4)과 같이 나타내고, 오른쪽에 나열된 순서대로 $2$-simplex를 각각 $L$, $U$, $1$-simplex를 각각 $a,b,c$, 그리고 $0$-simplex를 $p$라 하자. 두 $1$-simplex $b,c$에는 이미 방향이 주어져 있으므로, 이들이 simplex가 되기 위해서는 $a$가 왼쪽 아래에서 오른쪽 위로의 방향을 가져야 한다. 이제
...
이제 $C_k(X)$를 모든 singular $k$-simplex들로 생성된 free abelian group이라 하고, $\partial_k:C_k(X)\rightarrow C_{k-1}(X)$를 위의 식 (1)과 동일한 방식으로 정의하자. 그럼 정확히 [명제 5](#prop5)과 같은 방식으로 $(C_k(X), \partial_k)$가 chain complex가 된다는 것을 확인할 수 있으며, 이 때의 homology들을 *singular homology*라 부르고 $H_n(X)$로 적는다. 
...
그러나, 물론, 가장 중요하다고 할 수도 있는 성질은 functoriality일 것이다. 그런데 우리는 이미 $\Ab$들의 chain complex들의 category $\Ch_{\geq 0}(\Ab)$에서, 각각의 $n$에 대하여 $n$th homology를 계산하는 $H_n:\Ch_{\geq 0}(\Ab)\rightarrow \Ab$는 functor임을 안다. ([\[호몰로지 대수학\] §호몰로지, ⁋명제 3](/ko/math/homological_algebra/homology#prop3)) 따라서 합성 
