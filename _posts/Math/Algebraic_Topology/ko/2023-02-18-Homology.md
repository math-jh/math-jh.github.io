...
2. $\sigma_\alpha$를 $\Delta^{n(\alpha)}$의 한 면으로 제한한 $\sigma_\alpha\vert_{\Delta^{n(\alpha)-1}}:\Delta^{n(\alpha)-1}\rightarrow X$ 또한 이 함수들의 모임에 속한다.
...
이렇게 얻어지는 chain complex $(C^\Delta_k(X),\partial_k)$의 $n$th homology를 *$n$번째 simplicial homology<sub>$n$번째 심플렉스 homology</sub>*라 부르고, $H_n^\Delta(X)$로 적는다. ([\[호몰로지 대수학\] §호몰로지, ⁋정의 2](/ko/math/homological_algebra/homology#def2)) 
...
위에서 정의한 심플렉스 homology는 직관적으로 명확한 의미를 갖지만, 임의의 위상공간이 주어졌을 때 $X$의 homology를 계산하기 위해서는 이 위에 $\Delta$-complex 구조를 주어야 한다는 점에서 그 한계가 명확하다. 설령 $X$가 topological manifold라 하여도 그 차원이 $4$ 이상이면 $X$ 위에 $\Delta$-complex 구조를 주는 것이 불가능할 수도 있다는 것이 잘 알려져 있다. 
...
임을 안다. 즉 직관적으로 $Z_1(X)$는 $X$에서의 폐곡선들로 생성되는 subgroup이라 생각할 수 있다. 비슷하게 $B_1(X)=\im\partial_2$에 기하학적인 의미를 부여해보자면 이는 $X$에서의 폐곡선 중 어떠한 원판의 경계로 나오는 것을 의미하며 따라서 첫 번째 homology
