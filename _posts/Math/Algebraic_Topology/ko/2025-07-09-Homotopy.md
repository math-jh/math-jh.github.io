...
이 성립한다면 $F$가 *homotopy relative to $A$*라 말한다. 그럼 임의의 두 path $\alpha_0,\alpha_1:I\rightarrow X$에 대하여, 이들 사이의 *path homotopy*는 것은 homotopy relative to $\\{0,1\\}$를 의미한다. 즉, 두 path $\alpha_0,\alpha_1$가 끝점들을 공유하며 (즉 $\alpha_0(0)=\alpha_1(0)$이고 $\alpha_0(1)=\alpha_1(1)$이며) homotopy $(\alpha\_t)\_{0\leq t\leq 1}$가 끝점들을 보존하여 
...
![natural_isomorphism](/assets/images/Math/Algebraic_Topology/Homotopy-3.png){:style="width:18em" class="invert" .align-center}
