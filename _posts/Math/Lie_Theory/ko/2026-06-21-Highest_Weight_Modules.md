...
:::
::: 증명
$x\in\mathfrak{g}_\alpha$, $v\in V_\lambda$라 하자. 임의의 $H\in\mathfrak{h}$에 대하여 가군의 action이 $H\cdot(x\cdot v)=x\cdot(H\cdot v)+[H,x]\cdot v$를 만족하고, $H\cdot v=\lambda(H)v$이며 $[H,x]=\alpha(H)x$이므로
...
이제 $M(\lambda)$에서 irreducible 가군을 잘라낸다. 핵심은 $M(\lambda)$가 유일한 maximal submodule을 갖는다는 것이며, 이는 highest weight space가 $1$차원이라는 사실과, proper submodule이 highest weight space를 건드리지 못한다는 관찰에서 나온다.
...
:::
::: 증명
$W$가 submodule이므로 $\mathfrak{h}$-action에 대해 닫혀 있고, [명제 7](#prop7)에 의해 $M(\lambda)$가 weight module이므로 $W$도 weight module이다. 곧 $W=\bigoplus_\mu(W\cap M(\lambda)_\mu)$이다. 만일 $W\cap M(\lambda)_\lambda\neq 0$이면 $M(\lambda)_\lambda=\mathbb{C}v_\lambda^+$가 $1$차원이므로 $v_\lambda^+\in W$이고, $v_\lambda^+$가 $M(\lambda)$를 $U(\mathfrak{g})$-가군으로서 생성하므로 $W=M(\lambda)$가 되어 $W$가 proper submodule이라는 가정에 모순이다. 따라서 $W\cap M(\lambda)_\lambda=0$이다.
