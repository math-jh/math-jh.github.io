...
## Adjoint representation과 Weyl group

지금까지 우리는 Lie group $G$의 Weyl group을 $W=N(T)/T$로 정의하였다. 그런데 Lie algebra 이론에서도 Weyl group이라 불리는 대상이 등장한다. 이 절에서는 두 정의가 자연스럽게 일치함을 보인다.

우선 conjugation action의 미분으로 정의되는 adjoint representation을 생각하자.

<div class="definition" markdown="1">

<ins id="def15">**정의 15**</ins> Lie group $G$의 *adjoint representation<sub>수반 표현</sub>*은 다음의 Lie group homomorphism

$$\Ad:G\rightarrow\Aut(\mathfrak{g});\qquad \Ad(g)=d(c_g)_e$$

으로 정의된다. 여기서 $c_g:G\to G$는 $g$에 의한 conjugation $c_g(h)=ghg^{-1}$이다.

</div>

$\Ad$는 $G$의 Lie algebra $\mathfrak{g}$ 위에 작용하는 $G$의 representation이다. 이제 maximal torus $T$로 제한한 $\Ad|_T$를 생각하자. $T$는 compact torus이므로 weight decomposition이 존재하고, 이를 통해

$$\mathfrak{g}=\bigoplus_{\lambda}\mathfrak{g}_\lambda$$

를 얻는다. 여기서 $\lambda:T\to S^1$은 $T$의 character이고

$$\mathfrak{g}_\lambda=\{X\in\mathfrak{g}\mid \Ad(t)X=\lambda(t)X\text{ for all $t\in T$}\}$$

이다.

<div class="proposition" markdown="1">

<ins id="prop16">**명제 16**</ins> $\mathfrak{g}$의 weight decomposition에서 weight $0$에 해당하는 weight space $\mathfrak{g}_0$는 $\mathfrak{t}$이다.
...
이다. 이는 $T$의 Lie algebra $\mathfrak{t}$의 centralizer와 같다. 한편 $\mathfrak{t}$는 maximal abelian subalgebra이므로 그 centralizer는 $\mathfrak{t}$ 자신이다.

</details>

이제 $0$이 아닌 weight들을 *roots*라 부르고 그 모임을 $\Phi$로 표기한다. 각 root $\alpha\in\Phi$에 대하여 $\mathfrak{g}_\alpha$를 *root space*라 하며, decomposition

$$\mathfrak{g}=\mathfrak{t}\oplus\bigoplus_{\alpha\in\Phi}\mathfrak{g}_\alpha$$

을 $\mathfrak{g}$의 *root space decomposition*이라 한다.

이제 $N(T)$가 $\mathfrak{t}$ 위에 어떻게 작용하는지 살펴보자. 임의의 $n\in N(T)$에 대하여, $\Ad(n)$은 $\mathfrak{g}$ 위의 automorphism이다. 또한 $nTn^{-1}=T$이므로 $\Ad(n)$은 $\mathfrak{t}$를 보존한다. 따라서 $\Ad(n)|_\mathfrak{t}$는 $\mathfrak{t}$의 automorphism이고, 이를 dual space $\mathfrak{t}^*$로 옮기면 $\mathfrak{t}^*$ 위의 linear action을 얻는다.
...
$$[H,\Ad(n)X]=\Ad(n)[\Ad(n)^{-1}H,X]=\Ad(n)[\Ad(n)^{-1}H,X]$$

인데, $\Ad(n)^{-1}H\in\mathfrak{t}$이고 $X\in\mathfrak{g}_\alpha$이므로

$$[\Ad(n)^{-1}H,X]=\alpha(\Ad(n)^{-1}H)X=(\Ad(n)^{-1})^*\alpha(H)\cdot X$$

이다. 따라서

$$[H,\Ad(n)X]=(\Ad(n)^{-1})^*\alpha(H)\cdot\Ad(n)X$$

이고, 이는 $\Ad(n)X\in\mathfrak{g}_{(\Ad(n)^{-1})^*\alpha}$임을 의미한다. 즉 $n\cdot\alpha=(\Ad(n)^{-1})^*\alpha$로 정의하면 $\Ad(n)$은 $\mathfrak{g}_\alpha$를 $\mathfrak{g}_{n\cdot\alpha}$로 보낸다.

</details>

따라서 우리는 $W=N(T)/T$가 $\mathfrak{t}^*$ 위에, 특히 root system $\Phi$ 위에 잘 정의된 action을 갖는다는 것을 안다. 한편 [\[근계\] §근계](/ko/math/lie_theory/root_systems)에서 root system $\Phi$의 automorphism들 중 reflection $s_\alpha$들로 생성되는 부분군을 *Weyl group*이라 정의하였다. 우리의 주장은 두 정의가 일치한다는 것이다.
...
임의의 root $\alpha\in\Phi$에 대하여, [\[근계\] §근계](/ko/math/lie_theory/root_systems)에서 살펴본 것과 같이 $\mathfrak{g}_\alpha$와 $\mathfrak{g}_{-\alpha}$ 그리고 그 bracket으로 생성되는 subalgebra는 $\mathfrak{sl}(2,\mathbb{C})$와 isomorphic하다. 이에 대응하는 Lie group은 $\SU(2)$ 혹은 $\SO(3)$와 locally isomorphic하다.

이 subalgebra에 대응하는 Lie subgroup $G_\alpha\subset G$를 생각하자. $G_\alpha$는 $T$와 교차하여 1차원 torus $T_\alpha = T\cap G_\alpha$를 형성한다. $G_\alpha$의 normalizer 중 $T_\alpha$를 보존하는 원소들은 [예시 13](#ex13)에서 계산한 것과 같이 $T_\alpha$와 그 외의 원소 하나로 구성되며, 후자가 reflection $s_\alpha$에 해당한다.

구체적으로 적당한 $E_\alpha\in\mathfrak{g}_\alpha$, $F_\alpha\in\mathfrak{g}_{-\alpha}$를 택하여 $\exp(E_\alpha)\exp(F_\alpha)\exp(E_\alpha)$를 계산하면 이것이 $N(T)$의 원소 $n_\alpha$를 주고, $\Ad(n_\alpha)|_\mathfrak{t}$가 정확히 reflection $s_\alpha$가 됨을 확인할 수 있다.

</details>

이로부터 우리는 Lie group $G$의 Weyl group $W=N(T)/T$가 $\mathfrak{t}^*$의 root system $\Phi$의 대칭성을 완전히 포착한다는 것을 안다. 특히 $W$의 원소는 $\Phi$를 보존하는 orthogonal transformation으로 생각할 수 있으며, 이 관점에서 $W$는 유한 Coxeter group의 구조를 갖는다.
