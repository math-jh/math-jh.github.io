...
($\Leftarrow$) trace 조건을 가정한다. $\mathfrak{g}$가 solvable임을 보이는 것은 $[\mathfrak{g},\mathfrak{g}]$가 solvable임을 보이는 것으로 충분하다. $\mathfrak{g}/[\mathfrak{g},\mathfrak{g}]$는 abelian, 따라서 solvable이고, $[\mathfrak{g},\mathfrak{g}]$가 solvable이면 [§가해 Lie algebra와 nilpotent Lie algebra, ⁋명제 14](/ko/math/lie_theory/solvable_nilpotent_lie_algebras#prop14)의 확장에 대한 닫힘성에서 $\mathfrak{g}$가 solvable이기 때문이다. $[\mathfrak{g},\mathfrak{g}]\subseteq\mathfrak{gl}(V)$의 모든 원소가 nilpotent endomorphism임을 보이면, Engel 정리의 선형 형태에 의해 ([§가해 Lie algebra와 nilpotent Lie algebra, ⁋따름정리 17](/ko/math/lie_theory/solvable_nilpotent_lie_algebras#cor17)) $[\mathfrak{g},\mathfrak{g}]$가 동시에 strictly 상삼각화되어 nilpotent Lie algebra, 특히 solvable임이 따른다.
