...
이 글 전체에서 $k$는 고정된 체이고, $\mathfrak{g}$는 $k$ 위에 정의된 Lie algebra이다. ([§리 군, ⁋정의 8](/ko/math/lie_theory/Lie_groups#def8){: data-lid="8h049" }) Lie algebra의 정의는 임의의 체 위에서 동일하게 주어지므로, 우리는 기반 체를 $k$로 일반화하여 사용한다.
...
이제 우리는 $\mathfrak{g}$의 Lie bracket을 commutator로 강제하는 결합대수를 구성한다. $\mathfrak{g}$를 단순한 벡터공간으로 보고 그 tensor algebra $\T(\mathfrak{g})$를 취하면 ([\[다중선형대수학\] §텐서대수, ⁋정의 1](/ko/math/multilinear_algebra/tensor_algebras#def1){: data-lid="ji6j4" }), 이는 $\mathfrak{g}$로 생성되는 자유 결합대수이다. 여기에서 $x\otimes y-y\otimes x$가 $[x,y]$와 일치하도록 적절한 이데알로 quotient를 취한다.
...
로 적힌다. 다만 이 시점에서 $\iota$가 단사인지는 아직 알 수 없으며, 이는 PBW 정리의 따름정리로 비로소 확립된다. ([따름정리 11](#cor11){: data-lid="thuah" })
...
이고, $\mathfrak{g}$의 기저를 고정하면 이는 polynomial algebra $k[\x_i]_{i\in I}$가 된다. ([\[다중선형대수학\] §텐서대수, ⁋명제 8](/ko/math/multilinear_algebra/tensor_algebras#prop8){: data-lid="tldqa" }) 즉 abelian인 경우 보편 포락 대수는 가환대수이며, PBW 정리는 이 동형이 일반적인 $\mathfrak{g}$에 대해서도 그 associated graded 수준에서 성립함을 주장하는 것으로 이해할 수 있다.
:::
...
:::
::: 증명
합성 $\iota_{\mathfrak{h}}\circ\phi:\mathfrak{g}\rightarrow U(\mathfrak{h})_{\mathrm{Lie}}$는 Lie algebra 준동형의 합성이므로 다시 Lie algebra 준동형이다. 따라서 [명제 4](#prop4){: data-lid="6qhje" }를 $A=U(\mathfrak{h})$, $f=\iota_{\mathfrak{h}}\circ\phi$에 적용하면, $U(\phi)\circ\iota_{\mathfrak{g}}=\iota_{\mathfrak{h}}\circ\phi$를 만족하는 결합 단위 대수 준동형 $U(\phi):U(\mathfrak{g})\rightarrow U(\mathfrak{h})$가 유일하게 존재한다. Functoriality는 보편 성질의 유일성 부분에서 따라온다. $\id_{\mathfrak{g}}$에 대응되는 $U(\id_{\mathfrak{g}})$와 항등사상 $\id_{U(\mathfrak{g})}$은 모두 $\iota_{\mathfrak{g}}$를 $\iota_{\mathfrak{g}}$로 보내므로 유일성에 의해 같고, 두 Lie algebra 준동형 $\phi,\psi$에 대하여 $U(\psi)\circ U(\phi)$와 $U(\psi\circ\phi)$도 같은 방식으로 일치한다.
:::
...
핵심적인 관찰은 관계식 $xy-yx=[x,y]$가 여과의 관점에서 commutator의 degree를 떨어뜨린다는 것이다. $x,y\in\mathfrak{g}\subseteq U_1$이면 $xy,yx\in U_2$이지만 그 차 $xy-yx=[x,y]$는 $U_1$에 속한다. 일반적으로 $a\in U_m$, $b\in U_n$에 대하여 $ab-ba\in U_{m+n-1}$이 성립하며, 이는 아래 [보조정리 8](#lem8){: data-lid="e3id7" }에서 정밀하게 다룬다. 이로부터 associated graded는 commutative함이 예상된다.
...
$\gr U(\mathfrak{g})$가 commutative하고 $\mathfrak{g}$의 상으로 생성되므로, [\[다중선형대수학\] §텐서대수, ⁋명제 6](/ko/math/multilinear_algebra/tensor_algebras#prop6){: data-lid="4zq5g" }의 보편 성질에 의하여 자연스러운 결합대수 준동형이 하나 정해진다. $\mathfrak{g}$를 $U_1/U_0\subseteq\gr U(\mathfrak{g})$로 보내는 $k$-linear map은 가환대수로 들어가므로, 유일한 결합대수 준동형
...
들을 기저로 갖는 polynomial algebra이다. ([\[다중선형대수학\] §텐서대수, ⁋명제 8](/ko/math/multilinear_algebra/tensor_algebras#prop8){: data-lid="03o07" }) 여기에서 $S$는 $I$의 원소들로 이루어진 비내림차순<sub>nondecreasing</sub> 유한열을 가리킨다. 우리는 다음 두 조건을 만족하는 $k$-linear action $x_i\cdot(-):\S(\mathfrak{g})\rightarrow\S(\mathfrak{g})$들을 구성하고자 한다.
...
::: 정리 10
[정의 7](#def7){: data-lid="mjd1r" } 이후 구성한 자연스러운 전사 준동형
...
:::
::: 증명
$\iota(\mathfrak{g})$는 길이 $1$인 정렬된 단항식 $x_i$들로 생성되며, $x\in\mathfrak{g}$를 기저로 $x=\sum_i a_i x_i$로 쓰면 $\iota(x)=\sum_i a_i x_i$이다. [정리 9](#thm9){: data-lid="un9ed" }에 의하여 길이 $1$짜리 정렬 단항식 $x_i$들은 $U(\mathfrak{g})$ 안에서 일차독립이므로, $\iota(x)=0$이면 모든 $a_i=0$, 곧 $x=0$이다. 따라서 $\iota$는 단사이다.
:::
...
PBW 정리가 주는 구체적인 그림을 보기 위해 $\sl_2=\sl(2;k)$의 보편 포락 대수를 살펴본다. $\sl_2$는 traceless $2\times 2$ 행렬들의 Lie algebra이며 ([§리 군, ⁋명제 12](/ko/math/lie_theory/Lie_groups#prop12){: data-lid="jiyhd" }), standard basis
...
을 갖는다. 이 commutation relation은 $\sl_2$의 표현론에서 핵심적인 역할을 하며, $e,f$는 각각 weight을 올리고 내리는 raising, lowering operator로 작용한다. ([§근계, ⁋정의 8](/ko/math/lie_theory/root_systems#def8){: data-lid="aypnu" }) 따라서 $U(\sl_2)$는 세 generator $e,f,h$와 관계식
...
으로 다시 쓸 수 있는데, 이는 $ef=fe+h$를 사용해 $ef+fe=2fe+h$로 정렬한 것이다. 이 Casimir element는 $\sl_2$의 유한차원 representation 분류에서 각 irreducible representation 위에 스칼라로 작용하여, representation을 구별하는 불변량의 역할을 한다. ([§근계, ⁋정의 8](/ko/math/lie_theory/root_systems#def8){: data-lid="w1x6c" })
