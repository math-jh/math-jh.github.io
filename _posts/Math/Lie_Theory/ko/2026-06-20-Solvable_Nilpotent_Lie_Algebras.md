...
::: 명제 5
$\mathfrak{a}$가 $\mathfrak{g}$의 ideal이면, 몫 벡터공간 $\mathfrak{g}/\mathfrak{a}$ 위에
...
::: 보조정리 19
$k$가 대수적으로 닫힌 표수 $0$의 체이고, $V$가 $0$이 아닌 유한차원 $k$-벡터공간, $\mathfrak{g}\subseteq\mathfrak{gl}(V)$가 subalgebra라 하자. $\mathfrak{a}\subseteq\mathfrak{g}$가 ideal이고 $\lambda:\mathfrak{a}\rightarrow k$가 선형범함수일 때,
