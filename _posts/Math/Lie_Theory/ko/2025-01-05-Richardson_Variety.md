...
$$X_w^\circ$$는 $$B$$의 궤도이고 $$X^u_\circ$$는 $$B^-$$의 궤도이므로, 둘 다 $$X$$ 위에서 locally closed subset이다. Bruhat decomposition에 의해 $$X_w^\circ \cong \bbC^{\ell(w)}$$이고 $$X^u_\circ \cong \bbC^{\dim(G/P) - \ell(u)}$$이며, 이들은 $$X$$의 서로 다른 셀 분해에 속한다. 두 셀이 교차하여 비어있지 않으려면, 해당하는 Weyl군 원소들 사이에 Bruhat order $$u \leq w$$가 성립해야 한다. 이는 Deodhar [Deo85]와 Kazhdan–Lusztig [KL80]의 결과로, 구체적으로 $$X_w^\circ \cap X^u_\circ \neq \emptyset \iff u \leq w$$이고 이 경우 교차는 차원 $$\ell(w) - \ell(u)$$의 smooth irreducible affine variety가 된다. Kazhdan–Lusztig 다항식의 정확한 정의는 [§Kazhdan–Lusztig polynomial, ⁋정리 4](/ko/math/lie_theory/kazhdan_lusztig#thm4)를 참조한다.
...
<ins id="prop6">**명제 6**</ins> Marsh-Rietsch의 mirror construction에서, [정의 5](#def5)의 variety $$\check{X}^\circ$$ 위에는 superpotential $$W_q: \check{X}^\circ \times \bbC_q^\ast \to \bbC$$가 Plücker coordinate의 rational function으로 주어진다. 이 superpotential의 Jacobi ring $$\operatorname{Jac}(W_q)$$는 A-model의 quantum cohomology ring과 동형이다.
...
<ins id="ex7">**예시 7**</ins> $$\operatorname{Gr}(1, n) = \bbP^{n-1}$$의 경우, $$P$$는 첫 번째 좌표축을 고정하는 parabolic subgroup이고, $$w_P$$와 $$w_0$$는 각각 $$S_n$$에서 적절한 permutation이 된다. 이때 $$\check{X}^\circ$$는 $$(\bbC^\ast)^{n-1}$$와 isomorphic하며, 이는 toric variety의 경우 Hori-Vafa mirror construction에서 얻어지는 결과와 일치한다. Superpotential은
