...
핵심 도구는 representation마다 정의되는 Casimir element이다. Cartan의 판정법으로 semisimple Lie algebra의 Killing form이 nondegenerate임을 알고 있으므로 ([§Killing 형식과 Cartan 판정법, ⁋정리 9](/ko/math/lie_theory/killing_form_and_cartan_criterion#thm9)), representation이 정의하는 invariant bilinear form 역시 nondegenerate가 되도록 만들 수 있고, 그 dual basis로부터 $\rho(\mathfrak{g})$ 전체와 교환하는 보편 포락 대수의 원소를 얻는다. 이 원소가 각 irreducible factor 위에서 스칼라로 작용하면서도 trace가 representation의 차원과 무관하게 $\dim\mathfrak{g}$로 고정된다는 사실이, short exact sequence가 항상 분할됨을 보이는 정밀한 산술을 가능하게 한다. 이 글 전체에서 $k$는 characteristic $0$의 algebraically closed field이고, $\mathfrak{g}$는 $k$ 위의 유한차원 semisimple Lie algebra이며, representation이라 하면 모두 유한차원 $k$-representation을 뜻한다.
...
$\beta$가 nondegenerate이므로 임의의 basis $(x_i)$에 대하여 $\beta(x_i,x^j)=\delta_i^j$를 만족하는 dual basis $(x^i)$가 유일하게 존재한다. 표기를 절약하기 위해 $U(\mathfrak{g})$의 원소 $c_\rho=\sum_i x_i x^i$와 그 $\End(V)$에서의 image를 같은 기호로 적고, 문맥에 따라 둘을 구별한다. 아래 [명제 3](#prop3)에서 보듯 이 원소는 basis의 선택에 의존하지 않으며, $\rho(\mathfrak{g})$ 전체와 교환한다. $\sl_2$의 표준 representation에서 $\beta(x,y)=\tr(xy)$에 대한 dual basis를 계산하면 $c_\rho$가 [§보편 포락 대수, ⁋예시 12](/ko/math/lie_theory/universal_enveloping_algebra#ex12)의 $\Omega$와 (스칼라 배를 무시하고) 일치함을 확인할 수 있다.

::: 명제 3
[정의 2](#def2)의 Casimir element $\rho(c_\rho)$는 basis $(x_i)$의 선택에 의존하지 않으며, 임의의 $x\in\mathfrak{g}$에 대하여 $[\rho(x),\rho(c_\rho)]=0$이다. 곧 $\rho(c_\rho)$는 $\rho(\mathfrak{g})$의 모든 원소와 교환하는 $\mathfrak{g}$-equivariant endomorphism이다. 나아가
...
먼저 자명 representation으로의 morphism에 관한 한 가지 관찰을 둔다. $\mathfrak{g}$가 semisimple이면 $\mathfrak{g}=[\mathfrak{g},\mathfrak{g}]$이고, 따라서 $\mathfrak{g}$에서 abelian Lie algebra인 $1$차원 자명 representation $\gl(k)=k$로 가는 representation은 모두 $0$이다. 곧 $1$차원 representation 위에서 $\mathfrak{g}$는 자명하게 작용한다. ($[\mathfrak{g},\mathfrak{g}]=\mathfrak{g}$인 것은 [§Killing 형식과 Cartan 판정법, ⁋정리 10](/ko/math/lie_theory/killing_form_and_cartan_criterion#thm10)의 simple ideal 분해에서 각 simple 성분이 $[\mathfrak{g}_i,\mathfrak{g}_i]=\mathfrak{g}_i$를 만족하는 데에서 따라온다.)
...
따라서 $\mathcal{W}\subseteq\mathcal{V}$는 codimension $1$의 subrepresentation이고 quotient 위에서 $\mathfrak{g}$가 자명하게 작용하므로, [보조정리 5](#lem5)에 의해 $\mathfrak{g}$-불변 direct sum $\mathcal{V}=\mathcal{W}\oplus L$, $\dim L=1$이 존재한다. $L$의 generator $f_0$를 그 위에서 $\mathfrak{g}$가 자명하게 작용하도록, 곧 $x\cdot f_0=0$이 모든 $x$에 대해 성립하도록 택할 수 있고, $f_0\notin\mathcal{W}$이므로 $f_0\vert_W=\lambda\id_W$에서 $\lambda\neq 0$이다. $\pi=\lambda^{-1}f_0$로 두면 $\pi\vert_W=\id_W$이고, $x\cdot\pi=0$은 정확히 $\pi$가 $\mathfrak{g}$-equivariant라는 진술이다. 이 $\pi$는 $V$를 $W$ 위로 보내는 $\mathfrak{g}$-morphism projection이고, $\ker\pi$는 $\pi$가 equivariant이므로 subrepresentation이며 $V=W\oplus\ker\pi$가 분할을 준다.
:::
