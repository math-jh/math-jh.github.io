...
In a similar way, one can verify that classical matrix groups such as $\Omat(n)$, $\SO(n)$, $\Umat(n)$, $\SU(n)$, etc., also carry Lie group structures. More generally, [Theorem 5](#thm5){: data-lid="tie8d" } below shows that any closed subgroup of $\GL(n;\mathbb{R})$ is automatically a Lie group.
:::
...
In the same way as [Proposition 9](#prop9){: data-lid="agqg2" }, the following proposition can be proved.
...
When we learned [Theorem 15](#thm15){: data-lid="e1oh4" } above, one natural question is whether, given a Lie group $G$ and its Lie algebra $\mathfrak{g}$, and a Lie subalgebra $\mathfrak{h}$ of $\mathfrak{g}$, there exists a Lie subgroup $H$ of $G$ having $\mathfrak{h}$ as its tangent space at the identity. However, by definition a Lie subalgebra is closed under the Lie bracket, so by [\[Differential Manifolds\] §Distribution, ⁋Theorem 3](/en/math/manifolds/distribution#thm3){: data-lid="nl83g" } this defines a submanifold of $G$. These will also have the group operation by [Theorem 17](#thm17){: data-lid="sqt1y" } above, but the problem is that, as pointed out earlier, this theorem is effective only in a local region. However, if $G$ were simply connected, there would be no topological obstacle to extending this to all of $G$, so the following theorem holds.
