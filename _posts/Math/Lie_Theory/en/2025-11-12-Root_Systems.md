...
weight: 3
translated_at: 2026-05-25T05:00:02+00:00
...
and by [§Lie Groups, ⁋Theorem 15](/en/math/lie_theory/Lie_groups#thm15) we may essentially think that all the information $$\Ad$$ knows is contained here. In any case, thinking of the Lie algebra of a vector space is the same as thinking of the space itself, so we may regard $$\mathfrak{g}$$ as being represented by the representation space $$\mathfrak{g}$$, and then $$\ad$$ is explicitly given by

$$\ad(X)Y=[X,Y].$$

What was important in using the results of representation theory was that every finite-dimensional representation is always unitary. The logic used to prove this result is that we can choose a $$G$$-invariant inner product on $$V$$; to be precise, since we are interested in the orthogonal complement, a non-degenerate symmetric form suffices. But there is a natural bilinear form on a Lie algebra.
...
That the Killing form is symmetric and $$\mathbb{C}$$-bilinear is obvious from the definition. Moreover, this Killing form is already invariant under the adjoint action by $$G$$ without any further manipulation. That is, the formula
...
What remains is the condition that this be non-degenerate.
...
The proof of this is not immediately important, so we pass over it.
...
By definition, $$\mathfrak{h}$$ acts on itself by $$0$$. That is, $$\mathfrak{h}$$ is the part corresponding to eigenvalue $$0$$ when $$\mathfrak{g}$$ is decomposed into simultaneous eigenspaces, and from this we obtain the decomposition
...
1. For any $$\alpha,\beta\in \Phi$$, $$[\mathfrak{g}_\alpha,\mathfrak{g}_\beta]\subseteq \mathfrak{g}_{\alpha+\beta}$$.
...
1. For any $$X\in \mathfrak{g}_\alpha, Y\in \mathfrak{g}_\beta$$, and $$H\in \mathfrak{h}$$,
    ...
    holds.
2. For any $$X\in \mathfrak{g}_\alpha, Y\in \mathfrak{g}_\beta$$, and $$H\in \mathfrak{h}$$, by the $$\ad$$-invariance of $$K$$,
    
    $$0=K([H,X],Y)+K(X,[H,Y])=K(\alpha(H),X)+K(X,\beta(H)Y)=(\alpha+\beta)(H)K(X,Y)$$

    holds. If $$\alpha+\beta\neq 0$$, then for this to always hold we must always have $$K(X,Y)=0$$.
3. Since the Killing form is non-degenerate on $$\mathfrak{g}$$, for any given $$X\in \mathfrak{g}_\alpha$$ there exists $$Z\in \mathfrak{g}$$ such that $$K(X,Z)\neq 0$$. What we need to show is that we can arrange $$Z\in \mathfrak{g}_{-\alpha}$$. This is obvious because if we decompose $$Z$$ according to the root decomposition, by the second result the components corresponding to parts other than $$-\alpha$$ pair with $$X$$ to give $$0$$ anyway.
...
Hence the multiplicative structure of $$\sl(2;\mathbb{C})$$ is given by the following commutation relations:

$$[H,E]=2E,\quad [H,F]=-2F,\quad [E,F]=H.$$

We show that every $$\sl(2;\mathbb{C})$$-representation decomposes as a direct sum of irreducible representations. This is an obvious result for compact Lie groups, but remember that for non-compact groups such as $$\SL(n,\mathbb{C})$$ the existence of a Haar measure is not guaranteed, and so ideas such as averaging an inner product by integration cannot be used.
...
Then by the commutation relations we saw earlier,
...
there exists $$\mu$$ such that $$V_\mu\neq 0$$ but $$V_{\mu+2}=0$$. Such a $$\mu$$ is called the *highest weight*, and an element of $$V_\mu$$ is called a *highest weight vector*. Then for a highest weight vector $$v$$ we know that the following two equations hold:
...
The first two equations are obvious, so it suffices to show the equation for $$E$$. We proceed by induction. The case $$j=0$$ is obvious, and if the given formula holds for $$j$$, then
...
and from the equation for $$H$$ we have $$H\cdot v_j=(\mu-2j)v_j$$, so substituting these gives the desired result.
...
and by the minimality of $$m$$ we know that $$\mu=m$$. That is, the highest weight must always be a positive integer.
...
It is not difficult to show that $$V(m)$$ is irreducible. Now for an arbitrary $$\sl_2$$-representation $$V$$, we can find the highest weight of $$V$$, apply [Proposition 7](#prop7) to the highest weight vector, and if any highest weight vectors remain, repeat this process to decompose $$V$$ into irreducible $$\sl_2$$-representations.
...
<ins id="def9">**Definition 9**</ins> Fix a finite-dimensional vector space $$V$$ and an inner product $$( -,-)$$ on it. A finite set $$\Phi$$ of non-zero vectors of $$V$$ is called a *root system* if the following conditions are satisfied.
...
The first claim is obvious. For the second claim, for any $$X_\alpha\in \mathfrak{g}_\alpha, X_\beta\in \mathfrak{g}_\beta$$ and any $$H\in \mathfrak{h}$$, by the $$\ad$$-invariance of $$K$$ we obtain the formula
...
To show the third claim, recall that for any $$H\in \mathfrak{h}$$ there exists $$X\in \mathfrak{g}$$ such that $$K(H,X)\neq 0$$. What we newly need to show is that we can choose $$X$$ from $$\mathfrak{h}$$. For this, writing $$X$$ in the form of a root decomposition $$\sum X_\alpha$$, by the above result we know that when we take $$K(H,-)$$, all $$X_\alpha$$ except $$X_0\in \mathfrak{h}$$ give $$0$$. Therefore $$K(H,H_0)\neq 0$$.

The fourth claim is proved in exactly the same way as the third.

</details>

Now since the Killing form defined on $$\mathfrak{g}$$ is also non-degenerate on $$\mathfrak{h}$$, the following isomorphism induced from it exists:
...
<ins id="lem11">**Lemma 11**</ins> For any $$E\in \mathfrak{g}_\alpha$$ and $$F\in \mathfrak{g}_{-\alpha}$$, $$[E,F]=K(E,F)H_\alpha$$ holds.
...
holds for all $$H\in \mathfrak{h}$$. Since $$E\in \mathfrak{g}_\alpha$$,

$$[H,E]=\alpha(H)E=K(H_\alpha,H)E$$

and substituting this into the above formula gives

$$K([E,F],H)=K(F,[H,E])=K(F, K(H_\alpha,H)E)=K(H_\alpha,H)K(F,E)=K(K(F,E)H_\alpha,H)$$

which holds for all $$H$$, so we obtain the desired result.

</details>

On the other hand, from [Lemma 10](#lem10) we can choose $$E\in \mathfrak{g}_\alpha$$ and $$F\in \mathfrak{g}_{-\alpha}$$ such that $$K(E,F)\neq 0$$. Then from the above result we know that they satisfy the relations

$$[E,F]=K(E,F)H_\alpha,\quad [H_\alpha,E]=\alpha(H_\alpha)E=K(\alpha,\alpha)E,\quad [H_\alpha,F]=-\alpha(H_\alpha)F=-K(\alpha,\alpha)F.$$

This has a form similar to the commutation relations of the $$\sl_2$$-representation we examined above, and in fact it is not difficult to show that $$(\alpha,\alpha)\neq 0$$. Thus defining

$$h_\alpha=\frac{2}{K(\alpha,\alpha)}H_\alpha$$

and similarly, by appropriate scaling in place of $$E$$ and $$F$$, we can choose $$e_\alpha\in \mathfrak{g}_\alpha$$ and $$f_\alpha\in \mathfrak{g}_{-\alpha}$$ satisfying
...
That is, they give a subalgebra of $$\mathfrak{g}$$ isomorphic to $$\sl_2$$. Let us call this $$\sl_{2,\alpha}$$. Then via the adjoint action we can regard $$\mathfrak{g}$$ as an $$\sl_{2,\alpha}$$-representation.
...
Meanwhile, since we saw earlier that $$\mathfrak{h}$$ is generated by the $$H_\alpha$$, the $$h_\alpha$$, being their scalar multiples, also generate $$\mathfrak{h}$$. As mentioned earlier, to give a root system structure here we consider the *real* vector space generated by the $$h_\alpha$$:

$$\mathfrak{h}_\mathbb{R}=\span_\mathbb{R}\{h_\alpha\mid \alpha\in\Phi\}.$$

Then for two bases $$h_\alpha,h_\beta$$ on this,

$$K(h_\alpha,h_\beta)=\tr_\mathfrak{g}(\ad h_\alpha\ad h_\beta)=\sum_{\gamma\in\Phi}\gamma(h_\alpha)\gamma(h_\beta)$$

holds. We proved earlier that these $$\gamma(h_\alpha),\gamma(h_\beta)$$ are integers, and therefore so is $$K(h_\alpha,h_\beta)$$. That is, when restricted to $$\mathfrak{h}_\mathbb{R}$$, $$K$$ is real-valued, and now for any $$h\in \mathfrak{h}_\mathbb{R}$$,

$$K(h,h)=\tr(\ad_h\ad_h)=\sum_{\gamma\in\Phi}\gamma(h)^2\geq 0$$

so we know that $$K$$ is positive definite on $$\mathfrak{h}_\mathbb{R}$$. In particular, transferring this back to $$\mathfrak{h}^\ast$$, we can confirm that the real span of $$\Phi$$ in $$\mathfrak{h}^\ast$$ forms a Euclidean space, and in the process of showing this we also showed that these roots satisfy the fourth condition of [Definition 9](#def9). Now what we need to show are the remaining conditions.

First, we need to show that applying the reflection operator

$$s_\alpha(\beta)=\beta-\frac{2K(\alpha,\beta)}{K(\alpha,\alpha)}\alpha$$

again gives a root, but this is obvious because applying $$\ad f_\alpha$$ $$\lvert \beta(h_\alpha)\rvert$$ times yields an isomorphism between $$\mathfrak{g}_\beta$$ and $$\mathfrak{g}_{s_\alpha(\beta)}$$.
...
must both be integers, so $$c$$ must be one of $$\pm 1$$, $$\pm 2$$, $$\pm 1/2$$, and then applying $$\sl_2$$-representation theory and integrality gives the desired result. That is, we have proved the following.

<div class="proposition" markdown="1">

<ins id="prop12">**Proposition 12**</ins> The set of roots $$\Phi$$ defined in [Definition 4](#def4) is a root system of $$\mathfrak{h}^\ast$$.
...
and this is given by swapping the $$i$$-th and $$j$$-th components of $$\mathbf{x}$$. Therefore from this we know that the third condition of [Definition 5](#def5) holds, and the fourth condition is obvious.
...
<ins id="ex14">**Example 14**</ins> This time consider the standard Euclidean space $$\mathbb{R}^n$$. This time consider the set

$$\Phi(D_n)=\left\{\pm e_i\pm e_j\mid 1\leq i \neq j\leq n\right\}.$$

That these vectors span $$\mathbb{R}^n$$ is obvious. This time we need to examine what reflection any $$\mathbf{e}_{ij}^\pm =e_i\pm e_j$$ defines. We know that the $$e_i-e_j$$ swap the $$i$$-th and $$j$$-th components of a vector $$\mathbf{x}$$, so it suffices to know what reflection $$e_i+e_j$$ defines. That is, considering the calculation

$$s_{ij}^+(\mathbf{x})=\mathbf{x}-\langle\mathbf{x}, \mathbf{e}_{ij}^+\rangle\mathbf{e}_{ij}^+=(x_1,\ldots, x_n)-(x_i+x_j)\mathbf{e}_{ij}$$

the map $$s_{ij}^+$$ swaps the $$i$$-th and $$j$$-th components of the given vector and then flips the signs as well.

</div>

As we can see from the above examples, not all roots are needed to describe a root system. For instance, in the case of $$\Phi(A_n)$$,

$$e_i-e_k=(e_i-e_j)+(e_j-e_k)$$

holds, and from this we know that to describe $$\Phi(A_n)$$ only elements of the form $$e_i-e_{i+1}$$ are needed. In a similar way we define the following.

<div class="definition" markdown="1">

<ins id="def15">**Definition 15**</ins> For a root system $$\Phi$$, a subset $$\Phi^+$$ of $$\Phi$$ is called the set of *positive roots* if for each root $$\alpha\in \Phi$$, exactly one of $$\alpha$$ and $$-\alpha$$ belongs to $$\Phi$$, and whenever any two $$\alpha,\beta\in \Phi^+$$ are given, $$\alpha+\beta\in \Phi^+$$ also holds. Fixing a set of positive roots $$\Phi^+$$, an element $$\alpha$$ of $$\Phi^+$$ is called a *simple root* if $$\alpha$$ cannot be expressed as a sum of two elements of $$\Phi^+$$.

</div>

Thus if we know only how the integer values

$$\langle\alpha_i,\alpha_j\rangle=2\frac{(\alpha_i,\alpha_j)}{(\alpha_j,\alpha_j)}$$

between simple roots are defined, we can know how the reflection $$s_j$$ corresponding to any root $$\alpha_j$$ moves another root $$\alpha_i$$.
...
By the definition of a root system, each component $$a_{ij}$$ is an integer. Also it is obvious that for each $$i$$, $$a_{ii}=2$$.
...
Here the cases $$\pm 1$$ are excluded by the second condition of [Definition 5](#def5), so roots can only form angles of $$30$$ degrees (or $$150$$ degrees), $$45$$ degrees (or $$135$$ degrees), or $$60$$ degrees (or $$120$$ degrees).

For example, if the angle between two roots $$\alpha,\beta$$ is $$30$$ degrees or $$150$$ degrees, then from

$$\langle\alpha,\beta\rangle\langle\beta,\alpha\rangle=3$$

the value $$\langle\alpha,\beta\rangle$$ must be $$\pm 1$$ or $$\pm 3$$. Now from the formula
...
taking the value $$\pm 1$$ or $$\pm 3$$, we know that the length ratio of $$\alpha$$ and $$\beta$$ must be $$\sqrt{3}$$. Similarly, if the angle between two roots $$\alpha,\beta$$ is $$45$$ degrees or $$135$$ degrees, the length ratio of these two roots must be $$\sqrt{2}$$, and if the angle is $$60$$ degrees or $$120$$ degrees, the length ratio must be $$1$$.

## Weyl group

For each root $$\alpha$$ of a root system $$\Phi$$, the reflection $$s_\alpha$$ defines an automorphism of $$\Phi$$. Consider the group generated by these reflections.
...
Now let us examine how $$N(T)$$ acts on $$\mathfrak{t}^\ast$$. For any $$n\in N(T)$$, the adjoint representation $$\Ad(n)$$ is an automorphism of $$\mathfrak{g}$$ and since $$nTn^{-1}=T$$, it preserves $$\mathfrak{t}$$. Therefore $$\Ad(n)\vert_\mathfrak{t}$$ is an automorphism of $$\mathfrak{t}$$, and dualizing this gives a linear action on $$\mathfrak{t}^\ast$$.
...
For any root $$\alpha\in\Phi$$, as we saw earlier, $$\sl_{2,\alpha}=\langle e_\alpha, f_\alpha, h_\alpha\rangle$$ is a subalgebra of $$\mathfrak{g}$$ isomorphic to $$\sl(2;\mathbb{C})$$. The corresponding Lie subgroup $$G_\alpha$$ of $$G$$ is locally isomorphic to $$\SU(2)$$ or $$\SO(3)$$.

$$G_\alpha$$ forms a 1-dimensional torus $$T_\alpha=T\cap G_\alpha$$ when intersected with $$T$$. Now we need to find an element of $$N(T)$$ whose $$\Ad$$-action induces the reflection $$s_\alpha$$ on $$\mathfrak{t}$$. For this, consider the element

$$n_\alpha=\exp(e_\alpha)\exp(-f_\alpha)\exp(e_\alpha).$$

By calculation in $$\SU(2)$$, this corresponds to
...
From this we know that the Weyl groups from the two perspectives are essentially the same object. From the Lie group perspective, $$W=N(T)/T$$ captures the conjugation action of the maximal torus, and from the Lie algebra perspective, $$W$$ captures the symmetry of the root system. The agreement of the two definitions is a concrete expression of the fact that the structure of a compact Lie group is completely determined by the root system of its Lie algebra.

Integrating these two perspectives, the geometric objects that naturally emerge from the root system, Borel subgroups and flag varieties, are treated in [§Borel Subgroups and Flag Varieties](/en/math/lie_theory/borel_subgroup).
