...
In the example above, the jump set condition $\{1,4\}\leq\{v(1),v(2)\}\leq\{2,3\}$ requires $4\leq v(2)\leq3$, so no such $v$ exists; conversely, if $u\leq w$ then choosing $v=u$ makes the fixed point $uP$ a point of $R_{u,w}$. This is the precise geometric content of the condition $u\leq w$ in [Proposition 2](#prop2){: data-lid="h05xg" data-relation="forward" }.
