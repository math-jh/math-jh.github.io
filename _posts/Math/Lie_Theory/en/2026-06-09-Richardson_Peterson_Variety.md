...
Meanwhile, the key idea of [Proposition 9](#prop9) was to decompose $\mathcal{Y}$ by taking its intersection with the Bruhat decomposition of $G/B$. But we already examined in [§Bruhat Decomposition, ⁋Theorem 8 (Birkhoff decomposition)](/en/math/lie_theory/bruhat_decomposition#thm8) the decomposition via the opposite Borel subgroup, so we can also explore the properties of $\mathcal{Y}$ through $\mathcal{Y}\cap B^-wB/B$.
