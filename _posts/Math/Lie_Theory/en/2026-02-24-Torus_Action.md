...
Now for an arbitrary one-parameter subgroup $$\alpha:\mathbb{R}\rightarrow N_0$$, we must have $$\alpha(\mathbb{R})\cdot T=T$$, and from this we know that $$\alpha(\mathbb{R})\subset T$$. But by [\[Differential Manifolds\] §Vector Fields, ⁋Theorem 6](/en/math/manifold/vector_fields#thm6), these cover some open neighborhood of the identity in $$N_0$$, and therefore generate $$N_0$$. That is, $$N_0=T$$.
