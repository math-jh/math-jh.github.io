...
We write $$\mathfrak{X}(M)$$ for the set of $$C^\infty$$ vector fields on $$M$$, and henceforth assume all vector fields are $$C^\infty$$. Meanwhile, by a partition of unity, any vector field defined only on some open set $$U$$ of $$M$$ can be extended to all of $$M$$. ([§Tangent Space, footnote 1](/en/math/manifold/tangent_space#fn:1))
