...
As when we previously defined $$C^\infty$$ functions from a manifold to $$\mathbb{R}$$, one should verify that this definition is independent of the choice of coordinate system; but this is essentially identical to what we proved after [§Smooth Manifolds, ⁋Definition 2](/en/math/manifold/smooth_manifolds#def2), so we omit it.
