...
- 임의의 commutative ring $A$에 대하여, $(\lMod{A},\otimes_A, A)$의 monoid object는 associative unital $A$-algebra이다.
- 임의의 commutative ring $A$에 대하여, $(\Ch(A),\otimes_A, A)$의 monoid object는 differential graded $A$-algebra이다. 여기서 unit $A$는 degree $0$에 $A$가 있고, 나머지 degree는 모두 $0$인 chain complex이다.
:::
...
세 번째 예시를 살펴보기 위해서는 $\lMod{A}$의 symmetric monoidal category의 구조를 먼저 살펴보는 것이 좋다. Cartesian monoidal category들과는 다르게, $\lMod{A}$에서의 monoidal product는 categorical product가 아닌 tensor product로 주어져있고, 따라서 unit object 또한 terminal object가 아니라 $A$이다. Unitor들의 경우, $A$-module $M$이 하나 주어질 때마다 $\lambda_M$은

$$\lambda_M: A\otimes M \rightarrow M;\quad a\otimes m\mapsto am$$

으로 결정되는 isomorphism이고, 비슷하게 $\rho_M$은 $m\otimes a\mapsto am$을 통해 유일하게 결정되는 $A$-linear map이다.

$\lMod{A}$의 임의의 대상은 이미 덧셈구조가 존재한다. $\lMod{A}$에서의 monoid object $(M,\mu,\eta)$는 $M$이 가지고 있는 덧셈구조에 더하여 이 덧셈구조와 호환되는 곱셈구조가 생기는 것으로 이해할 수 있으며, 이러한 방식으로 $M$이 $A$-algebra가 된다. 이 때, 덧셈구조와 곱셈구조가 호환된다는 것, 즉 분배법칙과 같은 것들이 성립한다는 사실은 $M\otimes M \rightarrow M$의 임의의 $A$-linear map과, $M\times M$에서 $M$으로의 $A$-bilinear map 사이의 일대일대응이 존재한다는 사실로부터 얻어진다. 

$M$에 곱셈구조를 주기 위해 마지막으로 남는 것은 이 곱셈에 대한 항등원인데, 이 정보는 $\eta:A \rightarrow M$로부터 정해줄 수 있다. $A$ 위에 정의된 left $A$-module 구조를 생각해보면, $\eta$가 담고 있는 정보는 정확히 $\eta(1)$과 동등하고, 이 원소 $\eta(1)\in M$이 새로 정의한 곱셈에 대한 항등원 역할을 한다. 
...
가 되어야 할 것이다. 여기에서 $e_G$는 $G$의 모든 원소를 $G$의 항등원으로 보내는 group homomorphism이고, $((-)^{-1},\id_G)$는 두 map $(-)^{-1}:G \rightarrow G$와 $\id_G:G \rightarrow G$가 함께 결정하는 morphism, 즉 원소 $g$를 $(g^{-1},g)$로 보내는 map이다. 물론 두 데이터를 전부 추가해서 이를 group object라 할 수도 있겠지만, 그렇게 한다면 예컨대 (monoid object로서의) unit $\eta:I \rightarrow G$와 새로 정의한 morphism $e_G$가 서로 아무런 관련이 없을 것이기 때문에 좋은 해결책이 아니다.
...
[정의 3](#def3){: data-lid="8hyeh" data-relation="required" reviewed="" }은 cartesian monoidal category에서부터 시작했기 때문에, categorical product의 universal property를 이용하면 위와 같이 associator와 unitor들을 빼고 diagram을 그릴 수 있었다. 이들을 모두 살려서 적는다면 처음 두 diagram은 정확히 monoid object의 조건이고, 마지막 조건이 새롭게 추가된 것으로 볼 수 있다. 
...
마지막 예시만이 조금 덜 자명해보일 수 있지만, 이는 multiplication $\mu:G\times G \rightarrow G$가 group homomorphism이어야 한다는 조건에서 나온다. $\Grp$의 terminal object는 trivial group이므로 unit $\eta$의 image는 $G$의 항등원 $e$이고, [정의 3](#def3){: data-lid="zzp4i" data-relation="required" reviewed="" }의 둘째 조건에 의해 $e$는 $\mu$에 대한 항등원이기도 하다. 한편 $G\times G$의 연산이 성분별로 주어지므로, $\mu$가 group homomorphism이라는 것은 임의의 $x,y,z,w\in G$에 대하여 $\mu(xz,yw)=\mu(x,y)\mu(z,w)$가 성립한다는 뜻이고, 이는 곧 $\mu$와 $G$의 원래 곱 사이의 interchange law이다. 같은 항등원을 갖는 두 연산이 interchange law를 만족하므로 Eckmann–Hilton 논법에 의해 $\mu$는 원래의 곱과 일치하고 이 곱은 commutative이다. 

## Hopf monoid

위의 [정의 3](#def3){: data-lid="n0xkl" data-relation="required" reviewed="" }을 만들 때 필요했던 것을 살펴보면, 우리가 필요한 것은 정확히 diagonal map $\Delta: G \rightarrow G\otimes G$와 augmentation map $G \rightarrow I$, 그리고 inverse map $\iota: G \rightarrow G$이다. 여기에서 필요한 것들을 잘 나눠보면 우선 다음을 정의할 수 있다.
...
이를 풀어 써 보자면, comonoid가 담고 있는 정보는 *comultiplication<sub>쌍대곱</sub>* $\Delta: M \rightarrow M\otimes M$과 *counit<sub>쌍대단위원</sub>* $\epsilon:M \rightarrow I$으로 이루어지며, 이들은 [정의 1](#def1){: data-lid="egwxw" data-relation="required" reviewed="" }의 두 조건의 dual 버전을 만족한다. 
...
::: 정의 7
Symmetric monoidal category $(\mathcal{A},\otimes,I)$에서 $(H,\mu,\eta,\Delta,\epsilon,\iota)$가 *Hopf monoid<sub>호프 모노이드</sub>*라는 것은 $(H,\mu,\eta,\Delta,\epsilon)$이 bimonoid이고 $\iota$가 [정의 3](#def3){: data-lid="13tfg" data-relation="required" reviewed="" }의 마지막 diagram과 동일한 조건을 만족하는 것이다.
:::

$\iota$에 대한 조건을 명시적으로 쓰기 위해서는 [정의 3](#def3){: data-lid="m2rzb" data-relation="required" reviewed="" }에서 주어졌던 diagram을 모두 Hopf monoid가 갖고 있는 정보로 옮겨야 하는데, 가령 한쪽 삼각형은 다음의 diagram
