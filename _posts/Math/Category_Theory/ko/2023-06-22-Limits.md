...
::: 정리 9
임의의 diagram $F:\mathcal{I}\rightarrow \mathcal{A}$가 limit을 갖는다면 다음 natural isomorphism
...
::: 명제 10
임의의 bifunctor $F: \mathcal{I}\times \mathcal{J}\rightarrow \mathcal{A}$에 대하여, 만일 두 limit
...
[^1]: 여기서 *diagram*이란 그냥 functor $F: \mathcal{I} \rightarrow \mathcal{A}$를 의미한다. 이 용어는 특별히 $\mathcal{I}$가 partially ordered set을 category로 보았을 때, functor $F$는 $I$를 index set으로 갖는 $\mathcal{A}$의 object들의 directed system을 만드는 것으로 생각할 수 있으므로 특히 직관적이다.  