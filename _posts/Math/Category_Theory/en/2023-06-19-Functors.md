...
In the same manner as [Example 4](#ex4){: data-lid="834gz" }, we might try to define a functor $\Hom_\mathcal{A}(-,A)$. However, as can be seen from the following diagram,
...
Under this definition, [Definition 5](#def5){: data-lid="g0wew" } can be restated as follows: a contravariant functor from $\mathcal{A}$ to $\mathcal{B}$ is simply a (covariant) functor $\mathcal{A}^\op \rightarrow \mathcal{B}$.
...
- Composition of morphisms in $\Cat$ is given by functor composition as in [Lemma 2](#lem2){: data-lid="w3871" }.
