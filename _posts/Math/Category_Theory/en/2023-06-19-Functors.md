...
In the same manner as [Example 4](#ex4){: data-relation="required" }, we might try to define a functor $\Hom_\mathcal{A}(-,A)$. However, as can be seen from the following diagram,
...
Under this definition, [Definition 5](#def5){: data-relation="required" } can be restated as follows: a contravariant functor from $\mathcal{A}$ to $\mathcal{B}$ is simply a (covariant) functor $\mathcal{A}^\op \rightarrow \mathcal{B}$.
