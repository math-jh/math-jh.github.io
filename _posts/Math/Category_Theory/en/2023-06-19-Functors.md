...
We have seen that any preordered set can be viewed as a category. ([§Category, ⁋Example 3](/en/math/category_theory/categories#ex3){: data-lid="zgs28" data-relation="required" reviewed="" }) The following example offers a new perspective on this. For a fixed category $\mathcal{I}$ and any category $\mathcal{A}$, we call a functor $F:\mathcal{I}\rightarrow \mathcal{A}$ an *$\mathcal{I}$-shaped diagram in $\mathcal{A}$*.
...
In the same manner as [Example 4](#ex4){: data-lid="834gz" data-relation="required" reviewed="" }, we might try to define a functor $\Hom_\mathcal{A}(-,A)$. However, as can be seen from the following diagram,
...
Because of this definition, the functor in [Definition 1](#def1){: data-lid="i8gvh" data-relation="weak" reviewed="" } is sometimes more precisely called a *covariant functor*. However, when we say "functor" we usually mean a covariant functor, because by the following definition every contravariant functor can be viewed as a covariant functor of a special kind.
...
Under this definition, [Definition 5](#def5){: data-lid="g0wew" data-relation="required" reviewed="" } can be restated as follows: a contravariant functor from $\mathcal{A}$ to $\mathcal{B}$ is simply a (covariant) functor $\mathcal{A}^\op \rightarrow \mathcal{B}$.
...
- Composition of morphisms in $\Cat$ is given by functor composition as in [Lemma 2](#lem2){: data-lid="w3871" data-relation="required" reviewed="" }.
