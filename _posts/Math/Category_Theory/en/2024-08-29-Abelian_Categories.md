...
::: Definition 1
A category $$\mathcal{A}$$ is called an *$$\Ab$$-category* if each $$\Hom_\mathcal{C}(A,B)$$ carries the structure of an abelian group, and addition in $$\Hom_\mathcal{C}(A,B)$$ distributes over composition. That is, for any $$g_1,g_2\in\Hom_\mathcal{C}(B,C)$$ and any $$f:A\rightarrow B$$ or $$h:C\rightarrow D$$,
...
::: Proposition 2
For any additive category $$\mathcal{A}$$ and any two objects $$A,B\in\obj(\mathcal{A})$$, the zero map $$0_{AB}$$ defined above is the additive identity in $$\Hom_\mathcal{A}(A,B)$$.
:::
::: Proof
...
::: Definition 3
An additive category $$\mathcal{A}$$ is called an *abelian category* if the following additional conditions are satisfied.
...
::: Definition 5
Suppose a chain complex $$A_\bullet$$ is given. Then
