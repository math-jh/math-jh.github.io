...
::: Example 3
Any preordered set $(S,\preceq)$ can be regarded as a category through the following process. ([\[Set Theory\] §Definition of Order Relations, ⁋Definition 7](/en/math/set_theory/order_relations#def7){: data-lid="viu7c" })
...
we know that $g=g'$. Therefore, given any $f\in\Hom_\mathcal{A}(A_1,A_2)$, if there exists a $g\in\Hom_\mathcal{A}(A_2,A_1)$ satisfying the two conditions of [Definition 6](#def6){: data-lid="pndn6" }, it is unique, and hence we can write it as $g=f^{-1}$.

In many examples, an isomorphism is the same as a bijective morphism, but this is not always the case. ([\[Topology\] §Continuous Functions, ⁋Example 5](/en/math/topology/continuous_functions#ex5){: data-lid="v4oeg" }) In the first place, there is no guarantee that morphisms in an arbitrary category are necessarily functions, so it does not even make sense to say that a morphism is a bijection. Instead, we define the following.
