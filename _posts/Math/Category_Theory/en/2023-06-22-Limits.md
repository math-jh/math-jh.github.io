...
the assignment $\Cone(-,F)$ becomes a contravariant functor from $\mathcal{A}$ to $\Set$. When $\mathcal{I}$ is the category arising from the poset $(\mathbb{Z},\leq)$, we can illustrate [Definition 1](#def1){: data-relation="required" } as follows.
...
::: Example 4
$\Set$ is a complete category. A key ingredient in proving this is the natural isomorphism from [§Representable Functors, ⁋Example 2](/en/math/category_theory/representable_functors#ex2){: data-relation="required" }:
...
To aid understanding, restricting this example to the case of limits of sets defined in [Example 4](#ex4){: data-relation="weak" }, this universal property is exactly the same as the universal property of the product of sets defined in [\[Set Theory\] §Product of Sets, ⁋Theorem 3](/en/math/set_theory/product_of_sets#thm3){: data-relation="weak" }.
...
By appropriately combining [Example 6](#ex6){: data-relation="required" } and [Example 7](#ex7){: data-relation="required" }, the limit of a diagram $\mathcal{I}\rightarrow \Set$ in $\Set$ can be written as a suitable equalizer diagram. As seen in [Example 4](#ex4){: data-relation="required" }, an element $\lambda\in\lim F=\Cone(\ast,F)$ of the limit cone is a family $(\lambda_i)_{i\in\mathcal{I}}$ making the following diagram commute for every morphism $f$ of $\mathcal{I}$:
...
as $\Hom_\mathcal{A}(A,F-)$. Then this is a diagram from $\mathcal{I}$ to $\Set$. By [Example 4](#ex4){: data-relation="required" }, since $\Set$ is a complete category, the limit $\lim \Hom_\mathcal{A}(A,F-)=\Cone(\ast,\Hom_\mathcal{A}(A,F-))$ of this diagram exists.
