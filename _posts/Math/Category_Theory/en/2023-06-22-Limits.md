...
the assignment $\Cone(-,F)$ becomes a contravariant functor from $\mathcal{A}$ to $\Set$. When $\mathcal{I}$ is the category arising from the poset $(\mathbb{Z},\leq)$, we can illustrate [Definition 1](#def1){: data-lid="jtz7v" } as follows.
...
Meanwhile, in [§Representable Functors, ⁋Proposition 8](/en/math/category_theory/representable_functors#prop8){: data-lid="k2xjt" } we examined how to express a representation via a universal property. Applying this to our situation, consider the category of elements of the functor $\Cone(-,F):\mathcal{A}\rightarrow \Set$:
...
In particular, rewriting the universal property of the limit cone $\pi:\prod F(i)\Rightarrow F$ from [Example 6](#ex6){: data-lid="7brby" } gives the following.
...
To aid understanding, restricting this example to the case of limits of sets defined in [Example 4](#ex4){: data-lid="m5c2x" }, this universal property is exactly the same as the universal property of the product of sets defined in [\[Set Theory\] §Product of Sets, ⁋Theorem 3](/en/math/set_theory/product_of_sets#thm3){: data-lid="zuyhm" }.
...
By appropriately combining [Example 6](#ex6){: data-lid="heuia" } and [Example 7](#ex7){: data-lid="waeam" }, the limit of a diagram $\mathcal{I}\rightarrow \Set$ in $\Set$ can be written as a suitable equalizer diagram. As seen in [Example 4](#ex4){: data-lid="af1pl" }, an element $\lambda\in\lim F=\Cone(\ast,F)$ of the limit cone is a family $(\lambda_i)_{i\in\mathcal{I}}$ making the following diagram commute for every morphism $f$ of $\mathcal{I}$:
...
as $\Hom_\mathcal{A}(A,F-)$. Then this is a diagram from $\mathcal{I}$ to $\Set$. By [Example 4](#ex4){: data-lid="1x933" }, since $\Set$ is a complete category, the limit $\lim \Hom_\mathcal{A}(A,F-)=\Cone(\ast,\Hom_\mathcal{A}(A,F-))$ of this diagram exists.
