...
and consider the localization $\mathbb{K}[X]_{\mathfrak{m}_x}=\mathcal{O}_{X,x}$ of $\mathbb{K}[X]$ at this maximal ideal. ([\[Commutative Algebra\] §Localization, ⁋Definition 4](/en/math/commutative_algebra/localization#def4){: data-relation="required" reviewed="" }) Geometrically, keeping [§Affine Varieties, ⁋Definition 14](/en/math/algebraic_varieties/affine_varieties#def14){: data-relation="weak" reviewed="" } in mind, these can be defined as germs of regular functions at the point $x$.
...
Here the third equality follows from [§Dimension, ⁋Proposition 2](/en/math/algebraic_varieties/dimension#prop2){: data-relation="required" reviewed="" }.

In general, for a Noetherian local ring $(R, \mathfrak{m})$, we have $\dim_{\mathbb{K}}(\mathfrak{m}/\mathfrak{m}^2) \ge \dim R$. ([\[Commutative Algebra\] §System of Parameters, ⁋Proposition 2](/en/math/commutative_algebra/system_of_parameters#prop2){: data-relation="required" reviewed="" }) Therefore,
...
::: Proof
In [Proposition 3](#prop3){: data-relation="required" reviewed="" } we showed that $\dim T_x X = n - \rank(J_x)$. By [Definition 5](#def5){: data-relation="required" reviewed="" }, $x$ is a smooth point if and only if $\dim T_x X = \dim X$. Hence $x$ is a smooth point if and only if
...
::: Proof
Let $X = Z(f_1, \ldots, f_k) \subseteq \mathbb{A}^n$ have dimension $\dim X = d$. By [Proposition 8](#prop8){: data-relation="required" reviewed="" },
...
More generally, by writing $f$ as a polynomial in the $\x_i-x_i$ and making the analogous definition, one can define the tangent cone at an arbitrary point. The reason this is called a cone is that, just as in [§Projective Varieties, ⁋Definition 12](/en/math/algebraic_varieties/projective_varieties#def12){: data-relation="weak" reviewed="" }, it is the zero set of a homogeneous ideal.

Let us now see how this gives a finer classification of singular points.

::: Example 14
For the nodal curve $X = Z(\y^2 - \x^2(\x+1))$ from [Example 7](#ex7){: data-relation="weak" reviewed="" }, the lowest degree term of $f$ is $\y^2 - \x^2 = (\y-\x)(\y+\x)$, so
...
::: Example 15
For the curve $X = Z(\y^2 - \x^3)$ from [Example 7](#ex7){: data-relation="weak" reviewed="" }, the lowest degree term of $f$ is $\y^2$, so
