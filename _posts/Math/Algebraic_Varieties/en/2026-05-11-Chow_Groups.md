...
::: Definition 1
An *algebraic $k$-cycle* on a variety $X$ is a formal sum
...
::: Definition 2
Let $X$ be a variety, $Y \subset X$ a $(k+1)$-dimensional closed irreducible subvariety, and $f \in \mathbb{K}(Y)^\ast$ a rational function on $Y$. The *principal cycle* $\divisor(f) \in Z_k(X)$ is defined by the formula
...
::: Definition 3
Two $k$-cycles $Z_1, Z_2$ are *rationally equivalent* if there exist $(k+1)$-dimensional closed irreducible subvarieties $Y_j$ of $X$ and rational functions $f_j \in \mathbb{K}(Y_j)^\ast$ on them such that
...
::: Proposition 6
For a proper morphism $f: X \to Y$, there exists a pushforward $f_\ast: \CH_k(X) \to \CH_k(Y)$. In particular, for any subvariety $V\subset X$,
...
::: Proposition 7
For a flat morphism $f: X \to Y$, there exists a pullback $f^\ast: \CH^k(Y) \to \CH^k(X)$. For a subvariety $V \subset Y$, we have $f^\ast[V] = [f^{-1}(V)]$.
:::
...
If $Z \subset X$ is a closed subvariety and $U = X \setminus Z$, then the following exact sequence holds:
...
::: Example 10
To see the above example more intuitively, define a degree $d$ morphism $f: \mathbb{P}^1 \to \mathbb{P}^1$ by
