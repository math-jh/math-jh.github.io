...
Of course, this is not always possible. For example, in [Example 3](#ex3){: data-lid="r3i9y" }, if we choose the following two bases corresponding to $(a_{00},a_{02})=(1,0), (0,1)$:
...
We know that $\mathcal{O}_{\mathbb{P}^n}(1)$ is very ample, but $\mathcal{O}_{\mathbb{P}^n}(-1)$ is not. As examined in [§Line Bundles and Vector Bundles, ⁋Example 16](/en/math/algebraic_varieties/line_bundles#ex16){: data-lid="gc1d4" }, this is because the twisting direction of $\mathcal{O}_{\mathbb{P}^n}(-1)$ as the fiber moves along the base does not allow sections to cross the zero section, so no global sections exist. On the other hand, the twisting of $\mathcal{O}_{\mathbb{P}^n}(1)$ allows this, giving rise to global sections.
