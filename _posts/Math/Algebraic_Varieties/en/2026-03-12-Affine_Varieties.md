...
Conversely, since the field $\mathbb{K}$ is Noetherian, applying [\[Commutative Algebra\] §Basic Notions, ⁋Theorem 12](/en/math/commutative_algebra/basic_notions#thm12){: data-lid="zl91x" data-relation="required" } repeatedly shows that $\mathbb{K}[\x_1,\ldots, \x_n]$ is also Noetherian, and therefore any ideal $\mathfrak{a}$ is generated by finitely many polynomials $f_1,\ldots, f_k$, so that $Z(\mathfrak{a})=Z(f_1,\ldots, f_k)$. Thus the affine algebraic sets in the sense of [Definition 2](#def2){: data-lid="ye0uk" data-relation="required" } and the zero sets of ideals form exactly the same collection, so we need only consider affine algebraic sets defined by ideals $\mathfrak{a}$.
...
4. The affine space $\mathbb{A}^n$ itself and the empty set are affine varieties. This is obvious from $Z(0)=\mathbb{A}^n$ and $Z(1)=\emptyset$. This will be used importantly when defining the Zariski topology in [Proposition 4](#prop4){: data-lid="3xwzd" data-relation="forward" }.
:::
...
The following proposition shows that principal open sets form a base for the affine variety. ([\[Topology\] §Bases of a Topological Space, ⁋Definition 1](/en/math/topology/topological_bases#def1){: data-lid="8bagw" data-relation="weak" })
...
For example, we showed in [Example 3](#ex3){: data-lid="sn7ld" data-relation="weak" } that the twisted cubic corresponds to $\mathbb{A}^1$ via $t\mapsto (t,t^2,t^3)$, and the above definition shows that this is a morphism between affine varieties.
