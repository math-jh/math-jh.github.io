...
date: 2026-03-12
weight: 1
translated_at: 2026-08-17T07:17:25+00:00
...
Conversely, since the field $\mathbb{K}$ is Noetherian, applying [\[Commutative Algebra\] §Basic Notions, ⁋Theorem 12](/en/math/commutative_algebra/basic_notions#thm12) repeatedly shows that $\mathbb{K}[\x_1,\ldots, \x_n]$ is also Noetherian, and therefore any ideal $\mathfrak{a}$ is generated by finitely many polynomials $f_1,\ldots, f_k$, so that $Z(\mathfrak{a})=Z(f_1,\ldots, f_k)$ holds. That is, the affine algebraic sets in the sense of [Definition 2](#def2) and the zero sets of ideals form exactly the same collection, so we need only care about affine algebraic sets defined by ideals $\mathfrak{a}$.

In general, a space $X$ is said to be *irreducible* if it cannot be written as the union of two proper closed subsets. ([\[Topology\] §Dimension, ⁋Definition 6](/en/math/topology/dimension#def6)) Thus our definition means that we call an irreducible affine algebraic set an affine variety. This is to ensure that we deal with a single object that does not break into several pieces.
...
In the example above, we saw that familiar geometric objects can all be written as affine algebraic sets as sets. However, to think of them as geometric objects, there must be a topological structure on them. Our only tool is polynomials, so we will use them to define a topology.
...
First, the last result of the above proposition shows that for $Z(\mathfrak{a}\mathfrak{b})$ to be an affine variety, one must have either $Z(\mathfrak{a})\subseteq Z(\mathfrak{b})$ or $Z(\mathfrak{b})\subseteq Z(\mathfrak{a})$. This becomes one of the good intuitions for understanding algebraically what an affine variety is.
...
is a well-defined function. Here, by saying this function is well-defined, we mean that the value $f(x)$ is unique regardless of the choice of representative of $\overline{f}$, and this is possible precisely because $X$ is defined as the zero set of $I(X)$.
