...
::: Definition 6
For a rational map $\varphi: X\dashrightarrow Y$, we denote by $\dom(\varphi)$ the open set obtained by the above procedure.
:::

::: Example 7
One of the typical examples of a rational map is the projection from a point. For instance, considering the line $\{\x_2=0\}$ in $\mathbb{P}^2$, we may think of this as a projective line $\mathbb{P}^1$ inside $\mathbb{P}^2$. Now the point $[0:0:1]$ does not lie on this line, and the equation of the line joining this point and an arbitrary point $[x_0:x_1:x_2]$ is
...
::: Definition 8
A rational map $\varphi: X \dashrightarrow Y$ is said to be *dominant* if its image is dense in $Y$. That is, $\overline{\varphi(X)} = Y$ holds.
:::
