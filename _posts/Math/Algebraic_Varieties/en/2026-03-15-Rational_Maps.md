...
Now consider the quadric surface $Q = V(\x\y - \z\w) \subseteq \mathbb{P}^3$. Again by [Proposition 3](#prop3){: data-relation="required" } it suffices to compute on the affine patch $\{\w \ne 0\}$. On this patch set $\x' = \x/\w$, $\y' = \y/\w$, $\z' = \z/\w$; then the equation $\x\y - \z\w = 0$ becomes $\x'\y' - \z' = 0$. Hence $\z' = \x'\y'$, and the coordinate ring of this patch is $\mathbb{K}[\x', \y', \z']/(\x'\y' - \z') \cong \mathbb{K}[\x', \y']$. By [Proposition 2](#prop2){: data-relation="required" } we have $\mathbb{K}(Q) = \Frac(\mathbb{K}[\x', \y']) = \mathbb{K}(\x', \y') \cong \mathbb{K}(\t_1, \t_2)$.
