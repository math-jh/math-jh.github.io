...
::: Definition 1
The *projective $n$-space* $\mathbb{P}^n_{\mathbb{K}}$ over a field $\mathbb{K}$ is defined as follows. As a set,
...
::: Definition 3
For homogeneous polynomials $F_1, \ldots, F_k \in \mathbb{K}[\x_0, \ldots, \x_n]$, the *projective algebraic set* $Z(F_1, \ldots, F_k)$ is defined by
...
::: Proposition 9
Each $U_i$ is homeomorphic (in the subspace topology) to affine space $\mathbb{A}^n$.
:::
...
::: Proposition 10
For a projective variety $X \subseteq \mathbb{P}^n$ and a standard open set $U_i$, the intersection $X \cap U_i$ is an affine variety in $U_i \cong \mathbb{A}^n$.
:::
...
::: Example 16
First, the *Veronese embedding* (of degree 2) from $\mathbb{P}^1$ to $\mathbb{P}^2$,
