...
::: Definition 5
Let a projective variety $X$ and an ample line bundle $\mathcal{L}$ on it be fixed. Then a coherent sheaf $\mathcal{F}$ on $X$ is said to be *$m$-regular* if for all $i>0$,
...
::: Proposition 10
For an ample line bundle $\mathcal{L}$ defined on a projective variety $X$ and an arbitrary line bundle $\mathcal{M}$, for sufficiently large $n$ the sheaf $\mathcal{M} \otimes \mathcal{L}^{\otimes n}$ is very ample.
:::
