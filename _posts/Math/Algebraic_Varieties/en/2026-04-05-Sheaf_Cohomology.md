...
::: Definition 3
Given a topological space $$X$$, an open cover $$\mathcal{U} = \{U_i\}_{i \in I}$$, and a sheaf $$\mathcal{F}$$, fix an arbitrary total order $$<$$ on $$I$$. Then the *Čech complex* $$C^\bullet(\mathcal{U}, \mathcal{F})$$ of these data is defined as follows:
...
::: Definition 4
We define the *Čech cohomology* $$\check{H}^p(\mathcal{U}, \mathcal{F})$$ of the above data as the cohomology of the Čech complex:
...
Given a $$\Gamma(X, -)$$-acyclic resolution $$0 \to \mathcal{F} \to \mathcal{A}^0 \to \mathcal{A}^1 \to \cdots$$,
...
A *Cartan-Eilenberg resolution* of a cochain complex $$K^\bullet$$ in an abelian category is data consisting of a double complex $$I^{p,q}$$ and an augmentation $$K^\bullet \to I^{\bullet,0}$$, satisfying the following conditions:
...
For a continuous function $$f : X \to Y$$ and a sheaf $$\mathcal{F}$$, there exists a spectral sequence with the following $$E_2$$ page:
...
For a continuous function $$f : X \to Y$$ and a sheaf $$\mathcal{F}$$, from the Leray spectral sequence we obtain the exact sequence
