...
First, let us verify that $\Hom_{\Sh(X)}(i^U_!\mathbb{Z}_U, \mathcal{F}) \cong \mathcal{F}(U)$. Since extension by zero $i^U_!$ is left adjoint to restriction $\mathcal{G} \mapsto \mathcal{G}\vert_U$ ([\[Topology\] §Sheaves, ⁋Example 14](/en/math/topology/sheaves#ex14){: data-relation="required" }),
