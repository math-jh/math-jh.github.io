...
description: "Moving beyond global sections of vector bundles, we define sheaf cohomology using derived functors to capture the finer information of a sheaf, and explore its properties."
...
While sheaves are a powerful tool for systematically describing all the information on a topological space, in our discussion so far sheaves have appeared in full force only in [§Linear Systems](/en/math/algebraic_varieties/linear_systems), when we examined that the global section space $\Gamma(X, \mathcal{L})$ determines the projective embedding of the complete linear system.

However, if global sections were our only concern, we would not need to think about sheaves at all—it would have sufficed to consider only the global section functor. In fact, the global section functor does not capture all the information that a sheaf carries. Consider, for example, the global section functor

$$\Gamma(X, -): \QCoh(X) \rightarrow \Vect_\mathbb{K}; \qquad \mathcal{F} \mapsto \mathcal{F}(X).$$

Our motivation for defining quasi-coherent sheaves in [§Canonical Bundle, ⁋Definition 1](/en/math/algebraic_varieties/canonical_bundle#def1) was that the category $\Bun(X)$ of vector bundles is not an abelian category, so we wanted to consider a larger category where kernels and cokernels exist; from this perspective it is not surprising that $\QCoh(X)$ becomes an abelian category. [^1]
...
But as we saw in [§Line Bundles and Vector Bundles, ⁋Example 16](/en/math/algebraic_varieties/line_bundles#ex16), the only global section of $\mathcal{O}_{\mathbb{P}^n}(-1)$ is $0$, so
...
Intuitively, Čech cohomology $\check{H}^i(X, \mathcal{F})$ is a tool that measures the failure of gluing local information. That is, $\check{H}^0(X, \mathcal{F})$ is exactly the global section space, and $\check{H}^1(X, \mathcal{F})$ tells us how much the process of patching local sections together to obtain a global section fails. To make this precise, we start with the following.

::: Definition 3
Let $X$ be a topological space with an open cover $\mathcal{U} = \{U_i\}_{i \in I}$, and let $\mathcal{F}$ be a sheaf on $X$. Fix an arbitrary total order $<$ on $I$. Then the *Čech complex* $C^\bullet(\mathcal{U}, \mathcal{F})$ of this data is defined as follows:
...
Hence a nontrivial element of $\check{H}^1(\mathcal{U}, \mathcal{F})$ reflects the discrepancy that appears when trying to patch these three pieces of data $s_{ij}, s_{jk}, s_{ik}$ together, and this is precisely the failure of gluing mentioned above.
:::

So far we have defined Čech cohomology $\check{H}^p(\mathcal{U}, \mathcal{F})$ for a single open cover $\mathcal{U}$. However, different open covers can generally give different Čech cohomologies. For example, for the cover consisting of a single open set $U_0 = X$, all intersections are $X$, so $\check{H}^p$ is nonzero only for $p = 0$. The finer the cover, the more topological information it captures, so we need to understand the relationship between open covers and combine the information from all of them. That is, we put an order relation on *all* open covers using refinement. Then for a refinement $\mathcal{V} \preceq \mathcal{U}$, it is obvious that a natural map $\check{H}^p(\mathcal{U}, \mathcal{F}) \rightarrow \check{H}^p(\mathcal{V}, \mathcal{F})$ exists, and thus we can define a direct system $\check{H}^p(\mathcal{U}, \mathcal{F})$ indexed by all open covers. From this we define the following.
...
In simpler terms, the above argument means that we take cohomology data over increasingly finer open covers and combine all of it to define $\check{H}(X, \mathcal{F})$.

In general, it is not guaranteed that $\check{H}^p(X, \mathcal{F})$ from [Definition 7](#def7) is isomorphic to $H^p(X, \mathcal{F})$ from [Definition 1](#def1), but fortunately for most sheaves appearing in algebraic geometry the two coincide. Proving this requires some technical machinery.
...
3. If for every open subset $V\subseteq U$ the restriction map $$\mathcal{F}(U)
\rightarrow \mathcal{F}(V)$$ is surjective, then $\mathcal{F}$ is called a *flasque sheaf*.
:::

Of course, the condition we want at the cohomology level is the first one. Let us first examine the relationships among the above notions.
...
First, let us verify that $\Hom_{\Sh(X)}(i^U_!\mathbb{Z}_U, \mathcal{F}) \cong \mathcal{F}(U)$. Since extension by zero $i^U_!$ is left adjoint to the restriction $\mathcal{G} \mapsto \mathcal{G}\vert_U$ ([Topology §Sheaves, ⁋Example 14](/en/math/topology/sheaves#ex14)), we have

$$\Hom_{\Sh(X)}(i^U_!\mathbb{Z}_U, \mathcal{F}) \cong \Hom_{\Sh(U)}(\mathbb{Z}_U, \mathcal{F}\vert_U).$$

Now $\mathbb{Z}_U$ is the constant sheaf on $U$, so for any open subset $W \subseteq U$ we have $\mathbb{Z}_U(W) = \mathbb{Z}$, and every section is given by the restriction of a constant function. Therefore a sheaf morphism $\varphi: \mathbb{Z}_U \rightarrow \mathcal{F}\vert_U$ is completely determined by the image of the global section $\varphi_U(1) \in \mathcal{F}(U)$. Conversely, for any $s \in \mathcal{F}(U)$, defining $n \mapsto n \cdot s\vert_W$ on each $W \subseteq U$ gives a well-defined sheaf morphism. Hence

$$\Hom_{\Sh(U)}(\mathbb{Z}_U, \mathcal{F}\vert_U) \cong \Hom_{\Ab}(\mathbb{Z}, \mathcal{F}(U)) \cong \mathcal{F}(U).$$

Similarly $\Hom_{\Sh(X)}(i^V_!\mathbb{Z}_V, \mathcal{F}) \cong \mathcal{F}(V)$, and by naturality the map between these coincides exactly with the restriction $\mathcal{F}(U)\rightarrow \mathcal{F}(V)$. Now the assumption that $\mathcal{F}$ is injective implies that this is surjective, completing the proof.
:::
...
What we need to show is that this is exact for $p>0$, so it suffices to show that the identity chain map is nullhomotopic. To do this, for each $p\geq 1$ we must explicitly construct a homotopy operator $s^p : \check{C}^p(\mathcal{U}, \mathcal{F}) \rightarrow \check{C}^{p-1}(\mathcal{U}, \mathcal{F})$.
...
Essentially, what we want to do—as is always the case when defining a chain homotopy—is to fix an index $i_0$ and insert it into $j_0<\cdots< j_{p-1}$ (for convenience, assume $i_0< j_0<\cdots< j_{p-1}$), and define

$$s^p(t)_{j_0<\cdots< j_{p-1}}=t_{i_0< j_0<\cdots < j_{p-1}}\tag{$\ast$}.$$

Here, $s^p(t)_{j_0<\cdots< j_{p-1}}$ is by definition a section over $U_{j_0}\cap\cdots\cap U_{j_{p-1}}$, but note that the right-hand side $t_{i_0< j_0<\cdots < j_{p-1}}$ is a section over the smaller set $U_{i_0}\cap U_{j_0}\cap\cdots\cap U_{j_{p-1}}$. For a general $\mathcal{F}$ this would be impossible, but since we are assuming $\mathcal{F}$ is flasque, we can always extend this function to make it a section over $U_{j_0}\cap\cdots\cap U_{j_{p-1}}$, and equation ($\ast$) should be understood in this way. Then one can check by direct calculation that $s^p$ defined in this way is indeed a chain homotopy: the term in $d^{p-1}s^p$ that omits $i_0$ and the term in $s^{p+1}d^p$ that inserts $i_0$ cancel each other out.

A slight technical issue is that the fixed index $i_0$ might already be contained in $j_0<\cdots< j_{p-1}$. To handle this, instead of the usual Čech complex we use the *non-alternating* Čech complex, which takes coordinates from $p+1$ elements $i_0,\ldots, i_p\in I$ of $I$ without any ordering condition. This is quasi-isomorphic to the original Čech complex, so this detour is justified.
:::

::: Theorem 11 (Leray)
Let $\mathcal{F}$ be a sheaf on a topological space $X$, and let $\mathcal{U} = \{U_i\}$ be an open cover. If $\mathcal{F}$ is acyclic on every finite intersection
...
Then the only obstacle to our intuition is how restrictive this acyclicity condition is, but fortunately it is a more generous condition than one might think.

::: Proposition 12
For a quasi-coherent sheaf $\mathcal{F} = \widetilde{M}$ on an affine variety $X$, we have $H^i(X, \mathcal{F}) = 0$ for all $i > 0$.
:::

The proof of this is as follows: letting $A$ be the coordinate ring of $X$, if we find an injective resolution $I^\bullet$ of $M$ in the category $\lMod{A}$, this gives a resolution $\widetilde{I^\bullet}$ of sheaves on $X$, and the sheaf given by an injective module is always flasque, hence acyclic.

Now consider an arbitrary variety $X$ and a quasi-coherent sheaf $\mathcal{F}$ defined on it, and let $\mathcal{U}$ be an affine open cover of $X$. For these data to satisfy the hypotheses of [Theorem 11](#thm11), every finite intersection of elements of $\mathcal{U}$ must again be affine. If the diagonal

$$\Delta_X\hookrightarrow X\times X$$

is a *closed* immersion in $X\times X$, then we can show that this condition holds, and in this case $X$ is called a *separated* variety. As can be seen from its definition, this is the Zariski-topology analogue of the Hausdorff condition, and it is a reasonable condition; moreover, if by "variety" we mean quasi-projective variety as in our current definition, this condition is automatically satisfied. That is, in our present language this argument shows that Čech cohomology and sheaf cohomology agree for any quasi-coherent sheaf on an arbitrary variety, and moreover, if we pick an open cover $\mathcal{U}$ satisfying the hypotheses of [Theorem 11](#thm11), it suffices to compute the Čech cohomology for that cover without taking a direct limit.

## Godement Resolution

In [Definition 1](#def1) we defined sheaf cohomology via injective resolution, but since directly computing an injective resolution is generally difficult, we examined one solution to this problem: using the result [Theorem 11](#thm11) that Čech cohomology and sheaf cohomology are isomorphic.

The Godement resolution, which we examine in this section, also starts from the same problem. That is, computing sheaf cohomology in general is a very complicated task, so [Definition 1](#def1), while conceptually clean, is somewhat lacking in practicality. We now define a concrete resolution. It is not an injective resolution, but it is a flasque resolution, and for our purposes this is sufficient.
...
for each open subset $U \subseteq X$. Here $\mathcal{F}_x$ is the stalk of $\mathcal{F}$ at $x$.
:::

Then for each $x\in X$, from the identity $\mathcal{F}_x\rightarrow \mathcal{F}_x$ on the stalk we obtain a canonical morphism $\mathcal{F}\rightarrow C^0(\mathcal{F})$. Also, that $C^0(\mathcal{F})$ is a sheaf is almost obvious from the definition.
...
::: Proof
First, let us show that the given sheaf is flasque. For open subsets $V \subseteq U$, the restriction map $C^0(\mathcal{F})(U) = \prod_{x \in U} \mathcal{F}_x \rightarrow \prod_{x \in V} \mathcal{F}_x = C^0(\mathcal{F})(V)$ is a projection, hence surjective. Therefore $C^0(\mathcal{F})$ is flasque.

Exactness is obvious because the stalk functor $\mathcal{F} \mapsto \mathcal{F}_x$ is exact and $C^0(\mathcal{F})$ is nothing but a product of stalks.
:::
...
::: Proof
We proceed by induction on $i$. First, consider the case $i=1$. To show this, we embed $\mathcal{F}$ into an injective sheaf $\mathcal{I}$ and consider the cokernel exact sequence

$$0 \rightarrow \mathcal{F}\rightarrow\mathcal{I}\rightarrow\mathcal{Q}\rightarrow0.$$

Our claim is that $\mathcal{Q}$ is flasque, which can be shown by a diagram chase in the following commutative diagram for arbitrary open subsets $V\subseteq U$:

{% diagram Math/Algebraic_Varieties/Sheaf_Cohomology-1.svg width="23.07em" alt="Commutative diagram" %}

Here $\mathcal{F}$ is flasque by assumption and $\mathcal{I}$ is injective, hence flasque. Now for arbitrary $s\in \mathcal{Q}(V)$, since $\mathcal{I}(V)\rightarrow \mathcal{Q}(V)$ is surjective we can lift $s$ to $t\in \mathcal{I}(V)$, and then using that $\mathcal{I}$ is flasque we can lift $t$ to $\overline{t}\in\mathcal{I}(U)$ and push this down to $\mathcal{Q}$ to define $\overline{s}\in \mathcal{Q}(U)$. Since $\overline{t}$ is an extension of $t$, we have $\overline{t}\vert_V=t$, and therefore $\overline{s}\vert_V$ equals the image of $t$ in $\mathcal{Q}(V)$, which is $s$. Thus $\overline{s}$ restricts exactly to $s\in \mathcal{Q}(V)$, and from this we obtain the flasqueness of $\mathcal{Q}$.
...
Here $H^1(X, \mathcal{I}) = 0$ because $\mathcal{I}$ is injective. Hence

$$H^1(X, \mathcal{F}) \cong \coker(\Gamma(X, \mathcal{I}) \rightarrow \Gamma(X, \mathcal{Q})),$$

and to show that this is $0$ we must show that $\Gamma(X, \mathcal{I})\rightarrow \Gamma(X, \mathcal{Q})$ is surjective. So let $s\in \Gamma(X, \mathcal{Q})$ be given. Then for any $x\in X$, since $\mathcal{I}\rightarrow \mathcal{Q}$ is surjective at the stalk level, for each $x\in X$ there exists a suitable $t_x\in \mathcal{I}_x$ such that $t_x$ maps to $s_x\in \mathcal{Q}_x$. Now choose a representative of $t_x$ over a suitable neighborhood $U_x$ of $x$, and think of $t_x$ as an element of $\mathcal{I}(U_x)$; then since $\mathcal{I}$ is flasque we can extend each of these to global sections $T_x$ on $X$, and we have $T_x\mid_{U_x}=s\mid_{U_x}$.

Now let $S_x$ be the image of $T_x$ in $\Gamma(X,\mathcal{Q})$. Then $S_x-S_y$ is identically zero on $U_x\cap U_y$, so we can lift this to a section $f_{xy}$ of $\mathcal{F}$ over $U_x\cap U_y$. Again using the flasqueness of $\mathcal{F}$, we can extend this to $f_x\in \mathcal{F}(U_x)$ and $f_y\in \mathcal{F}(U_y)$ respectively, and then replacing $T_x$ by $T'_x=T_x-f_x$ in this situation satisfies the compatibility condition, so we know that gluing these gives a preimage of $s$.

Finally, from the long exact sequence we have
...
In particular, by [Proposition 16](#prop16), each term $\mathcal{G}^p(\mathcal{F})$ of the Godement resolution is flasque, hence $\Gamma(X, -)$-acyclic. That is, $H^i(X, \mathcal{G}^p(\mathcal{F})) = 0$ for all $i > 0$. To reach the conclusion, the result we need is the following.
...
Now consider the Godement resolution $\mathcal{G}^\bullet(\mathcal{F})$ of $\mathcal{F}$. Intuitively, what we want to do is to take an injective resolution of each $\mathcal{G}^p(\mathcal{F})$, and then define the horizontal differential using the differential $\mathcal{G}^p(\mathcal{F})\rightarrow \mathcal{G}^{p+1}(\mathcal{F})$ of the Godement resolution via [Homological Algebra §Resolutions, ⁋Theorem 6](/en/math/homological_algebra/resolutions#thm6).
...
Now consider the spectral sequence coming from the filtration in the $q$ direction. Its $E_1$ page is

$$E_1^{p,q} = H^q(\mathcal{I}^{p,\bullet}).$$

Here, for each $p$, since $\mathcal{I}^{p,\bullet}$ is an injective resolution of $f_\ast \mathcal{G}^p(\mathcal{F})$, by the exactness of injective resolutions we have
...
which by the definition of $R^q f_\ast$ is given by
...
with the differential being the Godement differential. Therefore the $E_2$ page is
...
For a continuous map $f : X \rightarrow Y$ and a sheaf $\mathcal{F}$, there exists a spectral sequence with $E_2$ page
...
To complete the proof, now look at the components $E_2^{2,0}$, $E_2^{1,1}$, $E_2^{0,2}$ with $p+q = 2$. For the same reason, the only possibly nontrivial differential is $d_2 : E_2^{0,1} \rightarrow E_2^{2,0}$, and on the $E_3$ page defined by this differential we have

$$E_3^{0,2} = \ker(d_2 : E_2^{0,2} \rightarrow E_2^{2,1}), \qquad E_3^{2,0} = \coker(d_2 : E_2^{0,1} \rightarrow E_2^{2,0}),$$

and again analyzing degrees we have $E_3^{p,q} = E_\infty^{p,q}$, so
...
so putting this into $F^2 H^2 \hookrightarrow H^2(X, \mathcal{F})$ via the filtration gives that

$$E_2^{0,1} \overset{d_2}{\rightarrow} E_2^{2,0} \rightarrow H^2(X, \mathcal{F})$$

is exact. Combining these yields the desired result.
:::
...
Conversely, any line bundle $\mathcal{L}$ can be expressed by transition functions $g_{ij}$ over a suitable open cover $\mathcal{U}$ by [§Line Bundles and Vector Bundles, ⁋Definition 1](/en/math/algebraic_varieties/line_bundles#def1), and these form a Čech 1-cocycle. A line bundle isomorphism corresponds exactly to the equivalence relation by coboundaries, so the kernel of this map consists of coboundaries. Hence $\check{H}^1(\mathcal{U}, \mathcal{O}_X^\times) \rightarrow \Pic(X)$ is injective. Now taking the direct limit gives $\check{H}^1(X, \mathcal{O}_X^\times) \cong \Pic(X)$.
:::
...
[^1]: More generally, as we saw in [Topology §Sheaves, §§The Abelian Category of Sheaves](/en/math/topology/sheaves#the-abelian-category-of-sheaves), the category $\Sh(X)$ of sheaves on an arbitrary topological space $X$ is an abelian category.
