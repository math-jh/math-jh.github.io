...
date: 2026-04-05
weight: 13
translated_at: 2026-08-16T01:20:46+00:00
translation_source: kimi-cli
---
We have verified that line bundles can be used to construct various invariants. For instance, in [§Line Bundles and Vector Bundles](/en/math/algebraic_varieties/line_bundles) we defined the global section space $\Gamma(X, \mathcal{L})$ of a line bundle $\mathcal{L}$. In particular, in [§Linear Systems, ⁋Definition 9](/en/math/algebraic_varieties/linear_systems#def9) we saw that its dimension plays a key role in determining the dimension of the complete linear system, and hence the projective embedding of the variety.

So far we have mainly used the language of line bundles for geometric intuition, but as we observed right after [§Canonical Line Bundle, ⁋Definition 1](/en/math/algebraic_varieties/canonical_bundle#def1), if we think in terms of the section sheaf of a line bundle, this can fundamentally be rephrased in the language of sheaves. In this post we define the notion of sheaf cohomology.

## Definition as a Derived Functor

While sheaves are a powerful tool for systematically encoding all the information on a topological space, they have appeared front and center in our discussion only once: in [§Linear Systems](/en/math/algebraic_varieties/linear_systems), when we examined how the global section space $\Gamma(X, \mathcal{L})$ determines the projective embedding of a complete linear system.
...
::: Definition 1
For a sheaf $\mathcal{F}$ on a variety $X$, the $i$th *sheaf cohomology* $H^i(X, \mathcal{F})$ is defined by
...
The following proposition is also a standard consequence of homological algebra. ([\[Homological Algebra\] §Derived Functors](/en/math/homological_algebra/derived_functors))
...
::: Definition 3
Let $X$ be a topological space with an open cover $\mathcal{U} = \{U_i\}_{i \in I}$, let $\mathcal{F}$ be a sheaf on $X$, and fix an arbitrary total order $<$ on $I$. Then the *Čech complex* $\check{C}^\bullet(\mathcal{U}, \mathcal{F})$ of this data is defined as follows.
...
where $\hat{i_k}$ means the index $i_k$ is omitted.
:::

For this definition to be well-defined, that is, for $\check{C}^\bullet(\mathcal{U}, \mathcal{F})$ to actually be a complex, the coboundary map must satisfy $d^2=0$. This can be checked directly by expanding the above formula and observing the sign cancellations. Consequently $\check{C}^\bullet(\mathcal{U}, \mathcal{F})$ is a cochain complex, and thus we can define the following.

::: Definition 4
The *Čech cohomology* $\check{H}^p(\mathcal{U}, \mathcal{F})$ determined by the above data is defined as the cohomology of the Čech complex:

$$\check{H}^p(\mathcal{U}, \mathcal{F}) = H^p(\check{C}^\bullet(\mathcal{U}, \mathcal{F}))$$
:::

We said earlier that Čech cohomology measures the failure of gluing, and this is encoded in the coboundary map. Let us examine the intuitive meaning of the coboundary map in low degrees $p = 0, 1$.
...
By the gluing condition of [\[Topology\] §Sheaves, ⁋Definition 1](/en/math/topology/sheaves#def1), such a family of sections coincides exactly with a section over all of $X$, namely $\Gamma(X, \mathcal{F})$. That is, $\check{H}^0(\mathcal{U}, \mathcal{F}) = H^0(X, \mathcal{F})$, and this is independent of the choice of open cover.
:::
...
Hence a nontrivial element of $\check{H}^1(\mathcal{U}, \mathcal{F})$ reflects the discrepancy that appears when trying to glue these three data $s_{ij}, s_{jk}, s_{ik}$ together, and this is precisely the failure of gluing mentioned above.
:::

So far we have defined Čech cohomology $\check{H}^p(\mathcal{U}, \mathcal{F})$ for a single open cover $\mathcal{U}$. However, different open covers can give different Čech cohomology groups in general. For example, for the cover consisting of a single open set $U_0 = X$, all intersections are $X$, so $\check{H}^p$ is nonzero only for $p = 0$. The finer the cover, the more topological information we can capture, so we need to understand the relationship between open covers and synthesize the information over all of them. That is, we impose an order relation on *all* open covers using refinement. Then for a refinement $\mathcal{V} = \{V_j\}_{j \in J} \preceq \mathcal{U} = \{U_i\}_{i \in I}$, choosing a function $\lambda: J \rightarrow I$ such that $V_j \subseteq U_{\lambda(j)}$ for each $j$ yields a cochain-level map $\lambda^\ast: \check{C}^p(\mathcal{U}, \mathcal{F}) \rightarrow \check{C}^p(\mathcal{V}, \mathcal{F})$ defined by
...
gives a chain homotopy between $\lambda^\ast$ and $\mu^\ast$, so at the cohomology level a single map $\check{H}^p(\mathcal{U}, \mathcal{F}) \rightarrow \check{H}^p(\mathcal{V}, \mathcal{F})$ is determined independently of the choice. For the same reason these maps are compatible with composition of refinements, and hence we can define a direct system $\check{H}^p(\mathcal{U}, \mathcal{F})$ indexed by all open covers. From this we define the following.

::: Definition 7
The *Čech cohomology* of $X$ is defined as the direct limit over all open covers:

$$\check{H}^p(X, \mathcal{F}) = \varinjlim_{\mathcal{U}} \check{H}^p(\mathcal{U}, \mathcal{F})$$
:::

To explain the above argument more simply, the meaning is that we take increasingly finer open covers and collect all the additional cohomology data that appears, defining this as $\check{H}(X, \mathcal{F})$.

In general, it is not guaranteed that the $\check{H}^p(X, \mathcal{F})$ of [Definition 7](#def7) is isomorphic to the $H^p(X, \mathcal{F})$ of [Definition 1](#def1), but fortunately for most sheaves that arise in algebraic geometry the two agree. Showing this requires some technical machinery.

::: Definition 8
For a sheaf $\mathcal{F}$ on a variety $X$, we define the following.

1. $\mathcal{F}$ is called *acyclic* if $H^i(X, \mathcal{F}) = 0$ for all $i > 0$.
2. An injective object of $\Sh(X)$ is called an *injective sheaf*.
3. If the restriction map $\mathcal{F}(U) \rightarrow \mathcal{F}(V)$ is surjective for every open subset $V\subseteq U$, then $\mathcal{F}$ is called a *flasque sheaf*.
:::

Of course, the condition we want at the cohomology level is the first one. We first examine the relationships among the above notions.
...
::: Proof
By definition, $\mathcal{F}$ being injective means that for every monomorphism $\mathcal{A} \hookrightarrow \mathcal{B}$, the map $\Hom_{\Sh(X)}(\mathcal{B}, \mathcal{F}) \rightarrow \Hom_{\Sh(X)}(\mathcal{A}, \mathcal{F})$ is surjective. ([[Homological Algebra] §Resolutions, ⁋Definition 1](/en/math/homological_algebra/resolutions#def1)) Now we show that for any open sets $V \subseteq U \subseteq X$, the restriction $\mathcal{F}(U) \rightarrow \mathcal{F}(V)$ is surjective.

This map is not a sheaf morphism but a morphism of abelian groups, and the tools we have are sheaf morphisms, so we must recast this condition in terms of sheaf morphisms. To do this, we introduce the open embeddings

$$i^U: U \hookrightarrow X,\qquad i^V: V \hookrightarrow X$$

and the sheaves obtained from their extensions by zero, $i^U_!\mathbb{Z}_U, i^V_!\mathbb{Z}_V$. Here $\mathbb{Z}_U, \mathbb{Z}_V$ are constant sheaves, and since $V \subseteq U$ by assumption, there is a natural monomorphism $i^V_!\mathbb{Z}_V \rightarrow i^U_!\mathbb{Z}_U$.
...
holds. But the restriction maps of $\underline{\mathbb{Z}}$ are all identities on $\mathbb{Z}$, so a presheaf morphism $\varphi: \underline{\mathbb{Z}} \rightarrow \mathcal{F}\vert_U$ must satisfy $\varphi_W(1) = \varphi_U(1)\vert_W$ for each $W \subseteq U$, and hence is completely determined by $\varphi_U(1) \in \mathcal{F}(U)$. Conversely, for any $s \in \mathcal{F}(U)$, defining $n \mapsto n \cdot s\vert_W$ for each $W \subseteq U$ gives a presheaf morphism. Therefore

$$\Hom_{\Sh(U)}(\mathbb{Z}_U, \mathcal{F}\vert_U) \cong \mathcal{F}(U)$$

holds. Similarly $\Hom_{\Sh(X)}(i^V_!\mathbb{Z}_V, \mathcal{F}) \cong \mathcal{F}(V)$, and now by naturality we know that the map between them coincides exactly with the restriction $\mathcal{F}(U)\rightarrow \mathcal{F}(V)$. Since $\mathcal{F}$ is injective by assumption, this is surjective, completing the proof.
:::
...
::: Proof
Let us think of the Čech complex at the sheaf level. That is, for each open set $V \subseteq X$, defining the sheaf $\mathcal{C}^p$ by

$$\mathcal{C}^p(V) = \prod_{i_0 < \cdots < i_p} \mathcal{F}(V \cap U_{i_0} \cap \cdots \cap U_{i_p})$$

we have $\Gamma(X, \mathcal{C}^p) = \check{C}^p(\mathcal{U}, \mathcal{F})$ by definition, and the Čech coboundary map is defined by the same formula at each $V$, giving a sheaf morphism $\mathcal{C}^p \rightarrow \mathcal{C}^{p+1}$. Also, taking the collection of restrictions $\epsilon : \mathcal{F} \rightarrow \mathcal{C}^0$ as augmentation, we obtain the complex

$$0 \rightarrow \mathcal{F} \xrightarrow{\epsilon} \mathcal{C}^0 \xrightarrow{d^0} \mathcal{C}^1 \xrightarrow{d^1} \cdots$$

What we need to show is that applying $\Gamma(X,-)$ to this gives an exact sequence at $p>0$.

First, let us show that this complex itself is exact. Fix an index $i_0 \in I$ and consider only open sets $V \subseteq U_{i_0}$. Then for each $t \in \mathcal{C}^p(V)$, defining

$$s^p(t)_{j_0 < \cdots < j_{p-1}} = t_{i_0 j_0 \cdots j_{p-1}}\tag{$\ast$}$$

we obtain a map $s^p : \mathcal{C}^p(V) \rightarrow \mathcal{C}^{p-1}(V)$. The right-hand side is a section over $V \cap U_{i_0} \cap U_{j_0} \cap \cdots \cap U_{j_{p-1}}$, but since $V \subseteq U_{i_0}$, this set equals $V \cap U_{j_0} \cap \cdots \cap U_{j_{p-1}}$, where the left-hand side should live, so no extension is involved in ($\ast$). Then one can check by direct computation that this $s^p$ gives a chain homotopy: in $d^{p-1}s^p$ the term omitting $i_0$ and in $s^{p+1}d^p$ the term inserting $i_0$ cancel out with opposite signs. Thus the complex of sections at such $V$ is exact, and since $\mathcal{U}$ covers $X$, every point has such a neighborhood $V$, giving exactness at every stalk. A slight technical issue is that the fixed index $i_0$ might appear among $j_0<\cdots< j_{p-1}$. To handle this, we use the *non-alternating* Čech complex with coordinates given by any $p+1$ elements $i_0,\ldots, i_p\in I$, instead of the usual Čech complex. This is quasi-isomorphic to the original Čech complex, so this detour is justified.

Now let us use the assumption that $\mathcal{F}$ is flasque. The restriction map of each $\mathcal{C}^p$ is a product of restriction maps of $\mathcal{F}$, hence surjective, so $\mathcal{C}^p$ is also flasque. Thus $\mathcal{C}^\bullet$ is a flasque resolution of $\mathcal{F}$, so by [Proposition 16](#prop16) each term is $\Gamma(X,-)$-acyclic and by [Proposition 17](#prop17)

$$\check{H}^p(\mathcal{U}, \mathcal{F}) = H^p(\Gamma(X, \mathcal{C}^\bullet)) \cong H^p(X, \mathcal{F})$$

Again since $\mathcal{F}$ is flasque, the right-hand side vanishes for $p>0$ by [Proposition 16](#prop16). The proofs of the two propositions used here do not depend on this lemma.
:::
...
Then in this double complex the horizontal differential $d_h$ is the Čech differential and the vertical differential $d_v$ is the differential coming from the injective resolution. Now, as examined in [[Homological Algebra] §Spectral Sequences, ⁋Example 11](/en/math/homological_algebra/spectral_sequences#ex11), we know that the two filtrations on the total complex $\Tot(K)^\bullet$

$$F_v^p\Tot(K)^\bullet,\qquad F_h^p\Tot(K)^\bullet$$

converge to the same filtered homology $H^\bullet(\Tot(K))$.

Therefore, let us consider the spectral sequences given by each filtered complex. First, for the vertical filtration, on the $E_1$ page we have $E_1^{p,q} = H^q(K^{p,\bullet})$, and $K^{p,\bullet} = \check{C}^p(\mathcal{U}, \mathcal{I}^\bullet)$. Now $\check{C}^p(\mathcal{U}, \mathcal{I}^\bullet)$, looking at each component, is taking the injective resolution restricted to each intersection $U_{i_0 \cdots i_p}$ and then taking cohomology, so it equals the $q$-th sheaf cohomology of $\mathcal{F}$ on $U_{i_0\cdots i_p}$, and hence by the acyclicity assumption $E_1^{p,q}=0$ for all $q>0$. Also, by definition $E_1^{p,0}=\check{C}^p(\mathcal{U}, \mathcal{F})$. Now the $E_2$ page is given by the cohomology of $E_1^{p,0}$ with respect to the horizontal differential $d_h$, so

$$E_2^{p,q}=\begin{cases}\check{H}^p(\mathcal{U}, \mathcal{F})&\text{$q=0$}\\0&\text{otherwise}\end{cases}$$

and $E_2^{p,q}=E_\infty^{p,q}$.

Now looking in the horizontal filtration direction, on the $E_1$ page we have $E_1^{p,q} = \check{H}^p(\mathcal{U}, \mathcal{I}^q)$. But by [Lemma 9](#lem9) and [Lemma 10](#lem10) above, injective sheaves are Čech-acyclic, so $E_1^{p,q} = 0$ for $p > 0$, and the cohomology with respect to the remaining vertical differential at $p=0$ is sheaf cohomology, so

$$E_2^{p,q}=\begin{cases}H^q(X, \mathcal{F})&\text{$p=0$}\\0&\text{otherwise}\end{cases}$$

and $E_2^{p,q}=E_\infty^{p,q}$. Since the two spectral sequences converge to the same $H^\bullet(\Tot(K))$, we know

$$\check{H}^n(\mathcal{U}, \mathcal{F}) \cong H^n(X, \mathcal{F})$$
:::

Then the only obstacle to our intuition is how demanding this acyclic condition is, but fortunately it is a more generous condition than one might think.
...
Now consider an arbitrary variety $X$ and a quasi-coherent sheaf $\mathcal{F}$ defined on it, and suppose an affine open cover $\mathcal{U}$ of $X$ is given. Then for these data to satisfy the hypotheses of [Theorem 11](#thm11), every finite intersection of members of $\mathcal{U}$ must again be affine. If the diagonal

$$\Delta_X\hookrightarrow X\times X$$

is a *closed* immersion of $X\times X$, then we can show that this condition holds, and in this case $X$ is called a *separated* variety. As can be seen from its definition, this can be regarded as the Zariski topology version of the Hausdorff condition, and is a reasonable condition; moreover, if we call quasi-projective varieties varieties as in our current definition, this condition is automatically satisfied. That is, in our current language this argument says that Čech cohomology and sheaf cohomology agree for any quasi-coherent sheaf on any variety, and moreover if we choose an open cover $\mathcal{U}$ satisfying the hypotheses of [Theorem 11](#thm11), it suffices to compute the Čech cohomology with respect to that open cover without taking a direct limit.

## Godement Resolution

In [Definition 1](#def1) we defined sheaf cohomology via injective resolution, but since injective resolutions are generally difficult to compute directly, we examined one solution: using the preceding result [Theorem 11](#thm11) that Čech cohomology is isomorphic to sheaf cohomology.
...
::: Definition 13
For a sheaf $\mathcal{F}$ on a topological space $X$, we define the *Godement sheaf* $C^0(\mathcal{F})$ by assigning to each open set $U \subseteq X$
...
Then for each $x\in X$, from the identity on the stalk $\mathcal{F}_x\rightarrow \mathcal{F}_x$ we obtain a canonical morphism $\mathcal{F}\rightarrow C^0(\mathcal{F})$. Also, that $C^0(\mathcal{F})$ is a sheaf is almost obvious from the definition.

Intuitively, $C^0(\mathcal{F})$ can be thought of as the collection of functions choosing an element of $\mathcal{F}_x$ at each point $x\in X$ with no constraints whatsoever; from this viewpoint it is sometimes called the *sheaf of discontinuous sections*. The following is a basic property of this sheaf.
...
::: Proof
First, let us show that the given sheaf is flasque. For open sets $V \subseteq U$, the restriction map $C^0(\mathcal{F})(U) = \prod_{x \in U} \mathcal{F}_x \rightarrow \prod_{x \in V} \mathcal{F}_x = C^0(\mathcal{F})(V)$ is a projection, hence surjective. Therefore $C^0(\mathcal{F})$ is flasque.
...
Intuitively, $\mathcal{Q}^1$ collects the purely discontinuous parts, and from this viewpoint, repeating this construction yields finer and finer information about discontinuity. That is, applying $C^0$ to the sheaf $\mathcal{Q}^1$ we obtain the next cokernel exact sequence
...
::: Proof
We proceed by induction on $i$. First we treat the case $i=1$. For this, embed $\mathcal{F}$ into an injective sheaf $\mathcal{I}$ and consider the cokernel exact sequence

$$0 \rightarrow \mathcal{F}\rightarrow\mathcal{I}\rightarrow\mathcal{Q}\rightarrow0$$

Our claim is that for every open set $V$, the map $\mathcal{I}(V)\rightarrow \mathcal{Q}(V)$ is surjective, and that from this it follows that $\mathcal{Q}$ is also flasque. The latter follows from the former by a diagram chase in the commutative diagram below for arbitrary open subsets $V\subseteq U$:

{% diagram Math/Algebraic_Varieties/Sheaf_Cohomology-1.svg width="23.07em" alt="Commutative diagram" %}

Here $\mathcal{F}$ is flasque by hypothesis and $\mathcal{I}$ is injective, hence flasque. Now for any $s\in \mathcal{Q}(V)$, the surjectivity we claimed allows us to lift $s$ to some $t\in \mathcal{I}(V)$, and then using that $\mathcal{I}$ is flasque we lift $t$ to $\overline{t}\in\mathcal{I}(U)$ and push it down to $\mathcal{Q}$ to define $\overline{s}\in \mathcal{Q}(U)$. Since $\overline{t}$ is an extension of $t$, we have $\overline{t}\vert_V=t$, and therefore $\overline{s}\vert_V$ equals the image of $t$ in $\mathcal{Q}(V)$, namely $s$. Thus $\overline{s}$ restricts exactly to $s\in \mathcal{Q}(V)$, and we obtain the flasqueness of $\mathcal{Q}$.
...
Since $\mathcal{I}$ is injective, $H^1(X, \mathcal{I}) = 0$. Hence

$$H^1(X, \mathcal{F}) \cong \coker(\Gamma(X, \mathcal{I}) \rightarrow \Gamma(X, \mathcal{Q}))$$

and to show this vanishes we must prove that $\Gamma(X, \mathcal{I})\rightarrow \Gamma(X, \mathcal{Q})$ is surjective. We prove this for an arbitrary open set. So let an open set $U\subseteq X$ and $s\in \mathcal{Q}(U)$ be given, and consider the collection $P$ of pairs $(V, t)$ where $V$ is an open subset of $U$ and $t\in \mathcal{I}(V)$ maps to $s\vert_V$ in $\mathcal{Q}(V)$. Define $(V, t)\leq (V', t')$ by $V\subseteq V'$ and $t'\vert_V=t$; then $P$ becomes a partially ordered set. Since $\mathcal{I}\rightarrow \mathcal{Q}$ is surjective at the stalk level, suitable local lifts of $s$ exist near each point, so $P$ is nonempty. Moreover, for any chain in $P$ the corresponding sections $t$ agree on overlaps, so by the gluing axiom for $\mathcal{I}$ they patch together to give an upper bound for the chain. Therefore, by Zorn's lemma, $P$ has a maximal element $(V, t)$.

It remains to show that $V=U$. If there existed $x\in U\setminus V$, we could choose a neighborhood $W\subseteq U$ of $x$ and $t'\in \mathcal{I}(W)$ mapping to $s\vert_W$. Then $t\vert_{V\cap W}-t'\vert_{V\cap W}$ vanishes in $\mathcal{Q}(V\cap W)$, so it comes from some $f\in \mathcal{F}(V\cap W)$, and because $\mathcal{F}$ is flasque we can extend this to $\widetilde{f}\in \mathcal{F}(W)$. Replacing $t'$ by $t'+\widetilde{f}$ gives another lift of $s\vert_W$ which agrees with $t$ on $V\cap W$, so the two patch to a lift of $s$ over $V\cup W$. This contradicts the maximality of $(V, t)$, hence $V=U$. In particular, the case $U=X$ is exactly the surjectivity of $\Gamma(X, \mathcal{I})\rightarrow \Gamma(X, \mathcal{Q})$ that we needed, and at the same time we also obtain the flasqueness of $\mathcal{Q}$ deferred earlier.

Finally, for $i\geq 2$, the terms $H^{i-1}(X, \mathcal{I})$ and $H^i(X, \mathcal{I})$ flanking the relevant spot in the long exact sequence vanish by the injectivity of $\mathcal{I}$, so
...
and since $\mathcal{I}^n$ is injective, it is flasque by [Lemma 9](#lem9), hence in particular $\Gamma(X, -)$-acyclic. Thus, considering the canonical short exact sequence
...
3. Similarly, for each $p$ the complexes $B^p(I^{\bullet,q})$ and $Z^p(I^{\bullet,q})$ arranged along $q$ form injective resolutions of $B^p(K^\bullet)$ and $Z^p(K^\bullet)$, respectively.
:::

The heart of this definition is that the intuition mentioned above is not enough to produce a Cartan-Eilenberg resolution; in particular, the fact that the cohomology of each row forms a horizontal resolution of $H^p(K^\bullet)$ is a crucial ingredient in the existence proof. We do not prove the existence of Cartan-Eilenberg resolutions separately, but basically they can be obtained by repeatedly applying [\[Homological Algebra\] §Resolutions, ⁋Lemma 7](/en/math/homological_algebra/resolutions#lem7).

Meanwhile, the third condition ensures that in each row the two short exact sequences
...
have left terms $Z^p(I^{\bullet,q})$ and $B^p(I^{\bullet,q})$ that are injective, so both sequences split and each row decomposes into a direct sum of injective objects. This is the reason that a left exact functor commutes with row-wise cohomology, as we shall see below.
...
Since this spectral sequence lies in the first quadrant, we know it converges to the cohomology of the total complex $\Tot(\mathcal{I})^\bullet$. For the concrete computation, filter by $p$ in the Godement direction. Then we can first write the $E_1$ page as

$$\mathcal{H}^{p,q} := H^p(\mathcal{I}^{\bullet, q})$$

Here the vertical differential is the morphism $\mathcal{H}^{p,q}\rightarrow \mathcal{H}^{p,q+1}$ induced on the cohomology level by the differential of the injective resolution, and the $E_2$ page is the cohomology sheaf of this vertical complex:
...
Here, for each $p$, the complex $\mathcal{I}^{p,\bullet}$ is an injective resolution of $f_\ast \mathcal{G}^p(\mathcal{F})$, so by the exactness of injective resolutions,

$$E_1^{p,q} = \begin{cases} f_\ast \mathcal{G}^p(\mathcal{F}) & \text{if $q = 0$} \\ 0 & \text{if $q > 0$} \end{cases}$$

and the $d_1$-differential is a morphism from $E_1^{p,0} = f_\ast \mathcal{G}^p(\mathcal{F})$ to $E_1^{p+1,0} = f_\ast \mathcal{G}^{p+1}(\mathcal{F})$, corresponding to the differential $f_\ast \mathcal{G}^p(\mathcal{F}) \rightarrow f_\ast \mathcal{G}^{p+1}(\mathcal{F})$ of the Godement resolution. Thus the $E_2$ page is the cohomology sheaf of the complex
...
Now apply the global section functor $\Gamma(Y,-)$ to this result and revisit the discussion above. That is, we consider the double complex
...
and here each $\mathcal{I}^{p,\bullet}$ is, by the definition of a Cartan-Eilenberg resolution, an injective resolution of $f_\ast \mathcal{G}^p(\mathcal{F})$, so $E_1^{p,q}=H^q(Y, f_\ast \mathcal{G}^p(\mathcal{F}))$. But $\mathcal{G}^p(\mathcal{F})$ is flasque and $f_\ast$ preserves flasqueness, so $f_\ast \mathcal{G}^p(\mathcal{F})$ is also flasque, and hence by [Proposition 16](#prop16) the terms with $q>0$ vanish, leaving only
...
Geometrically, this is most transparent when $f:X\rightarrow Y$ is a fibration; in this case, the spectral sequence says that to compute cohomology on $X$, we first compute cohomology on $Y$, then remember the cohomology on the fiber over each point as the higher sheaf $R^q f_\ast \mathcal{F}$, and finally compose these over $Y$.
...
To complete the proof, we now look at the components $E_2^{2,0}$, $E_2^{1,1}$, $E_2^{0,2}$ with $p+q = 2$. For the same reason, $d_2 : E_2^{0,1} \rightarrow E_2^{2,0}$ is the only nontrivial differential, and on the $E_3$ page defined by this differential,
...
so embedding via the filtration as $F^2 H^2 \hookrightarrow H^2(X, \mathcal{F})$,
...
::: Proof
Take the Godement resolution $\mathcal{G}^\bullet(\mathcal{F})$ of $\mathcal{F}$ and form the double complex $C^{p,q} = \check{C}^p(\mathcal{U}, \mathcal{G}^q(\mathcal{F}))$. That the two spectral sequences obtained from the two filtrations converge to the same total cohomology $H^{p+q}(X, \mathcal{F})$ follows from [\[Homological Algebra\] §Spectral Sequences, ⁋Example 11](/en/math/homological_algebra/spectral_sequences#ex11), and since the Godement sheaf $\mathcal{G}^q(\mathcal{F})$ is flasque, it is Čech-acyclic by [Lemma 10](#lem10), so we may use the same vanishing as in the computation above.
:::

This spectral sequence allows us to understand [Theorem 11](#thm11) in a broader context. If $\mathcal{F}$ is acyclic on all finite intersections of $\mathcal{U}$, then $\check{C}^\bullet(\mathcal{U}, \mathcal{H}^q(\mathcal{F})) = 0$ for all $q > 0$, so all terms with $q > 0$ on the $E_2$ page vanish and we obtain $E_2^{p,0} = \check{H}^p(\mathcal{U}, \mathcal{F}) \cong H^p(X, \mathcal{F})$. That is, the Čech-to-derived functor spectral sequence is a more general result that includes [Theorem 11](#thm11).

## Classification of Line Bundles

Earlier we saw that a line bundle is determined by transition functions $g_{ij} \in \mathcal{O}_X^\times(U_i \cap U_j)$ ([\[Line Bundles and Vector Bundles\] §Line Bundles and Vector Bundles, ⁋Proposition 2](/en/math/algebraic_varieties/line_bundles#prop2)). The transition functions satisfy the cocycle condition $g_{ij}g_{jk} = g_{ik}$, which exactly corresponds to the Čech 1-cocycle condition written in multiplicative notation. Moreover, an isomorphism of line bundles changes the transition function by $g_{ij} \mapsto h_i g_{ij} h_j^{-1}$ via functions $h_i \in \mathcal{O}_X^\times(U_i)$ on each $U_i$, which also coincides with the equivalence relation given by Čech 1-coboundaries. That is, the isomorphism class of a line bundle naturally corresponds to an element of $\check{H}^1(X, \mathcal{O}_X^\times)$.

Organizing this observation rigorously yields the following. Here one must be careful that since $\mathcal{O}_X^\times$ is a sheaf of (abelian) groups with multiplicative structure, the coboundary relation in Čech cohomology is expressed multiplicatively rather than additively. Concretely, a 1-coboundary is of the form $(g_{ij}) = (h_i \cdot h_j^{-1})$.
