...
By this proposition we know that line bundles and invertible sheaves are the same concept. For this reason, when denoting a line bundle we use calligraphic $\mathcal{L}$ rather than the upright $L$ used to denote a space.
...
<ins id="ex11">**Example 11**</ins> The coordinate ring $R = \mathbb{K}[\x_1, \ldots, \x_n]$ of $$\mathbb{A}^n$$ is a UFD, and by the discussion above line bundles over $$\mathbb{A}^n$$ correspond to invertible modules over $R$. By ([\[Commutative Algebra\] §Fractional Ideals, ⁋Theorem 4](/en/math/commutative_algebra/fractional_ideals#thm4)) invertible modules over a UFD are free, so $\Pic(\mathbb{A}^n) = 0$.
...
Now comparing the two trivializations on $U_i \cap U_j$ we can derive the transition function. That is, the transition function $\phi_i \circ \phi_j^{-1}\colon \mathcal{O}_{U_j}\vert_{U_i \cap U_j} \to \mathcal{O}_{U_i}\vert_{U_i \cap U_j}$ satisfies
...
<ins id="def13">**Definition 13**</ins> For each point $x = [x_0 : \cdots : x_n]$ of $$\mathbb{P}^n$$, consider the line $\ell_x = \{(\lambda x_0, \ldots, \lambda x_n) \mid \lambda \in \mathbb{K}\}$ passing through the origin in $\mathbb{A}^{n+1}$, and the space obtained by attaching this line to each point:
...
In particular, examining $\mathcal{O}(-1)$ on $\mathbb{P}^1$ makes the meaning of *twisting* explained intuitively above much clearer. The process of constructing $\mathbb{P}^1$ from $\mathbb{A}^2\setminus \{0\}$ can be thought of as first mapping $\mathbb{A}^2\setminus\{0\}$ onto the unit circle by radial projection and then identifying antipodal points on the unit circle; during this process, vectors pointing in opposite directions are identified, i.e. the fibers twist. One way to see this twisting is to look at sections of the line bundle $$\mathcal{L}$$.
...
with no restriction on $a,b$ whatsoever. Intuitively, the sections of $\mathcal{O}(-1)$ cannot cross the zero section because of the denominators, so every section is forced to encounter the problem created by the twisting where "1 attaches to $-1$". This same twisting creates the same problem in $\mathcal{O}(1)$: the "constant section" $s([x_0:x_1])$ is not a section in $\mathcal{O}(1)$ either. However, this time the sections of $\mathcal{O}(1)$ can cross the zero section, so $\Gamma(\mathbb{P}^1, \mathcal{O}(1))\neq 0$. From the viewpoint of transition functions or trivializations, one can also think of it as follows: $\mathcal{O}(d)$ multiplies by the degree $d$ polynomial $\x_i^d$, so poles of degree at most $d$ can be erased by this trivialization.
...
<ins id="def17">**Definition 17**</ins> For a Cartier divisor $D = \{(U_i, f_i)\}$, define the line bundle $\mathcal{O}_X(D)$ by the transition functions $g_{ij} = f_i/f_j$.
...
If two Cartier divisors $\{(U_i, f_i)\}$ and $\{(V_j, g_j)\}$ are equivalent, then $f_i/g_j \in \mathcal{O}_X(U_i \cap V_j)^\ast$. The transition functions of the line bundles defined from them are compatible, so they define isomorphic line bundles.
...
Given a morphism $\varphi: X \to Y$, the operation of "pulling back" a line bundle on $Y$ to $X$ is defined naturally. For example, if we pull back a hypersurface on $Y$ via $\varphi$ to $X$, the corresponding line bundle should also be pulled back. This pullback operation induces a group homomorphism between Picard groups, and in the case of an embedding it can be understood as restricting a line bundle from the ambient space to the subvariety.
...
Suppose the line bundle $$\mathcal{L}$$ is given by transition functions $\{g_{ij}\}$ over an open cover $\{U_i\}$. The pullback $\varphi^\ast \mathcal{L}$ is defined over the open cover $\{\varphi^{-1}(U_i)\}$ by transition functions $\{g_{ij} \circ \varphi\}$. To verify that $\varphi^\ast \mathcal{L}$ is a line bundle on $X$, it suffices to check that the transition functions satisfy the cocycle condition.
...
Let us verify this from the transition function viewpoint. The transition function of $\mathcal{L} \otimes \mathcal{M}$ is $g_{ij}^{\mathcal{L}} g_{ij}^{\mathcal{M}}$, so the transition function of $\varphi^\ast(\mathcal{L} \otimes \mathcal{M})$ is $(g_{ij}^{\mathcal{L}} g_{ij}^{\mathcal{M}}) \circ \varphi = (g_{ij}^{\mathcal{L}} \circ \varphi)(g_{ij}^{\mathcal{M}} \circ \varphi)$. These are precisely the transition functions of $\varphi^\ast\mathcal{L}$ and $\varphi^\ast\mathcal{M}$ respectively, so we obtain $\varphi^\ast(\mathcal{L} \otimes \mathcal{M}) \cong \varphi^\ast\mathcal{L} \otimes \varphi^\ast\mathcal{M}$. Moreover, since all transition functions of $\mathcal{O}_Y$ are $1$, the transition functions of $\varphi^\ast\mathcal{O}_Y$ are also $1$, i.e. $\varphi^\ast\mathcal{O}_Y \cong \mathcal{O}_X$.

</details>

<div class="example" markdown="1">

<ins id="ex22">**Example 22**</ins> For an embedding $i: C \hookrightarrow \mathbb{P}^n$, the pullback $i^\ast \mathcal{O}_{\mathbb{P}^n}(1)$ is a line bundle on the curve $C$. This is called the *hyperplane bundle* on $C$, and is denoted $\mathcal{O}_C(1)$. In general $\mathcal{O}_C(1)$ is nontrivial; for example, when $C = \mathbb{P}^1 \subset \mathbb{P}^n$, we have $\mathcal{O}_C(1) = \mathcal{O}_{\mathbb{P}^1}(1)$, which is a nontrivial line bundle as seen in [Example 12](#ex12). The name "hyperplane bundle" comes from the fact that it is obtained by pulling back to $C$ the line bundle $\mathcal{O}_{\mathbb{P}^n}(1)$ corresponding to a hyperplane $H$, which is a hypersurface of degree $1$ in $\mathbb{P}^n$.

</div>

## Vector Bundle

So far we have examined line bundles, whose fibers are one-dimensional vector spaces. We can generalize this concept to define *vector bundles*, whose fibers are higher-dimensional vector spaces. Vector bundles capture structures that arise naturally in geometry, such as tangent and normal spaces of a variety, and are the algebraic-geometric analogues of tangent bundles, vector fields, etc. in differential geometry. Line bundles are the special case of rank $1$ vector bundles, and from the viewpoint of vector bundle theory the properties of line bundles become even clearer.
...
Geometrically important objects are the tangent and cotangent bundles. The *tangent bundle* $\mathcal{T}_X$ is the vector bundle whose fiber at each point $p \in X$ is the tangent space $T_p X$; if $X$ is an $n$-dimensional smooth variety then it is a rank $n$ vector bundle, and in local coordinates $\x_1, \ldots, \x_n$ the vectors $\partial/\partial \x_1, \ldots, \partial/\partial \x_n$ form a local frame. The *cotangent bundle* $\Omega_X^1 = \mathcal{T}_X^\vee$ is the dual of the tangent bundle, and in local coordinates $d\x_1, \ldots, d\x_n$ form a local frame.

Intuitively $\Omega_X^1$ is the bundle of differential $1$-forms on $X$, so by tensoring them $r$ times we obtain the bundle of $r$-forms. Among these the most interesting is the top exterior power $\omega_X = \bigwedge^n \Omega_X^1$, which is a rank $1$ vector bundle, i.e. a line bundle; in differential geometry it would have been thought of as the bundle of volume forms. We call this the *canonical line bundle*.
