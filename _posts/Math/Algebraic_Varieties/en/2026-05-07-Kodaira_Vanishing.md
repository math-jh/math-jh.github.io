...
description: "While Serre vanishing guarantees cohomology vanishing only in sufficiently high degrees, the Kodaira vanishing theorem shows that higher cohomology always vanishes for the tensor product of the canonical bundle with an ample line bundle. We examine its uses and applications in algebraic geometry."
...
date: 2026-05-07
weight: 18
translated_at: 2026-08-16T03:47:32+00:00
translation_source: kimi-cli
---
[§Cohomology of Projective Space, ⁋Proposition 7](/en/math/algebraic_varieties/cohomology_of_projective_spaces#prop7)'s Serre vanishing theorem guarantees that for an ample line bundle $\mathcal{L}$ on a projective variety and a coherent sheaf $\mathcal{F}$, we have $H^i(X, \mathcal{F} \otimes \mathcal{L}^{\otimes m}) = 0$ ($i > 0$) for sufficiently large $m$. However, this result is merely an asymptotic property and gives no information about exactly which $m$ the vanishing begins at.
...
The basic setup we shall work with is as follows. $X$ is an $n$-dimensional smooth projective variety, $\mathcal{L}$ is an ample line bundle on $X$, and $\omega_X = \det \Omega_X^1 = \Omega_X^n$ is the canonical line bundle. ([§Canonical Line Bundle, ⁋Definition 5](/en/math/algebraic_varieties/canonical_bundle#def5)) Here $\Omega_X^q = \bigwedge^q \Omega_X^1$. Then the Kodaira vanishing theorem can be written as follows.
...
The first statement is obtained from the second by setting $q = n$, and was already stated in [§The Riemann–Roch Theorem for Surfaces, ⁋Proposition 7](/en/math/algebraic_varieties/riemann_roch_surfaces#prop7). The second statement extends this to arbitrary form degree $q$, and is called the Akizuki–Nakano vanishing. The proof of this proposition involves quite technical aspects, so in this post we focus on how it is used in algebraic geometry rather than giving a rigorous proof.
...
and from [§Line Bundles and Vector Bundles, ⁋Example 12](/en/math/algebraic_varieties/line_bundles#ex12) we checked that any line bundle on $\mathbb{P}^n$ is of the form $\mathcal{O}(d)$. Among these, the $\mathcal{O}(d)$ with $d > 0$ are ample line bundles. Therefore, Kodaira vanishing asserts that the vanishing
...
and from this all cohomology automatically vanishes for $q \neq 0, n$, so our only concern is when $q = n$. Now, according to the formula above, for this to be nonzero we must have $k \leq -n-1$. But in our situation $k = d - n - 1$ and $d > 0$, so this is impossible, and thus we can verify the Kodaira vanishing theorem again.
:::

## Applications of the Kodaira Vanishing Theorem

Now we examine applications of the Kodaira vanishing theorem as announced earlier. First, by the Riemann–Roch theorem from the previous post, for a divisor $D$ on a surface $S$ we have

$$\rchi(\mathcal{O}_S(D)) = \frac{1}{2} D \cdot (D - K_S) + \rchi(\mathcal{O}_S)$$

([§The Riemann–Roch Theorem for Surfaces, ⁋Proposition 4](/en/math/algebraic_varieties/riemann_roch_surfaces#prop4)). The power of this formula lies in the fact that $\rchi$ can be computed purely from algebraic and topological data, but the problem is that $\rchi$ is the alternating sum of $h^0, h^1, h^2$. Hence, when we simply want to know $h^0(S, \mathcal{O}_S(D))$, we must determine the values of the higher cohomologies separately, so the Riemann–Roch formula alone is difficult to give a direct answer.

In such a situation, to use the Kodaira vanishing theorem, suppose $\mathcal{L} \cong \mathcal{O}_S(L)$ is an ample line bundle. Then we know that
...
and therefore by simply computing the right-hand side of the Riemann–Roch formula we immediately obtain $h^0(S, \omega_S \otimes \mathcal{L})$.
...
For example, in the case of a curve $C$ we know that its plurigenera are determined by the genus, and indeed $P_m(g)$ is given as a function of $g$ (and $m$). That is, for a curve $C$ the plurigenera are not particularly interesting invariants. The interesting case is in higher dimensions such as surfaces, where the birational invariant is not determined by a single number and all the plurigenera become genuinely necessary.
...
Now if $m \geq 2$ and $K_S$ is ample, then $(m-1)K_S$ is also ample, so applying [Proposition 1](#prop1) to $mK_S = K_S + (m-1)K_S$ yields $h^1 = h^2 = 0$. Hence from this formula we can directly compute $P_m(S) = h^0(S, \mathcal{O}_S(mK_S))$.

On the other hand, in such a case the expression for the plurigenera can be thought of as asymptotically quadratic. This leads to the following definition.
...
In the above definition, the reason the set over which the minimum is taken is nonempty is that $P_m(X) = O(m^{\dim X})$ holds for any smooth projective variety, and from this we know that $\kappa(X)$ is well-defined and simultaneously $\kappa(X) \leq \dim X$ always holds. Hence for surfaces $\kappa \in \{-\infty, 0, 1, 2\}$. The [Enriques–Kodaira classification](https://en.wikipedia.org/wiki/Enriques-Kodaira_classification) classifies surfaces broadly by Kodaira dimension, and in the cases $\kappa = 0$ and $\kappa = -\infty$ it provides additional finer classification using the geometric genus $p_g$ and the irregularity $q$.

In [§Linear Systems, ⁋Definition 9](/en/math/algebraic_varieties/linear_systems#def9) we defined a line bundle $\mathcal{L}$ to be very ample if the morphism $\varphi_{\mathcal{L}}: X \rightarrow \mathbb{P}(\Gamma(X, \mathcal{L}))$ defined by the complete linear system $\lvert \mathcal{L} \rvert$ is a closed embedding. At the time we did not have the language of sheaf cohomology, but now that we have introduced sheaf cohomology we can use it somewhat better.

First, suppose a very ample line bundle $\mathcal{L}$ is given, and consider the closed embedding $\varphi_\mathcal{L}: X \rightarrow \mathbb{P}^N$ defined by it. Then from the fact that $\varphi$ is an embedding we know that $\varphi_\mathcal{L}(p) \neq \varphi_\mathcal{L}(q)$ holds, and moreover since $\varphi_\mathcal{L}$ is a closed embedding, $\dd{\varphi_\mathcal{L}}$ is injective, and therefore the dual map on cotangent spaces $\mathfrak{m}_{\varphi_{\mathcal{L}}(p)}/\mathfrak{m}_{\varphi_{\mathcal{L}}(p)}^2 \longrightarrow \mathfrak{m}_p/\mathfrak{m}_p^2$ is surjective. From this we know that the following two results hold.
...
Now let us see how these separation conditions are verified via cohomology. First, for (1), considering the closed subset $Z = \{p\} \cup \{q\}$ containing two points $p \neq q$, for the ideal sheaf $\mathcal{I}_Z$ defining $Z$ we obtain the short exact sequence
...
we see that if $H^1(X, \mathcal{I}_Z \otimes \mathcal{L}^{\otimes m}) = 0$ then the evaluation map becomes surjective and separation of points holds.

Similarly, for (2), considering the first infinitesimal neighborhood of the point $p$, $\Spec(\mathcal{O}_{X,p}/\mathfrak{m}_p^2)$, and letting $\mathcal{I}_p$ be the ideal sheaf of $p$, from the short exact sequence
...
we consider the induced long exact sequence
...
and if $H^1(X, \mathcal{I}_p^2 \otimes \mathcal{L}^{\otimes m}) = 0$ then separation of tangent vectors holds.

However, the targets of vanishing required in this way, $\mathcal{I}_Z$ and $\mathcal{I}_p^2$, are not locally free along $Z$ and $p$, so they are not of the form a tensor product of $\omega_X$ and a line bundle, and hence [Proposition 1](#prop1) does not apply directly to them. Instead, since they are coherent sheaves, we can apply [§Cohomology of Projective Space, ⁋Proposition 7](/en/math/algebraic_varieties/cohomology_of_projective_spaces#prop7) directly with $\mathcal{F} = \mathcal{I}_Z$ and $\mathcal{F} = \mathcal{I}_p^2$, and from this we obtain that for sufficiently large $m$ both of the above $H^1$ vanish, so the sections of $\mathcal{L}^{\otimes m}$ satisfy the two separation conditions. That is, what drives this argument is not Kodaira vanishing but Serre vanishing.

Kodaira vanishing enters the classical proof of [Proposition 6](#prop6) in a somewhat different way. In that proof, one applies vanishing to a line bundle on the blow-up $\pi: \widetilde{X} \rightarrow X$ of $p$ and $q$ with the twist lowered by the exceptional divisor, so the target of vanishing becomes a line bundle again and reduces to the form of [Proposition 1](#prop1). Furthermore, the condition that $\mathcal{L}^{\otimes m}$ be not only very ample but also that the embedding it defines be projectively normal can be obtained by verifying the surjectivity of the related multiplication map

$$S^\mu H^0(X, \mathcal{L}^{\otimes m}) \longrightarrow H^0(X, \mathcal{L}^{\otimes \mu m}),$$

and what gives the necessary vanishing together with a concrete range of $m$ is the Castelnuovo–Mumford regularity from [§Cohomology of Projective Space, §§Regularity](/en/math/algebraic_varieties/cohomology_of_projective_spaces#regularity). Such vanishing guarantees that higher cohomology does not obstruct the generation of sections, allowing one to quantitatively handle the abundance of a linear system.

## Kodaira Embedding Theorem

The most famous application of Kodaira vanishing is the Kodaira embedding theorem. However, this ventures into the realm of complex manifolds, so we only briefly introduce it here. First, a compact complex manifold $X$ being a *Kähler manifold* means that a compatible Riemannian metric, symplectic form, and complex structure are defined on $X$. In this case, if a Hermitian metric $h$ is given on a line bundle $\mathcal{L}$, then its curvature form $\Theta_h$ is defined, and $\mathcal{L}$ being *positive* means that $\frac{i}{2\pi}\Theta_h$ is a positive definite $(1,1)$-form. Then the following holds.
