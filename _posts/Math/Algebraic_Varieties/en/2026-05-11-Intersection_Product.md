...
---
In [§Chow Groups](/en/math/algebraic_varieties/chow_groups){: data-lid="x0eoz" data-relation="required" } we defined the Chow group $A^\ast(X)$. At the end of that post we claimed that one can define an intersection product on it, thereby giving it a ring structure; in this post we construct this product and examine its properties.
...
gives the definition. Here $R = \mathcal{O}_{\mathbb{A}^n, p}$. The formula above corresponds to the $i = 0$ term. In this post we only consider simple cases, so [Definition 1](#def1){: data-lid="ymwx5" data-relation="required" } suffices.
...
The above is an example of two 1-dimensional subvarieties meeting in dimension 2, which we already briefly introduced in [§The Riemann–Roch Theorem for Surfaces, ⁋Definition 1](/en/math/algebraic_varieties/riemann_roch_surfaces#def1){: data-lid="evy9g" data-relation="weak" }. In that post we introduced the notion of transversal intersection; let us define it formally.
...
Then the following two propositions are natural generalizations of [§The Riemann–Roch Theorem for Surfaces, ⁋Proposition 2](/en/math/algebraic_varieties/riemann_roch_surfaces#prop2){: data-lid="0vil9" data-relation="weak" }.
...
3. $i_p$ satisfies the symmetry and bilinearity of [§The Riemann–Roch Theorem for Surfaces, ⁋Proposition 2](/en/math/algebraic_varieties/riemann_roch_surfaces#prop2){: data-lid="fxgp2" data-relation="required" }.
:::
...
In particular [Definition 1](#def1){: data-lid="9bycy" data-relation="weak" } is the special case where $\codim (V\cap W)=n$. Now, if the above equality holds for every irreducible component of $V\cap W$, we can use it to define

$$V \cdot W = \sum_{T \subseteq V \cap W} i_T(V, W) [T].$$

Here $i_T(V, W)$ is the intersection multiplicity along the component $T$, the natural extension of the pointwise multiplicity of [Definition 1](#def1){: data-lid="p0f1o" data-relation="required" } to a component $T$. If $T$ is a point $p$ then $i_T(V, W) = i_p(V, W)$, and in general it is the value measuring how the two varieties meet at a general point of $T$, which can be defined rigorously as the intersection multiplicity at the generic point. The following proposition lifts the properties of intersection multiplicity to the intersection product.
...
The idea of this method is to deform $X$ continuously and shrink it onto the normal cone of $Y$. Geometrically, at $t=1$ we see the original space $X$, and as $t$ goes to $0$ the space $X$ becomes increasingly *flattened* along $Y$ until at $t=0$ it becomes the normal cone opened up along $Y$. If the blow-up in [§Rational Maps, ⁋Example 12](/en/math/algebraic_varieties/rational_maps#ex12){: data-lid="sium0" data-relation="weak" } was the deformation that blows up a point to a $\mathbb{P}^1$, then deformation to normal cone performs this for a more general embedding.
...
As a slightly more complicated example, we saw in [§Rational Maps, ⁋Example 11](/en/math/algebraic_varieties/rational_maps#ex11){: data-lid="7ucnh" data-relation="required" } that $\mathbb{P}^1 \times \mathbb{P}^1$ and the quadric surface $Q = V(\x\y - \z\w)$ are isomorphic. The Chow ring of $\mathbb{P}^1 \times \mathbb{P}^1$ is
...
::: Example 13
Consider the Segre embedding $\sigma: \mathbb{P}^1 \times \mathbb{P}^1 \rightarrow \mathbb{P}^3$. ([§Projective Varieties, ⁋Example 16](/en/math/algebraic_varieties/projective_varieties#ex16){: data-lid="xkn9i" data-relation="required" }) The image of this embedding is the quadric surface $Q = V(\x\y - \z\w)$. By [§Line Bundles and Vector Bundles, ⁋Proposition 20](/en/math/algebraic_varieties/line_bundles#prop20){: data-lid="v9be8" data-relation="required" } the pullback $\sigma^\ast \mathcal{O}_{\mathbb{P}^3}(1)$ is a line bundle on $\mathbb{P}^1 \times \mathbb{P}^1$, and indeed $\sigma^\ast \mathcal{O}_{\mathbb{P}^3}(1) \cong \mathcal{O}_{\mathbb{P}^1 \times \mathbb{P}^1}(1,1)$. This can also be checked at the level of Chow rings: pulling back the hyperplane class $H_{\mathbb{P}^3}$ in $\mathbb{P}^3$ gives $H_1 + H_2$, which corresponds to bidegree $(1,1)$.
