...
As mentioned above, the basic idea of a Weil divisor is to encode the zeros and poles of a rational function $f \in K(X)^\times$. Such a divisor is called a *principal divisor*.

::: Definition 3
The *principal divisor* $\divisor(f)$ of a rational function $f \in K(X)^\times$ is defined by
...
Since $X$ is Noetherian, only finitely many $Y$ satisfy $v_Y(f)\neq 0$, so the above sum is finite and defines a Weil divisor as in [Definition 1](#def1){: data-lid="jh5b5" data-relation="required" reviewed="" }.
...
Our goal is to extract properties of $X$ from $\Div(X)$. However, $\Div(X)$ is unnecessarily large for this purpose. Since we already understand the elements of $K(X)^\times$ reasonably well, we shall regard a divisor obtained from another by adding a principal divisor as equivalent to the original one. That is, we make the following definition.

::: Definition 7
Two Weil divisors $D_1, D_2$ are *linearly equivalent* if there exists a rational function $f \in K(X)^\times$ such that $D_1 - D_2 = \divisor(f)$. We denote this by $D_1 \sim D_2$.
:::
...
where $\deg(Y_i)$ is the degree of the homogeneous polynomial defining $Y_i$. Since every hypersurface is the zero set of some homogeneous polynomial, this definition is unproblematic. Moreover, by [\[Rational Maps\] §Rational Maps, ⁋Example 4](/en/math/algebraic_varieties/rational_maps#ex4){: data-lid="94ffm" data-relation="required" reviewed="" }, any rational function on $\mathbb{P}^n$ is a ratio $F/G$ of homogeneous polynomials of the same degree; hence for every principal divisor $\divisor(F)$ we have $\deg(\divisor(F))=0$, and the map is well-defined.
...
where $\{U_i\}$ is an open cover of $X$, each $f_i \in K(X)^\times$ is a nonzero rational function, and for all $i, j$ the ratio $f_i/f_j$ is regular and non-vanishing (hence invertible) on $U_i \cap U_j$.
...
Then the following definition is the Cartier analogue of [Definition 7](#def7){: data-lid="v04xd" data-relation="weak" reviewed="" }.
...
By [Proposition 14](#prop14){: data-lid="t892i" data-relation="required" reviewed="" }, on a smooth variety we have $\CaCl(X) \cong \Cl(X)$.
