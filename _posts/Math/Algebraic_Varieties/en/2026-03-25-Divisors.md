...
Since $X$ is Noetherian, only finitely many $Y$ satisfy $v_Y(f)\neq 0$, so the above sum is finite and defines a Weil divisor as in [Definition 1](#def1){: data-relation="required" reviewed="" }.
...
where $\deg(Y_i)$ is the degree of the homogeneous polynomial defining $Y_i$. Since every hypersurface is the zero set of some homogeneous polynomial, this definition is unproblematic. Moreover, by [\[Rational Maps\] §Rational Maps, ⁋Example 4](/en/math/algebraic_varieties/rational_maps#ex4){: data-relation="required" reviewed="" }, any rational function on $\mathbb{P}^n$ is a ratio $F/G$ of homogeneous polynomials of the same degree; hence for every principal divisor $\divisor(F)$ we have $\deg(\divisor(F))=0$, and the map is well-defined.
...
Then the following definition is the Cartier analogue of [Definition 7](#def7){: data-relation="weak" reviewed="" }.
...
By [Proposition 14](#prop14){: data-relation="required" reviewed="" }, on a smooth variety we have $\CaCl(X) \cong \Cl(X)$.
