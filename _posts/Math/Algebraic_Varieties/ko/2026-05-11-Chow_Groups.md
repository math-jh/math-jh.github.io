...
::: 명제 6
Proper morphism $f: X \to Y$에 대해 pushforward $f_\ast: \CH_k(X) \to \CH_k(Y)$가 존재한다. 특히, 임의의 subvariety $V\subset X$에 대하여, 
...
::: 명제 7
Flat morphism $f: X \to Y$에 대해 pullback $f^\ast: \CH^k(Y) \to \CH^k(X)$가 존재한다. Subvariety $V \subset Y$에 대해 $f^\ast[V] = [f^{-1}(V)]$이다.
:::
...
::: 예시 10
위의 예시를 더 직관적으로 보기 위해 degree $d$ morphism $f: \mathbb{P}^1 \to \mathbb{P}^1$을
