...
HRR 정리의 좌변은 바로 이 Euler characteristic이며, 우변은 $X$의 intersection theory 위에서의 적분으로 주어진다. 이를 위해서는 Chern character와 Todd class라는 두 가지 characteristic class가 필요하다. 이들의 정의와 성질은 [§Todd Class](/ko/math/algebraic_varieties/todd_class){: data-relation="required" reviewed="" }에서 자세히 다루었으므로, 본 글에서는 필요한 최소한의 사실만을 상기시킨다.
...
:::
::: 증명
Curve $C$의 차원은 $n=1$이다. Line bundle $\mathcal{L}=\mathcal{O}_C(D)$에 대하여 Chern character는 $\operatorname{ch}(\mathcal{L})=1+c_1(\mathcal{L})$이고, tangent bundle $T_C$의 rank가 $1$이므로 [§Todd Class, ⁋예시 5](/ko/math/algebraic_varieties/todd_class#ex5){: data-relation="required" reviewed="" }에 의해
...
이므로 classical Riemann-Roch 정리 [§곡선에서의 리만-로흐 정리, ⁋명제 3](/ko/math/algebraic_varieties/riemann_roch_theorem#prop3){: data-relation="weak" reviewed="" }을 복원한다. $\square$
:::
...
이며, [§Todd Class, ⁋예시 6](/ko/math/algebraic_varieties/todd_class#ex6){: data-relation="required" reviewed="" }에 의해 tangent bundle의 Todd class는
...
를 얻어 [§곡면에서의 리만-로흐 정리](/ko/math/algebraic_varieties/riemann_roch_surfaces){: data-relation="weak" reviewed="" }의 결과와 일치함을 확인한다. $\square$
:::
...
:::
::: 증명
[§Todd Class, ⁋예시 7](/ko/math/algebraic_varieties/todd_class#ex7){: data-relation="required" reviewed="" }에서 살펴보았듯이, $\mathbb{P}^n$의 Chow ring은 $A^\bullet(\mathbb{P}^n)\cong\mathbb{Z}[h]/(h^{n+1})$이며, tangent bundle의 total Chern class는 $c(T_{\mathbb{P}^n})=(1+h)^{n+1}$이다. 따라서 Chern roots는 모두 hyperplane class $h$이며,
