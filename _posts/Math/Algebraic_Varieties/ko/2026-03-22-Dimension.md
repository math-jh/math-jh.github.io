...
Algebraic variety는 이미 위상공간이므로, [\[위상수학\] §차원, ⁋정의 10](/ko/math/topology/dimension#def10){: data-lid="nsmdr" data-relation="required" reviewed="" }을 사용하여 $X$의 차원을 irreducible closed subset들의 strictly descending chain의 length의 supremum으로 정의할 수 있다. 
...
::: 명제 2
Algebraically closed field $\mathbb{K}$ 위에 정의된 affine variety $X$의 차원은 coordinate ring $\mathbb{K}[X]$의 Krull dimension과 같다. ([\[가환대수학\] §차원, ⁋정의 1](/ko/math/commutative_algebra/Krull_dimension#def1){: data-lid="2sksi" data-relation="required" reviewed="" })
:::

::: 증명
[§아핀다양체, ⁋명제 12](/ko/math/algebraic_varieties/affine_varieties#prop12){: data-lid="6kcrj" data-relation="required" reviewed="" }로부터 affine variety의 irreducible closed subset과 $\mathbb{K}[X]$의 prime ideal 사이의 일대일대응이 존재한다. 
:::
::: 따름정리 3
...
::: 증명
[\[가환대수학\] §매개계, ⁋따름정리 11](/ko/math/commutative_algebra/system_of_parameters#cor11){: data-lid="icc6f" data-relation="required" reviewed="" }
:::
...
이 성립하는 것을 안다. ([\[가환대수학\] §뇌터 정규화, ⁋정리 4](/ko/math/commutative_algebra/noether_normalization#thm4){: data-lid="8xxpv" data-relation="required" reviewed="" }) 여기서 codimension $\mathfrak{p}$는 [\[가환대수학\] §차원, ⁋정의 2](/ko/math/commutative_algebra/Krull_dimension#def2){: data-lid="hvv85" data-relation="weak" reviewed="" }에서 정의된 것으로, $\mathfrak{p}$에 포함되는 prime ideal들의 chain의 길이의 supremum이며, 기하적으로는 $X=Z(\mathfrak{p})$를 포함하는 $\mathbb{A}^n$의 closed subvariety들의 chain의 길이의 supremum이다. 기하적으로 우리는 $\dim \mathbb{K}[\x_1,\ldots, \x_n]/\mathfrak{p}$이 $Z(\mathfrak{p})$의 차원인 것을 알고 있으므로, 이를 통해 ($\ast$)에 기하적인 의미를 부여할 수 있다. 
...
이다. 여기서 첫째 등식은 [\[가환대수학\] §뇌터 정규화, ⁋정리 4](/ko/math/commutative_algebra/noether_normalization#thm4){: data-lid="cjjne" data-relation="required" reviewed="" }에 의한 것이다.
:::

## 함수체를 통한 차원

차원을 정의하는 또 다른 방법은 function field를 사용하는 것이다. Function field $K(X)$는 variety의 generic point에서의 정보를 담고 있으며, birational invariant이기도 하다. 다음 명제 또한 대수적인 사실로부터 유도된다. ([\[가환대수학\] §뇌터 정규화, ⁋정리 3](/ko/math/commutative_algebra/noether_normalization#thm3){: data-lid="5a9n3" data-relation="required" reviewed="" })
...
이는 [명제 6](#prop6){: data-lid="6a5ru" data-relation="weak" reviewed="" }의 약한 형태의 일반화라 생각할 수 있다. Hypersurface $Z(f)\subsetneq \mathbb{A}^n$에 적용하면 $\dim Z(f)\leq n-1$만이 나오므로, 하나의 방정식이 차원을 정확히 하나 떨어뜨린다는 것까지는 이로부터 얻을 수 없다. 이제 regular map과 차원의 관계에 대해 살펴보자. 

::: 명제 10
두 variety $X, Y$와 regular map $\varphi: X \rightarrow Y$에 대해 다음이 성립한다.

1. $\dim \varphi(X) \le \dim X$가 성립한다.
2. 만약 $\varphi$가 dominant라면 $\dim Y \le \dim X$이 성립한다.  ([§유리사상, ⁋정의 8](/ko/math/algebraic_varieties/rational_maps#def8){: data-lid="veb7w" data-relation="required" reviewed="" })
:::

::: 증명
둘째 결과부터 보이자. $\varphi$가 dominant라면, pullback $\varphi^\ast: K(Y)\rightarrow K(X)$가 injective이고, 따라서 [명제 7](#prop7){: data-lid="umr3o" data-relation="required" reviewed="" }로부터 원하는 결과를 얻는다. 
