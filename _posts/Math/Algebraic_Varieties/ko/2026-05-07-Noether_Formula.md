...
이는 [§천 특성류, ⁋명제 10](/ko/math/algebraic_varieties/chern_classes#prop10){: data-lid="o41ck" data-relation="required" reviewed="" }의 일반적인 진술을 surface의 경우에 적용한 것이며, 그 증명은 위상적 논증을 필요로 하므로 [BT] 등의 문헌으로 미룬다. 이 명제는 $c_2(S)$가 단순한 형식적 불변량이 아니라, surface의 위상적 구조를 측정하는 기하학적 양임을 보장한다. 예를 들어 $\mathbb{P}^2$의 경우 $\rchi_{\mathrm{top}}(\mathbb{P}^2)=3$이며, 이는 $c_2(T_{\mathbb{P}^2})=3H^2$의 적분값과 일치함을 아래 [예시 6](#ex6){: data-lid="np4nb" data-relation="forward" reviewed="" }에서 확인한다.
...
::: 증명
[§Hirzebruch-Riemann-Roch, ⁋정리 2](/ko/math/algebraic_varieties/hirzebruch_riemann_roch#thm2){: data-lid="xywkj" data-relation="required" reviewed="" }에 의해, smooth projective surface $S$ 위의 line bundle $\mathcal{O}_S$에 대하여

$$\rchi(S,\mathcal{O}_S)=\int_S\operatorname{ch}(\mathcal{O}_S)\cdot\operatorname{td}(T_S)$$

이 성립한다. Structure sheaf의 Chern character는 $\operatorname{ch}(\mathcal{O}_S)=1$이며, [§Todd Class, ⁋예시 6](/ko/math/algebraic_varieties/todd_class#ex6){: data-lid="ocf9c" data-relation="required" reviewed="" }에 의해 tangent bundle의 Todd class는
...
[정리 3](#thm3){: data-lid="0jisl" data-relation="weak" reviewed="" }의 증명에서 확인할 수 있듯이, Noether formula는 Hirzebruch-Riemann-Roch 정리에 $\mathcal{F}=\mathcal{O}_S$를 대입한 직접적인 결과이다. 이는 HRR 정리가 단순한 계산적 도구를 넘어, 서로 다른 영역의 불변량들 사이의 심층적인 연결을 제공함을 보여준다.
...
::: 증명
[명제 2](#prop2){: data-lid="e4ewl" data-relation="required" reviewed="" }에 의해 $c_2(S)=\rchi_{\mathrm{top}}(S)$이므로, 이를 Noether formula의 $c_2(S)$에 대입하면 즉각적으로 결과를 얻는다. $\square$
:::

[따름정리 4](#cor4){: data-lid="cyfjv" data-relation="weak" reviewed="" }는 Noether formula를 기하-위상적 관점에서 재해석한다. $K_S^2$은 surface의 "대수적 복잡도"를, $\rchi_{\mathrm{top}}(S)$는 "위상적 복잡도"를 측정하며, 이들의 합이 항상 $12$의 배수가 된다는 사실은 놀라운 정수성 조건을 제공한다.
...
이다. $S$가 connected projective variety이므로 $h^0(\mathcal{O}_S)=1$이다. [§세르 쌍대성, ⁋명제 2](/ko/math/algebraic_varieties/serre_duality#prop2){: data-lid="rpi4i" data-relation="required" reviewed="" }에 의해

$$h^2(\mathcal{O}_S)=h^0(K_S)=p_g$$

이며, $q$의 정의에 의해 $h^1(\mathcal{O}_S)=q$이다. 이들을 종합하면
...
[명제 5](#prop5){: data-lid="echil" data-relation="required" reviewed="" }에 의해 Noether formula는
...
이다. [§표준선다발, ⁋명제 7](/ko/math/algebraic_varieties/canonical_bundle#prop7){: data-lid="jbeqn" data-relation="required" reviewed="" }의 Euler exact sequence에 [§천 특성류, ⁋정의 1](/ko/math/algebraic_varieties/chern_classes#def1){: data-lid="dy7zs" data-relation="required" reviewed="" }의 Whitney sum formula를 적용하면 $c(T_{\mathbb{P}^2})=(1+H)^3=1+3H+3H^2$이다. 따라서 $c_2(T_{\mathbb{P}^2})=3H^2$이므로 $\int_{\mathbb{P}^2}c_2=3$이다. 한편 [§사영공간의 코호몰로지, ⁋명제 1](/ko/math/algebraic_varieties/cohomology_of_projective_spaces#prop1){: data-lid="p1qdl" data-relation="required" reviewed="" }에 의해 $h^i(\mathbb{P}^2,\mathcal{O}_{\mathbb{P}^2})=0$ for $i>0$이므로
...
$$K_{\widetilde{\mathbb{P}}^2}^2=9-r$$

이다. Topological Euler characteristic의 관점에서, 각 blow-up은 한 점을 exceptional $\mathbb{P}^1$로 교체하므로 Euler characteristic이 $1$씩 증가한다. $\rchi_{\mathrm{top}}(\mathbb{P}^2)=3$이므로

$$\rchi_{\mathrm{top}}(\widetilde{\mathbb{P}}^2)=3+r$$

이다. 한편 $\rchi(\mathcal{O})$는 smooth projective surface의 birational 불변량이므로
...
$S\subseteq\mathbb{P}^3$를 degree $d$인 smooth surface라 하자. [§표준선다발, ⁋명제 9](/ko/math/algebraic_varieties/canonical_bundle#prop9){: data-lid="8blfo" data-relation="required" reviewed="" }의 adjunction formula에 의해
...
$$0\longrightarrow T_S\longrightarrow T_{\mathbb{P}^3}\vert_S\longrightarrow\mathcal{O}_S(d)\longrightarrow 0$$

를 이용한다. Whitney sum formula에 의해 $c(T_{\mathbb{P}^3}\vert_S)=c(T_S)\cdot c(\mathcal{O}_S(d))$이며, $c(T_{\mathbb{P}^3})=(1+H)^4$로부터 $c(T_{\mathbb{P}^3}\vert_S)=1+4h+6h^2$이다. $c(\mathcal{O}_S(d))=1+dh$이므로, $c(T_S)=1+c_1+c_2$를 대입하여 비교하면
...
이다. 이제 $\rchi(\mathcal{O}_S)$를 계산한다. Lefschetz hyperplane theorem에 의해 $q=h^1(\mathcal{O}_S)=0$이고, Serre duality에 의해 $h^2(\mathcal{O}_S)=h^0(K_S)=h^0(\mathcal{O}_S(d-4))$이다. $d<4$이면 $p_g=0$이므로 $\rchi(\mathcal{O}_S)=1$이고, $d=4$이면 $K_S=0$이므로 $p_g=1$이어서 $\rchi(\mathcal{O}_S)=2$이다. 일반적으로

$$p_g=h^0(\mathcal{O}_S(d-4))=\binom{d-1}{3}=\frac{(d-1)(d-2)(d-3)}{6}$$

이므로

$$\rchi(\mathcal{O}_S)=1+\frac{(d-1)(d-2)(d-3)}{6}$$
...
[예시 9](#ex9){: data-lid="5d0jl" data-relation="required" reviewed="" }에서 $d=4$인 경우, 즉 quartic K3 surface의 경우 $K_S=0$이고 $K_S^2=0$, $\rchi_{\mathrm{top}}(S)=24$, $\rchi(\mathcal{O}_S)=2$이므로 $0+24=12\cdot 2$가 성립한다. 이는 K3 surface의 대표적인 불변량들과 일치하며, 이러한 계산이 surface의 classification에서 어떻게 활용되는지를 보여준다.
