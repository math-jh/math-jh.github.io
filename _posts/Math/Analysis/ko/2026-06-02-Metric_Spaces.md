...
역삼각부등식의 직접적 귀결은 거리 함수의 연속성이다. 점열 $$(x_n)$$이 $$x$$로 수렴하면 명제 5에 의해 임의의 고정점 $$z$$에 대해
...
명제 6은 완비성의 정의 ([정의 4](#def4))를 다시 음미하게 한다. 어떤 거리공간에서든 수렴하면 Cauchy이지만, 완비공간이란 정확히 그 역까지 성립하여 "Cauchy"와 "수렴"이 같은 뜻이 되는 공간이다. 완비성이 깨지는 전형적 상황은 점열이 극한처럼 행동하는데 그 극한이 공간 밖에 있을 때이며, 아래 예시들에서 이를 구체적으로 본다.

## 예시와 계산

거리공간의 풍부함은 같은 바탕집합에 서로 다른 거리를 줄 수 있다는 데 있다. 유클리드 거리 외에도 좌표의 절댓값 합이나 최대 절댓값으로 거리를 정의할 수 있으며, 이들은 모두 삼각부등식을 만족한다.

<div class="example" markdown="1">

<ins id="ex7">**예시 7 (택시 거리와 최대 거리)**</ins> $$\mathbb{R}^n$$ 위에서 두 점 $$x = (x_1, \ldots, x_n)$$, $$y = (y_1, \ldots, y_n)$$에 대해

$$d_1(x, y) = \sum_{i=1}^n \lvert x_i - y_i\rvert, \qquad d_\infty(x, y) = \max_{1 \leq i \leq n} \lvert x_i - y_i\rvert$$

로 정의하자. 각 좌표에서 실수의 삼각부등식 $$\lvert x_i - z_i\rvert \leq \lvert x_i - y_i\rvert + \lvert y_i - z_i\rvert$$이 성립하므로, $$d_1$$은 양변을 모두 더하여, $$d_\infty$$는 양변에 최댓값을 취하여 삼각부등식을 얻는다. 가령 평면에서 $$x = (0,0)$$, $$y = (3, 4)$$이면

$$d_1(x, y) = 3 + 4 = 7, \qquad d_2(x, y) = \sqrt{3^2 + 4^2} = 5, \qquad d_\infty(x, y) = \max(3, 4) = 4$$

로 세 거리가 모두 다른 값을 준다. 그러나 임의의 $$x, y$$에서

$$d_\infty(x, y) \leq d_2(x, y) \leq d_1(x, y) \leq n\, d_\infty(x, y)$$

가 성립하여, 세 거리는 서로를 상수배로 누른다. 따라서 한 거리로 수렴하는 점열은 다른 거리로도 수렴하며, 세 거리는 $$\mathbb{R}^n$$에 같은 위상을 준다.
...
<ins id="ex8">**예시 8 (이산거리에서의 수렴)**</ins> 집합 $$X$$에 이산거리 $$d$$를 주자. 점열 $$(x_n)$$이 $$x$$로 수렴한다고 하면, $$\varepsilon = \tfrac12$$에 대해 $$N$$이 있어 $$n \geq N$$이면 $$d(x_n, x) < \tfrac12$$이다. 그런데 이산거리에서 거리는 $$0$$ 또는 $$1$$뿐이므로 $$d(x_n, x) < \tfrac12$$은 곧 $$d(x_n, x) = 0$$, 즉 $$x_n = x$$를 뜻한다. 따라서
...
<ins id="ex9">**예시 9 (개구간의 비완비성)**</ins> 표준 거리를 준 개구간 $$X = (0, 1)$$을 생각하자. 점열 $$x_n = \tfrac1n$$ ($$n \geq 2$$)은 $$X$$ 안에 있고, $$\mathbb{R}$$의 점 $$0$$으로 다가가므로

$$d(x_m, x_n) = \left\lvert \frac1m - \frac1n \right\rvert \leq \frac1m + \frac1n \to 0$$

이라 Cauchy이다. 그러나 $$X$$ 안에는 이 점열의 극한이 없다. 만일 $$x_n \to L \in (0,1)$$이라면 $$\mathbb{R}$$에서의 극한 유일성 (명제 6) 에 의해 $$L = 0$$이어야 하는데 $$0 \notin (0,1)$$이기 때문이다. 따라서 $$(0,1)$$은 완비가 아니다. 반면 닫힌구간 $$[0,1]$$에서는 같은 점열이 $$0$$으로 수렴하며, 실제로 $$[0,1]$$은 완비이다 — 닫힌구간은 그 극한들을 모두 품고 있기 때문이다.

</div>

연속함수 공간의 sup 거리 ([예시 2](#ex2)) 는 무한차원 거리공간의 대표적 예이며, 그 완비성이 해석학의 여러 존재 정리를 떠받친다.

<div class="example" markdown="1">

<ins id="ex10">**예시 10 (sup 거리와 균등수렴)**</ins> $$C[a,b]$$에 sup 거리 $$d(f, g) = \sup_{x \in [a,b]} \lvert f(x) - g(x)\rvert$$를 주자. 이 거리에서 $$f_n \to f$$라는 것은

$$\sup_{x \in [a,b]} \lvert f_n(x) - f(x)\rvert \to 0,$$

곧 $$(f_n)$$이 $$f$$로 *균등수렴*한다는 것과 정확히 같다. 가령 $$[0, \tfrac12]$$에서 $$f_n(x) = x^n$$을 보면

$$d(f_n, 0) = \sup_{0 \leq x \leq 1/2} x^n = \left(\frac12\right)^n \to 0$$

이므로 $$f_n \to 0$$이다. 균등수렴하는 연속함수열의 극한은 다시 연속이고, 그 결과 $$C[a,b]$$의 모든 sup-Cauchy 함수열은 어떤 연속함수로 수렴한다. 즉 $$(C[a,b], d)$$는 완비 거리공간이다.

</div>

## 응용: 닫힌 부분공간과 고정점

완비 거리공간의 "닫힌" 부분집합은 다시 완비가 된다는 사실은, 큰 완비공간에서 출발해 그 안의 부분공간의 완비성을 거저 얻는 흔한 수단이다. 부분집합 $$Y \subseteq X$$가 *닫혀 있다<sub>closed</sub>*는 것은 $$Y$$ 안의 점열이 $$X$$에서 수렴할 때 그 극한이 다시 $$Y$$에 속한다는 것이며, 이는 다음 글에서 정식화한다.

<div class="proposition" markdown="1">

<ins id="prop11">**명제 11**</ins> $$(X, d)$$가 완비이고 $$Y \subseteq X$$가 닫혀 있으면, $$Y$$에 $$d$$를 제한한 거리공간도 완비이다.
...
이 명제는 닫힌구간 $$[a,b]$$가 완비라는 사실을 즉시 설명한다. $$[a,b]$$는 완비공간 $$\mathbb{R}$$의 닫힌 부분집합이므로 명제 11에 의해 완비이다 — 예시 9에서 본 $$(0,1)$$과의 차이가 바로 닫힘 여부에 있었던 것이다. 같은 논리로 $$\mathbb{R}^n$$의 닫힌 공이나 구면처럼 닫힌 집합들은 모두 완비 거리공간이 된다.

완비성이 마련해 주는 가장 중요한 도구는 *축약사상<sub>contraction</sub>*의 고정점이다. 거리를 일정 비율로 줄이는 사상을 반복하면, 완비성 덕분에 그 반복열이 Cauchy가 되어 유일한 고정점으로 수렴한다.

<div class="example" markdown="1">

<ins id="ex12">**예시 12 (축약사상의 반복)**</ins> $$X = \mathbb{R}$$에 표준 거리를 주고 $$T(x) = \tfrac12 x + 1$$로 두자. 임의의 두 점에서

$$d(T x, T y) = \left\lvert \tfrac12 x - \tfrac12 y\right\rvert = \tfrac12\, d(x, y)$$

이므로 $$T$$는 비율 $$\lambda = \tfrac12$$의 축약사상이다. $$x_0 = 0$$에서 반복 $$x_{n+1} = T(x_n)$$을 돌리면

$$x_1 = 1,\quad x_2 = \tfrac32,\quad x_3 = \tfrac74,\quad \ldots,\quad x_n = 2 - \frac{1}{2^{\,n-1}}$$

이 되어 $$x_n \to 2$$이고, 극한 $$2$$는 $$T(2) = \tfrac12\cdot 2 + 1 = 2$$를 만족하는 유일한 고정점이다. $$\mathbb{R}$$의 완비성이 이 수렴을 보장하는 핵심이며, 이러한 현상을 일반화한 것이 완비 거리공간 위의 바나흐 고정점 정리이다 ([§미분방정식의 존재성과 유일성, ⁋정리 2](/ko/math/analysis/existence_uniqueness_ode#thm2)).
