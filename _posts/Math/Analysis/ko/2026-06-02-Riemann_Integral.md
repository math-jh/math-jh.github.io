...
디리클레 함수는 모든 점에서 불연속이라는 점에서 [정리 4](#thm4){: data-relation="requires-review" }의 가설과 정반대 극단에 있다. 불연속점이 얼마나 많아야 적분가능성이 깨지는가는 정교한 질문으로, 르베그의 판정 기준에 따르면 Riemann 적분가능성은 불연속점 집합이 (측도 영의 의미에서) 충분히 작은 것과 동치이다. 그 정밀한 형태는 측도론을 필요로 하지만, 연속성을 넘어서는 충분조건 하나는 다르부 틀 안에서 곧바로 증명된다.
