...
[\[미적분학\] §함수와 극한](/ko/math/calculus/functions_and_limits)에서 우리는 극한을 다루며 실수의 "빈틈 없음"을 직관적으로 사용하였고, [\[미적분학\] §연속함수, ⁋정리 5](/ko/math/calculus/continuity#thm5)의 중간값 정리가 유리수 위에서는 성립하지 않음을 지적하였다. 해석학은 이 직관적 성질을 하나의 공리로 명확히 못박는 데서 출발한다. 그것이 *완비성*이며, 미적분학의 모든 존재 정리 — 극한, 최댓값, 적분의 존재 — 가 궁극적으로 여기에 기댄다.
