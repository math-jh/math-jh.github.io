...
Closed 구간이라는 조건은 없앨 수 없다. 반열린 구간 $I_n = \left(0, \frac1n\right]$을 보면 모두 중첩하지만 임의의 $x > 0$은 아르키메데스 성질로 $\frac1n < x$가 되는 순간 빠져나가므로 $\bigcap_n I_n = \emptyset$이다. 구간의 길이 $b_n - a_n$이 $0$으로 줄어드는 경우에는 교집합이 정확히 한 점이 되며, 이 한 점이 모든 $a_n$의 상한이자 모든 $b_n$의 하한이다.
