...
이 되어 $$T_y$$가 축약사상이고, $$\overline{B}$$는 [§거리공간, ⁋명제 9](/ko/math/analysis/metric_spaces#prop9)에 의해 완비 거리공간이다. [§미분방정식의 존재성과 유일성, ⁋정리 2](/ko/math/analysis/existence_uniqueness_ode#thm2)에 의해 각 $$y$$에 대해 유일한 고정점 $$x = f^{-1}(y)$$가 존재하므로 $$f$$가 국소적으로 일대일 대응이다. 역함수의 미분가능성과 $$Df^{-1} = (Df)^{-1}$$은 연쇄법칙을 $$f^{-1}\circ f = \mathrm{id}$$에 적용하여 얻는다. 곧 양변을 미분하면 $$Df^{-1}(f(a))\cdot Df(a) = I$$이므로 $$Df^{-1}(f(a)) = \bigl(Df(a)\bigr)^{-1}$$이다.
:::
