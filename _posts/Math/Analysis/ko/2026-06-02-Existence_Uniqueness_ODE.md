...
존재성과 유일성은 미분방정식 이론의 출발점이다. 물리 법칙이나 모형이 미분방정식으로 기술될 때, 해가 존재하지 않는다면 그 모형은 무의미하고, 해가 여러 개라면 미래가 초기 상태로 결정되지 않는다. 따라서 어떤 조건 아래 초기값 문제가 잘 정의되는가를 가려내는 것은 응용과 이론 모두에서 핵심이 된다.
...
위 증명은 고정점의 존재와 유일성뿐 아니라 *수렴 속도*에 대한 정량적 정보까지 준다. 부등식 $$d(x_m, x_n) \leq \frac{\lambda^n}{1-\lambda} d(x_1, x_0)$$에서 $$m \to \infty$$로 보내면
...
상수 $$L$$을 *립시츠 상수<sub>Lipschitz constant</sub>*라 부른다. 기하적으로 이 조건은 같은 $$t$$에서 그래프 $$y \mapsto f(t, y)$$의 두 점을 잇는 모든 할선의 기울기가 $$\lvert$$기울기$$\rvert \leq L$$로 균등하게 유계임을 뜻한다. 만약 $$f$$가 둘째 변수에 대해 연속미분가능하고 $$\partial f / \partial y$$가 유계이면, 평균값 정리에 의해
...
이다. 이제 구간의 반폭 $$h$$를 $$Lh < 1$$이 되도록, 가령 $$h < 1/L$$로 잡으면 $$T$$가 축약상수 $$\lambda = Lh$$인 축약사상이 된다. 정리 2(바나흐 고정점 정리)에 의해 $$T$$는 유일한 고정점 $$y \in C(I)$$를 가지며, 이 $$y$$가 초기값 문제의 유일한 해이다.
...
피카르 정리의 증명은 단순한 존재 보장에 그치지 않고, 해를 구체적으로 구하는 절차—피카르 반복—를 제공한다. 임의의 출발 함수(보통 상수함수 $$y_0 \equiv y_0$$)에서 시작해 작용소 $$T$$를 거듭 적용하면 그 극한이 해가 된다. 가장 깨끗한 예부터 본다.

<div class="example" markdown="1">

<ins id="ex5">**예시 5 (지수함수)**</ins> 초기값 문제 $$y' = y$$, $$y(0) = 1$$을 피카르 반복으로 푼다. 작용소는 $$(Ty)(t) = 1 + \int_0^t y(s)\,ds$$이다. $$y_0 \equiv 1$$에서 시작하면

$$\begin{aligned}
y_1(t) &= 1 + \int_0^t 1\,ds = 1 + t, \\
y_2(t) &= 1 + \int_0^t (1 + s)\,ds = 1 + t + \frac{t^2}{2}, \\
y_3(t) &= 1 + \int_0^t \Bigl(1 + s + \frac{s^2}{2}\Bigr)ds = 1 + t + \frac{t^2}{2} + \frac{t^3}{6}
\end{aligned}$$

이고, 귀납적으로 $$y_n(t) = \sum_{k=0}^n \dfrac{t^k}{k!}$$이다. 이 부분합은 모든 $$t$$에서 지수함수

$$y(t) = \sum_{k=0}^\infty \frac{t^k}{k!} = e^t$$

로 수렴하며, 알려진 해 $$y = e^t$$와 일치한다.

</div>

다음은 우변이 $$t$$와 $$y$$ 모두에 의존하는 경우로, 같은 절차가 그대로 작동함을 보인다.

<div class="example" markdown="1">

<ins id="ex6">**예시 6 (일차 비제차 방정식)**</ins> $$y' = t + y$$, $$y(0) = 0$$을 풀자. 여기서 $$f(t, y) = t + y$$는 $$\partial f / \partial y = 1$$이 유계이므로 $$L = 1$$의 립시츠 조건을 만족한다. 작용소는 $$(Ty)(t) = \int_0^t (s + y(s))\,ds$$이다. $$y_0 \equiv 0$$에서

$$\begin{aligned}
y_1(t) &= \int_0^t s\,ds = \frac{t^2}{2}, \\
y_2(t) &= \int_0^t \Bigl(s + \frac{s^2}{2}\Bigr)ds = \frac{t^2}{2} + \frac{t^3}{6}, \\
y_3(t) &= \int_0^t \Bigl(s + \frac{s^2}{2} + \frac{s^3}{6}\Bigr)ds = \frac{t^2}{2} + \frac{t^3}{6} + \frac{t^4}{24}
\end{aligned}$$

이다. 일반항은 $$y_n(t) = \sum_{k=2}^{n+1} \dfrac{t^k}{k!}$$이고, 극한은

$$y(t) = \sum_{k=2}^\infty \frac{t^k}{k!} = e^t - 1 - t$$

이다. 실제로 $$y = e^t - 1 - t$$를 대입하면 $$y' = e^t - 1 = t + (e^t - 1 - t) = t + y$$이고 $$y(0) = 0$$이므로 해가 맞다.

</div>

립시츠 조건이 빠지면 유일성이 어떻게 깨지는지를 명시적인 반례로 본다.

<div class="example" markdown="1">

<ins id="ex7">**예시 7 (유일성의 실패)**</ins> $$y' = \sqrt{\lvert y\rvert}$$, $$y(0) = 0$$을 생각하자. 여기서 $$f(t, y) = \sqrt{\lvert y\rvert}$$는 연속이지만 $$y = 0$$ 근방에서
...
$$y(t) = \begin{cases} \dfrac{t^2}{4}, & t \geq 0 \\ 0, & t < 0 \end{cases}$$

도 해이다 ($$t \geq 0$$에서 $$y' = t/2 = \sqrt{t^2/4} = \sqrt{y}$$). 더 나아가 임의의 $$a > 0$$에 대해 $$t \leq a$$에서는 $$0$$이고 $$t > a$$에서는 $$\tfrac14(t - a)^2$$인 함수도 모두 해이므로, 해가 무수히 많다. 립시츠 조건의 부재가 유일성을 송두리째 무너뜨림을 보여 준다.
...
<ins id="prop8">**명제 8 (전역 존재)**</ins> $$f(t, y)$$가 띠 영역 $$\lvert t - t_0\rvert \leq a$$, $$y \in \mathbb{R}$$ 전체에서 연속이고 $$y$$에 대해 상수 $$L$$의 전역 립시츠 조건을 만족하면, 초기값 문제의 해가 구간 $$[t_0 - a, t_0 + a]$$ 전체에서 유일하게 존재한다.
...
을 얻는다. 따라서 $$\lvert t - t_0\rvert \leq a$$에서 $$\lVert y_{n+1} - y_n\rVert_\infty \leq M\,\dfrac{(La)^{n+1}}{L\,(n+1)!}$$이고, 우변은 지수급수의 항이라 그 합이 유한하다. 망원합 $$y_n = y_0 + \sum_{k=0}^{n-1}(y_{k+1} - y_k)$$이 상한노름에서 절대수렴하므로 $$y_n$$은 어떤 연속함수 $$y$$로 균등수렴하고, 극한에서 $$y = Ty$$이다. 유일성은 정리 4의 논법을 구간 전체에서 그뢴발 부등식으로 잇거나, 구간을 길이 $$1/(2L)$$ 이하의 조각으로 나누어 각 조각에서 정리 2를 적용해 얻는다.

</details>

명제 8의 핵심은 립시츠 상수 $$L$$이 $$y$$의 크기에 무관하게 *하나의 상수*로 잡힌다는 데 있다. 그러면 피카르 반복의 차가 $$L^n / n!$$의 빠르기로 줄어 구간 길이에 관계없이 수렴하므로, 정리 4에서 구간을 줄일 필요가 없어진다. 전역 립시츠 조건이 성립하는 가장 중요한 경우가 우변이 $$y$$에 대해 선형인 방정식이다.

<div class="example" markdown="1">

<ins id="ex9">**예시 9 (선형 방정식의 전역해)**</ins> $$y' = a(t)\,y + b(t)$$에서 $$a, b$$가 닫힌구간 $$I$$에서 연속이면, $$\lvert a(t)\rvert \leq L$$인 상수 $$L$$이 존재하여

$$\lvert f(t, y_1) - f(t, y_2)\rvert = \lvert a(t)\rvert\,\lvert y_1 - y_2\rvert \leq L\,\lvert y_1 - y_2\rvert$$

이므로 전역 립시츠 조건이 성립한다. 명제 8에 의해 해가 $$I$$ 전체에서 유일하게 존재한다. 가령 $$y' = y$$의 해 $$e^t$$, $$y' = -2t\,y$$의 해 $$e^{-t^2}$$처럼 선형 방정식의 해는 정의 구간 밖으로 폭발하지 않고 끝까지 살아남는다.

</div>

반면 비선형 방정식에서는 해가 유한 시간에 발산할 수 있다. 다음 예가 그 전형이다.

<div class="example" markdown="1">

<ins id="ex10">**예시 10 (유한 시간 폭발)**</ins> $$y' = y^2$$, $$y(0) = 1$$을 보자. 여기서 $$f(t, y) = y^2$$는 임의의 유계 영역에서는 립시츠 조건을 만족하므로 국소해가 존재하고, 변수분리로
...
이다. 이 해는 $$t \to 1^-$$에서 $$+\infty$$로 발산하므로 구간 $$[0, 1)$$ 너머로 연장되지 않는다. $$\partial f / \partial y = 2y$$가 $$y$$와 함께 무한히 커져 전역 립시츠 조건이 깨지는 것이 폭발의 원인이며, 정리 4의 국소성이 본질적임을 보여 준다.
...
<ins id="prop11">**명제 11 (반복의 오차)**</ins> 정리 4의 가정 아래, 피카르 반복 $$y_n = T^n y_0$$과 참해 $$y$$ 사이에는

$$\lVert y_n - y\rVert_\infty \leq \frac{(Lh)^n}{1 - Lh}\,\lVert y_1 - y_0\rVert_\infty$$

이 성립한다. 여기서 $$h$$는 정리 4에서 잡은 구간의 반폭이고 $$Lh < 1$$이다.
...
작용소 $$T$$는 $$C(I)$$ 위에서 축약상수 $$\lambda = Lh < 1$$의 축약사상이고 $$y$$는 그 고정점이므로, 정리 2의 증명에서 얻은 선험적 오차 평가
...
이렇게 축약사상 원리는 미분방정식의 초기값 문제를 고정점 문제로 환원하여, 해의 존재·유일성을 한꺼번에 그리고 구성적으로 확립한다. $$f$$가 $$y$$에 대해 선형이면 립시츠 조건이 전역적으로 성립하여 해가 구간 전체로 확장되는데(명제 8, 예시 9), 이 중요한 경우를 다음 글 [§선형 미분방정식계](/ko/math/analysis/linear_ode)에서 더 깊이 다룬다.
