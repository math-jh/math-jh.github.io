...
여집합을 취하면 명제 2가 닫힌집합에 대한 쌍대 명제로 그대로 옮겨진다. 드모르간 법칙에 의해 합집합과 교집합의 역할이 뒤바뀌므로, 닫힌집합의 모임은 임의의 교집합과 유한 합집합에 대해 닫혀 있다. 이를 정리하면 다음과 같다.
...
을 쓰면, 임의 교집합의 여집합은 열린집합들의 합집합이라 명제 2의 (2)로 열려 있고, 유한 합집합의 여집합은 열린집합들의 유한 교집합이라 명제 2의 (3)으로 열려 있다. 따라서 각각의 본래 집합이 닫혀 있다. (1)은 $$\varnothing$$과 $$X$$가 서로의 여집합이며 둘 다 열려 있음에서 따른다.

</details>

명제 2에서 무한 교집합이 열림을 보장하지 못했던 것처럼, 여기서도 무한 합집합은 닫혀 있지 않을 수 있다. 닫힌구간들의 합집합 $$\bigcup_{n\geq 1} \bigl[\tfrac1n, 1\bigr] = (0, 1]$$은 $$\mathbb{R}$$에서 닫힌집합이 아니다. 점열 $$x_n = \tfrac1n$$이 이 집합 안에 있으면서 극한 $$0$$이 밖에 있기 때문이며, 이는 명제 4의 점열 판정으로도 곧장 확인된다.

## 예시와 계산

추상적 정의에 살을 붙이기 위해 실직선과 평면에서 구체적인 집합들을 분류한다. 핵심은 정의 1로 돌아가 "각 점에서 양의 반지름을 가진 공이 통째로 들어가는가"를 직접 확인하는 데 있다.
...
<ins id="ex6">**예시 6 (실직선의 구간)**</ins> $$\mathbb{R}$$에서 표준거리 $$d(x, y) = \lvert x - y\rvert$$를 쓰면 열린공은 열린구간 $$B(x, r) = (x - r, x + r)$$이다. 열린구간 $$(a, b)$$는 열린집합이다: 점 $$x \in (a, b)$$에 대해

$$r = \min\{\,x - a,\ b - x\,\} > 0$$

으로 두면 $$(x - r, x + r) \subseteq (a, b)$$이기 때문이다. 닫힌구간 $$[a, b]$$는 닫힌집합이다: 그 여집합 $$(-\infty, a) \cup (b, \infty)$$가 두 열린구간의 합집합이라 열려 있다. 반열린구간 $$[a, b)$$는 열려 있지도 닫혀 있지도 않다. 점 $$a$$에서는 어떤 공도 왼쪽으로 새어나가 $$[a, b)$$에 담기지 못하므로 열린집합이 아니고, 점열 $$x_n = b - \tfrac1n \in [a, b)$$의 극한 $$b$$가 밖에 있으므로 닫힌집합도 아니다.

</div>

열린집합도 닫힌집합도 아닌 집합이 존재한다는 사실은 두 개념이 서로의 부정이 아님을 분명히 한다. "열려 있지 않음"이 "닫혀 있음"을 뜻하지 않는다. 반대 극단으로 두 성질을 동시에 갖는 집합도 있다.

<div class="example" markdown="1">

<ins id="ex7">**예시 7 (동시에 열리고 닫힌 집합)**</ins> 임의의 거리공간 $$X$$에서 $$\varnothing$$과 $$X$$는 명제 2와 명제 5에 의해 둘 다 열려 있고 닫혀 있다. 이렇게 열리면서 닫힌 집합을 *열린닫힌집합<sub>clopen set</sub>*이라 한다. 연결되지 않은 공간에서는 자명하지 않은 열린닫힌집합이 나타난다. 부분공간 $$X = (0, 1) \cup (2, 3) \subseteq \mathbb{R}$$에서 조각 $$(0, 1)$$은 $$X$$의 열린집합이면서, 그 여집합 $$(2, 3)$$도 열려 있어 동시에 닫힌집합이다. 한 공간에서 자명하지 않은 열린닫힌집합의 유무가 곧 [§연결성과 중간값 정리](/ko/math/analysis/connectedness)에서 다루는 연결성의 척도가 된다.

</div>

폐포는 점열의 극한을 모두 더해 집합을 닫는 연산이다. 정의로 직접 계산하기보다 명제 4의 점열 특징화를 쓰면 폐포를 손쉽게 결정할 수 있다. 다음 예시는 대표적인 폐포 계산을 모은 것이다.

<div class="example" markdown="1">

<ins id="ex8">**예시 8 (폐포 계산)**</ins> $$\mathbb{R}$$에서 다음이 성립한다.

$$\overline{(a, b)} = [a, b], \qquad \overline{\mathbb{Q}} = \mathbb{R}, \qquad \overline{\Bigl\{\tfrac1n : n \in \mathbb{N}\Bigr\}} = \Bigl\{\tfrac1n : n \in \mathbb{N}\Bigr\} \cup \{0\}.$$

첫째는 끝점 $$a, b$$가 각각 $$x_n = a + \tfrac1n$$, $$y_n = b - \tfrac1n$$의 극한으로 도달되기 때문이다. 둘째는 유리수의 조밀성에서 따른다: 임의의 실수 $$x$$에 대해 $$x_n \to x$$인 유리수열을 잡을 수 있으므로 $$\mathbb{R} \subseteq \overline{\mathbb{Q}}$$이고, 역포함은 자명하다. 셋째에서 새로 더해지는 점은 점열 $$\tfrac1n$$의 유일한 극한 $$0$$뿐이다.

</div>

셋째 예시는 폐포가 점열의 극한을 더하는 연산임을 가장 또렷이 보여 준다. 집합 $$\{\tfrac1n\}$$ 자체는 닫혀 있지 않다. 점열

$$x_n = \frac1n \longrightarrow 0$$

의 극한 $$0$$이 집합 밖에 있어 명제 4의 판정에 걸리기 때문이다. 이 한 점을 더한 $$\{\tfrac1n\} \cup \{0\}$$은 더 이상 빠져나갈 극한이 없어 닫히며, 그것이 곧 폐포이다.

폐포는 닫힘 연산으로서 깔끔한 대수적 성질을 가진다. 특히 합집합과는 잘 어울리지만 교집합과는 그렇지 않다.

<div class="proposition" markdown="1">

<ins id="prop9">**명제 9**</ins> $$A, B \subseteq X$$에 대하여 $$\overline{A \cup B} = \overline{A} \cup \overline{B}$$이다. 그러나 일반적으로 $$\overline{A \cap B} \subseteq \overline{A} \cap \overline{B}$$만 성립하며 등호는 성립하지 않을 수 있다.
...
이다. 역포함을 위해 $$\overline{A} \cup \overline{B}$$가 닫힌집합임을 보면 충분하다 (명제 5의 (3), 두 닫힌집합의 합집합). $$A \cup B \subseteq \overline{A} \cup \overline{B}$$이고 우변이 $$A \cup B$$를 포함하는 닫힌집합이므로, 폐포가 그러한 가장 작은 닫힌집합이라는 정의에서 $$\overline{A \cup B} \subseteq \overline{A} \cup \overline{B}$$를 얻는다. 교집합의 경우 $$A \cap B \subseteq A$$에서 $$\overline{A \cap B} \subseteq \overline{A}$$, 같은 식으로 $$\overline{A \cap B} \subseteq \overline{B}$$이므로 $$\overline{A \cap B} \subseteq \overline{A} \cap \overline{B}$$이다. 등호가 깨지는 예로 $$A = (0, 1)$$, $$B = (1, 2)$$를 들면 $$A \cap B = \varnothing$$이라 $$\overline{A \cap B} = \varnothing$$이지만, $$\overline{A} \cap \overline{B} = [0, 1] \cap [1, 2] = \{1\}$$이다.
...
<ins id="ex10">**예시 10 (내부·폐포·경계)**</ins> $$\mathbb{R}^2$$에서 닫힌 단위원판 $$A = \{\,(x, y) : x^2 + y^2 \leq 1\,\}$$을 보자. 표준거리에서
...
<ins id="ex11">**예시 11 (이산거리)**</ins> 집합 $$X$$ 위에 *이산거리<sub>discrete metric</sub>*를

$$d(x, y) = \begin{cases} 0 & (x = y) \\ 1 & (x \neq y) \end{cases}$$

로 주자. 반지름 $$r \leq 1$$인 열린공은 $$B(x, r) = \{x\}$$ 한 점뿐이다. 따라서 모든 한원소집합 $$\{x\}$$가 열린집합이고, 임의의 부분집합은 한원소 열린집합들의 합집합이라 명제 2의 (2)에 의해 열려 있다. 곧 $$X$$의 모든 부분집합이 열린집합이며, 여집합도 모두 열려 있으므로 모든 부분집합이 동시에 닫힌집합이다. 이 공간에서는 수렴하는 점열이 결국 한 점에 머무는 점열뿐이라, 명제 4의 점열 판정으로도 모든 집합이 닫혀 있음을 다시 확인할 수 있다.
