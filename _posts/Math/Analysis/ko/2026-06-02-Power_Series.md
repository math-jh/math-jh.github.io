...
정리 3의 마지막 등식 $$c_n = f^{(n)}(a)/n!$$은 해석함수가 자신을 정의하는 멱급수에 의해 계수까지 완전히 결정됨을 말한다. 이로부터 곧바로 다음의 유일성이 따른다: 한 함수가 같은 중심에서 두 멱급수로 표현되면 두 급수의 모든 계수가 일치한다.
...
두 멱급수의 공통의 합을 $$f$$라 두면, 정리 3에 의해 $$f$$는 $$\lvert x - a\rvert < R$$에서 무한히 미분가능하고 양쪽 표현으로부터 계수가 동시에 도함수로 읽힌다. 곧
...
명제 5는 멱급수 전개가 유일함을 보장하므로, 어떤 방법으로든 함수를 멱급수로 적어내기만 하면 그것이 곧 테일러 급수임을 안다. 가령 점화식이나 미정계수법으로 얻은 급수도, 정리 3을 통해 자동으로 해당 함수의 테일러 전개가 된다.

## 수렴반경의 계산

코시–아다마르 공식 (정리 1) 은 모든 멱급수에 통하지만, 실제 계산에서는 비판정이 더 다루기 쉬운 경우가 많다. 계수의 비 $$\lvert c_{n+1}/c_n\rvert$$이 극한을 가지면 그 극한의 역수가 곧 수렴반경이 된다.

<div class="proposition" markdown="1">

<ins id="prop6">**명제 6 (비판정에 의한 수렴반경)**</ins> $$c_n \neq 0$$이 충분히 큰 $$n$$에서 성립하고 $$\displaystyle\lim_{n\to\infty}\left\lvert\frac{c_{n+1}}{c_n}\right\rvert = L$$이 존재하면, 멱급수 $$\sum c_n (x-a)^n$$의 수렴반경은 $$R = 1/L$$이다 ($$L = 0$$이면 $$R = \infty$$, $$L = \infty$$이면 $$R = 0$$).
...
이다. 이 값이 $$1$$보다 작으면, 곧 $$\lvert x - a\rvert < 1/L$$이면 절대수렴하고, $$1$$보다 크면 발산한다. 따라서 수렴반경은 $$R = 1/L$$이다. 비판정으로 극한이 존재할 때 근판정의 $$\limsup$$도 같은 값을 가지므로, 이는 정리 1과 모순되지 않는다.

</details>

비의 극한이 존재하지 않는 급수도 있는데, 이때는 코시–아다마르 공식의 $$\limsup$$만이 항상 답을 준다. 다음 두 예시로 두 방법을 비교한다.

<div class="example" markdown="1">

<ins id="ex7">**예시 7 (비판정으로 계산)**</ins> 멱급수 $$\displaystyle\sum_{n=0}^\infty \frac{n^2}{2^n}\,x^n$$의 수렴반경을 명제 6으로 구한다. $$c_n = n^2/2^n$$이므로

$$\begin{aligned}
\left\lvert\frac{c_{n+1}}{c_n}\right\rvert
&= \frac{(n+1)^2/2^{n+1}}{n^2/2^n} \\
&= \frac{1}{2}\cdot\frac{(n+1)^2}{n^2} \\
&\xrightarrow{n\to\infty} \frac{1}{2}
\end{aligned}$$

이므로 $$L = 1/2$$이고 따라서 $$R = 2$$이다. 즉 이 급수는 $$\lvert x\rvert < 2$$에서 수렴하고 $$\lvert x\rvert > 2$$에서 발산한다.

</div>

<div class="example" markdown="1">

<ins id="ex8">**예시 8 (근판정이 필요한 경우)**</ins> 계수가 짝·홀에 따라 달라 비의 극한이 없을 때는 근판정을 쓴다. 멱급수
...
이므로 코시–아다마르 공식 (정리 1) 에 의해 $$R = 1/3$$이다. 이처럼 $$\limsup$$은 진동하는 계수에도 항상 정의된다.

</div>

## 응용: 항별 미분의 계산

정리 3의 항별 미분은 알려진 멱급수에서 새 항등식을 끌어내는 도구이다. 수렴반경 안에서 멱급수는 다항식처럼 다룰 수 있으므로, 미분·적분과 대입을 자유롭게 조합해 함수의 값이나 수치급수의 합을 얻는다.

<div class="example" markdown="1">

<ins id="ex9">**예시 9 (등비급수의 미분)**</ins> 기하급수 $$\dfrac{1}{1 - x} = \displaystyle\sum_{n=0}^\infty x^n$$ ($$\lvert x\rvert < 1$$) 을 정리 3으로 항별 미분하면
...
이므로 $$\dfrac{1}{(1-x)^2} = \displaystyle\sum_{n=1}^\infty n\,x^{n-1}$$이고, 양변에 $$x$$를 곱하면

$$\sum_{n=1}^\infty n\,x^n = \frac{x}{(1-x)^2} \qquad (\lvert x\rvert < 1)$$

을 얻는다. $$x = \tfrac12$$을 대입하면 $$\displaystyle\sum_{n=1}^\infty \frac{n}{2^n} = \frac{1/2}{(1/2)^2} = 2$$라는 수치급수의 합이 따라 나온다.

</div>

<div class="example" markdown="1">

<ins id="ex10">**예시 10 (지수함수의 자기 미분)**</ins> $$e^x = \displaystyle\sum_{n=0}^\infty \frac{x^n}{n!}$$의 수렴반경은 명제 6에서 $$\lvert c_{n+1}/c_n\rvert = 1/(n+1) \to 0$$이므로 $$R = \infty$$이다. 정리 3으로 항별 미분하면

$$\begin{aligned}
(e^x)' &= \sum_{n=1}^\infty \frac{n\,x^{n-1}}{n!} \\
&= \sum_{n=1}^\infty \frac{x^{n-1}}{(n-1)!} \\
&= \sum_{m=0}^\infty \frac{x^m}{m!} = e^x
\end{aligned}$$

이므로 $$(e^x)' = e^x$$가 급수 차원에서 확인된다. 같은 계산이 $$\sin, \cos$$에도 적용되어 항별 미분만으로 기본 도함수 공식이 모두 회복된다.
