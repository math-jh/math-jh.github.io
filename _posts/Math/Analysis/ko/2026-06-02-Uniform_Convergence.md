...
[§함수열과 점별수렴](/ko/math/analysis/sequences_of_functions){: data-lid="cehwn" data-relation="required" reviewed="" }에서 점별수렴이 연속성과 적분을 보존하지 못함을 보았다. 그 병폐의 원인은 수렴 속도가 점마다 다른 데 있었다. 이를 막아 수렴이 정의역 전체에서 고르게 일어나도록 요구하는 것이 균등수렴이다.
...
::: 증명
$f$는 [정리 2](#thm2){: data-lid="5ag55" data-relation="required" reviewed="" }로 연속이므로 적분가능하다 ([§Riemann integral, ⁋정리 4](/ko/math/analysis/riemann_integral#thm4){: data-lid="e62ul" data-relation="required" reviewed="" }). 단조성에 의해
...
[§함수열과 점별수렴, ⁋예시 3](/ko/math/analysis/sequences_of_functions#ex3){: data-lid="fcab8" data-relation="weak" reviewed="" }의 반례에서 적분이 교환되지 않았던 것은 그 수렴이 균등하지 않았기 때문이다. (미분에 대해서는 더 강하게, $f_n'$ 자체의 균등수렴을 요구해야 극한과 미분이 교환된다.)
...
극한함수를 미리 알지 못하는 상황에서 균등수렴을 직접 확인하는 도구가 균등 Cauchy 조건이다. 수치열에서 Cauchy 조건이 극한을 언급하지 않고 수렴을 가려냈던 것처럼 ([§Cauchy 수열과 완비성, ⁋정리 4](/ko/math/analysis/cauchy_sequences#thm4){: data-lid="mxpef" data-relation="weak" reviewed="" }), 함수열에서도 같은 일을 한다.
...
역으로 $(f_n)$이 균등 Cauchy라 하자. 각 고정된 $x$에서 수열 $(f_n(x))$은 (균등 Cauchy 조건의 특수한 경우로) 수치 Cauchy 수열이므로 실수의 완비성에 의해 어떤 값으로 수렴한다 ([§Cauchy 수열과 완비성, ⁋정리 4](/ko/math/analysis/cauchy_sequences#thm4){: data-lid="nn0fw" data-relation="required" reviewed="" }). 그 극한을 $f(x)$로 두면 점별극한 $f$가 정의된다. 이제 균등 Cauchy 조건에서 $m, n \geq N$이면 모든 $x$에 대해 $\lvert f_m(x) - f_n(x)\rvert < \varepsilon$인데, 여기서 $m \rightarrow \infty$로 보내면 $f_m(x) \rightarrow f(x)$이므로
...
균등 Cauchy 조건은 극한 $f$를 식에 끌어들이지 않으므로, M-판정의 증명에서 부분합열의 균등수렴을 확인할 때처럼 극한을 모르는 채로 균등수렴을 결론짓는 데 특히 유용하다. 실제로 [정리 4](#thm4){: data-lid="zk5vs" data-relation="weak" reviewed="" }의 증명은 부분합열이 균등 Cauchy임을 보인 것에 다름 아니다.
