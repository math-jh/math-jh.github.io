...
M-판정의 위력은 극한함수의 정체를 전혀 몰라도, 각 항의 크기를 $$x$$에 무관한 수 $$M_n$$으로 위에서 누르기만 하면 균등수렴이 따라 나온다는 데 있다. 이때 $$\sum M_n$$이 수렴하는지는 [§무한급수](/ko/math/analysis/series)에서 익힌 수치급수 판정법으로 결정되므로, 함수항급수의 균등수렴 문제가 익숙한 수치급수의 수렴 문제로 환원된다. 다만 M-판정은 충분조건일 뿐이어서, 이를 통과하지 못한다고 해서 균등수렴이 곧 부정되는 것은 아니다.
...
균등 Cauchy 조건은 극한 $$f$$를 식에 끌어들이지 않으므로, M-판정의 증명에서 부분합열의 균등수렴을 확인할 때처럼 극한을 모르는 채로 균등수렴을 결론짓는 데 특히 유용하다. 실제로 정리 4의 증명은 부분합열이 균등 Cauchy임을 보인 것에 다름 아니다.

## 예시와 계산

<div class="example" markdown="1">

<ins id="ex6">**예시 6 (M-판정의 적용)**</ins> 급수 $$\displaystyle\sum_{n=1}^\infty \frac{\sin(nx)}{n^2}$$을 $$\mathbb{R}$$ 전체에서 보자. 모든 $$x$$에 대해

$$\left\lvert \frac{\sin(nx)}{n^2}\right\rvert \leq \frac{1}{n^2} =: M_n$$

이고 $$\sum 1/n^2$$이 ($$p=2$$인 $$p$$-급수로) 수렴하므로, M-판정에 의해 이 급수는 $$\mathbb{R}$$ 전체에서 균등수렴한다. 각 항이 연속이므로 정리 2에 의해 그 합 또한 $$\mathbb{R}$$에서 연속이다.

</div>

위 급수는 좌변의 $$x$$ 의존성을 $$\lvert\sin\rvert \leq 1$$로 한 번에 지워 버린 것이 핵심이다. 같은 방식으로 $$\sum \cos(nx)/n^2$$, 더 일반적으로 $$\sum a_n \sin(nx)$$이 $$\sum \lvert a_n\rvert < \infty$$이면 균등수렴한다.

<div class="example" markdown="1">

<ins id="ex7">**예시 7 (구간을 좁혀야 하는 기하급수)**</ins> 기하급수 $$\displaystyle\sum_{n=0}^\infty x^n$$을 보자. $$0 < r < 1$$로 고정하면 $$\lvert x\rvert \leq r$$인 모든 $$x$$에서 $$\lvert x^n\rvert \leq r^n =: M_n$$이고 $$\sum r^n$$이 수렴하므로, M-판정에 의해 급수는 $$[-r, r]$$에서 균등수렴한다. 그러나 열린구간 $$(-1, 1)$$ 전체에서는 균등수렴하지 않는다. 실제로 부분합과 합의 차는
...
예시 7은 균등수렴이 정의역에 민감함을 보여 준다. 같은 급수라도 컴팩트한 부분구간으로 제한하면 균등수렴하지만 경계에 다가가는 점들을 포함하면 그렇지 않을 수 있는데, 이 현상은 멱급수가 수렴반경 안의 컴팩트 부분에서만 균등수렴하는 일반 원리 ([§멱급수와 해석함수, ⁋정리 2](/ko/math/analysis/power_series#thm2)) 의 가장 단순한 사례이다.

<div class="example" markdown="1">

<ins id="ex8">**예시 8 (균등수렴이라도 미분은 안 됨)**</ins> $$\mathbb{R}$$에서 $$f_n(x) = \dfrac{\sin(nx)}{\sqrt{n}}$$을 보자. 모든 $$x$$에서
...
<ins id="ex9">**예시 9 (균등극한과 적분의 교환)**</ins> 적분 교환이 성립하는 예로 $$g_n(x) = \dfrac{x^n}{1 + x^n}$$을 $$[0, r]$$ ($$0 < r < 1$$) 에서 보자. $$0 \leq x \leq r$$이면

$$0 \leq g_n(x) = \frac{x^n}{1 + x^n} \leq x^n \leq r^n$$

이므로 $$\lVert g_n - 0\rVert_\infty \leq r^n \to 0$$, 곧 $$g_n$$은 $$[0,r]$$에서 $$0$$으로 균등수렴한다. 정리 3에 의해

$$\lim_{n\to\infty} \int_0^r g_n(x)\,dx = \int_0^r 0\,dx = 0$$

이고, 실제로 $$0 \leq \int_0^r g_n \leq \int_0^r r^n\,dx = r^{n+1} \to 0$$이어서 직접 계산과도 일치한다.

</div>

이 예들이 보여 주듯, 균등극한과 적분의 교환은 상한노름에 의한 통제(정리 3)로 손쉽게 정당화된다. 마지막으로 균등수렴이 함수의 유계성을 보존함을 확인해 두자.

<div class="proposition" markdown="1">

<ins id="prop10">**명제 10 (유계성의 보존)**</ins> 유계함수들의 열 $$(f_n)$$이 $$f$$로 균등수렴하면 $$f$$도 유계이다.
...
유계성의 보존은 점별수렴에서는 깨지는 성질이다 ([§함수열과 점별수렴, ⁋예시 6](/ko/math/analysis/sequences_of_functions#ex6)). 점별수렴은 각 점에서 독립적으로 수렴값을 줄 뿐이어서, 점마다 다른 속도로 수렴하면 극한이 무한정 커질 수 있다. 균등수렴은 단 하나의 $$N$$으로 모든 점을 동시에 잡아 두므로 이런 일이 일어나지 않으며, 명제 10은 그 차이를 가장 단순하게 드러낸다.
