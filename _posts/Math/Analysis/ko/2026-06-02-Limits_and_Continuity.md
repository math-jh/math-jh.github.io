...
거리 $\lvert x - a\rvert$를 $d_X(x, a)$로 바꾸었을 뿐, [\[미적분학\] §함수의 극한](/ko/math/calculus/functions_and_limits)의 정의와 형태가 같다. 같은 방식으로 $x \rightarrow a$일 때 $f(x) \rightarrow L$인 함수의 극한도 정의된다.
...
::: 증명
$f$가 $a$에서 연속이고 $x_n \rightarrow a$라 하자. 임의의 $\varepsilon > 0$에 대해 [정의 1](#def1)의 $\delta$를 잡고, $x_n \rightarrow a$이므로 큰 $n$에서 $d_X(x_n, a) < \delta$, 따라서 $d_Y(f(x_n), f(a)) < \varepsilon$이다. 즉 $f(x_n) \rightarrow f(a)$이다.
...
역으로 열린집합의 preimage가 항상 열려 있다 하자. $a \in X$와 $\varepsilon > 0$에 대해 $V = B(f(a), \varepsilon)$은 열려 있으므로 $f^{-1}(V)$도 열려 있고 $a$를 포함하므로 $B(a, \delta) \subseteq f^{-1}(V)$인 $\delta$가 있다. 이는 [정의 1](#def1)의 연속 조건이다.
:::
...
[정의 1](#def1)은 점 $a$에서의 *함숫값* $f(a)$와 그 근방에서의 값을 한꺼번에 비교한다. 그러나 많은 경우 우리는 $f$가 $a$에서 정의되어 있는지와 무관하게 $x$를 $a$에 가까이 보낼 때 $f(x)$가 어디로 다가가는지만을 묻는다. 이를 위해 함수의 극한을 따로 정의한다. 여기서 $a$는 정의역 $X$의 *집적점<sub>limit point</sub>*, 즉 임의의 $\delta > 0$에 대해 $0 < d_X(x, a) < \delta$인 $x \in X$가 존재하는 점이라 가정한다.
...
[정의 1](#def1)과의 차이는 두 가지뿐이다. 첫째, 비교 대상이 $f(a)$가 아니라 임의의 후보 값 $L$이다. 둘째, $x = a$ 자체는 $0 < d_X(x, a)$라는 조건으로 배제된다. 이로부터 $f$가 $a$에서 연속인 것은 $a$가 집적점일 때 $\lim_{x \rightarrow a} f(x) = f(a)$인 것과 동치임이 즉시 따른다. 즉 연속은 극한이 존재하고 그 값이 함숫값과 같다는 조건의 다른 이름이다.
...
::: 증명
$a$로 수렴하는 임의의 점열 $(x_n)$을 잡자. [명제 2](#prop2)에 의해 $f(x_n) \rightarrow f(a)$이고 $g(x_n) \rightarrow g(a)$이다. 실수열의 [§수열의 수렴, ⁋정리 4](/ko/math/analysis/convergence_of_sequences#thm4)에 의해
...
이므로, [명제 2](#prop2)에 의해 $f$는 $0$에서 연속이 아니다. 같은 사실을 [명제 3](#prop3)으로도 볼 수 있다. $f(0) = 0$을 품는 열린집합 $V = (-1/2,  1/2)$의 preimage는
