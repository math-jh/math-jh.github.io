...
$V,W$가 $n$차원 $\mathbb{R}$ 벡터공간이라 하고, linear map $L:V\rightarrow W$가 주어졌다 하자. 그럼 [\[다중선형대수학\] §텐서대수, ⁋명제 11](/ko/math/multilinear_algebra/tensor_algebras#prop11){: data-lid="9t16q" data-relation="required" }의 universal property에 의하여, 다음의 linear map
...
::: 예시 3
임의의 Lie group은 orientable이다. 이는 [\[리 이론\] §리 군, ⁋정의 13](/ko/math/lie_theory/Lie_groups#def13){: data-lid="xfh4p" data-relation="required" }의 $\Omega_\text{l.inv}^1(G)$에서의 임의의 basis $\omega_1,\ldots,\omega_n$을 택한 후, 이들의 wedge $\omega_1\wedge\cdots\wedge\omega_n$을 생각하면 이것이 $G$ 위에서 nonvanishing $n$-form을 정의하기 때문이다.
:::
