...
이제 $y\in P$가 임의로 주어졌다 하고, $\Phi(y)$를 포함하는 $M$의 coordinate system $(W,z^1,\ldots, z^m)$를 택하자. 그럼 [§부분다양체와 역함수 정리, ⁋따름정리 10](/ko/math/manifolds/submanifolds#cor10){: data-lid="lv5fb" }으로부터 집합 $\{z^k\circ\Phi\mid 1\leq k\leq m\}$의 부분집합을 적당한 열린근방 $V$로 제한한 것이 점 $y\in P$의 coordinate system을 이룬다는 것을 안다. 
...
위의 절에서 정의한 $(A,\iota)$는 equivalence class $[(N,\Phi)]$마다 <em-ko>유일하게</em-ko> 결정된다. 우선 [정의 3](#def3){: data-lid="z8657" }에서의 $\theta$는 diffeomorphism이므로 특히 전단사함수이고, 따라서 
...
:::
::: 증명
[명제 2](#prop2){: data-lid="zv4if" }의 첫째 주장에 의해 자명하다. 
:::
...
:::
::: 증명
우선 앞선 [명제 4](#prop4){: data-lid="k0mgi" }를 subspace topology $\mathcal{T}$에 적용하면, $(A,\mathcal{T},\iota)$를 $M$의 submanifold로 만드는 미분구조는 유일하다. 이를 $\mathcal{A}$라 하자. 이제 $(A,\iota)$를 $M$의 submanifold로 만드는 위상 $\mathcal{T}'$와 미분구조 $\mathcal{A}'$가 주어졌다 하고, 다음 diagram을 생각하자.

{% diagram Math/Manifolds/Uniqueness_of_Submanifold-4.svg width="12.99em" alt="uniqueness" %}

여기서 $\iota$와 $\iota'$는 모두 $A\hookrightarrow M$이지만, 구별을 위해 다른 이름으로 표기하였다. 정의에 의해 $(A,\mathcal{T},\mathcal{A})$가 $M$의 embedded submanifold이므로, 수직방향의 $\iota$는 embedding이고 따라서 [명제 2](#prop2){: data-lid="am0a8" }에 의하여 $\id$는 $C^\infty$이다. 또, 연쇄법칙에 의하여
