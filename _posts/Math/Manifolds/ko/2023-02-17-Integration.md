...
::: 보조정리 6
[정의 5](#def5)는 잘 정의된다. 즉 $\supp\omega$를 포함하는 positively oriented coordinate system $(U,x)$, $(V,y)$가 주어졌다면
...
::: 참고 10
$M$이 connected orientable일 때 orientation의 선택지는 $\Lambda^+$와 $\Lambda^-$ 두 가지이다. 반대쪽 component를 택하면 positively oriented coordinate system의 역할이 뒤바뀌고, [정의 5](#def5)의 각 항의 부호가 바뀌므로 적분 전체의 부호가 바뀐다. 즉 $\int_{-M}\omega=-\int_M\omega$이다. 여기서 $-M$은 반대 orientation이 주어진 $M$을 뜻한다.
:::
