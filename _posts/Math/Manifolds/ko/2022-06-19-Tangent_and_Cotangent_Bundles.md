...
이제 우리는 벡터장을 정의해야 하는데, $C^\infty$ 벡터장과 같은 개념을 정의하기 위해서는 vector bundle의 개념을 먼저 정의하는 것이 좋다. 우선 위상공간 위에 정의된 vector bundle을 먼저 정의하자.

::: 정의 1
위상공간 $B$ 위에 정의된 *vector bundle<sub>벡터다발</sub>* $\pi:E \rightarrow B$는 다음과 같이 정의된 대상이다.
...
을 basis로 하여 얻어진다. $W=\mathbb{R}^{2m}$으로 두면 위의 집합에 의해 만들어지는 위상에서 $\pi^{-1}(U)$들이 모두 열린집합임을 확인할 수 있고, 이 위상이 $2m$차원의 topological manifold를 만든다는 것도 확인할 수 있다. 
...
Tangent bundle $TM$이 중요한 것은 manifold 위에 정의된 대다수의 vector bundle이 $TM$으로부터 정의되기 때문이다. 가령 cotangent bundle $T^\ast M$은 각각의 $p\in M$마다 tangent space의 dual space인 cotangent space $T_p^\ast M$이 붙어있는 vector bundle이다. 이와 비슷하게, 각 점 $p$마다 선형대수에서의 연산들을 통해 다양한 vector bundle이 정의된다. ([예시 5](#ex5){: data-lid="s5l93" data-relation="forward" })
...
{% diagram Math/Manifolds/Tangent_and_Cotangent_Bundles-1.svg width="5.60em" alt="bundle_map" %}
...
만일 $f\in\Hom(V,V'), g\in\Hom(W,W')$가 isomorphism이라면 $F(f,g)\in\Hom(F(V,W),F(V',W'))$이다. 이들은 모두 벡터공간이므로 [§미분다양체의 예시들, ⁋예시 2](/ko/math/manifolds/examples_of_manifolds#ex2){: data-lid="48lcb" data-relation="required" }와 같은 미분구조가 주어져 있고, 이를 통해 위의 정의를 적용할 수 있다. 또, 어렵지 않게 이 정의를 일반적인 $k$-fold product 
...
::: 예시 5
$\Hom(-,-)$은 smooth functor이다. 임의의 두 isomorphism $f:V\rightarrow V'$, $g:W\rightarrow W'$이 주어져 있다 하자. 그럼 $\Hom(f,g)$는 $\Hom(V,W)$에서 $\Hom(V',W')$로의 linear map이며 아래 diagram

{% diagram Math/Manifolds/Tangent_and_Cotangent_Bundles-2.svg width="7.11em" alt="Hom_functor" %}
...
- Dual functor $(-)^\ast$ ([\[선형대수학\] §쌍대공간](/ko/math/linear_algebra/dual_space){: data-lid="cqup0" data-relation="weak" }),
- $k$-th tensor functor $\mathcal{T}^k(-)$ ([\[다중선형대수학\] §텐서대수](/ko/math/multilinear_algebra/tensor_algebras){: data-lid="y0f09" data-relation="weak" }),
- $k$-th symmetric functor $\mathcal{S}^k(-)$ ([\[다중선형대수학\] §텐서대수](/ko/math/multilinear_algebra/tensor_algebras){: data-lid="pba1v" data-relation="weak" }),
- $k$-th exterior functor $\bigwedge\nolimits^k(-)$ ([\[다중선형대수학\] §텐서대수](/ko/math/multilinear_algebra/tensor_algebras){: data-lid="rdkzz" data-relation="weak" }),
...
임의의 manifold $M$과 tangent bundle $E=TM\rightarrow M$, 그리고 dual functor $(-)^\ast$에 위의 [정리 6](#thm6){: data-lid="7iizs" data-relation="required" }을 적용하면 다음을 얻는다.

::: 정의 7
Manifold $M$ 위에 정의된 *cotangent bundle<sub>여접다발</sub>*은 [정리 6](#thm6){: data-lid="rs087" data-relation="required" }에 의해 얻어진 vector bundle $(TM)^\ast$을 의미한다. Cotangent space $T_p^\ast M$의 표기에 맞추어 이를 $T^\ast M$으로 표기한다.
:::
