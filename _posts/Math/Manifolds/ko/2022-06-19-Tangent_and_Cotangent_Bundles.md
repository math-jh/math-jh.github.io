...
Tangent bundle $TM$이 중요한 것은 manifold 위에 정의된 대다수의 vector bundle이 $TM$으로부터 정의되기 때문이다. 가령 cotangent bundle $T^\ast M$은 각각의 $p\in M$마다 tangent space의 dual space인 cotangent space $T_p^\ast M$이 붙어있는 vector bundle이다. 이와 비슷하게, 각 점 $p$마다 선형대수에서의 연산들을 통해 다양한 vector bundle이 정의된다. ([예시 5](#ex5){: data-lid="s5l93" })
...
만일 $f\in\Hom(V,V'), g\in\Hom(W,W')$가 isomorphism이라면 $F(f,g)\in\Hom(F(V,W),F(V',W'))$이다. 이들은 모두 벡터공간이므로 [§미분다양체의 예시들, ⁋예시 2](/ko/math/manifolds/examples_of_manifolds#ex2){: data-lid="48lcb" }와 같은 미분구조가 주어져 있고, 이를 통해 위의 정의를 적용할 수 있다. 또, 어렵지 않게 이 정의를 일반적인 $k$-fold product 
...
- Dual functor $(-)^\ast$ ([\[선형대수학\] §쌍대공간](/ko/math/linear_algebra/dual_space){: data-lid="cqup0" }),
- $k$-th tensor functor $\mathcal{T}^k(-)$ ([\[다중선형대수학\] §텐서대수](/ko/math/multilinear_algebra/tensor_algebras){: data-lid="y0f09" }),
- $k$-th symmetric functor $\mathcal{S}^k(-)$ ([\[다중선형대수학\] §텐서대수](/ko/math/multilinear_algebra/tensor_algebras){: data-lid="pba1v" }),
- $k$-th exterior functor $\bigwedge\nolimits^k(-)$ ([\[다중선형대수학\] §텐서대수](/ko/math/multilinear_algebra/tensor_algebras){: data-lid="rdkzz" }),
...
임의의 manifold $M$과 tangent bundle $E=TM\rightarrow M$, 그리고 dual functor $(-)^\ast$에 위의 [정리 6](#thm6){: data-lid="7iizs" }을 적용하면 다음을 얻는다.

::: 정의 7
Manifold $M$ 위에 정의된 *cotangent bundle<sub>여접다발</sub>*은 [정리 6](#thm6){: data-lid="rs087" }에 의해 얻어진 vector bundle $(TM)^\ast$을 의미한다. Cotangent space $T_p^\ast M$의 표기에 맞추어 이를 $T^\ast M$으로 표기한다.
:::
