...
[§접공간, ⁋명제 2](/ko/math/manifolds/tangent_space#prop2){: data-lid="kygrz" data-relation="required" }에 의하여, $\mathcal{C}^\infty_p$의 ideal들의 descending chain
...
으로 정의하는 것이 잘 정의된다. 이렇게 정의된 $v_L$이 linear map일 뿐만 아니라 [§접공간, ⁋정의 3](/ko/math/manifolds/tangent_space#def3){: data-lid="j4hgu" data-relation="required" }도 만족한다는 것을 보여야 하므로, 
...
$(U,\varphi)$가 $p$을 중심으로 하는 coordinate system이고, $\varphi=(x^i)_{i=1}^m$라 하자. 다루는 대상이 전부 germ이므로 $U$를 줄여도 무방하고, 따라서 $\varphi(U)$가 원점을 중심으로 하는 열린 공이라 가정한다. 또, $\mathbf{f}\in\mathfrak{m}_p$가 임의로 주어졌다 하자. 
