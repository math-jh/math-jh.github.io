...
::: Example 1
The *standard differentiable structure* on $\mathbb{R}^m$ with the standard topology is given by the atlas $\{(\mathbb{R}^m,\id_{\mathbb{R}^m})\}$. 
:::
...
::: Definition 3
Let $M$ be a manifold. Give an open subset $V$ of $M$ the following two structures:
...
::: Example 6
Consider the $n$-dimensional sphere $S^n$ in $\mathbb{R}^{n+1}$. Endowing $S^n$ with the subspace topology, the Hausdorff and second countability conditions are both satisfied. However, since $S^n$ is not an open set, verifying the locally Euclidean condition is more involved than in the case of an open submanifold. 
...
Now, just as in [Example 6](#ex6){: data-lid="6r0bl" data-relation="required" }, if we give $M$ the subspace topology and construct charts using the $(U, f)$ just obtained, we can verify that $M$ becomes an $(n-1)$-dimensional manifold.
:::

Using [Example 7](#ex7){: data-lid="gkahj" data-relation="forward" }, we can regard $S^n$ as the zero set of the function

$$F(x^1,\ldots, x^{n+1})=(x^1)^2+(x^2)^2+\cdots+(x^{n+1})^2-1$$

defined on $\mathbb{R}^{n+1}$; hence [Example 6](#ex6){: data-lid="l4dsp" data-relation="weak" } can be seen as a special case of the above example.
...
of $\mathbb{R}^{n+1}\setminus\{0\}$. Then $\tilde{U}_i$ is a saturated open set, so by [\[Topology\] §Quotient Spaces, ⁋Proposition 7](/en/math/topology/quotient_spaces#prop7){: data-lid="h9ulk" data-relation="required" } the quotient map $\pi$ restricts well to $\tilde{U}_i$. Therefore, if we define the function $\varphi_i:U_i\rightarrow\mathbb{R}^n$ by

$$\varphi_i[x^1,\ldots, x^{n+1}]=\left(\frac{x^1}{x^i},\ldots,\frac{x^{i-1}}{x^i},\frac{x^{i+1}}{x^i},\ldots, \frac{x^{n+1}}{x^i}\right)$$

[^1] the function $\varphi_i\circ\pi\vert_{\tilde{U}_i}$ is continuous, and hence by [\[Topology\] §Quotient Spaces, ⁋Proposition 4](/en/math/topology/quotient_spaces#prop4){: data-lid="3wmw5" data-relation="required" } $\varphi_i$ is also continuous. 
