...
([§Examples of Differentials, ⁋Definition 1](/en/math/manifolds/examples_of_differentials#def1){: data-lid="4gvw6" }) 
...
As we can see by applying the fact that $X$ is $C^\infty$ (from [§Vector Fields, ⁋Proposition 2](/en/math/manifolds/vector_fields#prop2){: data-lid="eu3wt" }) to each function $f$, whether $X$ and $Y$ are $F$-related can also be tested by applying them to each function.
