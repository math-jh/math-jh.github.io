...
Using [§Tangent and Cotangent Bundles, ⁋Example 5](/en/math/manifolds/tangent_and_cotangent_bundles#ex5){: data-lid="mqutq" data-relation="required" } and [§Tangent and Cotangent Bundles, ⁋Theorem 6](/en/math/manifolds/tangent_and_cotangent_bundles#thm6){: data-lid="1qusz" data-relation="required" }, we can make the following definitions.
...
Then $(-,-)$ is a non-degenerate pairing, so $\mathcal{T}^{r,s}(T_p^\ast M)\cong\mathcal{T}^{r,s}(T_pM)^\ast$. ([\[Linear Algebra\] §Dual Space, ⁋Corollary 5](/en/math/linear_algebra/dual_space#cor5){: data-lid="bg40l" data-relation="required" })
...
Among the objects introduced in [Definition 1](#def1){: data-lid="lmvxa" data-relation="required" }, the elements of $\Omega^\ast(M)$ are of particular interest. By definition, any differential form $\omega\in\Omega^\ast(M)$ is a function $M\rightarrow\bigwedge\nolimits^\ast(M)$, and we write its values as
