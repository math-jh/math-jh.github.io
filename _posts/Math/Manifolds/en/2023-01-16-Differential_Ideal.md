...
::: Definition 5
For a manifold $M$ and a submanifold $\Phi:N\rightarrow M$, we say that $N$ is an *integral manifold* of an ideal $\mathcal{I}$ if $(d\Phi)^\ast(\omega)\equiv 0$ for every $\omega\in\mathcal{I}$. 
:::
...
2. Let $\mathcal{I}$ be the ideal generated by the given set of forms. Then by Frobenius's theorem, $\mathcal{I}$ has an integral manifold $I$ of dimension $(m+n)-m=n$. It then suffices to show that the restriction of $d\pi_1$ to $I_q$ is a bijection for an arbitrary point $q\in I$. 
:::
