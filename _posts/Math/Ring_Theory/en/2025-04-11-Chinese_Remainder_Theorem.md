...
date: 2025-04-11
weight: 3
translated_at: 2026-08-19T20:15:04+00:00
translation_source: kimi-cli
last_polished_at: 2026-08-19T20:15:04+00:00
---
...
::: Definition 3
Two two-sided ideals $\mathfrak{a},\mathfrak{b}$ of a ring $A$ are called *comaximal* if $\mathfrak{a}+\mathfrak{b}=A$. Ideals $\mathfrak{a}_1,\ldots,\mathfrak{a}_n$ are called *pairwise comaximal* if $\mathfrak{a}_i+\mathfrak{a}_j=A$ for all $i\ne j$.
:::
...
Now let $n>2$. It is obvious that the ideal $\mathfrak{a}=\mathfrak{b}_n$ and $\mathfrak{b}_1,\ldots,\mathfrak{b}_{n-1}$ are pairwise comaximal, so applying [Proposition 4](#prop4){: data-lid="zto05" } gives
...
::: Proposition 6
Let a ring $A$ and pairwise comaximal two-sided ideals $\mathfrak{a}_1,\ldots, \mathfrak{a}_n$ of $A$ be given. Then the map $\pi:A \rightarrow \prod_1^n A/\mathfrak{a}_i$ defined above is surjective, and its kernel equals $\bigcap \mathfrak{a}_i$.
:::
...
exists. If $A$ is commutative, then by [Proposition 5](#prop5){: data-lid="kaf7q" } we can replace the intersection by a product, so
...
The isomorphism $A\cong\prod A/\mathfrak{a}_i$ obtained when $\bigcap\mathfrak{a}_i=0$ is the strong statement that the ring $A$ decomposes as a product of smaller rings. When $A$ is commutative, this condition is equivalent by [Proposition 5](#prop5){: data-lid="ve116" } to $\mathfrak{a}_1\cdots\mathfrak{a}_n=0$.

## The Noncommutative Case

In [Proposition 5](#prop5){: data-lid="zuww4" }, the commutativity assumption was used to guarantee that the intersection $\mathfrak{b}_1\cap\cdots\cap\mathfrak{b}_n$ collapses to a single product $\mathfrak{b}_1\cdots\mathfrak{b}_n$. Without this assumption, products in different orders can yield different ideals, and the intersection is expressed as the symmetric sum of all such ordered products. The following proposition gives the generalized version.
...
holds. In particular, if $A$ is a commutative ring then all products in different orders coincide, so we recover [Proposition 5](#prop5){: data-lid="zejo3" }.
:::

::: Proof
We prove this by induction, just as in [Proposition 5](#prop5){: data-lid="yy9zf" }. Since $\sum_{\sigma\in S_n}\mathfrak{b}_{\sigma(1)}\cdots\mathfrak{b}_{\sigma(n)}\subseteq\mathfrak{b}_1\cap\cdots\cap\mathfrak{b}_n$ always holds, we only need to show the reverse inclusion.
...
Applying [Proposition 7](#prop7){: data-lid="i4nkn" } to the kernel $\bigcap_i\mathfrak{a}_i$ in [Proposition 6](#prop6){: data-lid="wouhj" }, the Chinese remainder theorem in the noncommutative case also takes the form

$$A\Big/\left(\sum_{\sigma\in S_n}\mathfrak{a}_{\sigma(1)}\cdots\mathfrak{a}_{\sigma(n)}\right)\cong \prod_{i=1}^n A/\mathfrak{a}_i.$$

This contains essentially the same information as [Proposition 6](#prop6){: data-lid="5dwhh" }; the only difference is that in the commutative case this kernel collapses to the single product $\mathfrak{a}_1\cdots\mathfrak{a}_n$, simplifying the statement.
