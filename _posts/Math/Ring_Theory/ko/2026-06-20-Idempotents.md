...
::: 예시 8
$A=\mathbb{Z}$를 보고, $n\geq 2$의 소인수분해를 $n=p_1^{a_1}\cdots p_r^{a_r}$ (서로 다른 소수 $p_k$)이라 하자. $\mathfrak{a}_k=p_k^{a_k}\mathbb{Z}$로 두면, 서로 다른 $k,l$에 대해 $p_k^{a_k}$와 $p_l^{a_l}$이 서로소이므로 적당한 정수 $u,v$가 존재하여 $up_k^{a_k}+vp_l^{a_l}=1$이고, 따라서 $\mathfrak{a}_k+\mathfrak{a}_l=\mathbb{Z}$로 쌍별 comaximal하다. 또 $\bigcap_k\mathfrak{a}_k=\lcm(p_1^{a_1},\ldots, p_r^{a_r})\mathbb{Z}=n\mathbb{Z}$이다. [정리 7](#thm7)을 몫환 $\mathbb{Z}/n\mathbb{Z}$에 적용하면 환동형사상
