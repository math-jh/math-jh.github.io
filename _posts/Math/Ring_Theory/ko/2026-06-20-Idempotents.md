...
앞서 [§중국인의 나머지정리, ⁋명제 6](/ko/math/ring_theory/chinese_remainder_theorem#prop6){: data-lid="sdplk" }에서 우리는 pairwise comaximal이고 교집합이 $0$인 two-sided ideal들이 주어진 ring을 quotient ring들의 곱으로 분해한다는 것을 보았다. 그 분해에서 각 인자를 골라내는 역할을 한 것은 $i$번째 성분만 $1$이고 나머지가 $0$인 원소였으며, 이러한 원소는 ring의 center에 놓인 *idempotent*로 특징지어진다. 
...
가장 단순한 complete set은 $n=1$일 때의 $\{1\}$이며, [예시 2](#ex2){: data-lid="0dm9h" }에서 $\mathbb{Z}/6\mathbb{Z}$의 $\{0,1\}$과 $\{3,4\}$는 $n=2$인 central complete set의 예이다. 더 일반적으로, 하나의 idempotent $e$가 주어지면 $\{e,1-e\}$는 언제나 orthogonal idempotent의 complete set이 된다. 따라서 complete set은 idempotent를 여러 조각으로 분할하는 일반화로 볼 수 있다. 이제 이 분할이 ring을 어떻게 쪼개는지 살펴본다.
...
:::
::: 증명
우선 첫째 데이터가 주어졌다 하자. 위의 대응이 주는대로 $\mathfrak{a}_i:=Ae_i$로 두면 [명제 4](#prop4){: data-lid="7gqke" }에 의해 각 $\mathfrak{a}_i$는 two-sided ideal이고 $A=\mathfrak{a}_1\oplus\cdots\oplus\mathfrak{a}_n$은 left module로서의 direct sum이다. 따라서 남은 것은 이 direct sum이 ring의 direct product이기도 하다는 것이다. Morphism
...
에서 곱셈도 보존한다. 또 $\varphi(1)=(e_1,\ldots, e_n)$이고 각 $e_i$가 $Ae_i$의 항등원이므로 $\varphi$는 ring homomorphism이다. 이것이 isomorphism이 되는 이유는 [명제 4](#prop4){: data-lid="7am4x" }의 direct sum decomposition이 isomorphism이기 때문이다. 
...
를 만족하므로 orthogonal idempotent의 complete set이다. 따라서 [명제 4](#prop4){: data-lid="fwck4" }에 의해 left module 분해
...
안에서, $k$번째 block에만 항등행렬을 놓은 원소 $P_k$는 central이어서 [정리 5](#thm5){: data-lid="n6odi" }의 대응에 따라 이 direct product decomposition을 준다. 그러나 이 $P_k$가 위의 예시 ring $\Mat_n(A)$ 안에서는 central이 아닌 것을 이미 확인하였다. 

## 중국인의 나머지정리와의 연결

한편, 우리는 [정리 5](#thm5){: data-lid="ibt51" } 이전에 이미 주어진 ring을 direct product로 분해하는 방법을 살펴본 적이 있다. ([§중국인의 나머지정리, ⁋명제 6](/ko/math/ring_theory/chinese_remainder_theorem#prop6){: data-lid="fv0uf" }) 이 두 결과는 독립적인 것이 아니며, 우리는 글의 남은 부분에서 이들 둘의 관계를 살펴본다. 핵심은 pairwise comaximal 조건이 product ring의 자연스러운 idempotent들을 $A$ 안으로 끌어온다는 것으로, 다음 정리에서 $\pi:A\rightarrow\prod_{i=1}^n A/\mathfrak{a}_i$는 각 quotient로의 projection이 유도하는 morphism 
...
마지막으로 셋째 조건을 가정하고 첫째 조건을 보이자. [정리 5](#thm5){: data-lid="cy95o" }에 의해 central한 complete set $\{e_1,\ldots, e_n\}$은 ring isomorphism $x\mapsto (xe_1,\ldots, xe_n)$으로 주어지는 direct product decomposition $A\cong\prod_i Ae_i$를 준다. 한편 각 $i$에 대하여 morphism
...
::: 예시 8
$n\geq 2$의 소인수분해를 $n=p_1^{a_1}\cdots p_r^{a_r}$ (서로 다른 소수 $p_k$)이라 하고, $A=\mathbb{Z}/n\mathbb{Z}$의 ideal들 $\mathfrak{a}_k=p_k^{a_k}\mathbb{Z}/n\mathbb{Z}$를 생각하자. $\mathbb{Z}$ 안에서 $p_k^{a_k}\mathbb{Z}$들은 pairwise comaximal이고 그 교차가 $n\mathbb{Z}$이므로, $A$ 안에서 $\mathfrak{a}_k$들은 pairwise comaximal이고 $\bigcap_k\mathfrak{a}_k=0$이다. 따라서 [정리 7](#thm7){: data-lid="stdzj" }이 적용되어 고전적인 중국인의 나머지정리
