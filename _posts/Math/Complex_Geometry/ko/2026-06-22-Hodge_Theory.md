...
::: 정의 1
콤팩트 지향 리만다양체 $$(M, g)$$의 차원을 $$m$$이라 하자. *Hodge star operator<sub>Hodge 별작용소</sub>* $$\ast : \Lambda^k T^\ast M \to \Lambda^{m-k} T^\ast M$$를, 모든 $$k$$-형식 $$\alpha, \beta$$에 대하여
...
::: 정의 2
콤팩트 지향 리만다양체 $$M$$ 위의 두 매끄러운 $$k$$-형식 $$\alpha, \beta$$에 대하여 *$$L^2$$-inner product<sub>$$L^2$$-내적</sub>*를
...
::: 정의 3
콤팩트 지향 리만다양체 $$M$$ 위에서 외미분 $$d : \Omega^k(M) \to \Omega^{k+1}(M)$$의 *형식 수반작용소<sub>formal adjoint</sub>* $$d^\ast : \Omega^{k+1}(M) \to \Omega^k(M)$$를, 모든 $$\alpha \in \Omega^k$$, $$\beta \in \Omega^{k+1}$$에 대하여
...
::: 정의 4
콤팩트 지향 리만다양체 $$M$$ 위에서 *Hodge–de Rham Laplacian<sub>Hodge–de Rham Laplace 작용소</sub>* $$\Delta_d : \Omega^k(M) \to \Omega^k(M)$$를
...
::: 명제 5
콤팩트 지향 리만다양체 $$M$$ 위의 $$k$$-형식 $$\alpha$$에 대하여 다음이 동치이다.
...
::: 정리 6 (Hodge 정리)
$$M$$을 콤팩트 지향 리만다양체라 하자. 그러면 각 $$k$$에 대하여 조화 $$k$$-형식 공간 $$\mathcal{H}^k(M)$$은 유한차원이고, $$L^2$$-직교분해
...
::: 정의 8
콤팩트 Kähler 다양체 $$X$$에 대하여, $$(p,q)$$-차수의 *Hodge 부분공간<sub>Hodge subspace</sub>*을
...
::: 따름정리 10
$$X$$를 콤팩트 Kähler 다양체라 하자. 그러면 Betti 수 $$b_k(X) = \dim_{\mathbb{C}} H^k_{\mathrm{dR}}(X, \mathbb{C})$$에 대하여
...
복소사영공간 $$\mathbb{CP}^n$$은 Fubini–Study 계량에 대해 콤팩트 Kähler 다양체이다 ([§Kähler 다양체, ⁋예시 8](/ko/math/complex_geometry/kahler_manifolds#ex8)). 그 코호몰로지는
...
::: 정의 12
콤팩트 Kähler 다양체 $$X$$ ($$n = \dim_{\mathbb{C}} X$$)의 *Lefschetz operator<sub>Lefschetz 연산자</sub>* $$L : H^k(X, \mathbb{C}) \to H^{k+2}(X, \mathbb{C})$$를 Kähler 류와의 곱
