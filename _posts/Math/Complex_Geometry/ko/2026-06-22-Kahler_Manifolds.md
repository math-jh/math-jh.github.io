...
::: 정의 1
복소다양체 $$(X, J)$$ 위의 리만 계량 $$g$$가 *Hermitian metric<sub>Hermitian 계량</sub>*이라는 것은, 모든 점에서 모든 접벡터 $$X, Y$$에 대하여
...
::: 정의 2
Hermitian 다양체 $$(X, J, g)$$의 *fundamental form<sub>기본형식</sub>* $$\omega$$를 두 접벡터 $$X, Y$$에 대하여
...
::: 정의 3
Hermitian 다양체 $$(X, J, g)$$가 *Kähler manifold<sub>Kähler 다양체</sub>*라는 것은 그 기본형식 $$\omega$$가 닫힌형식인 것, 곧
...
::: 명제 10
Kähler 다양체 $$(X, J, g)$$의 Kähler 형식 $$\omega$$는 사교형식이다. 곧 $$(X, \omega)$$는 사교다양체이다. 더 나아가 $$\omega^n / n!$$은 $$X$$의 리만 부피형식과 일치하며, 따라서 $$\omega^n$$은 어디서도 소멸하지 않는다 ($$n = \dim_{\mathbb{C}} X$$).
:::
...
::: 명제 11
$$X$$가 콤팩트 연결 Kähler 다양체이고 $$n = \dim_{\mathbb{C}} X$$이면, 모든 $$1 \leq k \leq n$$에 대하여 코호몰로지류 $$[\omega^k] \in H^{2k}(X, \mathbb{R})$$는 $$0$$이 아니다. 특히 Betti 수에 대해 $$b_{2k}(X) \geq 1$$이고, 따라서 $$b_2(X) \geq 1$$이다.
:::
...
::: 정리 12 (Kähler 항등식)
$$(X, J, g)$$가 Kähler 다양체이면, $$\Omega^{p,q}(X)$$ 위에서 다음 교환관계가 성립한다.
