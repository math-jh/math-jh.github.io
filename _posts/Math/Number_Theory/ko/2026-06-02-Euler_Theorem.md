...
[§페르마 소정리](/ko/math/number_theory/fermat_little_theorem){: data-lid="095q1" data-relation="required" }는 법이 소수일 때 $a^{p-1} \equiv 1$임을 말했다. 그 증명의 핵심은 $0$이 아닌 coset이 $p-1$개라는 것과 그들이 곱셈에 대해 닫혀 있다는 점이었다. 법이 합성수이면 곱셈 역원을 갖는 coset만이 이런 성질을 가지므로, 그 개수를 세는 함수가 필요하다.
...
곱셈 역원을 갖는 coset이 정확히 $n$과 서로소인 것들이므로 ([§합동식, ⁋명제 7](/ko/math/number_theory/congruences#prop7){: data-lid="tf5km" data-relation="required" }), $\varphi(n)$은 $\mathbb{Z}/n\mathbb{Z}$에서 역원을 갖는 원소의 개수이다. 예컨대 $\varphi(1) = 1$, $\varphi(p) = p - 1$ ($p$ 소수), $\varphi(12) = 4$ ($1, 5, 7, 11$) 이다.
...
::: 증명
$r_1, r_2, \ldots, r_{\varphi(n)}$을 기약잉여계라 하자. $\gcd(a, n) = 1$이면 $a r_1, a r_2, \ldots, a r_{\varphi(n)}$ 역시 모두 $n$과 서로소이고, [§합동식, ⁋명제 6](/ko/math/number_theory/congruences#prop6){: data-lid="rhmq2" data-relation="required" }에 의해 법 $n$에 대해 서로 다르다. 따라서 이들은 기약잉여계 $\{r_1, \ldots, r_{\varphi(n)}\}$을 순서만 바꾸어 놓은 것이다. 양쪽을 모두 곱하면
...
이다. 곱셈성과 결합하면, $n = p_1^{e_1}\cdots p_r^{e_r}$의 소인수분해로부터 ([§소수와 산술의 기본정리, ⁋정리 3](/ko/math/number_theory/primes#thm3){: data-lid="zudv4" data-relation="required" })
...
오일러 정리는 역원을 명시적으로 적는 데에도 쓰인다. 페르마 소정리에서 소수 법의 역원이 $a^{p-2}$로 주어졌듯이 ([§페르마 소정리, ⁋명제 8](/ko/math/number_theory/fermat_little_theorem#prop8){: data-lid="kxb7q" data-relation="weak" }), 임의의 법에서는 지수를 $\varphi(n)$ 기준으로 한 칸 내린 거듭제곱이 역원이 된다.
...
::: 예시 8 (거듭제곱으로 역원 구하기)
법 $10$에 대한 $3$의 역원을 구하자. $\gcd(3, 10) = 1$이고 $\varphi(10) = 4$이므로 명제 7에 의해
...
::: 예시 10 (합 항등식의 확인)
$n = 12$에서 명제 9를 확인하자. $12$의 약수는 $1, 2, 3, 4, 6, 12$이고 각각의 $\varphi$ 값은
