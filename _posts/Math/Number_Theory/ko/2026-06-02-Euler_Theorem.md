...
[§페르마 소정리](/ko/math/number_theory/fermat_little_theorem){: data-lid="095q1" }는 법이 소수일 때 $a^{p-1} \equiv 1$임을 말했다. 그 증명의 핵심은 $0$이 아닌 coset이 $p-1$개라는 것과 그들이 곱셈에 대해 닫혀 있다는 점이었다. 법이 합성수이면 곱셈 역원을 갖는 coset만이 이런 성질을 가지므로, 그 개수를 세는 함수가 필요하다.
...
곱셈 역원을 갖는 coset이 정확히 $n$과 서로소인 것들이므로 ([§합동식, ⁋명제 7](/ko/math/number_theory/congruences#prop7){: data-lid="tf5km" }), $\varphi(n)$은 $\mathbb{Z}/n\mathbb{Z}$에서 역원을 갖는 원소의 개수이다. 예컨대 $\varphi(1) = 1$, $\varphi(p) = p - 1$ ($p$ 소수), $\varphi(12) = 4$ ($1, 5, 7, 11$) 이다.
...
오일러 정리는 역원을 명시적으로 적는 데에도 쓰인다. 페르마 소정리에서 소수 법의 역원이 $a^{p-2}$로 주어졌듯이 ([§페르마 소정리, ⁋명제 8](/ko/math/number_theory/fermat_little_theorem#prop8){: data-lid="kxb7q" }), 임의의 법에서는 지수를 $\varphi(n)$ 기준으로 한 칸 내린 거듭제곱이 역원이 된다.
