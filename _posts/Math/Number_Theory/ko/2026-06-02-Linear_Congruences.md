...
[§합동식](/ko/math/number_theory/congruences){: data-lid="0g6y4" }에서 합동의 연산과 곱셈 역원을 다루었다. 이제 가장 기본적인 합동방정식인 일차 합동식 $ax \equiv b \pmod{n}$의 해를 분석한다. 이는 일차 부정방정식 $ax + ny = b$를 푸는 것과 같은 문제이며, Bézout 항등식이 그 가해성을 완전히 결정한다.
...
가 성립한다는 것과 동치이다. 좌변 $ax - ny$가 $x, y$를 정수 전체에 걸쳐 움직일 때 취하는 값 전체는 $\gcd(a, n) = d$의 배수 전체와 정확히 일치하므로 ([§유클리드 호제법과 Bézout 항등식, ⁋명제 5](/ko/math/number_theory/euclidean_algorithm#prop5){: data-lid="kpw2v" }), 위 방정식이 풀릴 필요충분조건은 $d \mid b$이다.

이제 $d \mid b$라 하고 $a = da'$, $n = dn'$, $b = db'$로 쓰자. 이때 $\gcd(a', n') = 1$이며, $ax \equiv b \pmod{n}$의 양변과 법을 공통인수 $d$로 나누면

$$a'x \equiv b' \pmod{n'}$$

과 동치가 된다. (뒤의 [명제 7](#prop7){: data-lid="taksf" }) 합동식을 그 법과 서로소가 아닌 공약수로 약분할 때 법도 함께 나눠야 함에 주의한다. $\gcd(a', n') = 1$이므로 $a'$은 법 $n'$에 대해 곱셈 역원 $(a')^{-1}$을 가지고 ([§합동식, ⁋명제 7](/ko/math/number_theory/congruences#prop7){: data-lid="1gl09" }), 양변에 그것을 곱하면
...
이고 $63 = 7\cdot 9$이므로 $7x \equiv 63 \pmod{10}$이다. 양변에서 $\gcd(7, 10) = 1$이라 계수 $7$을 법을 바꾸지 않고 그대로 약분할 수 있어 ([명제 7](#prop7){: data-lid="8t9va" })
...
이고, $\gcd(a', n') = 1$이므로 $n' \mid x - x_0$이다 ([§합동식, ⁋명제 6](/ko/math/number_theory/congruences#prop6){: data-lid="m4ozz" }). 곧 $x = x_0 + n' k = x_0 + (n/d)k$ 꼴이며, 역으로 이런 모든 $x$가 $a(x - x_0) = a n' k = a' n k$를 만족해 해가 된다. 법 $n$ 아래에서 $k$와 $k + d$는 같은 coset을 주므로 ($(n/d)(k+d) = (n/d)k + n$), 서로 다른 해는 $k = 0, 1, \ldots, d-1$의 $d$개이다.
:::
...
이며, 마지막 동치는 $\gcd(c', n') = 1$에서 따른다. ([§합동식, ⁋명제 6](/ko/math/number_theory/congruences#prop6){: data-lid="b0se1" }) 곧 $a \equiv b \pmod{n/g}$이다. 특히 $\gcd(c, n) = 1$이면 $g = 1$, $n/g = n$이어서 법이 그대로 보존되는 통상의 cancellation law를 얻는다.
:::
...
따라서 $1^{-1} = 1$, $2^{-1} = 4$, $3^{-1} = 5$, $4^{-1} = 2$, $5^{-1} = 3$, $6^{-1} = 6$이다. 법이 소수일 때 $0$을 제외한 모든 원소가 역원을 가짐은 [§합동식, ⁋명제 7](/ko/math/number_theory/congruences#prop7){: data-lid="s8plh" }의 직접적 귀결이다.
:::
