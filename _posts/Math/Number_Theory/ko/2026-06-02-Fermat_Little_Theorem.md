...
명제 5의 등식 $(a+b)^p \equiv a^p + b^p \pmod p$는 흔히 "신입생의 꿈<sub>freshman's dream</sub>"이라 불리며, characteristic $p$를 가진 field에서 $p$제곱 morphism $x \mapsto x^p$가 덧셈과 곱셈을 모두 보존하는 준동형 — *프로베니우스 자기준동형<sub>Frobenius endomorphism</sub>* — 이 되는 현상의 산술적 그림자이다.
...
::: 예시 7 (지수가 더 클 때)
$2^{2^{10}}$을 $11$로 나눈 나머지를 구하자. $11$이 소수이므로 $2^{10} \equiv 1 \pmod{11}$이고, 따라서 지수 $2^{10} = 1024$를 $10$으로 나눈 나머지가 필요하다.
