...
[§합동식](/ko/math/number_theory/congruences){: data-lid="gk593" }에서 소수 $p$를 법으로 하면 $0$이 아닌 모든 coset이 곱셈 역원을 가져 $\mathbb{Z}/p\mathbb{Z}$가 field를 이룸을 보았다. 이 곱셈 구조에서 나오는 가장 기본적이고 강력한 사실이 페르마 소정리이며, 거듭제곱의 합동을 극적으로 단순화한다.
...
::: 증명
$0$이 아닌 coset들 $1, 2, \ldots, p-1$에 각각 $a$를 곱한 $a, 2a, \ldots, (p-1)a$를 생각한다. $p \nmid a$이고 $1 \leq i \leq p-1$이면 $p \nmid ia$이므로 이들은 모두 $0$이 아니고, $ia \equiv ja \pmod p$이면 [§합동식, ⁋명제 6](/ko/math/number_theory/congruences#prop6){: data-lid="o6nci" }에 의해 $i \equiv j$이므로 서로 다르다. 따라서 $a, 2a, \ldots, (p-1)a$는 법 $p$에 대해 $1, 2, \ldots, p-1$을 순서만 바꾸어 놓은 것이다. 양쪽을 모두 곱하면
...
페르마 소정리는 곱셈 역원을 명시적으로 주는 데에도 쓰인다. $p \nmid a$이면 $a \cdot a^{p-2} = a^{p-1} \equiv 1 \pmod p$이므로, $a$의 법 $p$에 대한 역원은 곧 $a^{p-2}$이다. 이는 [§일차 합동식, ⁋예시 4](/ko/math/number_theory/linear_congruences#ex4){: data-lid="bw054" }의 확장 유클리드 알고리즘과는 다른, 거듭제곱만으로 역원을 얻는 길을 열어 준다.
...
이고, 따라서 $a^{p-2}$가 $a$의 곱셈 역원이다. 역원은 유일하므로 ([§합동식, ⁋명제 8](/ko/math/number_theory/congruences#prop8){: data-lid="m1djd" }) 이것이 $a^{-1}$이다.
:::
