...
[§오일러 정리와 phi function](/ko/math/number_theory/euler_theorem){: data-lid="xs6lt" data-relation="weak" }에서 $\varphi$가 서로소인 인수에 대해 곱으로 분해됨을 보았다. 이런 성질을 가진 함수들은 정수론에서 거듭 나타나며, 그들 사이의 관계는 합성곱이라는 연산으로 깔끔하게 기술된다.
...
::: 예시 2
약수의 개수 $\tau(n) = \sum_{d \mid n} 1$과 약수의 합 $\sigma(n) = \sum_{d\mid n} d$은 multiplicative이다. 소수 거듭제곱에서 $\tau(p^e) = e + 1$, $\sigma(p^e) = 1 + p + \cdots + p^e = (p^{e+1} - 1)/(p - 1)$이므로, 일반적으로
...
::: 정의 3
두 산술적 함수 $f, g$의 *디리클레 합성곱<sub>Dirichlet convolution</sub>* $f \ast g$는
...
::: 명제 5
$\mu$는 multiplicative이며, $n > 1$이면 $\sum_{d \mid n} \mu(d) = 0$이다. 즉 $\mathbf{1} \ast \mu = \varepsilon$이다.
:::
...
::: 증명
왼쪽 조건은 $g = \mathbf{1}\ast f$를 뜻한다. 양변에 $\mu$를 합성곱하면 아래 [명제 8](#prop8){: data-lid="f9330" data-relation="required" }의 결합법칙과 명제 5에 의해 $\mu \ast g = \mu\ast\mathbf{1}\ast f = \varepsilon \ast f = f$이고, 이것이 오른쪽 식이다. 역방향도 같은 계산을 거꾸로 하면 된다.
:::

::: 예시 7
약수에 걸친 $\varphi$의 합은 $\sum_{d \mid n}\varphi(d) = n$이다. ([§오일러 정리와 phi function, ⁋명제 9](/ko/math/number_theory/euler_theorem#prop9){: data-lid="2lu0x" data-relation="weak" }) 실제로 $1$부터 $n$까지의 각 정수 $m$을 $\gcd(m, n) = n/d$에 따라 분류하면, $\gcd(m,n) = n/d$인 $m$의 개수가 $\varphi(d)$이므로 전체 합이 $n$이 된다. 이 관계 $\mathbf{1}\ast\varphi = \id$에 뫼비우스 반전을 적용하면 $\varphi = \mu \ast \id$, 즉 $\varphi(n) = \sum_{d\mid n}\mu(d)(n/d)$이라는 $\varphi$의 또 다른 공식을 얻는다.
:::

마지막 식을 곱셈성과 결합하면 $\varphi$의 곱공식이 다시 나온다. $\varphi = \mu \ast \id$가 multiplicative function 두 개의 합성곱이므로 (아래 [명제 9](#prop9){: data-lid="63slq" data-relation="required" }) multiplicative이고, 소수 거듭제곱에서
...
이라는 익숙한 형태를 얻는다. 합성곱과 반전이라는 대수적 도구만으로 [§오일러 정리와 phi function](/ko/math/number_theory/euler_theorem){: data-lid="61x2r" data-relation="weak" }에서 직접 센 결과가 재생산되는 것이다.
...
::: 명제 9
산술적 함수 $f, g$가 모두 multiplicative이면 $f \ast g$도 multiplicative이다.
:::
...
이다. 또 $f, g$가 항등적으로 $0$이 아니므로 $(f\ast g)(1) = f(1)g(1) = 1 \neq 0$이어서 $f \ast g$는 항등적 $0$이 아니다. 따라서 multiplicative이다.
:::

명제 9는 multiplicative 함수가 합성곱 아래 닫혀 있음을 말한다. $\mathbf{1}$과 $\id$가 multiplicative이므로 $\tau = \mathbf{1}\ast\mathbf{1}$과 $\sigma = \mathbf{1}\ast\id$이 multiplicative임이 예시 2와 별개로 곧장 따라 나오고, $\mathbf{1}$의 역원 $\mu$ 역시 multiplicative임을 같은 틀에서 재확인할 수 있다. 더 일반적으로 $\id_k(n) = n^k$로 두면 $\sigma_k = \mathbf{1}\ast\id_k$, 즉 $\sigma_k(n) = \sum_{d \mid n} d^k$이 multiplicative이며, $k = 0$이면 $\tau$, $k = 1$이면 $\sigma$가 된다.
...
::: 예시 10
$n = 360 = 2^3 \cdot 3^2 \cdot 5$에 대해 곱셈성을 이용하면
...
이다. $360$이 어떤 소수의 제곱으로 나누어떨어지므로 $\mu(360) = 0$이고, 약수에 걸친 합 $\sum_{d \mid 360}\varphi(d) = 360$ ([예시 7](#ex7){: data-lid="vq71k" data-relation="weak" }) 역시 곱셈성으로 인수마다 $\sum_{j}\varphi(p^j) = p^e$임을 확인하면 즉시 따라온다.
:::

다음으로 뫼비우스 반전을 적용해 약수합으로 정의된 함수를 풀어내는 전형적인 계산을 본다.

::: 예시 11
$g(n) = \sum_{d \mid n} \tau(d)$로 정의된 함수를 $\tau$로 되돌려 보자. 정의에 따라 $g = \mathbf{1}\ast\tau = \mathbf{1}\ast\mathbf{1}\ast\mathbf{1}$이므로 $g$는 multiplicative이고 소수 거듭제곱에서
...
::: 예시 12
*메르텐스 함수<sub>Mertens function</sub>* $M(n) = \sum_{k=1}^{n}\mu(k)$의 처음 값들을 명제 5의 곱셈성과 정의 4로 계산하면
...
Multiplicative 함수와 디리클레 합성곱은 소수의 분포를 함수의 언어로 다루는 해석적 정수론의 대수적 골격을 이룬다. 이들을 생성함수 $\sum_n f(n) n^{-s}$로 부호화한 디리클레 급수는 Riemann zeta function과 만나며, 그 해석적 성질은 [§소수의 무한성과 분포, ⁋참고 3](/ko/math/number_theory/distribution_of_primes#rmk3){: data-lid="qra3x" data-relation="forward" }에서 언급한 소수 정리로 이어진다.
