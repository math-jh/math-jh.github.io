...
::: 정의 1
*가우스 정수<sub>Gaussian integer</sub>*는 $a + bi$ ($a, b \in \mathbb{Z}$, $i^2 = -1$) 꼴의 복소수이며, 그 전체를 $\mathbb{Z}[i]$로 적는다. $\alpha = a + bi$의 *노름<sub>norm</sub>*을 $N(\alpha) = a^2 + b^2$으로 정의한다.
:::

::: 명제 2
노름은 곱셈적이다: $N(\alpha\beta) = N(\alpha)N(\beta)$. 따라서 $\alpha$가 $\mathbb{Z}[i]$의 가역원(단원)인 것은 $N(\alpha) = 1$인 것과 동치이며, 단원은 $\pm 1, \pm i$의 넷뿐이다.
:::
