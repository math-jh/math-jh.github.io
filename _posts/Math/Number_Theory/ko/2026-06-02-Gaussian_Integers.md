...
description: "정수에 √-1을 더한 가우스 정수환을 도입하고, 노름을 이용해 그것이 유클리드 정역, 따라서 unique factorization domain임을 보인다. 이를 이차 잉여 이론과 결합하여 어떤 소수가 두 제곱수의 합인지를 결정한다."
...
::: 정의 1
*가우스 정수<sub>Gaussian integer</sub>*는 $a + bi$ ($a, b \in \mathbb{Z}$, $i^2 = -1$) 꼴의 복소수이며, 그 전체를 $\mathbb{Z}[i]$로 적는다. $\alpha = a + bi$의 *norm<sub>노름</sub>*을 $N(\alpha) = a^2 + b^2$으로 정의한다.
:::

::: 명제 2
Norm은 multiplicative이다: $N(\alpha\beta) = N(\alpha)N(\beta)$. 따라서 $\alpha$가 $\mathbb{Z}[i]$의 가역원(단원)인 것은 $N(\alpha) = 1$인 것과 동치이며, 단원은 $\pm 1, \pm i$의 넷뿐이다.
:::

::: 증명
$N(\alpha) = \alpha\overline{\alpha}$ ($\overline{\alpha}$는 켤레) 이므로 $N(\alpha\beta) = \alpha\beta\overline{\alpha\beta} = \alpha\overline{\alpha}\beta\overline{\beta} = N(\alpha)N(\beta)$이다. $\alpha\beta = 1$이면 $N(\alpha)N(\beta) = 1$이고 norm은 양의 정수이므로 $N(\alpha) = 1$, 즉 $a^2 + b^2 = 1$이어서 $\alpha \in \{\pm 1, \pm i\}$이다. 역으로 이 넷은 모두 가역이다.
:::

## 유클리드 나눗셈과 unique factorization

$\mathbb{Z}$에서와 마찬가지로 norm을 "크기"로 삼아 나눗셈 정리가 성립한다.
...
이다. 유클리드 나눗셈이 성립하면 $\mathbb{Z}$에서와 똑같은 논증으로 ([§유클리드 호제법과 Bézout 항등식](/ko/math/number_theory/euclidean_algorithm){: data-lid="nz39g" data-relation="required" }, [§소수와 산술의 기본정리](/ko/math/number_theory/primes){: data-lid="dkhkt" data-relation="required" }) 최대공약수·Bézout·unique factorization이 모두 따라온다.
:::
...
::: 증명
$p \equiv 3 \pmod 4$이면, $x^2 + y^2 \equiv 0 \pmod p$에서 $p \nmid x$라면 $(xy^{-1})^2 \equiv -1 \pmod p$가 되어 $-1$이 이차 잉여여야 하는데, $\left(\frac{-1}{p}\right) = (-1)^{(p-1)/2} = -1$이므로 불가능하다. ([§이차 잉여, ⁋따름정리 5](/ko/math/number_theory/quadratic_residues#cor5){: data-lid="7y8x3" data-relation="required" }) 따라서 $p \mid x, p\mid y$이고 $x^2 + y^2$은 $p^2$의 배수가 되어 $p$와 같을 수 없다.
...
::: 증명
$\pi \in \mathbb{Z}[i]$가 비단원 소수라 하자. $N(\pi) = \pi\overline{\pi}$는 $1$보다 큰 정수이므로 유리소수들의 곱 $N(\pi) = p_1 \cdots p_r$로 쓰인다. 그러면

$$\pi \mid \pi\overline{\pi} = N(\pi) = p_1 \cdots p_r$$
...
::: 예시 6 (가우스 정수의 소인수분해)
$\alpha = 3 + i$를 인수분해하자. Norm은

$$N(3 + i) = 3^2 + 1^2 = 10 = 2 \cdot 5$$

이다. $2$는 $1 + i$에서, $5 = (2+i)(2-i)$는 $2 \pm i$에서 온다. 따라서 $\alpha$는 norm $2$인 소수 하나와 norm $5$인 소수 하나의 곱이어야 한다. $(3+i)/(1+i) = (3+i)(1-i)/2 = (4 - 2i)/2 = 2 - i$이므로

$$3 + i = (1 + i)(2 - i)$$

이고, 우변의 두 인수는 각각 norm $2, 5$인 가우스 소수이다.
:::

::: 예시 7 (유클리드 나눗셈)
정리 3의 알고리즘을 $\alpha = 11 + 7i$, $\beta = 3 + 2i$에 실행하자. 먼저

$$\frac{\alpha}{\beta} = \frac{(11 + 7i)(3 - 2i)}{N(\beta)} = \frac{(33 + 14) + (21 - 22)i}{13} = \frac{47 - i}{13} \approx 3.62 - 0.08i$$

이다. 가장 가까운 가우스 정수는 $\kappa = 4$ (실수부 $3.62 \rightarrow 4$, 허수부 $-0.08 \rightarrow 0$) 이며, 나머지는
...
이러한 나눗셈을 반복하면 두 가우스 정수의 최대공약수를 호제법으로 구할 수 있다 ([§유클리드 호제법과 Bézout 항등식](/ko/math/number_theory/euclidean_algorithm){: data-lid="r6g3j" data-relation="required" }의 논증이 그대로 옮겨진다). Norm이 매 단계 진감소하는 양의 정수이므로 알고리즘은 유한 번에 끝난다.
...
::: 증명
$n = x^2 + y^2$인 것은 $n = N(\alpha)$인 $\alpha = x + yi \in \mathbb{Z}[i]$가 존재하는 것과 같다. $\alpha$를 가우스 소수들로 인수분해하고 norm을 취하면, $n$은 가우스 소수들의 norm의 곱이다. 정리 5에 의해 그 norm들은 $2$, $p \equiv 1$인 $p$, 또는 $p \equiv 3$인 $p^2$ 중 하나이다. 따라서

$$\begin{aligned}
n = N(\alpha) &= 2^{a}\prod_{p \equiv 1} p^{b_p} \prod_{p \equiv 3} (p^2)^{c_p}
\end{aligned}$$

꼴이 되어, $p \equiv 3$인 소수는 모두 짝수 지수로만 나타난다. 역으로 $n = 2^a \prod_{p\equiv 1} p^{b_p}\prod_{p\equiv 3} p^{2c_p}$이면, $2 = N(1+i)$, $p = N(\pi_p)$ ($p \equiv 1$), $p^2 = N(p)$ ($p \equiv 3$) 이므로 각 인수가 어떤 가우스 정수의 norm이고, norm이 multiplicative이므로 그 곱 $n$도 norm, 곧 두 제곱수의 합이다.
:::
