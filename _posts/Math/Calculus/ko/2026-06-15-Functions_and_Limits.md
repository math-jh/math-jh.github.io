...
::: 예시 3
정의를 직접 적용해 극한을 증명할 때, 일차함수처럼 $\lvert f(x) - L\rvert$이 $\lvert x - a\rvert$의 상수배이면 $\delta$를 곧바로 읽어낼 수 있지만, 변화율이 일정하지 않은 함수에서는 한 가지 손질이 필요하다. $g(x) = x^2$을 생각하고 $x \rightarrow 2$일 때 극한값이 $4$임을 보이자. 우선
...
::: 명제 4 (극한의 유일성)
$\lim_{x\rightarrow a} f(x) = L$이고 $\lim_{x\rightarrow a} f(x) = L'$이면 $L = L'$이다.
:::

::: 증명
결론에 반하여 $L \neq L'$이라 가정하자. 그럼 $\epsilon = \frac{1}{2}\lvert L - L'\rvert > 0$이다. 이제 [정의 2](#def2){: data-lid="api8o" data-relation="required" reviewed="" }에 의해 각각에 대응하는 $\delta_1, \delta_2 > 0$이 존재하여 다음 두 조건
...
한편, [정의 2](#def2){: data-lid="u8ea3" data-relation="required" reviewed="" }는 원칙적으로 함수의 극한값의 후보 $L$이 주어졌을 때 그 극한값이 실제로 $L$이 맞다는 것을 보일 때만 사용할 수 있다. 즉, 이는 함수의 극한값이 <em-ko>무엇인지</em-ko> 알려주는 도구는 아니다. 이를 위해서는 다음 명제가 유용하다.

::: 명제 5 (극한법칙)
$\lim_{x\rightarrow a} f(x) = L$, $\lim_{x\rightarrow a} g(x) = M$이라 하자. 그러면

1. $\lim_{x\rightarrow a} \bigl(f(x) + g(x)\bigr) = L + M$,
2. 임의의 상수 $c$에 대해 $\lim_{x\rightarrow a} cf(x) = cL$,
3. $\lim_{x\rightarrow a} f(x)g(x) = LM$,
4. $M \neq 0$이면 $\lim_{x\rightarrow a} f(x)/g(x) = L/M$
...
3. 다음의 부등식
    ...
    트릭은 $\epsilon=1$로 두고 [정의 2](#def2){: data-lid="gyq6x" data-relation="required" reviewed="" }를 $f$와 $g$ 각각에 적용하는 것이다. 그럼 적당한 $\delta_1, \delta_2$가 존재하여
...
::: 따름정리 6 (거듭제곱과 거듭제곱근의 극한)
$\lim_{x\rightarrow a} f(x) = L$이면

1. 임의의 양의 정수 $k$에 대해 $\lim_{x\rightarrow a} \bigl(f(x)\bigr)^k = L^k$,
2. $L > 0$이면 임의의 양의 정수 $k$에 대해 $\lim_{x\rightarrow a} \sqrt[k]{f(x)} = \sqrt[k]{L}$

이 성립한다.
:::

::: 증명
1. [명제 5](#prop5){: data-lid="l1jgq" data-relation="required" reviewed="" }의 3을 적용하여 $k$에 대한 귀납법을 돌리면 된다.
...
에 대하여, [명제 5](#prop5){: data-lid="bkyke" data-relation="required" reviewed="" }의 합·상수배 법칙으로 극한을 각 항으로 분리하고 거듭제곱에 [따름정리 6](#cor6){: data-lid="kc5tq" data-relation="required" reviewed="" }을 적용하면

$$\lim_{x\rightarrow a}f(x)=c_n\Bigl(\lim_{x\rightarrow a}x\Bigr)^n+\cdots +c_1\lim_{x\rightarrow a}x+\lim_{x\rightarrow a}c_0$$

이 되어, 마지막으로 [예시 7](#ex7){: data-lid="oefr5" data-relation="required" reviewed="" }을 대입하면 $\lim_{x\rightarrow a}f(x)=f(a)$를 얻는다. 비슷한 방식으로, 다항함수의 비로 이루어진 유리함수의 극한도 분모의 극한이 $0$이 아니라면 분자와 분모의 극한의 비로 얻어진다.
...
::: 명제 8 (조임정리)
실수 $a$의 삭제된 근방에서 $g(x) \leq f(x) \leq h(x)$이고 $\lim_{x\rightarrow a} g(x) = \lim_{x\rightarrow a} h(x) = L$이면 $\lim_{x\rightarrow a} f(x) = L$이다.
:::
...
::: 명제 9 (극한의 대소 보존)
$a$의 근방에서 ($a$ 제외) $f(x) \leq g(x)$이고 두 극한 $L = \lim_{x\rightarrow a}f(x)$, $M = \lim_{x\rightarrow a}g(x)$이 존재하면 $L \leq M$이다.
:::
...
여기서 강한 부등식 $f < g$는 강한 부등식 $L < M$을 주지 않음에 유의한다. 예컨대 $f(x) = 0 < x^2 = g(x)$ ($x \neq 0$) 이지만 두 극한은 $x \rightarrow 0$에서 모두 $0$으로 같다. 즉 부등식은 극한에서 약해질 수 있다.

조임정리의 가장 유명한 응용은 다음의 삼각함수 극한으로, 미분에서 삼각함수를 다룰 때 결정적으로 쓰인다.

::: 예시 10
$\lim_{x\rightarrow 0}(\sin x)/x = 1$이다. $0 < x < \pi/2$에서 단위원의 넓이를 비교하면 부등식
...
이며, [명제 8](#prop8){: data-lid="cglpa" data-relation="required" reviewed="" }를 적용하면 $\cos x \rightarrow 1$임을 안다. 이제 이를 이용하여 앞선 부등식에 다시 [명제 8](#prop8){: data-lid="629s1" data-relation="required" reviewed="" }를 적용하면 $(\sin x)/x$의 극한값이 $1$임을 안다. 
:::
...
::: 정의 12
실수 $a$와 함수 $f$에 대해, 적당한 $c > 0$에 대하여 $f$가 $(a, a+c)$에서 정의되어 있다고 하자. 실수 $L$이 $x \rightarrow a^+$일 때 $f$의 *오른쪽 극한<sub>right limit</sub>*이라는 것은, 임의의 $\epsilon > 0$에 대해 어떤 $\delta > 0$이 존재하여

$$a < x < a+\delta \implies \lvert f(x) - L\rvert < \epsilon$$

이 성립하는 것이다. 이 때 $\lim_{x\rightarrow a^+} f(x) = L$로 적는다. 비슷하게 $f$가 $(a-c, a)$에서 정의되어 있을 때, $x \rightarrow a^-$일 때의 *왼쪽 극한<sub>left limit</sub>*은
...
예컨대 $\lim_{x\rightarrow 0}1/x^2 = \infty$이며, 이 경우 직선 $x = 0$을 그래프의 *수직점근선<sub>vertical asymptote</sub>*이라 한다.

::: 정의 14
어떤 실수 $N_0$보다 큰 $x$에서 정의된 함수 $f$에 대해, $\lim_{x\rightarrow\infty} f(x) = L$이란 임의의 $\epsilon > 0$에 대해 어떤 $N > N_0$이 존재하여 

$$x > N\implies\lvert f(x) - L\rvert < \epsilon$$

인 것이다. 비슷하게 어떤 $N_0$보다 작은 $x$에서 정의된 함수에 대해 $\lim_{x\rightarrow-\infty} f(x) = L$은 
...
직접 대입이 $0/0$ 꼴의 부정형을 줄 때는 인수분해나 분자의 유리화 같은 대수적 변형으로 분모의 영점을 약분하여 극한법칙이 적용되는 형태로 바꾸면 된다. [예시 10](#ex10){: data-lid="qvgeb" data-relation="weak" reviewed="" }의 $\lim(\sin x)/x = 1$ 또한 이러한 대수적 정리와 결합하여 $\sin$이 섞인 부정형을 처리하는 데 쓰인다.
