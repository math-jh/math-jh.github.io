...
::: 증명
결론에 반하여 $L \neq L'$이라 가정하자. 그럼 $\epsilon = \frac{1}{2}\lvert L - L'\rvert > 0$이다. 이제 [정의 2](#def2){: data-lid="api8o" }에 의해 각각에 대응하는 $\delta_1, \delta_2 > 0$이 존재하여 다음 두 조건
...
한편, [정의 2](#def2){: data-lid="u8ea3" }는 원칙적으로 함수의 극한값의 후보 $L$이 주어졌을 때 그 극한값이 실제로 $L$이 맞다는 것을 보일 때만 사용할 수 있다. 즉, 이는 함수의 극한값이 <em-ko>무엇인지</em-ko> 알려주는 도구는 아니다. 이를 위해서는 다음 명제가 유용하다.
...
3. 다음의 부등식
    ...
    트릭은 $\epsilon=1$로 두고 [정의 2](#def2){: data-lid="gyq6x" }를 $f$와 $g$ 각각에 적용하는 것이다. 그럼 적당한 $\delta_1, \delta_2$가 존재하여
...
::: 증명
1. [명제 5](#prop5){: data-lid="l1jgq" }의 3을 적용하여 $k$에 대한 귀납법을 돌리면 된다.
...
에 대하여, [명제 5](#prop5){: data-lid="bkyke" }의 합·상수배 법칙으로 극한을 각 항으로 분리하고 거듭제곱에 [따름정리 6](#cor6){: data-lid="kc5tq" }을 적용하면

$$\lim_{x\rightarrow a}f(x)=c_n\Bigl(\lim_{x\rightarrow a}x\Bigr)^n+\cdots +c_1\lim_{x\rightarrow a}x+\lim_{x\rightarrow a}c_0$$

이 되어, 마지막으로 [예시 7](#ex7){: data-lid="oefr5" }을 대입하면 $\lim_{x\rightarrow a}f(x)=f(a)$를 얻는다. 비슷한 방식으로, 다항함수의 비로 이루어진 유리함수의 극한도 분모의 극한이 $0$이 아니라면 분자와 분모의 극한의 비로 얻어진다.
...
이며, [명제 8](#prop8){: data-lid="cglpa" }를 적용하면 $\cos x \rightarrow 1$임을 안다. 이제 이를 이용하여 앞선 부등식에 다시 [명제 8](#prop8){: data-lid="629s1" }를 적용하면 $(\sin x)/x$의 극한값이 $1$임을 안다. 
:::
...
직접 대입이 $0/0$ 꼴의 부정형을 줄 때는 인수분해나 분자의 유리화 같은 대수적 변형으로 분모의 영점을 약분하여 극한법칙이 적용되는 형태로 바꾸면 된다. [예시 10](#ex10){: data-lid="qvgeb" }의 $\lim(\sin x)/x = 1$ 또한 이러한 대수적 정리와 결합하여 $\sin$이 섞인 부정형을 처리하는 데 쓰인다.
