...
이 두 정리의 증명은 실수의 *완비성*을 본질적으로 필요로 하므로, 엄밀한 증명은 완비성을 소개한 뒤에 해석학 카테고리에서 다루기로 한다. 
...
예컨대 $$f(x) = x^n$$ ($$x \geq 0$$, $$n$$은 자연수) 은 연속이고 순증가하므로, 그 역함수인 $$n$$제곱근 $$\sqrt[n]{x}$$도 연속이다. 마찬가지로 삼각함수를 제한한 것의 역함수인 역삼각함수 등도 모두 연속이다. 이 명제는 [§미분법](/ko/math/calculus/differentiation_rules)에서 역함수의 미분 공식을 세우는 토대가 된다.
