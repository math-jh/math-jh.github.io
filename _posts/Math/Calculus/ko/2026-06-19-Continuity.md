...
우리는 [§함수의 극한](/ko/math/calculus/functions_and_limits){: data-lid="a1hts" data-relation="required" reviewed="" }에서 극한을 엄밀하게 정의하였으니, 그 위에서 연속성을 정의한다.

## 연속의 정의

직관적으로 연속함수는 <em-ko>끊김 없이 이어진</em-ko> 함수로, 이를 엄밀하게 옮기는 데 필요한 도구는 이미 극한이라는 형태로 갖추어져 있다.
...
연속은 극한값이 함숫값과 일치한다는 진술이므로, [§함수의 극한, ⁋명제 5](/ko/math/calculus/functions_and_limits#prop5){: data-lid="nn0lo" data-relation="required" reviewed="" }이 그대로 연속함수의 연산으로 옮겨 온다.
...
이 항상 성립하는 것을 안다. 한편 [§함수의 극한, ⁋예시 10](/ko/math/calculus/functions_and_limits#ex10){: data-lid="7ms67" data-relation="required" reviewed="" }에서 얻은 부등식 $\sin t \leq t$ ($0 < t < \pi/2$) 는, $t \geq \pi/2$일 때 $\lvert \sin t\rvert \leq 1 < t$이고 $\sin(-t) = -\sin t$이므로, 모든 실수 $t$에 대한 부등식 $\lvert \sin t\rvert \leq \lvert t\rvert$로 확장된다. 이를 적용하면, 임의의 $a \in \mathbb{R}$에 대해
...
이제 $\cos$ 함수는 $\sin$함수를 평행이동하여 얻을 수 있으므로 [명제 2](#prop2){: data-lid="e9wis" data-relation="required" reviewed="" }에 의해 연속이며, 따라서 위에서 살펴본 논증에 의하여 $\tan x$도 분모가 $0$이 되는 지점을 제외한 곳에서는 모두 연속이다. 
