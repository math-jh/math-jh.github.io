...
로피탈 정리는 한 번 적용한 비가 다시 부정형이면 거듭 적용할 수 있다. 또한 같은 논증을 코시 평균값 정리로 다시 펴면 $$x \to a^+$$, $$x \to \infty$$ 같은 한쪽 극한이나 무한대에서의 극한, 그리고 $$\frac{\infty}{\infty}$$ 꼴에 대해서도 동일한 결론이 성립한다. 이러한 변형은 뒤의 예시에서 직접 활용한다.
...
예시 7은 정의역이 열린 구간이어서 임계점 하나가 곧 최대를 주었지만, 정의역이 닫힌 구간 $$[a, b]$$이면 사정이 다르다. 연속함수는 닫힌 유계 구간에서 최댓값과 최솟값을 가지며 (최대·최소 정리), 그 값은 임계점이거나 구간의 끝점에서만 일어난다. 따라서 닫힌 구간에서의 전역 최적화는 임계점에서의 값과 두 끝점에서의 값을 모두 비교하는 것으로 끝난다.

<div class="proposition" markdown="1">

<ins id="prop8">**명제 8 (닫힌 구간의 전역 극값)**</ins> $$f$$가 닫힌 유계 구간 $$[a, b]$$에서 연속이고 $$(a, b)$$에서 미분가능하면, $$f$$의 최댓값과 최솟값은 $$(a, b)$$의 임계점들과 두 끝점 $$a, b$$로 이루어진 유한 후보 집합에서 달성된다.
...
최대·최소 정리에 의해 $$f$$는 어떤 점 $$c \in [a, b]$$에서 최댓값을 가진다. $$c$$가 끝점이면 후보에 들어 있다. $$c$$가 내부 점 $$(a, b)$$에 있으면 그 점은 전역 최댓값이므로 특히 극댓값이고, 페르마 정리 ([§평균값 정리, ⁋정리 2](/ko/math/calculus/mean_value_theorem#thm2))에 의해 $$f'(c) = 0$$, 즉 임계점이다. 최솟값도 같다. 따라서 두 극값은 모두 명시한 유한 집합 안에서 일어난다.

</details>

이 절차를 실제 문제에 적용해 보자. 다음 예시는 정의역이 닫힌 구간인 함수의 전역 극값을 후보 비교로 결정한다.

<div class="example" markdown="1">

<ins id="ex9">**예시 9 (닫힌 구간의 최댓값과 최솟값)**</ins> $$f(x) = x^3 - 3x$$를 구간 $$[-2, 2]$$에서 최적화하자. $$f'(x) = 3x^2 - 3 = 3(x-1)(x+1)$$이므로 임계점은 $$x = \pm 1$$이다. 후보 네 점에서의 값을 모으면
...
이다. 따라서 최댓값은 $$2$$ ($$x = -1$$과 $$x = 2$$에서), 최솟값은 $$-2$$ ($$x = -2$$와 $$x = 1$$에서)이다. $$f''(x) = 6x$$이므로 $$f''(-1) = -6 < 0$$, $$f''(1) = 6 > 0$$이어서 $$x = -1$$은 극댓값, $$x = 1$$은 극솟값임이 이계도함수 판정과도 일치한다.

</div>

부정형 극한을 동반한 응용도 같은 도구로 처리한다. 로피탈 정리의 한쪽·무한대 변형을 쓰는 전형적 계산을 살펴보자.

<div class="example" markdown="1">

<ins id="ex10">**예시 10 (반복 적용과 무한대 극한)**</ins> 먼저 $$\frac{0}{0}$$ 꼴 $$\lim_{x\to 0}\dfrac{x - \sin x}{x^3}$$은 도함수의 비를 거듭 취하여
...
으로 계산된다. 각 단계가 여전히 $$\frac{0}{0}$$ 꼴임을 확인하고 정리를 적용했다. 한편 임의의 $$\alpha > 0$$에 대한 $$\frac{\infty}{\infty}$$ 꼴 극한은

$$\lim_{x\to\infty}\frac{\ln x}{x^\alpha} = \lim_{x\to\infty}\frac{1/x}{\alpha x^{\alpha-1}} = \lim_{x\to\infty}\frac{1}{\alpha x^\alpha} = 0$$

이어서, 로그함수가 임의의 양의 거듭제곱보다 느리게 발산함을 보인다.

</div>

곱이나 차의 부정형 $$0\cdot\infty$$, $$\infty - \infty$$도 적절히 분수로 바꾸면 로피탈 정리의 영역으로 들어온다. 다음 예시는 $$0\cdot\infty$$ 꼴을 $$\frac{\infty}{\infty}$$ 꼴로 옮겨 처리한다.

<div class="example" markdown="1">

<ins id="ex11">**예시 11 ($$0\cdot\infty$$ 꼴의 변형)**</ins> $$\lim_{x\to 0^+} x \ln x$$를 보자. 이는 $$0 \cdot (-\infty)$$ 꼴이므로 직접 정리를 적용할 수 없으나, $$x\ln x = \dfrac{\ln x}{1/x}$$로 다시 쓰면 $$\frac{-\infty}{\infty}$$ 꼴이 되어

$$\lim_{x\to 0^+} x \ln x = \lim_{x\to 0^+}\frac{\ln x}{1/x} = \lim_{x\to 0^+}\frac{1/x}{-1/x^2} = \lim_{x\to 0^+}(-x) = 0$$

이다. 이 결과는 $$\lim_{x\to 0^+} x^x = \exp\bigl(\lim_{x\to 0^+} x\ln x\bigr) = e^0 = 1$$과 같은 거듭제곱 부정형의 계산으로 이어진다.
...
<ins id="ex12">**예시 12 (그래프의 종합 분석)**</ins> $$f(x) = x e^{-x}$$를 $$\mathbb{R}$$ 위에서 분석하자. 일계·이계도함수는

$$f'(x) = (1 - x)e^{-x}, \qquad f''(x) = (x - 2)e^{-x}$$

이다. $$e^{-x} > 0$$이므로 $$f'$$의 부호는 $$1 - x$$가, $$f''$$의 부호는 $$x - 2$$가 결정한다. 따라서 $$x < 1$$에서 $$f' > 0$$ (순증가), $$x > 1$$에서 $$f' < 0$$ (순감소)이고, 일계도함수 판정 (명제 2)에 의해 $$x = 1$$에서 극댓값 $$f(1) = e^{-1}$$을 가진다. 또 $$x < 2$$에서 $$f'' < 0$$ (오목), $$x > 2$$에서 $$f'' > 0$$ (볼록)이므로 $$x = 2$$가 변곡점이며 그 값은 $$f(2) = 2e^{-2}$$이다. 끝으로 $$\lim_{x\to\infty} x e^{-x} = 0$$ (예시 10과 같은 유형의 극한)이고 $$\lim_{x\to -\infty} x e^{-x} = -\infty$$이므로, 그래프는 왼쪽에서 음의 무한대로 내려가 $$x = 1$$에서 최고점 $$e^{-1}$$에 이른 뒤 $$x$$축에 점근하며 내려간다.
