...
역시 $c$가 구간 내부에 있는 경우 위의 [정의 1](#def1){: data-lid="bof2y" data-relation="weak" }과 같은 미묘함이 여전히 존재한다. 가령
...
::: 증명
$F(t) = \int_a^t f(x) \dd{x}$는 $f \geq 0$이므로 $t$에 대해 증가하고, [§적분, ⁋명제 11](/ko/math/calculus/integration#prop11){: data-lid="uz0b9" data-relation="required" }의 단조성에 의해
...
직접 부등식 $0 \leq f \leq g$를 세우기 어려울 때는 급수에서처럼 극한비교를 쓴다. 즉, 두 양함수가 $f(x)/g(x) \rightarrow c$ ($0 < c < \infty$) 를 만족하면 [§무한급수, ⁋명제 7](/ko/math/calculus/series#prop7){: data-lid="hpcql" data-relation="required" }과 같은 논증으로 두 적분이 함께 수렴·발산하므로, 피적분함수가 $x \rightarrow \infty$에서 어떤 함수처럼 행동하는지만 알면 판정이 끝난다.
...
::: 증명
$0 \leq f + \lvert f\rvert \leq 2\lvert f\rvert$이므로 [명제 3](#prop3){: data-lid="xdge1" data-relation="required" }으로 $\int_a^\infty (f(x) + \lvert f(x)\rvert) \dd{x}$이 수렴하고, $\int_a^\infty f(x) \dd{x} = \int_a^\infty (f(x) + \lvert f(x)\rvert) \dd{x} - \int_a^\infty \lvert f(x)\rvert \dd{x}$도 수렴한다.
:::
...
로 무한구간 적분이 된다. 곱해진 $u^{-2} > 0$은 부등식과 절댓값을 보존하므로 [명제 3](#prop3){: data-lid="2czby" data-relation="required" }과 [명제 4](#prop4){: data-lid="dvtce" data-relation="required" }가 특이적분의 수렴 판정에도 그대로 성립한다.
...
는 $s > 0$에서 수렴한다. $0$ 근처는 $x^{s-1}$의 특이적분이 $s > 0$에서 수렴하고 ([예시 5](#ex5){: data-lid="oxlfh" data-relation="required" }), $\infty$ 근처는 $e^{-x}$가 거듭제곱을 압도하기 때문이다. 부분적분으로
