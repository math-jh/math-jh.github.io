...
::: 예시 5 (p-적분)
거듭제곱의 이상적분은 무한구간과 singular point에서 정확히 반대의 경계를 보인다. 무한구간 $\int_1^{\infty} x^{-p} \dd{x}$는 $p > 1$에서 수렴하고 $p \leq 1$에서 발산하는 반면, singular point를 포함하는 $\int_0^1 x^{-p} \dd{x}$는 거꾸로 $p < 1$에서 수렴하고 $p \geq 1$에서 발산한다. 이 계산은 둘 다 같은 원시함수에서 나오는데, $p \neq 1$이면
...
이고 $\Gamma(1) = \int_0^\infty e^{-x} \dd{x} = 1$이므로 $\Gamma(n) = (n-1)!$이다. 즉 gamma function은 팩토리얼을 실수로 확장한다.
:::
