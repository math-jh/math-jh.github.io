...
::: 명제 3 (비교판정)
$x \geq a$에서 $0 \leq f(x) \leq g(x)$라 하자. $\int_a^\infty g(x) \mathop{dx}$가 수렴하면 $\int_a^\infty f(x) \mathop{dx}$도 수렴하고, $\int_a^\infty f(x) \mathop{dx}$가 발산하면 $\int_a^\infty g(x) \mathop{dx}$도 발산한다.
:::

::: 증명
$F(t) = \int_a^t f(x) \mathop{dx}$는 $f \geq 0$이므로 $t$에 대해 증가하고, [§적분, ⁋명제 11](/ko/math/calculus/integration#prop11)의 단조성에 의해
...
::: 명제 4 (절대수렴)
$\int_a^\infty \lvert f(x)\rvert \mathop{dx}$이 수렴하면 $\int_a^\infty f(x) \mathop{dx}$도 수렴한다.
:::

::: 증명
$0 \leq f + \lvert f\rvert \leq 2\lvert f\rvert$이므로 [명제 3](#prop3)으로 $\int_a^\infty (f(x) + \lvert f(x)\rvert) \mathop{dx}$이 수렴하고, $\int_a^\infty f(x) \mathop{dx} = \int_a^\infty (f(x) + \lvert f(x)\rvert) \mathop{dx} - \int_a^\infty \lvert f(x)\rvert \mathop{dx}$도 수렴한다.
:::
