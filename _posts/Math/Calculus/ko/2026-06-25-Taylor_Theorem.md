...
[§미분과 도함수](/ko/math/calculus/derivatives){: data-lid="uf9ok" }에서 우리는 함수를 한번 미분하여 얻어진 미분계수가 함수의 접선 
...
만 남고, $h'(t) = -(n+1)(x-t)^n$이다. [§평균값 정리, ⁋정리 6](/ko/math/calculus/mean_value_theorem#thm6){: data-lid="j4wzu" }을 $a$와 $x$ 사이에 적용하면
...
::: 예시 3
$f(x) = e^x$의 임의의 미분은 자기 자신인 것을 [§미분법](/ko/math/calculus/differentiation_rules){: data-lid="0zmhp" }에서 확인하였으므로, 임의의 $k$에 대하여 $f^{(k)}(0) = 1$이다. 따라서 테일러 다항식은
...
이므로 ([§수열의 극한, ⁋예시 6](/ko/math/calculus/sequences#ex6){: data-lid="4l0s1" }) 모든 실수 $x$에서
...
::: 예시 4 (삼각함수)
$\sin x$는 도함수가 $\cos x, -\sin x, -\cos x, \sin x$로 주기적이므로, $f^{(k)}(0)$이 $0, 1, 0, -1$을 반복한다. 모든 미분이 $\lvert f^{(n+1)}\rvert \leq 1$로 유계이므로, 위의 [예시 3](#ex3){: data-lid="i8ddn" }과 같은 논법으로 나머지가 $0$으로 간다는 것을 보일 수 있고, 따라서 모든 $x$에서
...
을 얻는다. ([§미분법, ⁋명제 1](/ko/math/calculus/differentiation_rules#prop1){: data-lid="bvyms" }) 이는 더 일반적으로, 실수 $\alpha$에 대해 정의된 다음의 일반화된 이항급수
...
위의 [예시 4](#ex4){: data-lid="7jlla" }처럼 모든 도함수가 하나의 상수로 동시에 유계인 경우에는 그 테일러 급수가 자기자신과 같다. 이를 정식으로 적으면 다음과 같다. 
...
이다. 우변은 고정된 $x$에서 $n \rightarrow \infty$일 때 $0$으로 가므로 ([§수열의 극한, ⁋예시 6](/ko/math/calculus/sequences#ex6){: data-lid="zmb5i" }의 $r^n/n! \rightarrow 0$), $R_n(x) \rightarrow 0$이고 부분합이 $f(x)$로 수렴한다.
:::

한편, [정리 2](#thm2){: data-lid="12v03" }는 본질적으로 수치적인 것으로, 이를 사용하면 근삿값이 얼마나 정확한지를 손으로 평가할 수 있다. 가령 $\sin(0.1)$을 $P_3(x) = x - x^3/6$로 근사하면, 4차 나머지가 $\lvert R_3(0.1)\rvert \leq (0.1)^4/4! \approx 4.2\times 10^{-6}$이라 소수점 다섯째 자리까지 정확하다는 것을 확인할 수 있으며, $e = \sum_k 1/k!$를 처음 $n+1$항에서 끊은 오차는 $\lvert R_n(1)\rvert \leq 3/(n+1)!$ ($e^c < 3$) 임을 알 수 있다. 
