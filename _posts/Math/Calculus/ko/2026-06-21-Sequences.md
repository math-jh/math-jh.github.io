...
2. 더 일반적으로, 같은 차수를 갖는 다항식의 비는 최고차항의 비에 의해 결정된다.
   ...
   의 분자·분모를 $n^k$으로 나누면 분자·분모 모두 유한개의 $1/n^j$ 항과 상수항으로 이루어진다. 그럼 이 때 $1/n^j \rightarrow 0$이므로 분자와 분모는 각각 최고차항의 계수로 수렴하는 것을 알 수 있다. 만일 분모가 분자보다 큰 차수를 갖는다면 [명제 4](#prop4){: data-relation="required" }와 앞선 1번에 의하여 이 비율이 $0$으로 수렴함을 알 수 있고, 비슷하게 분자가 분모보다 큰 차수를 갖는다면 이 비율이 발산함을 알 수 있다. 
