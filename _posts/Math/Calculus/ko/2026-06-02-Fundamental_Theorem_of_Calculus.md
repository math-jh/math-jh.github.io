...
제1형이 보장한 가변상한 적분은 멱급수를 다룰 때에도 쓰인다. 멱급수는 수렴반경 안에서 항별로 적분할 수 있고, 그렇게 얻은 $$\int_0^x$$가 친숙한 함수의 새로운 급수 표현을 준다.
...
[§멱급수, ⁋정리 2](/ko/math/calculus/power_series#thm2)에서 수렴반경을 주는 상극한을 보면 $$n^{1/n} \to 1$$이므로 $$\limsup_n \lvert c_n/(n+1)\rvert^{1/n} = \limsup_n \lvert c_n\rvert^{1/n}$$이 되어, 적분한 급수의 수렴반경도 $$R$$이다. 한편 닫힌구간 $$[0, x] \subset (-R, R)$$에서 멱급수의 부분합은 $$f$$로 균등수렴하므로, [§적분, ⁋명제 11](/ko/math/calculus/integration#prop11)의 단조성으로부터 부분합의 적분이 $$\int_0^x f$$로 수렴한다. 각 단항식의 적분이 $$\int_0^x c_n t^ndt = c_n x^{n+1}/(n+1)$$이므로 주장이 따르며, 균등수렴이 무한합과 적분의 교환을 보장한다는 엄밀한 논증은 [\[해석학\] §균등수렴](/ko/math/analysis/uniform_convergence)에서 다룬다.

</details>

<div class="example" markdown="1">

<ins id="ex8">**예시 8 (오차함수의 멱급수)**</ins> [예시 3](#ex3)의 오차함수로 돌아가자. 지수함수의 멱급수로부터 $$e^{-t^2} = \sum_{n=0}^\infty \frac{(-1)^n}{n!}t^{2n}$$ (수렴반경 $$\infty$$) 이므로, [명제 7](#prop7)로 항별 적분하면
...
이다. 원시함수가 초등함수로 적히지 않던 오차함수가 모든 $$x$$에서 수렴하는 멱급수로는 이렇게 명시적으로 표현된다. 같은 방법으로 $$1/(1 + x^2)$$와 $$1/(1+x)$$를 항별 적분하면 각각 $$\arctan x = \sum_{n=0}^\infty \frac{(-1)^n}{2n+1} x^{2n+1}$$, $$\ln(1+x) = \sum_{n=1}^\infty \frac{(-1)^{n-1}}{n} x^n$$ ($$\lvert x\rvert < 1$$) 을 얻는다.
