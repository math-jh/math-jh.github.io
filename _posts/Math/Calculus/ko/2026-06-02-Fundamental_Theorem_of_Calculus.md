...
<ins id="ex6">**예시 6 (특수함수의 도함수)**</ins> *오차함수<sub>error function</sub>*는
...
<ins id="prop7">**명제 7 (일반 라이프니츠 규칙)**</ins> $$f$$가 연속이고 $$g, h$$가 미분가능하면
...
<ins id="ex8">**예시 8 (양끝 변수 적분)**</ins> $$\displaystyle\Phi(x) = \int_{x}^{x^2} e^{t^2}\,dt$$의 도함수를 구하자. 명제 7에서 $$f(t) = e^{t^2}$$, $$g(x) = x^2$$, $$h(x) = x$$이므로 $$g'(x) = 2x$$, $$h'(x) = 1$$이고
...
<ins id="ex9">**예시 9 (가정 위반)**</ins> $$\displaystyle\int_{-1}^{1} \frac{dx}{x^2}$$에 형식적으로 $$\bigl[-x^{-1}\bigr]_{-1}^{1} = -1 - 1 = -2$$를 대입하면 음수가 나오는데, 피적분함수 $$1/x^2$$는 항상 양수이므로 이는 명백히 틀렸다. 원인은 $$x = 0$$에서 피적분함수가 발산하여 $$[-1,1]$$에서 연속이 아니라는 데 있다. 정리 3의 가정이 깨졌으므로 평가정리를 그대로 쓸 수 없으며, 실제로 이 적분은 발산한다.
...
<ins id="prop10">**명제 10 (적분의 평균값 정리)**</ins> $$f$$가 $$[a,b]$$에서 연속이면, 어떤 $$\xi \in [a,b]$$가 존재하여
...
값 $$\dfrac{1}{b-a}\int_a^b f$$를 $$f$$의 $$[a,b]$$에서의 *평균값<sub>average value</sub>*이라 하며, 명제 10은 연속함수가 자신의 평균값을 구간 안 어딘가에서 실제로 취한다는 뜻이다.
