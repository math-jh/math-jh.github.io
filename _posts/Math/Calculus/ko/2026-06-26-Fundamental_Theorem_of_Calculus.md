...
이다. 마찬가지로 *적분로그* $\li(x) = \int_2^x \dd{t}/\ln t$도 $\li'(x) = 1/\ln x$로 도함수가 즉시 나온다. 이처럼 적분으로 정의된 함수는 그 자체로 미적분의 대상이 된다.
:::
...
특히 [정리 1](#thm1)은 적분의 상·하한이 변수에 의존할 때 [§미분법, ⁋정리 4](/ko/math/calculus/differentiation_rules#thm4)과 결합된다. 상한이 함수 $g(x)$이면 $F(u) = \int_a^u f$로 두어 $\int_a^{g(x)} f = F(g(x))$이고, $F'(u) = f(u)$이므로 연쇄법칙으로 $\frac{d}{\dd{x}}\int_a^{g(x)} f(t)\dd{t} = f(g(x))g'(x)$이다. 상·하한이 모두 변수이면 이를 두 개의 구간으로 나누어 양쪽에 적용하면 된다.
