...
한편, 위의 따름정리는 이는 임의의 함수의 적분이 초등함수의 꼴로 나타난다는 것과는 다른 이야기로, 오직 위의 식으로 정의한 $$\int_a^x f(t)\mathop{dt}$$가 <em-ko>그 자체로</em-ko> $$f$$의 원시함수라는 것이다. 가령 다음 예시를 보자. 
...
이다. $$F(u) = \int_c^v f(v)\mathop{dv}$$로 두면[정리 1](#thm1)에 의해 $$F'(u) = f(u)$$이고, 우변은 $$F(g(x)) - F(h(x))$$이다. 연쇄법칙을 두 항에 각각 적용하면
