...
$$f$$가 $$[a,b]$$에서 연속이므로 최대·최소 정리 ([§연속함수, ⁋정리 4](/ko/math/calculus/continuity#thm4))에 의해 $$[a,b]$$에서 최댓값과 최솟값을 가진다. 두 값이 모두 끝점에서만 일어난다면, $$f(a) = f(b)$$이므로 최댓값과 최솟값이 같아 $$f$$는 상수이고 모든 내부 점에서 $$f' = 0$$이다. 그렇지 않으면 최댓값이나 최솟값 중 적어도 하나가 내부 점 $$c$$에서 일어나며, 그 점은 극값이므로 페르마 정리에 의해 $$f'(c) = 0$$이다.
...
<ins id="ex12">**예시 12 (근의 유일성)**</ins> 방정식 $$x^3 + x - 1 = 0$$의 실근이 정확히 하나임을 보이자. $$f(x) = x^3 + x - 1$$로 두면 $$f(0) = -1 < 0$$이고 $$f(1) = 1 > 0$$이므로, 중간값 정리 ([§연속함수, ⁋정리 5](/ko/math/calculus/continuity#thm5)) 에 의해 $$(0, 1)$$에 근이 적어도 하나 있다. 한편
