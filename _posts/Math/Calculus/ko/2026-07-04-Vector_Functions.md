...
속도는 곡선의 진행 방향을 가리키고 그 크기가 점이 움직이는 빠르기이며, 단위접선벡터는 속도의 방향만 남긴 것이다.

한편, 매개곡선 $$\mathbf{r}$$이 *매끄럽다<sub>smooth</sub>*는 것은 속도 $$\mathbf{r}'(t)$$가 연속적으로 존재하며 한 번도 $$0$$이 아닌 것을 뜻한다. 즉 $$\mathbf{r}$$이 $$C^1$$이면서 정칙($$\mathbf{r}'\neq 0$$)인 경우로, 이 조건 아래에서 단위접선벡터 $$\mathbf{T}$$가 어디서나 잘 정의된다. 이 글의 뒤에서는 특별한 언급이 없는 한 매끄러운 곡선만 다룬다. 
