...
이므로, 결국 모든 정수 $n$에 대해 $(x^n)' = n x^{n-1}$이 성립한다 (단 $n < 0$인 경우 $x \neq 0$이어야 한다).
...
우변의 급수의 수렴반경이 $R$이 된다는 것 자체는 $n\rightarrow \infty$일 때 $n^{1/n}$이 $1$로 수렴한다는 것으로부터 얻어지지만, 이것이 실제로 $f'(x)$와 같다는 것을 엄밀히 정당화하기에는 우리의 도구가 부족하므로, 그 증명은 생략하고 [Ste]에 위임한 뒤 항별 미분의 등식만 받아들이고 넘어가기로 한다. 
