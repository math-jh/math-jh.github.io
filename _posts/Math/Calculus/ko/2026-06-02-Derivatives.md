...
우리는 다음 글에서 여러가지 함수의 곱셈법칙을 살펴볼 것이다. 이 때 이번 글에서 우리는 

거듭제곱함수의 도함수는 다음과 같다: 자연수 $$n$$에 대해 $$\frac{d}{dx}x^n = n x^{n-1}$$이다. 이는 $$\frac{(x+h)^n - x^n}{h}$$를 이항정리로 전개하면 $$nx^{n-1} + \binom n2 x^{n-2}h + \cdots$$이 되고, $$h \to 0$$일 때 첫 항만 남기 때문이다. 이 공식과 선형성을 결합하면 임의의 다항함수의 도함수를 즉시 계산할 수 있다. 예를 들어 $$\frac{d}{dx}(3x^4 - 5x + 2) = 12x^3 - 5$$이다.
