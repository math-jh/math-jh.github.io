...
우리는 [§연속함수](/ko/math/calculus/continuity){: data-relation="required" reviewed="" }에서 함수가 연속이라는 것을 앞서 도입한 $\epsilon-\delta$의 언어로 재정립했다. 이제 자연스러운 흐름은 함수의 미분을 정의하는 것이다.
...
이다. $x \rightarrow a$일 때 우변의 첫 인자는 $f'(a)$로, 둘째 인자는 $0$으로 수렴하므로, [§함수의 극한, ⁋명제 5](/ko/math/calculus/functions_and_limits#prop5){: data-relation="required" reviewed="" }에 의해 
...
를 보자. $a \neq 0$에서는 $a$에 수렴하는 유리수열과 무리수열을 따라 함숫값이 각각 $a^2$과 $0$으로 갈라지므로 $f$는 불연속이고, [명제 2](#prop2){: data-relation="required" reviewed="" }의 대우에 의해 미분가능하지 않다. 반면 $0$에서는 $\lvert f(x)\rvert \leq x^2$이라 연속이며, 평균변화율이
...
한편, 도함수는 결국엔 평균변화율이라는 함수의 극한이므로, [§함수의 극한, ⁋명제 5](/ko/math/calculus/functions_and_limits#prop5){: data-relation="required" reviewed="" }로부터 미분도 선형성을 갖는다는 것을 증명할 수 있다.
...
로 갈라지고, 각 항이 $f'(a), g'(a)$로 수렴하므로 [§함수의 극한, ⁋명제 5](/ko/math/calculus/functions_and_limits#prop5){: data-relation="required" reviewed="" }에 의해 합도 수렴한다. $cf$도 평균변화율 식을 살펴보면 동일한 결과를 얻는다.
:::
