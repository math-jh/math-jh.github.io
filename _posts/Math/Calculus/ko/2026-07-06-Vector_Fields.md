...
::: 예시 4
지름 방향으로 뻗는 $$\mathbf{F}(x,y,z) = (x, y, z)$$는 $$\div \mathbf{F} = 1 + 1 + 1 = 3$$으로 어디서나 양이고 (모든 점이 원천), $$\curl \mathbf{F} = 0$$이다. 실제로 $$\mathbf{F} = \nabla\bigl((x^2+y^2+z^2)/2\bigr)$$인 보존장이다. 반대로 $$z$$축 둘레로 도는 $$\mathbf{G}(x,y,z) = (-y, x, 0)$$은 $$\div \mathbf{G} = 0$$(솟음 없는 순환)이지만 $$\curl \mathbf{G} = (0, 0, 2)$$로 $$z$$축 방향으로 일정하게 휘돈다.
:::
...
$$\curl(\nabla f) = 0, \qquad \div(\curl \mathbf{F}) = 0$$

이다.
:::

::: 증명
$$\nabla f = (f_x, f_y, f_z)$$의 회전의 첫 성분은 $$\partial_y f_z - \partial_z f_y = f_{zy} - f_{yz}$$인데 클레로 정리로 $$0$$이고, 나머지 두 성분도 같은 이유로 $$0$$이다. 또 $$\curl \mathbf{F} = (R_y - Q_z,\ P_z - R_x,\ Q_x - P_y)$$의 발산은
