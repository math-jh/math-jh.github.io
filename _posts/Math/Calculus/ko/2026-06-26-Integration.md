...
::: 증명
$G$도 $f$의 원시함수라 하면 $(G - F)' = f - f = 0$이다. [§평균값 정리, ⁋따름정리 5](/ko/math/calculus/mean_value_theorem#cor5){: data-lid="urwiw" }에 의해 구간에서 도함수가 항등적으로 $0$인 함수는 상수이므로, $G - F = C$인 상수 $C$가 있어 $G = F + C$이다.
:::
...
로 적고, 이를 $f$의 *부정적분<sub>indefinite integral</sub>*이라 한다. 여기서 $C$를 *적분상수*, $f$를 *피적분함수*, 기호 $\dd{x}$를 적분변수의 표시로 본다. [명제 2](#prop2){: data-lid="p0x8b" }가 보장하는 "상수 차이뿐"이라는 사실 덕분에 적분상수 $C$ 하나로 모든 원시함수를 한꺼번에 나타낼 수 있다.
...
한편, 우리는 [§미분법](/ko/math/calculus/differentiation_rules){: data-lid="vu214" }에서 여러 함수의 미분을 살펴보았고, 부정적분은 미분의 반대이므로 이를 통해 적분공식들을 유도할 수 있다. 우선 그 전에 부정적분의 선형성을 보이자. 
...
::: 증명
$F, G$를 각각 $f, g$의 원시함수라 하면, [§미분과 도함수, ⁋명제 4](/ko/math/calculus/derivatives#prop4){: data-lid="3pj9t" }에 의해
...
이제 [§미분법](/ko/math/calculus/differentiation_rules){: data-lid="drxbj" }에서의 여러 함수들의 도함수 공식을 뒤집으면 다음의 기본 공식들을 얻는다. 즉, 각 공식의 우변을 미분하면 피적분함수가 된다.
...
특히 유용하게 쓰이는 치환적분과 부분적분은 각각 [§미분법, ⁋정리 4](/ko/math/calculus/differentiation_rules#thm4){: data-lid="vhmth" }와 [§미분법, ⁋명제 3](/ko/math/calculus/differentiation_rules#prop3){: data-lid="4c97f" }을 거꾸로 읽은 것이다.
...
::: 증명
$F$를 $f$의 원시함수라 하면 [§미분법, ⁋정리 4](/ko/math/calculus/differentiation_rules#thm4){: data-lid="lftt1" }에 의해
...
::: 증명
[§미분법, ⁋명제 3](/ko/math/calculus/differentiation_rules#prop3){: data-lid="wys3l" }으로 $(uv)' = u'v + uv'$이므로 $uv' = (uv)' - u'v$이고, 양변을 적분하면 $\int (uv)' \dd{x} = uv$에서 주장이 따른다.
:::
...
::: 증명
[§연속함수, ⁋정리 4](/ko/math/calculus/continuity#thm4){: data-lid="zio3w" }로 $f$는 $[a,b]$에서 최솟값 $m$과 최댓값 $M$을 가진다. 위 계산에서 평균값 

$$\frac{1}{b-a}\int_a^b f(x)\dd{x}$$

가 $[m, M]$에 속한다. 이 값이 $m$이거나 $M$이면 최솟값 또는 최댓값을 주는 점을 $c$로 택하면 되고 ($f$가 상수라 $m = M$인 경우도 여기에 포함된다), 그렇지 않으면 최솟값과 최댓값을 주는 두 점 사이의 구간에 [§연속함수, ⁋정리 5](/ko/math/calculus/continuity#thm5){: data-lid="4qkel" }를 적용하여 그 값을 취하는 $c$, 즉 $f(c) = \frac{1}{b-a}\int_a^b f(x)\dd{x}$인 $c$를 얻는다.
:::
...
를 $f$의 $[a,b]$에서의 *평균값*이라 하며, [명제 12](#prop12){: data-lid="t4k2u" }는 연속함수가 자신의 평균값을 적어도 한 점에서 실제로 취함을 말한다.
...
::: 예시 13
$\int_{-1}^{1} x\dd{x} = 0$인 까닭은 $[-1,0]$에서의 음의 넓이와 $[0,1]$에서의 양의 넓이가 정확히 상쇄되기 때문이다. 각 조각은 밑변과 높이가 $1$인 직각삼각형이라 넓이가 $\frac{1}{2}$이고, [명제 11](#prop11){: data-lid="p6uew" }의 둘째 결과를 사용해 갈라 쓰면
...
::: 증명
[§연속함수, ⁋정리 4](/ko/math/calculus/continuity#thm4){: data-lid="utufw" }로 $f$는 최솟값 $m$과 최댓값 $M$을 가진다. $\mu \geq 0$이므로 $m\mu(x) \leq f(x)\mu(x) \leq M\mu(x)$이고, 단조성과 선형성으로 적분하면
...
임을 얻는다. 이 값이 $m$이거나 $M$이면 최솟값 또는 최댓값을 주는 점을 $c$로 택하면 되고, 그렇지 않으면 최솟값과 최댓값을 주는 두 점 사이의 구간에 [§연속함수, ⁋정리 5](/ko/math/calculus/continuity#thm5){: data-lid="ryo7o" }를 적용하여 이 값을 취하는 $c$를 얻는다. 따라서 양변에 $\int_a^b \mu(x)\dd{x}$를 곱하면 주장하는 등식을 얻는다.
:::

만일 $\mu \equiv 1$로 두면 가중 평균값 정리는 [명제 12](#prop12){: data-lid="2y8p5" }로 환원되므로, [명제 14](#prop14){: data-lid="wvmxj" }는 평균값 정리의 일반화로, 일종의 밀도를 추가하는 것으로 생각할 수 있다.
