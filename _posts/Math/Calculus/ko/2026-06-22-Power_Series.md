...
으로 나타낸다. 특히 $$x = 1$$을 넣으면 $$e = \sum 1/n!$$인데, 이 수는 [§수열의 극한, ⁋예시 9](/ko/math/calculus/sequences#ex9)에서 극한 $$\lim(1 + 1/n)^n$$으로 정의했던 자연상수와 일치한다. 

이에 대한 증명은 다음과 같다. 해당 극한의 극한값을 $$L = \lim(1+1/n)^n$$, 급수의 부분합을 $$s_m = \sum 1/k!$$이라 두면, 위의 [§수열의 극한, ⁋예시 9](/ko/math/calculus/sequences#ex9)에서 이미 이항정리로
...
을 얻는다. 좌변은 $$L$$ 이하이므로 우변 또한 $$L$$ 이하이고, $$m$$을 고정한 채 $$n \to \infty$$를 보내면 우변은 유한합이고 각 인수가 $$1 - j/n \to 1$$이므로 극한법칙 ([§수열의 극한, ⁋명제 3](/ko/math/calculus/sequences#prop3))에 의해 $$s_m$$으로 수렴한다. 수렴하는 수열의 모든 항이 $$L$$ 이하이면 그 극한도 $$L$$ 이하이므로 $$s_m \leq L$$이고, 다시 $$m \to \infty$$를 보내면 $$s \leq L$$이다. 두 부등식을 합쳐 $$L = s$$, 즉 두 글에서 정의한 $$e$$는 같은 수이다.
...
<ins id="prop5">**명제 5**</ins> $$f(x) = \sum a_n x^n$$, $$g(x) = \sum b_n x^n$$이 각각 수렴반경 $$R_f, R_g$$를 가지면, $$\lvert x\rvert < \min(R_f, R_g)$$에서
...
<ins id="def6">**정의 6**</ins> 함수 $$f$$가 점 $$a$$ 근방에서 그곳을 중심으로 하는 멱급수와 일치하면 $$f$$가 $$a$$에서 *해석적<sub>analytic</sub>*이라 한다. 정의역의 모든 점에서 해석적이면 $$f$$를 *해석함수*라 한다.
