...
Again, when $c$ lies in the interior of the interval, the same subtlety as in [Definition 1](#def1){: data-lid="bof2y" data-relation="weak" } persists. For instance,
...
::: Proof
Since $f \geq 0$, the function $F(t) = \int_a^t f(x) \dd{x}$ is increasing in $t$, and by monotonicity from [§Integration, ⁋Proposition 11](/en/math/calculus/integration#prop11){: data-lid="uz0b9" data-relation="required" },
...
When the inequality $0 \leq f \leq g$ is difficult to establish directly, we use limit comparison just as for series. That is, if two positive functions satisfy $f(x)/g(x) \rightarrow c$ ($0 < c < \infty$), then the same argument as in [§Infinite Series, ⁋Proposition 7](/en/math/calculus/series#prop7){: data-lid="hpcql" data-relation="required" } shows that the two integrals converge or diverge together; hence it suffices to know which function the integrand resembles as $x \rightarrow \infty$.
...
converges for $s > 0$. Near $0$, the singular integral of $x^{s-1}$ converges for $s > 0$ ([Example 5](#ex5){: data-lid="oxlfh" data-relation="required" }), and near $\infty$, the exponential $e^{-x}$ dominates any power. By integration by parts,
