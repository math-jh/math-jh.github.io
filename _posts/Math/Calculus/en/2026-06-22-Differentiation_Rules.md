...
---
We covered the definition of differentiation and its basic properties in [§Differentiation and Derivatives](/en/math/calculus/derivatives){: data-lid="65feo" data-relation="required" }. In this post, we treat derivatives of concrete functions and differentiation rules applicable to general functions.
...
First, the exponential function was defined in [§Power Series, ⁋Example 3](/en/math/calculus/power_series#ex3){: data-lid="00w8x" data-relation="weak" } by $e^x = \sum_{n\geq 0} x^n/n!$. Applying [Proposition 1](#prop1){: data-lid="z8pkw" data-relation="required" }, its derivative is the sum of term-by-term differentiations
...
The first limit was obtained by the squeeze theorem in [§Limits of Functions, ⁋Example 10](/en/math/calculus/functions_and_limits#ex10){: data-lid="gvxns" data-relation="required" }, and the second follows from the same example's inequality $\lvert 1 - \cos h\rvert \leq h^2/2$, which gives $\lvert(1 - \cos h)/h\rvert \leq \lvert h\rvert/2$.
...
We now define differentiation rules applicable to general forms. In [§Differentiation and Derivatives, ⁋Proposition 4](/en/math/calculus/derivatives#prop4){: data-lid="ogd67" data-relation="weak" } we saw that differentiation behaves well with respect to constant multiples and addition, but it does not simply distribute over products.
...
::: Proof
Defining $h(t)=1/t$, we have $1/g =h \circ g$, and by the derivative of integer powers $h'(t)=-1/t^2$, so by [Theorem 4](#thm4){: data-lid="xpo63" data-relation="required" } we get $(1/g)'(a) = -g(a)^{-2}g'(a)$. Applying the product rule to $f/g = f\cdot(1/g)$ now yields
...
::: Proof
Let $I$ be the interval on which $f$ is defined and let $J = f(I)$ be its image. Since $f$ is monotone and has an inverse, it is injective, hence strictly monotone, and therefore $f^{-1} : J \rightarrow I$ is continuous by [§Continuous Functions, ⁋Proposition 7](/en/math/calculus/continuity#prop7){: data-lid="t16wr" data-relation="required" }.
...
Differentiating both sides of the geometric series ([§Infinite Series, ⁋Example 2](/en/math/calculus/series#ex2){: data-lid="we10a" data-relation="weak" })
...
and the right-hand side becomes $\sum_{n\geq 1} n x^{n-1}$ by [Proposition 1](#prop1){: data-lid="p7bvq" data-relation="required" }, so
