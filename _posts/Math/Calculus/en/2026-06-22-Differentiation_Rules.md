...
If a power series $f(x) = \sum_{n=0}^\infty c_n x^n$ has radius of convergence $R > 0$, then $f$ is differentiable for $\lvert x\rvert < R$ and
...
::: Example 7 (A Differentiable but Not $C^1$ Function)
The function
...
Differentiate both sides of the geometric series ([§Infinite Series, ⁋Example 2](/en/math/calculus/series#ex2))
...
::: Example 9 (Various Differentiation Rules)
When an equation determines $y$ as a function of $x$, to find $y'$ it is often inefficient or leads to an unclean derivative to obtain an explicit expression of the form $y=...$. In such situations, viewing $y$ as a function of $x$ and differentiating both sides, then solving for $y'$, is called *implicit differentiation*. For example, differentiating both sides of the unit circle $x^2 + y^2 = 1$ gives
