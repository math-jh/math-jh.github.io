...
description: "This post defines the limit of a function using the epsilon-delta language and covers the limit laws, limits of powers and roots, the squeeze theorem, one-sided limits, and limits at infinity with examples. Sequences are treated as functions with domain N, and their limits are presented using the epsilon-N language based on the theory of function limits."
excerpt: "Limits of functions via ε-δ, sequences via ε-N, limit laws, and squeeze theorem"
...
Assume for contradiction that $L \neq L'$. Then $\epsilon = \frac{1}{2}\lvert L - L'\rvert > 0$. By [Definition 2](#def2) there exist $\delta_1, \delta_2 > 0$ corresponding to each such that the following two conditions
...
and then applying the second law of [Proposition 5](#prop5) and [Corollary 6](#cor6) we convert this to

$$\lim_{x\rightarrow a}f(x)=c_n(\lim_{x\rightarrow a}x)^n+\cdots c_1\lim_{x\rightarrow a}x+\lim_{x\rightarrow a}c_0$$

and then apply [Example 7](#ex7). In a similar way, one can show that the limit of any rational function formed as a ratio of polynomials is obtained by taking the limits of the numerator and denominator separately and forming their quotient, provided the limit of the denominator is not $0$.
...
![Comparison of triangle-sector-right triangle areas](/assets/images/Math/Calculus/functions_and_limits-1.svg){:style="width:33.66em" class="invert" .align-center}
...
Then the following is the analogue of [Proposition 5](#prop5), and its proof is almost identical.
...
With this we have equipped ourselves with the definition of limit and basic tools. In the next post [§Continuous Functions](/en/math/calculus/continuity), we use limits to define continuity of functions and discuss properties of continuous functions. Meanwhile, why the intuitive "gaplessness" of the real numbers that we treated informally in this post underpins limit theory—for instance, why a monotone bounded sequence converges—is rigorously founded in analysis in [§Completeness of the Reals](/en/math/analysis/completeness_of_reals).
