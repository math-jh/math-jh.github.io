...
::: Proof
If $G$ is also an antiderivative of $f$, then $(G - F)' = f - f = 0$. By [§Mean Value Theorem, ⁋Corollary 5](/en/math/calculus/mean_value_theorem#cor5){: data-lid="urwiw" data-relation="required" }, a function whose derivative is identically $0$ on an interval is constant, so there exists a constant $C$ with $G - F = C$, hence $G = F + C$.
:::
...
and is called the *indefinite integral* of $f$. Here $C$ is the *constant of integration*, $f$ is the *integrand*, and the symbol $\dd{x}$ indicates the variable of integration. Because [Proposition 2](#prop2){: data-lid="p0x8b" data-relation="required" } guarantees that antiderivatives differ only by a constant, the single integration constant $C$ suffices to represent all of them at once.
...
::: Proof
Let $F$ and $G$ be antiderivatives of $f$ and $g$ respectively. Then by [§Differentiation and Derivatives, ⁋Proposition 4](/en/math/calculus/derivatives#prop4){: data-lid="3pj9t" data-relation="required" },
...
Especially useful are integration by substitution and integration by parts, which are respectively the reverses of [§Differentiation, ⁋Theorem 4](/en/math/calculus/differentiation_rules#thm4){: data-lid="vhmth" data-relation="weak" } and [§Differentiation, ⁋Proposition 3](/en/math/calculus/differentiation_rules#prop3){: data-lid="4c97f" data-relation="weak" }.
...
::: Proof
By [§Continuous Functions, ⁋Theorem 4](/en/math/calculus/continuity#thm4){: data-lid="zio3w" data-relation="required" }, $f$ attains a minimum $m$ and a maximum $M$ on $[a,b]$. From the above computation, the average value

$$\frac{1}{b-a}\int_a^b f(x)\dd{x}$$

lies in $[m, M]$. If this value equals $m$ or $M$, we choose $c$ to be a point attaining the minimum or maximum (the case $f$ constant, so $m = M$, is included here); otherwise we apply [§Continuous Functions, ⁋Theorem 5](/en/math/calculus/continuity#thm5){: data-lid="4qkel" data-relation="required" } to the interval between the points attaining the minimum and maximum to obtain a point $c$ with $f(c) = \frac{1}{b-a}\int_a^b f(x)\dd{x}$.
:::
...
::: Example 13
The reason $\int_{-1}^{1} x\dd{x} = 0$ is that the negative area on $[-1,0]$ and the positive area on $[0,1]$ cancel exactly. Each piece is a right triangle with base and height $1$, so its area is $\frac{1}{2}$, and splitting using the second result of [Proposition 11](#prop11){: data-lid="p6uew" data-relation="required" },
...
::: Proof
By [§Continuous Functions, ⁋Theorem 4](/en/math/calculus/continuity#thm4){: data-lid="utufw" data-relation="required" }, $f$ attains a minimum $m$ and a maximum $M$. Since $\mu \geq 0$, we have $m\mu(x) \leq f(x)\mu(x) \leq M\mu(x)$, and integrating using monotonicity and linearity gives
...
If this value equals $m$ or $M$, we choose $c$ to be a point attaining the minimum or maximum; otherwise we apply [§Continuous Functions, ⁋Theorem 5](/en/math/calculus/continuity#thm5){: data-lid="ryo7o" data-relation="required" } to the interval between the points attaining the minimum and maximum to obtain a point $c$ taking this value. Multiplying both sides by $\int_a^b \mu(x)\dd{x}$ then yields the claimed equality.
:::

If we set $\mu \equiv 1$, the Weighted Mean Value Theorem for Integrals reduces to [Proposition 12](#prop12){: data-lid="2y8p5" data-relation="weak" }; thus [Proposition 14](#prop14){: data-lid="wvmxj" data-relation="weak" } is a generalization of the Mean Value Theorem for Integrals, and may be thought of as adding a kind of density.
