...
Here differentiability at the endpoints $a,b$ means the right-hand and left-hand derivatives of [§Differentiation and Derivatives, ⁋Definition 6](/en/math/calculus/derivatives#def6){: data-relation="required" }.
:::
...
Since $f$ is continuous on $[x,x+h]$, by [§Continuous Functions, ⁋Theorem 4](/en/math/calculus/continuity#thm4){: data-relation="required" } it attains a minimum $m_h$ and a maximum $M_h$ on that interval, and by the third part of [§Integration, ⁋Proposition 11](/en/math/calculus/integration#prop11){: data-relation="required" } we have

$$m_h\le \frac{F(x+h)-F(x)}{h}\le M_h.$$

Now as $h\to 0$ the interval $[x,x+h]$ shrinks to the single point $x$, and by continuity of $f$ we have $m_h,M_h\to f(x)$; hence by [§Limits of Functions, ⁋Proposition 8](/en/math/calculus/functions_and_limits#prop8){: data-relation="required" } the average rate of change converges to $f(x)$. The case $h<0$ can be handled in a similar manner.
:::
...
The primitive of $e^{-t^2}$ cannot be written as an elementary function, so this integral has no closed form; however, by [Theorem 1](#thm1){: data-relation="required" } the function $\erf$ is differentiable and
...
A slightly more advanced version of this statement will be introduced again after we examine term-by-term integration of power series in [Proposition 7](#prop7){: data-relation="forward" }.
...
yields a negative number, but the integrand $1/x^2$ is always positive, so this is clearly a wrong calculation. The reason it fails is that the integrand diverges at $x=0$ and is therefore not continuous on $[-1,1]$. Since the hypothesis of [Theorem 4](#thm4){: data-relation="required" } is violated, one cannot apply the theorem as is; in fact, this integral diverges.
:::
...
Then we can see in what form the function $e^{-x^2}$ examined in [Example 3](#ex3){: data-relation="weak" } can be written.

::: Example 8
Returning to the error function of [Example 3](#ex3){: data-relation="required" }. From the power series of the exponential function,

$$e^{-t^2}=\sum_{n=0}^\infty \frac{(-1)^n}{n!}t^{2n},$$

whose radius of convergence is $\infty$. Now term-by-term integration via [Proposition 7](#prop7){: data-relation="required" } gives
