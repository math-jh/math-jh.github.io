...
Now as $h\to 0$ the interval $[x,x+h]$ shrinks to the single point $x$, and by continuity of $f$ we have $m_h,M_h\to f(x)$; hence by [§Limits of Functions, ⁋Proposition 8](/en/math/calculus/functions_and_limits#prop8){: data-lid="uw3oh" data-relation="required" } the average rate of change converges to $f(x)$. The case $h<0$ can be handled in a similar manner.
:::
...
The primitive of $e^{-t^2}$ cannot be written as an elementary function, so this integral has no closed form; however, by [Theorem 1](#thm1){: data-lid="adlov" data-relation="required" } the function $\erf$ is differentiable and
...
yields a negative number, but the integrand $1/x^2$ is always positive, so this is clearly a wrong calculation. The reason it fails is that the integrand diverges at $x=0$ and is therefore not continuous on $[-1,1]$. Since the hypothesis of [Theorem 4](#thm4){: data-lid="epnbq" data-relation="required" } is violated, one cannot apply the theorem as is; in fact, this integral diverges.
:::
...
whose radius of convergence is $\infty$. Now term-by-term integration via [Proposition 7](#prop7){: data-lid="z3dz8" data-relation="required" } gives
