...
so $dA = r dr d\theta$. This extra factor $r$ often unlocks integrals that were stuck in rectangular coordinates. For example, the double integral over the unit disk $D$
...
Let us find the value of the integral $I = \int_{-\infty}^{\infty} e^{-x^2} \mathop{dx}$. Regarding its square $I^2$ as an integral in two independent variables,
...
so $\mathop{dV} = r \mathop{dr} \mathop{d\theta} \mathop{dz}$.
...
so $\mathop{dV} = \rho^2\sin\phi \mathop{d\rho} \mathop{d\phi} \mathop{d\theta}$. For example, the volume of a ball of radius $R$ separates in spherical coordinates into
