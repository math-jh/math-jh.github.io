...
date: 2026-07-04
weight: 13
translated_at: 2026-08-17T23:17:52+00:00
translation_source: kimi-cli
---
So far, the only functions we have dealt with are $f:\mathbb{R}\rightarrow\mathbb{R}$ sending real numbers to real numbers; to be blunt, apart from the fact that the notion of limit was rigorously defined using $\epsilon$-$\delta$, there was little difference from what we learned in high school. We now generalize this and actually look at new material. The direction of generalization is to increase dimension, and there are two ways to do this: raising the dimension of the domain or raising the dimension of the codomain. In this post, we first treat the latter, namely functions $\mathbf{r}:\mathbb{R}\rightarrow\mathbb{R}^n$ sending one real number to several real numbers. These are vector-valued functions that trace a curve in space with a single parameter, and can be viewed as the locus of a point moving in time, so that differentiation acquires the physical meaning of velocity and acceleration, and integration measures the length of a curve.

## Vector Spaces

A curve is a function that takes one real number and returns a point consisting of several real numbers, that is, a function into $\mathbb{R}^n$, so we first briefly review the structure of this space. $\mathbb{R}^n$ is a *vector space*, and the elements of this space are called *vectors*. A vector is written as an ordered tuple
...
These operations are exactly the vector operations seen in the coordinate plane, lifted to $n$ dimensions, and the commutative, associative, and distributive laws hold naturally. In particular, the following vectors
...
Since the space we deal with is Euclidean space, we also use the inner product and norm together. The *inner product* of two vectors $\mathbf{v},\mathbf{w}\in\mathbb{R}^n$ is defined as the sum of coordinate-wise products,
...
In this post we mainly use the cross product as a tool to produce orthogonal vectors.
...
Intuitively, this is a rule by which a different point in vector space corresponds as $t$ varies, and can be thought of as expressing the locus of a point moving in time.

The limit of a vector-valued function is defined in the same way as the limit of a one-dimensional function. That is, whenever any $\epsilon>0$ is given, there always exists a suitable $\delta>0$ such that

$$0<\lvert t-t_0\rvert<\delta\implies \lVert \mathbf{r}(t)-\mathbf{v}\rVert<\epsilon$$

can be satisfied, we say this vector function $\mathbf{r}$ converges to the vector $\mathbf{v}$ as $t\rightarrow t_0$. Then by the Cauchy–Schwarz inequality
...
so $\mathbf{r}(t)\rightarrow\mathbf{v}$. That is, convergence of a vector function is the same as convergence of each component function, and in particular continuity and differentiation of a vector function are both well defined component-wise.
...
::: Proof
Writing everything out component-wise reduces to the product rule and chain rule for scalar functions. For example, the inner product is $\mathbf{u} \cdot \mathbf{v} = \sum_i u_i v_i$, so $(\mathbf{u} \cdot \mathbf{v})' = \sum_i (u_i' v_i + u_i v_i') = \mathbf{u}' \cdot \mathbf{v} + \mathbf{u} \cdot \mathbf{v}'$, and the cross product and scalar multiplication can be shown in the same way since each component is a sum of products of the form $u_i v_j$. The composition is applying [§Differentiation, ⁋Theorem 4](/en/math/calculus/differentiation_rules#thm4) to each component.
:::

One useful corollary of this proposition is that if $\lVert \mathbf{u}(t)\rVert$ is constant then $\mathbf{u} \cdot \mathbf{u} = \lVert \mathbf{u}\rVert^2$ is also constant, so $(\mathbf{u} \cdot \mathbf{u})' = 2 \mathbf{u} \cdot \mathbf{u}' = 0$, i.e., $\mathbf{u} \perp \mathbf{u}'$. That is, the rate of change of a vector of constant length is always perpendicular to that vector, and the fact that in circular motion the position vector and velocity are perpendicular is a special case of this fact.
...
Velocity points in the direction of the curve's progression and its magnitude is how fast the point is moving, and the unit tangent vector keeps only the direction of the velocity. For this to be well defined, the parametrized curve $\mathbf{r}$ must be a *$C^1$-regular curve*, which means a curve satisfying both of the following conditions.
...
Then in particular by the second condition the unit tangent vector $\mathbf{T}$ is well defined everywhere, and by the first condition $\mathbf{T}(t)$ is continuous. In this post, unless specially mentioned, we treat $C^1$-regular curves as the default.

When a moving object is given, one of the things we are curious about is the distance traveled over time. This is obtained, just as in one dimension, by integrating the speed, and rigorously it is shown by approximating the curve with a polygon connecting points on the curve, forming for a partition
...
and refining the partition to show that this converges to a Riemann sum of $\lVert \mathbf{r}'(t)\rVert$, which compresses to the following definition.
...
The integrand $\lVert \mathbf{r}'(t)\rVert$ is continuous so it is integrable ([§Integration, ⁋Theorem 10](/en/math/calculus/integration#thm10)). Now we call the arc length measured from the starting point
...
How much a curve bends is measured by how quickly the direction of the unit tangent vector changes as we proceed along the curve. However, if the curve moves quickly then the rate of change will also increase, so to examine how much the shape of the curve is bent we must normalize this rate. Also, intuitively this is obtained by differentiating the unit tangent vector obtained by differentiating the original curve once more, so we must now think of *second-order* curves. That is, the acceleration vector $\mathbf{r}''(t)$ exists continuously.
...
Note that by [Proposition 6](#prop6), $\mathbf{T} = d\mathbf{r}/\dd{s}$ is a unit vector. Then by the earlier observation, examined right after [Proposition 3](#prop3), that the rate of change of a vector of constant length is perpendicular to that vector, we have $d\mathbf{T}/\dd{s} \perp \mathbf{T}$. That is, the unit normal vector $\mathbf{N}$ is always perpendicular to the tangent, and points to the inside into which the curve is bending.

The above definition is for curves in general $n$-dimensional space, but in $3$-dimensional space the cross product allows us to compute this more conveniently. In particular, the following formula can be applied directly without arc length parametrization, so it is much more convenient.
...
giving the desired equality. The formula for a plane curve $y = f(x)$ follows by setting $\mathbf{r}(x) = (x, f(x),0)$ and applying the $3$-dimensional formula directly.
:::

Differentiating $\mathbf{r}' = v\mathbf{T}$ appearing in the proof once more gives $\mathbf{r}'' = v'\mathbf{T} + v\mathbf{T}'(t)$, and at points where $\kappa \neq 0$ we have $d\mathbf{T}/\dd{s} = \kappa\mathbf{N}$ so $\mathbf{T}'(t) = \kappa v\mathbf{N}$, and therefore there $\mathbf{r}'' = v'\mathbf{T} + \kappa v^2\mathbf{N}$. This equation is meaningful in itself. The acceleration splits into a tangential directional component and a normal directional component. Let us record this as a separate proposition.
