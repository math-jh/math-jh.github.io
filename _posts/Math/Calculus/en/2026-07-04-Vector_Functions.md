...
and $\lVert \mathbf{r}'\rVert = \sqrt{2}$, so the curvature is $\kappa = \sqrt{2}/(\sqrt{2})^3 = 1/2$, which is constant. Since the speed $v = \sqrt{2}$ is constant, by [Proposition 9](#prop9){: data-lid="oh44e" } the acceleration is purely normal and its magnitude is $\kappa v^2 = (1/2)\cdot 2 = 1$, which indeed matches $\lVert \mathbf{r}''(t)\rVert = \lVert(-\cos t, -\sin t, 0)\rVert = 1$.
:::
