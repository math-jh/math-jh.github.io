...
::: Proof
Writing everything out component-wise reduces each identity to the product rule and chain rule for scalar functions. For example, the inner product is $\mathbf{u} \cdot \mathbf{v} = \sum_i u_i v_i$, so $(\mathbf{u} \cdot \mathbf{v})' = \sum_i (u_i' v_i + u_i v_i') = \mathbf{u}' \cdot \mathbf{v} + \mathbf{u} \cdot \mathbf{v}'$; the cross product and scalar multiplication can be shown in the same way since each component is a sum of products of the form $u_i v_j$. The composition rule follows by applying [§Differentiation, ⁋Theorem 4](/en/math/calculus/differentiation_rules#thm4){: data-lid="odgwr" data-relation="required" reviewed="" } to each component.
:::
...
and $\lVert \mathbf{r}'\rVert = \sqrt{2}$, so the curvature is $\kappa = \sqrt{2}/(\sqrt{2})^3 = 1/2$, which is constant. Since the speed $v = \sqrt{2}$ is constant, by [Proposition 9](#prop9){: data-lid="oh44e" data-relation="required" reviewed="" } the acceleration is purely normal and its magnitude is $\kappa v^2 = (1/2)\cdot 2 = 1$, which indeed matches $\lVert \mathbf{r}''(t)\rVert = \lVert(-\cos t, -\sin t, 0)\rVert = 1$.
:::
