...
::: 정의 11
임의의 원소가 가역인 monoid를 *group<sub>군</sub>*이라 부른다. 만일 $\star$가 교환법칙을 만족한다면, 이를 *abelian group<sub>아벨군</sub>* (혹은 *commutative group<sub>가환군</sub>*)이라 부른다. 
:::
...
가장 작은 회전을 $r$, reflection 중 하나를 $s$라 하면 $r$은 $n$번 적용했을 때 항등원으로 돌아오며, $s$는 두 번 적용했을 때 항등원으로 돌아온다. 이러한 양의 정수 중 가장 작은 것을 각각 $r$과 $s$의 *order<sub>위수</sub>*라 부른다. 또한 dihedral gropu에서는  "회전한 뒤 뒤집기"와 "뒤집은 뒤 거꾸로 회전하기"가 같은 대칭이므로

$$srs^{-1}=r^{-1}$$

이 성립한다. 임의의 대칭은 $r^k$ 또는 $r^ks$ ($0\le k<n$)의 꼴로 유일하게 표현되므로 $D_n$의 모든 원소는 $r$과 $s$를 거듭 연산하여 얻어지며, 따라서 $\langle r,s\rangle=D_n$이다.

앞서 group $G$의 부분집합 $S$에 대하여 $S$를 포함하는 가장 작은 subgroup을 $\langle S\rangle$로 표기하였다. 만일 $\langle S\rangle=G$가 성립한다면 $S$가 $G$를 *생성한다<sub>generate</sub>*고 하고, 이 때 $S$의 각 원소를 $G$의 *생성원<sub>generator</sub>*이라 부른다. 특히 유한집합으로 생성되는 group을 *finitely generated<sub>유한생성</sub>*라 하는데, 위의 dihedral group은 두 원소 $r,s$로 생성되는 그 전형적인 예이다.

생성원의 집합만으로는 group이 결정되지 않으며, 생성원들 사이에 성립하는 등식들 — 각각을 *relation<sub>관계식</sub>*이라 한다 — 을 함께 지정해야 비로소 group이 정해진다. 이렇게 생성원과 relation의 목록으로 group을 기술하는 방식을 *presentation<sub>표시</sub>*이라 하며, 가령 dihedral group은

$$D_n=\langle r,s\mid r^n=s^2=e,\ srs^{-1}=r^{-1}\rangle$$

과 같이 적는다. 세로줄을 기준으로 왼쪽은 생성원, 오른쪽은 relation이며, 이 기호는 주어진 생성원으로 생성되면서 명시된 relation들이 성립하는 group 중 가장 일반적인 것을 뜻한다.
