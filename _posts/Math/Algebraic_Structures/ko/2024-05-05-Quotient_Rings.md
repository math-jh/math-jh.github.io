...
이번 글에서 우리는 quotient ring의 개념을 정의한다. [§몫군](/ko/math/algebraic_structures/quotient_groups){: data-relation="required" }을 정의할 때를 떠올려보면, group $G$의 임의의 subgroup $H$에 대해 $G/H$는 집합으로서는 항상 정의되지만 이것이 항상 group의 구조를 갖는 것은 아니었고, 이를 위해서는 $H$가 normal subgroup이라는 조건이 필요했다. 마찬가지로 ring $A$에 대해서도 quotient를 정의할 수 있는 방식은 제한되어 있다.
...
:::
::: 증명
1. $\pi$가 덧셈에 대해 abelian group homomorphism을 정의한다는 것은 [§몫군](/ko/math/algebraic_structures/quotient_groups){: data-relation="required" }의 결과이다. $\pi$가 곱셈을 보존하는 것은 다음 계산
...
:::
::: 증명
1번과 3번은 [§군 동형사상](/ko/math/algebraic_structures/isomorphism_theorems){: data-relation="required" }에서 다루었던 것과 거의 동일하게 진행하되, 거기서 얻어지는 group homomorphism이 실제로 ring homomorphism 또한 된다는 것을 [명제 2](#prop2){: data-relation="required" }의 2번과 같은 방식으로 확인하면 된다.
...
4번의 두 대응 $\bar{\mathfrak{b}}\mapsto\pi^{-1}(\bar{\mathfrak{b}})$와 $\mathfrak{b}\mapsto\pi(\mathfrak{b})$가 서로의 역이며 포함관계를 보존한다는 것은 [§군 동형사상, ⁋정리 7](/ko/math/algebraic_structures/isomorphism_theorems#thm7){: data-relation="required" }에 따른 것이다. 남는 것은 이 대응이 two-sided ideal을 two-sided ideal로 보낸다는 것이다. 우선 임의의 $\alpha\in A$와 $x\in\pi^{-1}(\bar{\mathfrak{b}})$에 대하여 ,
