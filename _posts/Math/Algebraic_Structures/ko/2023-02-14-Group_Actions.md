...
Monoidal category $(\Set,\times, I)$ 위의 monoid object $(M,\cdot,1)$을 고정하자. 그럼 이를 통해 임의의 집합 $E$ 위에 정의된 $M$의 left action을 생각할 수 있다. 그럼 $M\times E\cong E\times M$으로 성분의 순서를 바꾼 뒤 [\[집합론\] §집합의 곱, ⁋명제 4](/ko/math/set_theory/product_of_sets#prop4){: data-relation="required" reviewed="" }를 적용하면

$$\Hom_\Set(M\times E,E)\cong\Hom_\Set(M,\Hom_\Set(E,E))\cong\Hom_\Set(M, \End(E))$$

이므로, 임의의 left action은 함수 $M \rightarrow \End(E)$를 정의한다. 그럼 [정의 1](#def1){: data-relation="required" reviewed="" }의 두 diagram이 commutative라는 것은 이 함수가 실은 monoid homomorphism이라는 것과 같다. 
...
::: 정의 10
Group $G$가 주어졌다 하자. [명제 9](#prop9){: data-relation="required" reviewed="" }의 automorphism $\rho_g$를 $g$에 의해 정의되는 *inner automorphism<sub>내부자기동형사상</sub>*이라 부르고, 이들의 모임을 $\Inn(G)$로 적는다.
:::
...
한편, $\rho:G\rightarrow\Inn(G)$는 전사이며, 따라서 [§군 동형사상, ⁋정리 2](/ko/math/algebraic_structures/isomorphism_theorems#thm2){: data-relation="required" reviewed="" }에 의하여
...
::: 정의 12
Group $G$와, [명제 9](#prop9){: data-relation="required" reviewed="" }에서 정의한 group homomorphism $\rho:G\rightarrow\Inn(G)$에 대하여, $\ker\rho$를 $G$의 *center<sub>중심</sub>*이라 부르고 $Z(G)$로 표기한다.
:::
...
:::
::: 증명
함수 $p:G \rightarrow G\cdot x$를 $g\mapsto g\cdot x$로 정의하면 $G\cdot x$의 정의에 의해 이 함수는 surjective이다. 한편, $p(g_1)=p(g_2)\iff g_1^{-1}g_2\in \Stab(x)$이므로 [\[집합론\] §동치관계의 예시들, ⁋명제 7](/ko/math/set_theory/examples_of_equivalence#prop7){: data-relation="required" reviewed="" } 이후의 canonical decomposition으로부터 원하는 결과를 얻는다. 
:::

따라서, 만일 $G$가 finite이라면 [§몫군, ⁋명제 5](/ko/math/algebraic_structures/quotient_groups#prop5){: data-relation="required" reviewed="" }에 의하여 다음의 식
