...
비슷하게 $I$-graded right $A$-module도 또한 정의한다. 특별히 $A$를 $A$ 자기 자신에 대한 left $A$-module로 본다면, [정의 1](#def1){: data-lid="n5oyz" data-relation="required" reviewed="" }에 의해 모든 graded ring은 자기 자신에 대한 graded (left) $A$-module이다. 만일 $I$의 덧셈에 대하여, 모든 원소가 cancellable이라면 [§등급환, ⁋명제 2](/ko/math/algebraic_structures/graded_rings#prop2){: data-lid="kdbrw" data-relation="required" reviewed="" }에 의하여 $A_0$은 ring이다. 그럼 위의 식으로부터 각각의 $M_j$들이 $A_0$-module이 되는 것이 자명하다. 
...
그럼 [정의 2](#def2){: data-lid="zyg2l" data-relation="required" }의 graded homomorphism들은 모두 graded homomorphism of degree $0$에 불과하다. 만일 $I$의 모든 원소들이 cancellable이라면, 우리는 *graded homomorphism of degree $-i$*를 다음 조건
...
이 명제는 [§등급환, ⁋명제 6](/ko/math/algebraic_structures/graded_rings#prop6){: data-lid="h434k" data-relation="required" reviewed="" }의 일반화이며, 그 증명 또한 동일하다. 이 동치조건을 만족하는 submodule들을 *graded submodule<sub>등급부분가군</sub>*이라 부른다. 한편 graded submodule $N$에 대하여 [§등급환, ⁋명제 7](/ko/math/algebraic_structures/graded_rings#prop7){: data-lid="3cywy" data-relation="required" reviewed="" }의 증명이 그대로 옮겨가, quotient module $M/N$은 다음의 decomposition
...
:::
::: 증명
1번의 경우, $M=\bigoplus_j M_j$이므로 $\im(u)$는 homogeneous element $u(x_j)\in N_{d+j}$들로 생성되고, 따라서 [명제 4](#prop4){: data-lid="36rut" data-relation="required" reviewed="" }의 셋째 조건에 의해 graded submodule이다.

2번의 경우, $x=\sum_j x_j\in\ker(u)$라 하면 $0=u(x)=\sum_j u(x_j)$이고 각 $u(x_j)$는 $N_{d+j}$에 속한다. $d$가 cancellable이므로 $j\mapsto d+j$는 단사이고, 따라서 이 합의 항들이 서로 다른 degree에 놓여 성분별로 $u(x_j)=0$을 얻는다. 즉 각 $x_j$가 $\ker(u)$에 속하므로 [명제 4](#prop4){: data-lid="g4erp" data-relation="required" reviewed="" }의 둘째 조건이 성립한다.
