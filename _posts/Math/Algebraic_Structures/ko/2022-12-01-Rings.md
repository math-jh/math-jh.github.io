...
정의에 의하여 ring homomorphism $$\phi:A\rightarrow B$$은 두 abelian group $$(A,+,0)$$, $$(B,+,0)$$ 사이의 group homomorphism이기도 하다. 그럼 ring homomorphism $$\phi$$의 kernel $$\ker f$$는 이 group homomorphism의 kernel로 정의된다. 즉 $$\ker \phi=\phi^{-1}(0)$$이다. 그럼 [§준동형사상, ⁋명제 3](/ko/math/algebraic_structures/group_homomorphisms#prop3)에 의하여 $$\phi$$가 단사인 것은 $$\ker \phi=\{0\}$$인 것과 동치이다. 
