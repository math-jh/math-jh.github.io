...
<ins id="def1">**정의 1**</ins> 집합 $A$가 *ring<sub>환</sub>*이라는 것은 두 이항연산 $+,\cdot$과 원소 $0$이 존재하여, 다음 조건들이 성립하는 것이다.
...
<ins id="def4">**정의 4**</ins> Ring $(A,+,-,\cdot,0)$의 부분집합 $S$가 *subring<sub>부분환</sub>*이라는 것은 $(S,+,-,\cdot,0)$가 ring의 구조를 갖는 것이다. 만일 $A$가 $1$을 갖는다면, 마찬가지로 $1\in S$를 만족하는 것만 $A$의 subring인 것으로 생각한다.

</div>

[§준동형사상, ⁋명제 6](/ko/math/algebraic_structures/group_homomorphisms#prop6)과 마찬가지로 다음이 성립한다.

<div class="proposition" markdown="1">

<ins id="prop5">**명제 5**</ins> 임의의 ring homomorphism $f:A \rightarrow B$에 대하여, $\ker f$는 $A$의 subring이다.
...
위의 증명을 잘 살펴보면, 임의의 $x\in\ker f$와 임의의 $y\in A$에 대하여 $xy\in\ker f$임을 알 수 있다. ([명제 2](#prop2)) 이를 다음과 같이 정의한다.
