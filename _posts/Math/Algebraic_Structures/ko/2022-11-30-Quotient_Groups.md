...
앞서 [§대수적 구조, §§몫구조](/ko/math/algebraic_structures/algebraic_structures#몫구조){: data-lid="irc4h" data-relation="required" reviewed="" }에서 우리는 동치관계 $R$이 마그마 $A$의 연산과 compatible할 경우 그 quotient set $A/R$ 위에 자연스러운 방식으로 마그마 구조를 줄 수 있다는 것을 증명하였으며, 뿐만 아니라 [§반군, 모노이드, 군](/ko/math/algebraic_structures/groups){: data-lid="x680l" data-relation="required" reviewed="" }의 말미에서 우리는 $A$가 group일 경우, 이 방식으로 만들어진 마그마 $A/R$ 또한 group이 된다는 것을 살펴보았다. 이 때 group $A/R$을 *quotient group<sub>몫군</sub>*이라 부른다.

## 정규부분군

한편 [\[집합론\] §동치관계](/ko/math/set_theory/equivalence_relations){: data-lid="4mlmd" data-relation="required" reviewed="" }을 통해 우리는 다음 두 가지가 같다는 것을 안다.
...
:::
::: 증명
$a,b\in [e]$라 하자. 즉 $a\sim e\sim b$이다. 이제 $R$은 $G$의 연산과 compatible하므로, $a\sim b$의 양 변의 오른쪽에 $b^{-1}$을 곱하여 $ab^{-1}\sim e$를 얻는다. 즉 $ab^{-1}\in[e]$이므로 [§반군, 모노이드, 군, ⁋명제 15](/ko/math/algebraic_structures/groups#prop15){: data-lid="i4ig9" data-relation="required" reviewed="" }에 의하여 $[e]$는 subgroup인 것을 안다.
:::
...
[명제 1](#prop1){: data-lid="svcy9" data-relation="required" reviewed="" }에서, 임의의 $a\in [e]$에 대하여 다음 식
...
이제 group $G$와, 임의의 subgroup $H$를 생각하자. $H$가 normal이 아니더라도 위의 논의에서 얻어낸 $\sim_{\tiny r}$과 $\sim_{\tiny l}$은 어쨌든 동치관계이므로, quotient set $G/{\sim_{\tiny r}}$과 $G/{\sim_{\tiny l}}$이 어떻게 생겼는지를 살펴볼 수 있다. 
...
따라서 $G$의 임의의 subgroup $H$가 주어졌을 때, 두 동치관계 $\sim_{\tiny r}$과 $\sim_{\tiny l}$은 $G$를 각각 right coset들과 left coset들로 분할한다. 이 경우 $\sim_{\tiny r}$에 의한 $G$의 quotient set은 $H\setminus G$, 그리고 $\sim_{\tiny l}$에 의한 $G$의 quotient set은 $G/H$로 적는다.[^1] 일반적으로 $Ha\neq aH$이지만, 임의의 $a\in G$에 대하여 $Ha=aH$가 성립할 필요충분조건은 $H$가 normal이라는 것을 쉽게 확인할 수 있다.
