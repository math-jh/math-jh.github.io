...
위에서 정의한 세 functor들 사이에는 특정한 adjoint 관계들이 있다. ([\[범주론\] §수반함자, ⁋정의 1](/ko/math/category_theory/adjoints#def1){: data-lid="ssyat" data-relation="required" reviewed="" }) 우선 다음 보조정리를 보이자.
...
::: 명제 6
Adjoint $\phi_!\dashv\phi^\ast$가 존재한다. 
:::
::: 증명
...
이므로 $\tilde{u}(y)\in\phi_\ast M$이고, $\tilde{u}(\beta'\cdot_By)(\beta)=u((\beta\beta')\cdot_By)=\tilde{u}(y)(\beta\beta')$이 [정의 4](#def4){: data-lid="whytw" data-relation="required" reviewed="" } 직전에 정의한 $B$-action에 의해 $(\beta'\cdot_B\tilde{u}(y))(\beta)$이므로 $\tilde{u}\in\Hom_B(N,\phi_\ast M)$이다.
