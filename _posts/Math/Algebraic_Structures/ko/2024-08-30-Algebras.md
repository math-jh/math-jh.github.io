...
우선 첫 번째 문제는 일반적인 ring $A$에 대하여 $\lMod{A}$ 혹은 $\rMod{A}$는 monoidal category가 아니라는 것이다. 이는 만일 $A$가 commutative ring이라면 $(\lMod{A},\otimes_A, A)$가 symmetric monoidal category가 되므로, $A$가 commutative ring임을 가정하면 해결된다. ([§환의 곱, 쌍대곱, 텐서곱, §§환들의 텐서곱](/ko/math/algebraic_structures/operations_of_rings#환들의-텐서곱)) 
