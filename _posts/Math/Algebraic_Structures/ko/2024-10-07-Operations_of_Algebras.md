...
우리는 [§가군의 직접곱과 직합, 텐서곱](/ko/math/algebraic_structures/operations_of_modules){: data-lid="tio4h" data-relation="required" reviewed="" }에서 module들의 연산을, [§환의 곱, 쌍대곱, 텐서곱](/ko/math/algebraic_structures/operations_of_rings){: data-lid="q2gy0" data-relation="weak" reviewed="" }에서 ring들의 연산을 살펴보았다. $A$-algebra는 $A$-module 위에 bilinear한 곱셈을 추가한 구조이므로 ([§대수, ⁋정의 1](/ko/math/algebraic_structures/algebras#def1){: data-lid="0jrku" data-relation="required" reviewed="" }), module의 단계에서 정의된 연산들 위에 곱셈이 잘 따라오는지를 확인하는 것이 이번 글의 내용이다. [§대수](/ko/math/algebraic_structures/algebras){: data-lid="h8739" data-relation="weak" reviewed="" }에서와 마찬가지로 $A$는 항상 commutative ring이다.
...
::: 정의 2
[명제 1](#prop1){: data-lid="nt90p" data-relation="required" reviewed="" }에서 정의된 곱셈이 주어진 $A$-algebra $\prod_{i\in I}E_i$를 $E_i$들의 *direct product<sub>직접곱</sub>*라 부른다. Canonical projection $\pr_i:\prod E_i \rightarrow E_i$들은 모두 $A$-algebra homomorphism이다.
:::
...
:::
::: 증명
$A$-module의 단계에서 product의 universal property에 의하여 ([§가군의 직접곱과 직합, 텐서곱, ⁋정리 1](/ko/math/algebraic_structures/operations_of_modules#thm1){: data-lid="rr8gd" data-relation="required" reviewed="" }) 조건을 만족하는 유일한 $A$-linear map $u:F \rightarrow\prod E_i$, 즉 $u(x)=(u_i(x))_{i\in I}$가 존재한다. 이것이 곱셈을 보존하는 것은 각각의 $u_i$가 곱셈을 보존한다는 것으로부터
...
주의할 것은 이것이 $A$-algebra의 category에서의 coproduct가 <em-ko>아니라는</em-ko> 것이다. 우선 [§대수, ⁋정의 1](/ko/math/algebraic_structures/algebras#def1){: data-lid="rs8ki" data-relation="weak" reviewed="" }의 일반적인 의미에서 보더라도 canonical injection $\iota_j:E_j\hookrightarrow\bigoplus E_i$는 덧셈과 스칼라곱, 그리고 곱셈을 모두 보존하지만, 이들 데이터가 universal property를 만족하지는 않는다. 가령 $E_1=E_2=A$이고, 
...
Commutative associative unital $A$-algebra들의 category에서 올바른 coproduct의 개념을 주는 것은 tensor product이다. 기본적으로 이는 $A$-module $E\otimes_AE'$ ([§가군의 직접곱과 직합, 텐서곱, ⁋명제 8](/ko/math/algebraic_structures/operations_of_modules#prop8){: data-lid="9h7dd" data-relation="required" reviewed="" }) 위에 곱셈을 적당히 정의하여 얻어지는 $A$-algebra로, 우리가 원하는 곱셈은 다음의 식
...
은 $E,E'$의 곱셈이 각 변수에 대해 $A$-linear이므로 $A$-bilinear이고, 따라서 [§가군의 직접곱과 직합, 텐서곱, ⁋명제 8](/ko/math/algebraic_structures/operations_of_modules#prop8){: data-lid="8vyg4" data-relation="required" reviewed="" }의 universal property에 의하여 $x\otimes x'\mapsto xy\otimes x'y'$이도록 하는 유일한 $A$-linear map $m_{(y,y')}:E\otimes_AE' \rightarrow E\otimes_AE'$을 유도한다.
...
::: 정의 7
두 $A$-algebra $E,E'$에 대하여, [명제 6](#prop6){: data-lid="ip2ub" data-relation="required" reviewed="" }의 곱셈이 주어진 $A$-algebra $E\otimes_AE'$을 $E$와 $E'$의 *tensor product<sub>텐서곱</sub>*라 부른다.
:::
...
이 성립한다. 이는 [§대수, ⁋명제 8](/ko/math/algebraic_structures/algebras#prop8){: data-lid="vkmfg" data-relation="required" reviewed="" }에서 살펴본 functor $A[-]:\Set \rightarrow \cAlg{A}$가 left adjoint라는 사실로부터 나온다. Left adjoint는 colimit을 보존하므로, 한 점 집합들의 (집합에서의) coproduct $\{\x\}\sqcup\{\y\}=\{\x,\y\}$를 $\cAlg{A}$에서의 coproduct로 보내고, [정리 8](#thm8){: data-lid="his3j" data-relation="required" reviewed="" }에 의하여 이는 정확히 tensor product이기 때문이다. 물론 위 isomorphism이 두 식
