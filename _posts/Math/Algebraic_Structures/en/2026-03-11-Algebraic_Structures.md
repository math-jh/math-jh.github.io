...
Therefore, there exists a cartesian monoidal category $\Magma$ whose objects are magmas and whose morphisms are magma homomorphisms. ([\[Category Theory\] §Monoidal Categories, ⁋Definition 3](/en/math/category_theory/monoidal_categories#def3){: data-lid="f5u1i" })
...
If $R$ is an equivalence relation, we have already seen in set theory that as a set the *quotient set* $A/R$ is well-defined. ([\[Set Theory\] §Equivalence Relations, ⁋Definition 4](/en/math/set_theory/equivalence_relations#def4){: data-lid="wr8kj" }) On the set $A/R$, the most natural attempt to define an operation $\tiny\char"2606$ is
