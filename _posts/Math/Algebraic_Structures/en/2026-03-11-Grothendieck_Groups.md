...
In the discussion above, we obtained $K(S)$ by adding inverses of all elements of $S$. Meanwhile, looking at [Definition 6](#def6){: data-relation="weak" }, what we actually do is add inverses only for the elements of the subset $\mathbb{N}\setminus\{0\}$ of $\mathbb{N}$. This can also be obtained by slightly modifying the discussions above; we omit the proofs and examine only the process.
