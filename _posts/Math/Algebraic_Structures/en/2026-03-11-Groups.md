...
description: "This post covers the definitions of semigroups and monoids, and presents the concepts of identity elements and monoid homomorphisms from a categorical perspective."
...
date: 2022-11-29
weight: 2
translated_at: 2026-08-15T04:49:19+00:00
...
::: Definition 2
For any magma $(A,\star)$, if some $e\in A$ satisfies
...
Any magma $A$ has at most one identity element. Indeed, if both $e$ and $e'$ are identity elements of $A$, then

$$e=e\star e'=e'\star e=e'.$$

On the other hand, an arbitrary element of a set $A$ is the same thing as a function from the singleton $\ast$ to $A$. ([\[Category Theory\] §Representable Functors, ⁋Example 2](/en/math/category_theory/representable_functors#ex2)) From this point of view, the fact that $e:\ast\rightarrow A$ is an identity element means that the following diagram
...
::: Definition 6
A *submonoid* of a monoid $(A,\star, e)$ is a submagma of $A$ that contains the identity element $e$.
:::
...
::: Definition 8
For a monoid $(A,\star,e)$, an element $x$ is *left cancellable* if for any $a,b\in A$, $x\star a=x\star b$ implies $a=b$. Similarly, we can define an element to be *right cancellable*. Also, $x$ is a *left inverse* of $y$ if $x\star y=e$ holds. Similarly, we can define $x$ to be a *right inverse* of $y$.
...
Taking the inverse is a function from $G$ to itself[^1], and hence a group $G$ is determined by the data $(G,\star,e, (-)^{-1})$. The inverse $(-)^{-1}$ can be expressed by the following diagram
...
and operating on the right by the inverse of $f(e)$ on both sides yields $e'=f(e)$. Thus by the preceding argument, $\Grp$ is also a full subcategory of $\Magma$.
...
:::
::: Proof
Operate on the left or right by the inverse of $a$ on both sides.
:::
...
::: Example 16
The symmetries of the plane taking a regular $n$-gon ($n\ge3$) to itself form a group under composition, called the *dihedral group* $D_n$. Such symmetries consist of $n$ rotations about the center by integer multiples of $2\pi/n$ and $n$ reflections about $n$ axes of symmetry, so $\lvert D_n\rvert=2n$.
:::
