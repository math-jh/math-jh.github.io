...
There are certain adjoint relations between the three functors defined above. ([\[Category Theory\] §Adjoint Functors, ⁋Definition 1](/en/math/category_theory/adjoints#def1){: data-relation="required" reviewed="" }) First, we prove the following lemma.
...
so $\tilde{u}(y)\in\phi_\ast M$, and since $\tilde{u}(\beta'\cdot_By)(\beta)=u((\beta\beta')\cdot_By)=\tilde{u}(y)(\beta\beta')$ equals $(\beta'\cdot_B\tilde{u}(y))(\beta)$ by the $B$-action defined just before [Definition 4](#def4){: data-relation="required" reviewed="" }, we have $\tilde{u}\in\Hom_B(N,\phi_\ast M)$.
