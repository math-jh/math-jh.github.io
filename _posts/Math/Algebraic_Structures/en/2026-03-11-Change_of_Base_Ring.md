...
description: "We define two functors, restriction of scalars and extension of scalars, that change a module over one ring into a module over another ring via a ring homomorphism, and examine their properties."
...
last_polished_at: 2026-09-09T23:15:05+00:00
translation_polish_source: antigravity-gemini-3.8-flash-high
---
In this post, we examine how to turn an $A$-module into a $B$-module, or a $B$-module into an $A$-module, via a ring homomorphism $\phi:A \rightarrow B$. Therefore, because abbreviating scalar multiplication and operations as before leaves room for confusion, for multiplication maps we omit $\cdot$ as before, while denoting actions by $\cdot$ (or $\cdot_A$ and $\cdot_B$).
...
we know that associating $A$-modules in this way is functorial.

::: Definition 1
For a ring homomorphism $\phi:A \rightarrow B$, the functor defined in the above manner is written as $\phi^\ast: \lMod{B} \rightarrow \lMod{A}$, and is called *restriction of scalar*.
:::

In other words, given an arbitrary $B$-module $\rho_N: B\otimes N \rightarrow N$, this simply defines an action of $A$ on $N$ using it via the formula

$$\alpha\cdot_A y:=\phi(\alpha)\cdot_B y$$

The same construction applies to right modules as well, and in this case we also view $\phi^\ast$ as $\rMod{B} \rightarrow\rMod{A}$.

Let us consider the special case $N=B$. Since $\phi^\ast B$ and $B$ coincide as sets, we can examine the relationship between the original ring homomorphism $\phi:A \rightarrow B$ and the action on $\phi^\ast B$; in this case, one can check that $\phi$ becomes an $A$-linear map. Also, since $B$ is simultaneously a left $B$-module and a right $B$-module over itself, $\phi^\ast B$ becomes a $(B,A)$-bimodule where $B$ acts by multiplication on the left and $A$ acts on the right via $\beta\cdot_A\alpha=\beta\phi(\alpha)$. Henceforth, when we use $\phi^\ast B$ as the left factor of a tensor product, we mean this right $A$-structure.
...
Consider the tensor product $\phi^\ast B\otimes_AM$ of the two $A$-modules $\phi^\ast B$ and $M$. Then on this, through the following formula

$$\beta'\cdot_B(\beta\otimes_A x)=(\beta'\beta)\otimes_A x$$

we can define an action $\cdot_B$ of $B$. That this is indeed an action is not difficult to obtain even by a direct computation; or, from the composition
...
it can also be thought of as being obtained.[^1] Also, for any $A$-linear map $u:M \rightarrow M'$, one can check that $\id_{\phi^\ast B}\otimes_A u$ defines a $B$-linear map between the two $B$-modules defined in this way.
...
As before, fix an $A$-module $M$. This time, we consider homomorphisms between the two $A$-modules $\phi^\ast B$ and $M$. On the following abelian group

$$\Hom_A(\phi^\ast B,M)$$

let us define a $B$-module structure by the following:

$$\beta\cdot g: (\beta'\mapsto g(\beta'\beta))$$

Since for any $\alpha\in A$ and any $\beta'\in \phi^\ast B$,
...
$\beta\cdot g$ is also an $A$-linear map. A short computation verifies that this is also functorial, and thus the following is defined.
...
There are certain adjoint relations between the three functors defined above. ([\[Category Theory\] §Adjoint Functors, ⁋Definition 1](/en/math/category_theory/adjoints#def1){: data-lid="ssyat" data-relation="required" reviewed="" }) First, we prove the following lemma.

::: Lemma 5
For a right $B$-module $N_1$ and a left $B$-module $N_2$, consider the two abelian groups $\phi^\ast N_1\otimes_A \phi^\ast N_2$ and $N_1\otimes_B N_2$. Then there exists a unique homomorphism $\Phi:\phi^\ast N_1\otimes_A \phi^\ast N_2 \rightarrow N_1\otimes_BN_2$ such that any $y_1\otimes_A y_2\in \phi^\ast N_1\otimes_A\phi^\ast N_2$ is sent to $y_1\otimes_B y_2\in N_1\otimes_BN_2$.
...
:::
::: Proof
Fix an arbitrary $A$-module $M$ and a $B$-module $N$. First, for any $v\in\Hom_B(\phi_!M,N)$, via the composition of functions

{% diagram Math/Algebraic_Structures/Change_of_Base_Ring-3.svg width="22.24em" alt="Adjointness-1" %}

we obtain a map $M \rightarrow N$. Here $M \rightarrow A\otimes_AM \rightarrow \phi^\ast B\otimes_AM$ is a composition of $A$-linear maps, and $v:\phi^\ast B\otimes_A M \rightarrow N$ is a $B$-linear map. First, looking at the composition of the former $A$-linear maps for arbitrary $\alpha\in A$ and $x\in M$, we have
...
Conversely, suppose that an arbitrary $u\in\Hom_A(M, \phi^\ast N)$ is given. Then this time, through the following composition

{% diagram Math/Algebraic_Structures/Change_of_Base_Ring-4.svg width="30.40em" alt="Adjointness-2" %}

we obtain a map $\phi_!M \rightarrow N$. Then for arbitrary $\beta'\in B$ and $\beta\otimes_A x\in \phi^\ast B\otimes_AM$,

$$\Phi((\id_{\phi^\ast B}\otimes_A u)(\beta'\cdot_B(\beta\otimes_Ax)))=\Phi((\beta'\beta)\otimes_Au(x))=(\beta'\beta)\otimes_B u(x)$$

and via $B\otimes_BN\cong N$ this is sent to $(\beta'\beta)\cdot_Bu(x)=\beta'\cdot_B(\beta\cdot_Bu(x))$. That is, the map defined above is a $B$-linear map.

Now one can verify that the two maps defined above are inverses of each other, and moreover that they define a natural equivalence.
:::
...
so $\tilde{u}(y)\in\phi_\ast M$, and since $\tilde{u}(\beta'\cdot_By)(\beta)=u((\beta\beta')\cdot_By)=\tilde{u}(y)(\beta\beta')$ equals $(\beta'\cdot_B\tilde{u}(y))(\beta)$ by the $B$-action defined just before [Definition 4](#def4){: data-lid="whytw" data-relation="required" reviewed="" }, we have $\tilde{u}\in\Hom_B(N,\phi_\ast M)$.
...
[^1]: Strictly speaking, for the first isomorphism in this formula to make sense, one must use the fact that $$B$$ is a $$(\mathbb{Z},A)$$-bimodule.
