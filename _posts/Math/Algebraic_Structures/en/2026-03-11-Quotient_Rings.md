...
:::
::: Proof
1. That $\pi$ defines an abelian group homomorphism with respect to addition is a result of [§Quotient Groups](/en/math/algebraic_structures/quotient_groups){: data-relation="required" reviewed="" }. That $\pi$ preserves multiplication is obtained from the following calculation
...
That the two correspondences $\bar{\mathfrak{b}}\mapsto\pi^{-1}(\bar{\mathfrak{b}})$ and $\mathfrak{b}\mapsto\pi(\mathfrak{b})$ in 4 are inverses of each other and preserve inclusion follows from [§Group Isomorphisms, ⁋Theorem 7](/en/math/algebraic_structures/isomorphism_theorems#thm7){: data-relation="required" reviewed="" }. What remains is that this correspondence sends two-sided ideals to two-sided ideals. First, for any $\alpha\in A$ and $x\in\pi^{-1}(\bar{\mathfrak{b}})$,
