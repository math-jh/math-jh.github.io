...
date: 2022-12-01
weight: 101
translated_at: 2026-06-26T08:00:02+00:00
translation_source: kimi-cli
last_polished_at: 2026-06-26T08:00:02+00:00
---
## Ring
...
hold. Moreover, $\mu$ satisfies the associativity law, and
...
::: Definition 3
For two rings $A,B$, a function $\phi:A \rightarrow B$ is called a *ring homomorphism* if for any $\alpha,\beta$
...
::: Proposition 6
For any ring homomorphism $\phi:A \rightarrow B$, $\ker \phi$ is a subring of $A$.
:::
::: Proof
...
Subrings and ideals are not the same notion in general. First, a subring contains $1$ by definition, so if a subring is also an ideal, then by what we just observed it must be all of $A$. Even if we weaken the notion to $\Rng$ so that a subring need not contain $1$, the two notions still differ, since the requirement that an ideal be closed under multiplication by arbitrary elements of $A$ is not part of the subring condition. It is therefore natural to ask for an example of a subring that is not an ideal, and the most important one is the center of a ring.

::: Definition 8
The *center* $Z(A)$ of a ring $A$ is the set of elements that commute with every element of $A$, i.e.,

$$Z(A)=\{z\in A\mid za=az\text{ for all }a\in A\}$$

:::

$Z(A)$ is closed under addition and multiplication and contains $1$, so it is a subring of $A$, and it is commutative by definition. However, $Z(A)$ is not an ideal in general. To see whether $ax$ still lies in the center for $x\in Z(A)$ and $a\in A$, note that for any $y\in A$

$$(ax)y=a(xy)=a(yx)$$

so the central element $x$ commutes with $y$ but $a$ does not, and hence $ax$ need not be central. Indeed, taking $x=1$ gives $ax=a$, and as long as $a$ is not central there is some $y$ with $ay\neq ya$, i.e., $(ax)y\neq y(ax)$, so $ax=a\not\in Z(A)$. Hence $Z(A)$ fails the condition for ideals, and $Z(A)$ is an ideal only when $Z(A)=A$, i.e., when $A$ is commutative. Thus in a noncommutative ring, $Z(A)$ is a typical example of a subring that is not an ideal.

::: Definition 9
For a ring $A$ and an ideal $\mathfrak{m}$, if there is no ideal $\mathfrak{a}$ satisfying $\mathfrak{m}\subsetneq\mathfrak{a}\subsetneq A$, then we call $\mathfrak{m}$ a *maximal ideal*.
:::
