...
weight: 101
translated_at: 2026-05-26T02:00:01+00:00
...
In general, in [Definition 1](#def1) we do not assume that $$0\neq 1$$, but if $$0=1$$ then by 1 of [Proposition 2](#prop2), for any $$\alpha\in A$$
...
<ins id="def3">**Definition 3**</ins> For two rings $$A,B$$, a function $$\phi:A \rightarrow B$$ is called a *ring homomorphism* if for any $$x,y$$
...
<ins id="def5">**Definition 5**</ins> A subset $$S$$ of a ring $$(A,+,-,\cdot,0,1)$$ is called a *subring* if $$(S,+,-,\cdot,0,1)$$ has a ring structure.
...
<ins id="def8">**Definition 8**</ins> For a ring $$A$$ and an ideal $$\mathfrak{m}$$, if there exists no ideal $$\mathfrak{a}$$ satisfying $$\mathfrak{m}\subsetneq\mathfrak{a}\subsetneq A$$, then we call $$\mathfrak{m}$$ a *maximal ideal*.

</div>

Let any ideal $$\mathfrak{a}$$ of $$A$$ be given. Then we can consider the collection of ideals of $$A$$ containing $$\mathfrak{a}$$ and different from $$A$$ itself. Then this collection is an inductive set, so it has a maximal element. ([\[Set Theory\] §Axiom of Choice, ⁋Theorem 4](/en/math/set_theory/axiom_of_choice#thm4)) It is not difficult to see that this maximal element is a maximal ideal. That is, we obtain the following.
