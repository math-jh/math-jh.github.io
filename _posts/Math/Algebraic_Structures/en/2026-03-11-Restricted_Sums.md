...
---
Previously, we verified in [§Direct Product of Groups](/en/math/algebraic_structures/direct_products){: data-lid="fya78" } that arbitrary products exist in $\Grp$, and in [§Group Homomorphisms](/en/math/algebraic_structures/group_homomorphisms){: data-lid="mkzxm" } that every pair of parallel morphisms in $\Grp$ has an equalizer. Hence, by the argument following [\[Category Theory\] §Limits, ⁋Example 7](/en/math/category_theory/limits#ex7){: data-lid="e9tag" }, $\Grp$ is a complete category.

On the other hand, every pair of parallel morphisms in $\Grp$ also has a coequalizer. ([§Group Isomorphisms, ⁋Proposition 8](/en/math/algebraic_structures/isomorphism_theorems#prop8){: data-lid="eaggw" }) Therefore, if $\Grp$ has arbitrary coproducts, $\Grp$ would be a cocomplete category, and hence a bicomplete category. 

However, as in [§Direct Product of Groups, ⁋Lemma 1](/en/math/algebraic_structures/direct_products#lem1){: data-lid="04sw7" }, finding an obvious way to endow the coproduct in $\Set$, $\coprod G_i$, with a group structure seems difficult. ([\[Set Theory\] §Sum of Sets, ⁋Proposition 5](/en/math/set_theory/sum_of_sets#prop5){: data-lid="l35q3" })
...
is a condition that inevitably had to arise, because these conditions are precisely those satisfied by the $\iota_i$. Because of this, [Theorem 2](#thm2){: data-lid="29rat" } is the answer to our question only for abelian groups.
...
Given an element of $\prod^w(G_i/H_i)$, $y$, at indices where $\pr_iy$ is the identity, for such $i$ we choose $e$ as the representative, and for the remaining finitely many $i$ we choose arbitrary representatives to obtain an element of $\prod^wG_i$; hence $p$ is surjective. Also, $p(x)$ being the identity is equivalent to having for every $i$ that $\pr_ix\in H_i$, so $\ker p=\prod^wH_i$. Therefore, by [§Group Isomorphisms, ⁋Lemma 1](/en/math/algebraic_structures/isomorphism_theorems#lem1){: data-lid="9ialn" }, $\prod^wH_i$ is a normal subgroup of $\prod^wG_i$, and by [§Group Isomorphisms, ⁋Theorem 2](/en/math/algebraic_structures/isomorphism_theorems#thm2){: data-lid="wq0h3" }, the identity
...
Considering the form of the homomorphism $f$ constructed in [Theorem 2](#thm2){: data-lid="5887s" }, one can verify that $G$ being the internal weak direct product of the $H_i$ is equivalent to the condition
...
we see that the elements of $H_i$ and $H_j$ commute. Hence the inclusion homomorphisms $\iota_i$ induce $\iota$ properly as in [Theorem 2](#thm2){: data-lid="4dqye" }.
