...
::: Definition 1
Fix a monoid object $(A,\cdot, 1)$ in the symmetric monoidal category $(\Ab,\otimes, \mathbb{Z})$. Then an object $M\in\Ab$ equipped with a left $A$-action is called a *left $A$-module*, and one equipped with a right $A$-action is called a *right $A$-module*.
:::
...
::: Definition 2
For an $A$-module $M$ and a family $(x_i)_{i\in I}$ of its elements, a *linear combination* of these elements is an element of the form
...
::: Definition 3
For an arbitrary $A$-module $M$ and a subset $N$ of it, if $N$ itself carries an $A$-module structure, we call it a *submodule*.
:::
...
::: Proposition 7
The composition of linear maps is a linear map. Also, a bijective linear map is always an isomorphism.
:::
::: Proof
Obvious.
:::
...
::: Proposition 8
For any $\Hom_{\lMod{A}}(M,N)$ is an abelian group. Moreover, for any $A$-linear map $u:M \rightarrow M'$,
...
::: Definition 9
For an $A$-linear map $u:M \rightarrow N$, we define the kernel and image of $u$ respectively as
