...
---
We now define the notion of an $$A$$-algebra. An algebra is a structure obtained by adding a multiplication structure to an $$A$$-module $$M$$ that is compatible with the additive structure and scalar multiplication of $$M$$; this is the same process as defining a multiplication on an abelian group $$G$$ that is compatible with its addition. However, there are two problems with using [§Definition of a Ring, ⁋Definition 1](/en/math/algebraic_structures/rings#def1) directly.
