...
First, for a general ring $A$, neither $\lMod{A}$ nor $\rMod{A}$ is a monoidal category. This is resolved if we assume that $A$ is a commutative ring, since then $(\lMod{A},\otimes_A, A)$ becomes a symmetric monoidal category. ([§Product, Coproduct, and Tensor Product of Rings, §§Tensor product of rings](/en/math/algebraic_structures/operations_of_rings#tensor-product-of-rings))
