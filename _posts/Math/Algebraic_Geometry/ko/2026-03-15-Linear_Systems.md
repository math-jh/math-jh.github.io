...
([§Divisors](/ko/math/algebraic_geometry/divisors))에서 우리는 divisor의 개념을, ([§Line Bundles](/ko/math/algebraic_geometry/line_bundles))에서 line bundle과 global sections $$H^0(X, \mathcal{L})$$의 개념을 다루었다. Line bundle의 도입 동기 중 하나는, divisor를 함수의 zero와 pole이라는 제약에서 벗어나 독립적인 대상으로 다룰 수 있게 한다는 것이었다. [§Line Bundles, ⁋명제 19](/ko/math/algebraic_geometry/line_bundles#prop19)에서 우리는 $$\operatorname{Pic}(X) \cong \operatorname{CaCl}(X)$$ (Cartier divisor class group)라는 대응을 확인했으며, 특히 $$X$$가 smooth variety일 때는 $$\operatorname{CaCl}(X) \cong \operatorname{Cl}(X)$$가 성립한다.

이제 *linear system*을 정의한다. Line bundle $$\mathcal{L}$$의 global section들은 effective divisor의 family를 이루며, linear system은 이 family를 projective space로 parameterize하는 방법이다. 이 개념의 중요성은 여러 층면에서 드러난다. 첫째, linear system은 동일한 line bundle class에 속하는 effective divisor들의 모임을 projective space라는 자연스러운 공간으로 조직화한다. 둘째, basepoint-free linear system은 다양체에서 사영공간으로의 정칙사상을 정의하며, 이 사상의 image를 통해 다양체의 기하학적 성질을 연구할 수 있다. 셋째, ample line bundle은 충분히 높은 tensor power에서 다양체를 사영공간으로 embed하는 역할을 하므로, 다양체의 embedding을 결정하는 근본적인 대상이다. Linear system은 이러한 모든 이론의 구체적인 실현 수단이며, Riemann-Roch 정리의 주된 응용 대상이 된다.
...
앞서 [§Line Bundles, ⁋예시 19](/ko/math/algebraic_geometry/line_bundles#ex19)에서 $$H^0(\mathbb{P}^n, \mathcal{O}_{\mathbb{P}^n}(d))$$가 차수 $$d$$의 동차다항식들의 공간 $$\mathbb{K}[\x_0, \ldots, \x_n]_d$$와 동형임을 보았다. 따라서 ([§Line Bundles, ⁋명제 19](/ko/math/algebraic_geometry/line_bundles#prop19))의 맥락에서, divisor $$dH$$에 대응하는 line bundle $$\mathcal{O}_{\mathbb{P}^n}(d)$$의 complete linear system은 다음과 같다.
...
Closed subvariety $$X \subseteq \mathbb{P}^n$$ 위에서, 부분벡터공간 $$V \subseteq \mathbb{K}[\x_0, \ldots, \x_n]_d$$가 정의하는 *linear system*은 [정의 2](#def2)의 특수한 경우로, $$\mathbb{P}(V)$$의 각 원소 $$[F]$$가 $$X$$ 위의 교차 $$X \cap Z(F)$$에 대응되는 family이다.
...
<ins id="prop10">**명제 10**</ins> Closed subvariety $$X \subseteq \mathbb{P}^n$$ 위의 basepoint-free linear system $$L = \mathbb{P}(V)$$에서, $$V$$의 기저 $$F_0, \ldots, F_r \in \mathbb{K}[\x_0, \ldots, \x_n]_d$$가 다음 조건을 만족한다고 하자:
