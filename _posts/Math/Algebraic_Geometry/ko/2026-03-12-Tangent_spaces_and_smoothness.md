...
Affine case만 보인다. $$X = Z(f_1, \ldots, f_k) \subseteq \mathbb{A}^n$$이 irreducible하고 $$\dim X = d$$라 하자. 점 $$x \in X$$에서의 local ring $$\mathscr{O}_{X,x} = \mathbb{K}[X]_{\mathfrak{m}_x}$$를 생각하자. Localization은 차원을 보존하므로 $$\dim \mathscr{O}_{X,x} = \dim X = d$$이다. ([§차원, ⁋명제 2](/ko/math/algebraic_geometry/dimension#prop2))

일반적으로 Noetherian local ring $$(R, \mathfrak{m})$$에 대하여 $$\dim_{\mathbb{K}}(\mathfrak{m}/\mathfrak{m}^2) \ge \dim R$$이다. ([§매개계, ⁋명제 2](/ko/math/commutative_algebra/system_of_parameters#prop2)) 따라서
...
임의의 algebraic variety는 대부분의 점에서 매끄럽다. 이는 대수기하학의 기본적인 정리 중 하나이며, 대수다양체가 잘 작동한다는 것을 보여준다.

<div class="proposition" markdown="1">

<ins id="prop9">**명제 9**</ins> Variety $$X$$의 smooth points들의 집합 $$X_\sm$$은 $$X$$의 dense open subset이다. 특히, $$X_\sm \ne \emptyset$$이다.
...
$$X_{\text{sm}} = \{x \in X \mid \operatorname{rank}(J_x) = n - d\}$$

이다. 이제 이 집합이 dense open subset임을 보인다.

우선 이 집합이 열린집합임을 보이자. Rank가 정확히 $$n-d$$라는 것은 두 조건의 동시 성립을 의미한다. 첫째, rank가 $$n-d$$ *이상*이라는 것은, 어떤 $$(n-d) \times (n-d)$$ 부분행렬의 행렬식이 0이 아닌 것과 동치이고, 이는 Zariski 위상에서 열린조건이다. 둘째, rank가 $$n-d$$ *이하*라는 것은, 모든 $$(n-d+1) \times (n-d+1)$$ 부분행렬의 행렬식이 0인 것과 동치이고, 이는 닫힌조건이다. 따라서 rank가 정확히 $$n-d$$인 점들의 집합은 열린조건과 닫힌조건의 교집합으로, $$X$$의 위상에서 열린집합이다.

이제 $$X$$가 공집합임을 보이자. 

[명제 4](#prop4)에서 보였듯이, irreducible variety $$X$$에서 Jacobian $$J_x$$의 rank는 모든 점에서 $$n - d$$를 넘을 수 없다. 이제 이 상한이 실제로 달성됨을 보여야 한다.

Krull의 Hauptidealsatz에 의하면, $$n-d$$개의 defining equation은 variety의 차원을 기껏해야 $$n-d$$만큼 줄인다. 따라서 $$\dim X = d$$이려면, "대부분의 점"에서 정확히 $$n-d$$개의 equation이 일차독립이어야 한다. 구체적으로, $$X$$의 generic point[^1]에서는 $$J_x$$의 rank가 정확히 $$n-d$$에 도달하며, rank가 최댓값에 도달하는 점들의 집합은 $$X$$의 비어있지 않은 Zariski 열린집합이다. 따라서 $$X_{\text{sm}} \ne \emptyset$$이다.
...
2. Parabola $$Z(\y - \x^2)$$는 smooth하다. 모든 점에서 $$\nabla(\y - \x^2) \ne 0$$이다. 이는 모든 conic(2차 다항식으로 정의되는 사영평면 위의 곡선)이 smooth임의 특별한 경우이다.
3. Cusp $$Z(\y^2 - \x^3)$$은 singular이다. 원점이 singular point이다. 이는 "가장 극단적인" 특이점의 예시이다.
4. Node $$Z(\y^2 - \x^2(\x+1))$$은 singular이다. 원점이 singular point이다. 이는 "가장 온화한" 특이점의 예시이다.
5. Smooth projective curve는 모든 점에서 매끄러운 1차원 사영다양체이다. 예를 들어 smooth plane curve $$Z(F) \subset \mathbb{P}^2$$ ($$F$$가 irreducible 동차다항식)는 모든 점 $$x$$에서 $$\nabla F(x) \ne 0$$이면 smooth하다. 이는 대수기하학에서 가장 중요한 다양체들 중 하나이다.
...
<ins id="def12">**정의 12**</ins> 점 $$x \in X$$에서의 *tangent cone* $$TC_x X$$는 $$x$$에서의 initial ideal로 정의되는 cone이다. 구체적으로, $$X = Z(f_1, \ldots, f_k)$$이고 각 $$f_i$$를 $$x$$에서 전개하면

$$f_i = f_i^{(d_i)} + \text{higher order terms}$$

이고, 여기서 $$f_i^{(d_i)}$$는 $$f_i$$의 lowest degree homogeneous part (initial form)이다. 그럼

$$TC_x X = Z(f_1^{(d_1)}, \ldots, f_k^{(d_k)})$$
...
<ins id="ex13">**예시 13**</ins> **Cusp의 tangent cone**: [예시 6(3)](#ex6)의 곡선 $$X = Z(\y^2 - \x^3)$$에서, $$f$$의 lowest degree term은 $$\y^2$$이므로
...
<ins id="ex14">**예시 14**</ins> **Node의 tangent cone**: [예시 7](#ex7)의 곡선 $$X = Z(\y^2 - \x^2(\x+1))$$에서, $$f$$의 lowest degree term은 $$\y^2 - \x^2 = (\y-\x)(\y+\x)$$이므로
