...
Grothendieck duality를 기술하기 위해서는 derived category의 기본 개념이 필요하다. 여기서는 그 핵심 아이디어를 요약하고, 자세한 내용은 [§유도카테고리, ⁋정의 1–10](/ko/math/homological_algebra/derived_categories#def1)을 참고한다.

Derived category의 기본 아이디어는 sheaf 하나가 아니라 complex 전체를 하나의 대상으로 취급하는 것이다. ([§유도카테고리, ⁋정의 1](/ko/math/homological_algebra/derived_categories#def1)) Resolution $$0 \to \mathcal{F} \to \mathcal{I}^0 \to \mathcal{I}^1 \to \cdots$$과 원래 sheaf $$\mathcal{F}$$는 quasi-isomorphism으로 연결되므로, derived category에서는 이 둘이 isomorphism이다. 따라서 exact하지 않은 functor를 다룰 때 sheaf를 resolution으로 치환한 뒤 functor를 적용하면 정보 손실 없이 결과를 얻을 수 있다.

이를 통해 derived functor를 정의할 수 있다. ([§유도카테고리, ⁋정의 6](/ko/math/homological_algebra/derived_categories#def6)) 예를 들어 right derived functor $$R f_\ast$$는 higher direct image $$R^i f_\ast$$를 모든 차수에서 동시에 포착하는 complex 수준의 functor이며, $$\mathbf{R}\mathcal{H}om$$은 $$\operatorname{Ext}^i$$를 모든 차수에서 포착한다. ([§유도카테고리, ⁋명제 8](/ko/math/homological_algebra/derived_categories#prop8)) Derived category $$D(\mathcal{A})$$는 *triangulated category*의 구조를 갖으며, ([§유도카테고리, ⁋정의 9](/ko/math/homological_algebra/derived_categories#def9)) 이 구조는 abelian category에서 short exact sequence가 하던 역할을 대체한다. Distinguished triangle $$A \to B \to C \to A[1]$$에서 $$C$$는 사상의 "isomorphism에서 벗어나는 정도"를 측정하는 mapping cone이다.

기하적 응용에서 derived category의 핵심은 sheaf 대신 complex 전체를 다룸으로써 duality를 깔끔하게 기술할 수 있다는 점이다. 특히 derived adjunction에 의해 $$R f_\ast$$의 right adjoint인 $$f^!$$가 자연스럽게 정의되며, ([§유도카테고리, ⁋명제 13](/ko/math/homological_algebra/derived_categories#prop13)) 이 functor는 sheaf category 수준에서는 올바른 의미를 갖지 않는다. Canonical bundle $$\omega_X$$는 $$f \colon X \to \operatorname{Spec}(\mathbb{K})$$에 대해 $$f^! \mathcal{O}_Y = \omega_X[n]$$으로 나타나며, 이것이 Serre duality에서 $$\omega_X$$가 등장하는 근본적인 이유이다.
