...
Kähler differential module $$\Omega_{A/\mathbb{K}}^1$$로부터 cotangent bundle $$\Omega_X^1$$이 복원됨을 보이는 것은, 두 vector bundle 사이에 bundle isomorphism이 존재함을 의미한다. Bundle isomorphism을 보이기 위해서는 임의의 열린 부분집합 $$U \subset X$$에 대해 두 bundle의 section space가 일치함을 확인하면 충분하다.

먼저 fiber 수준에서의 일치를 직관적으로 확인하자. 각 점 $$x \in X$$에서 cotangent space $$\mathfrak{m}_x/\mathfrak{m}_x^2$$와 $$\Omega_{A/\mathbb{K}}^1$$을 residue field $$\kappa(x)$$로 base change한 것이 isomorphic함은 [\[가환대수학\] §미분, ⁋명제 6](/ko/math/commutative_algebra/differentials#prop6)을 $$A = \mathbb{K}$$, $$E = \mathcal{O}_{X,x}$$, $$K = \mathfrak{m}_x$$, $$E' = \kappa(x)$$에 적용하면 즉시 얻어진다. 그러나 fiber가 pointwise로 isomorphic하다는 사실만으로는 두 bundle이 isomorphic하다고 결론 내릴 수 없다. Vector bundle의 구조는 fiber뿐 아니라 fiber들이 서로 어떻게 연결되는지, 즉 transition map에 의해 결정되며, fiber 정보만으로는 이를 알 수 없다.

따라서 section 수준에서 논증을 전개한다. Cotangent bundle $$\Omega_X^1$$의 $$U$$ 위의 section은 $$U$$ 위의 regular function들에 대해 미분값을 돌려주는 것, 즉 $$U$$ 위의 derivation $$D\colon \mathcal{O}_X(U) \to \mathcal{O}_X(U)$$로 이해할 수 있다. 이러한 관점에서 $$\Omega_X^1(U)$$를 "$$U$$ 위의 derivations의 공간"으로 간주한다.

한편 [\[가환대수학\] §미분, ⁋보조정리 2](/ko/math/commutative_algebra/differentials#lem2)에 의해 Kähler differential $$\Omega_{A/\mathbb{K}}^1$$은 다음의 universal property를 만족한다. 임의의 derivation $$D\colon A \to M$$에 대해, $$D = \varphi \circ d$$를 만족하는 유일한 $$A$$-linear map $$\varphi\colon \Omega_{A/\mathbb{K}}^1 \to M$$가 존재한다. 이 universal property는 "$$A$$ 위의 derivation을 명시하는 것"과 "$$\Omega_{A/\mathbb{K}}^1$$-valued map을 명시하는 것"이 본질적으로 동일한 정보임을 보여준다.

이 사실을 사용하여 $$\Omega_X^1(U)$$가 $$\Omega_{A/\mathbb{K}}^1$$에 의해 결정됨을 보인다. 임의의 열린 부분집합 $$U \subset X$$와 $$U$$ 위의 derivation $$D\colon \mathcal{O}_X(U) \to \mathcal{O}_X(U)$$를 생각하자. 이를 $$A$$로 제한하면 derivation $$D\vert_A\colon A \to \mathcal{O}_X(U)$$를 얻는다. Universal property에 의해 $$D\vert_A = \varphi \circ d$$를 만족하는 유일한 $$A$$-linear map $$\varphi\colon \Omega_{A/\mathbb{K}}^1 \to \mathcal{O}_X(U)$$가 존재한다.

반대로, 임의의 $$A$$-linear map $$\varphi\colon \Omega_{A/\mathbb{K}}^1 \to \mathcal{O}_X(U)$$가 주어지면, 합성 $$\varphi \circ d\colon A \to \mathcal{O}_X(U)$$는 $$A$$ 위의 derivation이며, 이는 자연스럽게 $$U$$ 위의 derivation으로 확장된다.

따라서 "$$U$$ 위의 derivations의 공간"과 "$$A$$-linear map $$\Omega_{A/\mathbb{K}}^1 \to \mathcal{O}_X(U)$$의 공간" 사이에 bijection이 존재한다. 즉, $$\Omega_X^1(U)$$의 section space가 $$\Omega_{A/\mathbb{K}}^1$$에 의해 완전히 결정되므로, 두 bundle은 isomorphic하다.
...
$$0 \to \mathcal{I}_D / \mathcal{I}_D^2 \to \Omega_X^1\vert_D \to \Omega_D^1 \to 0$$
