...
으로 주어지는 것을 계산할 수 있다. 한편 $$X$$와 $$D$$의 tangent sheaf $$\mathcal{T}_X=\Der(\mathcal{O}_X)$$, $$\mathcal{T}_D=\Der(\mathcal{O}_D)$$를 각각 계산하자. 그럼 natural inclusion $$\mathcal{T}_D\rightarrow \mathcal{T}_X\vert_D$$이 존재하며, 이것의 cokernel을 *normal bundle* $$\mathcal{N}_{D/X}$$으로 정의한다. 즉 다음의 short exact sequence

$$0\rightarrow \mathcal{T}_D\rightarrow \mathcal{T}_X\vert_D\rightarrow \mathcal{N}_{D/X}\rightarrow 0$$

이 존재한다. 그럼 우리는 이 normal bundle $$\mathcal{N}_{D/X}$$의 dual이 곧 

$$\mathcal{I}_D/\mathcal{I}_D^2$$
...
를 얻는다. 여기서 마지막 등호에서 tensor product와 restriction의 관계를 사용하였는데, 이에 대해 간단히 설명하자면 다음과 같다. 임의의 $$\mathcal{O}_X$$-module $$\mathcal{E}, \mathcal{F}$$와 closed subvariety $$D \subset X$$에 대하여

$$(\mathcal{E} \otimes_{\mathcal{O}_X} \mathcal{F})\vert_D \cong \mathcal{E}\vert_D \otimes_{\mathcal{O}_D} \mathcal{F}\vert_D$$

가 항상 성립한다. 이는 $$\mathcal{E}\vert_D = \mathcal{E} \otimes_{\mathcal{O}_X} \mathcal{O}_D$$로 정의되므로, tensor product의 결합법칙으로부터 자연스럽게 따라온다. 또한 exterior power에 대해서도 $$\det(\mathcal{E}\vert_D) \cong (\det \mathcal{E})\vert_D$$가 성립하며, 따라서 위의 증명에서 $$\omega_X \otimes \mathcal{O}_X(D)$$를 먼저 계산한 후 restriction $$\vert_D$$를 취하는 것이 $$\omega_X\vert_D \otimes \mathcal{O}_X(D)\vert_D$$를 얻는 것과 같은 결과를 준다.

</details>

Adjunction formula는 hypersurface의 canonical bundle을 계산하는 데 매우 유용하다.

Adjunction formula의 직관을 이해하기 위해 $$\mathcal{O}_X(D)$$의 기하학적 의미를 생각해보자. [§선다발과 벡터다발]에서 우리는 divisor $$D$$에 대해 line bundle $$\mathcal{O}_X(D)$$를 정의하였다. 한편 명제 9의 증명에서 살펴본 것처럼, smooth divisor $$D$$의 ideal sheaf $$\mathcal{I}_D$$는 $$\mathcal{O}_X(-D)$$에 해당하며, conormal sheaf $$\mathcal{I}_D/\mathcal{I}_D^2$$는 $$\mathcal{O}_X(-D)\vert_D$$에 해당한다. 이 conormal sheaf의 dual이 곧 $$D$$의 *normal bundle* $$N_{D/X}$$이며, $$N_{D/X} \cong \mathcal{O}_X(D)\vert_D$$로 주어진다. 즉 $$\mathcal{O}_X(D)$$는 $$D$$의 normal direction을 나타내는 line bundle의 전역적 확장이다.

이 관점에서 adjunction formula $$\omega_D \cong (\omega_X \otimes \mathcal{O}_X(D))\vert_D$$는 다음과 같이 해석된다. Ambient variety $$X$$의 canonical bundle $$\omega_X$$를 normal bundle $$\mathcal{O}_X(D)$$로 twist한 후 $$D$$로 restrict하면, subvariety $$D$$의 canonical bundle $$\omega_D$$가 나온다. 쉽게 말해, $$D$$ 위의 differential form은 ambient space의 differential form에 normal direction의 정보를 추가하여 얻어진다는 것이다. 이는 나중에 [§선다발과 벡터다발] 글에서 $$\mathcal{O}_X(D)$$ 정의 이후에 좀 더 자세히 다룰 예정이다.
...
이다. 따라서 $$K_C \sim (d-3)H\vert_C$$이며, 이 때 $$H\vert_C$$의 degree가 $$d$$이므로 $$\deg K_C = d(d-3)$$이다. 한편 plane curve의 genus는 $$g = (d-1)(d-2)/2$$로 주어지므로, $$2g - 2 = (d-1)(d-2) - 2 = d^2 - 3d = d(d-3) = \deg K_C$$임을 확인할 수 있다. 즉 $$\deg K_C = 2g - 2$$가 성립한다. 

</div>

한편 canonical divisor $$K_X$$의 관점을 좀 더 일반적으로 살펴보자. Canonical bundle $$\omega_X \cong \mathcal{O}_X(K_X)$$의 global section은 regular $$n$$-form들이다. 반대로, 임의의 divisor $$D$$에 대해 $$\mathcal{O}_X(D)$$의 global section은 meromorphic function $$f$$ 중 $$\operatorname{div}(f) + D \ge 0$$인 것들, 즉 $$D$$가 허용하는 pole을 갖는 meromorphic function들이다. 따라서 tensor product $$\omega_X \otimes \mathcal{O}_X(D) \cong \mathcal{O}_X(K_X + D)$$의 global section은 pole이 $$D$$로 제한된 meromorphic $$n$$-form들로 이해할 수 있다. 이 관점에서 $$\mathcal{O}_X(D)$$가 divisor $$D$$를 normal direction으로 해석하는 line bundle이라는 점([명제 9](#prop9)의 직관 설명 참조)을 다시 한번 떠올리면, $$K_X + D$$의 section들이 ambient space의 differential form에 $$D$$ 방향의 pole을 추가한 형태라는 것이 자연스럽게 이해된다.


## Smooth Curve의 Canonical Bundle

[예시 10](#ex10)을 확장하여 임의의 smooth projective curve $$C$$의 canonical bundle을 더 깊게 살펴본다. 우리의 핵심적인 도구는 다음 글에서 다룰 Riemann-Roch theorem으로, 이는 위의 계산을 단순한 degree 계산이 아니라 위상적 데이터를 포함하는 정보로 바꿔준다.

정의에 의하여, $$\omega_C$$의 global section $$s \in H^0(C, \omega_C)$$는 $$C$$ 위의 *regular 1-form*이다. $$C$$의 genus를 $$g$$라 할 때, Riemann-Roch theorem에 의해 $$h^0(C, \omega_C) = g$$이므로, regular 1-form의 공간은 $$g$$차원이다.

Canonical divisor $$K_C$$의 degree를 알아보자. [예시 8](#ex8)에서 $$\omega_{\mathbb{P}^n} \cong \mathcal{O}_{\mathbb{P}^n}(-n-1)$$임을 보았고, [명제 9](#prop9))에 의해 $$\mathbb{P}^2$$ 안의 smooth plane curve $$C$$ of degree $$d$$에 대해 $$\omega_C \cong \mathcal{O}_C(d-3)$$이 된다. 따라서 $$\deg K_C = d(d-3)$$이고, plane curve의 genus가 $$g = (d-1)(d-2)/2$$이므로 $$\deg K_C = 2g - 2$$를 얻는다. 이 결과는 임의의 smooth projective curve에 대해 성립한다.
