...
<ins id="def1">\ast\ast정의 1\ast\ast</ins> $$n$$차원 벡터공간 $$V$$의 $$k$$차원 부분공간들의 집합을 *Grassmannian<sub>그라스만다양체</sub>* $$G(k, V)$$ 또는 $$G(k, n)$$이라 부른다.
...
<ins id="ex2">\ast\ast예시 2\ast\ast</ins> 그라스만다양체의 기본 예시들이다.
...
<ins id="def3">\ast\ast정의 3\ast\ast</ins> *Plücker embedding* $$\iota: G(k, n) \to \mathbb{P}(\bigwedge^k \mathbb{K}^n)$$을 다음과 같이 정의한다. $$k$$차원 부분공간 $$W = \text{span}(v_1, \ldots, v_k)$$에 대해
...
<ins id="prop4">\ast\ast명제 4\ast\ast</ins> Plücker embedding은 well-defined이며 injective이다.
...
\ast\astWell-defined\ast\ast: $$W$$의 다른 basis $$w_1, \ldots, w_k$$를 선택하자. 그럼 $$w_i = \sum_{j=1}^k a_{ij} v_j$$ for some invertible matrix $$(a_{ij})$$이다. Exterior product의 multilinearity에 의해
...
<ins id="prop5">\ast\ast명제 5\ast\ast</ins> Plücker embedding의 image는 $$\mathbb{P}^{\binom{n}{k}-1}$$의 사영다양체이다. 즉, $$G(k, n)$$은 사영다양체이다.
...
<ins id="prop6">\ast\ast명제 6\ast\ast</ins> $$\dim G(k, n) = k(n - k)$$이다.
...
<ins id="prop7">\ast\ast명제 7\ast\ast</ins> $$v \in \bigwedge^k \mathbb{K}^n$$이 decomposable일 필요충분조건은 모든 $$w \in \bigwedge^{k-1} \mathbb{K}^n$$에 대해
...
<ins id="ex8">\ast\ast예시 8\ast\ast</ins> \ast\ast$$G(2, 4)$$의 Plücker relation\ast\ast: $$G(2, 4)$$를 생각하자. Plücker coordinates는 $$p_{12}, p_{13}, p_{14}, p_{23}, p_{24}, p_{34}$$이고, 이는 $$\mathbb{P}^5$$의 homogeneous coordinates이다. 유일한 Plücker relation은
...
<ins id="def9">\ast\ast정의 9\ast\ast</ins> 각 $$k$$개의 index들 $$I = \{i_1 < \cdots < i_k\}$$에 대해 open set $$U_I$$를
...
<ins id="prop10">\ast\ast명제 10\ast\ast</ins> 각 $$U_I \cong \mathbb{A}^{k(n-k)}$$이다.
...
<ins id="def11">\ast\ast정의 11\ast\ast</ins> *Flag* $$0 = V_0 \subset V_1 \subset \cdots \subset V_n = \mathbb{K}^n$$와 partition $$\lambda = (\lambda_1 \ge \cdots \ge \lambda_k \ge 0)$$ (단, $$\lambda_1 \le n-k$$)에 대해 *Schubert variety* $$\Omega_\lambda$$를
...
<ins id="prop12">\ast\ast명제 12\ast\ast</ins> $$\dim \Omega_\lambda = \lvert\lambda\rvert = \lambda_1 + \cdots + \lambda_k$$이다.
...
<ins id="ex13">\ast\ast예시 13\ast\ast</ins> \ast\ast$$G(2, 4)$$의 Schubert varieties\ast\ast: $$G(2, 4)$$에서 가능한 partitions은 $$(0,0)$$, $$(1,0)$$, $$(2,0)$$, $$(1,1)$$, $$(2,1)$$, $$(2,2)$$이다.
...
1. \ast\astCell decomposition\ast\ast: $$G(k, n)$$은 Schubert cells (Schubert varieties의 interior)들의 disjoint union으로 분해된다.
2. \ast\astCohomology\ast\ast: Schubert varieties의 cohomology classes는 $$G(k, n)$$의 cohomology ring의 basis를 이룬다.
3. \ast\astIntersection theory\ast\ast: Schubert calculus는 Schubert varieties의 교차를 계산하는 방법이다.

## 응용

<div class="example" markdown="1">

<ins id="ex14">\ast\ast예시 14\ast\ast</ins> \ast\ast$$\mathbb{P}^3$$의 직선들\ast\ast: $$G(2, 4)$$는 $$\mathbb{P}^3$$의 직선들의 공간이다. $$\mathbb{P}^3$$의 두 직선 $$L_1, L_2$$가 만나는 조건은 Plücker coordinates로 다음과 같이 표현된다. $$L_1, L_2$$의 Plücker coordinates를 $$p_{ij}, q_{ij}$$라 하면, $$L_1$$과 $$L_2$$가 만날 필요충분조건은
...
<ins id="ex15">\ast\ast예시 15\ast\ast</ins> \ast\astVeronese embedding과 Segre embedding\ast\ast: 그라스만다양체는 다른 중요한 embedding들의 일반화이다.
...
<ins id="ex16">\ast\ast예시 16\ast\ast</ins> \ast\astModuli of vector bundles\ast\ast: 그라스만다양체는 vector bundle들의 moduli space를 연구하는 기본 도구이다. 주어진 vector bundle $$E$$ on $$X$$의 section들의 공간 $$H^0(X, E)$$의 $$k$$차원 부분공간들의 모임은 $$G(k, H^0(X, E))$$의 닫힌부분다양체로 나타난다. 이를 통해 vector bundle의 성질을 기하학적으로 연구할 수 있다.
...
\ast\ast[GH]\ast\ast P. Griffiths and J. Harris, *Principles of Algebraic Geometry*, Wiley, 1978.  
\ast\ast[Ful]\ast\ast W. Fulton, *Young Tableaux*, Cambridge University Press, 1997.
