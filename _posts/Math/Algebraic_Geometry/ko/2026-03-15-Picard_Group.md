...
<ins id="prop11">**명제 11**</ins> Affine variety $$X$$에 대해, $$\operatorname{Pic}(X) \cong \operatorname{Cl}(X)$$이다. 만약 $$X$$의 coordinate ring가 unique factorization domain이면 $$\operatorname{Pic}(X) = 0$$이다.
...
$$X$$의 coordinate ring가 UFD이면 모든 height 1 prime ideal은 principal이다. 따라서 모든 Weil divisor가 principal divisor와 동치이고, $$\operatorname{Cl}(X) = 0$$이다. [\[Line Bundles\] §명제 16](/ko/math/algebraic_geometry/line_bundles#prop16)에 의해 $$\operatorname{Pic}(X) \cong \operatorname{Cl}(X) = 0$$이다.
