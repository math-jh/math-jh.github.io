...
<ins id="prop5">**명제 5**</ins> 대수적 토러스 $T_N$의 좌표환은 $\mathbb{C}[M]$과 동형이다.
...
$T_N = N \otimes \mathbb{C}^\ast$의 원소 $t = \sum_i e_i \otimes t_i$에 대하여, $m \in M$에 대응하는 character $\chi^m: T_N \to \mathbb{C}^\ast$를

$$\chi^m(t) = \chi^m\left(\sum_i e_i \otimes t_i\right) = \prod_i t_i^{m(e_i)}$$

으로 정의하자. 이들 character들의 polynomial ring이 바로 $\mathbb{C}[M]$이며, 이는 $T_N$의 정칙함수환과 일치한다.
