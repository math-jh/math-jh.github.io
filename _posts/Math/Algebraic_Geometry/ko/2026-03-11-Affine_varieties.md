...
Principal open set은 Zariski topology에서 열린집합이다. 다음 명제는 모든 열린집합이 principal open set들로 덮인다는 것을 보여준다. 즉, principal open set들은 affine space의 base를 이룬다. 

<div class="proposition" markdown="1">

<ins id="prop6">**명제 6**</ins> Affine variety $$X \subseteq \mathbb{A}^n$$의 임의의 열린부분집합 $$U$$에 대하여,

$$U = \bigcup_\alpha (D(f_\alpha) \cap X)$$

을 만족하는 principal open set들의 family $$D(f_\alpha)$$가 존재한다. 
...
$$U$$가 $$X$$의 열린부분집합이면 $$X \setminus U$$는 닫힌집합이고, 따라서 적당한 ideal $$I\subset \mathbb{K}[\x_1,\ldots, \x_n]$$가 존재하여 

$$X\setminus U=V(I)\cap X$$
...
원래의 정의에 의하여, affine variety의 임의의 닫힌집합은 affine variety이다. 그러나 일반적으로 affine variety의 열린집합은 affine variety일 필요가 없으며, 실제로도 그렇지 않다. 그러나 affine variety의 principal open subset은 반드시 affine variety이다.
...
<ins id="def8">**정의 8**</ins> 부분집합 $$X \subseteq \mathbb{A}^n$$에 대하여, $$\mathbb{K}[\x_1,\ldots, \x_n]$$의 부분집합 $$I(X)$$를
...
<ins id="def10">**정의 10**</ins> 아핀다양체 $$X = V(I) \subseteq \mathbb{A}^n$$의 *coordinate ring<sub>좌표환</sub>* $$\mathbb{K}[X]$$를
...
<ins id="ex11">**예시 11**</ins> [예시 3](#ex3)에서 살펴본 affine variety들의 경우, ideal들 $$(\x^2+\y^2-1)$$ 그리고 $$(\y-\x^2,\z-\x^3)$$은 radical임을 보일 수 있다. 따라서 단위원 $$X = V(\x^2+\y^2-1)$$의 coordinate ring은 $$\mathbb{K}[X] = \mathbb{K}[\x, \y]/(\x^2+\y^2-1)$$이고 twisted cubic $$C$$의 coordinate ring은 $$\mathbb{K}[C] = \mathbb{K}[\x, \y, \z]/(\y-\x^2, \z-\x^3) \cong \mathbb{K}[t]$$이다.
...
<ins id="def12">**정의 12**</ins> 두 affine variety들 $$X \subseteq \mathbb{A}^n$$과 $$Y \subseteq \mathbb{A}^m$$ 사이의 *morphism<sub>사상</sub>* (또는 *regular map*)이란 함수 $$\varphi: X \to Y$$로서, 다항식 $$f_1, \ldots, f_m \in \mathbb{K}[\x_1, \ldots, \x_n]$$들이 존재하여
