...
2. 초곡면 $$V(f)$$의 coordinate ring은 $$\mathbb{K}[V(f)] = \mathbb{K}[\x_1, \ldots, \x_n]/I(V(f)) = \mathbb{K}[\x_1, \ldots, \x_n]/\sqrt{(f)}$$이다. 
3. 
...
즉, morphism $$\varphi: X \to Y$$는 coordinate ring homomorphism $$\varphi^\ast: \mathbb{K}[Y] \to \mathbb{K}[X]$$를 유도한다. 여기서 화살표의 방향이 바뀌는 것에 주의하라. 이는 $$X \mapsto \mathbb{K}[X]$$가 아핀다양체의 category에서 ring의 category로 가는 contravariant functor를 정의한다는 것을 의미한다.
...
Isomorphism은 두 아핀다양체가 "기하학적으로 같다"는 것을 의미한다. 위의 명제에 의하면, 이는 두 coordinate ring이 ring으로서 동형인 것과 같다.
