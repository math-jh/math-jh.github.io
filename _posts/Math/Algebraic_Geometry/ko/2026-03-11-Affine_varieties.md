...
즉, $$V$$와 $$I$$는 antitone Galois connection을 정의한다. ([\[집합론\] §필터와 아이디얼, 갈루아 대응, ⁋정의 6](/ko/math/set_theory/filter_and_ideal#def6)) 따라서 [\[집합론\] §필터와 아이디얼, 갈루아 대응, ⁋정의 8](/ko/math/set_theory/filter_and_ideal#def8)에 의하여, 두 연산 $$VI$$와 $$IV$$는 각각의 ordered set 위에서 closure operator를 정의한다.

우선 $$VI$$의 경우를 생각하자. $$X \subseteq V(I(X))$$이고 $$V(I(X))$$는 affine variety이므로 닫힌집합이다. 따라서 $$V(I(X))$$는 $$X$$를 포함하는 가장 작은 닫힌집합, 즉 $$X$$의 *Zariski closure*이다. 그렇다면 $$IV$$는 어떨까? 
...
<ins id="ex10">**예시 10**</ins> [예시 3](#ex3)에서 살펴본 아핀다양체들의 coordinate ring을 계산해보자.
...
<ins id="def11">**정의 11**</ins> 아핀다양체 $$X$$ 위의 *regular function*이란 coordinate ring $$\mathbb{K}[X]$$의 원소를 의미한다.
...
<ins id="def12">**정의 12**</ins> 아핀다양체 $$X \subseteq \mathbb{A}^n$$과 $$Y \subseteq \mathbb{A}^m$$ 사이의 *morphism<sub>사상</sub>* (또는 *regular map*)이란 함수 $$\varphi: X \to Y$$로서, 다항식 $$f_1, \ldots, f_m \in \mathbb{K}[\x_1, \ldots, \x_n]$$들이 존재하여
...
<ins id="prop14">**명제 14**</ins> Morphism $$\varphi: X \to Y$$는 coordinate ring homomorphism $$\varphi^\ast: \mathbb{K}[Y] \to \mathbb{K}[X]$$를 유도한다. 구체적으로, $$\bar{g} \in \mathbb{K}[Y]$$에 대하여
