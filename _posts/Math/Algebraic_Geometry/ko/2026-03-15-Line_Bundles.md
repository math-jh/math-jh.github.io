...
이 명제는 고정된 variety $$X$$ 위에 정의된 line bundle들의 모임을 생각하면, 이 위에 $$\otimes$$를 연산으로 갖는 group structure가 존재함을 보여준다. 이 때 역원은 $$(-)^\vee$$를 통해 얻어지며 항등원은 $$\mathcal{O}_X$$이다. 
...
여기서 trivial bundle이 실제로 항등원의 역할을 한다는 것은 [예시 3](#ex3)과 [명제 6](#prop6)에 의해 자명하다. 뿐만 아니라, tensor product의 성질에 의해 다음이 성립한다. 
...
<ins id="ex11">**예시 11**</ins> 우선 $$\mathbb{A}^n$$의 Picard group을 직접 계산하자. $$\mathbb{A}^n$$은 affine variety이므로, line bundle과 finitely generated projective module 사이의 표준적인 대응 관계가 존재한다. 구체적으로, line bundle $$\mathcal{L}$$의 global section $$\Gamma(\mathbb{A}^n, \mathcal{L})$$은 coordinate ring $$\mathbb{K}[\x_1, \ldots, \x_n]$$ 위의 finitely generated projective module을 이루며, 역으로 모든 finitely generated projective module은 어떤 line bundle로부터 온다. 이 대응 하에서 trivial line bundle $$\mathcal{O}_{\mathbb{A}^n}$$은 free module $$\mathbb{K}[\x_1, \ldots, \x_n]$$에 대응된다.

이제 $$\mathbb{K}[\x_1, \ldots, \x_n]$$이 [\[환론\] §다항식환, ⁋정리 16](/ko/math/ring_theory/polynomial_rings#thm16)에 의해 unique factorization domain이라는 사실을 사용하자. UFD 위에서는 모든 finitely generated projective module of rank 1이 free module이다. 이는 다음과 같이 확인할 수 있다. Projective module $$M$$ of rank 1에 대하여, 그 exterior power $$\bigwedge^1 M = M$$의 embedding $$M \hookrightarrow \mathbb{K}[\x_1, \ldots, \x_n]$$을 생각하자. UFD에서 rank 1 projective module은 어떤 ideal $$I$$와 isomorphic하며, $$I$$가 invertible이면 (즉, 어떤 $$J$$에 대해 $$IJ = (1)$$이면) $$I$$는 principal ideal이다. UFD에서는 모든 height 1 prime ideal이 principal이므로, rank 1 projective module들은 모두 free이다.

따라서 $$\mathbb{A}^n$$ 위의 모든 line bundle이 trivial line bundle $$\mathcal{O}_{\mathbb{A}^n}$$과 isomorphic하다. 즉, $$\Pic(\mathbb{A}^n) = \{[\mathcal{O}_{\mathbb{A}^n}]\} \cong 0$$이다.
...
<ins id="ex12">**예시 12**</ins> $$\mathbb{P}^n$$의 Picard group을 직접 계산하여 $$\mathbb{Z}$$와 isomorphic임을 보이자. $$\mathbb{P}^n$$의 standard open cover $$U_i = \{[x_0 : \cdots : x_n] \mid x_i \ne 0\}$$를 생각하자. 각 $$d \in \mathbb{Z}$$에 대하여 line bundle $$\mathcal{O}_{\mathbb{P}^n}(d)$$를 transition functions $$g_{ij} = (x_i/x_j)^d$$로 정의한다. 이들은 [명제 2](#prop2)의 cocycle condition을 만족함이 자명하다.

이제 *degree map* $$\deg: \Pic(\mathbb{P}^n) \to \mathbb{Z}$$를 $$[\mathcal{O}_{\mathbb{P}^n}(d)] \mapsto d$$로 정의하자. 이 map이 well-defined임을 보이기 위해, $$\mathcal{O}_{\mathbb{P}^n}(d) \cong \mathcal{O}_{\mathbb{P}^n}(d')$$이면 $$d = d'$$임을 증명해야 한다. 두 bundle의 isomorphism은 각 $$U_i$$ 위에서 $$\mathcal{O}_{\mathbb{P}^n}(d)|_{U_i} \to \mathcal{O}_{\mathbb{P}^n}(d')|_{U_i}$$ 형태의 isomorphism $$h_i: \mathcal{O}_{U_i} \to \mathcal{O}_{U_i}$$들로 주어지며, 이들은 $$U_i \cap U_j$$에서 다음 compatibility condition을 만족해야 한다.

$$h_i \circ h_j^{-1} = \frac{(x_i/x_j)^{d'}}{(x_i/x_j)^d} = (x_i/x_j)^{d'-d}$$

각 $$h_i$$는 $$U_i$$ 위에서의 nowhere vanishing regular function, 즉 $$h_i \in \mathcal{O}_{\mathbb{P}^n}(U_i)^*$$에 의한 multiplication으로 주어진다. $$U_i = \mathbb{A}^n$$이므로 $$h_i$$는 $$\x_0/x_i, \ldots, \x_n/x_i$$들의 polynomial이다. 이제 $$h_i/h_j = (x_i/x_j)^{d'-d}$$를 $$U_0 \cap U_1 \cap U_2$$에서 생각하자. $$h_i$$가 $$x_0/x_i$$에 대해 차수 $$a_i$$, $$x_1/x_i$$에 대해 차수 $$b_i$$인 polynomial이라고 하면, $$h_i/h_j$$에서 $$x_0/x_1$$에 대한 차수는 $$(a_i - b_i) - (a_j - b_j)$$이다. 반면 $$(x_i/x_j)^{d'-d}$$에서 $$x_0/x_1$$에 대한 차수는 $$d'-d$$와 무관하게 $$0$$이다. 따라서 $$h_i/h_j$$가 $$(x_i/x_j)^{d'-d}$$와 같으려면 $$d' = d$$이어야 한다. 이로써 $$\deg$$ map은 well-defined이며 injective이다.

Surjectivity는 자명하다. 임의의 $$d \in \mathbb{Z}$$에 대해 $$\mathcal{O}_{\mathbb{P}^n}(d)$$가 존재하므로, $$\deg([\mathcal{O}_{\mathbb{P}^n}(d)]) = d$$이다. 또한 $$\deg$$는 group homomorphism이다. $$\mathcal{O}_{\mathbb{P}^n}(d) \otimes \mathcal{O}_{\mathbb{P}^n}(d')$$의 transition functions이 $$(x_i/x_j)^d \cdot (x_i/x_j)^{d'} = (x_i/x_j)^{d+d'}$$이므로 $$\mathcal{O}_{\mathbb{P}^n}(d) \otimes \mathcal{O}_{\mathbb{P}^n}(d') \cong \mathcal{O}_{\mathbb{P}^n}(d+d')$$이기 때문이다. 따라서 $$\deg: \Pic(\mathbb{P}^n) \to \mathbb{Z}$$는 group isomorphism이다.

</div>

$$\mathcal{O}_{\mathbb{P}^n}(1)$$은 다음과 같이 정의된다. $$\mathbb{P}^n$$의 standard open cover $$U_i = \{[x_0 : \cdots : x_n] \mid x_i \ne 0\}$$에서, transition functions은 $$g_{ij} = x_i/x_j$$이다. 일반적으로 $$\mathcal{O}_{\mathbb{P}^n}(d)$$는 $$g_{ij} = (x_i/x_j)^d$$를 transition functions으로 갖는 line bundle이며, $$\divisor(x_0^d) = dH$$에 대응된다. 여기서 $$H = \mathbb{V}(x_0)$$는 hyperplane divisor이다.
...
3. 각 $$i, j$$에 대해 $$\phi_j \circ \phi_i^{-1}: (U_i \cap U_j) \times \mathbb{A}^r \to (U_i \cap U_j) \times \mathbb{A}^r$$이 $$(p, v) \mapsto (p, g_{ij}(p)v)$$의 꼴인 조건. 여기서 $$g_{ij} \in \GL_r(\mathcal{O}_X(U_i \cap U_j))$$는 *transition function*이라 부른다.

</div>

Line bundle의 정의와 비교하면, 유일한 차이는 fiber가 $$\mathbb{A}^1$$ 대신 $$\mathbb{A}^r$$이고, transition function이 $$\mathcal{O}_X(U_i \cap U_j)^\ast = \GL_1(\mathcal{O}_X(U_i \cap U_j))$$ 대신 $$\GL_r(\mathcal{O}_X(U_i \cap U_j))$$ 값을 갖는다는 점이다. 따라서 line bundle은 정확히 rank 1 vector bundle이다.
