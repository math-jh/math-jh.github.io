...
<ins id="prop13">**명제 13** (Leray Spectral Sequence)</ins> 연속 사상 $$f : X \to Y$$와 위상공간 $$X$$ 위의 sheaf $$\mathcal{F}$$에 대하여, Leray spectral sequence
...
<ins id="prop14">**명제 14** (Five-Term Exact Sequence)</ins> 연속 사상 $$f : X \to Y$$와 sheaf $$\mathcal{F}$$에 대하여, Leray spectral sequence로부터 다음의 exact sequence를 얻는다.
...
Leray spectral sequence $$E_2^{p,q} = H^p(Y, R^q f_* \mathcal{F}) \Rightarrow H^{p+q}(X, \mathcal{F})$$의 $$E_2$$ page에서 $$p+q \leq 2$$인 항목들을 고려하자. ([§스펙트럼 열, ⁋정의 5](/ko/math/homological_algebra/spectral_sequences#def5))에 의해 $$E_\infty^{p,q} \cong \gr^p H^{p+q} = F^p H^{p+q}/F^{p+1}H^{p+q}$$이다. 또한 이는 first quadrant spectral sequence이므로 ([§스펙트럼 열, ⁋명제 6](/ko/math/homological_algebra/spectral_sequences#prop6)) 충분히 큰 $$r$$에서 $$E_r^{p,q} = E_\infty^{p,q}$$이다.
...
$$\mathcal{F}$$의 injective resolution $$\mathcal{I}^\bullet$$을 잡고, double complex $$C^{p,q} = \check{C}^p(\mathcal{U}, \mathcal{I}^q)$$를 구성한다. 두 filtration으로부터 얻어지는 두 spectral sequence가 같은 total cohomology $$H^{p+q}(X, \mathcal{F})$$에 수렴함은 ([§스펙트럼 열, ⁋예시 11](/ko/math/homological_algebra/spectral_sequences#ex11))의 프레임워크에 의한다.
