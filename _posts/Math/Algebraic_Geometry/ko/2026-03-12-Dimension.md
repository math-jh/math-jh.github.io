...
[StackExchange의 깔끔한 증명](https://math.stackexchange.com/q/3585513)을 따른다. Projective variety $$X \subseteq \mathbb{P}^n$$의 homogeneous coordinate ring을 $$S(X) = \mathbb{K}[x_0, \ldots, x_n]/I(X)$$라 하자. 그럼 cone $$C(X) \subseteq \mathbb{A}^{n+1}$$의 coordinate ring은 $$\mathbb{K}[C(X)] = S(X)$$이므로

$$\dim C(X) = \dim S(X)$$

이다. 이제 projective variety의 차원이 homogeneous coordinate ring의 Krull dimension보다 1 작다는 것을 보이면 된다.

**Step 1:** $$\dim X \le \dim S(X) - 1$$.

$$X$$의 irreducible closed subset들의 chain

$$X = X_0 \supsetneq X_1 \supsetneq \cdots \supsetneq X_r \neq \emptyset$$

을 생각하자. 각 $$X_i$$에 대응하는 graded prime ideal $$\mathfrak{p}_i = I(X_i)$$들은 $$S(X)$$에서의 chain

$$\mathfrak{p}_0 \subsetneq \mathfrak{p}_1 \subsetneq \cdots \subsetneq \mathfrak{p}_r$$

을 준다. 여기서 $$\mathfrak{p}_r \neq \mathfrak{m} = (x_0, \ldots, x_n)$$인데, 이는 $$X_r \neq \emptyset$$이므로 $$\mathfrak{p}_r$$이 irrelevant ideal $$\mathfrak{m}$$을 포함할 수 없기 때문이다. 따라서 $$\mathfrak{p}_0 \subsetneq \cdots \subsetneq \mathfrak{p}_r \subsetneq \mathfrak{m}$$은 길이 $$r+1$$의 chain이고, $$\dim S(X) \ge r+1$$이다. 즉 $$r \le \dim S(X) - 1$$이고, 이로부터 $$\dim X \le \dim S(X) - 1$$을 얻는다.

**Step 2:** $$\dim X \ge \dim S(X) - 1$$.

$$S(X)$$의 prime ideal chain

$$\mathfrak{q}_0 \subsetneq \mathfrak{q}_1 \subsetneq \cdots \subsetneq \mathfrak{q}_s = \mathfrak{m}$$

을 생각하자 (여기서 $$\mathfrak{m}$$은 irrelevant ideal). [\[가환대수학\] §Graded ring과 module, ⁋명제 6](/ko/math/commutative_algebra/graded_rings_and_modules#prop6)에 의해 graded ring의 임의의 prime ideal은 graded prime ideal 사이에 끼어들 수 있으므로, 위 chain을 graded prime ideal들로 refine할 수 있다. 따라서 graded prime ideal chain

$$\mathfrak{p}_0 \subsetneq \mathfrak{p}_1 \subsetneq \cdots \subsetneq \mathfrak{p}_t = \mathfrak{m}$$

이 존재하며 $$t \ge s$$이다. 이제 $$\mathfrak{p}_i = I(Y_i)$$로 대응되는 projective variety $$Y_i$$들을 생각하면

$$Y_0 \supsetneq Y_1 \supsetneq \cdots \supsetneq Y_{t-1} \neq \emptyset$$

은 $$X$$의 irreducible closed subset들의 chain이다. 여기서 $$Y_{t-1} \neq \emptyset$$인 것은 $$\mathfrak{p}_{t-1} \subsetneq \mathfrak{m}$$이므로 $$\mathfrak{p}_{t-1}$$이 irrelevant ideal이 아니기 때문이다. 따라서 $$\dim X \ge t-1 \ge s-1$$이고, 이로부터 $$\dim X \ge \dim S(X) - 1$$을 얻는다.

**결론:** 두 부등식으로부터 $$\dim X = \dim S(X) - 1 = \dim C(X) - 1$$이다.
