...
우선 $$X,Y$$가 birationally equivalent하다 가정하자. 그럼 regular map $$\varphi:\dom(\varphi)\rightarrow Y$$가 정의하는 pullback homomorphism $$\varphi^\ast:\mathbb{K}(Y)\rightarrow \mathbb{K}(\dom(\varphi))$$이 존재한다. 그런데 [명제 2](#prop2)로부터 $$\mathbb{K}(\dom(\varphi))=\mathbb{K}(X)$$임을 알고, 이로부터 둘째 조건을 얻는다. 

이제 둘째 조건을 가정하여 field isomorphism $$\mathbb{K}(X)\rightarrow \mathbb{K}(Y)$$가 주어졌다 하자. 그럼 $$X$$의 affine open subset $$U$$에 대하여, coordinate ring 사이의 포함관계 $$\mathbb{K}[U]\subset \mathbb{K}[X]$$가 존재한다. 이제 $$\Phi$$는 $$\mathbb{K}(X)$$에서 $$\mathbb{K}(Y)$$의 함수를 주며, 
...
**(2) $$\Rightarrow$$ (3)**: Field isomorphism $$\Phi: \mathbb{K}(X) \xrightarrow{\sim} \mathbb{K}(Y)$$이 주어졌다 하자. $$X$$의 affine open (다양체의 열린부분집합 가운데 그 자체가 affine variety인 것) $$U \subseteq X$$을 고르면, $$U$$의 [coordinate ring](/ko/math/algebraic_geometry/affine_varieties#def11) $$\mathbb{K}[U]$$는 $$\mathbb{K}(X)$$의 부분환이다. $$\mathbb{K}[U]$$가 $$\mathbb{K}$$-대수로서 유한생성이라 하고, $$f_1, \ldots, f_r \in \mathbb{K}[U]$$를 그 생성원이라 하자.

각 $$\Phi(f_i) \in \mathbb{K}(Y)$$는 $$Y$$ 위의 rational function이다. $$\Phi(f_i) = g_i/h_i$$ ($$g_i, h_i \in \mathbb{K}[Y']$$, $$Y' \subseteq Y$$는 어떤 affine open)로 표현하면, $$h_i \ne 0$$인 점들에서 $$\Phi(f_i)$$는 정의된다. 모든 $$i$$에 대해 $$h_i \ne 0$$인 점들의 집합을 $$V$$라 하자. 즉,

$$V = \bigcap_{i=1}^{r} D(h_i) \subseteq Y'$$

이면 $$V$$는 $$Y$$의 affine open이고, 모든 $$\Phi(f_i)$$가 $$V$$에서 정의된다. 따라서 $$\Phi$$는 ring homomorphism $$\mathbb{K}[U] \to \mathbb{K}[V]$$를 유도한다.

같은 논리를 $$\Phi^{-1}: \mathbb{K}(Y) \to \mathbb{K}(X)$$에 적용하자. $$\mathbb{K}[V]$$의 생성원 $$g_1, \ldots, g_s$$를 잡고, 각 $$\Phi^{-1}(g_j)$$가 정의되는 principal open set들의 교집합을 $$U' \subseteq U$$로 잡는다. 즉, $$\Phi^{-1}(g_j) = f'_j/h'_j$$로 표현할 때,

$$U' = \bigcap_{j=1}^{s} D(h'_j) \subseteq U$$

이면 $$\Phi^{-1}$$는 $$\mathbb{K}[V] \to \mathbb{K}[U']$$를 유도한다.

이제 $$\Phi: \mathbb{K}[U] \to \mathbb{K}[V]$$와 $$\Phi^{-1}: \mathbb{K}[V] \to \mathbb{K}[U']$$가 서로 inverse임에 주의하자. 특히 $$\Phi^{-1} \circ \Phi: \mathbb{K}[U] \to \mathbb{K}[U']$$는 포함 $$\mathbb{K}[U'] \hookrightarrow \mathbb{K}[U]$$와 같다. 그러나 $$U' \subseteq U$$에서 $$\Phi^{-1} \circ \Phi$$가 항등이므로, 사실 $$\Phi$$는 $$\mathbb{K}[U']$$에서 $$\mathbb{K}[V]$$로의 isomorphism을 정의한다. 마찬가지로 $$\Phi^{-1}$$는 $$\mathbb{K}[V]$$에서 $$\mathbb{K}[U']$$로의 isomorphism을 정의한다. 따라서 $$\mathbb{K}[U'] \cong \mathbb{K}[V]$$이고, [아핀다양체의 좌표환이 다양체를 결정한다](/ko/math/algebraic_geometry/affine_varieties#prop9)는 사실로부터 $$U' \cong V$$를 얻는다.

예를 들어, $$X = \mathbb{A}^1$$, $$Y = V(\y - \x^2) \subset \mathbb{A}^2$$를 생각하자. 둘 다 함수체 $$\mathbb{K}(t)$$를 가지므로 birationally equivalent이다. $$\mathbb{K}(X) = \mathbb{K}(\x)$$, $$\mathbb{K}(Y) = \mathbb{K}(\x)$$ (where $$\y = \x^2$$)이고, $$\Phi: \x \mapsto \x$$로 두면 이는 isomorphism이다. $$U = \mathbb{A}^1 \setminus \{0\} = D(\x)$$를 잡으면 $$\mathbb{K}[U] = \mathbb{K}[\x, \x^{-1}]$$이고, $$V = Y \setminus \{(0,0)\}$$에서 $$\mathbb{K}[V] \cong \mathbb{K}[\x, \x^{-1}]$$이다. 실제로 $$U \cong V$$이다.
...
이제 [예시 6](#ex6)과 도입부에서 언급한 rational map $$\varphi: \mathbb{A}^2 \dashrightarrow \mathbb{P}^1$$, $$(x, y) \mapsto [x : y]$$를 생각하자. 이는 원점 $$(0, 0)$$에서 정의되지 않는다. 그러나 blow-up $$\operatorname{Bl}_0 \mathbb{A}^2$$ 위에서는 이 map을 *everywhere defined* regular map으로 확장할 수 있다. 구체적으로, map $$\tilde{\varphi}: \operatorname{Bl}_0 \mathbb{A}^2 \to \mathbb{P}^1$$을
