...
[§준사영다양체](/ko/math/algebraic_geometry/quasi_projective_varieties)에서 우리는 아핀다양체와 사영다양체를 모두 포함하는 준사영다양체의 개념을 정의했다. 아직 우리가 확보한 도구는 coordinate ring, homogeneous polynomials, 그리고 그들의 zero sets뿐이었다. 이제 이 도구들을 활용하여 다양체들 사이의 "사상"을 정의할 필요가 있다. 실제로, 수학에서 어떤 대상을 연구할 때 그 대상 사이의 사상을 정의하는 것이 필수적이다. 다양체에서 사상이란 무엇일까? 우리는 이 질문에 대해 대답하고자 한다. 

아핀다양체의 경우, 우리는 [§아핀다양체, ⁋명제 10](/ko/math/algebraic_geometry/affine_varieties#prop10)에서 morphism을 coordinate ring의 homomorphism으로 정의했다. 사영다양체의 경우, [§사영다양체, ⁋정의 13](/ko/math/algebraic_geometry/projective_varieties#def13)에서 homogeneous polynomials로 표현되는 함수로 정의했다. 그러나 준사영다양체는 아핀과 사영의 중간적인 개념이므로, 이들 사이의 morphism을 정의하는 것은 직관적인 아이디어를 formalize해야 한다. 이 절에서 우리는 regular function과 regular map을 정의하고, 이들이 아핀 및 사영 경우의 구체적인 정의와 일치함을 보일 것이다.

## 정칙함수

준사영다양체 위에서 함수를 정의할 때, 우리의 유일한 도구는 다항식이다. 이는 아핀다양체에서 coordinate ring이 다항식들의 집합이기 때문에 자명하다. 문제는 준사영다양체가 전역적으로 하나의 좌표계를 갖지 않는다는 점이다. 아핀다양체 전체에 걸쳐 다항식으로 표현되는 함수가 없을 수도 있다. 이를 해결하기 위해 우리는 함수를 국소적으로 정의하려고 한다. 이는 미분다양체에서 smooth function을 국소적으로 정의하는 것과 같은 발상이다. 즉, 각 점에서 근방에서 함수가 다항식으로 표현되는지를 확인하는 방식이다.
...
이 정의는 미분다양체에서의 smooth function 정의와 비슷하다. 미분다양체에서 각 점 $$p$$의 열린근방이 $$\mathbb{R}^n$$과 동형인 chart $$\varphi: U \to \mathbb{R}^n$$을 갖고, 함수 $$f: M \to \mathbb{R}$$가 smooth인 것은 $$f \circ \varphi^{-1}: \mathbb{R}^n \to \mathbb{R}$$이 smooth함으로 정의된다. Quasi-projective variety의 경우, [§준사영다양체](/ko/math/algebraic_geometry/quasi_projective_varieties)에서 논의했듯 각 점 $$p$$가 어떤 affine chart $$U_i \cong \mathbb{A}^n$$에 속하고, 함수 $$f$$가 regular인 것은 이 affine chart에서 다항식으로 표현됨으로 정의된다. 즉, regularity는 local property이므로 각 점에서 근방에서 확인하면 충분하다.

<div class="example" markdown="1">

<ins id="ex2">**예시 2**</ins> Regular function의 예시들을 살펴보자.

1. 아핀다양체 $$X$$에서 $$\mathcal{O}(X) = \mathbb{K}[X]$$이다. 이는 [§아핀다양체, ⁋정의 7](/ko/math/algebraic_geometry/affine_varieties#def7)에서 coordinate ring의 원소들을 regular function이라 불렀던 것과 일치한다. coordinate ring의 원소 $$f$$는 전역에서 정의되는 다항식이므로 자연스럽게 regular function이다.
2. $$\mathbb{P}^n$$에서 $$\mathcal{O}(\mathbb{P}^n) = \mathbb{K}$$이다. 즉, 상수함수만 존재한다. 이는 homogeneous coordinates $$[x_0 : \cdots : x_n]$$에서 비상수 다항식은 좌표의 scaling에 대해 잘 정의되지 않기 때문이다. 가령 $$x_0/x_1$$은 $$U_1 = \{x_1 \ne 0\}$$에서 정의되지만, $$\mathbb{P}^n$$ 전체에서는 정의되지 않는다. 이는 projective space가 affine space가 아님을 보여주는 좋은 예시이다.
3. $$\mathbb{A}^2 \setminus \{(0,0)\}$$에서 $$\mathcal{O}(X) = \mathbb{K}[\x, \y]$$이다. 직관적으로, 원점을 제거해도 다항식은 여전히 잘 정의된다. 더 일반적으로, 기약 아핀다양체에서 여집합의 여차원이 2 이상이면 regular function은 변하지 않는다. 이는 projective space에서는 좌표 비율만 잘 정의되지만, affine variety에서는 전역 다항식이 잘 정의됨을 의미한다.

</div>

## 정칙사상의 정의

Regular function을 정의했으니, 이제 준사영다양체 사이의 사상을 정의할 수 있다. 여기서 핵심 아이디어는 사상이 regular function을 "보존"해야 한다는 점이다. 즉, 사상의 pullback이 regular function을 얻어야 한다는 것이다. 이는 category theory에서 functor의 의미와 비슷하다. 사상은 다양체를 하나에서 다른 곳으로 보내지만, 동시에 coordinate ring을 다른 ring으로 보내는 것이므로, natural하게 regular function들을 pullback한다고 생각할 수 있다.
...
이므로 $$\psi \circ \varphi$$는 regular map이다. 합성 연산이 associative하기 때문에, regular map들과 합성 연산에 의해 준사영다양체들의 category를 형성한다.

항등사상 $$\text{id}_X: X \to X$$의 경우, $$f \circ \text{id}_X = f$$이므로 자명하게 regular map이다. 이는 category의 axioms를 만족한다.

</details>

따라서 준사영다양체들과 regular map들은 category를 이룬다. 이 category에서의 isomorphism을 정의할 수 있다. 이는 category theory의 기본적인 개념이지만, 기하학적으로 중요한 의미를 갖는다.
...
<ins id="ex9">**예시 9**</ins> **Projection**: $$\mathbb{A}^2 \to \mathbb{A}^1$$, $$(x, y) \mapsto x$$는 regular map이다. 이는 좌표함수 $$x$$가 $$\mathbb{K}[\x,\y]$$의 원소이므로 자명하다. 더 일반적으로, $$\mathbb{A}^n \to \mathbb{A}^m$$, $$(x_1, \ldots, x_n) \mapsto (f_1, \ldots, f_m)$$는 각 $$f_i$$가 다항식일 때 regular map이다. 이는 coordinate functions가 regular map을 정의하는 기본적인 도구임을 보여준다.
...
<ins id="ex11">**예시 11**</ins> **Veronese embedding**: $$\mathbb{P}^n \to \mathbb{P}^N$$, $$[x_0 : \cdots : x_n] \mapsto [\cdots : x_i x_j : \cdots]$$는 regular map이다. 여기서 $$N = \binom{n+2}{2} - 1$$이고, 좌표들은 모든 2차 단항식 $$x_i x_j$$ ($$0 \le i \le j \le n$$)을 순서대로 나열한 것이다. 이 사상은 $$\mathbb{P}^n$$을 $$\mathbb{P}^N$$의 사영다양체로 embedding하며, 그 image는 *Veronese variety*라 불린다. 예를 들어 $$n=1$$일 때, $$\mathbb{P}^1 \to \mathbb{P}^2$$, $$[x : y] \mapsto [x^2 : xy : y^2]$$는 image가 conic $$V(\x_0 \x_2 - \x_1^2)$$인 isomorphism을 정의한다. 이는 smooth conic이 projective line과 isomorphic함을 보여준다.
