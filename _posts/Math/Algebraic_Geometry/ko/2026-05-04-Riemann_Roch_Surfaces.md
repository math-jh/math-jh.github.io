...
<ins id="ex7">**예시 7 ($$\mathbb{P}^1 \times \mathbb{P}^1$$)**</ins> $$\mathbb{P}^1 \times \mathbb{P}^1$$을 생각하자. 이 surface의 divisor class group은 $$\mathbb{Z} \oplus \mathbb{Z}$$이며, 각 factor의 hyperplane class $$H_1, H_2$$가 generator이다. 이들은 첫 번째와 두 번째 factor를 고정하고 $$\mathbb{P}^1$$의 copy를 붙인 것들이다. 즉 기하학적으로 $$H_1$$은 첫 번째 factor의 점들에 대응하는 "수평" fiber들이고, $$H_2$$는 두 번째 factor의 점들에 대응하는 "수직" fiber들이다. 두 수평 fiber는 서로 평행이므로 만나지 않아 $$H_1^2 = 0$$이고, 마찬가지로 $$H_2^2 = 0$$이다. 반면 수평 fiber와 수직 fiber는 항상 한 점에서 만나므로 $$H_1 \cdot H_2 = 1$$이다.
...
<div class="proposition" markdown="1">
<ins id="prop12">**명제 12 (Kodaira Vanishing Theorem)**</ins> Smooth projective variety $$X$$와 ample line bundle $$L$$에 대하여,
$$H^i(X, K_X \otimes L) = 0$$
가 모든 $$i > 0$$에 대해 성립한다.
</div>

이 정리의 증명은 Bochner-Kodaira 기법을 사용하는 해석적 방법이나, Deligne-Illusie의 특수성 $$p$$ 방법 등 여러 가지가 있으며, 이 글에서는 증명을 생략한다.
