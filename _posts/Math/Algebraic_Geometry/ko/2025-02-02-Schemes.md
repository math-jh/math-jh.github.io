...
을 통해 항상 affine scheme이 된다. 이는 정의로부터 바로 확인할 수도 있지만, 나중을 위해 이를 증명해보자. 우선 canonical morphism $\epsilon: A \rightarrow A_f$를 생각하면, [§스펙트럼, ⁋명제 9](/ko/math/algebraic_geometry/spectrums#prop9)의 셋째 결과로부터 위상공간 사이의 injective continuous map

$$\Spec\epsilon:\Spec A_f \rightarrow  A$$

을 얻고 그 image는 $D(f)$와 같다. 한편, [§아핀스킴, ⁋명제 9](/ko/math/algebraic_geometry/affine_schemes#prop9)에 의해 $(\Spec\epsilon)^\sharp: \mathscr{O}\_{\Spec A} \rightarrow (\Spec\epsilon)_\ast \mathscr{O}\_{\Spec A_f}$가 정의되며 이 때 다음의 adjoint ([\[위상수학\] §층, ⁋정의 10](/ko/math/topology/sheaves#def10))
...
를 생각하면 $(\Spec\epsilon)^\sharp$은 $\Spec A_f$ 위에서의 sheaf morphism

$$(\Spec\epsilon)^{-1}\mathscr{O}_{\Spec A} \rightarrow \mathscr{O}_{\Spec A_f}$$

으로 생각할 수 있으며, [\[위상수학\] §층, ⁋정의 10](/ko/math/topology/sheaves#def10) 이후의 논증에 의해 $(\Spec\epsilon)^{-1} \mathscr{O}\_{\Spec A}$는 $\mathscr{O}\_{\Spec A}\vert_{D(f)}$와 같으므로 이는 sheaf morphism $\mathscr{O}\_{\Spec A}\vert\_{D(f)}\rightarrow \mathscr{O}\_{\Spec A_f}$와 같고 이것이 isomorphism이 된다는 것을 쉽게 보일 수 있다. 
...
<ins id="lem2">**보조정리 2**</ins> Scheme $(X, \mathscr{O}\_X)$의 임의의 열린집합 $U$가 주어졌다 하자. 그럼 $(U, \mathscr{O}\_X\vert\_U)$도 scheme이다.
...
<ins id="def3">**정의 3**</ins> Scheme $(X, \mathscr{O}\_X)$의 임의의 열린집합 $U$에 대하여, scheme $(U, \mathscr{O}\_X\vert\_U)$를 $X$의 *open subscheme<sub>열린 부분스킴</sub>*이라 부른다. 
...
를 얻으며, 따라서 [정의 4](#def4)가 affine scheme에서의 함숫값의 개념을 잘 일반화한다는 것을 안다. 그럼 이 표현에서, 임의의 $f\in \mathscr{O}\_X(X)$에 대하여

$$D(f)=\{x\in X\mid\text{$f_x\neq 0$ in $\kappa(x)$}\}=\{x\in X\mid f_x\not\in \mathfrak{m}_x\}$$

으로 정의하자. 그럼 임의의 $x\in D(f)$에 대하여, $x$를 포함하는 affine open neighborhood $U=\Spec A$를 택하면 위의 논의로부터 $U\cap X_f$는 열린집합 $D(f\vert\_{U\cap D(f)})$와 같다는 것을 알고, 따라서 [§스펙트럼, ⁋보조정리 11](/ko/math/algebraic_geometry/spectrums#lem11)로부터 $D(f)$는 $X$의 열린집합임을 안다. 

한편 점 $x\in X$에서 함수 $f$는 함숫값 뿐이 아니라, germ을 사용해도 살펴볼 수 있다. 다음을 정의하자.

<div class="definition" markdown="1">

<ins id="def5">**정의 5**</ins> Scheme $(X, \mathscr{O}\_X)$와 $f\in \mathscr{O}\_X(X)$에 대하여, $f$의 *support<sub>지지집합</sub>*는 다음 식
...
$$X\setminus \supp(f)\subseteq X\setminus D(f)\iff D(f)\subseteq \supp(f)$$
...
<ins id="ex6">**예시 6**</ins> 우리는 [§스펙트럼, §§고전적인 대수기하학](/ko/math/algebraic_geometry/spectrums#고전적인-대수기하학)에서 (classical) affine $n$-space를
...
<ins id="ex7">**예시 7**</ins> Affine plane $\mathbb{A}_\mathbb{k}^2=\Spec \mathbb{k}[\x_1,\x_2]$를 생각하자. 그럼 
...
<ins id="lem8">**보조정리 8**</ins> Index set $I$를 고정하고, 다음의 데이터들이 주어졌다 하자.
...
<ins id="ex9">**예시 9**</ins> 두 개의 affine line $X\_0=\mathbb{A}\_\mathbb{k}^1=\Spec \mathbb{k}[\x_0]$, $X\_1=\mathbb{A}\_\mathbb{k}^1=\Spec \mathbb{k}[\x_1]$, 그리고 이들의 열린집합
...
위의 [예시 9](#ex9)의 마지막 계산은 $\mathbb{P}^1$이 affine scheme이 아님을 보여준다. 만일 $\mathbb{P}^1$이 affine scheme이었다면 이는 반드시 $\Spec \mathbb{k}$가 되어야 하는데, $\Spec \mathbb{k}$는 한 점으로만 이루어진 scheme이기 때문이다. 
