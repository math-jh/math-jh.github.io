...
Quasi-projective variety $$X \subseteq Y \subseteq \mathbb{P}^n$$에는 $$Y$$로부터 물려받는 subspace topology가 존재한다. 그런데 $$X$$가 affine variety의 열린부분집합으로 표현될 수도 있고, projective variety의 열린부분집합으로 표현될 수도 있으므로, 이들의 topology가 일관되게 정의되어야 한다. 다행히 affine variety의 Zariski topology와 projective variety의 Zariski topology는 standard affine chart에서 일치하므로, subspace topology로 정의한 Zariski topology는 well-defined이다. 
...
Open affine cover가 항상 존재하는 이유는 다음과 같다. Quasi-projective variety $$X$$는 projective variety $$Y$$의 열린부분집합이고, $$Y$$는 standard affine chart $$Y \cap U_i$$들로 덮인다. 따라서 $$X$$는 $$X \cap U_i$$들로 덮이는데, 각 $$X \cap U_i$$는 affine variety $$Y \cap U_i$$의 열린부분집합이다. Affine variety의 열린부분집합은 principal open set들의 합집합으로 표현되며, principal open set $$D(f)$$는 여전히 affine variety이므로, $$X$$는 결국 affine variety들의 열린덮개를 갖는다.
...
3. $$\mathbb{A}^2 \setminus \{(0,0)\}$$에서 $$\mathcal{O}(X) = \mathbb{K}[\x, \y]$$이다. 이는 [명제 8](#prop8)에서 일반적으로 증명할 것이다. 직관적으로, codimension(여차원, 닫힌집합을 포함하는 가장 작은 irreducible 닫힌집합의 차원과의 차이) 2 이상의 닫힌집합을 제거하면 regular function이 변하지 않는다.
...
반대로 $$f$$가 [정의 6](#def6)의 의미에서 regular라 하자. 즉 open affine cover $$\{U_i\}$$가 존재하여 각 $$f\vert_{U_i} \in \mathcal{O}(U_i)$$이다. $$X$$가 Zariski 위상에서 [quasi-compact](/ko/math/topology/compact_spaces)이므로 (모든 affine variety는 [Noetherian](/ko/math/commutative_algebra/basic_notions)이고 따라서 quasi-compact), 유한개의 $$U_1, \ldots, U_r$$만으로 $$X$$를 덮을 수 있다. 각 $$U_i$$는 principal open set $$D(g_i) = \{g_i \ne 0\}$$의 형태이고, 따라서
