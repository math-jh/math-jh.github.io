...
$$\mathbb{C}[S_\sigma] = \mathbb{C}[\rchi^{e_1^\ast}, \rchi^{e_2^\ast}] = \mathbb{C}[\x, \y]$$
...
$$\chi^m$$이 well-defined한 group homomorphism임을 확인하자. 먼저 $$T_N$$의 임의의 원소는 유한합 $$t = \sum_i v_i \otimes z_i$$의 꼴로 주어지며, 이에 대해
$$\chi^m(t) := \prod_i z_i^{m(v_i)}$$
으로 정의한다. Bilinearity 관계 $$v \otimes (z_1 z_2) = v \otimes z_1 + v \otimes z_2$$, $$(v_1 + v_2) \otimes z = v_1 \otimes z + v_2 \otimes z$$, 그리고 $$(kv) \otimes z = v \otimes z^k$$ ($$k \in \mathbb{Z}$$) 하에서 이 값이 일정함은 각각
$$(z_1 z_2)^{m(v)} = z_1^{m(v)} z_2^{m(v)}, \qquad z^{m(v_1 + v_2)} = z^{m(v_1)} z^{m(v_2)}, \qquad z^{m(kv)} = z^{k \cdot m(v)} = (z^k)^{m(v)}$$
에 의해 확인되므로, 이 정의는 well-defined하다.

이제 $$t = \sum_i v_i \otimes z_i$$와 $$t' = \sum_j v_j' \otimes z_j'$$에 대해,
$$\chi^m(t + t') = \chi^m\!\left(\sum_i v_i \otimes z_i + \sum_j v_j' \otimes z_j'\right) = \prod_i z_i^{m(v_i)} \prod_j (z_j')^{m(v_j')} = \chi^m(t)\chi^m(t')$$
가 성립하므로 $$\chi^m$$은 group homomorphism이다. $$N$$의 basis $$e_1, \ldots, e_n$$과 dual basis $$e_1^\ast, \ldots, e_n^\ast \in M$$을 도입하면, $$m \in M$$은 $$m = m_1 e_1^\ast + \cdots + m_n e_n^\ast$$로 쓸 수 있고, 이에 따라
...
여기서 $$\mathbb{C}[M] = \mathbb{C}[\chi^m \mid m \in M]$$은 $$T_N$$의 coordinate ring이며, $$\rchi^u \in \mathbb{C}[S_\sigma]$$는 weight $$u$$를 갖는 eigenvector이다. $$\rho$$가 well-defined한 algebra homomorphism임은 bilinearity에 의해 직접 확인할 수 있다.
...
$$u \in S_\sigma$$이고 $$\tau = \sigma \cap u^\perp$$라고 하자. 그러면 dual cone의 성질에 의해 $$\tau^\vee = \sigma^\vee + \mathbb{R}_{\ge 0}(-u)$$이 성립한다. 따라서

$$S_\tau = \tau^\vee \cap M = (\sigma^\vee + \mathbb{R}_{\ge 0}(-u)) \cap M = S_\sigma + \mathbb{Z}_{\ge 0}(-u)$$

이다. 이로부터 semigroup algebra의 localization이

$$\mathbb{C}[S_\tau] = \mathbb{C}[S_\sigma + \mathbb{Z}_{\ge 0}(-u)] = \mathbb{C}[S_\sigma][\chi^{-u}] = \mathbb{C}[S_\sigma]_{\chi^u}$$

임을 얻는다. 여기서 $$\chi^u \in \mathbb{C}[S_\sigma]$$이고 $$\chi^{-u}$$는 $$\chi^u$$의 역원이다. 따라서

$$U_\tau = \Spec(\mathbb{C}[S_\tau]) = \Spec(\mathbb{C}[S_\sigma]_{\chi^u}) = (U_\sigma)_{\chi^u} = \{ x \in U_\sigma \mid \chi^u(x) \neq 0 \}$$
...
**1. 정규성(Normality).** Gordan의 보조정리(Gordan's lemma)에 의해 $$S_\sigma = \sigma^\vee \cap M$$은 finitely generated semigroup이다. 더욱이 $$S_\sigma$$는 *saturated*이다: 즉 $$k \cdot u \in S_\sigma$$인 $$k \in \mathbb{Z}_{>0}$$와 $$u \in M$$에 대해 $$u \in S_\sigma$$이다. 이는 $$\langle u, v \rangle \ge 0$$가 $$\sigma$$ 위에서 성립함을 의미한다. 따라서 $$\mathbb{C}[S_\sigma]$$는 normal domain이 되고, 그 스펙트럼인 $$U_\sigma$$는 정규 다양체이다.
...
**3. 차원(Dimension).** $$\mathbb{C}[S_\sigma]$$의 fraction field는 $$\mathbb{C}(M)$$을 포함한다. $$M \cong \mathbb{Z}^n$$이므로 $$\mathbb{C}(M)$$은 $$n$$개의 독립 변수에 대한 유리함수체이며, 이의 transcendence degree는 $$n$$이다. 한편 $$S_\sigma$$가 $$\sigma^\vee$$와 $$M$$의 교집합이고 $$\sigma$$가 strongly convex이므로 $$\sigma^\vee$$는 $$M_{\mathbb{R}}$$에서 full-dimensional cone을 이룬다. 따라서 $$S_\sigma$$가 생성하는 group은 $$M$$의 finite index subgroup이 되고, 그 fraction field의 transcendence degree는 여전히 $$n$$이다. 이로부터 $$\dim U_\sigma = n$$이 성립한다.
...
$$U_\sigma = \Spec(\mathbb{C}[M]) = T_N$$

이 성립한다.

일반적인 경우, $$S_\sigma \subseteq M$$이므로 $$\mathbb{C}[S_\sigma] \subseteq \mathbb{C}[M]$$이다. 이는 자연스러운 dominant morphism

$$T_N = \Spec(\mathbb{C}[M]) \longrightarrow \Spec(\mathbb{C}[S_\sigma]) = U_\sigma$$

을 유도한다. 더욱이 $$\sigma$$가 strongly convex rational polyhedral cone이므로 $$\sigma^\vee$$는 $$M_{\mathbb{R}}$$에서 non-empty interior를 가지며, $$S_\sigma = \sigma^\vee \cap M$$이 생성하는 group은 $$M$$과 같은 rank를 갖는다. 따라서 $$\mathbb{C}[S_\sigma]$$의 fraction field는 $$\mathbb{C}(M)$$과 일치하고, $$T_N \hookrightarrow U_\sigma$$는 열린 조밀한 부분집합이 된다.
