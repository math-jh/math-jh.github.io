...
을 정의한다. ([\[대수적 구조\] §대수, ⁋정의 5](/ko/math/algebraic_structures/algebras#def5){: data-lid="117ax" data-relation="weak" }) 여기서 $\rchi^u$는 $M$의 원소 $u$에 대응되는 monomial이다.
:::
...
위의 [예시 6](#ex6){: data-lid="c33hh" data-relation="weak" }의 $\sigma = \{0\}$과 [예시 7](#ex7){: data-lid="v0sbv" data-relation="weak" }의 standard quadrant $\sigma = \mathrm{cone}(e_1, e_2)$에서 $U_\sigma$가 각각 $(\mathbb{C}^\ast)^n$, $\mathbb{C}^2$로 모두 smooth algebraic variety였다. 이제 우리는 이것이 우연이 아닐 뿐만 아니라 $U_\sigma$의 smoothness가 cone $\sigma$의 조합론적 데이터만으로 완전히 판정된다는 것을 살펴볼 것이다.
...
특히 $N = \mathbb{Z}^2$에서 2차원 cone에 대해서는 두 ray generator로 만든 $2 \times 2$ 행렬의 determinant가 $\pm 1$인지만 확인하면 되고, 실제로 [예시 7](#ex7){: data-lid="cwte2" data-relation="weak" }은 이를 만족한다. 
...
::: 명제 13
Cone $\sigma$의 face $\tau$에 대하여, $U_\tau$는 $U_\sigma$의 principal open subset이다. ([\[대수다양체\] §아핀다양체, ⁋정의 5](/ko/math/algebraic_varieties/affine_varieties#def5){: data-lid="kc019" data-relation="required" }) 구체적으로, $u \in S_\sigma$를 $\tau = \sigma \cap u^{\perp}$를 만족하는 것으로 선택하면
...
::: 증명
$u \in S_\sigma$이고 $\tau = \sigma \cap u^\perp$라고 하자. [보조정리 12](#lem12){: data-lid="cww19" data-relation="required" }에 의해 $\tau^\vee = \sigma^\vee + \mathbb{R}_{\ge 0}(-u)$이 성립하므로,
...
가령 [예시 7](#ex7){: data-lid="jz4bh" data-relation="weak" }의 2차원 cone $\sigma$의 경우, $\sigma$의 face $\tau_1=\mathbb{R}_{\geq 0}e_1$과 $\tau_2=\mathbb{R}_{\geq 0}e_2$는 그 각각 또한 1차원 cone이 되며, 이들의 face는 원점이다. 이 구조 하에서 전체 $U_\sigma$ 안에서 $U_{\tau_1}$과 $U_{\tau_2}$ 각각은 principal open set이며, 이러한 face 구조에 따라 $U_\sigma$가 CW complex와 유사한 stratification을 가지게 된다. 
...
::: 예시 14
[예시 7](#ex7){: data-lid="w82de" data-relation="required" }에서 $N = \mathbb{Z}^2$이고 $\sigma = \mathbb{R}_{\geq 0}e_1+ \mathbb{R}_{\geq 0}e_2$일 때 $U_\sigma = \mathbb{C}^2$임을 보았다. 이제 torus $T_N = (\mathbb{C}^\ast)^2$가 $U_\sigma = \mathbb{C}^2$ 위에 어떻게 작용하는지 구체적으로 살펴보자.

우선 [예시 7](#ex7){: data-lid="5hr15" data-relation="required" }에서 본 대로 $\mathbb{C}[S_\sigma] = \mathbb{C}[\z_1, \z_2]$이며 ($\z_i = \rchi^{e_i^\ast}$), $U_\sigma = \Spec(\mathbb{C}[\z_1, \z_2]) = \mathbb{C}^2$이다. 이제 [명제 10](#prop10){: data-lid="k15rn" data-relation="required" }에 따르면, $t = (t_1, t_2) \in T_N = (\mathbb{C}^\ast)^2$는 coordinate ring 위에 다음의 식
...
이제 이 action의 orbit 구조를 살펴보자. [명제 11](#prop11){: data-lid="0x8w0" data-relation="required" }이 주는 open dense orbit은 
