...
이제 위와 같이 torus에 대한 이해를 바탕으로 $$U_\sigma = \Spec(\mathbb{C}[S_\sigma])$$ 위에 $$T_N$$-action을 정의한다. $$\Spec$$은 contravariant functor이므로, $$U_\sigma$$ 위에 $$T_N$$이 작용하는 것은 coordinate ring $$\mathbb{C}[S_\sigma]$$ 위에 comodule structure를 주는 것과 같다.
