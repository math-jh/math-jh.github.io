...
Cox ring $$S = \mathbb{C}[\x_\rho \mid \rho \in \Sigma(1)]$$의 $$\beta$$차 성분은 monomial $$\prod_\rho \x_\rho^{a_\rho}$$들로 생성되며, 여기서 $$\sum_\rho a_\rho D_\rho$$가 class $$\beta$$를 갖습니다. 한편, [§Toric divisors와 line bundles, ⁋명제 8](/ko/math/toric_geometry/toric_divisors#prop8)에 의해, $$H^0(X_\Sigma, \mathcal{O}_{X_\Sigma}(D))$$의 원소는 lattice point $$m \in M$$에 대응하는 character $$\chi^m$$들로 이루어지며, 이들은 $$\langle m, u_\rho \rangle \geq -a_\rho$$를 만족합니다. 이 조건은 $$\beta$$차 monomial의 정의와 정확히 일치하므로, 자연스러운 isomorphism이 존재한다.
