...
이는 universal property이므로 $Y$가 존재한다면 유일성은 자동이다. 다만 정의는 $Y$가 한 점으로 붕괴할 가능성을 배제하지 않으며, 위에서 본 $\mathbb{C}^\ast$의 scaling 예시에서는 실제로 categorical quotient가 한 점에 불과하다 ([예시 5](#ex5){: data-lid="0geno" data-relation="required" }). 즉 categorical quotient는 *너무 적은 정보*를 담을 수 있다.
...
Geometric quotient는 통상 $Y = X/G$로 표기하며, $Y$의 점은 정확히 $X$의 $G$-orbit과 일대일 대응한다. Categorical quotient는 closure가 서로 만나는 orbit들을 한 점으로 묶지만 ([명제 3](#prop3){: data-lid="bxt9h" data-relation="required" }), geometric quotient는 모든 orbit을 분리한다. 직관적으로 두 개념 사이의 간극은 $X$ 안에 *다른 orbit의 closure에 포함되는 orbit*이 있을 때 발생한다.
...
으로 작용한다고 하자. 우선 trivial linearization, 즉 character $\chi = 1$의 경우 affine GIT을 본다. Coordinate ring $A = \mathbb{C}[\z_1, \z_2]$ 위의 $G$-action은 (이 글에서는 자매 글인 [§아핀 토릭 다양체](/ko/math/toric_geometry/affine_toric_varieties){: data-lid="myhcf" data-relation="weak" }의 *inverse 없는* convention $t \cdot f = f \circ t$와는 반대로, 표준 GIT 컨벤션을 따라 $(t \cdot f)(x) = f(t^{-1} x)$를 채택한다) $\z_i$를 $t^{-1}\z_i$로 보내며, 임의의 monomial $\z_1^a \z_2^b$는 $t^{-(a+b)}$ 배가 된다. 따라서

$$A^G = \mathbb{C}$$

이고, [명제 3](#prop3){: data-lid="1puvs" data-relation="required" }에 의해

$$\mathbb{C}^2 /\!/ \mathbb{C}^\ast = \Spec \mathbb{C} = \{\mathrm{pt}\}$$

으로 한 점으로 붕괴한다. 모든 비원점 orbit의 closure가 원점을 포함하여 [명제 3](#prop3){: data-lid="m4xmz" data-relation="required" }에 의해 모든 점이 같은 equivalence class에 속한 결과이다.
...
즉 $n$차 homogeneous polynomial들의 공간이다. 따라서
...
이고, semistable 집합은 $\mathbb{C}^n \setminus \{0\}$이다. 이는 통상의 projective space 정의
...
이제 $S$ 위의 grading을 도입한다. [§토러스 인자와 선다발, ⁋정의 1](/ko/math/toric_geometry/toric_divisors#def1){: data-lid="1jcbv" data-relation="required" }에서 살펴본 torus-invariant prime divisor $D_\rho$에 대해, [§토러스 인자와 선다발, ⁋명제 4](/ko/math/toric_geometry/toric_divisors#prop4){: data-lid="zgsgx" data-relation="required" }에서 우리는 Weil divisor class group $\Cl(X_\Sigma)$가 exact sequence
...
로부터 얻어짐을 확인하였다 (첫 번째 arrow는 [§토러스 인자와 선다발, ⁋명제 3](/ko/math/toric_geometry/toric_divisors#prop3){: data-lid="772c0" data-relation="required" }에 의해 $m \mapsto \sum_\rho \langle m, u_\rho \rangle D_\rho$). 이 exact sequence에 $\Hom_\mathbb{Z}(-, \mathbb{C}^\ast)$를 적용하면
...
::: 증명
Cox의 원래 논문의 증명을 따라간다. 먼저 $G$가 $\mathbb{C}^{\Sigma(1)} \setminus Z(\Sigma)$ 위에 잘 정의된 action을 가짐을 본다: $Z(\Sigma)$는 coordinate subspace들의 union이며 $G$의 action은 $(\mathbb{C}^\ast)^{\Sigma(1)}$의 action으로부터 유도된 diagonal action이므로 coordinate subspace들을 보존한다.
...
Simplicial인 경우, 각 cone이 simplicial이므로 local chart $U_\sigma$에 대한 action의 stabilizer가 유한군이 되어 ([명제 3](#prop3){: data-lid="rjnto" data-relation="required" }의 orbit closure 기준이 단순화됨) geometric quotient가 된다. 역으로 geometric quotient이려면 stabilizer가 유한해야 하고, 이는 각 cone이 simplicial임을 의미한다.
:::
...
::: 예시 10 (projective space)
$\mathbb{P}^n$의 fan $\Sigma$는 원점을 center로 하는 $n+1$개의 1차원 cone $\rho_0, \ldots, \rho_n$으로 구성되며, primitive generator는 $u_0, \ldots, u_{n-1}$이 표준기저이고 $u_n = -u_0 - \cdots - u_{n-1}$이다. 각 maximal cone $\sigma_i = \mathrm{cone}(u_0, \ldots, \hat{u}_i, \ldots, u_n)$에 대해
...
이므로 $G = \Hom_\mathbb{Z}(\mathbb{Z}, \mathbb{C}^\ast) = \mathbb{C}^\ast$이고, 그 action은 diagonal scaling

$$t \cdot (\x_0, \ldots, \x_n) = (t\x_0, \ldots, t\x_n)$$

이다. 그러므로 [명제 9](#prop9){: data-lid="gsbwd" data-relation="forward" }는 익숙한

$$\mathbb{P}^n = (\mathbb{C}^{n+1} \setminus \{0\}) / \mathbb{C}^\ast$$

를 재현한다. 이는 [예시 6](#ex6){: data-lid="qvhn9" data-relation="weak" }에서 본 GIT 시점과 정확히 일치한다.
:::
...
Cox 구성은 toric variety의 birational geometry를 이해하는 데도 강력한 도구이다. [§토릭 다양체의 정의, ⁋명제 8](/ko/math/toric_geometry/toric_varieties#prop8){: data-lid="l0u8x" data-relation="required" }에서 보았듯이 lattice polytope으로부터 projective toric variety가 구성되며, Cox 구성의 관점에서 이러한 polytope의 변화는 GIT quotient에서 linearization의 변화에 정확히 대응한다.
