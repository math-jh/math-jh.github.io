...
<ins id="def5">**정의 5**</ins> $$X_\Sigma$$의 *toric boundary<sub>토릭 경계</sub>* $$D \subset X_\Sigma$$를

$$D = X_\Sigma \setminus T_N = \sum_{\rho \in \Sigma(1)} D_\rho$$

로 정의한다 (reduced divisor로 본다). $$X_\Sigma$$의 *logarithmic cotangent sheaf<sub>로그 코탄젠트 층</sub>* $$\Omega^1_{X_\Sigma}(\log D)$$는 $$D$$를 따라 1차 이하의 극을 갖는 rational 1-form들로 이루어진 $$\mathcal{O}_{X_\Sigma}$$-module subsheaf이다. 구체적으로 affine open $$U \subset X_\Sigma$$에 대해

$$\Omega^1_{X_\Sigma}(\log D)(U) = \{\omega \in \Omega^1_{\mathbb{C}(X_\Sigma)/\mathbb{C}} \mid \omega \text{와 } d\omega \text{가 } U \text{ 위에서 } D\text{를 따라 1차 이하의 극을 가짐}\}$$

로 주어지며, 더 높은 차수의 $$\Omega^p_{X_\Sigma}(\log D) := \bigwedge^p \Omega^1_{X_\Sigma}(\log D)$$를 외적으로 정의한다.

</div>

위 정의는 임의의 normal crossing divisor에 대해 Saito-Deligne가 도입한 표준적인 정의이며, $$X_\Sigma$$가 smooth toric variety일 때 boundary $$D$$가 정확히 simple normal crossing이라는 점이 핵심적이다. ([§아핀 토릭 다양체, ⁋명제 9](/ko/math/toric_geometry/affine_toric_varieties#prop9)에서 smooth cone $$\sigma$$가 $$N$$의 기저의 일부 $$v_1, \ldots, v_k$$로 생성되어 $$U_\sigma \cong \mathbb{C}^k \times (\mathbb{C}^\ast)^{n-k}$$이고, boundary는 좌표 hyperplane $$\{\x_i = 0\}_{i=1, \ldots, k}$$의 합이므로 simple normal crossing이다.) 한편 $$\Omega^1_{X_\Sigma}$$ 자체는 일반적인 toric variety에 대해 trivial bundle이 *아니지만*, $$\log D$$ twist를 가하면 놀랍게도 항상 trivial이 된다는 것이 다음 결과의 핵심이다.

<div class="proposition" markdown="1">

<ins id="prop6">**명제 6**</ins> Smooth toric variety $$X_\Sigma$$에 대해 자연스러운 isomorphism
...
이 명제는 두 가지 측면에서 매우 강한 결과이다. 첫째, [정의 5](#def5)는 임의의 simple normal crossing divisor에 대해서도 의미가 있지만, 일반적인 pair $$(X, D)$$에 대해서는 $$\Omega^1_X(\log D)$$가 결코 trivial bundle이 되지 않는다. Toric variety의 경우에만 boundary $$D$$가 character lattice $$M$$ 전체의 정보를 정확히 흡수해주어 $$\Omega^1(\log D)$$가 rank $$n$$의 trivial bundle이 되는 것이며, 이는 fan의 ray 구조가 $$M$$의 전 정보를 결정한다는 toric duality의 또 다른 표현이라 할 수 있다.
...
<ins id="ex7">**예시 7**</ins> $$X_\Sigma = \mathbb{P}^1$$의 경우, fan은 $$N = \mathbb{Z}$$에서 두 ray $$\rho_+ = \mathbb{R}_{\ge 0} \cdot 1$$과 $$\rho_- = \mathbb{R}_{\ge 0} \cdot (-1)$$로 이루어지며 boundary는 $$D = D_{\rho_+} + D_{\rho_-} = \{0\} + \{\infty\}$$이다. $$M = \mathbb{Z}$$의 generator $$1 \in M$$에 대해 $$\rchi^1 = \x$$가 $$\mathbb{P}^1$$의 standard inhomogeneous 좌표이며

$$d\log \x = \frac{d\x}{\x}$$

는 $$\x = 0$$과 $$\x = \infty$$ 양쪽에서 1차 pole만을 가지는 logarithmic form이다 ($$\x = \infty$$ 근방에서는 $$\y = 1/\x$$로 좌표를 잡으면 $$d\x/\x = -d\y/\y$$이므로 logarithmic pole임이 확인된다). [명제 6](#prop6)이 주장하는 바는 $$\Omega^1_{\mathbb{P}^1}(\log D) \cong \mathcal{O}_{\mathbb{P}^1}$$이 $$d\x/\x$$를 generator로 갖는 trivial bundle이라는 것이며, 이는 $$\Omega^1_{\mathbb{P}^1} \cong \mathcal{O}_{\mathbb{P}^1}(-2)$$가 *not* trivial인 것과 비교된다.

</div>

<div class="example" markdown="1">

<ins id="ex8">**예시 8**</ins> $$X_\Sigma = \mathbb{P}^n$$의 경우 fan은 $$N = \mathbb{Z}^n$$의 standard 기저 $$e_1, \ldots, e_n$$과 $$e_0 = -e_1 - \cdots - e_n$$의 $$n+1$$개 ray로 이루어진다. 각 $$e_i$$에 대응하는 divisor를 $$D_i$$라 하면 boundary는 $$D = D_0 + D_1 + \cdots + D_n$$이고 $$[D_0] = [D_1] = \cdots = [D_n] = H$$이므로 $$D \sim (n+1) H$$이다. $$M$$의 standard 기저 $$e_1^\ast, \ldots, e_n^\ast$$에 대해 $$\rchi^{e_i^\ast} = \x_i$$ ($$1 \le i \le n$$)가 affine 좌표가 되어
...
<ins id="ex9">**예시 9**</ins> $$X_\Sigma = \mathbb{P}^1 \times \mathbb{P}^1$$의 경우 $$N = \mathbb{Z}^2$$이고 fan은 네 개의 ray $$\pm e_1, \pm e_2$$와 네 개의 maximal cone으로 이루어진다. Boundary는 네 boundary divisor의 합이며, $$M = \mathbb{Z}^2$$의 standard 기저에 대해
...
와 [예시 7](#ex7)과 비교하면 두 인자가 정확히 $$d\x/\x$$, $$d\y/\y$$를 기여함을 알 수 있다.

</div>

<div class="remark" markdown="1">

<ins id="rmk10">**참고 10**</ins> [명제 6](#prop6)은 smooth toric variety를 가정하였다. $$X_\Sigma$$가 simplicial이지만 smooth가 아닌 경우에도 $$\mathbb{Q}$$-Cartier 수준에서 비슷한 진술이 성립하며, 더 일반적인 normal toric variety에 대해서는 *Zariski differentials* $$\widehat{\Omega}^1_{X_\Sigma}(\log D)$$를 사용하면 동일한 fan-theoretic 묘사가 가능하다. 자세한 논의는 Cox-Little-Schenck의 §8.1–8.2 참고.

</div>

## 표준 인자의 fan-이론적 공식

[명제 6](#prop6)의 즉각적인 귀결로, top form $$\omega_\Sigma := \bigwedge_i d\log(\rchi^{m_i})$$가 $$X_\Sigma$$ 위의 $$\Omega^n_{X_\Sigma}(\log D)$$의 nowhere-vanishing global section이 되는 것을 알 수 있다. 이 형식은 정확히 canonical bundle의 정보와 연결되며, 이를 통해 [§파노 다양체, ⁋명제 3](/ko/math/toric_geometry/reflexive_polytope_fano#prop3)에서 이미 진술된 $$K_{X_\Sigma} = -\sum_\rho D_\rho$$를 fan으로부터 직접 유도할 수 있다.

<div class="proposition" markdown="1">

<ins id="prop11">**명제 11**</ins> Smooth toric variety $$X_\Sigma$$에 대해, $$M$$의 임의의 $$\mathbb{Z}$$-기저 $$m_1, \ldots, m_n$$를 잡으면 top form
...
[명제 11](#prop11)의 $$\omega_\Sigma$$가 갖는 또 하나의 핵심적 응용은 anticanonical hypersurface 위의 자연스러운 holomorphic volume form 구성이다. $$X_\Sigma$$가 Gorenstein Fano toric variety라 가정하고 ([§파노 다양체, ⁋정의 2](/ko/math/toric_geometry/reflexive_polytope_fano#def2)), $$f \in H^0(X_\Sigma, \mathcal{O}(-K_{X_\Sigma}))$$를 anticanonical section이라 하자. 그럼 $$Y := \{f = 0\} \subset X_\Sigma$$는 $$-K_{X_\Sigma}$$ linear system 내의 hypersurface이며, [§파노 다양체, ⁋명제 7](/ko/math/toric_geometry/reflexive_polytope_fano#prop7)의 adjunction에 의해 $$Y$$가 smooth이면 $$K_Y = 0$$, 즉 Calabi-Yau hypersurface가 된다.

이 경우 *Poincaré residue* 

$$\Res_Y\!\left(\frac{\omega_\Sigma}{f}\right) \in H^0(Y, \omega_Y)$$

가 정의되며, 이것이 $$Y$$ 위의 nowhere-vanishing canonical volume form을 정의한다. 핵심은 $$\omega_\Sigma$$가 [명제 11](#prop11)에 의해 $$\divisor(\omega_\Sigma) = -D$$를 가지므로, $$f$$가 anticanonical, 즉 $$\divisor(f) \sim D$$인 한 $$\omega_\Sigma / f$$가 $$Y$$를 따라 정확히 1차 pole을 갖는 rational $$n$$-form이라는 점이다. Residue map은 이러한 simple pole form을 hypersurface 위의 holomorphic $$(n-1)$$-form으로 떨어뜨려준다.
