...
이제 character가 정의하는 principal divisor를 살펴 본다. 각각의 $$m \in M$$에 대해, character $$\chi^m: T_N \to \mathbb{C}^\ast$$는 $$X_\Sigma$$ 위의 유리함수로 볼 수 있다. [§아핀 토릭 다양체, ⁋명제 8](/ko/math/toric_geometry/affine_toric_varieties#prop8)에서 보았듯이, $$\chi^m$$은 $$T_N$$ 위에서 eigenvalue $$\chi^m(t)$$를 갖는 rational function이다.
...
Line bundle의 가장 중요한 성질 중 하나는 ample함이다. [\[대수다양체\] §선다발과 벡터다발, ⁋정의 5](/ko/math/algebraic_varieties/line_bundles#def5)에서 정의한 것처럼, line bundle이 ample하다는 것은 해당 divisor가 projective 임베딩을 유도한다는 것이다. Toric variety에서 이 기하학적 조건은 support function의 볼록성으로 완전히 기술된다.
...
(3) 자연스러운 inclusion $$\CDiv_T(X_\Sigma) \hookrightarrow \Div_T(X_\Sigma)$$는 quotient에 의해 $$\Pic(X_\Sigma) \to \Cl(X_\Sigma)$$를 유도한다. 이 homomorphism의 kernel은 Cartier divisor 중에서 principal divisor와 linearly equivalent한 Weil divisor들의 모임이며, 이는 정확히 $$\CDiv_T(X_\Sigma) \cap \div(\mathbb{C}(X_\Sigma)^\times)$$에 대응한다. 위의 commutative diagram은 자연스러운 inclusion으로부터 유도된다.
