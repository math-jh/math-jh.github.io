...
우리가 관심있는 것은 toric variety의 line bundle이므로, 우리는 *Cartier* divisor들에 집중해야 한다. Cartier divisor를 support function의 언어로 기술하고 line bundle과 잇는 자세한 논의는 다음 글인 [§Toric divisor와 line bundle](/ko/math/toric_geometry/toric_divisors)에서 다룰 것이지만, 이번 글에서의 논의를 위해 우리는 우선 [§Toric divisor와 line bundle, ⁋명제 6](/ko/math/toric_geometry/toric_divisors#prop6)의 결과를 먼저 번역해오기로 한다. 
...
($$\Rightarrow$$) $$X_\Sigma$$가 projective라면 그 위에 very ample line bundle $$\mathcal{L}$$이 존재하며, 위의 대응에 의해 이는 $$T_N$$-invariant Cartier divisor $$D$$로 표현된다. 앞서 본 바, 이로부터 얻어지는 $$P_D$$는 lattice polytope이며, $$\mathcal{L}$$이 very ample이라는 사실은 대응되는 support function $$\psi_D$$가 strictly convex임과 동치이고 ([§Toric divisor와 line bundle, ⁋명제 10](/ko/math/toric_geometry/toric_divisors#prop10)), 이로부터 $$P_D$$의 normal fan이 $$\Sigma$$와 일치함을 얻는다.
...
이제 우리는 가장 기본적인 projective toric variety, 즉 projective space $$\mathbb{P}^n$$의 예시를 살펴보며 글을 마친다.
...
일반 $$n$$에 대해 $$\Sigma_{\Delta_n}$$은 $$\mathbb{P}^n$$의 표준 fan과 일치하므로

$$X_{\Delta_n} \cong \mathbb{P}^n$$

이 성립한다. 또한 $$\Delta_n$$의 lattice points는 정확히 꼭짓점들 $$\{0, e_1, \ldots, e_n\}$$이므로 ($$\Delta_n$$의 unimodular 구조로부터), 대응하는 monomial map은
