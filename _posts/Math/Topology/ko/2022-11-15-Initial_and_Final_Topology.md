...
[§연속함수](/ko/math/topology/continuous_functions){: data-lid="z7rab" }에서 우리는 연속함수 $f:X \rightarrow Y$에 대하여 다음의 식 
...
::: 명제 2
[정의 1](#def1){: data-lid="vmp2g" }의 initial topology는 정확하게 다음의 집합
...
::: 명제 3
[정의 1](#def1){: data-lid="2oq1d" }의 상황에서, 추가로 위상공간 $Z$와 $g:Z\rightarrow X$가 주어졌다 하자. 그럼 $g$가 연속인 것은 각각의 $f_i\circ g$가 연속인 것과 동치이다. 
:::
::: 증명
만일 $g$가 연속이라면 $f_i\circ g$는 연속함수들의 합성이므로 자명하게 연속이다. 따라서 반대방향만 보이자.

각각의 함수 $f_i\circ g$가 연속이라 하자. $X$의 임의의 열린 진부분집합 $U$에 대하여, [명제 2](#prop2){: data-lid="jb8nc" }에 의해 
...
::: 명제 5
[정의 4](#def4){: data-lid="or8n8" }의 위상은 다음의 집합
...
:::
::: 증명
주어진 위상 $\mathcal{T}_\fin$가 실제로 위상이 된다는 것은 쉽게 확인할 수 있다. 따라서 $\mathcal{T}_\fin$가 [정의 4](#def4){: data-lid="tpw0n" }의 조건을 모두 만족한다는 것만 보이면 충분하다. 
...
::: 명제 6
[정의 4](#def4){: data-lid="w03d2" }의 상황에서, 추가로 위상공간 $Z$와 $g:X\rightarrow Z$가 주어졌다 하자. 그럼 $g$가 연속인 것은 각각의 $g\circ f_i$가 연속인 것과 동치이다. 
:::
::: 증명
...
이 $Y_i$에서 각각 열린집합이다. 그런데 [명제 5](#prop5){: data-lid="hyckx" }에 의하여, 이는 곧 $g^{-1}(U)$가 $X$에서 열린집합이라는 것과 같은 말이고 따라서 $g$는 연속이다.
:::
