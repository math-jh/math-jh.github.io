...
앞서 살펴본 [§부분공간, ⁋명제 8](/ko/math/topology/subspaces#prop8){: data-lid="o5hv3" }은 연속함수 $f:X \rightarrow Y$가 주어졌다 할 때, 이를 [§부분공간, ⁋명제 6](/ko/math/topology/subspaces#prop6){: data-lid="ifhee" }의 두 조건 중 하나를 만족하는 부분집합들의 family로 제한한 것이 연속임을 말해준다. 특히 이 조건은 다음 두 조건
...
::: 보조정리 1
위상공간 $X$와 [§부분공간, ⁋명제 6](/ko/math/topology/subspaces#prop6){: data-lid="btg09" }의 두 조건 중 하나를 만족하는 부분집합들의 family $(A_i)$가 주어졌다 하자. 만일 연속함수들의 family $(f_i: A_i \rightarrow Y)$가 다음 조건
...
:::
::: 증명
우선 함수 $f$는 [\[집합론\] §집합의 합, ⁋명제 2](/ko/math/set_theory/sum_of_sets#prop2){: data-lid="jm8pb" }에 의해 얻어진다. 이 함수가 연속이라는 것은 [§부분공간, ⁋명제 8](/ko/math/topology/subspaces#prop8){: data-lid="xv91t" }에서 얻어진다.
:::
...
특별히 이 정의는 projection $p:Y \rightarrow X$가 주어졌을 때, 열린집합 $U$마다 $U$ 위에서 정의된 $p$의 continuous section들의 ([\[집합론\] §Retraction과 section, ⁋정의 2](/ko/math/set_theory/retraction_and_section#def2){: data-lid="vr0o9" }) 모임을 대응시키는 presheaf $\mathcal{F}$를 생각하는 식으로 응용할 수 있다. 이를 일반화하여 다음과 같은 정의를 내린다. 
...
으로 정의할 수 있다. 그럼 $\mathcal{F}\vert_U$는 presheaf가 된다. ([§부분공간, ⁋보조정리 2](/ko/math/topology/subspaces#lem2){: data-lid="9cnyb" }) 
:::
...
이 존재한다. 이제 우리는 $\Spe(\mathcal{F})$에 이들 함수들의 family가 정의하는 final topology를 부여하고 ([§Initial topology와 final topology, ⁋정의 4](/ko/math/topology/initial_and_final_topology#def4){: data-lid="ywi2i" }) 이 공간을 $\mathcal{F}$의 *étalé space*라 부른다.
...
즉 $X$ 위에서 정의된 $\mathcal{A}$-valued presheaf들의 category는 functor category $[\Open(X)^\op, \mathcal{A}]$이다. 이를 $\PSh(X; \mathcal{A})$로 표기하며, 문맥상 혼동의 여지가 없을 때에는 $\PSh(X)$로만 적기도 한다. 여담으로 [예시 8](#ex8){: data-lid="9024c" }의 $f_\ast$는 functor $\PSh(X; \mathcal{A})\rightarrow \PSh(Y; \mathcal{A})$이다. 

우리에게 직관을 주는 [예시 3](#ex3){: data-lid="sr494" }를 생각해보면, 열린집합 $U$에 대하여 정의된 $\phi(U):\mathcal{F}(U) \rightarrow \mathcal{G}(U)$는 $\phi:\mathcal{F}\rightarrow \mathcal{G}$를 열린집합 $U$로 제한하여 얻어지는 함수라 생각할 수 있으므로, 이를 종종 $\phi(U)$ 대신 $\phi\vert_U$로 적는다. 
...
다음 예시들은 위의 [준층의 예시들](#준층의-예시들){: data-lid="mjdmf" } 아래에 있었어야 했지만, 아직 presheaf morphism을 정의하지 않았었기 때문에 뒤로 밀렸다. 
