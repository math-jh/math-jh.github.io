...
[\[집합론\] §합집합과 교집합](/ko/math/set_theory/union_and_intersection){: data-relation="weak" }에서의 관례를 도입하면 위 정의의 둘째 조건과 셋째 조건에서 두 집합
...
:::
::: 증명
[정의 4](#def4){: data-relation="required" }을 통해 뒤의 조건을 풀어쓰면
...
둘째 성질을 보이기 위해, $V_1,\ldots, V_n\in\mathcal{N}(x)$가 주어졌다 하자. 그럼 $x\in U_i\subseteq V_i$이도록 하는 $x$의 열린근방들 $U_1,\ldots, U_n$이 존재한다. 이제 [정의 1](#def1){: data-relation="required" }의 셋째 조건에 의하여 이들의 유한한 교집합 $U_1\cap\cdots\cap U_n$도 열린집합이고, 따라서
