...
::: Proposition 2
Let $\mathcal{C}$ be a collection of subsets of a set $X$ satisfying the following conditions.
...
::: Definition 5
For any subset $A$ of a topological space $X$, the smallest closed set containing $A$ is called the *closure* of $A$, and the largest open set contained in $A$ is called the *interior* of $A$; we denote them by $\cl(A)$ and $\interior(A)$, respectively.
:::
...
::: Definition 10
For any subset $A$ of a topological space $X$, $A$ is called a *dense subset* if $\cl(A)=X$.
:::
