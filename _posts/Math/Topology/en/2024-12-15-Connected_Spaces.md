...
:::
::: Proof
The reverse direction follows immediately by applying [Proposition 4](#prop4){: data-relation="required" } to the projections $\pr_i$, so there is nothing to prove.
...
Moreover, by [Proposition 2](#prop2){: data-relation="required" } each $U_i$ must be closed. If $I$ is finite, then the $U_i$ are all simultaneously open and closed. Of course this does not apply when there are infinitely many connected components, but any clopen set in an arbitrary topological space must be a union of connected components. For if some connected component $C$ met a clopen set $A$ and also met its complement, then $C\cap A$ and $C\setminus A$ would be two open sets partitioning $C$.
