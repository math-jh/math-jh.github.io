...
By [§Interior, Closure, and Boundary, ⁋Proposition 2](/en/math/topology/other_concepts#prop2){: data-lid="6rdoy" data-relation="required" }, defining a collection $\mathcal{C}$ of subsets of $X$ that tells us *which sets are closed* suffices to give $X$ a topological structure. This is hardly different from the original definition, but it is particularly useful when defining the Zariski topology in algebraic geometry.
...
A function satisfying these conditions is called a *closure operator*. ([\[Set Theory\] §Filters, Ideals, and Galois Connections, ⁋Definition 8](/en/math/set_theory/filter_and_ideal#def8){: data-lid="rqkh0" data-relation="weak" })
:::
...
::: Theorem 2
Suppose a function $\cl:\mathcal{P}(X)\rightarrow\mathcal{P}(X)$ satisfying all the conditions of [Definition 1](#def1){: data-lid="es421" data-relation="required" } is given. Define $\mathcal{C}$ to be <phrase>the collection of all $C$ satisfying $\cl(C)=C$</phrase>. Then $\mathcal{C}$ satisfies all the conditions of [§Interior, Closure, and Boundary, ⁋Proposition 2](/en/math/topology/other_concepts#prop2){: data-lid="hnwpo" data-relation="required" }, and therefore defines a unique topological structure.
:::
...
Regarding the ordered set $(\mathcal{P}(X),\subseteq)$, the above definition coincides with that given in [\[Set Theory\] §Filters, Ideals, and Galois Connections, ⁋Definition 1](/en/math/set_theory/filter_and_ideal#def1){: data-lid="ufndh" data-relation="weak" }, except for the additional condition $\emptyset\not\in\mathcal{F}$. Likewise, an *ultrafilter* on $X$ is understood as a maximal filter not containing $\emptyset$.
...
Moreover, regarding $\mathcal{N}(x)$ as playing a role similar to a local base, we showed that a topological space can also be defined via a base $\mathcal{B}$. ([§Bases of a Topological Space, ⁋Corollary 6](/en/math/topology/topological_bases#cor6){: data-lid="7k4j9" data-relation="weak" })
