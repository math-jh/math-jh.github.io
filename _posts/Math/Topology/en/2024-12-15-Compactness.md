...
date: 2024-12-15
weight: 16
translated_at: 2026-07-13T20:00:02+00:00
...
The arbitrary product of compact spaces is again compact. If this product is finite, the result can be shown in a more intuitive way, but if the product is infinite, we need the following lemma. It generalizes [§Compactness and Convergence of Filters, ⁋Proposition 5](/en/math/topology/filter_convergence#prop5) to the language of filters.

::: Lemma 1
A topological space $X$ is compact if and only if every ultrafilter converges.
:::
::: Proof
First, assume $X$ is compact and let an arbitrary ultrafilter $\mathcal{F}$ be given. Suppose for contradiction that $\mathcal{F}$ has no limit point. That is, for every $x\in X$, there exists an open neighborhood $U_x$ such that $U_x\not\in \mathcal{F}$. Then by compactness of $X$, there exists a finite subcover $U_{x_1},\ldots, U_{x_n}$ of $X$.
...
and since $U_{x_i}\not\in \mathcal{F}$ by assumption, each $A\cap U_{x_i}$ is not in $\mathcal{F}$ either; and since $\mathcal{F}$ is maximal, we must have $X\setminus (A\cap U_{x_i})\in \mathcal{F}$. Then their finite intersection
...
For the converse direction, for any ultrafilter $\mathcal{F}$ defined on $X$, we first verify that $\pr_i(\mathcal{F})$ defines an ultrafilter base on $X_i$, then from the assumption that $X_i$ is compact and [Lemma 1](#lem1) we obtain a limit point $x_i$ of this ultrafilter, and we can show that $x=(x_i)_{i\in I}$ is a limit point of $\mathcal{F}$; thus the proof is complete again by [Lemma 1](#lem1).
:::
