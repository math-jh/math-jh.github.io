...
of $\mathcal{F}(V_{x_1}\cap V_{x_2})$ become the same element by [Proposition 2](#prop2). Therefore, using the second condition of [Definition 1](#def1), we can find a suitable $\tilde{s}\in \mathcal{F}(U)$ whose stalk is $s_x$.
...
Moreover, the above proof still works if we replace $\Set$ with $\Ab$, $\Ring$, etc. Also, in all these cases, if $\mathcal{F}$ was a sheaf to begin with, the morphism $\mathcal{F}\rightarrow \mathcal{F}^\dagger$ obtained from [Lemma 6](#lem6) is an isomorphism.

As can be seen from the proof, for a presheaf $\mathcal{F}$ and its sheafification $\mathcal{F}^\dagger$, the stalks $\mathcal{F}_x$ and $\mathcal{F}^\dagger_x$ at any point $x\in X$ are isomorphic. This is clear from the fact that the stalk is a colimit and $(-)^\dagger$ is a left adjoint functor. ([\[Category Theory\] §Adjoint Functors, ⁋Theorem 9](/en/math/category_theory/adjoints#thm9)) Then by definition we can see that $\mathcal{F}^\dagger$ is *exactly* the sheaf of sections of the canonical projection $\Spe(\mathcal{F})\rightarrow X$. ([§Presheaves, ⁋Example 3](/en/math/topology/presheaves#ex3))
...
On the other hand, applying [Lemma 6](#lem6), we can show that to describe an arbitrary sheaf $\mathcal{F}$ defined on a topological space $X$, it suffices to know how this sheaf is defined on a base $\mathcal{B}$ of the topology.
...
- If $\mathcal{F}$ is a sheaf, then the restriction $\mathcal{F}\vert_U$ and pushforward $f_\ast \mathcal{F}$ are also sheaves. ([§Presheaves, ⁋Example 8](/en/math/topology/presheaves#ex8))
...
- If $\mathcal{G}$ is a sheaf then $\sHom(\mathcal{F},\mathcal{G})$ is also a sheaf. ([§Presheaves, ⁋Example 12](/en/math/topology/presheaves#ex12)) Likewise, the product of sheaves is a sheaf.
:::
...
::: Lemma 11
The $f^{-1}$ defined by the above equation ($\ast$) satisfies the adjunction of [Definition 10](#def10):
...
::: Example 12
Consider the case where the function $f$ is an open embedding $i:V\hookrightarrow Y$ sending an open set $V$ of $Y$ into $Y$. Then by [§Subspaces, ⁋Lemma 2](/en/math/topology/subspaces#lem2), any open set $U$ of $V$ is also open in all of $Y$, so
...
::: Example 13
Consider a point $i_y:\{y\}\rightarrow Y$ of $Y$. Then the adjunction of [Lemma 11](#lem11) gives
