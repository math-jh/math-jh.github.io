...
For example, by [§Presheaves, ⁋Lemma 1](/en/math/topology/presheaves#lem1){: data-relation="required" }, the presheaf of continuous functions from a topological space $X$ to $Y$ is a sheaf. ([§Presheaves, ⁋Example 3](/en/math/topology/presheaves#ex3){: data-relation="weak" })
...
of $\mathcal{F}(V_{x_1}\cap V_{x_2})$ become the same element by [Proposition 2](#prop2){: data-relation="required" }. Therefore, using the second condition of [Definition 1](#def1){: data-relation="required" }, we can find a suitable $\tilde{s}\in \mathcal{F}(U)$ whose stalk is $s_x$.
...
In a similar way, cokernels, quotients, and anything else that does not take values in the category of sheaves can all be defined using sheafification, and the fact that these objects satisfy the corresponding universal property in the sheaf category $\Sh(X;\mathcal{A})$ can be shown using only the universal property of [Definition 5](#def5){: data-relation="required" } (without a construction like [Lemma 6](#lem6){: data-relation="required" }).
...
- Skyscraper sheaf $(i_x)_\ast A$ ([§Presheaves, ⁋Example 5](/en/math/topology/presheaves#ex5){: data-relation="weak" })
- If $\mathcal{F}$ is a sheaf, then the restriction $\mathcal{F}\vert_U$ and pushforward $f_\ast \mathcal{F}$ are also sheaves. ([§Presheaves, ⁋Example 8](/en/math/topology/presheaves#ex8){: data-relation="weak" })
- The sheafification of the constant presheaf, $\underline{A}$ ([§Presheaves, ⁋Example 6](/en/math/topology/presheaves#ex6){: data-relation="weak" }); then $(i_x)_\ast A$ is the pushforward of the constant sheaf $\underline{A}=A$ on $\{x\}$ via the inclusion $i_x:\{p\}\rightarrow X$.
- Sheaf of sections ([§Presheaves, ⁋Example 3](/en/math/topology/presheaves#ex3){: data-relation="weak" })
- If $\mathcal{G}$ is a sheaf then $\sHom(\mathcal{F},\mathcal{G})$ is also a sheaf. ([§Presheaves, ⁋Example 12](/en/math/topology/presheaves#ex12){: data-relation="weak" }) Likewise, the product of sheaves is a sheaf.
:::
...
::: Lemma 11
The $f^{-1}$ defined by the above equation ($\ast$) satisfies the adjunction of [Definition 10](#def10){: data-relation="required" }:
...
::: Example 12
Consider the case where the function $f$ is an open embedding $i:V\hookrightarrow Y$ sending an open set $V$ of $Y$ into $Y$. Then by [§Subspaces, ⁋Lemma 2](/en/math/topology/subspaces#lem2){: data-relation="required" }, any open set $U$ of $V$ is also open in all of $Y$, so
...
::: Example 13
Consider a point $i_y:\{y\}\rightarrow Y$ of $Y$. Then the adjunction of [Lemma 11](#lem11){: data-relation="required" } gives
...
In the adjunction of [Lemma 11](#lem11){: data-relation="required" }, pushforward $j_\ast$ is the right adjoint of $j^{-1}$. Symmetrically, $j_!$ is the left adjoint of $j^{-1}$. That is,
