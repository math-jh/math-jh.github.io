...
<ins id="def1">**Definition 1**</ins> A presheaf $$\mathcal{F}$$ defined on a topological space $$X$$ is called a *sheaf* if the following two conditions hold.
...
<ins id="prop2">**Proposition 2**</ins> For a topological space $$X$$, a sheaf $$\mathcal{F}$$ defined on it, and any open set $$U$$ of $$X$$, the function
...
<ins id="prop4">**Proposition 4**</ins> Suppose a morphism $$\phi:\mathcal{F}\rightarrow \mathcal{G}$$ between sheaves defined on $$X$$ is given. Then $$\phi$$ being an isomorphism is equivalent to the morphisms $$\phi_x:\mathcal{F}_x \rightarrow \mathcal{G}_x$$ on stalks all being isomorphisms.
...
<ins id="def7">**Definition 7**</ins> Suppose a sheaf $$\mathcal{F}$$ defined on $$X$$ and a sheaf morphism $$\phi:\mathcal{F}\rightarrow \mathcal{G}$$ are given.
...
<ins id="prop8">**Proposition 8**</ins> Suppose a topological space $$X$$ and a base $$\mathcal{B}$$ for it are given, and for each $$B\in \mathcal{B}$$ we are given $$\mathcal{F}(B)$$ and restriction maps
...
- If $$\mathcal{G}$$ is a sheaf then $$\mathcal{Hom}(\mathcal{F},\mathcal{G})$$ is also a sheaf. ([§Presheaves, ⁋Example 12](/en/math/topology/presheaves#ex12)) Likewise, the product of sheaves is a sheaf.
...
<ins id="ex14">**Example 14**</ins> In Example 13 we dealt with the inclusion of a single point $$i_y:\{y\}\rightarrow Y$$. Similarly, for an open embedding $$j:U\hookrightarrow X$$ we can define a sheaf functor $$j_!$$ called *extension by zero*. For a sheaf $$\mathcal{F}$$ defined on $$U$$, consider the presheaf
...
<ins id="prop15">**Proposition 15**</ins> Suppose a sheaf morphism $$\phi: \mathcal{F}\rightarrow \mathcal{G}$$ defined on a topological space $$X$$ is given. Then the following are equivalent.
...
<ins id="prop16">**Proposition 16**</ins> Suppose a sheaf morphism $$\phi: \mathcal{F}\rightarrow \mathcal{G}$$ defined on a topological space $$X$$ is given. Then the following are all equivalent.
