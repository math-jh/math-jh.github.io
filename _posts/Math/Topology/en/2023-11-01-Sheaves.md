...
---
A presheaf contains all local information, but since there is no way to glue this into global information, we cannot expect a local-to-global property. A sheaf is a presheaf that satisfies a gluing condition as in [§Presheaves, ⁋Lemma 1](/en/math/topology/presheaves#lem1){: data-lid="6fk3d" data-relation="weak" }.

::: Definition 1
A presheaf $\mathcal{F}$ defined on a topological space $X$ is called a *sheaf* if the following two conditions hold.
...
For example, by [§Presheaves, ⁋Lemma 1](/en/math/topology/presheaves#lem1){: data-lid="ixc98" data-relation="required" }, the presheaf of continuous functions from a topological space $X$ to $Y$ is a sheaf. ([§Presheaves, ⁋Example 3](/en/math/topology/presheaves#ex3){: data-lid="uert1" data-relation="weak" })
...
::: Proposition 2
For a topological space $X$, a sheaf $\mathcal{F}$ defined on it, and any open set $U$ of $X$, the function
...
From the description of the stalk $\mathcal{F}_x$ that we saw in [§Presheaves, §§Stalks](/en/math/topology/presheaves#stalks){: data-lid="t2h49" data-relation="required" }, for each $x\in U$ there exists an open set $x\in V_x\subseteq U$ such that $s\vert_{V_x}=t\vert_{V_x}$. The collection of such $V_x$ forms an open covering of $U$, and therefore from the first condition of [Definition 1](#def1){: data-lid="zn6ch" data-relation="required" } we know $s=t$.
:::
...
of $\mathcal{F}(V_{x_1}\cap V_{x_2})$ become the same element by [Proposition 2](#prop2){: data-lid="wemna" data-relation="required" }. Therefore, using the second condition of [Definition 1](#def1){: data-lid="5fp85" data-relation="required" }, we can find a suitable $\tilde{s}\in \mathcal{F}(U)$ whose stalk is $s_x$.
...
::: Proposition 4
Suppose a morphism $\phi:\mathcal{F}\rightarrow \mathcal{G}$ between sheaves defined on $X$ is given. Then $\phi$ being an isomorphism is equivalent to the morphisms $\phi_x:\mathcal{F}_x \rightarrow \mathcal{G}_x$ on stalks all being isomorphisms.
:::
::: Proof
...
if necessary we may restrict $W$ to a smaller open neighborhood of $p$ so that $(\phi\vert_W)(s)=t\vert_W$. Our claim is that the sections constructed at each point $p$ in this way satisfy the gluing condition well, so we can produce a section defined over all of $U$, and this can be proved as follows. Let $s\in \mathcal{F}(W)$ be the section obtained by applying the above argument to $p\in U$, and let $s'\in \mathcal{F}(W')$ be the section obtained by applying it to $p'\in U$. Then on $W\cap W'$,
...
::: Definition 5
For any topological space $X$ and category $\mathcal{A}$, the left adjoint of the forgetful functor $\Sh(X;\mathcal{A})\rightarrow \PSh(X;\mathcal{A})$ is called the *sheafification functor*, and is denoted by $(-)^\dagger$.
:::

To complete this definition, the following lemma is needed.

::: Lemma 6
$(-)^\dagger:\PSh(X;\Set) \rightarrow \Sh(X;\Set)$ exists.
:::
...
Moreover, the above proof still works if we replace $\Set$ with $\Ab$, $\Ring$, etc. Also, in all these cases, if $\mathcal{F}$ was a sheaf to begin with, the morphism $\mathcal{F}\rightarrow \mathcal{F}^\dagger$ obtained from [Lemma 6](#lem6){: data-lid="0wyb0" data-relation="required" } is an isomorphism.

As can be seen from the proof, for a presheaf $\mathcal{F}$ and its sheafification $\mathcal{F}^\dagger$, the stalks $\mathcal{F}_x$ and $\mathcal{F}^\dagger_x$ at any point $x\in X$ are isomorphic. This is clear from the fact that the stalk is a colimit and $(-)^\dagger$ is a left adjoint functor. ([\[Category Theory\] §Adjoint Functors, ⁋Theorem 9](/en/math/category_theory/adjoints#thm9){: data-lid="5as3j" data-relation="required" }) Then by definition we can see that $\mathcal{F}^\dagger$ is *exactly* the sheaf of sections of the canonical projection $\Spe(\mathcal{F})\rightarrow X$. ([§Presheaves, ⁋Example 3](/en/math/topology/presheaves#ex3){: data-lid="4vmb4" data-relation="weak" })

::: Definition 7
Suppose a sheaf $\mathcal{F}$ defined on $X$ and a sheaf morphism $\phi:\mathcal{F}\rightarrow \mathcal{G}$ are given.
...
In a similar way, cokernels, quotients, and anything else that does not take values in the category of sheaves can all be defined using sheafification, and the fact that these objects satisfy the corresponding universal property in the sheaf category $\Sh(X;\mathcal{A})$ can be shown using only the universal property of [Definition 5](#def5){: data-lid="3ij9f" data-relation="required" } (without a construction like [Lemma 6](#lem6){: data-lid="zu74q" data-relation="required" }).

## Sheaves Defined on a Base

On the other hand, applying [Lemma 6](#lem6){: data-lid="an6os" data-relation="required" }, we can show that to describe an arbitrary sheaf $\mathcal{F}$ defined on a topological space $X$, it suffices to know how this sheaf is defined on a base $\mathcal{B}$ of the topology.

::: Proposition 8
Suppose a topological space $X$ and a base $\mathcal{B}$ for it are given, and for each $B\in \mathcal{B}$ we are given $\mathcal{F}(B)$ and restriction maps
...
Then for the same reason as in [Lemma 6](#lem6){: data-lid="93nko" data-relation="required" }, this defines a sheaf.
:::
...
::: Example 9
The following are examples of sheaves.

- Skyscraper sheaf $(i_x)_\ast A$ ([§Presheaves, ⁋Example 5](/en/math/topology/presheaves#ex5){: data-lid="7txdz" data-relation="weak" })
- If $\mathcal{F}$ is a sheaf, then the restriction $\mathcal{F}\vert_U$ and pushforward $f_\ast \mathcal{F}$ are also sheaves. ([§Presheaves, ⁋Example 8](/en/math/topology/presheaves#ex8){: data-lid="6m5x0" data-relation="weak" })
- The sheafification of the constant presheaf, $\underline{A}$ ([§Presheaves, ⁋Example 6](/en/math/topology/presheaves#ex6){: data-lid="n97ax" data-relation="weak" }); then $(i_x)_\ast A$ is the pushforward of the constant sheaf $\underline{A}=A$ on $\{x\}$ via the inclusion $i_x:\{p\}\rightarrow X$.
- Sheaf of sections ([§Presheaves, ⁋Example 3](/en/math/topology/presheaves#ex3){: data-lid="kp6a2" data-relation="weak" })
- If $\mathcal{G}$ is a sheaf then $\sHom(\mathcal{F},\mathcal{G})$ is also a sheaf. ([§Presheaves, ⁋Example 12](/en/math/topology/presheaves#ex12){: data-lid="hry83" data-relation="weak" }) Likewise, the product of sheaves is a sheaf.
:::
...
::: Definition 10
The left adjoint of $f_\ast:\Sh(X) \rightarrow \Sh(Y)$ is called the *inverse image sheaf*, and is denoted by $f^{-1}$.
:::
...
::: Example 12
Consider the case where the function $f$ is an open embedding $i:V\hookrightarrow Y$ sending an open set $V$ of $Y$ into $Y$. Then by [§Subspaces, ⁋Lemma 2](/en/math/topology/subspaces#lem2){: data-lid="wk2f8" data-relation="required" }, any open set $U$ of $V$ is also open in all of $Y$, so
...
::: Example 13
Consider a point $i_y:\{y\}\rightarrow Y$ of $Y$. Then the adjunction of [Lemma 11](#lem11){: data-lid="lw166" data-relation="required" } gives
...
::: Example 14
In Example 13 we dealt with the inclusion of a single point $i_y:\{y\}\rightarrow Y$. Similarly, for an open embedding $j:U\hookrightarrow X$ we can define a sheaf functor $j_!$ called *extension by zero*. For a sheaf $\mathcal{F}$ defined on $U$, consider the presheaf
...
In the adjunction of [Lemma 11](#lem11){: data-lid="st7xl" data-relation="required" }, pushforward $j_\ast$ is the right adjoint of $j^{-1}$. Symmetrically, $j_!$ is the left adjoint of $j^{-1}$. That is,
...
::: Proposition 15
Suppose a sheaf morphism $\phi: \mathcal{F}\rightarrow \mathcal{G}$ defined on a topological space $X$ is given. Then the following are equivalent.
...
::: Proposition 16
Suppose a sheaf morphism $\phi: \mathcal{F}\rightarrow \mathcal{G}$ defined on a topological space $X$ is given. Then the following are all equivalent.
