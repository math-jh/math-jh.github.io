...
::: Lemma 1
Let $X$ be a set and $(A_i)$ a family of subsets satisfying one of the two conditions in [§Subspaces, ⁋Proposition 6](/en/math/topology/subspaces#prop6){: data-lid="btg09" data-relation="required" }. If a family of continuous functions $(f_i: A_i \rightarrow Y)$ satisfies
...
Then $\mathcal{F}\vert_U$ is a presheaf. ([§Subspaces, ⁋Lemma 2](/en/math/topology/subspaces#lem2){: data-lid="9cnyb" data-relation="required" })
:::
...
exist. We endow $\Spe(\mathcal{F})$ with the final topology defined by this family of functions ([§Initial and Final Topology, ⁋Definition 4](/en/math/topology/initial_and_final_topology#def4){: data-lid="ywi2i" data-relation="required" }), and call this space the *étalé space* of $\mathcal{F}$.
...
Thus the category of $\mathcal{A}$-valued presheaves on $X$ is the functor category $[\Open(X)^\op, \mathcal{A}]$. We denote this by $\PSh(X; \mathcal{A})$, and when there is no risk of confusion from context, we simply write $\PSh(X)$. As a side note, the $f_\ast$ from [Example 8](#ex8){: data-lid="9024c" data-relation="weak" } is a functor $\PSh(X; \mathcal{A})\rightarrow \PSh(Y; \mathcal{A})$.

Thinking of the intuitive [Example 3](#ex3){: data-lid="sr494" data-relation="weak" }, for an open set $U$ the map $\phi(U):\mathcal{F}(U) \rightarrow \mathcal{G}(U)$ can be thought of as the function obtained by restricting $\phi:\mathcal{F}\rightarrow \mathcal{G}$ to $U$, so we sometimes write $\phi\vert_U$ instead of $\phi(U)$.
...
The following examples should have appeared under [Examples of presheaves](#examples-of-presheaves){: data-lid="mjdmf" data-relation="weak" } above, but were postponed because we had not yet defined presheaf morphisms.
