...
::: Lemma 1
Let $X$ be a set and $(A_i)$ a family of subsets satisfying one of the two conditions in [§Subspaces, ⁋Proposition 6](/en/math/topology/subspaces#prop6){: data-lid="btg09" }. If a family of continuous functions $(f_i: A_i \rightarrow Y)$ satisfies
...
:::
::: Proof
First, the function $f$ is obtained from [\[Set Theory\] §Sum of Sets, ⁋Proposition 2](/en/math/set_theory/sum_of_sets#prop2){: data-lid="jm8pb" }. Its continuity follows from [§Subspaces, ⁋Proposition 8](/en/math/topology/subspaces#prop8){: data-lid="xv91t" }.
:::
...
::: Definition 2
For a topological space $X$, a contravariant functor $\mathcal{F}:\Open(X)^\op \rightarrow \Set$ is called a *presheaf* of sets on $X$.
:::
...
::: Example 3
Let $X, Y$ be two topological spaces, and define $\mathcal{F}$ as follows.
...
Meanwhile, in [Definition 2](#def2){: data-lid="yvxpx" } above, $\Set$ can be replaced by an appropriate category, such as $\Ab$. For instance, in [Example 3](#ex3){: data-lid="tjnzz" }, if $Y=\mathbb{R}$, we could use addition on $\mathbb{R}$ to define addition of continuous functions, and then $\mathcal{F}(U)$ would carry the structure of an abelian group. In such a case, $\mathcal{F}$ is called a presheaf of abelian groups on $X$. For convenience, we shall call a presheaf $\mathcal{F}: \Open(X)^\op \rightarrow \mathcal{A}$ an $\mathcal{A}$-valued presheaf. Among presheaves, those satisfying the gluing condition ([Lemma 1](#lem1){: data-lid="9yqhp" }) are called sheaves; we define these in the next post.

## Examples of presheaves

Next, we examine several examples of presheaves.

::: Example 5 (Skyscraper sheaf)
Fix a topological space $X$ and a point $i_x:\{x\}\hookrightarrow X$, and fix an object $A\in \mathcal{A}$. Then defining
...
::: Example 6 (Constant presheaf)
Now fix a topological space $X$ and an object $A\in \mathcal{A}$, assign $A$ to every open set, and let all restriction maps be $\id_A$. This defines a presheaf, called the *constant presheaf*.
:::

The following examples show ways to obtain new presheaves from an arbitrary presheaf.

::: Example 7
Given a presheaf $\mathcal{F}$ on $X$, for any open set $U$ we can define $\mathcal{F}\vert_U$ by

$$\mathcal{F}\vert_U(V)=\mathcal{F}(V)\quad\text{for all open $V\subseteq U$}$$

Then $\mathcal{F}\vert_U$ is a presheaf. ([§Subspaces, ⁋Lemma 2](/en/math/topology/subspaces#lem2){: data-lid="9cnyb" })
:::
...
::: Definition 9
Let $\mathcal{F}$ be a presheaf on a topological space $X$. For any point $x\in X$, the *stalk* $\mathcal{F}_x$ at $x$ is defined by
...
exist. We endow $\Spe(\mathcal{F})$ with the final topology defined by this family of functions ([§Initial and Final Topology, ⁋Definition 4](/en/math/topology/initial_and_final_topology#def4){: data-lid="ywi2i" }), and call this space the *étalé space* of $\mathcal{F}$.
...
Thus the category of $\mathcal{A}$-valued presheaves on $X$ is the functor category $[\Open(X)^\op, \mathcal{A}]$. We denote this by $\PSh(X; \mathcal{A})$, and when there is no risk of confusion from context, we simply write $\PSh(X)$. As a side note, the $f_\ast$ from [Example 8](#ex8){: data-lid="9024c" } is a functor $\PSh(X; \mathcal{A})\rightarrow \PSh(Y; \mathcal{A})$.

Thinking of the intuitive [Example 3](#ex3){: data-lid="sr494" }, for an open set $U$ the map $\phi(U):\mathcal{F}(U) \rightarrow \mathcal{G}(U)$ can be thought of as the function obtained by restricting $\phi:\mathcal{F}\rightarrow \mathcal{G}$ to $U$, so we sometimes write $\phi\vert_U$ instead of $\phi(U)$.
...
The following examples should have appeared under [Examples of presheaves](#examples-of-presheaves){: data-lid="mjdmf" } above, but were postponed because we had not yet defined presheaf morphisms.
...
::: Definition 15
For a topological space $X$, a contravariant functor $\Open(X)\rightarrow\Ab$ is called an *abelian presheaf*.
:::

::: Definition 16
Let $\phi:\mathcal{F}\rightarrow\mathcal{G}$ be a morphism of abelian presheaves on a topological space $X$. The *presheaf kernel* $\ker\phi$ of $\phi$ is the data consisting of:
...
::: Lemma 17
The $\ker\phi$ defined above is an (abelian) presheaf on $X$.
:::
::: Proof
