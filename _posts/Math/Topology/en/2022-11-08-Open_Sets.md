...
Adopting the convention from [\[Set Theory\] §Union and Intersection](/en/math/set_theory/union_and_intersection){: data-lid="7494p" }, one can show that the two sets
...
:::
::: Proof
By [Definition 4](#def4){: data-lid="j25lo" }, the latter condition is equivalent to:
...
For the second property, let $V_1,\ldots, V_n\in\mathcal{N}(x)$ be given. Then there exist open neighborhoods $U_1,\ldots, U_n$ of $x$ with $x\in U_i\subseteq V_i$. By the third condition of [Definition 1](#def1){: data-lid="lufpt" }, their finite intersection $U_1\cap\cdots\cap U_n$ is also open, and therefore
