...
Adopting the convention from [\[Set Theory\] §Union and Intersection](/en/math/set_theory/union_and_intersection){: data-relation="weak" }, one can show that the two sets
...
:::
::: Proof
By [Definition 4](#def4){: data-relation="required" }, the latter condition is equivalent to:
...
Now we must show the converse. Suppose collections $\mathcal{N}(x)$ satisfying the above conditions are given. By [Proposition 5](#prop5){: data-relation="required" }, the elements of $\mathcal{T}$ must satisfy the condition
