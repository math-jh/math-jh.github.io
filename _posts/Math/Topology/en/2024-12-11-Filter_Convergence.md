...
::: Definition 1
A topological space $X$ is called *limit point compact* if every infinite subset of $X$ has a limit point. ([§Interior, Closure, and Boundary, ⁋Definition 8](/en/math/topology/other_concepts#def8){: data-lid="hg2d1" data-relation="required" })
:::
...
This collection satisfies the conditions of [§Bases of a Topological Space, ⁋Corollary 6](/en/math/topology/topological_bases#cor6){: data-lid="ipoaw" data-relation="required" }, and thus defines a topology on $\mathbb{R}$.
...
The above example shows that convergence of sequences is not a very good notion for detecting limit points. On the other hand, by [§Interior, Closure, and Boundary, ⁋Proposition 6](/en/math/topology/other_concepts#prop6){: data-lid="1ie7y" data-relation="required" }, any limit point of a set $A$ belongs to the closure of $A$.
...
The above contents show that convergence of sequences is not an appropriate notion for expressing the concepts we have dealt with so far. Looking at the proof of [Proposition 11](#prop11){: data-lid="73s9b" data-relation="forward" }, one can see in what direction it is good to generalize this.
...
What played the essential role in the proof of [Proposition 11](#prop11){: data-lid="o924j" data-relation="required" } was that $x$ possesses a decreasing sequence of open sets
...
Therefore, by the argument after [Lemma 7](#lem7){: data-lid="7930i" data-relation="required" }, the following proposition is also obvious.
...
The arbitrary product of compact spaces is again compact. If the product is finite, this can be shown in a more intuitive manner, but for an infinite product we need the following lemma. It generalizes [Proposition 5](#prop5){: data-lid="yf0r0" data-relation="weak" } to the language of filters.
...
On the other hand, by [\[Set Theory\] §Filters, Ideals, and Galois Connections, ⁋Proposition 5](/en/math/set_theory/filter_and_ideal#prop5){: data-lid="p9nwn" data-relation="required" }, $\mathcal{F}$ is prime. That is, for any subset $A\subseteq X$, exactly one of $A\in \mathcal{F}$ or $X\setminus A\in \mathcal{F}$ holds. Then for any $A\in \mathcal{F}$,
