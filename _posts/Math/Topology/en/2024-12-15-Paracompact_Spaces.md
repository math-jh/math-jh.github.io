...
Let $V=\bigcup\mathcal{D}$; then $V$ is an open set containing $B$. Since $\mathcal{D}$ is a subfamily of the locally finite family $\mathcal{C}$, it is locally finite, and by [Lemma 5](#lem5){: data-relation="required" } we have $\cl(V)=\bigcup_{C\in\mathcal{D}}\cl(C)$. Since each $\cl(C)$ does not contain $a$, we have $a\notin\cl(V)$. Then $W=X\setminus\cl(V)$ is an open set containing $a$ and disjoint from $V\supseteq B$. Therefore $a$ and $B$ are separated by neighborhoods and $X$ is regular.
:::
...
**(2) Two shrinkings.** $X$ is normal by [Theorem 7](#thm7){: data-lid="662w5" data-relation="required" }, and the locally finite $(V_\alpha)$ is point-finite. Apply [Lemma 10](#lem10){: data-relation="required" } to $(V_\alpha)$ to obtain an open cover $(P_\alpha)_{\alpha\in J}$ with $\cl(P_\alpha)\subseteq V_\alpha$ for all $\alpha$. Since $P_\alpha\subseteq V_\alpha$, $(P_\alpha)$ is also point-finite, and applying [Lemma 10](#lem10){: data-relation="required" } again to $(P_\alpha)$ yields an open cover $(Q_\alpha)_{\alpha\in J}$ with $\cl(Q_\alpha)\subseteq P_\alpha$ for all $\alpha$. In summary, both covers $(P_\alpha)$ and $(Q_\alpha)$ cover $X$ and satisfy
