...
Now we can apply [§Open Sets, ⁋Proposition 6](/en/math/topology/open_sets#prop6){: data-relation="required" } to obtain a topology $\mathcal{T}$, and in this topological space $\mathcal{B}(x)$ is a local base at $x$, so by [Proposition 5](#prop5){: data-lid="r8rkd" data-relation="required" }, $\mathcal{B}$ is a base for $\mathcal{T}$.
:::
