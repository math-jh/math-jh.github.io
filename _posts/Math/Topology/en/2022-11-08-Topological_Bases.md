...
Meanwhile, by refining [Definition 1](#def1){: data-lid="v70fw" }, we can define a *subbase* as follows.
...
Earlier we asked whether, given an arbitrary collection satisfying the first and second conditions of [Proposition 2](#prop2){: data-lid="5uxv2" }, there exists a topology on $X$ having it as a base. This becomes an easy corollary of the above proposition.
...
Now we can apply [§Open Sets, ⁋Proposition 6](/en/math/topology/open_sets#prop6){: data-lid="tytud" } to obtain a topology $\mathcal{T}$, and in this topological space $\mathcal{B}(x)$ is a local base at $x$, so by [Proposition 5](#prop5){: data-lid="r8rkd" }, $\mathcal{B}$ is a base for $\mathcal{T}$.
:::
