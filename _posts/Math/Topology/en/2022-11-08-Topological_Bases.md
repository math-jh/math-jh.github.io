...
In particular, since $X\in\mathcal{T}$, the collection $\mathcal{B}$ is also a covering of $X$. ([\[Set Theory\] §Sum of Sets, ⁋Definition 1](/en/math/set_theory/sum_of_sets#def1){: data-lid="y9yup" }) Since the elements of $\mathcal{B}$ are all open sets by definition, it is appropriate to say that $\mathcal{B}$ is an *open covering* of $X$.
...
Meanwhile, by refining [Definition 1](#def1){: data-lid="v70fw" data-relation="required" }, we can define a *subbase* as follows.
...
[§Open Sets, ⁋Proposition 6](/en/math/topology/open_sets#prop6){: data-lid="11lvo" data-relation="required" } shows that if we can describe the neighborhood filter $\mathcal{N}(x)$ centered at a point $x$ in a topological space $X$, then we can completely recover the topology of $X$. On the other hand, since $\mathcal{N}(x)$ satisfies the conditions of [§Open Sets, ⁋Proposition 6](/en/math/topology/open_sets#prop6){: data-lid="lengf" data-relation="required" }, in particular the first condition, we do not need all of $\mathcal{N}(x)$ to describe it.
...
As in [§Open Sets, ⁋Definition 4](/en/math/topology/open_sets#def4){: data-lid="3m9t1" data-relation="required" }, when $A$ is a singleton $\{x\}$, we call a local base at $A$ a local base at the point $x$. Then the following holds.
...
Earlier we asked whether, given an arbitrary collection satisfying the first and second conditions of [Proposition 2](#prop2){: data-lid="5uxv2" data-relation="required" }, there exists a topology on $X$ having it as a base. This becomes an easy corollary of the above proposition.
...
Now we can apply [§Open Sets, ⁋Proposition 6](/en/math/topology/open_sets#prop6){: data-lid="tytud" data-relation="required" } to obtain a topology $\mathcal{T}$, and in this topological space $\mathcal{B}(x)$ is a local base at $x$, so by [Proposition 5](#prop5){: data-lid="r8rkd" data-relation="required" }, $\mathcal{B}$ is a base for $\mathcal{T}$.
:::
