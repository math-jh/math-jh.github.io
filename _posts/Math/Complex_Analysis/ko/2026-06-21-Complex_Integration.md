...
로 정의한다. Closed 곡선 $\gamma$에 대하여는 같은 적분을 $\oint_\gamma f(z)\,dz$로도 적는다.
:::
...
피적분함수 $\lvert\gamma'(t)\rvert = \sqrt{x'(t)^2 + y'(t)^2}$은 곡선의 순간 속력이므로, 그 적분은 곡선을 따라 이동한 총 거리, 곧 arc length이다. 가령 단위원 $\gamma(t) = e^{it}$ ($t \in [0, 2\pi]$) 은 $\lvert\gamma'(t)\rvert = \lvert i e^{it}\rvert = 1$이라 $\mathrm{length}(\gamma) = \int_0^{2\pi} 1\,dt = 2\pi$로, 반지름 $1$인 원의 둘레와 일치한다. 이 길이를 써서 경로적분의 크기를 위로 어림한다.
...
2. (closed 경로) 자취가 $\Omega$에 들어 있는 임의의 closed piecewise $C^1$ 곡선 $\gamma$에 대하여 $\displaystyle\oint_\gamma f\,dz = 0$이다.
:::
