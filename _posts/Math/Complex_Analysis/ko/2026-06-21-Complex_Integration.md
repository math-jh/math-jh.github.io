...
로 정의한다. Closed curve $\gamma$에 대하여는 같은 적분을 $\oint_\gamma f(z)\dd{z}$로도 적는다.
:::

우변의 적분은 복소숫값 함수 $g(t) = f(\gamma(t))\gamma'(t)$를 실구간에서 적분한 것이며, 복소숫값 함수의 적분은 실허부를 따로 적분하여 $\int_a^b g\dd{t} = \int_a^b \Real g\dd{t} + i\int_a^b \Img g\dd{t}$로 정의된다. 피적분함수 $g$는 각 부분구간 $[t_{k-1}, t_k]$ 위에서 연속이므로 (유한 개의 분할점을 빼고) 적분이 잘 정의되며, 전체 적분은 부분구간별 적분의 합 $\sum_k \int_{t_{k-1}}^{t_k} g\dd{t}$이다. 정의가 자취뿐 아니라 매개변수 $\gamma$ 자체에 기대어 적혀 있지만, 곧 보듯 적분값은 곡선이 그리는 방향만 같으면 매개변수에 무관하다.
...
경로적분은 정의가 실변수 적분으로 환원되므로, 실적분의 성질이 그대로 옮겨 온다. 먼저 피적분함수에 대한 선형성과 곡선의 방향·이어붙이기에 대한 성질을 정리한다.
...
명제 4의 둘째 항은 적분값이 곡선의 자취와 그 진행 방향만으로 결정되고 구체적인 매개변수 선택에는 무관함을 말한다. 단조 증가 재매개화는 같은 자취를 같은 방향으로 다시 훑는 것이므로, 가령 단위원을 $e^{it}$로 한 바퀴 돌든 $e^{2\pi i s^2}$ 류로 속도를 바꿔 돌든 적분값이 같다. 셋째 항의 역방향 부호는 정적분 $\int_a^b = -\int_b^a$의 복소판이며, 넷째 항과 결합하면 closed 곡선을 여러 호로 쪼개 따로 적분한 뒤 합산할 수 있게 해 준다. 다음으로 적분의 크기를 곡선의 길이로 가늠하는 부등식을 세우는데, 이를 위해 곡선의 길이를 정의한다.

::: 정의 5
Piecewise $C^1$ curve $\gamma : [a, b] \rightarrow \mathbb{C}$의 *길이<sub>length</sub>*를

$$\mathrm{length}(\gamma) = \int_a^b \lvert \gamma'(t)\rvert\dd{t}$$
...
::: 증명
먼저 임의의 복소숫값 연속함수 $g : [a, b] \rightarrow \mathbb{C}$에 대하여 $\bigl\lvert\int_a^b g\dd{t}\bigr\rvert \leq \int_a^b \lvert g\rvert\dd{t}$임에 유의한다. 적분값 $I = \int_a^b g\dd{t}$가 $0$이면 자명하고, $I \neq 0$이면 $I = \lvert I\rvert e^{i\theta}$로 적어 $e^{-i\theta}$를 곱하면

$$\lvert I\rvert = e^{-i\theta} I = \int_a^b e^{-i\theta} g(t)\dd{t} = \Real \int_a^b e^{-i\theta} g(t)\dd{t} = \int_a^b \Real\bigl(e^{-i\theta} g(t)\bigr)\dd{t}$$

이다 ($\lvert I\rvert$이 실수이므로 가운데 적분은 자기 실수부와 같다). 피적분함수에 $\Real w \leq \lvert w\rvert$를 적용하면 $\Real(e^{-i\theta}g(t)) \leq \lvert e^{-i\theta}g(t)\rvert = \lvert g(t)\rvert$이므로 $\lvert I\rvert \leq \int_a^b \lvert g(t)\rvert\dd{t}$이다.
...
ML 부등식은 적분의 정확한 값을 모르더라도 그 크기에 상한을 주므로, 곡선을 변형시키며 적분이 $0$으로 가는지를 가늠하는 데 두루 쓰인다. 이름의 $M$은 피적분함수의 최댓값(maximum), $L$은 곡선의 길이(length)를 가리킨다. 가령 반지름 $R$인 원호 위에서 $\lvert f\rvert \leq M_R$이고 $M_R \cdot R \rightarrow 0$이면 그 호를 따른 적분이 $0$으로 감을 즉시 알 수 있는데, 이런 어림이 closed curve 적분의 평가에서 결정적 역할을 한다.
...
원시함수는 실변수의 부정적분에 해당하는 복소판이며, holomorphic function $F$의 도함수 $F'$이 다시 $f$가 되는 관계로 정의된다. 가령 $n \geq 0$인 정수에 대해 $z^n$은 평면 전체에서 원시함수 $z^{n+1}/(n+1)$을 가지고, $e^z$는 자기 자신을 원시함수로 가진다 ([§복소정칙함수, ⁋정의 10](/ko/math/complex_analysis/holomorphic_functions#def10){: data-lid="v6ebi" data-relation="required" reviewed="" }에서 $(e^z)' = e^z$). 원시함수가 존재할 때, 경로적분은 다음과 같이 끝값의 차로 환원된다.
...
::: 증명
먼저 $\gamma$이 한 구간 $[a, b]$ 전체에서 $C^1$인 경우를 본다. 합성함수 $t \mapsto F(\gamma(t))$를 생각하면, $F$가 holomorphic이므로 [§복소정칙함수, ⁋명제 3](/ko/math/complex_analysis/holomorphic_functions#prop3){: data-lid="6kxae" data-relation="required" reviewed="" }의 합성 규칙 증명에 쓰인 보조함수 논증이 실수 증분 $h \rightarrow 0$에 대해서도 그대로 통하여
...
$$\int_\gamma f\dd{z} = \int_a^b H'(t)\dd{t} = H(b) - H(a) = F(\gamma(b)) - F(\gamma(a))$$

이다. 일반적인 piecewise $C^1$ 곡선의 경우, 분할 $a = t_0 < \cdots < t_n = b$의 각 부분구간 $[t_{k-1}, t_k]$에서 $\gamma$이 $C^1$이므로 위 결과를 적용하면

$$\int_\gamma f\dd{z} = \sum_{k=1}^n \bigl( F(\gamma(t_k)) - F(\gamma(t_{k-1})) \bigr) = F(\gamma(t_n)) - F(\gamma(t_0)) = F(\gamma(b)) - F(\gamma(a))$$

로, 중간항이 망원합으로 소거되어 같은 결론을 얻는다.
:::

정리 8의 우변은 곡선의 시점과 종점에만 의존하고 그 사이를 어떤 경로로 잇는지에는 전혀 무관하다. 이로부터 원시함수의 존재가 곧 경로 독립성과 closed 경로 적분의 소멸로 번역됨이 따라 나온다.

::: 따름정리 9
$\Omega \subseteq \mathbb{C}$가 열려 있고 연속함수 $f$가 $\Omega$에서 원시함수 $F$를 가진다고 하자. 그러면 다음이 성립한다.

1. (경로 독립) 자취가 $\Omega$에 들어 있고 시점·종점이 같은 두 piecewise $C^1$ curve $\gamma_1, \gamma_2$에 대하여 $\int_{\gamma_1} f\dd{z} = \int_{\gamma_2} f\dd{z}$이다.
2. (closed 경로) 자취가 $\Omega$에 들어 있는 임의의 closed piecewise $C^1$ curve $\gamma$에 대하여 $\oint_\gamma f\dd{z} = 0$이다.
:::

::: 증명
(1) $\gamma_1, \gamma_2$이 공통 시점 $p$와 공통 종점 $q$를 가진다고 하자. 정리 8을 각각에 적용하면 $\int_{\gamma_1} f\dd{z} = F(q) - F(p) = \int_{\gamma_2} f\dd{z}$이다.

(2) $\gamma$이 닫혀 있으면 시점과 종점이 같은 점 $p = \gamma(a) = \gamma(b)$이므로, 정리 8에 의해 $\oint_\gamma f\dd{z} = F(p) - F(p) = 0$이다.
:::

따름정리 9는 원시함수의 존재가 적분을 얼마나 단순하게 만드는지를 보여 준다. 원시함수가 있으면 적분은 사실상 끝값의 뺄셈이고, closed 곡선을 따라 한 바퀴 돌면 출발점으로 돌아와 차가 $0$이 된다. 거꾸로 보면, 어떤 closed 곡선에서 적분이 $0$이 아니라는 사실은 그 함수가 해당 영역에서 원시함수를 가질 수 없다는 강한 증거가 된다. 바로 이 현상이 다음 절의 핵심 계산에서 드러난다.
...
::: 명제 10
$r > 0$이고 $\gamma_r(t) = r e^{it}$ ($t \in [0, 2\pi]$) 이 반시계방향으로 한 바퀴 도는 원이라 하자. 그러면 임의의 정수 $n$에 대하여

$$\oint_{\gamma_r} z^n\dd{z} = \begin{cases} 2\pi i, & n = -1, \\ 0, & n \neq -1 \end{cases}$$
...
인데, $m$이 정수이므로 $e^{2\pi i m} = 1$ ([§복소수와 복소평면, ⁋정의 5](/ko/math/complex_analysis/complex_numbers#def5){: data-lid="lqu4z" data-relation="required" reviewed="" }의 Euler 형식에서 $e^{2\pi i m} = \cos(2\pi m) + i\sin(2\pi m) = 1$) 이기 때문이다. 따라서 $n \neq -1$이면 적분이 $0$이다.
:::
...
::: 참고 11
$1/z$이 원시함수를 가지지 못하는 까닭은 그 자연스러운 원시함수인 complex logarithm $\log z$가 원점을 둘러싼 영역에서 단일한 값으로 정의되지 않기 때문이다. 곡선을 따라 한 바퀴 돌면 편각 $\arg z$가 ([§복소수와 복소평면, ⁋정의 5](/ko/math/complex_analysis/complex_numbers#def5){: data-lid="07w86" data-relation="weak" reviewed="" }) $2\pi$만큼 증가하여 $\log z$가 $2\pi i$만큼 점프하는데, 명제 10의 $2\pi i$가 바로 이 점프의 크기이다. 이는 $1/z$이 국소적으로는(원점을 둘러싸지 않는 작은 영역에서는) 원시함수를 갖지만 전역적으로는 갖지 못한다는 뜻으로, 영역의 위상이 적분에 개입하는 첫 신호이다.
:::
