...
우리는 [§체, ⁋명제 2](/ko/math/field_theory/fields#prop2){: data-lid="ysg0g" data-relation="required" }에 의하여 field들 사이의 morphism은 injective이거나 zero map 뿐이라는 것을 살펴보았다. 이번 글에서 우리는 전자의 경우에 대하여 살펴본다. 
...
임의의 $\mathbb{K}$-algebra는 $\mathbb{K}$-module이기도 하므로 그 차원이 잘 정의된다. ([\[다중선형대수학\] §기저, ⁋명제 6](/ko/math/multilinear_algebra/basis_of_free_modules#prop6){: data-lid="4yolo" data-relation="required" })
...
한편 [정의 4](#def4){: data-lid="7v7xb" data-relation="required" }의 field $\mathbb{K}(A)$를 얻기 위해서는 $\mathbb{K}$의 extension $\mathbb{L}$을 하나 고정한 다음, $A$를 포함하는 $\mathbb{L}$의 모든 subextension들을 교집합하면 될 것이다. 한편, 우리는 $\mathbb{K}$의 extension들의 category에서의 morphism은 오직 extension 뿐임을 보였으므로, 다음이 성립한다.
...
::: 증명
우선  $\mathbb{L}$과 $\mathbb{M}_2$가 linearly disjoint하다고 가정하자. 그럼 [명제 12](#prop12){: data-lid="pgho1" data-relation="required" }에 의하여 $\mathbb{L}$와 $\mathbb{M}_1$도 linearly disjoint이다. 한편, $\mathbb{L}$의 $\mathbb{K}$-basis는 $\mathbb{M}_1[\mathbb{L}]$의 $\mathbb{M}_1$-basis이기도 하다. 그런데 가정에 의해 이 basis는 $\mathbb{M}_2$-free이므로, $\mathbb{M}_1[\mathbb{L}]$와 $\mathbb{M}_2$는 $\mathbb{M}_1$ 위에서 linearly disjoint이다. 또, [명제 11](#prop11){: data-lid="3303o" data-relation="required" }에 의해 $\mathbb{L}(\mathbb{M}_1) = \mathbb{M}_1(\mathbb{L})$와 $\mathbb{M}_2$ 역시 $\mathbb{M}_1$ 위에서 linearly disjoint하다.
