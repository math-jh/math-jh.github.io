...
::: 보조정리 2
임의의 subextension $\mathbb{M}\in \mathscr{K}$에 대하여, $\mathbb{L}/\mathbb{M}$ 또한 Galois extension이다. 이 때, Galois group $\Gal(\mathbb{L}/\mathbb{M})$을 자명한 방식으로 $\Gal(\mathbb{L}/\mathbb{K})$의 subgroup으로 보면, 이는 $\Gal(\mathbb{L}/\mathbb{K})$의 *closed* subgroup이며 따라서 $g$가 잘 정의된다. 
:::
::: 증명
...
::: 따름정리 4
[정리 1](#thm1)의 상황에서, closed subgroup $H\in\mathscr{G}$가 $\Gal(\mathbb{L}/\mathbb{K})$의 normal subgroup인 것과 $\mathbb{M}=k(H)$가 $\mathbb{K}$의 Galois extension인 것이 동치이다. 이 경우 restriction은 group isomorphism
