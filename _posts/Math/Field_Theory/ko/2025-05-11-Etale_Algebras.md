...
<ins id="thm4">**정리 4**</ins> 무한한 field $$\mathbb{K}$$와 extension $$\mathbb{L}/\mathbb{K}$$가 주어졌다 하고, $$\mathbb{K}$$-algebra $$A$$를 고정하자. 만일 $$\mathbb{K}$$-algebra homomorphism들 $$u_1,\ldots, u_n:A \rightarrow \mathbb{L}$$에 대하여, 다항식 $$f\in \mathbb{L}[\x_1,\ldots, \x_n]$$이 항등적으로 $$f(u_1,\ldots, u_n)=0$$을 만족한다면 $$f=0$$이어야 한다.
...
<ins id="def10">**정의 10**</ins> 자연수 $$h(\overline{\mathbb{K}})$$를 $$A$$의 *separable degree<sub>분리가능차수</sub>*로 정의하고 $$[A:\mathbb{K}]_s$$로 적는다. 
