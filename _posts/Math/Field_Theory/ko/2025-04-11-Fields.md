...
한편 우리는 임의의 commutative ring $A$와 그 polynomial algebra $E=A[\x_i]_{i\in I}$에 대하여, Kähler differential들의 module $\Omega_{E/A}$를 살펴본 적이 있다. ([\[다중선형대수학\] §미분가군, ⁋예시 10](/ko/math/multilinear_algebra/differential_modules#ex10)) 이 구조에 따르면, 임의의 다항식 $f\in E$에 대하여 $df\in\Omega_{E/A}$는 다음의 식
...
::: 명제 19
Commutative ring $A$, polynomial algebra $E=A[\x_i]_{i\in I}$에 대하여, $E$의 부분집합 $S$를 $df=0$을 만족하는 다항식들 $f$의 집합이라 하자. 그럼 다음이 성립한다. 
