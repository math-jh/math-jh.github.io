...
::: 정의 7
Ring $A$가 subfield를 가진다고 가정하자. 이 때, [정리 4](#thm4)의 증명에서 $f:\mathbb{Z} \rightarrow A$의 kernel의 generator인 양의 정수 $n$을 ring $A$의 *characteristic<sub>표수</sub>*으로 정의하고 $\ch(A)$로 적는다. 
:::
...
즉, field $\mathbb{K}$가 perfect이기 위해서는 (1) characteristic exponent가 $1$이거나, (2) characteristic exponent가 $q$이고 [정의 13](#def13)의 센스에서 perfect ring이어야 한다. 
