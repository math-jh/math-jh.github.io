...
date: 2025-04-11
weight: 1
translated_at: 2026-06-26T17:30:02+00:00
translation_source: kimi-cli
last_polished_at: 2026-06-26T17:30:02+00:00
---
In the posts of this category we discuss fields. In particular, the purpose of this category is to introduce Galois theory.
...
::: Definition 1
A commutative division ring is called a *field*. ([\[Algebraic Structures\] §Field of Fractions, ⁋Definition 3](/en/math/algebraic_structures/field_of_fractions#def3){: data-lid="lugls" data-relation="required" })
:::
...
Earlier we defined every object of $\Ring$ to have $1$, and every ring homomorphism to preserve $1$. ([\[Algebraic Structures\] §Definition of a Ring, ⁋Definition 3](/en/math/algebraic_structures/rings#def3){: data-lid="bjgbo" data-relation="required" }) Thus any subring of a ring $A$ also has a multiplicative identity, which is exactly the multiplicative identity $1$ of $A$. From this we also know that $\mathbb{Z}$ is an initial object of $\Ring$.
...
::: Proposition 2
Let $\mathbb{K},\mathbb{K}'$ be arbitrary fields, and consider a ring homomorphism $f:\mathbb{K} \rightarrow \mathbb{K}'$. Then $f$ is either the zero map or an inclusion.
:::
::: Proof
$\ker f$ is an ideal of $\mathbb{K}$. Now see [\[Algebraic Structures\] §Field of Fractions, ⁋Proposition 4](/en/math/algebraic_structures/field_of_fractions#prop4){: data-lid="eqvq3" data-relation="required" }.
:::

Meanwhile, in the post [\[Algebraic Structures\] §Field of Fractions](/en/math/algebraic_structures/field_of_fractions){: data-lid="r2lsl" data-relation="required" } we obtained $\mathbb{Q}$ as the total field of fractions $\Frac(\mathbb{Z})$ of $\mathbb{Z}$. The following example gives a field different from $\mathbb{Q}$.

::: Example 3
Fix a prime $p$. Then the quotient ring $\mathbb{F}_p := \mathbb{Z}/p\mathbb{Z}$ of $\mathbb{Z}$ is a field.

To verify this, let an arbitrary nonzero $a+p\mathbb{Z}\in \mathbb{F}_p$ be given. Since $\gcd(a,p)=1$, by [\[Ring Theory\] §Integral Domains, ⁋Corollary 9](/en/math/ring_theory/integral_domains#cor9){: data-lid="kk03f" data-relation="required" } there exist $x, b\in \mathbb{Z}$ satisfying
...
::: Theorem 4
Suppose a ring $A$ has some subfield. Then $A$ has a unique prime subfield $\mathbb{P}$. In this case, $\mathbb{P}$ lies in the center of $A$ and is contained in every subfield of $A$.
:::

::: Proof
Let $\mathbb{K}$ be a subfield of $A$. Let $Z$ be the center of $A$; then $\mathbb{K}':= \mathbb{K} \cap Z$ is also a subfield of $A$.

Consider the unique ring homomorphism $f: \mathbb{Z} \rightarrow A$ and let its kernel be $\mathfrak{p}$. Then every subring of $A$ contains $\im(f)$, so in particular $\mathbb{K}'$ also contains $\im(f)$. Now since $\mathbb{K}'$ is a field it is an integral domain, and therefore $(0)$ is a prime ideal of $\mathbb{K}'$; hence by [\[Algebraic Structures\] §Field of Fractions, ⁋Proposition 10](/en/math/algebraic_structures/field_of_fractions#prop10){: data-lid="g00jz" data-relation="required" } we have either $\mathfrak{p} = (0)$ or $\mathfrak{p} = (p)$ for some prime $p$.

If $\mathfrak{p}=0$, then $f$ is an embedding of $\mathbb{Z}$ into $\mathbb{K}'$, and thus by [\[Algebraic Structures\] §Field of Fractions, ⁋Theorem 1](/en/math/algebraic_structures/field_of_fractions#thm1){: data-lid="gfhri" data-relation="required" } we obtain $\mathbb{Q}\hookrightarrow\mathbb{K}'$.
...
Thus in both cases we see that a prime field $\mathbb{P}$ exists inside $\mathbb{K}'$, and this $\mathbb{P}$ is a subfield of $A$ and lies in the center $Z$.
...
::: Corollary 5
A field $\mathbb{K}$ has a unique prime subfield $\mathbb{P}$, and $\mathbb{P}$ is the smallest of the subfields of $\mathbb{K}$.
:::

::: Corollary 6
A necessary and sufficient condition for a field $\mathbb{K}$ to be a prime field is that $\mathbb{K}$ has no subfield other than itself.
:::
...
Using prime fields we can define the characteristic of a ring. Suppose a ring $A$ has some subfield. Then by [Theorem 4](#thm4){: data-lid="4dkil" data-relation="required" } this $A$ has a unique prime subfield $\mathbb{P}$. Moreover, looking at the proof of [Theorem 4](#thm4){: data-lid="4mmbu" data-relation="required" }, the way to obtain this $\mathbb{P}$ is given quite explicitly: since $\mathbb{P}$ must contain the multiplicative identity $1$ of $A$, from this
...
::: Definition 7
Suppose a ring $A$ has a subfield. Then we define the positive integer $n$ generating the kernel of $f:\mathbb{Z} \rightarrow A$ in the proof of [Theorem 4](#thm4){: data-lid="wqh7c" data-relation="required" } as the *characteristic* of the ring $A$, and write it as $\ch(A)$.
:::
...
::: Proposition 8
Let $A$ be a nonzero ring. The following hold:
...
::: Lemma 9
Fix a prime $p$. Then for an integer $i$ with $1 \leq i \leq p-1$, the binomial coefficient $\binom{p}{i}$ is a multiple of $p$.
:::
...
::: Theorem 10
Let $A$ be a commutative ring of characteristic $p>0$. Then the function
...
::: Proof
The identity for multiplication requires no further proof since $A$ is commutative. For the first identity, considering the binomial expansion

$$(a + b)^p = a^p + b^p + \sum_{i = 1}^{p-1} \binom{p}{i}. a^i b^{p-i}$$

by [Lemma 9](#lem9){: data-lid="39ccx" data-relation="required" } we have $\binom{p}{i} \equiv 0 \pmod{p}$ for all $i$ with $1 \leq i \leq p-1$, and therefore by [Proposition 8](#prop8){: data-lid="9jna7" data-relation="required" }
...
::: Definition 11
When a commutative ring $A$ has characteristic $p$, the function of [Theorem 10](#thm10){: data-lid="e05mq" data-relation="required" }
...
We introduce a few more notations. For a subring $K$ of $A$ and an arbitrary subset $S\subseteq A$, we denote by $K[S]$ the subring of $A$ generated by $K\cup S$. If $A$ is a field, then by [\[Algebraic Structures\] §Field of Fractions, ⁋Theorem 1](/en/math/algebraic_structures/field_of_fractions#thm1){: data-lid="doxai" data-relation="required" } from $K[S]\hookrightarrow A$ a ring homomorphism
...
::: Proposition 12
Fix a commutative ring $A$ of characteristic $p>0$, a subring $K$ of $A$, a subset $S$ of $A$, and a positive integer $f$.
...
2. Since $K[S]$ is generated by $K$-linear combinations of the $a_i$, from the binomial expansion and [Theorem 10](#thm10){: data-lid="zyyb7" data-relation="required" } we know that $K[S]^{p^f}$ is generated by $K^{p^f}$-linear combinations of the respective $a_i^{p^f}$.
:::
...
::: Definition 13
Consider a ring $A$ of characteristic $p \neq 0$ and the Frobenius map $\Frob_p : A \rightarrow A$.
...
::: Definition 14
For a commutative ring $A$ of characteristic $p \ne 0$, a *perfect closure* is a pair $(A^{1/p^\infty}, \phi)$ satisfying the following two conditions.

- $A^{1/p^\infty}$ is a perfect ring of characteristic $p$.
- $\phi : A \rightarrow A^{1/p^\infty}$ is a ring homomorphism satisfying the following universal property.
    >
...
::: Proposition 16
Consider a field $\mathbb{K}$ of characteristic exponent $q$. Then for each $f\geq 0$, the map $x\mapsto x^{q^f}$ induces an isomorphism from $\mathbb{K}$ onto some subfield $\mathbb{K}^{q^f}$ of $\mathbb{K}$.
:::
::: Proof
...
::: Definition 17
For a field $\mathbb{K}$ of characteristic exponent $q$, if the function of [Proposition 16](#prop16){: data-lid="1gpqv" data-relation="required" } is an isomorphism from $\mathbb{K}$ onto $\mathbb{K}$ we call $\mathbb{K}$ a *perfect field*; otherwise we call it an *imperfect field*.
:::

That is, for a field $\mathbb{K}$ to be perfect, either (1) the characteristic exponent is $1$, or (2) the characteristic exponent is $q$ and it is a perfect ring in the sense of [Definition 13](#def13){: data-lid="uq1ta" data-relation="required" }.

Then the following holds.

::: Proposition 18
For a field $\mathbb{K}$, the following hold.
...
::: Proof
It suffices to show only the second claim; we already observed in the proof of that proposition that the function of [Proposition 16](#prop16){: data-lid="n68tr" data-relation="required" }, $\mathbb{K} \rightarrow \mathbb{K}^{q^f}\subseteq \mathbb{K}$, is always injective regardless of conditions on $\mathbb{K}$. Now if $\mathbb{K}$ is a finite set, then by its size we must have $\mathbb{K}^{q^f}=\mathbb{K}$.
:::

Meanwhile, for an arbitrary commutative ring $A$ and its polynomial algebra $E=A[\x_i]_{i\in I}$, we have examined the module of Kähler differentials $\Omega_{E/A}$. ([\[Multilinear Algebra\] §Differential Modules, ⁋Example 10](/en/math/multilinear_algebra/differential_modules#ex10){: data-lid="9df1b" data-relation="required" }) According to this structure, for an arbitrary polynomial $f\in E$ the element $\dd{f}\in\Omega_{E/A}$ is given by the formula
...
::: Proposition 19
For a commutative ring $A$ and polynomial algebra $E=A[\x_i]_{i\in I}$, let $S$ be the subset of $E$ consisting of polynomials $f$ satisfying $\dd{f}=0$. Then the following hold.

1. If $\ch(A)=0$ then $S=A$.
2. If $\ch(A)=p$ then $S=A[\x_i^p]_{i\in I}$, and moreover if $A$ is perfect then $S=E^p$ holds.
:::
