...
::: Definition 1
A commutative division ring is called a *field*. ([\[Algebraic Structures\] §Field of Fractions, ⁋Definition 3](/en/math/algebraic_structures/field_of_fractions#def3){: data-relation="required" })
:::
...
Earlier we defined every object of $\Ring$ to have $1$, and every ring homomorphism to preserve $1$. ([\[Algebraic Structures\] §Definition of a Ring, ⁋Definition 3](/en/math/algebraic_structures/rings#def3){: data-relation="required" }) Thus any subring of a ring $A$ also has a multiplicative identity, which is exactly the multiplicative identity $1$ of $A$. From this we also know that $\mathbb{Z}$ is an initial object of $\Ring$.
...
:::
::: Proof
$\ker f$ is an ideal of $\mathbb{K}$. Now see [\[Algebraic Structures\] §Field of Fractions, ⁋Proposition 4](/en/math/algebraic_structures/field_of_fractions#prop4){: data-relation="required" }.
:::

Meanwhile, in the post [\[Algebraic Structures\] §Field of Fractions](/en/math/algebraic_structures/field_of_fractions){: data-relation="weak" } we obtained $\mathbb{Q}$ as the total field of fractions $\Frac(\mathbb{Z})$ of $\mathbb{Z}$. The following example gives a field different from $\mathbb{Q}$.

::: Example 3
Fix a prime $p$. Then the quotient ring $\mathbb{F}_p := \mathbb{Z}/p\mathbb{Z}$ of $\mathbb{Z}$ is a field.

To verify this, let an arbitrary nonzero $a+p\mathbb{Z}\in \mathbb{F}_p$ be given. Since $\gcd(a,p)=1$, by [\[Ring Theory\] §Integral Domains, ⁋Corollary 9](/en/math/ring_theory/integral_domains#cor9){: data-relation="required" } there exist $x, b\in \mathbb{Z}$ satisfying
...
::: Definition 7
Suppose a ring $A$ has a subfield. Then we define the positive integer $n$ generating the kernel of $f:\mathbb{Z} \rightarrow A$ in the proof of [Theorem 4](#thm4){: data-relation="required" } as the *characteristic* of the ring $A$, and write it as $\ch(A)$.
:::
...
by [Lemma 9](#lem9){: data-relation="required" } we have $\binom{p}{i} \equiv 0 \pmod{p}$ for all $i$ with $1 \leq i \leq p-1$, and therefore by [Proposition 8](#prop8){: data-relation="required" }
...
::: Definition 11
When a commutative ring $A$ has characteristic $p$, the function of [Theorem 10](#thm10){: data-relation="required" }
...
2. Since $K[S]$ is generated by $K$-linear combinations of the $a_i$, from the binomial expansion and [Theorem 10](#thm10){: data-relation="required" } we know that $K[S]^{p^f}$ is generated by $K^{p^f}$-linear combinations of the respective $a_i^{p^f}$.
:::
...
::: Definition 17
For a field $\mathbb{K}$ of characteristic exponent $q$, if the function of [Proposition 16](#prop16){: data-relation="required" } is an isomorphism from $\mathbb{K}$ onto $\mathbb{K}$ we call $\mathbb{K}$ a *perfect field*; otherwise we call it an *imperfect field*.
:::

That is, for a field $\mathbb{K}$ to be perfect, either (1) the characteristic exponent is $1$, or (2) the characteristic exponent is $q$ and it is a perfect ring in the sense of [Definition 13](#def13){: data-relation="weak" }.
...
::: Proof
It suffices to show only the second claim; we already observed in the proof of that proposition that the function of [Proposition 16](#prop16){: data-relation="required" }, $\mathbb{K} \rightarrow \mathbb{K}^{q^f}\subseteq \mathbb{K}$, is always injective regardless of conditions on $\mathbb{K}$. Now if $\mathbb{K}$ is a finite set, then by its size we must have $\mathbb{K}^{q^f}=\mathbb{K}$.
:::

Meanwhile, for an arbitrary commutative ring $A$ and its polynomial algebra $E=A[\x_i]_{i\in I}$, we have examined the module of Kähler differentials $\Omega_{E/A}$. ([\[Multilinear Algebra\] §Differential Modules, ⁋Example 10](/en/math/multilinear_algebra/differential_modules#ex10){: data-relation="required" }) According to this structure, for an arbitrary polynomial $f\in E$ the element $\dd{f}\in\Omega_{E/A}$ is given by the formula
