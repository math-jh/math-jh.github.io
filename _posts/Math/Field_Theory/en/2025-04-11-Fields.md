...
::: Definition 1
A commutative division ring is called a *field*. ([\[Algebraic Structures\] §Field of Fractions, ⁋Definition 3](/en/math/algebraic_structures/field_of_fractions#def3))
:::
...
Earlier we defined every object of $\Ring$ to have $1$, and every ring homomorphism to preserve $1$. ([\[Algebraic Structures\] §Definition of a Ring, ⁋Definition 3](/en/math/algebraic_structures/rings#def3)) Thus any subring of a ring $A$ also has a multiplicative identity, which is exactly the multiplicative identity $1$ of $A$. From this we also know that $\mathbb{Z}$ is an initial object of $\Ring$.
...
Meanwhile, in the post [\[Algebraic Structures\] §Field of Fractions](/en/math/algebraic_structures/field_of_fractions) we obtained $\mathbb{Q}$ as the total field of fractions $\Frac(\mathbb{Z})$ of $\mathbb{Z}$. The following example gives a field different from $\mathbb{Q}$.

::: Example 3
Fix a prime $p$. Then the quotient ring $\mathbb{F}_p := \mathbb{Z}/p\mathbb{Z}$ of $\mathbb{Z}$ is a field.

To verify this, let an arbitrary nonzero $a+p\mathbb{Z}\in \mathbb{F}_p$ be given. Since $\gcd(a,p)=1$, by [\[Ring Theory\] §Integral Domains, ⁋Corollary 9](/en/math/ring_theory/integral_domains#cor9) there exist $x, b\in \mathbb{Z}$ satisfying
...
Using prime fields we can define the characteristic of a ring. Suppose a ring $A$ has some subfield. Then by [Theorem 4](#thm4) this $A$ has a unique prime subfield $\mathbb{P}$. Moreover, looking at the proof of [Theorem 4](#thm4), the way to obtain this $\mathbb{P}$ is given quite explicitly: since $\mathbb{P}$ must contain the multiplicative identity $1$ of $A$, from this
...
::: Definition 7
Suppose a ring $A$ has a subfield. Then we define the positive integer $n$ generating the kernel of $f:\mathbb{Z} \rightarrow A$ in the proof of [Theorem 4](#thm4) as the *characteristic* of the ring $A$, and write it as $\ch(A)$.
:::
...
::: Definition 11
When a commutative ring $A$ has characteristic $p$, the function of [Theorem 10](#thm10)
...
::: Definition 17
For a field $\mathbb{K}$ of characteristic exponent $q$, if the function of [Proposition 16](#prop16) is an isomorphism from $\mathbb{K}$ onto $\mathbb{K}$ we call $\mathbb{K}$ a *perfect field*; otherwise we call it an *imperfect field*.
:::
...
::: Proof
It suffices to show only the second claim; we already observed in the proof of that proposition that the function of [Proposition 16](#prop16), $\mathbb{K} \rightarrow \mathbb{K}^{q^f}\subseteq \mathbb{K}$, is always injective regardless of conditions on $\mathbb{K}$. Now if $\mathbb{K}$ is a finite set, then by its size we must have $\mathbb{K}^{q^f}=\mathbb{K}$.
:::
