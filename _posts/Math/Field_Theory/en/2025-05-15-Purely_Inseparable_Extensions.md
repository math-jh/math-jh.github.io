...
Now suppose $e\geq 1$, and assume that $a=b^p$ for some $b\in\mathbb{K}$. Then $(x^{p^{e-1}})^p=x^{p^e}=a=b^p$, but since the Frobenius endomorphism is always injective in a field, we get $x^{p^{e-1}}=b\in \mathbb{K}$, contradicting the minimality of $e$. Hence $a\not\in \mathbb{K}^p$, and by [Lemma 3](#lem3){: data-lid="5mf0e" data-relation="required" }, $\x^{p^e}-a$ is irreducible. This is a monic polynomial having $x$ as a root, so it is the minimal polynomial of $x$, and since its degree is $p^e$, we have $[\mathbb{K}(x):\mathbb{K}]=p^e$.
:::

The following definition would have been natural even immediately after [Definition 1](#def1){: data-lid="famqy" data-relation="weak" }.
...
In [§Algebraic Closures](/en/math/field_theory/algebraically_closed_extensions){: data-lid="hnzgf" data-relation="required" } we saw that every field $\mathbb{K}$ has an algebraic closure $\overline{\mathbb{K}}$. Hence in [Proposition 5](#prop5){: data-lid="hx9dy" data-relation="required" } we may take $\mathbb{L}=\overline{\mathbb{K}}$. Then $\overline{\mathbb{K}}$ is a perfect field, and moreover we know that for each $n$, $\overline{\mathbb{K}}_n$ is exactly $\mathbb{K}^{1/p^n}$. Let us write the (relative) $p$-radical closure in this situation as $\mathbb{K}^{1/p^\infty}$. This is the same thing as the perfect closure of $\mathbb{K}$ defined in [§Fields, ⁋Definition 14](/en/math/field_theory/fields#def14){: data-lid="0yhn5" data-relation="required" } and whose existence was shown in [§Fields, ⁋Theorem 15](/en/math/field_theory/fields#thm15){: data-lid="z95z3" data-relation="required" }. If $\mathbb{K}$ is imperfect, i.e. $\mathbb{K}\neq \mathbb{K}^p$, then the above ascending sequence is strictly increasing, and therefore $\mathbb{K}^{1/p^\infty}/\mathbb{K}$ becomes an extension of infinite degree.
...
::: Example 9
Consider the field $\mathbb{K}=\mathbb{F}_p(t)$. Since the elements of $\mathbb{F}_p$ are fixed by the Frobenius endomorphism, we have $\mathbb{K}^p=\mathbb{F}_p(t^p)$, and hence $t\not\in \mathbb{K}^p$. Then the polynomial $f(\x)=\x^p-t\in \mathbb{K}[\x]$ is irreducible by [Lemma 3](#lem3){: data-lid="im3it" data-relation="required" }, so $\mathbb{L}=\mathbb{K}[\x]/(\x^p-t)$ is an extension of $\mathbb{K}$. If we let $\alpha$ be the residue class of $\x$, then $\alpha^p=t\in\mathbb{K}$ and $\alpha\not\in \mathbb{K}$, so $\alpha$ is a $p$-radical element of height $1$, and therefore $\mathbb{L}/\mathbb{K}$ is a $p$-radical extension and the minimal polynomial of $\alpha$ is $f(\x)$. ([Proposition 2](#prop2){: data-lid="b7e39" data-relation="required" }) Differentiating this gives $Df=p\x^{p-1}=0$, so by [\[Ring Theory\] §Polynomial Rings, ⁋Proposition 11](/en/math/ring_theory/polynomial_rings#prop11){: data-lid="itccv" data-relation="required" } we know that $\alpha$ is a multiple root of $f$. In fact, by [§Fields, ⁋Theorem 10](/en/math/field_theory/fields#thm10){: data-lid="qa1rv" data-relation="required" } we have $(\x-\alpha)^p=\x^p-\alpha^p=\x^p-t$, so $\alpha$ has multiplicity $p$.
:::
