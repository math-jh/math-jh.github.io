...
Therefore, by this proposition, the connected component containing $\id_\mathbb{L}$ is $\left\{\id_\mathbb{L}\right\}$. On the other hand, by [Proposition 2](#prop2){: data-lid="tbehr" }, left translation by any $\sigma$ is a homeomorphism, so the connected component containing any point is also a singleton, and hence $\Gal(\mathbb{L}/\mathbb{K})$ is a totally disconnected space. ([\[Topology\] §Connected Spaces, ⁋Definition 7](/en/math/topology/connected_spaces#def7){: data-lid="qk2i3" }) Moreover, the following holds.
