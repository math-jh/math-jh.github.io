...
defines a function $D:A \rightarrow \mathfrak{m}/\mathfrak{m}^2$ that is evidently a $\mathbb{K}$-derivation. By the universal property of [\[Multilinear Algebra\] §Differential Modules, ⁋Proposition 8](/en/math/multilinear_algebra/differential_modules#prop8){: data-lid="3fmpo" }, $D$ must factor through $\Omega_{A/\mathbb{K}}$, which is $0$ by assumption, so $D=0$.
:::
...
:::
::: Proof
First, since both $\Omega_{A/\mathbb{K}}$ and the notion of an étale algebra behave well under base change, we may assume that $\mathbb{K}$ is algebraically closed. More precisely, $A$ being étale is equivalent to the $\overline{\mathbb{K}}$-algebra $A_{(\overline{\mathbb{K}})}=A\otimes_\mathbb{K}\overline{\mathbb{K}}$ being diagonalizable for the algebraic closure $\overline{\mathbb{K}}$ of $\mathbb{K}$ ([§Étale Algebras, ⁋Proposition 8](/en/math/field_theory/etale_algebras#prop8){: data-relation="required" }), while by the canonical isomorphism of [\[Multilinear Algebra\] §Differential Modules, ⁋Proposition 12](/en/math/multilinear_algebra/differential_modules#prop12){: data-lid="7ekiz" } and the definition of the $\overline{\mathbb{K}}$-algebra $A_{(\overline{\mathbb{K}})}$ we obtain the isomorphism
...
and multiplying both sides by $e$ gives $e\dd{(e)}=0$, so substituting back we see that $\dd{e}=0$. On the other hand, $\Omega_{A/\mathbb{K}}$ is explicitly presented as a free module generated by such elements $\dd{e}$ modulo appropriate relations ([\[Multilinear Algebra\] §Differential Modules, ⁋Example 10](/en/math/multilinear_algebra/differential_modules#ex10){: data-lid="8jvjx" }), from which we conclude that $\Omega_{A/\mathbb{K}}=0$.

Conversely, assume $\Omega_{A/\mathbb{K}}=0$ and let us show that $A$ is diagonalizable. We proceed by induction on the degree of $A$; the degree $1$ case is trivial. For the general case, choose a maximal ideal $\mathfrak{m}$ of $A$. Then by [Lemma 1](#lem1){: data-lid="mpa5p" data-relation="required" } we have $\mathfrak{m}=\mathfrak{m}^2$, so applying [Lemma 2](#lem2){: data-relation="required" } we find an idempotent $e$ such that $\mathfrak{m}=Ae$. Then decomposing $A$ as a direct sum of $\mathfrak{a}=(1-e)A$ and $\mathfrak{m}$, we see from the assumption that $\mathbb{K}$ is algebraically closed that the extension $A/\mathfrak{m}$ has degree $1$. That is, we can write this direct sum as

$$A\cong \mathfrak{a}\oplus\mathfrak{m}\cong \mathbb{K}\times A/\mathfrak{a}$$

Now since $\Omega$ is a right exact functor ([\[Multilinear Algebra\] §Differential Modules, ⁋Proposition 13](/en/math/multilinear_algebra/differential_modules#prop13){: data-lid="649h0" }), $\Omega_{(A/\mathfrak{a})/\mathbb{K}}$ is a quotient of $\Omega_{A/\mathbb{K}}=0$ and hence is $0$. Therefore, by the inductive hypothesis $A/\mathfrak{a}$ is diagonalizable, and so is $A$, which is its product with $\mathbb{K}$.
:::

Earlier, in [§Purely Inseparable Extensions, ⁋Example 9](/en/math/field_theory/purely_inseparable_extensions#ex9){: data-lid="ys0cs" data-relation="weak" }, we examined a situation that could cause problems when developing Galois theory; let us revisit this example in light of [Theorem 3](#thm3){: data-relation="required" }.

::: Example 4
A favorable case is $\mathbb{Q}(\sqrt{2})/\mathbb{Q}$, as we saw in the introduction to [§Purely Inseparable Extensions](/en/math/field_theory/purely_inseparable_extensions){: data-lid="ek8pv" data-relation="weak" }. From the computation in [\[Multilinear Algebra\] §Differential Modules, ⁋Example 10](/en/math/multilinear_algebra/differential_modules#ex10){: data-lid="vk5z0" }, we know that $\Omega_{\mathbb{Q}[\x]/\mathbb{Q}}$ is the free $\mathbb{Q}[\x]$-module generated by $\dd{\x}$. On the other hand, considering the ideal $\mathfrak{I}=(\x^2-2)$ of $\mathbb{Q}[\x]$, from [\[Multilinear Algebra\] §Differential Modules, ⁋Proposition 14](/en/math/multilinear_algebra/differential_modules#prop14){: data-lid="oenpk" } we have the exact sequence
...
To see this, we show that $A$ has an idempotent other than $0,1$. Any ideal of $A$ is a finite-dimensional $\mathbb{K}$-vector space, so we can choose an ideal $\mathfrak{a}$ of $A$ of minimal dimension as a $\mathbb{K}$-vector space. Then by the minimality of $\mathfrak{a}$ and the assumption that $A$ is reduced, we have $\mathfrak{a}^2=\mathfrak{a}$, so [Lemma 2](#lem2){: data-relation="required" } applies.
:::

Then the claim of [Proposition 9](#prop9){: data-relation="forward" } mentioned above is essentially contained in the following lemma.

::: Lemma 6
For a perfect field $\mathbb{K}$, any finite degree reduced $\mathbb{K}$-algebra is étale.
:::
::: Proof
Consider a $\mathbb{K}$-algebra $A$ satisfying the hypotheses. First, by [Proposition 5](#prop5){: data-lid="bufnc" data-relation="required" } there exist extensions such that $A\cong \mathbb{L}_1\times\cdots \times\mathbb{L}_n$. Since the product of étale algebras is étale, the claim reduces to showing that any finite degree field extension over a perfect field $\mathbb{K}$ is étale. Thus by [Theorem 3](#thm3){: data-relation="required" }, it suffices to show that $\dd{\alpha}=0$ for any $\alpha\in A$. By the computation in [Example 4](#ex4){: data-relation="required" }, we know that the equation $f'(\alpha)\dd{\alpha}=0$ must hold, and by similar logic we must show that $f'(\alpha)\neq 0$.
...
:::
::: Proof
That the second condition implies the third is trivial from the existence of a perfect closure, and that the first implies the second is the result of [§Étale Algebras, ⁋Proposition 8](/en/math/field_theory/etale_algebras#prop8){: data-relation="required" }. Thus it suffices to show that the third implies the first. This follows again from [Lemma 6](#lem6){: data-lid="w3awf" data-relation="required" } and [§Étale Algebras, ⁋Proposition 8](/en/math/field_theory/etale_algebras#prop8){: data-relation="required" }.
:::
...
Each $\mathbb{K}[x_i]$ is a separable extension by the assumption on $A$, so they are étale; then $\mathbb{K}[x_1,\ldots, x_m]$ is obtained from their tensor product $\mathbb{K}[x_1]\otimes\cdots\otimes \mathbb{K}[x_n]$ by quotienting by the relations expressing their associativity and commutativity, and since $\mathbb{M}$ is a subalgebra of this, $\mathbb{M}$ is étale by [§Étale Algebras, ⁋Corollary 14](/en/math/field_theory/etale_algebras#cor14){: data-relation="required" }.
:::
...
[Theorem 14](#thm14){: data-relation="forward" } shows that a primitive element can always be found for finite degree separable extensions. For this we need the following lemma.
...
Now assuming the second condition, since $\mathbb{L}/\mathbb{K}$ has only finitely many subextensions, setting $A=\mathbb{L}$ satisfies the hypotheses of [Lemma 13](#lem13){: data-relation="required" }, and we obtain the desired result.
:::
...
[Theorem 14](#thm14){: data-relation="weak" } always holds even when $\mathbb{K}$ is a finite field, but to prove this requires a somewhat more delicate counting argument than [Lemma 13](#lem13){: data-relation="weak" }, so we postpone it.

On the other hand, separability is essentially (almost) an étale algebra, and since étale algebras behave well under base change ([§Étale Algebras, ⁋Corollary 14](/en/math/field_theory/etale_algebras#cor14){: data-relation="weak" }), with slight modifications as in the proof of [Proposition 12](#prop12){: data-relation="weak" }, we can show that separability also behaves well under base change in the following two cases.
