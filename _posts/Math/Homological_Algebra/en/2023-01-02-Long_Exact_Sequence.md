...
The main theorem of this post is the following [Theorem 1](#thm1){: data-lid="8gqgm" }. The snake lemma plays a key role in its proof, and when the category is $\lMod{A}$, the proof is easily completed using the explicitly constructed connecting map. Thus, just as in [§Diagram chasing](/en/math/homological_algebra/diagram_chasing){: data-lid="2megj" }, we make free use of the Freyd–Mitchell embedding theorem and carry out the proof of the following theorem in $\lMod{A}$.
...
Applying [§Diagram chasing, ⁋Lemma 5](/en/math/homological_algebra/diagram_chasing#lem5){: data-lid="plcik" } to this diagram, we obtain the two exact sequences
...
commutes. Following the construction in the proof of [Theorem 1](#thm1){: data-lid="aja6h" }, $\partial$ is computed as follows. For any $[c]\in H_n(C)$, since $v_n$ is surjective there exists $b\in B_n$ with $v_n(b)=c$, and since $v_{n-1}(d_n^Bb)=d_n^Cv_n(b)=d_n^Cc=0$, exactness gives a unique $a\in A_{n-1}$ with $u_{n-1}(a)=d_n^Bb$, and $\partial[c]=[a]$. One can also check that this value is independent of the choice of lift $b$: if $v_n(b)=v_n(\tilde{b})=c$, then there exists $\alpha\in A_n$ with $b-\tilde{b}=u_n(\alpha)$, and the two corresponding elements $a,\tilde{a}$ then satisfy $u_{n-1}(a-\tilde{a})=d^B_n(b-\tilde{b})=u_{n-1}(d^A_n\alpha)$, so by the injectivity of $u_{n-1}$ we have $a-\tilde{a}=d_n^A\alpha$, whence $[a]=[\tilde{a}]$.
...
Thanks to [Proposition 6](#prop6){: data-lid="fh6iy" }, we can define the *homotopy category* $\mathbf{K}(\mathcal{A})$ through the following procedure. First, the following lemma is immediate.
...
Here, $D \rightarrow\Cone(f)$ sends $y$ to $(0,y)$, and $\delta$ sends $(x,y)$ to $-x$. Since it is immediate from the definitions of these maps that the sequence above is a short exact sequence, [Theorem 1](#thm1){: data-lid="nxgqc" } gives the long exact sequence
