...
In [\[Category Theory\] §Abelian Categories](/en/math/category_theory/abelian_categories){: data-lid="rr659" } we mentioned that the category of chain complexes, $\Ch(\mathcal{A})$, exists and is an abelian category. In this section we examine this claim more concretely. First, morphisms in $\Ch(\mathcal{A})$ are given as follows.
...
:::
::: Proof
For an arbitrary chain map $f_\bullet:C_\bullet\rightarrow D_\bullet$, applying [§Diagram chasing, ⁋Lemma 4](/en/math/homological_algebra/diagram_chasing#lem4){: data-lid="art4w" } shows that $f_n$ induces $Z_n(C)\rightarrow Z_n(D)$ and $B_n(C)\rightarrow B_n(D)$. Therefore $f_n$ induces
