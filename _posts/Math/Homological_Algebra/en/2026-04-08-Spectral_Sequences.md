...
As mentioned above, the following [Definition 2](#def2){: data-lid="4s5zq" } is a much more general version, but the philosophy of decomposing a complex more finely remains essentially the same.
...
::: Proposition 7
The $E_r^{p,q}$ and $d_r$ constructed from a filtered complex $(A^\bullet, F)$ as above satisfy the conditions of a spectral sequence in [Definition 1](#def1){: data-lid="4ju1l" }. Namely,
...
That is, fixing [Definition 2](#def2){: data-lid="pviw6" } in degree $n$ and considering the filtration
...
::: Proposition 10
Let $(A^\bullet, F)$ be a bounded filtered complex, and let $(E_r^{p,q})$ be the spectral sequence it defines. Then $(E_r^{p,q})$ converges to the filtered graded object $(H^\bullet, F)$ of [Definition 3](#def3){: data-lid="1nm6o" }. That is, $E_r^{p,q}\Rightarrow H^{p+q}(A^\bullet)$.
:::

## Spectral Sequences of Double Complexes

We have taken the proof of the balancing of $\Ext$ and $\Tor$ in [§Ext and Tor, ⁋Proposition 3](/en/math/homological_algebra/ext_and_tor#prop3){: data-lid="pysxe" } as the motivation for our theory. We close this post by examining the spectral sequence defined from a double complex.
...
In particular, let $K^{p,q}$ be a first quadrant double complex. Then both filtrations define bounded filtered complexes, so by [Proposition 10](#prop10){: data-lid="sqlml" } each spectral sequence converges to $H^\bullet(\Tot(K))$. From this we can reconstruct the proof of [§Ext and Tor, ⁋Proposition 3](/en/math/homological_algebra/ext_and_tor#prop3){: data-lid="31ppi" } in fancier language.
