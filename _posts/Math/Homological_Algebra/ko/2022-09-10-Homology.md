...
<ins id="def5">**정의 5**</ins> Double complex $$C_{p,q}$$의 *total complex* $$\mathrm{Tot}(C)_\bullet$$는 각 차수 $$n$$에서

$$ (\mathrm{Tot}(C))_n = \bigoplus_{p+q=n} C_{p,q} $$

로 정의되며, 그 differential은

$$ d = d^h + (-1)^p d^v : (\mathrm{Tot}(C))_n \rightarrow (\mathrm{Tot}(C))_{n-1} $$
...
$$d^2=0$$임은 다음과 같이 확인한다:

$$ (d^h + (-1)^p d^v)^2 = (d^h)^2 + (-1)^p d^vd^h + (-1)^{p-1} d^hd^v + (d^v)^2 $$

여기서 $$(d^h)^2=0$$, $$(d^v)^2=0$$이며, double complex의 조건 $$d^vd^h + d^hd^v=0$$에서 $$d^vd^h = -d^hd^v$$이므로 나머지 두 항도 상쇄된다.
...
</div> $$f[p]:C[p]\rightarrow D[p]$$을 만들 수 있다. 즉 translation은 $$\mathbf{Ch}(\mathcal{C})$$에서 자기 자신으로의 functor이다.
