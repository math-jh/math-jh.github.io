...
:::
::: 증명
두 개의 projective resolution들을 두고, [§분해, ⁋정리 6](/ko/math/homological_algebra/resolutions#thm6){: data-lid="ah953" }을 identity map에 대해 적용하면 된다.
:::
...
이 주어졌다 하자. $A$와 $C$의 projective resolution들 $P_\bullet$, $R_\bullet$이 주어졌다 하면 [§분해, ⁋보조정리 7](/ko/math/homological_algebra/resolutions#lem7){: data-lid="s9agy" }을 이용하여 projective resolution $Q_\bullet \rightarrow B$를 얻는다. 한편, 각각의 $n$에 대해 $R_n$이 projective이므로 
...
또한 short exact sequence이며 ([\[다중선형대수학\] §Hom과 텐서곱, ⁋명제 1](/ko/math/multilinear_algebra/hom_and_tensor#prop1){: data-lid="hbaz7" }), 여기에서 homology sequence를 생각하면 원하는 connecting map들과, left derived functor들의 long exact sequence
...
를 얻는다. 이렇게 얻어진 정보가 [정의 1](#def1){: data-lid="7kyzg" }의 두 번째 조건을 만족한다는 것은 [§분해, ⁋정리 6](/ko/math/homological_algebra/resolutions#thm6){: data-lid="zwx4w" }을 사용하면 된다.
:::

뿐만 아니라, 이들은 [정의 3](#def3){: data-lid="ejbqi" }의 센스에서 *universal* homological $\delta$-functor를 정의한다. 이에 대한 증명은 생략하였다. 
...
위에서의 논의와 마찬가지로, 우리는 left exact functor에 대해서도 right derived functor를 정의할 수 있다. 그 정의는 [정의 4](#def4){: data-lid="2m78g" }의 "dual"이다.
...
그럼 이들이 universal cohomological $\delta$-functor들이라는 것도 알 수 있다. [정의 4](#def4){: data-lid="jy33o" }와 다르게 위첨자를 쓰는 이유는 이들이 말 그대로 *cohomological* $\delta$-functor들로서, 주로 cohomology에 관련된 것들을 다룰 때 나오기 때문이다.
