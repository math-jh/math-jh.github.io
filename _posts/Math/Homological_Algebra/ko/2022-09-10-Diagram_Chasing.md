...
Homology 대수학은 말 그대로 chain complex들이 주어졌을 때 이들의 homology를 이용하여 그 성질을 살펴보는 학문이다. ([\[범주론\] §아벨 카테고리, ⁋정의 4](/ko/math/category_theory/abelian_categories#def4){: data-lid="yy10y" data-relation="weak" reviewed="" }) Chain complex는 임의의 abelian category에서 정의되지만, Freyd-Mitchell embedding theorem에 의해 이들은 모두 적당한 $\lMod{A}$의 full subcategory로 embed될 수 있다. ([\[범주론\] §아벨 카테고리, ⁋정리 8 (Freyd-Mitchell embedding theorem)](/ko/math/category_theory/abelian_categories#thm8){: data-lid="0tnor" data-relation="required" reviewed="" }) 
...
남은 글에서 우리의 주된 목표는 snake lemma를 증명하는 것인데, 이를 위해서는 두 개의 보조정리가 필요하다.
...
을 유도하는 것은 [보조정리 4](#lem4){: data-lid="yt1hi" data-relation="required" reviewed="" }의 결과이다. 뿐만 아니라, $i_A, i_B, i_C$를 각각 kernel들에서 $A,B,C$로의 자명한 함수들이라 하면
...
이 주어졌다 하자. 이 때, 위와 아래의 행은 각각 exact이다. 그럼 [보조정리 5](#lem5){: data-lid="ug6c6" data-relation="required" reviewed="" }에서부터 얻어진 두 개의 exact sequence 
