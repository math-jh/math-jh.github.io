...
우리는 [\[다중선형대수학\] §사영가군, 단사가군, 평탄가군, ⁋정의 3](/ko/math/multilinear_algebra/various_modules#def3){: data-lid="r6nwy" data-relation="required" }에서 projective module과 injective module을 정의하였다. 이를 diagram의 언어로 바꾸어 쓰면 일반적인 abelian category에서 projective object와 injective object의 개념을 얻는다.
...
이번 글에서 우리의 목표 중 하나는 임의의 $A$-module은 항상 projective resolution과 injective resolution을 갖는다는 것을 증명하는 것이다. [명제 3](#prop3){: data-lid="f3ivp" data-relation="required" }을 사용하면 이는 $\lMod{A}$가 enough projective와 enough injective를 갖는다는 것을 증명하면 충분하다. $\lMod{A}$가 enough projective를 갖는다는 것은 자명하다.

::: 명제 4
Category $\lMod{A}$는 enough projective를 갖는다.
:::
::: 증명
임의의 $M\in\lMod{A}$에 대하여 [\[다중선형대수학\] §기저, ⁋명제 2](/ko/math/multilinear_algebra/basis_of_free_modules#prop2){: data-lid="39ptc" data-relation="required" }로부터 free module $F$와 surjection $F \rightarrow M$을 얻고, [\[다중선형대수학\] §사영가군, 단사가군, 평탄가군, ⁋명제 4](/ko/math/multilinear_algebra/various_modules#prop4){: data-lid="vumg5" data-relation="required" }로부터 $F$는 projective이다.
:::
...
이제 $P_0''$이 projective라는 조건으로부터 $P_0'' \rightarrow A$을 정의할 수 있다. 한편 $P_0' \rightarrow A$는 $i$와 $\epsilon'$의 합성으로 이미 주어지므로, 이들의 direct sum을 생각하면 $\epsilon:P_0 \rightarrow A$를 얻는다. 이제 [§Diagram chasing, ⁋정리 6](/ko/math/homological_algebra/diagram_chasing#thm6){: data-lid="4wu4j" data-relation="required" }으로부터 다음의 diagram
