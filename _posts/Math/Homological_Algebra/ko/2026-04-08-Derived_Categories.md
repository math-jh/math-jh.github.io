...
Chain complex와 cochain complex, 그리고 homology와 cohomology는 [§호몰로지, ⁋정의 1](/ko/math/homological_algebra/homology#def1)에서 정의되었다.
...
$$\mathbf{K}(\mathcal{A})$$는 additive category이며, shift functor $$[n]$$이 $$\Ch(\mathcal{A})$$에서 $$\mathbf{K}(\mathcal{A})$$로 descend한다. 또한 [정의 13](#def13)에서 정의할 mapping cone도 homotopy category에서 잘 정의되어, $$\mathbf{K}(\mathcal{A})$$는 [정의 12](#def12)에서 정의할 triangulated category의 구조를 갖는다. Quasi-isomorphism은 $$\mathbf{K}(\mathcal{A})$$에서 일반적으로 isomorphism이 아니다. $$\mathbf{K}(\mathcal{A})$$에서 quasi-isomorphism을 invert한 category가 바로 derived category이다.
...
이 정의가 의미를 갖기 위해서는 $$S$$가 *multiplicative system*이어야 한다. Multiplicative system이 주어진 category에 대해 category of fractions를 구성하는 것은 calculus of fractions의 표준적 기법이며, 자세한 구성은 [Wei]을 참고하라. $$S$$는 다음 조건을 만족한다. 첫째, 항등사상이 $$S$$에 속하고, $$S$$의 원소들의 합성은 다시 $$S$$에 속한다. 둘째, Ore condition이 성립한다: $$s \in S$$와 임의의 사상 $$f$$가 주어졌을 때, commute하는 diagram
...
Canonical functor $$Q \colon \mathbf{K}(\mathcal{A}) \to D(\mathcal{A})$$는 $$S$$의 원소들을 isomorphism으로 보내며, [정의 12](#def12)의 의미에서 triangulated functor이다. Verdier quotient의 기본 성질에 의해 $$D(\mathcal{A})$$도 triangulated category가 된다. 구체적으로, $$\mathbf{K}(\mathcal{A})$$의 distinguished triangle을 $$Q$$로 보낸 삼각형은 $$D(\mathcal{A})$$에서 distinguished triangle이다.
