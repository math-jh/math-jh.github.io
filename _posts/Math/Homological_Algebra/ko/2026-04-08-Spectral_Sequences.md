...
만일 각 $$n$$에 대해 $$\bigcap_p F^p H^n = 0$$이고 $$\bigcup_p F^p H^n = H^n$$라면 각 $$\gr^p H^{p+q}$$의 정보를 모아서 $$H^n$$을 유일하게 재구성할 수 있다. 이러한 경우 spectral sequence가 $$(H^n, F)$$에 *strongly converge*한다고 한다. 반면, $$\bigcap_p F^p H^n \neq 0$$이거나 $$\bigcup_p F^p H^n \neq H^n$$인 경우에는 *weakly converge*한다고 하며, 이 때에는 $$E_\infty^{p,q}$$만으로는 $$H^n$$을 유일하게 결정할 수 없다.
...
$$\im(d_0^{p,q-1}) = (dF^p A^{p+q-1} + F^{p+1} A^{p+q}) / F^{p+1} A^{p+q}$$

이므로

$$E_1^{p,q} = \frac{\{a \in F^p A^{p+q} : da \in F^{p+1} A^{p+q+1}\}}{dF^p A^{p+q-1} + F^{p+1} A^{p+q}}$$

을 얻는다. 문제는 이 위에 differential $$d_1$$을 정의하는 과정이다.

$$d_1:E_1^{p,q}\rightarrow E_1^{p+1,q}$$은 다음과 같이 정의된다. $$E_1^{p,q}$$의 원소 $$[a]$$를 대표하는 cocycle $$a \in F^p A^{p+q}$$를 선택하자. $$d(a) = 0$$이므로 특히 $$d(a) \in F^{p+1} A^{p+q+1}$$이다. 따라서 $$d(a)$$는 $$E_0^{p+1,q}$$에서 $$d_0$$에 대한 cocycle을 정의하며, 이것이 $$d_1([a])$$를 이룬다.
...
를 정의하자. $$E_r^{p,q} = Z_r^{p,q} / (B_r^{p,q} + Z_{r-1}^{p+1,q-1})$$로 정의되며, $$Z_0 = F^p$$, $$B_0 = F^{p+1}$$이므로 $$E_0^{p,q} = \gr^p A^{p+q}$$와 일치한다.
