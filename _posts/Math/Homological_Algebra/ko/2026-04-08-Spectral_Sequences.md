...
도 well-defined surjection을 유도한다. 이제 $$Z_\infty^{p,q} / B_\infty^{p,q} \to F^p H^{p+q}$$의 kernel을 계산하자. Cocycle $$z \in Z_\infty^{p,q}$$의 class가 $$F^p H^{p+q}$$에서 $$0$$이라는 것은, $$z$$가 $$A^\bullet$$에서 boundary라는 뜻이다: $$z = d(y)$$인 $$y \in A^{p+q-1}$$가 존재한다. $$z \in F^p A^{p+q}$$이므로 $$d(y) \in F^p A^{p+q}$$, 즉 $$y \in Z_1^{p-1,q}$$에 의해 대표되고, 따라서 $$z = d(y) \in B_1^{p,q}$$이다. 즉 $$[z] = 0$$ in $$E_1^{p,q}$$. 그러나 $$[z] \neq 0$$ in $$E_r^{p,q}$$일 수 있으며, $$z$$가 각 $$r$$에 대해 $$B_r^{p,q}$$에 속하려면 $$y$$가 $$F^{p-r+1} A^{p+q-1}$$에서 오면 된다. Bounded filtration 조건 하에서는 $$r$$이 충분히 커지면 $$y \in F^{p-r+1} A^{p+q-1}$$이 성립하므로, 충분히 큰 $$r$$에 대해 $$[z] = 0$$ in $$E_r^{p,q}$$이고, 따라서 $$z \in B_\infty^{p,q}$$. 결국 kernel의 원소들은 $$B_\infty^{p,q}$$에 의해 대표되고, $$Z_\infty^{p+1,q-1} \subset B_\infty^{p,q}$$이므로 kernel은 $$Z_\infty^{p+1,q-1}/B_\infty^{p+1,q-1}$$에 의해 지배된다.
...
$$E_\infty^{p,q} = \frac{Z_\infty^{p,q}}{B_\infty^{p,q} + Z_\infty^{p+1,q-1}} \cong \frac{F^p H^{p+q}}{F^{p+1} H^{p+q}} = \gr^p H^{p+q}$$
...
[정의 1](#def1)에 의해 이 filtration으로부터 spectral sequence가 유도된다. $$E_0$$ page는 associated graded이며, $$E_0^{p,q} = \gr^p \Tot(K)^{p+q} = K^{p,q}$$이다. $$d_0$$는 double complex의 vertical differential $$d_v$$에 해당하므로
...
이제 $$E_1$$에서 $$E_2$$로의 page 전이를 살펴보자. $$E_1^{p,0} = \Hom_A(M, I^p)$$이며, $$d_1$$은 double complex의 horizontal differential $$d_h$$에 의해 유도된 사상이다. 즉 $$I^p \to I^{p+1}$$로부터 얻어지는 $$\Hom_A(M, I^p) \to \Hom_A(M, I^{p+1})$$이므로, $$E_1^{\bullet, 0}$$는 $$\Hom_A(M, I^\bullet)$$ 복합체를 이룬다. 따라서 $$q \neq 0$$에서는 $$E_1^{p,q} = 0$$이므로 $$E_2^{p,q} = 0$$이고,

$$E_2^{p,0} = H^p(\Hom_A(M, I^\bullet)) = \Ext^p_A(M, N)$$
...
[정의 5](#def5)에 의해 $$E_\infty^{p,q} = \gr^p H^{p+q}(\Tot(K)^\bullet)$$이다. 그러나 명제 3의 증명에서 확인했듯, 각 $$n$$에 대해 $$p > n$$이면 $$H^n(F^p) \cong H^n(F^{p+1})$$이고 $$p < 0$$이면 $$F^p H^n = H^n$$이므로, 사실상 filtration은 split된다. 따라서
