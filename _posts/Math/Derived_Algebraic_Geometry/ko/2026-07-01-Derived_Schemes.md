...
여기서 limit은 cosimplicial diagram 위의 homotopy limit이며, 이것이 고전적 stack의 하강 조건, 곧 두 겹 겹침에서의 cocycle 조건을 모든 degree의 겹침으로 정합적으로 확장한 것이다. ([\[Stacks\] §그로텐디크 위상, ⁋예시 9](/ko/math/stacks/grothendieck_topology#ex9)와 [\[Stacks\] §그로텐디크 위상, ⁋정의 11](/ko/math/stacks/grothendieck_topology#def11)의 site 위 sheaf 조건을 space값으로 승격한 것이다.) 값을 truncated groupoid로 제한하고 밑을 discrete ring으로 제한하면 고전적 stack의 정의가 정확히 되살아난다. Fpqc 위상이 subcanonical이어서 representable functor가 모두 sheaf였듯 ([\[Stacks\] §그로텐디크 위상, ⁋정리 18](/ko/math/stacks/grothendieck_topology#thm18)), affine derived scheme의 점functor는 자동으로 derived stack이며, 이로써 $\mathrm{dSch}$이 $\mathrm{dSt}$에 완전 충실하게 들어간다.
