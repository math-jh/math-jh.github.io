...
앞선 글을 따라 minimal mistakes 테마를 설치했다면, 저장소에는 minimal-mistakes에서 제공하는 샘플 파일들이 같이 들어있을 것이다. 예를 들어 `/docs` 혹은 `/test` 폴더를 보면 minimal mistakes 데모 사이트에 있는 글들이 그대로 들어있다. 이들은 블로그 설정파일인 `_config.yml`에 exclude 목록에 속해있어 페이지를 생성할 때는 무시되지만, 어쨌든 우리 저장소에서는 용량만 차지하니 지우는 게 낫다. 내가 지운 파일들은 다음과 같다.
...
마찬가지로 로컬에서 작업을 할 때 생성되는 폴더 `_site`와 `.jekyll-cache`를 `.gitignore`에 추가하였으며, 로컬에서 필요한 gem들을 `Gemfile`에 넣어주었다. 

Gemfile에 대한 내용은 나중에 언젠가 컴퓨터를 바꿔서 Jekyll을 새로 설치하거나 할 때 적어보려 한다. 
...
로컬에서 작업할 때, `_config.yml`의 수정사항은 `jekyll serve --livereload`를 사용하고 있더라도 실시간으로 반영되지 않으므로, 이것이 잘 적용되었나 확인하기 위해서는 잠시 서버를 내렸다가 다시 열어야 한다. 
...
맨 첫 글에서 이야기했지만, Jekyll을 통해 블로그를 만드는 것의 가장 큰 장점 중 하나는 markdown이라 생각한다. 마크다운 문법은 위에서 링크해두었던 블로그의 마크다운 문법 [관련 글](https://ansohxxn.github.io/blog/markdown/)에 잘 정리되어 있다. 
