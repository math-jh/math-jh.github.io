...
미리보기 카드에 도식이 들어간 절을 띄우면 그 자리가 하얗게 비었다. 원인은 카드를 만드는 쪽의 [한 줄](https://github.com/math-jh/math-jh.github.io/commit/367af0e5)이었다.

카드는 대상 절의 DOM을 복제해 만든다. 복제본이 원본과 같은 `id`를 들고 있으면 앵커 이동이 어느 쪽으로 갈지 알 수 없으므로, 복제 직후에 `id`를 전부 지우고 있었다.

```js
// 복제본의 중복 id 제거 (원본 앵커와 충돌 방지). 단 svg 안은 건드리지 않는다.
// 도식 svg 는 <defs> 에 글리프를 두고 <use xlink:href="#…"> 로 참조하므로,
// id 를 지우면 참조가 전부 끊겨 카드에 아무것도 안 그려진다.
...
인라인 SVG로 바뀐 도식은 글리프를 `<defs>`에 한 번 정의해 두고 본문에서 `<use xlink:href="#…">`로 여러 번 참조한다. 문서 앵커와 SVG 내부 참조가 같은 `id` 이름공간을 쓰는 것이다. 앵커 충돌을 막으려고 판 것이 SVG의 내부 배선까지 같이 끊었고, 참조가 전부 죽으니 그릴 것이 남지 않아 빈 칸이 됐다.

고친 것은 `svg` 안쪽을 예외로 두는 조건 하나다. SVG 안의 `id`는 문서 앵커로 쓰이지 않으니 남겨도 충돌하지 않는다. 정확히 말하면 복제된 SVG의 `id`는 원본과 중복되지만, 브라우저가 `<use>`를 해석할 때 문서에서 먼저 만나는 것을 쓰므로 카드 쪽은 원본의 글리프를 빌려 그린다. 어느 쪽 정의를 쓰든 같은 그림이라 문제가 되지 않는다.

이 버그가 도식을 인라인 SVG로 바꾸기 전에는 존재할 수 없었다는 점이 눈에 띈다. 도식이 `<img>`였을 때는 카드 안에 지울 `id`가 없었다. 표현 방식을 바꾸면 그 방식이 기대던 전제도 같이 바뀌는데, 그 전제가 다른 기능 안에 한 줄로 들어 있으면 바뀐 줄을 아무도 모른다.
