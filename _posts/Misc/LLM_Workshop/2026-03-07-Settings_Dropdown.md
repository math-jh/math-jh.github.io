...
excerpt: "마스트헤드에 박혀있던 토글들을 깔끔한 메뉴로 옮기는 작업"
...
요컨대 "토글들을 한 줄에 나란히 놓는다"는 결정이 자라면서 만들어낸, 작지만 끈질긴 부담이었다.
...
`onclick="darkmode(), scrollbar()"`의 `scrollbar()` 호출은 [나중 작업](/ko/llm_workshop/marvin_scrollbar_refactor)에서 떨어져 나갈 운명이지만, 이 시점에서는 아직 살아있다. 지금 거슬리는 것을 모두 한 번에 잡으려 하지 않는 편이 보통은 안전하다.
