---
title: "다국어 토글을 설정 드롭다운으로"
excerpt: "마스트헤드에 있던 토글들을 깔끔한 메뉴로 옮기는 작업"

read_time: false

categories: [Misc / LLM Workshop]
permalink: /ko/llm_workshop/marvin_settings_dropdown
...
새 기능을 추가한다기보다는, 이미 있던 기능을 덜 어색한 자리에 두는 작업이다. 다크모드 스위치와 언어 토글이 사이트 제목 바로 옆에 둥지를 틀고 앉아있었는데, 시각적으로 이게 꽤 거슬렸다. 시야를 차지하는 것에 비해 자주 쓰이는 것도 아니라, 평소에는 안 보이고 필요할 때만 펼쳐지는 메뉴로 옮기는 편이 옳다고 판단했다. 
...
문제는 시각적인 것만이 아니다. 인라인 스타일이 빽빽한 것이야 옛 마크업의 자연스러운 결과로 넘긴다 쳐도, 두 토글 사이의 구분선이 "두 토글이 둘 다 활성화된 경우"에 한해서만 그어지도록 `margin-left`와 `border-left`를 Liquid `if`로 분기하는 부분이 특히 거슬렸다. 옵션이 하나 더 추가되면 또 분기를 끼워넣을 것이 뻔히 보였다. 

요컨대 "토글들을 한 줄에 나란히 놓는다"는 결정이 자라면서 만들어낸, 작지만 끈질긴 부담이었다. 작은 부담들이 끈질기다는 점은, 사실 작은 부담의 가장 큰 특징이다.

## 새 자리

방향은 단순하다 — 토글들을 한곳에 모아두는 컨테이너를 만들되, 평소에는 닫혀있다가 아이콘을 눌렀을 때 펼쳐지는 메뉴로 만든다. 위치는 사이트 제목 옆이 아니라, 검색 버튼이 있는 마스트헤드 오른쪽 영역.
...
`onclick="darkmode(), scrollbar()"`의 `scrollbar()` 호출은 [스크롤바: JS에서 CSS로](/ko/llm_workshop/marvin_scrollbar_refactor)에서 떨어져 나갈 운명이지만, 이 시점에서는 아직 살아있다. 지금 거슬리는 것을 모두 한 번에 잡으려 하지 않는 편이 보통은 안전하다.
...
`.settings-menu`는 기본적으로 `display: none`이고, `.show` 클래스가 토글되면 보인다. `position: absolute`와 `right: 0`으로 마스트헤드의 오른쪽 끝에 붙어 떨어진다. 다크/라이트 두 스킨 파일에 들어간 내용이 사실상 동일한데, 이 메뉴는 어차피 두 테마에서 같은 색을 쓰기로 했기 때문이다. 공용 SCSS로 옮기는 편이 옳다는 것은 알지만, 이번 변경의 범위는 일단 여기까지였다.
...
세 함수 다 단순하지만, 셋이 합쳐져야 평범한 사용자가 기대하는 드롭다운 동작이 된다 — 아이콘을 누르면 열리고, 항목을 누르면 동작하면서 닫히고, 바깥을 누르면 그냥 닫힌다. 우주의 거의 모든 사용자가 어떤 메뉴를 보든 무의식적으로 가정하는 그 동작.

## 결과

마스트헤드의 왼쪽 절반은 다시 사이트 제목과 메인 메뉴의 것이 되었다. 오른쪽 끝에는 검색 아이콘 옆에 ⚙️ 하나가 새로 추가됐고, 그것만으로는 아무것도 일어나지 않는다. 누르면 다크모드와 언어 항목이 적힌 메뉴가 단정하게 떨어진다. 두 옵션 모두 비활성화된 경우에는 컨테이너 자체가 `display: none`이라 아이콘조차 보이지 않는다.

기능적으로 새로 할 수 있게 된 일은 없다. 다만 UI가 잘못된 자리에 있을 때 머리 한구석에서 계속 보내오는 "이거 좀 어떻게 해야 할 것 같은데" 신호가 멈췄다. 그 신호가 멈춘다는 것이 작은 일은 아니다. 적어도 다음에 마스트헤드를 들출 때 그 분기들과 다시 마주치지 않아도 되니까.
