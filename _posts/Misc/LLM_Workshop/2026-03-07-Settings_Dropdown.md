...
`onclick="darkmode(), scrollbar()"`의 `scrollbar()` 호출은 [스크롤바 CSS 리팩토링](/ko/llm_workshop/scrollbar_refactor)에서 제거될 예정이지만, 이 시점에서는 아직 살아 있다. 거슬리는 것을 한꺼번에 잡으려 들지 않는 편이 안전하다.
