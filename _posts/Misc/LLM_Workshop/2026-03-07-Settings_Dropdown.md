...
사용자가 요구한 것은 두 가지였다. 토글들을 한곳에 모아두는 컨테이너를 만들되 평소에는 닫아두고 아이콘을 눌렀을 때만 펼치는 것, 그리고 그 자리를 사이트 제목 옆이 아니라 검색 버튼이 있는 마스트헤드 오른쪽에 두는 것.
...
`onclick="darkmode(), scrollbar()"`의 `scrollbar()` 호출은 [스크롤바 CSS 리팩토링](/ko/llm_workshop/scrollbar_refactor){: data-relation="forward" }에서 제거될 예정이지만, 이 시점에서는 아직 살아 있다. 거슬리는 것을 한꺼번에 잡으려 들지 않는 편이 안전하다.
