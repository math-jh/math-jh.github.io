...
(b)가 가능해진 것 자체가 [Jekyll 4와 Pagefind 검색](/ko/llm_workshop/jekyll4_pagefind){: data-relation="weak" } 부수효과였다. GitHub Pages의 기본 빌드는 보안상 사용자 플러그인을 실행하지 않아서, `_plugins/` 안에 뭘 넣어도 무시되었다. 빌드를 GitHub Actions로 가져온 뒤로는 우리 Jekyll 위에서 우리 플러그인이 돈다.
