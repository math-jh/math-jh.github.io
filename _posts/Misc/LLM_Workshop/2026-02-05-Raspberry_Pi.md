...
장단점은 명확하다. 기존 AI에 비해서 훨씬 편리한 대신, 보안상의 우려가 매우 크다. 또, `OpenClaw`의 장점 중 하나는 AI의 기억을 마크다운 형태로 저장해서, 기억이 휘발되지 않도록 할 수 있다는 것이다. 가령 `OpenClaw`에게 블로그에 쓸 Raspberry Pi에 `OpenClaw` 설정하는 과정을 안내하는 글의 초안을 부탁해 보았는데, 한 번의 명령으로 다음 과정들을 모두 수행하는 것을 확인할 수 있었다. 
...
해당 파일을 직접 읽어본 결과, 작년 말부터 RPi5를 사용했다던가, 특정 케이스를 샀다던가 하는 사실과 다른 것들이 들어있기는 하지만 대략적으로 글의 구색은 갖춘 것을 확인할 수 있었다. 나는 `OpenClaw` 에이전트를 `Kimi-K2.5` 모델로 돌리고 있는데, 더 유려한 글을 쓰고 환각률이 적다고 평가받는 `Claude` 등등을 썼다면 사정이 조금 나았을 것이다. 
...
나는 원래 `Homebridge`용 Raspberry Pi Zero 2W를 갖고 있었어서, `OpenClaw` 이야기를 듣고 Raspberry Pi에 설치할 생각을 하는 것은 어찌보면 당연한 일이었다. 그러나 Raspberry Pi Zero는 아무래도 사양이 많이 딸려서 (RAM이 512MB다...), Raspberry Pi 5를 새로 구매했다. 여기서 가격 관련 부분을 짚고 넘어가지 않을 수가 없는데, agentic AI는 그 특성 상 API 소모가 아주 많다. 가령 내가 Raspberry Pi에 설치하고, 이것저것 설정한 게 2월 1일부터 2월 3일까지인데, 이 과정에서 API 사용료만 24달러가 넘게 깨졌다. 
...
개인적으로 재미있는 장난감이라 생각하지만 이곳저곳에서 야단법석 떠는 것만큼 유의미하지는 않아보였다. 다만 다음은 Kimi 모델의 한계와 openclaw의 한계가 섞여있으므로 주의.

가령 나는 블로그 글을 작성할 때 이전에 정의한 개념을 참고할 때가 많다. 이를 위해서는 보통 미리보기 용으로 띄워둔 브라우저에서 해당 내용이 있을법한 페이지로 이동한 후, 내용을 확인하고 이를 올바른 포맷으로 바꿔서 적어줘야 한다. 예를 들어 대수적 위상수학 카테고리의 코호몰로지 글의 예시 1이라면 이를

```
\[대수적 위상수학\] §코호몰로지, ⁋예시 1](/ko/math/algebraic_topology/cohomology#ex1))
```

로 포맷팅 해줘야 한다. 원래는 이를 수동으로 했었는데, 귀찮아서 해당 예시 (혹은 명제, ... 등등) 번호를 누르면 자동으로 위의 형태로 된 문자열을 클립보드로 복사해주는 스크립트를 짰었다. 

문제는 서버를 RPi에 둔 이상, 이것이 더 이상 신뢰되는 소스가 아니라 이 js파일이 작동하지 않았다. 이를 해결하는 올바른 방법은 리버스 프록시 서버를 둬서 HTTPS를 적용하도록 하는 것일텐데, 해결을 지시했더니 tailscale 포트를 18789 포트에서 4000 포트로 돌려버렸다. 우선 tailscale이 https로 된 magic DNS를 지원하므로, 이 방법은 부분적으로는 맞다. 그러나 18789 포트는 OpenClaw Web UI에 할당되었던 포트이므로, web UI로 지시를 내리자마자 연결이 끊기는 황당한 상황이 일어났다. 

다른 부분은 보안에 대한 것으로, github 관련된 ssh 키를 만들 일이 있었는데, 에이전트가 이 키를 블로그 레포지토리에 저장하고, `.gitignore`에 적어두지 않아 cron job이 자동으로 push해버렸다. 이러한 키를 git 추적되는 곳에 넣는 것에 대한 가드레일이나, 하다못해 remote로 push하는 것에 대한 가드레일이 있었다면 막을 수 있었을 일이다.

마지막으로, 만듦새가 상당히 좋지 않다. `Ollama`로 로컬모델을 돌린다면 `thinking` 태그가 잘못 붙어 응답이 전체 삭제된다. (`Ollama`에서는 응답을 뱉어내지만 `OpenClaw`에이전트 단에서 이를 추론과정이라 판단하여 무시한다. 물론 이는 `Ollama`에 올릴 때 신경을 덜 쓴 탓도 있다.) 이를 수정하려면 `isReasoningProvider` 함수에서 `Ollama`를 하나하나 빼줘야 한다. 이를 수정해서 로컬 모델을 띄운다고 해도, 새 세션을 시작할 때 사용자와의 기억 파일들 및 프롬프트을 통째로 AI에 입력하는 바람에 컨텍스트 윈도우가 작은 로컬모델에서는 사용이 거의 불가능하거나 무의미하다. (새 세션 시작할 때 기본으로 16k 정도는 쓴다.) 유의미한 로컬 모델을 돌리려면, 위에서 말했듯 mac mini를 사용하는 것이 정석이긴 하겠지만, mac mini를 성능 맞춰 사더라도 저런 태그 문제로 답이 안 나오는 것은 상당히 답답한 상황일 것이다.

물론 위의 오류들은 내가 사용한 AI 모델의 잘못인 것이 크다. 그러나 만일 누군가가 이를 완전히 AI 모델의 탓으로만 돌린다면, 이 워크플로우에서 `OpenClaw`라는 플랫폼의 역할은 무엇인지 묻지 않을 수 없다. 결국 `OpenClaw`라는 플랫폼에서 에이전틱 AI는 별개의 것으로 둔다면 남는 것은 광범위한 시스템 접근 권한 정도 뿐인데, 어차피 이러한 오류들이 가드레일 없이 튀어나와서 사람이 파일을 저장하거나 수정해야 한다면 이 플랫폼의 역할은 아무것도 없다. 
