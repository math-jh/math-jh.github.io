...
명제 10(결합법칙: $$\lim_i\lim_j F(i,j)\cong\lim_j\lim_i F(i,j)$$)은 "이중 극한의 순서를 바꿔도 같다"는 것인데, 다만 일반적으로 $$\lim$$과 $$\colim$$의 순서는 바꿀 수 없다는 마지막 관찰이 중요하다. $$\colim_i\lim_j\to\lim_j\colim_i$$라는 canonical morphism만 존재한다는 것이 "극한과 여극한은 대偶적이지만 교환 불가능하다"는 메시지를 전한다. 집합론의 Limits에서 inverse limit과 direct limit을 다룰 때도 비슷한 관찰이 있었는데, 범주론적 맥락에서 다시 보니 그 의미가 더 명확해진다.
