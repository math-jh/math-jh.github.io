...
재작성은 모든 커밋 SHA를 바꿨다. 그런데 LLM 작업실과 블로그 개발일지의 글들은 특정 커밋을 `/commit/<hash>` 링크로 자주 가리킨다. force-push가 끝나자 그 해시들은 원격에서 사라졌고, 헤더의 "관련 커밋"과 본문 인용이 404가 됐다. `_posts/Misc` 아래 커밋 링크 69개 중 19개가 그렇게 죽었다. 살아남은 것은 모두 2026-07-20 이후를 가리키고, 그 이전을 가리키던 참조는 전멸했다.

> 우리가 지난번에 그 Git history 좀 수정하면서 한번 크게 Git을 갈아엎었거든? 그래서 그 과정에서 히스토리가 다 새로 쓰여져서 해시값이 다 달라졌단 말이야. [...] 지금 local 쪽에는 혹시 그 커밋이 남아 있는지, 그리고 그게 가능하다면 그 커밋의 실제 링크로 옮길 수 있는지 확인해 줄 수 있을까

로컬에는 근거가 없었다. `--push` 단계가 `git gc --prune=now`까지 돌려 옛 객체는 작업 저장소에 하나도 안 남았고, `prepare`가 떠 둔 `backup.git`은 다른 기계에 있었다. `.git/filter-repo/commit-map`은 남아 있었지만 7/7판, 곧 한 재작성 회차의 옛→새 대응이었고, 그 뒤 `.claude` 제거가 히스토리를 한 번 더 갈아서 지금 히스토리에는 닿지 않았다.

닿는 근거는 GitHub 쪽에 있었다. 앞 절에서 노출로 적은 바로 그 보관 창이다. 죽은 커밋 19개 중 13개는 옛 SHA로 API를 조회하면 아직 응답했다. 재작성이 커밋 메시지와 작성 시각은 건드리지 않았으므로(blob-callback은 블롭 내용만 다시 쓴다), 이 둘을 키로 현재 히스토리에서 한 건으로 좁힐 수 있었다.

```bash
gh api repos/math-jh/math-jh.github.io/commits/<옛-sha> \
  --jq '.commit.author.date + "  " + (.commit.message | split("\n")[0])'
# 이 (시각, 제목) 쌍을 git log --all 에서 유일 매칭
```

GitHub에서도 이미 gc된 6개는 API가 404였다. 그중 5개는 글이 그 커밋을 인용한 맥락, 곧 어떤 파일을 어떻게 건드린 커밋인지로 특정했다. 남은 하나, 가중치 카테고리 글의 "링크 2"(`55e27f8e`)는 후보를 하나로 줄일 근거가 없었다.

> 둘 다 그렇게 하고 링크2를 지우는 선택지로 갈게. [...] development 커밋에 가깝고 후속 조치, history rewrite 여파로 후속 조치라고 해서 커밋하면 될 것 같을까?

그래서 그 링크는 지우고, 나란히 있던 "링크 1"의 표기를 원래대로 "링크"로 되돌렸다. 나머지 18개는 16개 글 21곳에서 새 SHA로 바뀌었고, `_posts/Misc`에 남은 커밋 참조 68개는 전부 origin/main에서 열린다. 앞 절에서는 GitHub이 옛 객체를 붙들고 있는 것을 아직 닫지 못한 노출이라고 적었다. 그 창이 이번엔 끊어진 링크 열여덟 개를 도로 이어 줬으니, 같은 창을 두 번 다르게 적은 꼴이다. ([커밋](https://github.com/math-jh/math-jh.github.io/commit/8073dfb7))
