...
있었다. 처음부터 그렇게 했어야 했다. 치환 자체는 판단이 아니라 사전 조회이고, 판단이 필요한 소수만 사람에게 넘기면 된다. 모델 수백 개를 사전 찾기에 동원하고 있었던 것이다.
...
## 사후: 배치 린트가 보지 못하는 2글자

`deprecated_terms_lint.py`와 그 판정 로직인 `md_lint.py`의 `_load_deprecated`는 `primary: en` 항목의 한국어형을 순한글 3글자 이상으로만 잡는다.

```python
if not re.fullmatch(r"[가-힣 ]{3,}", ko):
    continue  # 혼합 표기($n$번째 호몰로지 등)는 순한글 부분이 커버한다
```

경계를 3글자에 그은 이유는 명확하다. "핵"·"환"·"극" 같은 2글자어는 다른 뜻의 일상어와 그대로 겹치고, 자동 검사기가 이런 것까지 신설로 잡으면 오탐이 검사기 자체를 무의미하게 만든다. 그런데 그 경계 밑으로 실제 영어화 대상이 조용히 빠져나간다. 스윕과 배치 린트 양쪽에서 3글자 미만인 용어는 애초에 셈해지지 않았다는 뜻이다.

`short_terms_scan.py`가 이 틈을 메우되([diff](https://github.com/math-jh/math-jh.github.io/commit/4abddd9843ef6afe4fdc32ca4fc28fd0e2cbde4c)) 배치 린트와 같은 자리에 서지는 않는다. `main`에서 판정을 다시 짜는 대신 `md_lint`의 마스킹 정규식(`_SUB_GLOSS_RE`, `_LINK_ALL_RE`, `_FM_KO_KEY_RE`, `_HEADING_LINE_RE`, `_MATH_SPAN_RE`)을 그대로 import해서 쓴다. 조사 결합형까지 잡아야 하니 최장일치 조사 목록(`으로서`·`이라는`부터 단일 문자 `은`·`는`까지)을 따로 두고, 표제어 뒤에 조사가 붙어도 매치되게 정규식을 짠다.
...
그리고 exit code는 언제나 0이다. 배치 린트처럼 CI를 막는 게이트가 아니라, 사람이 훑어볼 후보 목록을 뽑아내는 스캐너로 설계했다. 2글자 후보는 일상어와 겹치는 비율이 높아서, 자동으로 신설 취급했다가는 3글자 미만을 아예 안 보는 것보다 나쁜 소음을 만든다. `과 → and` 같은 오탐을 걸러내는 일은 여전히 사람 몫으로 남겨 둔 것이다.

## 새 글이 되살린 잡종어

7월 스윕이 정리한 것은 그 시점까지의 코퍼스였다. 그 뒤로도 스킴 이론·가환대수·유도대수기하학 카테고리에 새 글이 계속 붙었고(`Fitting_Ideals`, `Buchsbaum_Eisenbud_Criterion`, `Smooth_and_Etale_Morphisms` 등), 이 글들은 베이스라인 이후에 쓰여서 애초에 스윕 대상이 아니었다. terms.yml에 이미 `primary: en`으로 등록된 표기들이 새 글에서 다시 한국어나 잡종어로 들어갔고, 사용자가 직접 짚었다.

> 닫힌 부분scheme은 명백한 잡종어인데 왜 놔둬? cloased subscheme으로 바꿔야지 (header랑 title 제외하고)

같은 자리에서 국소자유, 유한표시, 소행렬식, "자유 module" 같은 표기도 계속 잘못 쓰고 있다는 지적이 이어졌고, 범위는 이렇게 정해졌다.

> 그리고 그 다른 글에 남은 것들은 헤더랑 타이틀 빼고 나머지만 스윕해줘.

지적된 표기 중 상당수는 terms.yml에 등록된 `ko` 값 그대로가 아니라 반쪽만 영어로 바뀐 제3의 잡종어였다. "닫힌 부분scheme"은 등록된 "닫힌 부분스킴"과도, 목표인 "closed subscheme"과도 다르다. 이런 미등록 변형은 `ko`/`alias` 문자열을 찾는 `mech_sweep.py`의 결정론 엔진이 애초에 찾을 수 없는 대상이라, terms.yml은 건드리지 않고 정규식 규칙을 직접 적은 `scripts/sweeps/term_english_20260818.py`를 새로 짰다.

마스킹 골격은 `mech_sweep.py`(프런트매터·코드펜스·인라인코드·수식·링크)를 그대로 가져왔고, 여기에 사용자가 말한 "헤더랑 타이틀 빼고"를 마스크 두 개로 반영했다.
...
판단이 필요한 나머지는 여전히 사람에게 넘긴다. `REVIEW` 목록은 문장을 다시 써야 하는 "국소자유성"과, terms.yml에 짝이 없는 bare "부분스킴"을 기계 치환에서 빼고 사람이 볼 후보로만 뽑는다. 7월 스윕에서 정한 "치환은 사전조회, 판단은 사람" 원칙 그대로다.

`minor` 하나는 뒤늦게 사용자가 되짚었다.

> 크기가 조금 애매한데, minor를 왜 알파벳 수문자로 썼어?
>
> 아 minor가 그 행렬식 뜻하는거지? 아냐 그럼 맞아.

`minor`는 terms.yml에 `ko: 소행렬식`으로 이미 등록된 표준 영어 용어이지 약어가 아니므로, 다른 용어들과 마찬가지로 조사만 발음 기준("마이너", 모음 끝)으로 다시 골라 붙이면 되는 경우였다. 크론과 린트는 *새로 생기는* 오염을 잡도록 설계했지, 새 글이 옛 표기를 그대로 담고 태어나는 경우까지 잡지는 못한다. 이번에도 사람 눈이 먼저 짚었고 스크립트가 뒤따랐다.
