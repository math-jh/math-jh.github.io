...
[last_modified_at을 git에 맡기기](/ko/llm_workshop/last_modified_git){: data-relation="weak" }에서 수정일을 git에게 넘겼다. 파일의 마지막 commit 시각이 곧 수정일이고, 프론트매터에 남은 `last_modified_at` 줄은 아무 일도 하지 않는 채로 남겨뒀다. 그 글의 마지막 문단은 그 줄들을 절대 일괄 삭제하지 말라는 경고였다. 일괄 삭제하면 삭제 커밋 하나가 모든 파일을 동시에 건드려, 각 파일의 "마지막 커밋"이 죄다 그 커밋이 되고 수정일이 한 시각으로 붕괴하기 때문이다.

그 경고는 옳았다. 다만 한 가지를 놓쳤다. git은 파일이 *언제* 마지막으로 바뀌었는지는 알아도, 그 변경이 *진짜 수정인지*는 모른다.

[다이어그램을 SVG로](/ko/llm_workshop/diagram_svg){: data-relation="weak" } 대수술이 그 빈틈을 드러냈다. 수십 편의 글에서 이미지 참조 한 줄씩만 바꾸는, 내용과 무관한 기계적 작업이었는데도, autopush를 타고 커밋되자 그 글들의 수정일이 모두 작업 시각으로 튀었다. 멀쩡한 옛 글 수십 편이 한날한시에 "수정됨"으로 표시됐고, 검색엔진에도 그렇게 신고됐다. 수정일을 사람에게서 git으로 옮겼더니, 이번엔 git의 노이즈가 그 자리에 들어왔다.

사용자가 내린 답은 이번에도 짧았다. 맡긴 김에, 무엇을 무시할지도 가르친다.
...
위에서 만든 분류에는 뒤늦게 드러난 빈틈이 있었다. autopush는 바뀐 `.md`의 diff를 haiku에게 통째로 건네 기계적인지 내용인지 묻는데, 새로 쓴 글은 파일 전체가 그 diff에 들어간다. 글 한 편을 새로 올리면 diff가 커지고, 그 크기가 한도를 넘으면 분류기는 판정을 포기하고, autopush는 설계대로 아무것도 커밋하지 않고 멈춰 텔레그램으로 알렸다. 날짜를 지키자고 둔 분류기가, 새 글이 나올 때마다 푸시 전체를 세워버렸다.

고친 방향은 질문 자체를 없애는 것이었다. 새로 생긴 글은 정의상 기계적일 수가 없다 — 표기 치환도 이미지 교체도 아니고, 파일 하나가 통째로 새로 생긴 것이다. 그러니 haiku에게 물을 것 없이 곧장 내용 쪽으로 보낸다. 내용 커밋은 "기계적으로 분류되지 않은 전부"라서, 새 글을 분류 목록에서 빼기만 하면 저절로 그리로 간다. 새 글인지 아닌지는 git이 파일별로 알려주는 상태(추가는 `A`)로 가른다.

```python
# git diff --cached --name-status: A=추가, D=삭제, M=수정
add_del     = [f for f in changed if is_post_md(f) and status.get(f) in ("A", "D")]
to_classify = [f for f in changed if is_post_md(f) and f not in add_del]
# 내용 == 기계적이 아닌 전부. add_del을 분류에서 빼면 그대로 내용 커밋으로 간다.
```
{: data-filename="~/.local/bin/blog-autopush.py"}

삭제(`D`)도 같이 뺐다. 지워진 글 역시 기계적 수정이 아니라 내용 변경이고 — 분류기 자신의 규칙도 새 글과 지워진 글을 내용으로 부른다 — 무엇보다 삭제된 파일도 통째로 diff에 실려 한도를 밀어 올린다. 추가든 삭제든, 통째로 생기거나 사라진 파일은 이제 묻지 않는다. 그만큼 haiku가 보는 diff도 작아져, 한도에 걸려 멈추는 일이 드물어졌다.

답이 이미 정해진 질문을 하느라 푸시를 세우고 있었던 셈이다. 물어볼 것만 묻게 하니 멈출 일이 줄었다.
