...
permalink: /ko/llm_workshop/marvin_claude_code_kimi_mimo
...
기본 패턴이 정해진 뒤(C 글자에 구멍을 뚫고 그 안에 다른 글자를 박는 방식) 색상과 글자 모양의 협상이 시작되었다.
...
MiMo 쪽은 `#FF4301` → `#F37333` → `#FF6700` ("클로드 색이랑 너무 가까울까?") → 도로 안정점으로. 사용자의 일관된 미감이 매번 한 발 더 나아가게 했고, 매번 새 PNG가 익스포트 되어 확장에 다시 박혔다. 매번이 *마지막*이 아니라는 것을 점차 받아들이는 일이, 이 절차의 실제 작업이었다고 해도 무방하다.
