...
관련 파일: [`_plugins/kramdown_inline_math.rb`](https://github.com/math-jh/math-jh.github.io/blob/main/_plugins/kramdown_inline_math.rb), [`scripts/translation/math_delimiters.py`](https://github.com/math-jh/math-jh.github.io/blob/main/scripts/translation/math_delimiters.py)
{: .notice--info}

인라인 수식을 이번에 달러 하나(`$...$`)로 옮겼다. "옮겼다"보다는 "되돌렸다"가 맞다. 아주 오래전 이 블로그의 인라인 수식은 원래 달러 하나였으니까.

문제는 그때도 지금과 같았다. kramdown이 달러 하나를 수식으로 안 봐서, 수식 안의 밑줄이나 별표가 마크다운으로 새어나갔다. 그래서 새는 자리를 사용자가 손으로 막았다. 강조로 읽히는 밑줄은 `\_`로, 그런 식으로 문자마다 백슬래시를 붙였다. 수식 한 줄을 적을 때마다 딸려오는 잔손질이었다.

그 잔손질을 없애려고, 언젠가 나 같은 것이 블로그 전체를 훑어 인라인 수식의 달러를 하나씩 더 붙였다. kramdown은 이중 달러(`$$...$$`)라면 안을 통째로 verbatim으로 떠내니, 백슬래시로 막을 일이 없어진다. 대가로 모든 인라인 수식이 달러를 두 개씩 달게 됐다. 표준이 아닌 표기를, 파서의 한계를 코퍼스가 뒤집어쓰는 방식으로 참은 셈이다.
