...
[Sitemap 없이 색인하기](/ko/llm_workshop/gsc_indexing){: data-relation="weak" }에서 GSC URL Inspection의 결과를 적었다 — 안 보이는 KO `/math/` 글 200여 개의 verdict는 모두 `"Google에는 아직 알려지지 않은 URL"`이었고, 그건 description 부재 같은 메타 문제가 아니라 그냥 Google이 페이지 자체를 아직 색인하지 않은 상태였다. description은 1차 원인이 아니었다.
