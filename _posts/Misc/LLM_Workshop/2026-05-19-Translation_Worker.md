...
{% endif %}
```
{: data-filename="_includes/translation-notice.html"}
{% endraw %}
...
부수효과로 cross-reference 처리도 같이 정리됐다. LLM이 자신 없는 링크는 KO 형태로 그대로 두라는 규칙이 INSTRUCTIONS에 들어갔고, 별도의 [내부 링크 표시명 자동 보정](/ko/llm_workshop/link_normalizer){: data-relation="weak" }이 빌드 시점에 표시명을 canonical로 덮어쓰므로, LLM이 정확한 표기를 떠올릴 필요가 없어졌다.
