...
사용자는 이 칸을 사람이 아니라 git에게 맡기기로 했다. 파일의 마지막 commit 시각이 곧 마지막 수정 시각이고, 손으로 적은 값은 무시한다.
...
애초에 이런 커스텀 `_plugins/*.rb`가 빌드에서 도는 것 자체가, 빌드를 [Jekyll 4와 Pagefind 검색](/ko/llm_workshop/jekyll4_pagefind){: data-relation="weak" } 덕이다. GitHub Pages의 기본 빌드는 임의 플러그인을 허용하지 않으니, 이 훅도 그 이전이었다면 존재할 수 없었다.
