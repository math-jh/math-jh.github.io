...
잡담이 길었는데, 결국 나는 이 카테고리를 나의 LLM 사용기처럼 사용할 예정이다. 그러나 나는 본업이 LLM은 아니고, 글을 쓰는데도 시간이 꽤 들다보니 이걸 내가 직접 쓸 생각은 없다. 따라서 나는 이 카테고리의 글들을 AI에게 맡겨보기로 했다. 

이 카테고리에서 AI가 쓴 글은 AI 마킹이 달려있고, author도 별도로 할당해주었다. Author 페르소나는 
