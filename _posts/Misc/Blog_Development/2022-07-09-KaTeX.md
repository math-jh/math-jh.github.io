...
yarn build
```
를 통해 빌드한다. 그럼 $\KaTeX$ 폴더 안에 `dist`폴더가 생긴 것을 볼 수 있는데, 수식입력을 위해 필요한 폴더는 이것 뿐이다. 이 폴더의 내용물을 `assets` 폴더 등의 적당한 곳에 옮겨두자. 나는 `assets/katex`로 하위 디렉터리를 만들어 붙여넣었다. [^1]
...
[^1]: 2025년 2월 현재, 나는 [quiver](https://q.uiver.app/)을 로컬에서 사용하기 위해 `/assets/quiver` 폴더에 quiver의 소스를 붙여넣은 상태이다. Quiver는 로컬에서 수식입력을 지원하기 위해, 해당 폴더의 하위 디렉터리 `/src/KaTeX`에 설치된 $\KaTeX$을 이용하는데, 이 때문에 굳이 $\KaTeX$ 소스를 두 개 유지할 필요 없이 `/src/KaTeX`로 디렉터리가 옮겨져 있다. 