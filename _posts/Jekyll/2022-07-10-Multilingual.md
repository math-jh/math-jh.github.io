...
나는 코딩을 배운 적도 없고, 굳이 배울 생각도 없어서 내가 짠 코드들은 좀 지저분하지만, 그래도 혹시 비슷한 삽질을 하시는 분이 있을까 싶어 기록을 남겨둔다.
...
위의 코드에서 보듯이 minimal-mistakes는 각 용어들의 번역이 담긴 `data/ui-text.yml` 파일에서 `site.locale`에 해당하는 언어를 골라서 페이지를 빌드한다. 따라서 `site.locale`자리에 현재 페이지의 언어를 감지하여 넣어주면 한/영 버전의 사이트 모두에서 자신의 언어에 해당하는 메뉴를 볼 수 있게 된다. 
